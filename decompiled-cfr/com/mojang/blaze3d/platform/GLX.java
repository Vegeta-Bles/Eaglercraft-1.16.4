...
public class GLX {
    ...
    public static int _getRefreshRate(Window window) {
        ...
        GLFWVidMode _snowman2 = l == 0L ? null : GLFW.glfwGetVideoMode((long)l);
        return _snowman2 == null ? 0 : _snowman2.refreshRate();
    }
    ...
    public static LongSupplier _initGlfw() {
        ...
        RenderSystem.assertThread(RenderSystem::isInInitPhase);
        Window.acceptError((n, string) -> {
            throw new IllegalStateException(String.format("GLFW error before init: [0x%X]%s", n, string));
        });
        ArrayList arrayList = Lists.newArrayList();
        GLFWErrorCallback _snowman2 = GLFW.glfwSetErrorCallback((n, l) -> arrayList.add(String.format("GLFW error during init: [0x%X]%s", n, l)));
        if (GLFW.glfwInit()) {
        ...
        RenderSystem.setErrorCallback((GLFWErrorCallbackI)_snowman2);
        return longSupplier;
    }
    ...
    public static void _renderCrosshair(int size, boolean drawX, boolean drawY, boolean drawZ) {
        ...
        if (drawX) {
            _snowman2.vertex(0.0, 0.0, 0.0).color(0, 0, 0, 255).next();
            _snowman2.vertex(size, 0.0, 0.0).color(0, 0, 0, 255).next();
        }
        if (drawY) {
            _snowman2.vertex(0.0, 0.0, 0.0).color(0, 0, 0, 255).next();
            _snowman2.vertex(0.0, size, 0.0).color(0, 0, 0, 255).next();
        }
        if (drawZ) {
            _snowman2.vertex(0.0, 0.0, 0.0).color(0, 0, 0, 255).next();
            _snowman2.vertex(0.0, 0.0, size).color(0, 0, 0, 255).next();
        }
        tessellator.draw();
        GL11.glLineWidth((float)2.0f);
        _snowman2.begin(1, VertexFormats.POSITION_COLOR);
        if (drawX) {
            _snowman2.vertex(0.0, 0.0, 0.0).color(255, 0, 0, 255).next();
            _snowman2.vertex(size, 0.0, 0.0).color(255, 0, 0, 255).next();
        }
        if (drawY) {
            _snowman2.vertex(0.0, 0.0, 0.0).color(0, 255, 0, 255).next();
            _snowman2.vertex(0.0, size, 0.0).color(0, 255, 0, 255).next();
        }
        if (drawZ) {
            _snowman2.vertex(0.0, 0.0, 0.0).color(127, 127, 255, 255).next();
            _snowman2.vertex(0.0, 0.0, size).color(127, 127, 255, 255).next();
        }
        tessellator.draw();
