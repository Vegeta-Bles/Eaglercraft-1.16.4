...
public class Bootstrap {
    ...
    private static void collectMissingGameRuleTranslations(Set<String> translations) {
        Language language = Language.getInstance();
        GameRules.accept(new GameRules.Visitor(language, translations){
            ...
            public <T extends GameRules.Rule<T>> void visit(GameRules.Key<T> key, GameRules.Type<T> type) {
                if (!this.field_24373.hasTranslation(key.getTranslationKey())) {
                    this.field_24374.add(key.getName());
                }
            }
        });
    }
    ...
    public static Set<String> getMissingTranslations() {
        ...
        Bootstrap.collectMissingTranslations(Registry.ATTRIBUTE, EntityAttribute::getTranslationKey, treeSet);
        Bootstrap.collectMissingTranslations(Registry.ENTITY_TYPE, EntityType::getTranslationKey, treeSet);
        Bootstrap.collectMissingTranslations(Registry.STATUS_EFFECT, StatusEffect::getTranslationKey, treeSet);
        Bootstrap.collectMissingTranslations(Registry.ITEM, Item::getTranslationKey, treeSet);
        Bootstrap.collectMissingTranslations(Registry.ENCHANTMENT, Enchantment::getTranslationKey, treeSet);
        Bootstrap.collectMissingTranslations(Registry.BLOCK, Block::getTranslationKey, treeSet);
        Bootstrap.collectMissingTranslations(Registry.CUSTOM_STAT, identifier -> "stat." + identifier.toString().replace(':', '.'), treeSet);
        Bootstrap.collectMissingGameRuleTranslations(treeSet);
        return treeSet;
    }
