...
extends AbstractCriterion<Conditions> {
    ...
    public Conditions conditionsFromJson(JsonObject jsonObject, EntityPredicate.Extended extended, AdvancementEntityPredicateDeserializer advancementEntityPredicateDeserializer) {
        ...
        StatePredicate _snowman2 = StatePredicate.fromJson(jsonObject.get("state"));
        if (block != null) {
            _snowman2.check(block.getStateManager(), name -> {
                throw new JsonSyntaxException("Block " + block + " has no property " + name + ":");
            });
        }
        LocationPredicate _snowman3 = LocationPredicate.fromJson(jsonObject.get("location"));
        ItemPredicate _snowman4 = ItemPredicate.fromJson(jsonObject.get("item"));
        return new Conditions(extended, block, _snowman2, _snowman3, _snowman4);
    }
