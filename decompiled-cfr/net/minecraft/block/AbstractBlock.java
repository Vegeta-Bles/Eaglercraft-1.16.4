...
public abstract class AbstractBlock {
    ...
    public List<ItemStack> getDroppedStacks(BlockState state, LootContext.Builder builder) {
        ...
        LootContext _snowman2 = builder.parameter(LootContextParameters.BLOCK_STATE, state).build(LootContextTypes.BLOCK);
        ServerWorld _snowman3 = _snowman2.getWorld();
        LootTable _snowman4 = _snowman3.getServer().getLootManager().getTable(identifier);
        return _snowman4.generateLoot(_snowman2);
    }
    ...
    public float calcBlockBreakingDelta(BlockState state, PlayerEntity player, BlockView world, BlockPos pos) {
        ...
        int _snowman2 = player.isUsingEffectiveTool(state) ? 30 : 100;
        return player.getBlockBreakingSpeed(state) / f / (float)_snowman2;
    }
    ...
    extends State<Block, BlockState> {
        ...
        public Vec3d getModelOffset(BlockView world, BlockPos pos) {
            ...
            long _snowman2 = MathHelper.hashCode(pos.getX(), 0, pos.getZ());
            return new Vec3d(((double)((float)(_snowman2 & 0xFL) / 15.0f) - 0.5) * 0.5, offsetType == OffsetType.XYZ ? ((double)((float)(_snowman2 >> 4 & 0xFL) / 15.0f) - 1.0) * 0.2 : 0.0, ((double)((float)(_snowman2 >> 8 & 0xFL) / 15.0f) - 0.5) * 0.5);
        }
        ...
        public final void updateNeighbors(WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
            ...
            for (Direction direction : FACINGS) {
                ...
                _snowman = blockState.getStateForNeighborUpdate(direction.getOpposite(), this.asBlockState(), world, mutable, pos);
                Block.replace(blockState, _snowman, world, mutable, flags, maxUpdateDepth);
            }
        }
    ...
    public static class Settings {
        ...
        public static Settings copy(AbstractBlock block) {
            Settings settings = new Settings(block.material, block.settings.materialColorFactory);
