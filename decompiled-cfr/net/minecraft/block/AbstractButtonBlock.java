...
extends WallMountedBlock {
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        boolean _snowman2 = state.get(POWERED);
        switch ((WallMountLocation)state.get(FACE)) {
            case FLOOR: {
                if (direction.getAxis() == Direction.Axis.X) {
                    return _snowman2 ? FLOOR_X_PRESSED_SHAPE : FLOOR_X_SHAPE;
                }
                return _snowman2 ? FLOOR_Z_PRESSED_SHAPE : FLOOR_Z_SHAPE;
            }
            case WALL: {
                switch (direction) {
                    case EAST: {
                        return _snowman2 ? EAST_PRESSED_SHAPE : EAST_SHAPE;
                    }
                    case WEST: {
                        return _snowman2 ? WEST_PRESSED_SHAPE : WEST_SHAPE;
                    }
                    case SOUTH: {
                        return _snowman2 ? SOUTH_PRESSED_SHAPE : SOUTH_SHAPE;
                    }
                }
                return _snowman2 ? NORTH_PRESSED_SHAPE : NORTH_SHAPE;
            }
        }
        if (direction.getAxis() == Direction.Axis.X) {
            return _snowman2 ? CEILING_X_PRESSED_SHAPE : CEILING_X_SHAPE;
        }
        return _snowman2 ? CEILING_Z_PRESSED_SHAPE : CEILING_Z_SHAPE;
    }
    ...
    private void tryPowerWithProjectiles(BlockState state, World world, BlockPos pos) {
        ...
        boolean _snowman2 = !list.isEmpty();
        if (_snowman2 != (_snowman = state.get(POWERED).booleanValue())) {
            world.setBlockState(pos, (BlockState)state.with(POWERED, _snowman2), 3);
            this.updateNeighbors(state, world, pos);
            this.playClickSound(null, world, pos, _snowman2);
        }
        if (_snowman2) {
