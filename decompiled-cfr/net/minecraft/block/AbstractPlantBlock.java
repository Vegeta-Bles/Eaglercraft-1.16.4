...
implements Fertilizable {
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        if (direction == this.growthDirection && (_snowman = newState.getBlock()) != this && _snowman != abstractPlantStemBlock) {
    ...
    private Optional<BlockPos> method_25960(BlockView blockView, BlockPos blockPos, BlockState blockState) {
        ...
        while ((_snowman = blockView.getBlockState(blockPos2 = blockPos2.offset(this.growthDirection)).getBlock()) == blockState.getBlock()) {
        }
        if (_snowman == this.getStem()) {
