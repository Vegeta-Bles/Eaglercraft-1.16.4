...
implements Fertilizable {
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        int _snowman2 = Math.min(state.get(AGE) + 1, 25);
        int _snowman3 = this.method_26376(random);
        for (int i = 0; i < _snowman3 && this.chooseStemState(world.getBlockState(blockPos)); ++i) {
            world.setBlockState(blockPos, (BlockState)state.with(AGE, _snowman2));
            blockPos = blockPos.offset(this.growthDirection);
            _snowman2 = Math.min(_snowman2 + 1, 25);
        }
    }
