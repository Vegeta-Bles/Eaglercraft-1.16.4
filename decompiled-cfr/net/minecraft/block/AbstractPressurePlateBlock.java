...
extends Block {
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        int n = this.getRedstoneOutput(state);
        if (n > 0) {
            this.updatePlateState(world, pos, state, n);
        }
    }
    ...
    protected void updatePlateState(World world, BlockPos pos, BlockState state, int rsOut) {
        boolean bl;
        int n = this.getRedstoneOutput(world, pos);
        boolean _snowman2 = rsOut > 0;
        boolean bl2 = bl = n > 0;
        if (rsOut != n) {
            BlockState blockState = this.setRedstoneOutput(state, n);
