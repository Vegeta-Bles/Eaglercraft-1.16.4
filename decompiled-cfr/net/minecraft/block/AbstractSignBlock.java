...
implements Waterloggable {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        boolean bl = _snowman = itemStack.getItem() instanceof DyeItem && player.abilities.allowModifyWorld;
        if (world.isClient) {
            return _snowman ? ActionResult.SUCCESS : ActionResult.CONSUME;
        }
        BlockEntity _snowman2 = world.getBlockEntity(pos);
        if (_snowman2 instanceof SignBlockEntity) {
            SignBlockEntity signBlockEntity = (SignBlockEntity)_snowman2;
            if (_snowman && (_snowman = signBlockEntity.setTextColor(((DyeItem)itemStack.getItem()).getColor())) && !player.isCreative()) {
