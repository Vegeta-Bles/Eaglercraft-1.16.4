...
extends BlockWithEntity {
    ...
    private void angerNearbyBees(World world, BlockPos pos) {
        ...
        if (!list.isEmpty()) {
            ...
            int _snowman2 = list2.size();
            for (BeeEntity beeEntity : list) {
                if (beeEntity.getTarget() != null) continue;
                beeEntity.setTarget(list2.get(world.random.nextInt(_snowman2)));
            }
        }
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player2, Hand hand, BlockHitResult hit) {
        ...
        int _snowman2 = state.get(HONEY_LEVEL);
    ...
    private void spawnHoneyParticles(World world, BlockPos pos, BlockState state) {
        ...
        if (_snowman2 >= 1.0 && !state.isIn(BlockTags.IMPERMEABLE)) {
            double d = voxelShape.getMin(Direction.Axis.Y);
            if (d > 0.0) {
                this.addHoneyParticle(world, pos, voxelShape, (double)pos.getY() + d - 0.05);
            } else {
                BlockPos blockPos = pos.down();
                BlockState _snowman3 = world.getBlockState(blockPos);
                VoxelShape _snowman4 = _snowman3.getCollisionShape(world, blockPos);
                double _snowman5 = _snowman4.getMax(Direction.Axis.Y);
                if ((_snowman5 < 1.0 || !_snowman3.isFullCube(world, blockPos)) && _snowman3.getFluidState().isEmpty()) {
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (!world.isClient && player.isCreative() && world.getGameRules().getBoolean(GameRules.DO_TILE_DROPS) && (blockEntity = world.getBlockEntity(pos)) instanceof BeehiveBlockEntity) {
            ...
            BeehiveBlockEntity beehiveBlockEntity = (BeehiveBlockEntity)blockEntity;
            ItemStack _snowman2 = new ItemStack(this);
            int _snowman3 = state.get(HONEY_LEVEL);
            boolean bl = _snowman = !beehiveBlockEntity.hasNoBees();
            if (!_snowman && _snowman3 == 0) {
                return;
            }
            if (_snowman) {
            ...
            compoundTag = new CompoundTag();
            compoundTag.putInt("honey_level", _snowman3);
            _snowman2.putSubTag("BlockStateTag", compoundTag);
            ItemEntity _snowman4 = new ItemEntity(world, pos.getX(), pos.getY(), pos.getZ(), _snowman2);
            _snowman4.setToDefaultPickupDelay();
    ...
    public List<ItemStack> getDroppedStacks(BlockState state, LootContext.Builder builder) {
        ...
        if ((entity instanceof TntEntity || entity instanceof CreeperEntity || entity instanceof WitherSkullEntity || entity instanceof WitherEntity || entity instanceof TntMinecartEntity) && (_snowman = builder.getNullable(LootContextParameters.BLOCK_ENTITY)) instanceof BeehiveBlockEntity) {
            BeehiveBlockEntity beehiveBlockEntity = (BeehiveBlockEntity)_snowman;
