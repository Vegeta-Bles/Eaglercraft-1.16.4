...
extends BlockWithEntity {
    ...
    public void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        ...
        PlayerEntity _snowman2 = entity instanceof PlayerEntity ? (PlayerEntity)entity : null;
        this.ring(world, state, hit, _snowman2, true);
    }
    ...
    public boolean ring(World world, BlockState state, BlockHitResult blockHitResult, @Nullable PlayerEntity playerEntity, boolean bl) {
        ...
        boolean bl2 = _snowman = !bl || this.isPointOnBell(state, direction, blockHitResult.getPos().y - (double)_snowman2.getY());
        if (_snowman) {
            boolean bl3 = this.ring(world, _snowman2, direction);
    ...
    private boolean isPointOnBell(BlockState state, Direction side, double y) {
        ...
        Attachment _snowman2 = state.get(ATTACHMENT);
        switch (_snowman2) {
            case FLOOR: {
    ...
    public boolean ring(World world, BlockPos pos, @Nullable Direction direction2) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (!world.isClient && blockEntity instanceof BellBlockEntity) {
            if (direction2 == null) {
                Direction direction2 = world.getBlockState(pos).get(FACING);
            }
            ((BellBlockEntity)blockEntity).activate(direction2);
    ...
    private VoxelShape getShape(BlockState state) {
        Direction direction = state.get(FACING);
        Attachment _snowman2 = state.get(ATTACHMENT);
        if (_snowman2 == Attachment.FLOOR) {
            if (direction == Direction.NORTH || direction == Direction.SOUTH) {
                return NORTH_SOUTH_SHAPE;
            }
            return EAST_WEST_SHAPE;
        }
        if (_snowman2 == Attachment.CEILING) {
            return HANGING_SHAPE;
        }
        if (_snowman2 == Attachment.DOUBLE_WALL) {
            if (direction == Direction.NORTH || direction == Direction.SOUTH) {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        World _snowman3 = ctx.getWorld();
        Direction.Axis _snowman4 = direction.getAxis();
        if (_snowman4 == Direction.Axis.Y) {
            ...
            if (blockState.canPlaceAt(ctx.getWorld(), _snowman2)) {
                return blockState;
            }
        } else {
            boolean bl = _snowman4 == Direction.Axis.X && _snowman3.getBlockState(_snowman2.west()).isSideSolidFullSquare(_snowman3, _snowman2.west(), Direction.EAST) && _snowman3.getBlockState(_snowman2.east()).isSideSolidFullSquare(_snowman3, _snowman2.east(), Direction.WEST) || _snowman4 == Direction.Axis.Z && _snowman3.getBlockState(_snowman2.north()).isSideSolidFullSquare(_snowman3, _snowman2.north(), Direction.SOUTH) && _snowman3.getBlockState(_snowman2.south()).isSideSolidFullSquare(_snowman3, _snowman2.south(), Direction.NORTH);
            BlockState _snowman5 = (BlockState)((BlockState)this.getDefaultState().with(FACING, direction.getOpposite())).with(ATTACHMENT, bl ? Attachment.DOUBLE_WALL : Attachment.SINGLE_WALL);
            if (_snowman5.canPlaceAt(ctx.getWorld(), ctx.getBlockPos())) {
                return _snowman5;
            }
            _snowman = _snowman3.getBlockState(_snowman2.down()).isSideSolidFullSquare(_snowman3, _snowman2.down(), Direction.UP);
            if ((_snowman5 = (BlockState)_snowman5.with(ATTACHMENT, _snowman ? Attachment.FLOOR : Attachment.CEILING)).canPlaceAt(ctx.getWorld(), ctx.getBlockPos())) {
                return _snowman5;
            }
        }
        return null;
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        Direction _snowman2 = BellBlock.getPlacementSide(state).getOpposite();
        if (_snowman2 == direction && !state.canPlaceAt(world, pos) && attachment != Attachment.DOUBLE_WALL) {
        ...
        if (direction.getAxis() == state.get(FACING).getAxis()) {
            ...
            if (attachment == Attachment.SINGLE_WALL && _snowman2.getOpposite() == direction && newState.isSideSolidFullSquare(world, posFrom, state.get(FACING))) {
