...
implements Waterloggable {
    ...
    private static final DoubleBlockProperties.PropertyRetriever<ChestBlockEntity, Optional<NamedScreenHandlerFactory>> NAME_RETRIEVER = new DoubleBlockProperties.PropertyRetriever<ChestBlockEntity, Optional<NamedScreenHandlerFactory>>(){
        ...
        public Optional<NamedScreenHandlerFactory> getFromBoth(ChestBlockEntity chestBlockEntity, ChestBlockEntity chestBlockEntity2) {
            DoubleInventory doubleInventory = new DoubleInventory(chestBlockEntity, chestBlockEntity2);
            return Optional.of(new NamedScreenHandlerFactory(this, chestBlockEntity, chestBlockEntity2, doubleInventory){
                ...
                final /* synthetic */ 2 field_17361;
                {
                    this.field_17361 = _snowman;
                ...
                public ScreenHandler createMenu(int n, PlayerInventory playerInventory, PlayerEntity playerEntity) {
                    if (this.field_17358.checkUnlocked(playerEntity) && this.field_17359.checkUnlocked(playerEntity)) {
                        this.field_17358.checkLootInteraction(playerInventory.player);
                        this.field_17359.checkLootInteraction(playerInventory.player);
                        return GenericContainerScreenHandler.createGeneric9x6(n, playerInventory, this.field_17360);
                    }
                    return null;
                }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        Direction _snowman2 = ctx.getPlayerFacing().getOpposite();
        FluidState _snowman3 = ctx.getWorld().getFluidState(ctx.getBlockPos());
        boolean _snowman4 = ctx.shouldCancelInteraction();
        Direction _snowman5 = ctx.getSide();
        if (_snowman5.getAxis().isHorizontal() && _snowman4 && (_snowman = this.getNeighborChestDirection(ctx, _snowman5.getOpposite())) != null && _snowman.getAxis() != _snowman5.getAxis()) {
            _snowman2 = _snowman;
            ChestType chestType2 = chestType = _snowman2.rotateYCounterclockwise() == _snowman5.getOpposite() ? ChestType.RIGHT : ChestType.LEFT;
        }
        if (chestType == ChestType.SINGLE && !_snowman4) {
            if (_snowman2 == this.getNeighborChestDirection(ctx, _snowman2.rotateYClockwise())) {
                chestType = ChestType.LEFT;
            } else if (_snowman2 == this.getNeighborChestDirection(ctx, _snowman2.rotateYCounterclockwise())) {
                chestType = ChestType.RIGHT;
            }
        }
        return (BlockState)((BlockState)((BlockState)this.getDefaultState().with(FACING, _snowman2)).with(CHEST_TYPE, chestType)).with(WATERLOGGED, _snowman3.getFluid() == Fluids.WATER);
    }
