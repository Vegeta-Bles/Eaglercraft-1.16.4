...
implements InventoryProvider {
    ...
    private static final VoxelShape[] LEVEL_TO_COLLISION_SHAPE = Util.make(new VoxelShape[9], voxelShapeArray2 -> {
        VoxelShape[] voxelShapeArray2;
        for (int i = 0; i < 8; ++i) {
            voxelShapeArray2[i] = VoxelShapes.combineAndSimplify(RAY_TRACE_SHAPE, Block.createCuboidShape(2.0, Math.max(2, 1 + i * 2), 2.0, 14.0, 16.0, 14.0), BooleanBiFunction.ONLY_FIRST);
        }
        voxelShapeArray2[8] = voxelShapeArray2[7];
    });
    ...
    public static void playEffects(World world, BlockPos pos, boolean fill) {
        ...
        double _snowman2 = blockState.getOutlineShape(world, pos).getEndingCoord(Direction.Axis.Y, 0.5, 0.5) + 0.03125;
        double _snowman3 = 0.13125f;
        double _snowman4 = 0.7375f;
        Random _snowman5 = world.getRandom();
        for (int i = 0; i < 10; ++i) {
            double d = _snowman5.nextGaussian() * 0.02;
            _snowman = _snowman5.nextGaussian() * 0.02;
            _snowman = _snowman5.nextGaussian() * 0.02;
            world.addParticle(ParticleTypes.COMPOSTER, (double)pos.getX() + (double)0.13125f + (double)0.7375f * (double)_snowman5.nextFloat(), (double)pos.getY() + _snowman2 + (double)_snowman5.nextFloat() * (1.0 - _snowman2), (double)pos.getZ() + (double)0.13125f + (double)0.7375f * (double)_snowman5.nextFloat(), d, _snowman, _snowman);
        }
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (n < 8 && ITEM_TO_LEVEL_INCREASE_CHANCE.containsKey((Object)_snowman2.getItem())) {
            if (n < 7 && !world.isClient) {
                BlockState blockState = ComposterBlock.addToComposter(state, world, pos, _snowman2);
    ...
    public static BlockState compost(BlockState state, ServerWorld world, ItemStack stack, BlockPos pos) {
        int n = state.get(LEVEL);
        if (n < 7 && ITEM_TO_LEVEL_INCREASE_CHANCE.containsKey((Object)stack.getItem())) {
    ...
    public static BlockState emptyFullComposter(BlockState state, World world, BlockPos pos) {
        if (!world.isClient) {
            float f = 0.7f;
            double _snowman2 = (double)(world.random.nextFloat() * 0.7f) + (double)0.15f;
            double _snowman3 = (double)(world.random.nextFloat() * 0.7f) + 0.06000000238418579 + 0.6;
            double _snowman4 = (double)(world.random.nextFloat() * 0.7f) + (double)0.15f;
            ItemEntity _snowman5 = new ItemEntity(world, (double)pos.getX() + _snowman2, (double)pos.getY() + _snowman3, (double)pos.getZ() + _snowman4, new ItemStack(Items.BONE_MEAL));
    ...
    private static BlockState addToComposter(BlockState state, WorldAccess world, BlockPos pos, ItemStack item) {
        int n = state.get(LEVEL);
        float _snowman2 = ITEM_TO_LEVEL_INCREASE_CHANCE.getFloat((Object)item.getItem());
        if (n == 0 && _snowman2 > 0.0f || world.getRandom().nextDouble() < (double)_snowman2) {
            _snowman = n + 1;
            BlockState blockState = (BlockState)state.with(LEVEL, _snowman);
            world.setBlockState(pos, blockState, 3);
            if (_snowman == 7) {
                world.getBlockTickScheduler().schedule(pos, state.getBlock(), 20);
            }
