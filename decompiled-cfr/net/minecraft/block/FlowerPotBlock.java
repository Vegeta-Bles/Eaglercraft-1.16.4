...
extends Block {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        Block _snowman3 = _snowman2 instanceof BlockItem ? CONTENT_TO_POTTED.getOrDefault(((BlockItem)_snowman2).getBlock(), Blocks.AIR) : Blocks.AIR;
        boolean _snowman4 = _snowman3 == Blocks.AIR;
        boolean bl = _snowman = this.content == Blocks.AIR;
        if (_snowman4 != _snowman) {
            if (_snowman) {
                world.setBlockState(pos, _snowman3.getDefaultState(), 3);
                player.incrementStat(Stats.POT_FLOWER);
                if (!player.abilities.creativeMode) {
                    itemStack.decrement(1);
                }
            } else {
                _snowman = new ItemStack(this.content);
                if (itemStack.isEmpty()) {
                    player.setStackInHand(hand, _snowman);
                } else if (!player.giveItemStack(_snowman)) {
