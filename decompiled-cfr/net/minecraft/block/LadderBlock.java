...
implements Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        FluidState _snowman4 = ctx.getWorld().getFluidState(ctx.getBlockPos());
        for (Direction direction : ctx.getPlacementDirections()) {
            if (!direction.getAxis().isHorizontal() || !(blockState = (BlockState)blockState.with(FACING, direction.getOpposite())).canPlaceAt(_snowman2, _snowman3)) continue;
            return (BlockState)blockState.with(WATERLOGGED, _snowman4.getFluid() == Fluids.WATER);
        }
        return null;
    }
