...
implements Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        for (Direction direction : ctx.getPlacementDirections()) {
            if (direction.getAxis() != Direction.Axis.Y || !(_snowman = (BlockState)this.getDefaultState().with(HANGING, direction == Direction.UP)).canPlaceAt(ctx.getWorld(), ctx.getBlockPos())) continue;
            return (BlockState)_snowman.with(field_26441, fluidState.getFluid() == Fluids.WATER);
        }
        return null;
    }
