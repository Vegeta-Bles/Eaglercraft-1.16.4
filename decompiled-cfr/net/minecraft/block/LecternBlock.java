...
extends BlockWithEntity {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        ItemStack _snowman2 = ctx.getStack();
        CompoundTag _snowman3 = _snowman2.getTag();
        PlayerEntity _snowman4 = ctx.getPlayer();
        boolean _snowman5 = false;
        if (!world.isClient && _snowman4 != null && _snowman3 != null && _snowman4.isCreativeLevelTwoOp() && _snowman3.contains("BlockEntityTag") && (_snowman = _snowman3.getCompound("BlockEntityTag")).contains("Book")) {
            _snowman5 = true;
        }
        return (BlockState)((BlockState)this.getDefaultState().with(FACING, ctx.getPlayerFacing().getOpposite())).with(HAS_BOOK, _snowman5);
    }
    ...
    private void dropBook(BlockState state, World world, BlockPos pos) {
        ...
        if (blockEntity instanceof LecternBlockEntity) {
            ...
            ItemStack _snowman3 = lecternBlockEntity.getBook().copy();
            float _snowman4 = 0.25f * (float)_snowman2.getOffsetX();
            float _snowman5 = 0.25f * (float)_snowman2.getOffsetZ();
            ItemEntity _snowman6 = new ItemEntity(world, (double)pos.getX() + 0.5 + (double)_snowman4, pos.getY() + 1, (double)pos.getZ() + 0.5 + (double)_snowman5, _snowman3);
            _snowman6.setToDefaultPickupDelay();
            world.spawnEntity(_snowman6);
