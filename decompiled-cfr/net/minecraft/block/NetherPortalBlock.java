...
extends Block {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (world.getDimension().isNatural() && world.getGameRules().getBoolean(GameRules.DO_MOB_SPAWNING) && random.nextInt(2000) < world.getDifficulty().getId()) {
            ...
            if (world.getBlockState(pos).allowsSpawning(world, pos, EntityType.ZOMBIFIED_PIGLIN) && (zombifiedPiglinEntity = EntityType.ZOMBIFIED_PIGLIN.spawn(world, null, null, null, pos.up(), SpawnReason.STRUCTURE, false, false)) != null) {
                zombifiedPiglinEntity.resetNetherPortalCooldown();
            }
        }
    }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        _snowman = state.get(AXIS);
        boolean bl = _snowman = _snowman != axis && axis.isHorizontal();
        if (_snowman || newState.isOf(this) || new AreaHelper(world, pos, _snowman).wasAlreadyValid()) {
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            _snowman = ((double)random.nextFloat() - 0.5) * 0.5;
            _snowman = ((double)random.nextFloat() - 0.5) * 0.5;
            _snowman = ((double)random.nextFloat() - 0.5) * 0.5;
            int _snowman2 = random.nextInt(2) * 2 - 1;
            if (world.getBlockState(pos.west()).isOf(this) || world.getBlockState(pos.east()).isOf(this)) {
                _snowman = (double)pos.getZ() + 0.5 + 0.25 * (double)_snowman2;
                _snowman = random.nextFloat() * 2.0f * (float)_snowman2;
            } else {
                d = (double)pos.getX() + 0.5 + 0.25 * (double)_snowman2;
                _snowman = random.nextFloat() * 2.0f * (float)_snowman2;
            }
