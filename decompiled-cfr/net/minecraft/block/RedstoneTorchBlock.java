...
extends TorchBlock {
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        List<BurnoutEntry> _snowman2 = BURNOUT_MAP.get(world);
        while (_snowman2 != null && !_snowman2.isEmpty() && world.getTime() - _snowman2.get(0).time > 60L) {
            _snowman2.remove(0);
        }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        _snowman = (double)pos.getY() + 0.7 + (random.nextDouble() - 0.5) * 0.2;
        _snowman = (double)pos.getZ() + 0.5 + (random.nextDouble() - 0.5) * 0.2;
        world.addParticle(this.particle, d, _snowman, _snowman, 0.0, 0.0, 0.0);
    }
    ...
    private static boolean isBurnedOut(World world, BlockPos pos, boolean addNew) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            BurnoutEntry burnoutEntry = (BurnoutEntry)list.get(i);
            if (!burnoutEntry.pos.equals(pos) || ++_snowman2 < 8) continue;
            return true;
        }
        return false;
    }
