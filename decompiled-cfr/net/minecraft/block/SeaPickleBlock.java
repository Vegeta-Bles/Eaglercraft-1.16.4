...
Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        boolean _snowman3 = _snowman2.getFluid() == Fluids.WATER;
        return (BlockState)super.getPlacementState(ctx).with(WATERLOGGED, _snowman3);
    }
        ...
        if (!SeaPickleBlock.isDry(state) && world.getBlockState(pos.down()).isIn(BlockTags.CORAL_BLOCKS)) {
            ...
            for (_snowman = 0; _snowman < 5; ++_snowman) {
                ...
                for (_snowman = 0; _snowman < _snowman; ++_snowman) {
                    _snowman = 2 + pos.getY() - 1;
                    for (_snowman = _snowman - 2; _snowman < _snowman; ++_snowman) {
                        BlockPos blockPos = new BlockPos(_snowman + _snowman, _snowman, pos.getZ() - _snowman + _snowman);
                        if (blockPos == pos || random.nextInt(6) != 0 || !world.getBlockState(blockPos).isOf(Blocks.WATER) || !(_snowman = world.getBlockState(blockPos.down())).isIn(BlockTags.CORAL_BLOCKS)) continue;
