...
extends BlockWithEntity {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (blockEntity instanceof ShulkerBoxBlockEntity) {
            ...
            if (shulkerBoxBlockEntity.getAnimationStage() == ShulkerBoxBlockEntity.AnimationStage.CLOSED) {
                ...
                _snowman2 = world.isSpaceEmpty(ShulkerLidCollisions.getLidCollisionBox(pos, direction));
            } else {
                _snowman2 = true;
            }
            if (_snowman2) {
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (blockEntity instanceof ShulkerBoxBlockEntity) {
            ...
            if (!world.isClient && player.isCreative() && !shulkerBoxBlockEntity.isEmpty()) {
                ...
                CompoundTag _snowman2 = shulkerBoxBlockEntity.serializeInventory(new CompoundTag());
                if (!_snowman2.isEmpty()) {
                    itemStack.putSubTag("BlockEntityTag", _snowman2);
                }
                if (shulkerBoxBlockEntity.hasCustomName()) {
                    itemStack.setCustomName(shulkerBoxBlockEntity.getCustomName());
                }
                ItemEntity _snowman3 = new ItemEntity(world, (double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5, itemStack);
                _snowman3.setToDefaultPickupDelay();
                world.spawnEntity(_snowman3);
            } else {
        ...
        if (compoundTag != null) {
            ...
            if (compoundTag.contains("Items", 9)) {
                ...
                for (ItemStack itemStack : defaultedList) {
                    ...
                    ++_snowman3;
                    if (_snowman2 > 4) continue;
                    ++_snowman2;
                    MutableText mutableText = itemStack.getName().shallowCopy();
                    mutableText.append(" x").append(String.valueOf(itemStack.getCount()));
                    tooltip.add(mutableText);
                }
                if (_snowman3 - _snowman2 > 0) {
                    tooltip.add(new TranslatableText("container.shulkerBox.more", _snowman3 - _snowman2).formatted(Formatting.ITALIC));
                }
            }
        }
    }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        ...
        ShulkerBoxBlockEntity _snowman2 = (ShulkerBoxBlockEntity)world.getBlockEntity(pos);
        CompoundTag _snowman3 = _snowman2.serializeInventory(new CompoundTag());
        if (!_snowman3.isEmpty()) {
            itemStack.putSubTag("BlockEntityTag", _snowman3);
        }
        return itemStack;
    }
