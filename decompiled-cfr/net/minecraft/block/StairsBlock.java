...
implements Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        FluidState _snowman3 = ctx.getWorld().getFluidState(_snowman2);
        BlockState _snowman4 = (BlockState)((BlockState)((BlockState)this.getDefaultState().with(FACING, ctx.getPlayerFacing())).with(HALF, direction == Direction.DOWN || direction != Direction.UP && ctx.getHitPos().y - (double)_snowman2.getY() > 0.5 ? BlockHalf.TOP : BlockHalf.BOTTOM)).with(WATERLOGGED, _snowman3.getFluid() == Fluids.WATER);
        return (BlockState)_snowman4.with(SHAPE, StairsBlock.getStairShape(_snowman4, ctx.getWorld(), _snowman2));
    }
    ...
    private static StairShape getStairShape(BlockState state, BlockView world, BlockPos pos) {
        ...
        if (StairsBlock.isStairs(_snowman2) && state.get(HALF) == _snowman2.get(HALF) && ((Direction)(_snowman3 = _snowman2.get(FACING))).getAxis() != state.get(FACING).getAxis() && StairsBlock.method_10678(state, world, pos, ((Direction)_snowman3).getOpposite())) {
            if (_snowman3 == direction.rotateYCounterclockwise()) {
                return StairShape.OUTER_LEFT;
            }
            return StairShape.OUTER_RIGHT;
        }
        Object _snowman3 = world.getBlockState(pos.offset(direction.getOpposite()));
        if (StairsBlock.isStairs((BlockState)_snowman3) && state.get(HALF) == ((State)_snowman3).get(HALF) && (_snowman = ((State)_snowman3).get(FACING)).getAxis() != state.get(FACING).getAxis() && StairsBlock.method_10678(state, world, pos, _snowman)) {
            if (_snowman == direction.rotateYCounterclockwise()) {
    ...
    public BlockState mirror(BlockState state, BlockMirror mirror) {
        Direction direction = state.get(FACING);
        StairShape _snowman2 = state.get(SHAPE);
        switch (mirror) {
            case LEFT_RIGHT: {
