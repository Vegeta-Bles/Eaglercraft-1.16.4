...
implements Fertilizable {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextInt((int)(25.0f / f) + 1) == 0) {
            int n = state.get(AGE);
            if (n < 7) {
                state = (BlockState)state.with(AGE, n + 1);
                world.setBlockState(pos, state, 2);
            } else {
                Direction direction = Direction.Type.HORIZONTAL.random(random);
                BlockPos _snowman2 = pos.offset(direction);
                BlockState _snowman3 = world.getBlockState(_snowman2.down());
                if (world.getBlockState(_snowman2).isAir() && (_snowman3.isOf(Blocks.FARMLAND) || _snowman3.isOf(Blocks.DIRT) || _snowman3.isOf(Blocks.COARSE_DIRT) || _snowman3.isOf(Blocks.PODZOL) || _snowman3.isOf(Blocks.GRASS_BLOCK))) {
                    world.setBlockState(_snowman2, this.gourdBlock.getDefaultState());
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        int n = Math.min(7, state.get(AGE) + MathHelper.nextInt(world.random, 2, 5));
        BlockState _snowman2 = (BlockState)state.with(AGE, n);
        world.setBlockState(pos, _snowman2, 2);
        if (n == 7) {
            _snowman2.randomTick(world, pos, world.random);
        }
    }
