...
extends Block {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (world.isAir(pos.up())) {
            int n = 1;
            while (world.getBlockState(pos.down(n)).isOf(this)) {
                ++n;
            }
            if (n < 3) {
                _snowman = state.get(AGE);
                if (_snowman == 15) {
                    world.setBlockState(pos.up(), this.getDefaultState());
                    world.setBlockState(pos, (BlockState)state.with(AGE, 0), 4);
                } else {
                    world.setBlockState(pos, (BlockState)state.with(AGE, _snowman + 1), 4);
                }
            }
        }
    }
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        if (blockState.isOf(Blocks.GRASS_BLOCK) || blockState.isOf(Blocks.DIRT) || blockState.isOf(Blocks.COARSE_DIRT) || blockState.isOf(Blocks.PODZOL) || blockState.isOf(Blocks.SAND) || blockState.isOf(Blocks.RED_SAND)) {
            ...
            for (Direction direction : Direction.Type.HORIZONTAL) {
                ...
                FluidState _snowman2 = world.getFluidState(blockPos.offset(direction));
                if (!_snowman2.isIn(FluidTags.WATER) && !blockState2.isOf(Blocks.FROSTED_ICE)) continue;
                return true;
            }
        }
        return false;
