...
implements Waterloggable {
    ...
    protected void playToggleSound(@Nullable PlayerEntity player, World world, BlockPos pos, boolean open) {
        if (open) {
            int n = this.material == Material.METAL ? 1037 : 1007;
            world.syncWorldEvent(player, n, pos, 0);
        } else {
            int n = this.material == Material.METAL ? 1036 : 1013;
            world.syncWorldEvent(player, n, pos, 0);
        }
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        FluidState _snowman2 = ctx.getWorld().getFluidState(ctx.getBlockPos());
        Direction _snowman3 = ctx.getSide();
        blockState = ctx.canReplaceExisting() || !_snowman3.getAxis().isHorizontal() ? (BlockState)((BlockState)blockState.with(FACING, ctx.getPlayerFacing().getOpposite())).with(HALF, _snowman3 == Direction.UP ? BlockHalf.BOTTOM : BlockHalf.TOP) : (BlockState)((BlockState)blockState.with(FACING, _snowman3)).with(HALF, ctx.getHitPos().y - (double)ctx.getBlockPos().getY() > 0.5 ? BlockHalf.TOP : BlockHalf.BOTTOM);
        if (ctx.getWorld().isReceivingRedstonePower(ctx.getBlockPos())) {
            blockState = (BlockState)((BlockState)blockState.with(OPEN, true)).with(POWERED, true);
        }
        return (BlockState)blockState.with(WATERLOGGED, _snowman2.getFluid() == Fluids.WATER);
    }
