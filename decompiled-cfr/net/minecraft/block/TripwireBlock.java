...
extends Block {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return (BlockState)((BlockState)((BlockState)((BlockState)this.getDefaultState().with(NORTH, this.shouldConnectTo(world.getBlockState(_snowman2.north()), Direction.NORTH))).with(EAST, this.shouldConnectTo(world.getBlockState(_snowman2.east()), Direction.EAST))).with(SOUTH, this.shouldConnectTo(world.getBlockState(_snowman2.south()), Direction.SOUTH))).with(WEST, this.shouldConnectTo(world.getBlockState(_snowman2.west()), Direction.WEST));
    }
    ...
    private void update(World world, BlockPos pos, BlockState state) {
        block0: for (Direction direction : new Direction[]{Direction.SOUTH, Direction.WEST}) {
            for (int i = 1; i < 42; ++i) {
                ...
                if (_snowman2.isOf(this.hookBlock)) {
                    if (_snowman2.get(TripwireHookBlock.FACING) != direction.getOpposite()) continue block0;
                    this.hookBlock.update(world, blockPos, _snowman2, false, true, i, state);
                    continue block0;
                }
                if (!_snowman2.isOf(this)) continue block0;
            }
        }
    }
    ...
    private void updatePowered(World world, BlockPos pos) {
        ...
        boolean _snowman2 = blockState.get(POWERED);
        boolean _snowman3 = false;
        List<Entity> _snowman4 = world.getOtherEntities(null, blockState.getOutlineShape(world, pos).getBoundingBox().offset(pos));
        if (!_snowman4.isEmpty()) {
            for (Entity entity : _snowman4) {
                if (entity.canAvoidTraps()) continue;
                _snowman3 = true;
                break;
            }
        }
        if (_snowman3 != _snowman2) {
            blockState = (BlockState)blockState.with(POWERED, _snowman3);
