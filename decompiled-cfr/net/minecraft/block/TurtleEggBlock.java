...
extends Block {
    ...
    private void tryBreakEgg(World world, BlockPos blockPos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && (_snowman = world.getBlockState(blockPos)).isOf(Blocks.TURTLE_EGG)) {
            this.breakEgg(world, blockPos, _snowman);
        }
    }
    ...
    private void breakEgg(World world, BlockPos pos, BlockState state) {
        ...
        int n = state.get(EGGS);
        if (n <= 1) {
            world.breakBlock(pos, false);
        } else {
            world.setBlockState(pos, (BlockState)state.with(EGGS, n - 1), 2);
