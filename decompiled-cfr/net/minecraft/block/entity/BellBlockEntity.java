...
implements Tickable {
    ...
    private void applyParticlesToRaiders(World world) {
        ...
        BlockPos blockPos = this.getPos();
        MutableInt _snowman2 = new MutableInt(16700985);
        int _snowman3 = (int)this.hearingEntities.stream().filter(livingEntity -> blockPos.isWithinDistance(livingEntity.getPos(), 48.0)).count();
        this.hearingEntities.stream().filter(this::isRaiderEntity).forEach(livingEntity -> {
            float f = 1.0f;
            _snowman = MathHelper.sqrt((livingEntity.getX() - (double)blockPos.getX()) * (livingEntity.getX() - (double)blockPos.getX()) + (livingEntity.getZ() - (double)blockPos.getZ()) * (livingEntity.getZ() - (double)blockPos.getZ()));
            double _snowman2 = (double)((float)blockPos.getX() + 0.5f) + (double)(1.0f / _snowman) * (livingEntity.getX() - (double)blockPos.getX());
            double _snowman3 = (double)((float)blockPos.getZ() + 0.5f) + (double)(1.0f / _snowman) * (livingEntity.getZ() - (double)blockPos.getZ());
            int _snowman4 = MathHelper.clamp((_snowman3 - 21) / -2, 3, 15);
            for (int i = 0; i < _snowman4; ++i) {
                _snowman = _snowman2.addAndGet(5);
                double d = (double)BackgroundHelper.ColorMixer.getRed(_snowman) / 255.0;
                _snowman = (double)BackgroundHelper.ColorMixer.getGreen(_snowman) / 255.0;
                _snowman = (double)BackgroundHelper.ColorMixer.getBlue(_snowman) / 255.0;
                world.addParticle(ParticleTypes.ENTITY_EFFECT, _snowman2, (float)blockPos.getY() + 0.5f, _snowman3, d, _snowman, _snowman);
            }
        });
    }
