...
Tickable {
    ...
    public void tick() {
        ...
        if (playerEntity != null) {
            ...
            _snowman = playerEntity.getZ() - ((double)this.pos.getZ() + 0.5);
            this.field_11962 = (float)MathHelper.atan2(_snowman, d);
            this.nextPageTurningSpeed += 0.1f;
            if (this.nextPageTurningSpeed < 0.5f || RANDOM.nextInt(40) == 0) {
                float f2 = this.field_11969;
                do {
                    this.field_11969 += (float)(RANDOM.nextInt(4) - RANDOM.nextInt(4));
                } while (f2 == this.field_11969);
            }
        } else {
        ...
        for (f = this.field_11962 - this.field_11964; f >= (float)Math.PI; f -= (float)Math.PI * 2) {
        }
        while (f < (float)(-Math.PI)) {
            f += (float)Math.PI * 2;
        }
        this.field_11964 += f * 0.4f;
        this.nextPageTurningSpeed = MathHelper.clamp(this.nextPageTurningSpeed, 0.0f, 1.0f);
        ++this.ticks;
        this.pageAngle = this.nextPageAngle;
        _snowman = (this.field_11969 - this.nextPageAngle) * 0.4f;
        _snowman = 0.2f;
        _snowman = MathHelper.clamp(_snowman, -0.2f, 0.2f);
        this.field_11967 += (_snowman - this.field_11967) * 0.9f;
