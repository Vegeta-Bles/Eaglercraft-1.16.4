...
extends BlockEntity {
    ...
    public void fromTag(BlockState state, CompoundTag tag) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            MutableText _snowman2 = Text.Serializer.fromJson(string.isEmpty() ? "\"\"" : string);
            if (this.world instanceof ServerWorld) {
                try {
                    this.text[i] = Texts.parse(this.getCommandSource(null), _snowman2, null, 0);
                }
                catch (CommandSyntaxException commandSyntaxException) {
                    this.text[i] = _snowman2;
                }
            } else {
                this.text[i] = _snowman2;
            }
    ...
    public boolean onActivate(PlayerEntity player) {
        for (Text text : this.text) {
            Style style = _snowman = text == null ? null : text.getStyle();
            if (_snowman == null || _snowman.getClickEvent() == null || (_snowman = _snowman.getClickEvent()).getAction() != ClickEvent.Action.RUN_COMMAND) continue;
            player.getServer().getCommandManager().execute(this.getCommandSource((ServerPlayerEntity)player), _snowman.getValue());
        }
        return true;
    }
    ...
    public ServerCommandSource getCommandSource(@Nullable ServerPlayerEntity player) {
        ...
        Text _snowman2 = player == null ? new LiteralText("Sign") : player.getDisplayName();
        return new ServerCommandSource(CommandOutput.DUMMY, Vec3d.ofCenter(this.pos), Vec2f.ZERO, (ServerWorld)this.world, 2, string, _snowman2, this.world.getServer(), player);
    }
