...
public class Framebuffer {
    ...
    public void copyDepthFrom(Framebuffer framebuffer) {
        ...
        if (GlStateManager.supportsGl30()) {
            ...
            int n = GlStateManager.getFramebufferDepthAttachment();
            if (n != 0) {
                _snowman = GlStateManager.getActiveBoundTexture();
                ...
                GlStateManager.bindTexture(_snowman);
            }
        }
        GlStateManager.bindFramebuffer(FramebufferInfo.FRAME_BUFFER, 0);
    }
    ...
    public void setTexFilter(int n) {
        RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
        this.texFilter = n;
        GlStateManager.bindTexture(this.colorAttachment);
    ...
    public void checkFramebufferStatus() {
        RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
        int n = GlStateManager.checkFramebufferStatus(FramebufferInfo.FRAME_BUFFER);
        if (n == FramebufferInfo.FRAME_BUFFER_COMPLETE) {
            return;
        }
        if (n == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_ATTACHMENT) {
            throw new RuntimeException("GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT");
        }
        if (n == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_MISSING_ATTACHMENT) {
            throw new RuntimeException("GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT");
        }
        if (n == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_DRAW_BUFFER) {
            throw new RuntimeException("GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER");
        }
        if (n == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_READ_BUFFER) {
    ...
    private void drawInternal(int width, int height, boolean bl) {
        ...
        float f = width;
        _snowman = height;
        _snowman = (float)this.viewportWidth / (float)this.textureWidth;
        _snowman = (float)this.viewportHeight / (float)this.textureHeight;
        Tessellator _snowman2 = RenderSystem.renderThreadTesselator();
        BufferBuilder _snowman3 = _snowman2.getBuffer();
        _snowman3.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
        _snowman3.vertex(0.0, _snowman, 0.0).texture(0.0f, 0.0f).color(255, 255, 255, 255).next();
        _snowman3.vertex(f, _snowman, 0.0).texture(_snowman, 0.0f).color(255, 255, 255, 255).next();
        _snowman3.vertex(f, 0.0, 0.0).texture(_snowman, _snowman).color(255, 255, 255, 255).next();
        _snowman3.vertex(0.0, 0.0, 0.0).texture(0.0f, _snowman).color(255, 255, 255, 255).next();
        _snowman2.draw();
        this.endRead();
    ...
    public void clear(boolean getError) {
        ...
        GlStateManager.clear(n, getError);
        this.endWrite();
    }
