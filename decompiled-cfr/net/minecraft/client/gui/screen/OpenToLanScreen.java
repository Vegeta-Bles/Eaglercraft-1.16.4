...
extends Screen {
    ...
    protected void init() {
        this.addButton(new ButtonWidget(this.width / 2 - 155, this.height - 28, 150, 20, new TranslatableText("lanServer.start"), buttonWidget -> {
            ...
            int n = NetworkUtils.findLocalPort();
            TranslatableText _snowman2 = this.client.getServer().openToLan(GameMode.byName(this.gameMode), this.allowCommands, n) ? new TranslatableText("commands.publish.started", n) : new TranslatableText("commands.publish.failed");
            this.client.inGameHud.getChatHud().addMessage(_snowman2);
