...
extends Screen {
    ...
    protected void init() {
        ...
        this.buttonDelete = this.addButton(new ButtonWidget(this.width / 2 - 74, this.height - 28, 70, 20, new TranslatableText("selectServer.delete"), buttonWidget -> {
            MultiplayerServerListWidget.Entry entry = (MultiplayerServerListWidget.Entry)this.serverListWidget.getSelected();
            if (entry instanceof MultiplayerServerListWidget.ServerEntry && (_snowman = ((MultiplayerServerListWidget.ServerEntry)entry).getServer().name) != null) {
                TranslatableText translatableText = new TranslatableText("selectServer.deleteQuestion");
                _snowman = new TranslatableText("selectServer.deleteWarning", _snowman);
                _snowman = new TranslatableText("selectServer.deleteButton");
                Text _snowman2 = ScreenTexts.CANCEL;
                this.client.openScreen(new ConfirmScreen(this::removeEntry, translatableText, _snowman, _snowman, _snowman2));
            }
        }));
