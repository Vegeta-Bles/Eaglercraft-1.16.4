...
extends GameOptionsScreen {
    ...
    protected void init() {
        int n = 0;
        this.addButton(new SoundSliderWidget(this.client, this.width / 2 - 155 + n % 2 * 160, this.height / 6 - 12 + 24 * (n >> 1), SoundCategory.MASTER, 310));
        n += 2;
        for (SoundCategory soundCategory : SoundCategory.values()) {
            if (soundCategory == SoundCategory.MASTER) continue;
            this.addButton(new SoundSliderWidget(this.client, this.width / 2 - 155 + n % 2 * 160, this.height / 6 - 12 + 24 * (n >> 1), soundCategory, 150));
            ++n;
        }
        this.addButton(new OptionButtonWidget(this.width / 2 - 75, this.height / 6 - 12 + 24 * (++n >> 1), 150, 20, Option.SUBTITLES, Option.SUBTITLES.getDisplayString(this.gameOptions), button -> {
