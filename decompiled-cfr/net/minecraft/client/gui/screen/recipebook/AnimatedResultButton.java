...
extends AbstractButtonWidget {
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        boolean bl = _snowman = this.bounce > 0.0f;
        if (_snowman) {
        ...
        ItemStack _snowman4 = list.get(this.currentResultIndex).getOutput();
        int _snowman5 = 4;
        if (this.results.hasSingleOutput() && this.getResults().size() > 1) {
            minecraftClient.getItemRenderer().renderInGuiWithOverrides(_snowman4, this.x + _snowman5 + 1, this.y + _snowman5 + 1);
            --_snowman5;
        }
        minecraftClient.getItemRenderer().renderInGui(_snowman4, this.x + _snowman5, this.y + _snowman5);
        if (_snowman) {
    ...
    public List<Text> getTooltip(Screen screen) {
        ...
        ArrayList _snowman2 = Lists.newArrayList(screen.getTooltipFromItem(itemStack));
        if (this.results.getResults(this.recipeBook.isFilteringCraftable(this.craftingScreenHandler)).size() > 1) {
            _snowman2.add(field_26595);
        }
        return _snowman2;
    }
