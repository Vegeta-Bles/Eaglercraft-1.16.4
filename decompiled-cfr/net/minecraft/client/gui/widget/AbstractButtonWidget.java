...
Element {
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        int _snowman3 = this.getYImage(this.isHovered());
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        RenderSystem.enableDepthTest();
        this.drawTexture(matrices, this.x, this.y, 0, 46 + _snowman3 * 20, this.width / 2, this.height);
        this.drawTexture(matrices, this.x + this.width / 2, this.y, 200 - this.width / 2, 46 + _snowman3 * 20, this.width / 2, this.height);
        this.renderBg(matrices, minecraftClient, mouseX, mouseY);
        int _snowman4 = this.active ? 0xFFFFFF : 0xA0A0A0;
        AbstractButtonWidget.drawCenteredText(matrices, _snowman2, this.getMessage(), this.x + this.width / 2, this.y + (this.height - 8) / 2, _snowman4 | MathHelper.ceil(this.alpha * 255.0f) << 24);
    }
