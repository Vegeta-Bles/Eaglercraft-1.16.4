...
implements ClientLoginPacketListener {
    ...
    public void onHello(LoginHelloS2CPacket packet) {
        LoginKeyC2SPacket _snowman6;
        Cipher _snowman5;
        Cipher _snowman4;
        String _snowman3;
        try {
            SecretKey secretKey = NetworkEncryptionUtils.generateKey();
            PublicKey _snowman2 = packet.getPublicKey();
            _snowman3 = new BigInteger(NetworkEncryptionUtils.generateServerId(packet.getServerId(), _snowman2, secretKey)).toString(16);
            _snowman4 = NetworkEncryptionUtils.cipherFromKey(2, secretKey);
            _snowman5 = NetworkEncryptionUtils.cipherFromKey(1, secretKey);
            _snowman6 = new LoginKeyC2SPacket(secretKey, _snowman2, packet.getNonce());
        }
        ...
        NetworkUtils.downloadExecutor.submit(() -> {
            Text text = this.joinServerSession(_snowman3);
            if (text != null) {
            ...
            this.connection.send(_snowman6, (GenericFutureListener<? extends Future<? super Void>>)((GenericFutureListener)future -> this.connection.setupEncryption(_snowman4, _snowman5)));
        });
    }
