...
extends AbstractClientPlayerEntity {
    ...
    public void tickMovement() {
        ...
        this.tickHandSwing();
        float f = !this.onGround || this.isDead() ? 0.0f : Math.min(0.1f, MathHelper.sqrt(OtherClientPlayerEntity.squaredHorizontalLength(this.getVelocity())));
        if (this.onGround || this.isDead()) {
            _snowman = 0.0f;
        } else {
            _snowman = (float)Math.atan(-this.getVelocity().y * (double)0.2f) * 15.0f;
        }
