...
public abstract class Option {
    public static final DoubleOption BIOME_BLEND_RADIUS = new DoubleOption("options.biomeBlendRadius", 0.0, 7.0, 1.0f, gameOptions -> gameOptions.biomeBlendRadius, (gameOptions, biomeBlendRadius) -> {
        ...
        int _snowman2 = (int)d * 2 + 1;
        return option.getGenericLabel(new TranslatableText("options.biomeBlendRadius." + _snowman2));
    });
    ...
    public static final CyclingOption GRAPHICS = new CyclingOption("options.graphics", (gameOptions, amount) -> {
        ...
        VideoWarningManager _snowman2 = minecraftClient.getVideoWarningManager();
        if (gameOptions.graphicsMode == GraphicsMode.FANCY && _snowman2.canWarn()) {
            _snowman2.scheduleWarning();
            return;
        }
        gameOptions.graphicsMode = gameOptions.graphicsMode.next();
        if (gameOptions.graphicsMode == GraphicsMode.FABULOUS && (!GlStateManager.supportsGl30() || _snowman2.hasCancelledAfterWarning())) {
        ...
        TranslatableText translatableText = new TranslatableText(gameOptions.graphicsMode.getTranslationKey());
        if (gameOptions.graphicsMode == GraphicsMode.FABULOUS) {
            return option.getGenericLabel(translatableText.formatted(Formatting.ITALIC));
        }
        return option.getGenericLabel(translatableText);
    });
    ...
    public static final BooleanOption field_26924 = new BooleanOption("options.hideMatchedNames", field_26925, gameOptions -> gameOptions.field_26926, (gameOptions, bl) -> {
        gameOptions.field_26926 = bl;
    });
