...
public abstract class Particle {
    ...
    public Particle(ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
        ...
        _snowman = MathHelper.sqrt(this.velocityX * this.velocityX + this.velocityY * this.velocityY + this.velocityZ * this.velocityZ);
        this.velocityX = this.velocityX / (double)_snowman * (double)f * (double)0.4f;
        this.velocityY = this.velocityY / (double)_snowman * (double)f * (double)0.4f + (double)0.1f;
        this.velocityZ = this.velocityZ / (double)_snowman * (double)f * (double)0.4f;
    }
    ...
    protected void setBoundingBoxSpacing(float spacingXZ, float spacingY) {
        if (spacingXZ != this.spacingXZ || spacingY != this.spacingY) {
            ...
            double _snowman2 = (box.minX + box.maxX - (double)spacingXZ) / 2.0;
            double _snowman3 = (box.minZ + box.maxZ - (double)spacingXZ) / 2.0;
            this.setBoundingBox(new Box(_snowman2, box.minY, _snowman3, _snowman2 + (double)this.spacingXZ, box.minY + (double)this.spacingY, _snowman3 + (double)this.spacingXZ));
        }
    }
    ...
    public void setPos(double x, double y, double z) {
        ...
        _snowman = this.spacingY;
        this.setBoundingBox(new Box(x - (double)f, y, z - (double)f, x + (double)f, y + (double)_snowman, z + (double)f));
    }
    ...
    public void move(double dx, double dy, double dz) {
        ...
        if (Math.abs(d) >= (double)1.0E-5f && Math.abs(dy) < (double)1.0E-5f) {
            this.field_21507 = true;
        }
        boolean bl = this.onGround = d != dy && d < 0.0;
        if (d2 != dx) {
            this.velocityX = 0.0;
        }
        if (_snowman != dz) {
