...
extends ValueObject {
    ...
    public void updateServerPing(RealmsServerPlayerList serverPlayerList) {
        ...
        for (String string : serverPlayerList.players) {
            ...
            _snowman = "";
            try {
                _snowman = RealmsUtil.uuidToName(string);
            }
            catch (Exception exception) {
                LOGGER.error("Could not get name for " + string, (Throwable)exception);
                continue;
            }
            arrayList.add(_snowman);
            ++_snowman2;
        }
        this.serverPing.nrOfPlayers = String.valueOf(_snowman2);
    ...
    public static RealmsServer parse(JsonObject node) {
        ...
        try {
            ...
            LOGGER.error("Could not parse McoServer: " + _snowman2.getMessage());
        }
        return realmsServer;
    }
    ...
    private static List<PlayerInfo> parseInvited(JsonArray jsonArray) {
        ...
        for (JsonElement jsonElement : jsonArray) {
            try {
                ...
                PlayerInfo _snowman2 = new PlayerInfo();
                _snowman2.setName(JsonUtils.getStringOr("name", jsonObject, null));
                _snowman2.setUuid(JsonUtils.getStringOr("uuid", jsonObject, null));
                _snowman2.setOperator(JsonUtils.getBooleanOr("operator", jsonObject, false));
                _snowman2.setAccepted(JsonUtils.getBooleanOr("accepted", jsonObject, false));
                _snowman2.setOnline(JsonUtils.getBooleanOr("online", jsonObject, false));
    ...
    private static Map<Integer, RealmsWorldOptions> parseSlots(JsonArray json) {
        ...
        for (JsonElement jsonElement : json) {
            try {
                ...
                JsonElement _snowman3 = _snowman2.parse(jsonObject.get("options").getAsString());
                RealmsWorldOptions _snowman4 = _snowman3 == null ? RealmsWorldOptions.getDefaults() : RealmsWorldOptions.parse(_snowman3.getAsJsonObject());
                int _snowman5 = JsonUtils.getIntOr("slotId", jsonObject, -1);
                hashMap.put(_snowman5, _snowman4);
            }
            catch (Exception exception) {}
        }
        for (int i = 1; i <= 3; ++i) {
            if (hashMap.containsKey(i)) continue;
            hashMap.put(i, RealmsWorldOptions.getEmptyDefaults());
        }
        return hashMap;
    }
