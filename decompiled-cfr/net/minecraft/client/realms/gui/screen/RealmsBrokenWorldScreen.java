...
extends RealmsScreen {
    ...
    private void addButtons() {
        for (Map.Entry<Integer, RealmsWorldOptions> entry : this.field_20492.slots.entrySet()) {
            int n = entry.getKey();
            boolean _snowman2 = n != this.field_20492.activeSlot || this.field_20492.worldType == RealmsServer.WorldType.MINIGAME;
            ButtonWidget _snowman3 = _snowman2 ? new ButtonWidget(this.getFramePositionX(n), RealmsBrokenWorldScreen.row(8), 80, 20, new TranslatableText("mco.brokenworld.play"), buttonWidget -> {
                if (this.field_20492.slots.get((Object)Integer.valueOf((int)n2)).empty) {
                    ...
                    int n2;
                    this.client.openScreen(new RealmsLongRunningMcoTaskScreen(this.parent, new SwitchSlotTask(this.field_20492.id, n, this::method_25123)));
                }
            }) : new ButtonWidget(this.getFramePositionX(n), RealmsBrokenWorldScreen.row(8), 80, 20, new TranslatableText("mco.brokenworld.download"), buttonWidget -> {
            ...
            if (this.slotsThatHasBeenDownloaded.contains(n)) {
                ...
                _snowman3.setMessage(new TranslatableText("mco.brokenworld.downloaded"));
            }
            this.addButton(_snowman3);
            this.addButton(new ButtonWidget(this.getFramePositionX(n), RealmsBrokenWorldScreen.row(10), 80, 20, new TranslatableText("mco.brokenworld.reset"), buttonWidget -> {
                RealmsResetWorldScreen realmsResetWorldScreen = new RealmsResetWorldScreen(this, this.field_20492, this::method_25123, () -> {
                    this.client.openScreen(this);
                    this.method_25123();
                });
                if (n != this.field_20492.activeSlot || this.field_20492.worldType == RealmsServer.WorldType.MINIGAME) {
                    realmsResetWorldScreen.setSlot(n);
                }
                this.client.openScreen(realmsResetWorldScreen);
            }));
    ...
    private void fetchServerData(long worldId) {
        new Thread(() -> {
            ...
            try {
                ...
                this.client.openScreen(new RealmsGenericErrorScreen(Text.of(_snowman2.getMessage()), this.parent));
            }
        }).start();
    }
    ...
    private void downloadWorld(int slotId) {
        ...
        try {
            ...
            RealmsDownloadLatestWorldScreen _snowman2 = new RealmsDownloadLatestWorldScreen(this, worldDownload, this.field_20492.getWorldName(slotId), bl -> {
                if (bl) {
    ...
    private void drawSlotFrame(MatrixStack matrixStack2, int y, int xm, int ym, int n, boolean bl, String string, int n2, long l, String string2, boolean bl2) {
        ...
        DrawableHelper.drawTexture(matrixStack2, y + 3, xm + 3, 0.0f, 0.0f, 74, 74, 74, 74);
        ...
        DrawableHelper.drawTexture(matrixStack2, y, xm, 0.0f, 0.0f, 80, 80, 80, 80);
        RealmsBrokenWorldScreen.drawCenteredString(matrixStack2, this.textRenderer, string, y + 40, xm + 66, 0xFFFFFF);
    }
}
