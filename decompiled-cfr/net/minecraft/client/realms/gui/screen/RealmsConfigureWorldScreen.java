...
extends RealmsScreenWithCallback {
    ...
    private void addSlotButton(int slotIndex) {
        int n = this.frame(slotIndex);
        _snowman = RealmsConfigureWorldScreen.row(5) + 5;
        RealmsWorldSlotButton _snowman2 = new RealmsWorldSlotButton(n, _snowman, 80, 80, () -> this.server, text -> {
            this.toolTip = text;
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        int _snowman2 = this.textRenderer.getWidth(string);
        int _snowman3 = this.server.state == RealmsServer.State.CLOSED ? 0xA0A0A0 : 0x7FFF7F;
        int _snowman4 = this.textRenderer.getWidth(field_26480);
        RealmsConfigureWorldScreen.drawCenteredText(matrices, this.textRenderer, field_26480, this.width / 2, 12, 0xFFFFFF);
        RealmsConfigureWorldScreen.drawCenteredString(matrices, this.textRenderer, string, this.width / 2, 24, _snowman3);
        int _snowman5 = Math.min(this.buttonCenter(2, 3) + 80 - 11, this.width / 2 + _snowman2 / 2 + _snowman4 / 2 + 10);
        this.drawServerStatus(matrices, _snowman5, 7, mouseX, mouseY);
        if (this.isMinigame()) {
    ...
    private void fetchServerData(long worldId) {
        new Thread(() -> {
            ...
            try {
                ...
                this.client.execute(() -> this.client.openScreen(new RealmsGenericErrorScreen(Text.of(_snowman2.getMessage()), (Screen)this.parent)));
            }
        }).start();
    }
    ...
    private void switchToFullSlot(int selectedSlot, RealmsServer serverData) {
        TranslatableText translatableText = new TranslatableText("mco.configure.world.slot.switch.question.line1");
        _snowman = new TranslatableText("mco.configure.world.slot.switch.question.line2");
    ...
    protected void renderMousehoverTooltip(MatrixStack matrixStack, @Nullable Text text, int n, int n2) {
        ...
        if (_snowman + _snowman + 3 > this.right_x) {
            _snowman = _snowman - _snowman - 20;
        }
    ...
    private void drawServerStatus(MatrixStack matrixStack, int n, int n2, int n3, int n4) {
        if (this.server.expired) {
            this.drawExpired(matrixStack, n, n2, n3, n4);
        } else if (this.server.state == RealmsServer.State.CLOSED) {
            this.drawClosed(matrixStack, n, n2, n3, n4);
        } else if (this.server.state == RealmsServer.State.OPEN) {
            if (this.server.daysLeft < 7) {
                this.drawExpiring(matrixStack, n, n2, n3, n4, this.server.daysLeft);
            } else {
                this.drawOpen(matrixStack, n, n2, n3, n4);
            }
        }
    }
    ...
    private void drawExpiring(MatrixStack matrixStack, int n, int n2, int n3, int n4, int n5) {
        ...
        if (n3 >= n && n3 <= n + 9 && n4 >= n2 && n4 <= n2 + 27) {
            this.toolTip = n5 <= 0 ? field_26483 : (n5 == 1 ? field_26484 : new TranslatableText("mco.selectServer.expires.days", n5));
        }
    }
    ...
    private void drawClosed(MatrixStack matrixStack, int n, int n2, int n3, int n4) {
    ...
    public void saveSlotSettings(RealmsWorldOptions options) {
        ...
        try {
            _snowman2.updateSlot(this.server.id, this.server.activeSlot, options);
            ...
            this.client.openScreen(new RealmsGenericErrorScreen(_snowman3, (Screen)this));
            return;
        }
        this.client.openScreen(this);
    }
    ...
    public void saveSettings(String name, String desc) {
        ...
        RealmsClient _snowman2 = RealmsClient.createRealmsClient();
        try {
            _snowman2.update(this.server.id, name, string);
