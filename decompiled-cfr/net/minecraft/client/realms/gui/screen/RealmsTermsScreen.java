...
extends RealmsScreen {
    ...
    public void init() {
        ...
        this.addButton(new ButtonWidget(this.width / 4, RealmsTermsScreen.row(12), n, 20, new TranslatableText("mco.terms.buttons.agree"), buttonWidget -> this.agreedToTos()));
        this.addButton(new ButtonWidget(this.width / 2 + 4, RealmsTermsScreen.row(12), n, 20, new TranslatableText("mco.terms.buttons.disagree"), buttonWidget -> this.client.openScreen(this.parent)));
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        int n = this.textRenderer.getWidth(field_26524);
        _snowman = this.width / 2 - 121 + n;
        _snowman = RealmsTermsScreen.row(5);
        _snowman = _snowman + this.textRenderer.getWidth(field_26525) + 1;
        _snowman = _snowman + 1 + this.textRenderer.fontHeight;
        this.onLink = _snowman <= mouseX && mouseX <= _snowman && _snowman <= mouseY && mouseY <= _snowman;
        this.textRenderer.draw(matrices, field_26525, (float)(this.width / 2 - 120 + n), (float)RealmsTermsScreen.row(5), this.onLink ? 7107012 : 0x3366BB);
