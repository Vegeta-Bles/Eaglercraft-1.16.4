...
implements VertexConsumerProvider {
    ...
    public VertexConsumer getBuffer(RenderLayer renderLayer2) {
        ...
        if (renderLayer2.isOutline()) {
            VertexConsumer vertexConsumer = this.plainDrawer.getBuffer(renderLayer2);
            return new OutlineVertexConsumer(vertexConsumer, this.red, this.green, this.blue, this.alpha);
        }
        VertexConsumer _snowman2 = this.parent.getBuffer(renderLayer2);
        Optional<RenderLayer> _snowman3 = renderLayer2.getAffectedOutline();
        if (_snowman3.isPresent()) {
            VertexConsumer vertexConsumer = this.plainDrawer.getBuffer(_snowman3.get());
            OutlineVertexConsumer _snowman4 = new OutlineVertexConsumer(vertexConsumer, this.red, this.green, this.blue, this.alpha);
            return VertexConsumers.dual(_snowman4, _snowman2);
        }
        return _snowman2;
    }
