...
extends RenderPhase {
    ...
    public static RenderLayer getEndPortal(int layer) {
        RenderPhase.Texture _snowman2;
        if (layer <= 1) {
            RenderPhase.Transparency transparency = TRANSLUCENT_TRANSPARENCY;
            _snowman2 = new RenderPhase.Texture(EndPortalBlockEntityRenderer.SKY_TEXTURE, false, false);
        } else {
            transparency = ADDITIVE_TRANSPARENCY;
            _snowman2 = new RenderPhase.Texture(EndPortalBlockEntityRenderer.PORTAL_TEXTURE, false, false);
        }
        return RenderLayer.of("end_portal", VertexFormats.POSITION_COLOR, 7, 256, false, true, MultiPhaseParameters.builder().transparency(transparency).texture(_snowman2).texturing(new RenderPhase.PortalTexturing(layer)).fog(BLACK_FOG).build(false));
    }
