...
extends BlockEntityRenderer<BedBlockEntity> {
    ...
    @Override
    public void render(BedBlockEntity bedBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int n, int n2) {
        SpriteIdentifier spriteIdentifier = TexturedRenderLayers.BED_TEXTURES[bedBlockEntity.getColor().getId()];
        World _snowman2 = bedBlockEntity.getWorld();
        if (_snowman2 != null) {
            BlockState blockState = bedBlockEntity.getCachedState();
            DoubleBlockProperties.PropertySource<BedBlockEntity> _snowman3 = DoubleBlockProperties.toPropertySource(BlockEntityType.BED, BedBlock::getBedPart, BedBlock::getOppositePartDirection, ChestBlock.FACING, blockState, _snowman2, bedBlockEntity.getPos(), (worldAccess, blockPos) -> false);
            int _snowman4 = ((Int2IntFunction)_snowman3.apply(new LightmapCoordinatesRetriever())).get(n);
            this.method_3558(matrixStack, vertexConsumerProvider, blockState.get(BedBlock.PART) == BedPart.HEAD, blockState.get(BedBlock.FACING), spriteIdentifier, _snowman4, n2, false);
        } else {
            this.method_3558(matrixStack, vertexConsumerProvider, true, Direction.SOUTH, spriteIdentifier, n, n2, false);
            this.method_3558(matrixStack, vertexConsumerProvider, false, Direction.SOUTH, spriteIdentifier, n, n2, true);
        }
    }
