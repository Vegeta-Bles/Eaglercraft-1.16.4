...
extends MobEntityRenderer<WolfEntity, WolfEntityModel<WolfEntity>> {
    ...
    @Override
    public void render(WolfEntity wolfEntity2, float f, float f2, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int n) {
        WolfEntity wolfEntity2;
        if (wolfEntity2.isFurWet()) {
            float f3 = wolfEntity2.getFurWetBrightnessMultiplier(f2);
            ((WolfEntityModel)this.model).setColorMultiplier(f3, f3, f3);
        }
        super.render(wolfEntity2, f, f2, matrixStack, vertexConsumerProvider, n);
        if (wolfEntity2.isFurWet()) {
