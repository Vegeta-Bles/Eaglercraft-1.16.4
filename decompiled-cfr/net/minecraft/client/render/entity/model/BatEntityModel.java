...
extends CompositeEntityModel<BatEntity> {
    ...
    public BatEntityModel() {
        ...
        _snowman = new ModelPart(this, 24, 0);
        _snowman.mirror = true;
        _snowman.addCuboid(1.0f, -6.0f, -2.0f, 3.0f, 4.0f, 1.0f);
    ...
    @Override
    public void setAngles(BatEntity batEntity, float f, float f2, float f3, float f4, float f5) {
        if (batEntity.isRoosting()) {
            this.head.pitch = f5 * ((float)Math.PI / 180);
            this.head.yaw = (float)Math.PI - f4 * ((float)Math.PI / 180);
            ...
            this.body.pitch = 0.7853982f + MathHelper.cos(f3 * 0.1f) * 0.15f;
            this.body.yaw = 0.0f;
            this.rightWing.yaw = MathHelper.cos(f3 * 1.3f) * (float)Math.PI * 0.25f;
            this.leftWing.yaw = -this.rightWing.yaw;
