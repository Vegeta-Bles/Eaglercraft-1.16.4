...
extends AnimalModel<T> {
    ...
    public void animateModel(T t, float f, float f2, float f3) {
        ...
        this.stinger.visible = !((BeeEntity)t).hasStung();
    }
    ...
    public void setAngles(T t, float f, float f2, float f3, float f4, float f5) {
        ...
        boolean bl = _snowman = ((Entity)t).isOnGround() && ((Entity)t).getVelocity().lengthSquared() < 1.0E-7;
        if (_snowman) {
            this.rightWing.yaw = -0.2618f;
            this.rightWing.roll = 0.0f;
            this.leftWing.pitch = 0.0f;
            ...
            this.rightWing.roll = MathHelper.cos(_snowman) * (float)Math.PI * 0.15f;
        ...
        if (!t.hasAngerTime()) {
            ...
            this.body.roll = 0.0f;
            if (!_snowman) {
                _snowman = MathHelper.cos(f3 * 0.18f);
                this.body.pitch = 0.1f + _snowman * (float)Math.PI * 0.025f;
                this.leftAntenna.pitch = _snowman * (float)Math.PI * 0.03f;
                this.rightAntenna.pitch = _snowman * (float)Math.PI * 0.03f;
                this.frontLegs.pitch = -_snowman * (float)Math.PI * 0.1f + 0.3926991f;
                this.backLegs.pitch = -_snowman * (float)Math.PI * 0.05f + 0.7853982f;
                this.body.pivotY = 19.0f - MathHelper.cos(f3 * 0.18f) * 0.9f;
            }
        }
