...
extends CompositeEntityModel<GuardianEntity> {
    ...
    public void setAngles(GuardianEntity guardianEntity, float f, float f2, float f3, float f4, float f5) {
        _snowman = f3 - (float)guardianEntity.age;
        this.body.yaw = f4 * ((float)Math.PI / 180);
        this.body.pitch = f5 * ((float)Math.PI / 180);
        _snowman = (1.0f - guardianEntity.getTailAngle(_snowman)) * 0.55f;
        this.method_24185(f3, _snowman);
        ...
        if (entity != null) {
            ...
            _snowman = guardianEntity.getCameraPosVec(0.0f);
            double _snowman2 = vec3d.y - _snowman.y;
            this.eye.pivotY = _snowman2 > 0.0 ? 0.0f : 1.0f;
            _snowman = guardianEntity.getRotationVec(0.0f);
            _snowman = new Vec3d(_snowman.x, 0.0, _snowman.z);
            _snowman = new Vec3d(_snowman.x - vec3d.x, 0.0, _snowman.z - vec3d.z).normalize().rotateY(1.5707964f);
            double _snowman3 = _snowman.dotProduct(_snowman);
            this.eye.pivotX = MathHelper.sqrt((float)Math.abs(_snowman3)) * 2.0f * (float)Math.signum(_snowman3);
        }
        this.eye.visible = true;
        float f6 = guardianEntity.getSpikesExtension(_snowman);
        this.field_3378[0].yaw = MathHelper.sin(f6) * (float)Math.PI * 0.05f;
        this.field_3378[1].yaw = MathHelper.sin(f6) * (float)Math.PI * 0.1f;
        this.field_3378[1].pivotX = -1.5f;
        this.field_3378[1].pivotY = 0.5f;
        this.field_3378[1].pivotZ = 14.0f;
        this.field_3378[2].yaw = MathHelper.sin(f6) * (float)Math.PI * 0.15f;
    ...
    private void method_24185(float f, float f2) {
        for (int i = 0; i < 12; ++i) {
            ...
            this.field_3380[i].pivotX = field_17134[i] * (1.0f + MathHelper.cos(f * 1.5f + (float)i) * 0.01f - f2);
            this.field_3380[i].pivotY = 16.0f + field_17135[i] * (1.0f + MathHelper.cos(f * 1.5f + (float)i) * 0.01f - f2);
            this.field_3380[i].pivotZ = field_17136[i] * (1.0f + MathHelper.cos(f * 1.5f + (float)i) * 0.01f - f2);
        }
    }
}
