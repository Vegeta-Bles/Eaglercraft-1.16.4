...
extends QuadrupedEntityModel<T> {
    ...
    public void animateModel(T t, float f, float f2, float f3) {
        ...
        this.scaredAnimationProgress = ((PandaEntity)t).getScaredAnimationProgress(f3);
        this.lieOnBackAnimationProgress = ((PandaEntity)t).getLieOnBackAnimationProgress(f3);
        this.playAnimationProgress = ((PassiveEntity)t).isBaby() ? 0.0f : ((PandaEntity)t).getRollOverAnimationProgress(f3);
    }

    @Override
    public void setAngles(T t, float f, float f2, float f32, float f4, float f5) {
        float f32;
        super.setAngles(t, f, f2, f32, f4, f5);
        boolean bl = ((PandaEntity)t).getAskForBambooTicks() > 0;
        _snowman = ((PandaEntity)t).isSneezing();
        int _snowman2 = ((PandaEntity)t).getSneezeProgress();
        bl2 = ((PandaEntity)t).isEating();
        _snowman = ((PandaEntity)t).isScaredByThunderstorm();
        if (bl) {
            this.head.yaw = 0.35f * MathHelper.sin(0.6f * f32);
            this.head.roll = 0.35f * MathHelper.sin(0.6f * f32);
            this.frontRightLeg.pitch = -0.75f * MathHelper.sin(0.3f * f32);
            this.frontLeftLeg.pitch = 0.75f * MathHelper.sin(0.3f * f32);
        } else {
            this.head.roll = 0.0f;
        }
        if (_snowman) {
            if (_snowman2 < 15) {
                this.head.pitch = -0.7853982f * (float)_snowman2 / 14.0f;
            } else if (_snowman2 < 20) {
                float f6 = (_snowman2 - 15) / 5;
                this.head.pitch = -0.7853982f + 0.7853982f * f6;
            }
        }
        if (this.scaredAnimationProgress > 0.0f) {
            ...
            if (bl2) {
                this.head.pitch = 1.5707964f + 0.2f * MathHelper.sin(f32 * 0.6f);
                this.frontRightLeg.pitch = -0.4f - 0.2f * MathHelper.sin(f32 * 0.6f);
                this.frontLeftLeg.pitch = -0.4f - 0.2f * MathHelper.sin(f32 * 0.6f);
            }
            if (_snowman) {
        ...
        if (this.lieOnBackAnimationProgress > 0.0f) {
            this.backRightLeg.pitch = -0.6f * MathHelper.sin(f32 * 0.15f);
            this.backLeftLeg.pitch = 0.6f * MathHelper.sin(f32 * 0.15f);
            this.frontRightLeg.pitch = 0.3f * MathHelper.sin(f32 * 0.25f);
            this.frontLeftLeg.pitch = -0.3f * MathHelper.sin(f32 * 0.25f);
        ...
        if (this.playAnimationProgress > 0.0f) {
            ...
            this.backRightLeg.pitch = -0.5f * MathHelper.sin(f32 * 0.5f);
            this.backLeftLeg.pitch = 0.5f * MathHelper.sin(f32 * 0.5f);
            this.frontRightLeg.pitch = 0.5f * MathHelper.sin(f32 * 0.5f);
            this.frontLeftLeg.pitch = -0.5f * MathHelper.sin(f32 * 0.5f);
        }
    }
}
