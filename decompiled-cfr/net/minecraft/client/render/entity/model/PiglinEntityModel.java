...
extends PlayerEntityModel<T> {
    ...
    public void setAngles(T t2, float f, float f2, float f3, float f4, float f5) {
        ...
        super.setAngles(t2, f, f2, f3, f4, f5);
        _snowman = 0.5235988f;
        _snowman = f3 * 0.1f + f * 0.5f;
        _snowman = 0.08f + f2 * 0.4f;
        this.rightEar.roll = -0.5235988f - MathHelper.cos(_snowman * 1.2f) * _snowman;
        this.leftEar.roll = 0.5235988f + MathHelper.cos(_snowman) * _snowman;
        if (t2 instanceof AbstractPiglinEntity) {
            AbstractPiglinEntity abstractPiglinEntity = (AbstractPiglinEntity)t2;
            PiglinActivity _snowman2 = abstractPiglinEntity.getActivity();
            if (_snowman2 == PiglinActivity.DANCING) {
                float f6 = f3 / 60.0f;
                this.leftEar.roll = 0.5235988f + (float)Math.PI / 180 * MathHelper.sin(f6 * 30.0f) * 10.0f;
                this.rightEar.roll = -0.5235988f - (float)Math.PI / 180 * MathHelper.cos(f6 * 30.0f) * 10.0f;
                this.head.pivotX = MathHelper.sin(f6 * 10.0f);
                this.head.pivotY = MathHelper.sin(f6 * 40.0f) + 0.4f;
                this.rightArm.roll = (float)Math.PI / 180 * (70.0f + MathHelper.cos(f6 * 40.0f) * 10.0f);
                this.leftArm.roll = this.rightArm.roll * -1.0f;
                this.rightArm.pivotY = MathHelper.sin(f6 * 40.0f) * 0.5f + 1.5f;
                this.leftArm.pivotY = MathHelper.sin(f6 * 40.0f) * 0.5f + 1.5f;
                this.torso.pivotY = MathHelper.sin(f6 * 40.0f) * 0.35f;
            } else if (_snowman2 == PiglinActivity.ATTACKING_WITH_MELEE_WEAPON && this.handSwingProgress == 0.0f) {
                this.method_29354(t2);
            } else if (_snowman2 == PiglinActivity.CROSSBOW_HOLD) {
                CrossbowPosing.hold(this.rightArm, this.leftArm, this.head, !((MobEntity)t2).isLeftHanded());
            } else if (_snowman2 == PiglinActivity.CROSSBOW_CHARGE) {
                CrossbowPosing.charge(this.rightArm, this.leftArm, t2, !((MobEntity)t2).isLeftHanded());
            } else if (_snowman2 == PiglinActivity.ADMIRING_ITEM) {
                this.head.pitch = 0.5f;
                this.head.yaw = 0.0f;
                if (((MobEntity)t2).isLeftHanded()) {
        ...
        } else if (((Entity)t2).getType() == EntityType.ZOMBIFIED_PIGLIN) {
            CrossbowPosing.method_29352(this.leftArm, this.rightArm, ((MobEntity)t2).isAttacking(), this.handSwingProgress, f3);
        }
    ...
    protected void method_29353(T t, float f) {
        if (this.handSwingProgress > 0.0f && t instanceof PiglinEntity && ((PiglinEntity)t).getActivity() == PiglinActivity.ATTACKING_WITH_MELEE_WEAPON) {
            CrossbowPosing.method_29351(this.rightArm, this.leftArm, t, this.handSwingProgress, f);
            return;
        }
