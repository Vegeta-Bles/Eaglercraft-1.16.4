...
extends CompositeEntityModel<T> {
    ...
    public void setAngles(StriderEntity striderEntity, float f, float f2, float f3, float f4, float f5) {
        ...
        if (striderEntity.getPassengerList().size() <= 0) {
            this.field_23355.pitch = f5 * ((float)Math.PI / 180);
            this.field_23355.yaw = f4 * ((float)Math.PI / 180);
        } else {
            this.field_23355.pitch = 0.0f;
            this.field_23355.yaw = 0.0f;
        }
        _snowman = 1.5f;
        this.field_23355.roll = 0.1f * MathHelper.sin(f * 1.5f) * 4.0f * f2;
        this.field_23355.pivotY = 2.0f;
        this.field_23355.pivotY -= 2.0f * MathHelper.cos(f * 1.5f) * 2.0f * f2;
        this.field_23354.pitch = MathHelper.sin(f * 1.5f * 0.5f) * 2.0f * f2;
        this.field_23353.pitch = MathHelper.sin(f * 1.5f * 0.5f + (float)Math.PI) * 2.0f * f2;
        this.field_23354.roll = 0.17453292f * MathHelper.cos(f * 1.5f * 0.5f) * f2;
        this.field_23353.roll = 0.17453292f * MathHelper.cos(f * 1.5f * 0.5f + (float)Math.PI) * f2;
        this.field_23354.pivotY = 8.0f + 2.0f * MathHelper.sin(f * 1.5f * 0.5f + (float)Math.PI) * 2.0f * f2;
        this.field_23353.pivotY = 8.0f + 2.0f * MathHelper.sin(f * 1.5f * 0.5f) * 2.0f * f2;
        ...
        _snowman = MathHelper.cos(f * 1.5f + (float)Math.PI) * f2;
        this.field_23356.roll += _snowman * 1.3f;
        this.field_23357.roll += _snowman * 1.2f;
        this.field_23358.roll += _snowman * 0.6f;
        this.field_23359.roll += _snowman * 0.6f;
        this.field_23360.roll += _snowman * 1.2f;
        this.field_23361.roll += _snowman * 1.3f;
        _snowman = 1.0f;
        _snowman = 1.0f;
        this.field_23356.roll += 0.05f * MathHelper.sin(f3 * 1.0f * -0.4f);
        this.field_23357.roll += 0.1f * MathHelper.sin(f3 * 1.0f * 0.2f);
        this.field_23358.roll += 0.1f * MathHelper.sin(f3 * 1.0f * 0.4f);
        this.field_23359.roll += 0.1f * MathHelper.sin(f3 * 1.0f * 0.4f);
        this.field_23360.roll += 0.1f * MathHelper.sin(f3 * 1.0f * 0.2f);
        this.field_23361.roll += 0.05f * MathHelper.sin(f3 * 1.0f * -0.4f);
    }

    public void method_26415(ModelPart modelPart, float f, float f2, float f3) {
        modelPart.pitch = f;
        modelPart.yaw = f2;
        modelPart.roll = f3;
    }
