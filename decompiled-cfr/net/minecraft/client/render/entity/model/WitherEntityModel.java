...
extends CompositeEntityModel<T> {
    ...
    public void setAngles(T t, float f, float f2, float f3, float f4, float f5) {
        _snowman = MathHelper.cos(f3 * 0.1f);
        this.bodySegments[1].pitch = (0.065f + 0.05f * _snowman) * (float)Math.PI;
        ...
        this.bodySegments[2].pitch = (0.265f + 0.1f * _snowman) * (float)Math.PI;
        this.heads[0].yaw = f4 * ((float)Math.PI / 180);
        this.heads[0].pitch = f5 * ((float)Math.PI / 180);
    }

    @Override
    public void animateModel(T t, float f, float f2, float f3) {
        for (int i = 1; i < 3; ++i) {
            this.heads[i].yaw = (((WitherEntity)t).getHeadYaw(i - 1) - ((WitherEntity)t).bodyYaw) * ((float)Math.PI / 180);
            this.heads[i].pitch = ((WitherEntity)t).getHeadPitch(i - 1) * ((float)Math.PI / 180);
        }
    }
