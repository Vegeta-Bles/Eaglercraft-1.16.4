...
extends MovingSoundInstance {
    ...
    public void tick() {
        ...
        float _snowman2 = MathHelper.sqrt(Entity.squaredHorizontalLength(this.bee.getVelocity()));
        if ((double)_snowman2 >= 0.01) {
            this.pitch = MathHelper.lerp(MathHelper.clamp(_snowman2, this.getMinPitch(), this.getMaxPitch()), this.getMinPitch(), this.getMaxPitch());
            this.volume = MathHelper.lerp(MathHelper.clamp(_snowman2, 0.0f, 0.5f), 0.0f, 1.2f);
        } else {
            this.pitch = 0.0f;
            this.volume = 0.0f;
        }
