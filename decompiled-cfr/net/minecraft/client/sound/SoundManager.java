...
extends SinglePreparationResourceReloadListener<SoundList> {
    ...
    @Override
    protected SoundList prepare(ResourceManager resourceManager, Profiler profiler2) {
        Profiler profiler2;
        SoundList soundList = new SoundList();
        profiler2.startTick();
        for (String string : resourceManager.getAllNamespaces()) {
            profiler2.push(string);
            try {
                ...
                for (Resource resource : list) {
                    ...
                    try (InputStream inputStream = resource.getInputStream();
                        ...
                        profiler2.push("parse");
                        Map<String, SoundEntry> map = JsonHelper.deserialize(GSON, (Reader)inputStreamReader, TYPE);
                        profiler2.swap("register");
    ...
    protected void apply(SoundList soundList, ResourceManager resourceManager, Profiler profiler) {
        ...
        for (Identifier identifier : this.sounds.keySet()) {
            ...
            if (!(weightedSoundSet.getSubtitle() instanceof TranslatableText) || I18n.hasTranslation(_snowman = ((TranslatableText)weightedSoundSet.getSubtitle()).getKey())) continue;
            LOGGER.debug("Missing subtitle {} for event: {}", (Object)_snowman, (Object)identifier);
        }
    ...
    public static class SoundList {
        ...
        private void register(Identifier id, SoundEntry entry, ResourceManager resourceManager) {
            ...
            boolean bl = _snowman = weightedSoundSet == null;
            if (_snowman || entry.canReplace()) {
                if (!_snowman) {
            ...
            block4: for (final Sound sound : entry.getSounds()) {
                SoundContainer<Sound> _snowman2;
                ...
                switch (sound.getRegistrationType()) {
                    ...
                    case SOUND_EVENT: {
                        _snowman2 = new SoundContainer<Sound>(){
                            ...
                            public Sound getSound() {
                                ...
                                Sound _snowman2 = weightedSoundSet.getSound();
                                return new Sound(_snowman2.getIdentifier().toString(), _snowman2.getVolume() * sound.getVolume(), _snowman2.getPitch() * sound.getPitch(), sound.getWeight(), Sound.RegistrationType.FILE, _snowman2.isStreamed() || sound.isStreamed(), _snowman2.isPreloaded(), _snowman2.getAttenuation());
                            }
                ...
                weightedSoundSet.add(_snowman2);
            }
        }
