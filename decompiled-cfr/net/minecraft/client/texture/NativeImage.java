...
implements AutoCloseable {
    ...
    public static NativeImage read(@Nullable Format format, ByteBuffer byteBuffer) throws IOException {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ...
            _snowman = memoryStack.mallocInt(1);
            ByteBuffer _snowman2 = STBImage.stbi_load_from_memory((ByteBuffer)byteBuffer, (IntBuffer)intBuffer, (IntBuffer)_snowman, (IntBuffer)_snowman, (int)(format == null ? 0 : format.channelCount));
            if (_snowman2 == null) {
                throw new IOException("Could not load image: " + STBImage.stbi_failure_reason());
            }
            NativeImage nativeImage = new NativeImage(format == null ? Format.getFormat(_snowman.get(0)) : format, intBuffer.get(0), _snowman.get(0), true, MemoryUtil.memAddress((ByteBuffer)_snowman2));
            return nativeImage;
        }
    }
    ...
    public byte getPixelOpacity(int x, int y) {
        ...
        int n = (x + y * this.width) * this.format.getChannelCount() + this.format.getOpacityOffset() / 8;
        return MemoryUtil.memGetByte((long)(this.pointer + (long)n));
    }
    ...
    public int[] makePixelArray() {
        ...
        for (int i = 0; i < this.getHeight(); ++i) {
            for (_snowman = 0; _snowman < this.getWidth(); ++_snowman) {
                _snowman = this.getPixelColor(_snowman, i);
                _snowman = NativeImage.getAlpha(_snowman);
                _snowman = NativeImage.getBlue(_snowman);
                _snowman = NativeImage.getGreen(_snowman);
                _snowman = NativeImage.getRed(_snowman);
                nArray[_snowman + i * this.getWidth()] = _snowman = _snowman << 24 | _snowman << 16 | _snowman << 8 | _snowman;
            }
        }
        return nArray;
    }
    ...
    public void loadFromTextureImage(int level, boolean removeAlpha) {
        ...
        if (removeAlpha && this.format.hasAlphaChannel()) {
            for (int i = 0; i < this.getHeight(); ++i) {
                for (_snowman = 0; _snowman < this.getWidth(); ++_snowman) {
                    this.setPixelColor(_snowman, i, this.getPixelColor(_snowman, i) | 255 << this.format.getAlphaChannelOffset());
                }
            }
        }
    }
    ...
    public void writeFile(Path path) throws IOException {
        ...
        this.checkAllocated();
        try (SeekableByteChannel seekableByteChannel = Files.newByteChannel(path, WRITE_TO_FILE_OPEN_OPTIONS, new FileAttribute[0]);){
    ...
    private boolean write(WritableByteChannel writableByteChannel) throws IOException {
        ...
        try {
            int n = Math.min(this.getHeight(), Integer.MAX_VALUE / this.getWidth() / this.format.getChannelCount());
            if (n < this.getHeight()) {
                LOGGER.warn("Dropping image height from {} to {} to fit the size into 32-bit signed int", (Object)this.getHeight(), (Object)n);
            }
            if (STBImageWrite.nstbi_write_png_to_func((long)writeCallback.address(), (long)0L, (int)this.getWidth(), (int)n, (int)this.format.getChannelCount(), (long)this.pointer, (int)0) == 0) {
                boolean bl = false;
                return bl;
            }
    ...
    public void copyFrom(NativeImage image) {
        ...
        int n = this.format.getChannelCount();
        this.checkAllocated();
        ...
        if (this.width == image.width) {
            ...
            _snowman = Math.min(this.getWidth(), image.getWidth());
            _snowman = Math.min(this.getHeight(), image.getHeight());
            for (_snowman = 0; _snowman < _snowman; ++_snowman) {
                _snowman = _snowman * image.getWidth() * n;
                _snowman = _snowman * this.getWidth() * n;
                MemoryUtil.memCopy((long)(image.pointer + (long)_snowman), (long)(this.pointer + (long)_snowman), (long)_snowman);
            }
        }
    }
    ...
    public void fillRect(int x, int y, int width, int height, int color) {
        for (int i = y; i < y + height; ++i) {
            for (_snowman = x; _snowman < x + width; ++_snowman) {
                this.setPixelColor(_snowman, i, color);
            }
        }
    }
    ...
    public void copyRect(int x, int y, int translateX, int translateY, int width, int height, boolean flipX, boolean flipY) {
        for (int i = 0; i < height; ++i) {
            for (_snowman = 0; _snowman < width; ++_snowman) {
                _snowman = flipX ? width - 1 - _snowman : _snowman;
                _snowman = flipY ? height - 1 - i : i;
                _snowman = this.getPixelColor(x + _snowman, y + i);
                this.setPixelColor(x + translateX + _snowman, y + translateY + _snowman, _snowman);
            }
        }
    }
    ...
    public void mirrorVertically() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            int n = this.format.getChannelCount();
            _snowman = this.getWidth() * n;
            long _snowman2 = memoryStack.nmalloc(_snowman);
            for (_snowman = 0; _snowman < this.getHeight() / 2; ++_snowman) {
                _snowman = _snowman * this.getWidth() * n;
                _snowman = (this.getHeight() - 1 - _snowman) * this.getWidth() * n;
                MemoryUtil.memCopy((long)(this.pointer + (long)_snowman), (long)_snowman2, (long)_snowman);
                MemoryUtil.memCopy((long)(this.pointer + (long)_snowman), (long)(this.pointer + (long)_snowman), (long)_snowman);
                MemoryUtil.memCopy((long)_snowman2, (long)(this.pointer + (long)_snowman), (long)_snowman);
            }
        }
    }
    ...
    public void resizeSubRectTo(int x, int y, int width, int height, NativeImage targetImage) {
        ...
        int n = this.format.getChannelCount();
        STBImageResize.nstbir_resize_uint8((long)(this.pointer + (long)((x + y * this.getWidth()) * n)), (int)width, (int)height, (int)(this.getWidth() * n), (long)targetImage.pointer, (int)targetImage.getWidth(), (int)targetImage.getHeight(), (int)0, (int)n);
    }
    ...
    public static NativeImage read(String dataUri) throws IOException {
        byte[] byArray = Base64.getDecoder().decode(dataUri.replaceAll("\n", "").getBytes(Charsets.UTF_8));
        try (MemoryStack _snowman2 = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = _snowman2.malloc(byArray.length);
