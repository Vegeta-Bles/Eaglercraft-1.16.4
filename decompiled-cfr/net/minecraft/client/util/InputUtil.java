...
public class InputUtil {
    ...
    public static void setCursorParameters(long handler, int n, double d, double d2) {
        GLFW.glfwSetCursorPos((long)handler, (double)d, (double)d2);
        GLFW.glfwSetInputMode((long)handler, (int)208897, (int)n);
    }
    ...
    static {
        ...
        MethodType _snowman2 = MethodType.methodType(Boolean.TYPE);
        MethodHandle _snowman3 = null;
        int _snowman4 = 0;
        try {
            _snowman3 = lookup.findStatic(GLFW.class, "glfwRawMouseMotionSupported", _snowman2);
            MethodHandle methodHandle = lookup.findStaticGetter(GLFW.class, "GLFW_RAW_MOUSE_MOTION", Integer.TYPE);
            _snowman4 = methodHandle.invokeExact();
        }
        ...
        GLFW_RAW_MOUSE_MOTION_SUPPORTED_HANDLE = _snowman3;
        GLFW_RAW_MOUSE_MOTION = _snowman4;
    ...
    public static enum Type {
        KEYSYM("key.keyboard", (n, string) -> {
            _snowman = GLFW.glfwGetKeyName((int)n, (int)-1);
            return _snowman != null ? new LiteralText(_snowman) : new TranslatableText((String)string);
        }),
        SCANCODE("scancode", (n, string) -> {
            _snowman = GLFW.glfwGetKeyName((int)-1, (int)n);
            return _snowman != null ? new LiteralText(_snowman) : new TranslatableText((String)string);
        }),
        MOUSE("key.mouse", (n, string) -> Language.getInstance().hasTranslation((String)string) ? new TranslatableText((String)string) : new TranslatableText("key.mouse", n + 1));
        ...
        public Key createFromCode(int code2) {
            return (Key)this.map.computeIfAbsent(code2, code -> {
                ...
                String _snowman2 = this.name + "." + n;
                return new Key(_snowman2, this, code);
            });
        }
