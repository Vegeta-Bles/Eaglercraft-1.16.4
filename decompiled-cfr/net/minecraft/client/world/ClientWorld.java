...
extends World {
    ...
    public void tickEntities() {
        ...
        ObjectIterator _snowman2 = this.regularEntities.int2ObjectEntrySet().iterator();
        while (_snowman2.hasNext()) {
            Int2ObjectMap.Entry entry = (Int2ObjectMap.Entry)_snowman2.next();
            Entity _snowman3 = (Entity)entry.getValue();
            if (_snowman3.hasVehicle()) continue;
            profiler.push("tick");
            if (!_snowman3.removed) {
                this.tickEntity(this::tickEntity, _snowman3);
            }
            profiler.pop();
            profiler.push("remove");
            if (_snowman3.removed) {
                _snowman2.remove();
                this.finishRemovingEntity(_snowman3);
            }
            profiler.pop();
        }
    ...
    public void resetChunkColor(int n, int n2) {
        this.colorCache.forEach((colorResolver, biomeColorCache) -> biomeColorCache.reset(n, n2));
    }
    ...
    public void addEntitiesToChunk(WorldChunk chunk) {
        for (Int2ObjectMap.Entry entry : this.regularEntities.int2ObjectEntrySet()) {
            ...
            int _snowman2 = MathHelper.floor(entity.getX() / 16.0);
            int _snowman3 = MathHelper.floor(entity.getZ() / 16.0);
            if (_snowman2 != chunk.getPos().x || _snowman3 != chunk.getPos().z) continue;
    ...
    public void doRandomBlockDisplayTicks(int xCenter, int yCenter, int zCenter) {
        ...
        for (int i = 0; i < 667; ++i) {
            this.randomBlockDisplayTick(xCenter, yCenter, zCenter, 16, _snowman2, _snowman3, mutable);
            this.randomBlockDisplayTick(xCenter, yCenter, zCenter, 32, _snowman2, _snowman3, mutable);
        }
    }
    ...
    public void randomBlockDisplayTick(int xCenter, int yCenter, int zCenter, int radius, Random random, boolean spawnBarrierParticles, BlockPos.Mutable pos) {
        int n = xCenter + this.random.nextInt(radius) - this.random.nextInt(radius);
        _snowman = yCenter + this.random.nextInt(radius) - this.random.nextInt(radius);
        _snowman = zCenter + this.random.nextInt(radius) - this.random.nextInt(radius);
        pos.set(n, _snowman, _snowman);
        BlockState _snowman2 = this.getBlockState(pos);
        _snowman2.getBlock().randomDisplayTick(_snowman2, this, pos, random);
        FluidState _snowman3 = this.getFluidState(pos);
        if (!_snowman3.isEmpty()) {
            _snowman3.randomDisplayTick(this, pos, random);
            ParticleEffect particleEffect = _snowman3.getParticle();
            if (particleEffect != null && this.random.nextInt(10) == 0) {
                boolean bl = _snowman2.isSideSolidFullSquare(this, pos, Direction.DOWN);
                Vec3i _snowman4 = pos.down();
                this.addParticle((BlockPos)_snowman4, this.getBlockState((BlockPos)_snowman4), particleEffect, bl);
            }
        }
        if (spawnBarrierParticles && _snowman2.isOf(Blocks.BARRIER)) {
            this.addParticle(ParticleTypes.BARRIER, (double)n + 0.5, (double)_snowman + 0.5, (double)_snowman + 0.5, 0.0, 0.0, 0.0);
        }
        if (!_snowman2.isFullCube(this, pos)) {
    ...
    private void addParticle(BlockPos pos, BlockState state, ParticleEffect parameters, boolean bl) {
        ...
        double _snowman2 = voxelShape.getMax(Direction.Axis.Y);
        if (_snowman2 < 1.0) {
            if (bl) {
            ...
            double d = voxelShape.getMin(Direction.Axis.Y);
            if (d > 0.0) {
                this.addParticle(pos, parameters, voxelShape, (double)pos.getY() + d - 0.05);
            } else {
                BlockPos blockPos = pos.down();
                BlockState _snowman3 = this.getBlockState(blockPos);
                VoxelShape _snowman4 = _snowman3.getCollisionShape(this, blockPos);
                double _snowman5 = _snowman4.getMax(Direction.Axis.Y);
                if (_snowman5 < 1.0 && _snowman3.getFluidState().isEmpty()) {
    ...
    public void finishRemovingEntities() {
        ...
        while (objectIterator.hasNext()) {
            ...
            Entity _snowman2 = (Entity)entry.getValue();
            if (!_snowman2.removed) continue;
            objectIterator.remove();
            this.finishRemovingEntity(_snowman2);
        }
    }
    ...
    public void playSound(double x, double y, double z, SoundEvent sound, SoundCategory category, float volume, float pitch, boolean bl) {
        ...
        PositionedSoundInstance _snowman2 = new PositionedSoundInstance(sound, category, volume, pitch, x, y, z);
        if (bl && d > 100.0) {
            _snowman = Math.sqrt(d) / 40.0;
            this.client.getSoundManager().play(_snowman2, (int)(_snowman * 20.0));
        } else {
    ...
    public void syncWorldEvent(@Nullable PlayerEntity player, int eventId, BlockPos pos, int data) {
        try {
            ...
            CrashReportSection _snowman2 = crashReport.addElement("Level event being played");
            _snowman2.add("Block coordinates", CrashReportSection.createPositionString(pos));
            _snowman2.add("Event source", player);
            _snowman2.add("Event type", eventId);
            _snowman2.add("Event data", data);
    ...
    public float method_23783(float f) {
        _snowman = this.getSkyAngle(f);
        _snowman = 1.0f - (MathHelper.cos(_snowman * ((float)Math.PI * 2)) * 2.0f + 0.2f);
        _snowman = MathHelper.clamp(_snowman, 0.0f, 1.0f);
        _snowman = 1.0f - _snowman;
        _snowman = (float)((double)_snowman * (1.0 - (double)(this.getRainGradient(f) * 5.0f) / 16.0));
        _snowman = (float)((double)_snowman * (1.0 - (double)(this.getThunderGradient(f) * 5.0f) / 16.0));
        return _snowman * 0.8f + 0.2f;
    }

    public Vec3d method_23777(BlockPos blockPos, float f) {
        float f2;
        _snowman = this.getSkyAngle(f);
        _snowman = MathHelper.cos(_snowman * ((float)Math.PI * 2)) * 2.0f + 0.5f;
        _snowman = MathHelper.clamp(_snowman, 0.0f, 1.0f);
        Biome biome = this.getBiome(blockPos);
        int _snowman2 = biome.getSkyColor();
        ...
        _snowman4 *= _snowman;
        _snowman5 *= _snowman;
        float _snowman6 = this.getRainGradient(f);
        if (_snowman6 > 0.0f) {
            f2 = (_snowman3 * 0.3f + _snowman4 * 0.59f + _snowman5 * 0.11f) * 0.6f;
            _snowman = 1.0f - _snowman6 * 0.75f;
            _snowman3 = _snowman3 * _snowman + f2 * (1.0f - _snowman);
            _snowman4 = _snowman4 * _snowman + f2 * (1.0f - _snowman);
            _snowman5 = _snowman5 * _snowman + f2 * (1.0f - _snowman);
        }
        if ((f2 = this.getThunderGradient(f)) > 0.0f) {
            _snowman = (_snowman3 * 0.3f + _snowman4 * 0.59f + _snowman5 * 0.11f) * 0.2f;
            _snowman = 1.0f - f2 * 0.75f;
            _snowman3 = _snowman3 * _snowman + _snowman * (1.0f - _snowman);
            _snowman4 = _snowman4 * _snowman + _snowman * (1.0f - _snowman);
            _snowman5 = _snowman5 * _snowman + _snowman * (1.0f - _snowman);
        }
        if (this.lightningTicksLeft > 0) {
            _snowman = (float)this.lightningTicksLeft - f;
            if (_snowman > 1.0f) {
                _snowman = 1.0f;
            }
            _snowman3 = _snowman3 * (1.0f - (_snowman *= 0.45f)) + 0.8f * _snowman;
            _snowman4 = _snowman4 * (1.0f - _snowman) + 0.8f * _snowman;
            _snowman5 = _snowman5 * (1.0f - _snowman) + 1.0f * _snowman;
        }
    ...
    public Vec3d getCloudsColor(float tickDelta) {
        ...
        _snowman = MathHelper.cos(f * ((float)Math.PI * 2)) * 2.0f + 0.5f;
        _snowman = MathHelper.clamp(_snowman, 0.0f, 1.0f);
        _snowman = 1.0f;
        _snowman = 1.0f;
        _snowman = 1.0f;
        _snowman = this.getRainGradient(tickDelta);
        if (_snowman > 0.0f) {
            _snowman = (_snowman * 0.3f + _snowman * 0.59f + _snowman * 0.11f) * 0.6f;
            _snowman = 1.0f - _snowman * 0.95f;
            _snowman = _snowman * _snowman + _snowman * (1.0f - _snowman);
            _snowman = _snowman * _snowman + _snowman * (1.0f - _snowman);
            _snowman = _snowman * _snowman + _snowman * (1.0f - _snowman);
        }
        _snowman *= _snowman * 0.9f + 0.1f;
        _snowman *= _snowman * 0.9f + 0.1f;
        _snowman *= _snowman * 0.85f + 0.15f;
        _snowman = this.getThunderGradient(tickDelta);
        if (_snowman > 0.0f) {
            _snowman = (_snowman * 0.3f + _snowman * 0.59f + _snowman * 0.11f) * 0.2f;
            _snowman = 1.0f - _snowman * 0.95f;
            _snowman = _snowman * _snowman + _snowman * (1.0f - _snowman);
            _snowman = _snowman * _snowman + _snowman * (1.0f - _snowman);
            _snowman = _snowman * _snowman + _snowman * (1.0f - _snowman);
        }
        return new Vec3d(_snowman, _snowman, _snowman);
    }

    public float method_23787(float f) {
        _snowman = this.getSkyAngle(f);
        _snowman = 1.0f - (MathHelper.cos(_snowman * ((float)Math.PI * 2)) * 2.0f + 0.25f);
        _snowman = MathHelper.clamp(_snowman, 0.0f, 1.0f);
        return _snowman * _snowman * 0.5f;
    }
    ...
    public int calculateColor(BlockPos pos, ColorResolver colorResolver) {
        int n = MinecraftClient.getInstance().options.biomeBlendRadius;
        if (n == 0) {
            return colorResolver.getColor(this.getBiome(pos), pos.getX(), pos.getZ());
        }
        _snowman = (n * 2 + 1) * (n * 2 + 1);
        _snowman = 0;
        _snowman = 0;
        _snowman = 0;
        CuboidBlockIterator _snowman2 = new CuboidBlockIterator(pos.getX() - n, pos.getY(), pos.getZ() - n, pos.getX() + n, pos.getY(), pos.getZ() + n);
        BlockPos.Mutable _snowman3 = new BlockPos.Mutable();
        while (_snowman2.step()) {
            _snowman3.set(_snowman2.getX(), _snowman2.getY(), _snowman2.getZ());
            _snowman = colorResolver.getColor(this.getBiome(_snowman3), _snowman3.getX(), _snowman3.getZ());
            _snowman += (_snowman & 0xFF0000) >> 16;
            _snowman += (_snowman & 0xFF00) >> 8;
            _snowman += _snowman & 0xFF;
        }
        return (_snowman / _snowman & 0xFF) << 16 | (_snowman / _snowman & 0xFF) << 8 | _snowman / _snowman & 0xFF;
    }
