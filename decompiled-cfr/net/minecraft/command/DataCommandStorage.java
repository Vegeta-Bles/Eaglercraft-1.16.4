...
public class DataCommandStorage {
    ...
    public CompoundTag get(Identifier id) {
        ...
        PersistentState _snowman2 = this.stateManager.get(() -> this.method_22549(string, _snowman = DataCommandStorage.getSaveKey(string)), _snowman);
        return _snowman2 != null ? _snowman2.get(id.getPath()) : new CompoundTag();
    }

    public void set(Identifier id, CompoundTag tag) {
        String string = id.getNamespace();
        _snowman = DataCommandStorage.getSaveKey(string);
        this.stateManager.getOrCreate(() -> this.createStorage(string, _snowman), _snowman).set(id.getPath(), tag);
    }
