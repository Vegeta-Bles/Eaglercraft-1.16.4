...
implements ArgumentType<ItemPredicateArgument> {
    ...
    public ItemPredicateArgument parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        Identifier _snowman2 = itemStringReader.getId();
        return commandContext -> {
            Tag<Item> tag = ((ServerCommandSource)commandContext.getSource()).getMinecraftServer().getTagManager().getItems().getTag(_snowman2);
            if (tag == null) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        ItemStringReader _snowman2 = new ItemStringReader(stringReader, true);
        try {
            _snowman2.consume();
        }
