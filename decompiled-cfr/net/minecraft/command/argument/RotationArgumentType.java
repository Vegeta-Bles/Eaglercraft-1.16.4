...
implements ArgumentType<PosArgument> {
    ...
    public PosArgument parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        CoordinateArgument _snowman2 = CoordinateArgument.parse(stringReader, false);
        if (!stringReader.canRead() || stringReader.peek() != ' ') {
            stringReader.setCursor(n);
            throw INCOMPLETE_ROTATION_EXCEPTION.createWithContext((ImmutableStringReader)stringReader);
        }
        stringReader.skip();
        CoordinateArgument _snowman3 = CoordinateArgument.parse(stringReader, false);
        return new DefaultPosArgument(_snowman3, _snowman2, new CoordinateArgument(true, 0.0));
    }
