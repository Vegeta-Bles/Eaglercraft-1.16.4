...
implements ArgumentType<TestFunction> {
    ...
    public TestFunction parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        Optional<TestFunction> _snowman2 = TestFunctions.getTestFunction(string);
        if (_snowman2.isPresent()) {
            return _snowman2.get();
        }
        LiteralText _snowman3 = new LiteralText("No such test: " + string);
        throw new CommandSyntaxException((CommandExceptionType)new SimpleCommandExceptionType((Message)_snowman3), (Message)_snowman3);
    }
