...
implements ArgumentType<UUID> {
    ...
    public UUID parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        Matcher _snowman2 = VALID_CHARACTERS.matcher(string);
        if (_snowman2.find()) {
            _snowman = _snowman2.group(1);
            try {
                UUID uUID = UUID.fromString(_snowman);
                stringReader.setCursor(stringReader.getCursor() + _snowman.length());
                return uUID;
            }
