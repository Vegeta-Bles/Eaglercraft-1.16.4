...
implements ArgumentType<PosArgument> {
    ...
    public PosArgument parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        CoordinateArgument _snowman2 = CoordinateArgument.parse(stringReader, this.centerIntegers);
        if (!stringReader.canRead() || stringReader.peek() != ' ') {
            stringReader.setCursor(n);
            throw INCOMPLETE_EXCEPTION.createWithContext((ImmutableStringReader)stringReader);
        }
        stringReader.skip();
        CoordinateArgument _snowman3 = CoordinateArgument.parse(stringReader, this.centerIntegers);
        return new DefaultPosArgument(_snowman2, new CoordinateArgument(true, 0.0), _snowman3);
    }

    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        if (context.getSource() instanceof CommandSource) {
            String string = builder.getRemaining();
            Collection<CommandSource.RelativePosition> _snowman2 = !string.isEmpty() && string.charAt(0) == '^' ? Collections.singleton(CommandSource.RelativePosition.ZERO_LOCAL) : ((CommandSource)context.getSource()).getPositionSuggestions();
            return CommandSource.suggestColumnPositions(string, _snowman2, builder, CommandManager.getCommandValidator(this::parse));
        }
