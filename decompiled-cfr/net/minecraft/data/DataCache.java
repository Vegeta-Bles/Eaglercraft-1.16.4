...
public class DataCache {
    ...
    public DataCache(Path path2, String string2) throws IOException {
        ...
        this.recordFile = path3.resolve(string2);
        ...
        if (Files.isReadable(this.recordFile)) {
            IOUtils.readLines((InputStream)Files.newInputStream(this.recordFile, new OpenOption[0]), (Charset)Charsets.UTF_8).forEach(string -> {
                ...
                this.oldSha1.put(path2.resolve(string.substring(n + 1)), string.substring(0, n));
            });
        }
    }
    ...
    public void write() throws IOException {
        this.deleteAll();
        try {
            BufferedWriter bufferedWriter = Files.newBufferedWriter(this.recordFile, new OpenOption[0]);
        }
        catch (IOException iOException) {
            LOGGER.warn("Unable write cachefile {}: {}", (Object)this.recordFile, (Object)iOException.toString());
            return;
        }
        IOUtils.writeLines((Collection)this.newSha1.entrySet().stream().map(entry -> (String)entry.getValue() + ' ' + this.root.relativize((Path)entry.getKey())).collect(Collectors.toList()), (String)System.lineSeparator(), (Writer)bufferedWriter);
