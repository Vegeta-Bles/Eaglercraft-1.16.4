...
extends DataFix {
    ...
    protected Typed<?> updateTyped(Typed<?> typed2, String name, Function<Dynamic<?>, Dynamic<?>> updater) {
        ...
        _snowman = this.getOutputSchema().getChoiceType(this.typeReference, name);
        return typed2.updateTyped(DSL.namedChoice((String)name, (Type)type), _snowman, typed -> typed.update(DSL.remainderFinder(), updater));
    }
    ...
    protected static Optional<Dynamic<?>> updateRegularMostLeast(Dynamic<?> dynamic, String oldKey, String newKey) {
        ...
        _snowman = oldKey + "Least";
        return AbstractUuidFix.createArrayFromMostLeastTags(dynamic, string, _snowman).map(dynamic2 -> dynamic.remove(string).remove(_snowman).set(newKey, dynamic2));
    }
    ...
    protected static Optional<Dynamic<?>> createArrayFromMostLeastTags(Dynamic<?> dynamic, String mostBitsKey, String leastBitsKey) {
        ...
        _snowman = dynamic.get(leastBitsKey).asLong(0L);
        if (l == 0L || _snowman == 0L) {
            return Optional.empty();
        }
        return AbstractUuidFix.createArray(dynamic, l, _snowman);
    }
