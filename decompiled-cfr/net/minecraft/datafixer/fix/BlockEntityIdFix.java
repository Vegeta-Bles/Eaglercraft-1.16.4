...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        _snowman = this.getOutputSchema().getType(TypeReferences.ITEM_STACK);
        TaggedChoice.TaggedChoiceType _snowman2 = this.getInputSchema().findChoiceType(TypeReferences.BLOCK_ENTITY);
        TaggedChoice.TaggedChoiceType _snowman3 = this.getOutputSchema().findChoiceType(TypeReferences.BLOCK_ENTITY);
        return TypeRewriteRule.seq((TypeRewriteRule)this.convertUnchecked("item stack block entity name hook converter", type, _snowman), (TypeRewriteRule)this.fixTypeEverywhere("BlockEntityIdFix", (Type)_snowman2, (Type)_snowman3, dynamicOps -> pair -> pair.mapFirst(string -> RENAMED_BLOCK_ENTITIES.getOrDefault(string, (String)string))));
    }
}
