...
extends DataFix {
    ...
    public Dynamic<?> fixHealth(Dynamic<?> _snowman22) {
        Optional optional;
        Dynamic _snowman22;
        float f;
        Optional optional2 = _snowman22.get("HealF").asNumber().result();
        optional = _snowman22.get("Health").asNumber().result();
        if (optional2.isPresent()) {
            f = ((Number)optional2.get()).floatValue();
            _snowman22 = _snowman22.remove("HealF");
        } else if (optional.isPresent()) {
            f = ((Number)optional.get()).floatValue();
        } else {
            return _snowman22;
        }
        return _snowman22.set("Health", _snowman22.createFloat(f));
    }
