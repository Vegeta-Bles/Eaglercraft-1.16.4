...
extends DataFix {
    private static final Map<String, String> RENAMED_ENTITIES = (Map)DataFixUtils.make((Object)Maps.newHashMap(), hashMap -> {
    ...
    public TypeRewriteRule makeRule() {
        ...
        Type _snowman2 = this.getInputSchema().getType(TypeReferences.ITEM_STACK);
        Type _snowman3 = this.getOutputSchema().getType(TypeReferences.ITEM_STACK);
        return TypeRewriteRule.seq((TypeRewriteRule)this.convertUnchecked("item stack entity name hook converter", _snowman2, _snowman3), (TypeRewriteRule)this.fixTypeEverywhere("EntityIdFix", (Type)taggedChoiceType, (Type)_snowman, dynamicOps -> pair -> pair.mapFirst(string -> RENAMED_ENTITIES.getOrDefault(string, (String)string))));
    }
}
