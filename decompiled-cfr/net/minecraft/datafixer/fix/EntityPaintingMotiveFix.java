...
extends ChoiceFix {
    ...
    public Dynamic<?> renameMotive(Dynamic<?> dynamic2) {
        Dynamic<?> dynamic2;
        Optional optional = dynamic2.get("Motive").asString().result();
        if (optional.isPresent()) {
            String string = ((String)optional.get()).toLowerCase(Locale.ROOT);
            return dynamic2.set("Motive", dynamic2.createString(new Identifier(RENAMED_MOTIVES.getOrDefault(string, string)).toString()));
        }
        return dynamic2;
    }
