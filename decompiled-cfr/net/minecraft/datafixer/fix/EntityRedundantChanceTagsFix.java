...
extends DataFix {
    ...
    private static boolean method_30073(OptionalDynamic<?> optionalDynamic, int n) {
        return optionalDynamic.flatMap(arg_0 -> field_25695.parse(arg_0)).map(list -> list.size() == n && list.stream().allMatch(f -> f.floatValue() == 0.0f)).result().orElse(false);
    }
}
