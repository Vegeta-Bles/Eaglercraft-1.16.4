...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ItemStackEnchantmentFix", type, typed2 -> typed2.updateTyped(_snowman2, typed -> typed.update(DSL.remainderFinder(), this::fixEnchantments)));
    }

    private Dynamic<?> fixEnchantments(Dynamic<?> dynamic22) {
        Dynamic dynamic22;
        Optional optional = dynamic22.get("ench").asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)ID_TO_ENCHANTMENTS_MAP.getOrDefault(dynamic.get("id").asInt(0), (Object)"null"))))).map(arg_0 -> dynamic22.createList(arg_0)).result();
        if (optional.isPresent()) {
            dynamic22 = dynamic22.remove("ench").set("Enchantments", (Dynamic)optional.get());
        }
        return dynamic22.update("StoredEnchantments", dynamic -> (Dynamic)DataFixUtils.orElse((Optional)dynamic.asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)ID_TO_ENCHANTMENTS_MAP.getOrDefault(dynamic.get("id").asInt(0), (Object)"null"))))).map(arg_0 -> ((Dynamic)dynamic).createList(arg_0)).result(), (Object)dynamic));
    }
}
