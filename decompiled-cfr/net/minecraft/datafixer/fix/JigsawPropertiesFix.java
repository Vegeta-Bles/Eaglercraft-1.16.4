...
extends ChoiceFix {
    ...
    private static Dynamic<?> renameProperties(Dynamic<?> dynamic) {
        ...
        _snowman = dynamic.get("target_pool").asString("minecraft:empty");
        return dynamic.set("name", dynamic.createString(string)).set("target", dynamic.createString(string)).remove("attachement_type").set("pool", dynamic.createString(_snowman)).remove("target_pool");
    }
