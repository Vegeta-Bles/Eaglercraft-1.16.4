...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        OpticFinder _snowman2 = compoundListType.finder();
    ...
    private <SF> TypeRewriteRule method_17334(CompoundList.CompoundListType<String, SF> compoundListType) {
        ...
        _snowman = this.getInputSchema().getType(TypeReferences.STRUCTURE_FEATURE);
        OpticFinder _snowman2 = type.findField("Level");
        OpticFinder _snowman3 = _snowman2.type().findField("Structures");
        OpticFinder _snowman4 = _snowman3.type().findField("Starts");
        OpticFinder _snowman5 = compoundListType.finder();
        return TypeRewriteRule.seq((TypeRewriteRule)this.fixTypeEverywhereTyped("NewVillageFix", type, typed2 -> typed2.updateTyped(_snowman2, typed -> typed.updateTyped(_snowman3, typed2 -> typed2.updateTyped(_snowman4, typed -> typed.update(_snowman5, list -> list.stream().filter(pair -> !Objects.equals(pair.getFirst(), "Village")).map(pair -> pair.mapFirst(string -> string.equals("New_Village") ? "Village" : string)).collect(Collectors.toList()))).update(DSL.remainderFinder(), dynamic2 -> dynamic2.update("References", dynamic -> {
        ...
        }))))), (TypeRewriteRule)this.fixTypeEverywhereTyped("NewVillageStartFix", _snowman, typed -> typed.update(DSL.remainderFinder(), dynamic2 -> dynamic2.update("id", dynamic -> Objects.equals(IdentifierNormalizingSchema.normalize(dynamic.asString("")), "minecraft:new_village") ? dynamic.createString("minecraft:village") : dynamic))));
    }
}
