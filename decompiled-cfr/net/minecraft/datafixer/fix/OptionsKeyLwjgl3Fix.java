    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(TypeReferences.OPTIONS), typed -> typed.update(DSL.remainderFinder(), dynamic -> dynamic.getMapValues().map(map -> dynamic.createMap(map.entrySet().stream().map(entry -> {
            if (((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
                ...
                if (n < 0) {
                    _snowman = n + 100;
                    String string = _snowman == 0 ? "key.mouse.left" : (_snowman == 1 ? "key.mouse.right" : (_snowman == 2 ? "key.mouse.middle" : "key.mouse." + (_snowman + 1)));
                    return Pair.of(entry.getKey(), (Object)((Dynamic)entry.getValue()).createString(string));
                }
                String _snowman2 = (String)NUMERICAL_KEY_IDS_TO_KEY_NAMES.getOrDefault(n, (Object)"key.unknown");
                return Pair.of(entry.getKey(), (Object)((Dynamic)entry.getValue()).createString(_snowman2));
            }
