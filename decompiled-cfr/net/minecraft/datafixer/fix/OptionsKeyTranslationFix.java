...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("OptionsKeyTranslationFix", this.getInputSchema().getType(TypeReferences.OPTIONS), typed -> typed.update(DSL.remainderFinder(), dynamic -> dynamic.getMapValues().map(map -> dynamic.createMap(map.entrySet().stream().map(entry -> {
            if (((Dynamic)entry.getKey()).asString("").startsWith("key_") && !(_snowman = ((Dynamic)entry.getValue()).asString("")).startsWith("key.mouse") && !_snowman.startsWith("scancode.")) {
                return Pair.of(entry.getKey(), (Object)dynamic.createString("key.keyboard." + _snowman.substring("key.".length())));
            }
