...
extends DataFix {
    ...
    private static <T> Dynamic<T> updateWallValueReference(Dynamic<T> dynamic2, String string) {
        return dynamic2.update(string, dynamic -> (Dynamic)DataFixUtils.orElse(dynamic.asString().result().map(WallPropertyFix::booleanToWallType).map(arg_0 -> ((Dynamic)dynamic).createString(arg_0)), (Object)dynamic));
    }
