...
extends Enchantment {
    ...
    public ProtectionEnchantment(Enchantment.Rarity weight, Type type, EquipmentSlot ... equipmentSlotArray) {
        super(weight, type == Type.FALL ? EnchantmentTarget.ARMOR_FEET : EnchantmentTarget.ARMOR, equipmentSlotArray);
    ...
    public static int transformFireDuration(LivingEntity entity, int duration) {
        int n = EnchantmentHelper.getEquipmentLevel(Enchantments.FIRE_PROTECTION, entity);
        if (n > 0) {
            duration -= MathHelper.floor((float)duration * ((float)n * 0.15f));
        }
        return duration;
    }

    public static double transformExplosionKnockback(LivingEntity entity, double velocity) {
        int n = EnchantmentHelper.getEquipmentLevel(Enchantments.BLAST_PROTECTION, entity);
        if (n > 0) {
            velocity -= (double)MathHelper.floor(velocity * (double)((float)n * 0.15f));
        }
        return velocity;
    }
