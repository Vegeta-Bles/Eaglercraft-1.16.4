...
public class EntityType<T extends Entity> {
    ...
    public EntityType(EntityFactory<T> factory, SpawnGroup spawnGroup, boolean saveable, boolean summonable, boolean fireImmune, boolean spawnableFarFromPlayer, ImmutableSet<Block> immutableSet, EntityDimensions entityDimensions, int n, int n2) {
        ...
        this.maxTrackDistance = n;
        this.trackTickInterval = n2;
    }
    ...
    public T spawn(ServerWorld serverWorld, @Nullable CompoundTag itemTag, @Nullable Text name, @Nullable PlayerEntity player, BlockPos pos, SpawnReason spawnReason, boolean alignPosition, boolean invertY) {
        T t = this.create(serverWorld, itemTag, name, player, pos, spawnReason, alignPosition, invertY);
        if (t != null) {
            serverWorld.spawnEntityAndPassengers((Entity)t);
        }
        return t;
    }
    ...
    public T create(ServerWorld serverWorld, @Nullable CompoundTag itemTag, @Nullable Text name, @Nullable PlayerEntity player, BlockPos pos, SpawnReason spawnReason, boolean alignPosition, boolean invertY) {
        ...
        if (alignPosition) {
            ((Entity)t).updatePosition((double)pos.getX() + 0.5, pos.getY() + 1, (double)pos.getZ() + 0.5);
            d = EntityType.getOriginY(serverWorld, pos, invertY, ((Entity)t).getBoundingBox());
        } else {
            d = 0.0;
        }
        ((Entity)t).refreshPositionAndAngles((double)pos.getX() + 0.5, (double)pos.getY() + d, (double)pos.getZ() + 0.5, MathHelper.wrapDegrees(serverWorld.random.nextFloat() * 360.0f), 0.0f);
        ...
        if (name != null && t instanceof LivingEntity) {
            ((Entity)t).setCustomName(name);
        }
        EntityType.loadFromEntityTag(serverWorld, player, t, itemTag);
        return t;
    }
    ...
    protected static double getOriginY(WorldView world, BlockPos pos, boolean invertY, Box boundingBox) {
        ...
        Stream<VoxelShape> _snowman2 = world.getCollisions(null, box, entity -> true);
        return 1.0 + VoxelShapes.calculateMaxOffset(Direction.Axis.Y, boundingBox, _snowman2, invertY ? -2.0 : -1.0);
    }
    ...
    public static void loadFromEntityTag(World world, @Nullable PlayerEntity player, @Nullable Entity entity, @Nullable CompoundTag itemTag) {
        ...
        CompoundTag _snowman2 = entity.toTag(new CompoundTag());
        UUID _snowman3 = entity.getUuid();
        _snowman2.copyFrom(itemTag.getCompound("EntityTag"));
        entity.setUuid(_snowman3);
        entity.fromTag(_snowman2);
    }
    ...
    public boolean isInvalidSpawn(BlockState blockState) {
        if (this.canSpawnInside.contains((Object)blockState.getBlock())) {
            return false;
        }
    ...
    public static Entity loadEntityWithPassengers(CompoundTag compoundTag, World world, Function<Entity, Entity> entityProcessor) {
        return EntityType.loadEntityFromTag(compoundTag, world).map(entityProcessor).map(entity2 -> {
            Entity entity2;
            if (compoundTag.contains("Passengers", 9)) {
                ListTag listTag = compoundTag.getList("Passengers", 10);
                for (int i = 0; i < listTag.size(); ++i) {
                    Entity entity3 = EntityType.loadEntityWithPassengers(listTag.getCompound(i), world, entityProcessor);
    ...
    public static class Builder<T extends Entity> {
        ...
        public Builder<T> allowSpawningInside(Block ... blockArray) {
            this.canSpawnInside = ImmutableSet.copyOf((Object[])blockArray);
            return this;
        }
