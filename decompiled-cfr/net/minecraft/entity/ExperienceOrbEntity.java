...
extends Entity {
    ...
    public void tick() {
        ...
        if (this.target != null && (_snowman = (_snowman = new Vec3d(this.target.getX() - this.getX(), this.target.getY() + (double)this.target.getStandingEyeHeight() / 2.0 - this.getY(), this.target.getZ() - this.getZ())).lengthSquared()) < 64.0) {
            _snowman = 1.0 - Math.sqrt(_snowman) / 8.0;
            this.setVelocity(this.getVelocity().add(_snowman.normalize().multiply(_snowman * _snowman * 0.1)));
        }
        this.move(MovementType.SELF, this.getVelocity());
        float _snowman2 = 0.98f;
        if (this.onGround) {
            _snowman2 = this.world.getBlockState(new BlockPos(this.getX(), this.getY() - 1.0, this.getZ())).getBlock().getSlipperiness() * 0.98f;
        }
        this.setVelocity(this.getVelocity().multiply(_snowman2, 0.98, _snowman2));
        if (this.onGround) {
    ...
    public void onPlayerCollision(PlayerEntity player) {
        ...
        if (this.pickupDelay == 0 && player.experiencePickUpDelay == 0) {
            ...
            if (entry != null && !(_snowman = entry.getValue()).isEmpty() && _snowman.isDamaged()) {
                int n = Math.min(this.getMendingRepairAmount(this.amount), _snowman.getDamage());
                this.amount -= this.getMendingRepairCost(n);
                _snowman.setDamage(_snowman.getDamage() - n);
            }
            if (this.amount > 0) {
