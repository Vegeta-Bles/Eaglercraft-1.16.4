...
extends Entity {
    ...
    public void tick() {
        ...
        float _snowman2 = this.getStandingEyeHeight() - 0.11111111f;
        if (this.isTouchingWater() && this.getFluidHeight(FluidTags.WATER) > (double)_snowman2) {
            this.applyBuoyancy();
        } else if (this.isInLava() && this.getFluidHeight(FluidTags.LAVA) > (double)_snowman2) {
            this.method_24348();
        ...
        int n = _snowman = bl ? 2 : 40;
        if (this.age % _snowman == 0) {
        ...
        if (!this.world.isClient && (_snowman = this.getVelocity().subtract(vec3d).lengthSquared()) > 0.01) {
    ...
    private void tryMerge(ItemEntity other) {
        ItemStack itemStack = this.getStack();
        _snowman = other.getStack();
        if (!Objects.equals(this.getOwner(), other.getOwner()) || !ItemEntity.canMerge(itemStack, _snowman)) {
            return;
        }
        if (_snowman.getCount() < itemStack.getCount()) {
            ItemEntity.merge(this, itemStack, other, _snowman);
        } else {
            ItemEntity.merge(other, _snowman, this, itemStack);
        }
    }
    ...
    public static ItemStack merge(ItemStack stack1, ItemStack stack2, int maxCount) {
        int n = Math.min(Math.min(stack1.getMaxCount(), maxCount) - stack1.getCount(), stack2.getCount());
        ItemStack _snowman2 = stack1.copy();
        _snowman2.increment(n);
        stack2.decrement(n);
        return _snowman2;
    }
    ...
    public void onPlayerCollision(PlayerEntity player) {
        ...
        int _snowman3 = itemStack.getCount();
        if (this.pickupDelay == 0 && (this.owner == null || this.owner.equals(player.getUuid())) && player.inventory.insertStack(itemStack)) {
            player.sendPickup(this, _snowman3);
            if (itemStack.isEmpty()) {
                this.remove();
                itemStack.setCount(_snowman3);
            }
            player.increaseStat(Stats.PICKED_UP.getOrCreateStat(_snowman2), _snowman3);
