...
extends Entity {
    ...
    public void tick() {
        ...
        if (this.ambientTick >= 0) {
            if (!(this.world instanceof ServerWorld)) {
                ...
                double d = 3.0;
                List<Entity> _snowman2 = this.world.getOtherEntities(this, new Box(this.getX() - 3.0, this.getY() - 3.0, this.getZ() - 3.0, this.getX() + 3.0, this.getY() + 6.0 + 3.0, this.getZ() + 3.0), Entity::isAlive);
                for (Entity entity : _snowman2) {
                    entity.onStruckByLightning((ServerWorld)this.world, this);
                }
                if (this.channeler != null) {
                    Criteria.CHANNELED_LIGHTNING.trigger(this.channeler, _snowman2);
                }
            }
        }
    }
    ...
    private void spawnFire(int spreadAttempts) {
        ...
        BlockState _snowman2 = AbstractFireBlock.getState(this.world, blockPos);
        if (this.world.getBlockState(blockPos).isAir() && _snowman2.canPlaceAt(this.world, blockPos)) {
            this.world.setBlockState(blockPos, _snowman2);
        }
        for (int i = 0; i < spreadAttempts; ++i) {
            BlockPos blockPos2 = blockPos.add(this.random.nextInt(3) - 1, this.random.nextInt(3) - 1, this.random.nextInt(3) - 1);
            _snowman2 = AbstractFireBlock.getState(this.world, blockPos2);
            if (!this.world.getBlockState(blockPos2).isAir() || !_snowman2.canPlaceAt(this.world, blockPos2)) continue;
            this.world.setBlockState(blockPos2, _snowman2);
        }
    }
