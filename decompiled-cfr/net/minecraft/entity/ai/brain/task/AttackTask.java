...
extends Task<E> {
    ...
    protected boolean shouldRun(ServerWorld serverWorld, E e) {
        return this.isAttackTargetVisible(e) && this.isNearAttackTarget(e);
    }

    @Override
    protected void run(ServerWorld serverWorld, E e, long l) {
        ((LivingEntity)e).getBrain().remember(MemoryModuleType.LOOK_TARGET, new EntityLookTarget(this.getAttackTarget(e), true));
        ((MobEntity)e).getMoveControl().strafeTo(-this.forwardMovement, 0.0f);
        ((MobEntity)e).yaw = MathHelper.stepAngleTowards(((MobEntity)e).yaw, ((MobEntity)e).headYaw, 0.0f);
    }
