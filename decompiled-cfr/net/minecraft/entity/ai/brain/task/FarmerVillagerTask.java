...
extends Task<VillagerEntity> {
    ...
    @Override
    protected boolean shouldRun(ServerWorld serverWorld2, VillagerEntity villagerEntity) {
        ServerWorld serverWorld2;
        if (!serverWorld2.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
            return false;
        }
        ...
        for (int i = -1; i <= 1; ++i) {
            for (_snowman = -1; _snowman <= 1; ++_snowman) {
                for (_snowman = -1; _snowman <= 1; ++_snowman) {
                    mutable.set(villagerEntity.getX() + (double)i, villagerEntity.getY() + (double)_snowman, villagerEntity.getZ() + (double)_snowman);
                    if (!this.isSuitableTarget(mutable, serverWorld2)) continue;
                    this.targetPositions.add(new BlockPos(mutable));
                }
            }
        }
        this.currentTarget = this.chooseRandomTarget(serverWorld2);
    ...
    private boolean isSuitableTarget(BlockPos pos, ServerWorld world) {
        ...
        Block _snowman3 = world.getBlockState(pos.down()).getBlock();
        return _snowman2 instanceof CropBlock && ((CropBlock)_snowman2).isMature(blockState) || blockState.isAir() && _snowman3 instanceof FarmlandBlock;
    }
    ...
    protected void keepRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
        ...
        if (this.currentTarget != null && l > this.nextResponseTime) {
            ...
            Block _snowman2 = blockState.getBlock();
            Block _snowman3 = serverWorld.getBlockState(this.currentTarget.down()).getBlock();
            if (_snowman2 instanceof CropBlock && ((CropBlock)_snowman2).isMature(blockState)) {
                serverWorld.breakBlock(this.currentTarget, true, villagerEntity);
            }
            if (blockState.isAir() && _snowman3 instanceof FarmlandBlock && villagerEntity.hasSeedToPlant()) {
