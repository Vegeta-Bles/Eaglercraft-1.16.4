...
extends Task<PathAwareEntity> {
    ...
    protected void run(ServerWorld serverWorld, PathAwareEntity pathAwareEntity, long l) {
        ...
        if (serverWorld.isNearOccupiedPointOfInterest(blockPos)) {
            ...
            _snowman = LookTargetUtil.getPosClosestToOccupiedPointOfInterest(serverWorld, chunkSectionPos, 2);
            if (_snowman != chunkSectionPos) {
                this.updateWalkTarget(pathAwareEntity, _snowman);
            } else {
                this.updateWalkTarget(pathAwareEntity);
            }
        }
