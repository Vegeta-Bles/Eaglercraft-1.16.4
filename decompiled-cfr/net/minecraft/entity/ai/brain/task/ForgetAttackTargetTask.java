...
extends Task<E> {
    ...
    protected void run(ServerWorld serverWorld, E e, long l) {
        if (ForgetAttackTargetTask.cannotReachTarget(e)) {
            this.forgetAttackTarget(e);
            return;
        }
        if (this.isAttackTargetDead(e)) {
            this.forgetAttackTarget(e);
            return;
        }
        if (this.isAttackTargetInAnotherWorld(e)) {
            this.forgetAttackTarget(e);
            return;
        }
        if (!EntityPredicates.EXCEPT_CREATIVE_SPECTATOR_OR_PEACEFUL.test(this.getAttackTarget(e))) {
            this.forgetAttackTarget(e);
            return;
        }
        if (this.alternativeCondition.test(this.getAttackTarget(e))) {
            this.forgetAttackTarget(e);
            return;
        }
    }
