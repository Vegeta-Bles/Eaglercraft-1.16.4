...
extends Task<VillagerEntity> {
    ...
    private List<ItemStack> getGifts(VillagerEntity villager) {
        ...
        if (GIFTS.containsKey(villagerProfession)) {
            ...
            LootContext.Builder _snowman2 = new LootContext.Builder((ServerWorld)villager.world).parameter(LootContextParameters.ORIGIN, villager.getPos()).parameter(LootContextParameters.THIS_ENTITY, villager).random(villager.getRandom());
            return lootTable.generateLoot(_snowman2.build(LootContextTypes.GIFT));
        }
    ...
    private boolean isCloseEnough(VillagerEntity villager, PlayerEntity player) {
        ...
        _snowman = villager.getBlockPos();
        return _snowman.isWithinDistance(blockPos, 5.0);
    }
