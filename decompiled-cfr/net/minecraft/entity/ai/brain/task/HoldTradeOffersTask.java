...
extends Task<VillagerEntity> {
    ...
    public boolean shouldRun(ServerWorld serverWorld, VillagerEntity villagerEntity) {
        ...
        LivingEntity _snowman2 = brain.getOptionalMemory(MemoryModuleType.INTERACTION_TARGET).get();
        return _snowman2.getType() == EntityType.PLAYER && villagerEntity.isAlive() && _snowman2.isAlive() && !villagerEntity.isBaby() && villagerEntity.squaredDistanceTo(_snowman2) <= 17.0;
    }
    ...
    private void setupOffers(LivingEntity customer, VillagerEntity villager) {
        boolean bl = false;
        ItemStack _snowman2 = customer.getMainHandStack();
        if (this.customerHeldStack == null || !ItemStack.areItemsEqualIgnoreDamage(this.customerHeldStack, _snowman2)) {
            this.customerHeldStack = _snowman2;
            bl = true;
            this.offers.clear();
        }
    ...
    private LivingEntity findPotentialCustomer(VillagerEntity villager) {
        ...
        LivingEntity _snowman2 = brain.getOptionalMemory(MemoryModuleType.INTERACTION_TARGET).get();
        brain.remember(MemoryModuleType.LOOK_TARGET, new EntityLookTarget(_snowman2, true));
        return _snowman2;
    }
