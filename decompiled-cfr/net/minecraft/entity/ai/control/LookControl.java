...
public class LookControl {
    ...
    protected float getTargetPitch() {
        ...
        _snowman = this.lookY - this.entity.getEyeY();
        _snowman = this.lookZ - this.entity.getZ();
        _snowman = MathHelper.sqrt(d * d + _snowman * _snowman);
        return (float)(-(MathHelper.atan2(_snowman, _snowman) * 57.2957763671875));
    }

    protected float getTargetYaw() {
        double d = this.lookX - this.entity.getX();
        _snowman = this.lookZ - this.entity.getZ();
        return (float)(MathHelper.atan2(_snowman, d) * 57.2957763671875) - 90.0f;
    }

    protected float changeAngle(float from, float to, float max) {
        float f = MathHelper.subtractAngles(from, to);
        _snowman = MathHelper.clamp(f, -max, max);
        return from + _snowman;
    }
