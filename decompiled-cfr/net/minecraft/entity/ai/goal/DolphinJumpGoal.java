...
extends DiveJumpingGoal {
    ...
    public boolean canStart() {
        ...
        int _snowman3 = direction.getOffsetZ();
        BlockPos _snowman4 = this.dolphin.getBlockPos();
        for (int n : OFFSET_MULTIPLIERS) {
            if (this.isWater(_snowman4, _snowman2, _snowman3, n) && this.isAirAbove(_snowman4, _snowman2, _snowman3, n)) continue;
            return false;
        }
        return true;
    }
    ...
    public void tick() {
        ...
        if (!bl) {
            _snowman2 = this.dolphin.world.getFluidState(this.dolphin.getBlockPos());
            this.inWater = ((FluidState)_snowman2).isIn(FluidTags.WATER);
        }
        if (this.inWater && !bl) {
            this.dolphin.playSound(SoundEvents.ENTITY_DOLPHIN_JUMP, 1.0f, 1.0f);
        }
        _snowman2 = this.dolphin.getVelocity();
        if (((Vec3d)_snowman2).y * ((Vec3d)_snowman2).y < (double)0.03f && this.dolphin.pitch != 0.0f) {
            this.dolphin.pitch = MathHelper.lerpAngle(this.dolphin.pitch, 0.0f, 0.2f);
        } else {
            double d = Math.sqrt(Entity.squaredHorizontalLength((Vec3d)_snowman2));
            _snowman = Math.signum(-((Vec3d)_snowman2).y) * Math.acos(d / ((Vec3d)_snowman2).length()) * 57.2957763671875;
            this.dolphin.pitch = (float)_snowman;
        }
    }
}
