...
extends Goal {
    ...
    public boolean canStart() {
        ...
        Path _snowman2 = mobNavigation.getCurrentPath();
        if (_snowman2 == null || _snowman2.isFinished() || !mobNavigation.canEnterOpenDoors()) {
            return false;
        }
        for (int i = 0; i < Math.min(_snowman2.getCurrentNodeIndex() + 2, _snowman2.getLength()); ++i) {
            PathNode pathNode = _snowman2.getNode(i);
    ...
    public void tick() {
        ...
        _snowman = this.xOffset * f + this.zOffset * (_snowman = (float)((double)this.doorPos.getZ() + 0.5 - this.mob.getZ()));
        if (_snowman < 0.0f) {
