...
extends Goal {
    ...
    public void tick() {
        ...
        if (GRASS_PREDICATE.test(this.world.getBlockState(blockPos))) {
            ...
            if (this.world.getBlockState(_snowman).isOf(Blocks.GRASS_BLOCK)) {
                if (this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                    this.world.syncWorldEvent(2001, _snowman, Block.getRawIdFromState(Blocks.GRASS_BLOCK.getDefaultState()));
                    this.world.setBlockState(_snowman, Blocks.DIRT.getDefaultState(), 2);
                }
                this.mob.onEatingGrass();
            }
        }
