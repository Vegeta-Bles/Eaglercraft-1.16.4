...
extends Goal {
    ...
    private void tryTeleport() {
        ...
        for (int i = 0; i < 10; ++i) {
            _snowman = this.getRandomInt(-3, 3);
            _snowman = this.getRandomInt(-1, 1);
            _snowman = this.getRandomInt(-3, 3);
            boolean bl = this.tryTeleportTo(blockPos.getX() + _snowman, blockPos.getY() + _snowman, blockPos.getZ() + _snowman);
            if (!bl) continue;
            return;
        }
    }
    ...
    private boolean canTeleportTo(BlockPos pos) {
        ...
        BlockState _snowman2 = this.world.getBlockState(pos.down());
        if (!this.leavesAllowed && _snowman2.getBlock() instanceof LeavesBlock) {
            return false;
        }
        BlockPos _snowman3 = pos.subtract(this.tameable.getBlockPos());
        return this.world.isSpaceEmpty(this.tameable, this.tameable.getBoundingBox().offset(_snowman3));
    }
