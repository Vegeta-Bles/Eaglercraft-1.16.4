...
extends Goal {
    ...
    public void tick() {
        ...
        boolean _snowman2 = this.mob.getVisibilityCache().canSee(this.target);
        this.seenTargetTicks = _snowman2 ? ++this.seenTargetTicks : 0;
        ...
        if (--this.updateCountdownTicks == 0) {
            ...
            float f = _snowman = MathHelper.sqrt(d) / this.maxShootRange;
            f = MathHelper.clamp(f, 0.1f, 1.0f);
            this.owner.attack(this.target, f);
            this.updateCountdownTicks = MathHelper.floor(_snowman * (float)(this.maxIntervalTicks - this.minIntervalTicks) + (float)this.minIntervalTicks);
        } else if (this.updateCountdownTicks < 0) {
            float _snowman3 = MathHelper.sqrt(d) / this.maxShootRange;
            this.updateCountdownTicks = MathHelper.floor(_snowman3 * (float)(this.maxIntervalTicks - this.minIntervalTicks) + (float)this.minIntervalTicks);
        }
    }
}
