...
extends MoveToTargetPosGoal {
    ...
    public void tick() {
        ...
        BlockPos _snowman2 = this.stepAndDestroyMob.getBlockPos();
        BlockPos _snowman3 = this.tweakToProperPos(_snowman2, world);
        Random _snowman4 = this.stepAndDestroyMob.getRandom();
        if (this.hasReached() && _snowman3 != null) {
            double d;
            Vec3d vec3d;
            if (this.counter > 0) {
                ...
                if (!world.isClient) {
                    d = 0.08;
                    ((ServerWorld)world).spawnParticles(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(Items.EGG)), (double)_snowman3.getX() + 0.5, (double)_snowman3.getY() + 0.7, (double)_snowman3.getZ() + 0.5, 3, ((double)_snowman4.nextFloat() - 0.5) * 0.08, ((double)_snowman4.nextFloat() - 0.5) * 0.08, ((double)_snowman4.nextFloat() - 0.5) * 0.08, 0.15f);
                }
            }
            if (this.counter % 2 == 0) {
                vec3d = this.stepAndDestroyMob.getVelocity();
            ...
            if (this.counter > 60) {
                world.removeBlock(_snowman3, false);
                if (!world.isClient) {
                    for (int i = 0; i < 20; ++i) {
                        d = _snowman4.nextGaussian() * 0.02;
                        _snowman = _snowman4.nextGaussian() * 0.02;
                        _snowman = _snowman4.nextGaussian() * 0.02;
                        ((ServerWorld)world).spawnParticles(ParticleTypes.POOF, (double)_snowman3.getX() + 0.5, _snowman3.getY(), (double)_snowman3.getZ() + 0.5, 1, d, _snowman, _snowman, 0.15f);
                    }
                    this.onDestroyBlock(world, _snowman3);
                }
            }
            ++this.counter;
        }
    ...
    private BlockPos tweakToProperPos(BlockPos pos, BlockView world) {
        ...
        for (BlockPos _snowman2 : blockPosArray = new BlockPos[]{pos.down(), pos.west(), pos.east(), pos.north(), pos.south(), pos.down().down()}) {
            if (!world.getBlockState(_snowman2).isOf(this.targetBlock)) continue;
            return _snowman2;
        }
        return null;
    }
