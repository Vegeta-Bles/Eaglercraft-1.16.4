...
public abstract class EntityNavigation {
    ...
    public EntityNavigation(MobEntity mob, World world) {
        this.entity = mob;
        this.world = world;
        int n = MathHelper.floor(mob.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE) * 16.0);
        this.pathNodeNavigator = this.createPathNodeNavigator(n);
    }
    ...
    @Nullable
    public Path method_29934(Set<BlockPos> set, int n) {
        return this.findPathToAny(set, 8, false, n);
    }
    ...
    protected Path findPathToAny(Set<BlockPos> positions, int range, boolean bl, int distance) {
        ...
        BlockPos _snowman2 = bl ? this.entity.getBlockPos().up() : this.entity.getBlockPos();
        int _snowman3 = (int)(f + (float)range);
        ChunkCache _snowman4 = new ChunkCache(this.world, _snowman2.add(-_snowman3, -_snowman3, -_snowman3), _snowman2.add(_snowman3, _snowman3, _snowman3));
        Path _snowman5 = this.pathNodeNavigator.findPathToAny(_snowman4, this.entity, positions, f, distance, this.rangeMultiplier);
        this.world.getProfiler().pop();
        if (_snowman5 != null && _snowman5.getTarget() != null) {
            this.currentTarget = _snowman5.getTarget();
    ...
    public void tick() {
        ...
        if (this.isAtValidPosition()) {
            ...
            Vec3d vec3d = this.getPos();
            _snowman2 = this.currentPath.getNodePosition(this.entity);
            if (vec3d.y > ((Vec3d)_snowman2).y && !this.entity.isOnGround() && MathHelper.floor(vec3d.x) == MathHelper.floor(((Vec3d)_snowman2).x) && MathHelper.floor(vec3d.z) == MathHelper.floor(((Vec3d)_snowman2).z)) {
        ...
        _snowman2 = new BlockPos(vec3d);
        this.entity.getMoveControl().moveTo(vec3d.x, this.world.getBlockState(((BlockPos)_snowman2).down()).isAir() ? vec3d.y : LandPathNodeMaker.getFeetY(this.world, (BlockPos)_snowman2), vec3d.z, this.speed);
    }

    protected void continueFollowingPath() {
        Vec3d vec3d = this.getPos();
        this.nodeReachProximity = this.entity.getWidth() > 0.75f ? this.entity.getWidth() / 2.0f : 0.75f - this.entity.getWidth() / 2.0f;
        BlockPos _snowman2 = this.currentPath.method_31032();
        double _snowman3 = Math.abs(this.entity.getX() - ((double)_snowman2.getX() + 0.5));
        double _snowman4 = Math.abs(this.entity.getY() - (double)_snowman2.getY());
        double _snowman5 = Math.abs(this.entity.getZ() - ((double)_snowman2.getZ() + 0.5));
        boolean bl = _snowman = _snowman3 < (double)this.nodeReachProximity && _snowman5 < (double)this.nodeReachProximity && _snowman4 < 1.0;
        if (_snowman || this.entity.method_29244(this.currentPath.method_29301().type) && this.method_27799(vec3d)) {
            this.currentPath.next();
        }
    ...
    private boolean method_27799(Vec3d vec3d) {
        ...
        _snowman = Vec3d.ofBottomCenter(this.currentPath.method_31032());
        if (!vec3d.isInRange(_snowman, 2.0)) {
            return false;
        }
        _snowman = Vec3d.ofBottomCenter(this.currentPath.method_31031(this.currentPath.getCurrentNodeIndex() + 1));
        _snowman = _snowman.subtract(_snowman);
        return _snowman.dotProduct(_snowman = vec3d.subtract(_snowman)) > 0.0;
    }
    ...
    protected void adjustPath() {
        ...
        for (int i = 0; i < this.currentPath.getLength(); ++i) {
            ...
            _snowman = i + 1 < this.currentPath.getLength() ? this.currentPath.getNode(i + 1) : null;
            BlockState _snowman2 = this.world.getBlockState(new BlockPos(pathNode.x, pathNode.y, pathNode.z));
            if (!_snowman2.isOf(Blocks.CAULDRON)) continue;
            this.currentPath.setNode(i, pathNode.copyWithNewPosition(pathNode.x, pathNode.y + 1, pathNode.z));
            if (_snowman == null || pathNode.y < _snowman.y) continue;
            this.currentPath.setNode(i + 1, pathNode.copyWithNewPosition(_snowman.x, pathNode.y + 1, _snowman.z));
        }
    }
    ...
    public void onBlockChanged(BlockPos pos) {
        ...
        Vec3d _snowman2 = new Vec3d(((double)pathNode.x + this.entity.getX()) / 2.0, ((double)pathNode.y + this.entity.getY()) / 2.0, ((double)pathNode.z + this.entity.getZ()) / 2.0);
        if (pos.isWithinDistance(_snowman2, (double)(this.currentPath.getLength() - this.currentPath.getCurrentNodeIndex()))) {
            this.recalculatePath();
        }
    }
