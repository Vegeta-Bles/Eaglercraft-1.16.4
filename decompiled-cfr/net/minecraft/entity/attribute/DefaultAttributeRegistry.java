...
public class DefaultAttributeRegistry {
    ...
    public static void checkMissing() {
        Registry.ENTITY_TYPE.stream().filter(entityType -> entityType.getSpawnGroup() != SpawnGroup.MISC).filter(entityType -> !DefaultAttributeRegistry.hasDefinitionFor(entityType)).map(Registry.ENTITY_TYPE::getId).forEach(identifier -> {
