...
extends AbstractDecorationEntity {
    ...
    protected void updateAttachmentPosition() {
        ...
        double d = 0.46875;
        _snowman = (double)this.attachmentPos.getX() + 0.5 - (double)this.facing.getOffsetX() * 0.46875;
        _snowman = (double)this.attachmentPos.getY() + 0.5 - (double)this.facing.getOffsetY() * 0.46875;
        _snowman = (double)this.attachmentPos.getZ() + 0.5 - (double)this.facing.getOffsetZ() * 0.46875;
        this.setPos(_snowman, _snowman, _snowman);
        _snowman = this.getWidthPixels();
        _snowman = this.getHeightPixels();
        _snowman = this.getWidthPixels();
        Direction.Axis _snowman2 = this.facing.getAxis();
        switch (_snowman2) {
            case X: {
                _snowman = 1.0;
                break;
        ...
        this.setBoundingBox(new Box(_snowman - (_snowman /= 32.0), _snowman - (_snowman /= 32.0), _snowman - (_snowman /= 32.0), _snowman + _snowman, _snowman + _snowman, _snowman + _snowman));
    }
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        ...
        if (compoundTag != null && !compoundTag.isEmpty()) {
            ...
            if (!(_snowman = this.getHeldItemStack()).isEmpty() && !ItemStack.areEqual(itemStack, _snowman)) {
                this.removeFromFrame(_snowman);
            }
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        ...
        boolean _snowman2 = !this.getHeldItemStack().isEmpty();
        boolean bl = _snowman = !itemStack.isEmpty();
        if (this.fixed) {
            return ActionResult.PASS;
        }
        if (this.world.isClient) {
            return _snowman2 || _snowman ? ActionResult.SUCCESS : ActionResult.PASS;
        }
        if (!_snowman2) {
            if (_snowman && !this.removed) {
