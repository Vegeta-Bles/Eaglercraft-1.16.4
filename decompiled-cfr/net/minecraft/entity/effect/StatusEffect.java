...
public class StatusEffect {
    ...
    public void applyInstantEffect(@Nullable Entity source, @Nullable Entity attacker, LivingEntity target, int amplifier, double proximity) {
        if (this == StatusEffects.INSTANT_HEALTH && !target.isUndead() || this == StatusEffects.INSTANT_DAMAGE && target.isUndead()) {
            int n = (int)(proximity * (double)(4 << amplifier) + 0.5);
            target.heal(n);
        } else if (this == StatusEffects.INSTANT_DAMAGE && !target.isUndead() || this == StatusEffects.INSTANT_HEALTH && target.isUndead()) {
            int n = (int)(proximity * (double)(6 << amplifier) + 0.5);
            if (source == null) {
                target.damage(DamageSource.MAGIC, n);
            } else {
                target.damage(DamageSource.magic(source, attacker), n);
            }
        } else {
    ...
    public void onApplied(LivingEntity entity, AttributeContainer attributes, int amplifier) {
        for (Map.Entry<EntityAttribute, EntityAttributeModifier> entry : this.attributeModifiers.entrySet()) {
            EntityAttributeInstance entityAttributeInstance = attributes.getCustomInstance(entry.getKey());
            if (entityAttributeInstance == null) continue;
            EntityAttributeModifier _snowman2 = entry.getValue();
            entityAttributeInstance.removeModifier(_snowman2);
            entityAttributeInstance.addPersistentModifier(new EntityAttributeModifier(_snowman2.getId(), this.getTranslationKey() + " " + amplifier, this.adjustModifierAmount(amplifier, _snowman2), _snowman2.getOperation()));
        }
    }
