...
public interface Angerable {
    ...
    default public void angerFromTag(ServerWorld world, CompoundTag tag) {
        ...
        Entity _snowman2 = world.getEntity(uUID);
        if (_snowman2 == null) {
            return;
        }
        if (_snowman2 instanceof MobEntity) {
            this.setAttacker((MobEntity)_snowman2);
        }
        if (_snowman2.getType() == EntityType.PLAYER) {
            this.setAttacking((PlayerEntity)_snowman2);
        }
    }

    default public void tickAngerLogic(ServerWorld world, boolean bl) {
        LivingEntity livingEntity = this.getTarget();
        UUID _snowman2 = this.getAngryAt();
        if ((livingEntity == null || livingEntity.isDead()) && _snowman2 != null && world.getEntity(_snowman2) instanceof MobEntity) {
            this.stopAnger();
            return;
        }
        if (livingEntity != null && !Objects.equals(_snowman2, livingEntity.getUuid())) {
