...
extends HostileEntity {
    ...
    private void setFireActive(boolean fireActive) {
        byte by = this.dataTracker.get(BLAZE_FLAGS);
        by = fireActive ? (byte)(by | 1) : (byte)(by & 0xFFFFFFFE);
        this.dataTracker.set(BLAZE_FLAGS, by);
    }
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            boolean _snowman2 = this.blaze.getVisibilityCache().canSee(livingEntity);
            this.targetNotVisibleTicks = _snowman2 ? 0 : ++this.targetNotVisibleTicks;
            double _snowman3 = this.blaze.squaredDistanceTo(livingEntity);
            if (_snowman3 < 4.0) {
                if (!_snowman2) {
                    return;
                }
            ...
            } else if (_snowman3 < this.getFollowRange() * this.getFollowRange() && _snowman2) {
                double d = livingEntity.getX() - this.blaze.getX();
                _snowman = livingEntity.getBodyY(0.5) - this.blaze.getBodyY(0.5);
                _snowman = livingEntity.getZ() - this.blaze.getZ();
                if (this.fireballCooldown <= 0) {
                    ++this.fireballsFired;
                    ...
                    if (this.fireballsFired > 1) {
                        float f = MathHelper.sqrt(MathHelper.sqrt(_snowman3)) * 0.5f;
                        if (!this.blaze.isSilent()) {
                            this.blaze.world.syncWorldEvent(null, 1018, this.blaze.getBlockPos(), 0);
                        }
                        for (int i = 0; i < 1; ++i) {
                            SmallFireballEntity smallFireballEntity = new SmallFireballEntity(this.blaze.world, this.blaze, d + this.blaze.getRandom().nextGaussian() * (double)f, _snowman, _snowman + this.blaze.getRandom().nextGaussian() * (double)f);
