...
implements RangedAttackMob {
    ...
    public static boolean canSpawn(EntityType<DrownedEntity> type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        ...
        boolean bl = _snowman = world.getDifficulty() != Difficulty.PEACEFUL && DrownedEntity.isSpawnDark(world, pos, random) && (spawnReason == SpawnReason.SPAWNER || world.getFluidState(pos).isIn(FluidTags.WATER));
        if (Objects.equals(optional, Optional.of(BiomeKeys.RIVER)) || Objects.equals(optional, Optional.of(BiomeKeys.FROZEN_RIVER))) {
            return random.nextInt(15) == 0 && _snowman;
        }
        return random.nextInt(40) == 0 && DrownedEntity.isValidSpawnDepth(world, pos) && _snowman;
    }
    ...
    protected boolean hasFinishedCurrentPath() {
        ...
        return path != null && (_snowman = path.getTarget()) != null && (_snowman = this.squaredDistanceTo(_snowman.getX(), _snowman.getY(), _snowman.getZ())) < 4.0;
    }
    ...
    public void attack(LivingEntity target, float pullProgress) {
        ...
        double _snowman3 = target.getBodyY(0.3333333333333333) - tridentEntity.getY();
        double _snowman4 = target.getZ() - this.getZ();
        double _snowman5 = MathHelper.sqrt(_snowman2 * _snowman2 + _snowman4 * _snowman4);
        tridentEntity.setVelocity(_snowman2, _snowman3 + _snowman5 * (double)0.2f, _snowman4, 1.6f, 14 - this.world.getDifficulty().getId() * 4);
    ...
    extends MoveControl {
        ...
        public void tick() {
            ...
            if (this.drowned.isTargetingUnderwater() && this.drowned.isTouchingWater()) {
                ...
                _snowman = this.targetY - this.drowned.getY();
                _snowman = this.targetZ - this.drowned.getZ();
                ...
                this.drowned.bodyYaw = this.drowned.yaw = this.changeAngle(this.drowned.yaw, _snowman2, 90.0f);
                float _snowman3 = (float)(this.speed * this.drowned.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED));
                float _snowman4 = MathHelper.lerp(0.125f, this.drowned.getMovementSpeed(), _snowman3);
                this.drowned.setMovementSpeed(_snowman4);
                this.drowned.setVelocity(this.drowned.getVelocity().add((double)_snowman4 * d * 0.005, (double)_snowman4 * _snowman * 0.1, (double)_snowman4 * _snowman * 0.005));
            } else {
    ...
    extends Goal {
        ...
        private Vec3d getWanderTarget() {
            ...
            for (int i = 0; i < 10; ++i) {
                BlockPos blockPos = _snowman2.add(random.nextInt(20) - 10, 2 - random.nextInt(8), random.nextInt(20) - 10);
                if (!this.world.getBlockState(blockPos).isOf(Blocks.WATER)) continue;
    ...
    extends ProjectileAttackGoal {
        ...
        public TridentAttackGoal(RangedAttackMob rangedAttackMob, double d, int n, float f) {
