...
extends SpellcastingIllagerEntity {
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        public boolean canStart() {
            ...
            int n = EvokerEntity.this.world.getTargets(VexEntity.class, this.closeVexPredicate, EvokerEntity.this, EvokerEntity.this.getBoundingBox().expand(16.0)).size();
            return EvokerEntity.this.random.nextInt(8) + 1 > n;
        }
        ...
        protected void castSpell() {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                VexEntity _snowman2 = EntityType.VEX.create(EvokerEntity.this.world);
                _snowman2.refreshPositionAndAngles(blockPos, 0.0f, 0.0f);
                _snowman2.initialize(serverWorld, EvokerEntity.this.world.getLocalDifficulty(blockPos), SpawnReason.MOB_SUMMONED, null, null);
                _snowman2.setOwner(EvokerEntity.this);
                _snowman2.setBounds(blockPos);
                _snowman2.setLifeTicks(20 * (30 + EvokerEntity.this.random.nextInt(90)));
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        protected void castSpell() {
            ...
            double _snowman2 = Math.min(livingEntity.getY(), EvokerEntity.this.getY());
            double _snowman3 = Math.max(livingEntity.getY(), EvokerEntity.this.getY()) + 1.0;
            float _snowman4 = (float)MathHelper.atan2(livingEntity.getZ() - EvokerEntity.this.getZ(), livingEntity.getX() - EvokerEntity.this.getX());
            if (EvokerEntity.this.squaredDistanceTo(livingEntity) < 9.0) {
                float f;
                int n;
                for (n = 0; n < 5; ++n) {
                    f = _snowman4 + (float)n * (float)Math.PI * 0.4f;
                    this.conjureFangs(EvokerEntity.this.getX() + (double)MathHelper.cos(f) * 1.5, EvokerEntity.this.getZ() + (double)MathHelper.sin(f) * 1.5, _snowman2, _snowman3, f, 0);
                }
                for (n = 0; n < 8; ++n) {
                    f = _snowman4 + (float)n * (float)Math.PI * 2.0f / 8.0f + 1.2566371f;
                    this.conjureFangs(EvokerEntity.this.getX() + (double)MathHelper.cos(f) * 2.5, EvokerEntity.this.getZ() + (double)MathHelper.sin(f) * 2.5, _snowman2, _snowman3, f, 3);
                }
            } else {
                for (int i = 0; i < 16; ++i) {
                    double d = 1.25 * (double)(i + 1);
                    int _snowman5 = 1 * i;
                    this.conjureFangs(EvokerEntity.this.getX() + (double)MathHelper.cos(_snowman4) * d, EvokerEntity.this.getZ() + (double)MathHelper.sin(_snowman4) * d, _snowman2, _snowman3, _snowman4, _snowman5);
                }
            }
        }
        ...
        private void conjureFangs(double x, double z, double maxY, double y, float yaw, int warmup) {
            ...
            double _snowman3 = 0.0;
            do {
                if (!(_snowman = EvokerEntity.this.world.getBlockState(_snowman = blockPos.down())).isSideSolidFullSquare(EvokerEntity.this.world, _snowman, Direction.UP)) continue;
                if (!EvokerEntity.this.world.isAir(blockPos) && !(_snowman = (_snowman = EvokerEntity.this.world.getBlockState(blockPos)).getCollisionShape(EvokerEntity.this.world, blockPos)).isEmpty()) {
                    _snowman3 = _snowman.getMax(Direction.Axis.Y);
                }
                _snowman2 = true;
                break;
            } while ((blockPos = blockPos.down()).getY() >= MathHelper.floor(maxY) - 1);
            if (_snowman2) {
                EvokerEntity.this.world.spawnEntity(new EvokerFangsEntity(EvokerEntity.this.world, x, (double)blockPos.getY() + _snowman3, z, yaw, warmup, EvokerEntity.this));
            }
        }
