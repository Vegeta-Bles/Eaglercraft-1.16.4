...
extends Entity {
            ...
            if (this.playingAnimation) {
                ...
                if (this.ticksLeft == 14) {
                    for (int i = 0; i < 12; ++i) {
                        ...
                        _snowman = this.getY() + 0.05 + this.random.nextDouble();
                        _snowman = this.getZ() + (this.random.nextDouble() * 2.0 - 1.0) * (double)this.getWidth() * 0.5;
                        _snowman = (this.random.nextDouble() * 2.0 - 1.0) * 0.3;
                        _snowman = 0.3 + this.random.nextDouble() * 0.3;
                        _snowman = (this.random.nextDouble() * 2.0 - 1.0) * 0.3;
                        this.world.addParticle(ParticleTypes.CRIT, d, _snowman + 1.0, _snowman, _snowman, _snowman, _snowman);
                    }
                }
            }
    ...
    public float getAnimationProgress(float tickDelta) {
        ...
        int n = this.ticksLeft - 2;
        if (n <= 0) {
            return 1.0f;
        }
        return 1.0f - ((float)n - tickDelta) / 20.0f;
    }
