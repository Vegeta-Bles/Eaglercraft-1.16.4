...
implements Monster {
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            if (livingEntity.squaredDistanceTo(this.ghast) < 4096.0 && this.ghast.canSee(livingEntity)) {
                ...
                if (this.cooldown == 20) {
                    double d = 4.0;
                    Vec3d _snowman3 = this.ghast.getRotationVec(1.0f);
                    _snowman = livingEntity.getX() - (this.ghast.getX() + _snowman3.x * 4.0);
                    _snowman = livingEntity.getBodyY(0.5) - (0.5 + this.ghast.getBodyY(0.5));
                    _snowman = livingEntity.getZ() - (this.ghast.getZ() + _snowman3.z * 4.0);
                    if (!this.ghast.isSilent()) {
                        world.syncWorldEvent(null, 1016, this.ghast.getBlockPos(), 0);
                    }
                    FireballEntity _snowman4 = new FireballEntity(world, this.ghast, _snowman, _snowman, _snowman);
                    _snowman4.explosionPower = this.ghast.getFireballStrength();
                    _snowman4.updatePosition(this.ghast.getX() + _snowman3.x * 4.0, this.ghast.getBodyY(0.5) + 0.5, _snowman4.getZ() + _snowman3.z * 4.0);
        ...
        public void tick() {
            if (this.ghast.getTarget() == null) {
                ...
                if (livingEntity.squaredDistanceTo(this.ghast) < 4096.0) {
                    double d = livingEntity.getX() - this.ghast.getX();
                    _snowman = livingEntity.getZ() - this.ghast.getZ();
                    this.ghast.bodyYaw = this.ghast.yaw = -((float)MathHelper.atan2(d, _snowman)) * 57.295776f;
                }
            }
        }
    }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            double _snowman2 = moveControl.getTargetX() - this.ghast.getX();
            double _snowman3 = _snowman2 * _snowman2 + (_snowman = moveControl.getTargetY() - this.ghast.getY()) * _snowman + (_snowman = moveControl.getTargetZ() - this.ghast.getZ()) * _snowman;
            return _snowman3 < 1.0 || _snowman3 > 3600.0;
        }
        ...
        public void start() {
            ...
            double _snowman2 = this.ghast.getX() + (double)((random.nextFloat() * 2.0f - 1.0f) * 16.0f);
            double _snowman3 = this.ghast.getY() + (double)((random.nextFloat() * 2.0f - 1.0f) * 16.0f);
            double _snowman4 = this.ghast.getZ() + (double)((random.nextFloat() * 2.0f - 1.0f) * 16.0f);
            this.ghast.getMoveControl().moveTo(_snowman2, _snowman3, _snowman4, 1.0);
        }
    }
    ...
    extends MoveControl {
        ...
        public void tick() {
            ...
            if (this.collisionCheckCooldown-- <= 0) {
                ...
                if (this.willCollide(vec3d = vec3d.normalize(), MathHelper.ceil(_snowman2))) {
