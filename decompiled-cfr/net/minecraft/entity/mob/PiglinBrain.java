...
public class PiglinBrain {
    ...
    protected static void setHuntedRecently(PiglinEntity piglin) {
        int n = HUNT_MEMORY_DURATION.choose(piglin.world.random);
        piglin.getBrain().remember(MemoryModuleType.HUNTED_RECENTLY, true, n);
    }

    private static void addCoreActivities(Brain<PiglinEntity> piglin) {
        piglin.setTaskList(Activity.CORE, 0, (ImmutableList<Task<PiglinEntity>>)ImmutableList.of((Object)new LookAroundTask(45, 90), (Object)new WanderAroundTask(), (Object)new OpenDoorsTask(), PiglinBrain.method_30090(), PiglinBrain.makeGoToZombifiedPiglinTask(), new RemoveOffHandItemTask(), new AdmireItemTask(120), (Object)new DefeatTargetTask(300, PiglinBrain::method_29276), new ForgetAngryAtTargetTask()));
    }
    ...
    protected static void loot(PiglinEntity piglin, ItemEntity drop) {
        ...
        if (PiglinBrain.isGoldenItem(_snowman2)) {
        ...
        if (PiglinBrain.isFood(_snowman2) && !PiglinBrain.hasAteRecently(piglin)) {
            PiglinBrain.setEatenRecently(piglin);
            return;
        }
        boolean _snowman3 = piglin.tryEquip(itemStack);
        if (_snowman3) {
            return;
        }
    ...
    protected static void consumeOffHandItem(PiglinEntity piglin, boolean bl) {
        ...
        if (piglin.isAdult()) {
            ...
            } else if (!bl2 && !(_snowman = piglin.tryEquip(itemStack))) {
                PiglinBrain.barterItem(piglin, itemStack);
            }
        } else {
            boolean _snowman2 = piglin.tryEquip(itemStack);
            if (!_snowman2) {
                _snowman = piglin.getMainHandStack();
                if (PiglinBrain.isGoldenItem(_snowman.getItem())) {
                    PiglinBrain.barterItem(piglin, _snowman);
                } else {
                    PiglinBrain.doBarter(piglin, Collections.singletonList(_snowman));
                }
    ...
    private static List<ItemStack> getBarteredItem(PiglinEntity piglin) {
        ...
        List<ItemStack> _snowman2 = lootTable.generateLoot(new LootContext.Builder((ServerWorld)piglin.world).parameter(LootContextParameters.THIS_ENTITY, piglin).random(piglin.world.random).build(LootContextTypes.BARTER));
        return _snowman2;
    }
    ...
    protected static boolean canGather(PiglinEntity piglin, ItemStack stack) {
        ...
        boolean _snowman2 = piglin.canInsertIntoInventory(stack);
        if (item == Items.GOLD_NUGGET) {
            return _snowman2;
        }
        if (PiglinBrain.isFood(item)) {
            return !PiglinBrain.hasAteRecently(piglin) && _snowman2;
        }
        if (PiglinBrain.isGoldenItem(item)) {
            return PiglinBrain.doesNotHaveGoldInOffHand(piglin) && _snowman2;
        }
    ...
    private static Optional<? extends LivingEntity> getPreferredTarget(PiglinEntity piglin) {
        ...
        Optional<LivingEntity> _snowman2 = LookTargetUtil.getEntity(piglin, MemoryModuleType.ANGRY_AT);
        if (_snowman2.isPresent() && PiglinBrain.shouldAttack(_snowman2.get())) {
            return _snowman2;
        }
        if (brain.hasMemoryModule(MemoryModuleType.UNIVERSAL_ANGER) && (_snowman3 = brain.getOptionalMemory(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER)).isPresent()) {
            return _snowman3;
        }
        Optional<LivingEntity> _snowman3 = brain.getOptionalMemory(MemoryModuleType.NEAREST_VISIBLE_NEMESIS);
        if (_snowman3.isPresent()) {
            return _snowman3;
        }
        Optional<PlayerEntity> _snowman4 = brain.getOptionalMemory(MemoryModuleType.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GOLD);
        if (_snowman4.isPresent() && PiglinBrain.shouldAttack(_snowman4.get())) {
            return _snowman4;
        }
        return Optional.empty();
    }
    ...
    public static ActionResult playerInteract(PiglinEntity piglin, PlayerEntity player, Hand hand) {
        ...
        if (PiglinBrain.isWillingToTrade(piglin, itemStack)) {
            _snowman = itemStack.split(1);
            PiglinBrain.swapItemWithOffHand(piglin, _snowman);
            PiglinBrain.setAdmiringItem(piglin);
            PiglinBrain.stopWalking(piglin);
    ...
    private static void angerAtIfCloser(AbstractPiglinEntity piglin, LivingEntity target) {
        ...
        LivingEntity _snowman2 = LookTargetUtil.getCloserEntity((LivingEntity)piglin, optional, target);
        if (optional.isPresent() && optional.get() == _snowman2) {
            return;
        }
        PiglinBrain.becomeAngryWith(piglin, _snowman2);
    }
    ...
    private static void runAwayFromClosestTarget(PiglinEntity piglin, LivingEntity target) {
        Brain<PiglinEntity> brain = piglin.getBrain();
        LivingEntity _snowman2 = target;
        _snowman2 = LookTargetUtil.getCloserEntity((LivingEntity)piglin, brain.getOptionalMemory(MemoryModuleType.AVOID_TARGET), _snowman2);
        _snowman2 = LookTargetUtil.getCloserEntity((LivingEntity)piglin, brain.getOptionalMemory(MemoryModuleType.ATTACK_TARGET), _snowman2);
        PiglinBrain.runAwayFrom(piglin, _snowman2);
    }
    ...
    private static boolean shouldRunAwayFromHoglins(PiglinEntity piglin) {
        ...
        LivingEntity _snowman2 = brain.getOptionalMemory(MemoryModuleType.AVOID_TARGET).get();
        EntityType<?> _snowman3 = _snowman2.getType();
        if (_snowman3 == EntityType.HOGLIN) {
            return PiglinBrain.hasNoAdvantageAgainstHoglins(piglin);
        }
        if (PiglinBrain.isZombified(_snowman3)) {
            return !brain.method_29519(MemoryModuleType.NEAREST_VISIBLE_ZOMBIFIED, _snowman2);
        }
        return false;
    }
    ...
    private static boolean hasOutnumberedHoglins(PiglinEntity piglins) {
        int n = piglins.getBrain().getOptionalMemory(MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT).orElse(0) + 1;
        _snowman = piglins.getBrain().getOptionalMemory(MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT).orElse(0);
        return _snowman > n;
    }
