...
implements CrossbowUser {
    ...
    protected boolean method_24846(ItemStack stack) {
        ...
        return this.prefersNewEquipment(stack, _snowman2);
    }
    ...
    protected boolean prefersNewEquipment(ItemStack newStack, ItemStack oldStack) {
        ...
        boolean bl2 = _snowman = PiglinBrain.isGoldenItem(oldStack.getItem()) || oldStack.getItem() == Items.CROSSBOW;
        if (bl && !_snowman) {
            return true;
        }
        if (!bl && _snowman) {
    ...
    private Entity method_26089(Entity entity, int n) {
        List<Entity> list = entity.getPassengerList();
        if (n == 1 || list.isEmpty()) {
            return entity;
        }
        return this.method_26089(list.get(0), n - 1);
    }
