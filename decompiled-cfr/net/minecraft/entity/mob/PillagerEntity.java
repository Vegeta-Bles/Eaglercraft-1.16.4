...
implements CrossbowUser {
    ...
    protected void method_30759(float f) {
        ...
        if (this.random.nextInt(300) == 0 && (_snowman = this.getMainHandStack()).getItem() == Items.CROSSBOW) {
            Map<Enchantment, Integer> map = EnchantmentHelper.get(_snowman);
            map.putIfAbsent(Enchantments.PIERCING, 1);
            EnchantmentHelper.set(map, _snowman);
    ...
    public void addBonusForWave(int wave, boolean unused) {
        ...
        boolean bl = _snowman = this.random.nextFloat() <= raid.getEnchantmentChance();
        if (_snowman) {
            ItemStack itemStack = new ItemStack(Items.CROSSBOW);
            HashMap _snowman2 = Maps.newHashMap();
            if (wave > raid.getMaxWaves(Difficulty.NORMAL)) {
                _snowman2.put(Enchantments.QUICK_CHARGE, 2);
            } else if (wave > raid.getMaxWaves(Difficulty.EASY)) {
                _snowman2.put(Enchantments.QUICK_CHARGE, 1);
            }
            _snowman2.put(Enchantments.MULTISHOT, 1);
