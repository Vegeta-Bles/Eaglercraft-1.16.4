...
extends RaiderEntity {
    ...
    public void tickMovement() {
        ...
        if (this.isImmobile()) {
            ...
            _snowman = this.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).getBaseValue();
            this.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).setBaseValue(MathHelper.lerp(0.1, _snowman, d));
        }
        if (this.horizontalCollision && this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
            boolean _snowman4 = false;
            Box _snowman2 = this.getBoundingBox().expand(0.2);
            for (BlockPos blockPos : BlockPos.iterate(MathHelper.floor(_snowman2.minX), MathHelper.floor(_snowman2.minY), MathHelper.floor(_snowman2.minZ), MathHelper.floor(_snowman2.maxX), MathHelper.floor(_snowman2.maxY), MathHelper.floor(_snowman2.maxZ))) {
                BlockState blockState = this.world.getBlockState(blockPos);
                Block _snowman3 = blockState.getBlock();
                if (!(_snowman3 instanceof LeavesBlock)) continue;
                _snowman4 = this.world.breakBlock(blockPos, true, this) || _snowman4;
            }
            if (!_snowman4 && this.onGround) {
                this.jump();
            }
        }
        if (this.roarTick > 0) {
    ...
    private void spawnStunnedParticles() {
        if (this.random.nextInt(6) == 0) {
            ...
            _snowman = this.getY() + (double)this.getHeight() - 0.3;
            _snowman = this.getZ() + (double)this.getWidth() * Math.cos(this.bodyYaw * ((float)Math.PI / 180)) + (this.random.nextDouble() * 0.6 - 0.3);
            this.world.addParticle(ParticleTypes.ENTITY_EFFECT, d, _snowman, _snowman, 0.4980392156862745, 0.5137254901960784, 0.5725490196078431);
        }
    }
    ...
    private void roar() {
        if (this.isAlive()) {
            ...
            for (int i = 0; i < 40; ++i) {
                ...
                _snowman = this.random.nextGaussian() * 0.2;
                _snowman = this.random.nextGaussian() * 0.2;
                this.world.addParticle(ParticleTypes.POOF, vec3d.x, vec3d.y, vec3d.z, d, _snowman, _snowman);
            }
        }
    }
    ...
    private void knockBack(Entity entity) {
        ...
        _snowman = entity.getZ() - this.getZ();
        _snowman = Math.max(d * d + _snowman * _snowman, 0.001);
        entity.addVelocity(d / _snowman * 4.0, 0.2, _snowman / _snowman * 4.0);
    }
