...
implements Monster {
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        ...
        if (tag.contains("APX")) {
            int n = tag.getInt("APX");
            _snowman = tag.getInt("APY");
            _snowman = tag.getInt("APZ");
            this.dataTracker.set(ATTACHED_BLOCK, Optional.of(new BlockPos(n, _snowman, _snowman)));
        } else {
    ...
    public void tick() {
        ...
        if (this.hasVehicle()) {
            ...
            if (!this.canStay(blockPos, direction = this.getAttachedFace())) {
                Direction direction = this.findAttachSide(blockPos);
                if (direction != null) {
        ...
        if (blockPos != null) {
            ...
            _snowman = 0.5 - (double)MathHelper.sin((0.5f + this.prevOpenProgress) * (float)Math.PI) * 0.5;
            Direction _snowman2 = this.getAttachedFace().getOpposite();
            this.setBoundingBox(new Box(this.getX() - 0.5, this.getY(), this.getZ() - 0.5, this.getX() + 0.5, this.getY() + 1.0, this.getZ() + 0.5).stretch((double)_snowman2.getOffsetX() * d, (double)_snowman2.getOffsetY() * d, (double)_snowman2.getOffsetZ() * d));
            _snowman = d - _snowman;
            if (_snowman > 0.0 && !(_snowman = this.world.getOtherEntities(this, this.getBoundingBox())).isEmpty()) {
                for (Entity entity : _snowman) {
                    if (entity instanceof ShulkerEntity || entity.noClip) continue;
                    entity.move(MovementType.SHULKER, new Vec3d(_snowman * (double)_snowman2.getOffsetX(), _snowman * (double)_snowman2.getOffsetY(), _snowman * (double)_snowman2.getOffsetZ()));
                }
            }
        }
    }
    ...
    public void updatePosition(double x, double y, double z) {
        ...
        _snowman = Optional.of(new BlockPos(x, y, z));
        if (!_snowman.equals(optional)) {
            this.dataTracker.set(ATTACHED_BLOCK, _snowman);
    ...
    protected boolean tryTeleport() {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (blockPos2.getY() <= 0 || !this.world.isAir(blockPos2) || !this.world.getWorldBorder().contains(blockPos2) || !this.world.isSpaceEmpty(this, new Box(blockPos2)) || (_snowman = this.findAttachSide(blockPos2)) == null) continue;
            this.dataTracker.set(ATTACHED_FACE, _snowman);
    ...
    public DyeColor getColor() {
        Byte by = this.dataTracker.get(COLOR);
        if (by == 16 || by > 15) {
            return null;
        }
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            double _snowman2 = ShulkerEntity.this.squaredDistanceTo(livingEntity);
            if (_snowman2 < 400.0) {
                if (this.counter <= 0) {
