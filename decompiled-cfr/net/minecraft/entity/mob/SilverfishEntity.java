    ...
    extends WanderAroundGoal {
        ...
        public boolean canStart() {
            ...
            if (this.mob.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING) && random.nextInt(10) == 0) {
                ...
                BlockState _snowman2 = this.mob.world.getBlockState(blockPos);
                if (InfestedBlock.isInfestable(_snowman2)) {
                    this.canInfest = true;
                    return true;
                }
            }
        ...
        public void start() {
            ...
            BlockPos _snowman2 = new BlockPos(this.mob.getX(), this.mob.getY() + 0.5, this.mob.getZ()).offset(this.direction);
            BlockState _snowman3 = world.getBlockState(_snowman2);
            if (InfestedBlock.isInfestable(_snowman3)) {
                world.setBlockState(_snowman2, InfestedBlock.fromRegularBlock(_snowman3.getBlock()), 3);
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            if (this.delay <= 0) {
                ...
                Random _snowman2 = this.silverfish.getRandom();
                BlockPos _snowman3 = this.silverfish.getBlockPos();
                int _snowman4 = 0;
                block0: while (_snowman4 <= 5 && _snowman4 >= -5) {
                    int n = 0;
                    while (n <= 10 && n >= -10) {
                        _snowman7 = 0;
                        while (_snowman7 <= 10 && _snowman7 >= -10) {
                            BlockPos blockPos = _snowman3.add(n, _snowman4, _snowman7);
                            BlockState _snowman5 = world.getBlockState(blockPos);
                            Block _snowman6 = _snowman5.getBlock();
                            if (_snowman6 instanceof InfestedBlock) {
                                if (world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                                    world.breakBlock(blockPos, true, this.silverfish);
                                } else {
                                    world.setBlockState(blockPos, ((InfestedBlock)_snowman6).getRegularBlock().getDefaultState(), 3);
                                }
                                if (_snowman2.nextBoolean()) break block0;
                            }
                            int _snowman7 = (_snowman7 <= 0 ? 1 : 0) - _snowman7;
                        }
                        n = (n <= 0 ? 1 : 0) - n;
                    }
                    _snowman4 = (_snowman4 <= 0 ? 1 : 0) - _snowman4;
                }
            }
        }
    }
