...
extends IllagerEntity {
    ...
    public void tick() {
        ...
        if (this.world.isClient && this.isSpellcasting()) {
            ...
            double _snowman2 = spell.particleVelocity[0];
            double _snowman3 = spell.particleVelocity[1];
            double _snowman4 = spell.particleVelocity[2];
            float _snowman5 = this.bodyYaw * ((float)Math.PI / 180) + MathHelper.cos((float)this.age * 0.6662f) * 0.25f;
            float _snowman6 = MathHelper.cos(_snowman5);
            float _snowman7 = MathHelper.sin(_snowman5);
            this.world.addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() + (double)_snowman6 * 0.6, this.getY() + 1.8, this.getZ() + (double)_snowman7 * 0.6, _snowman2, _snowman3, _snowman4);
            this.world.addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() - (double)_snowman6 * 0.6, this.getY() + 1.8, this.getZ() - (double)_snowman7 * 0.6, _snowman2, _snowman3, _snowman4);
        }
    }
