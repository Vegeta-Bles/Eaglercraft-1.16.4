...
extends HostileEntity {
    ...
    private boolean areFlagsSet(int mask) {
        byte by = this.dataTracker.get(VEX_FLAGS);
        return (by & mask) != 0;
    }

    private void setVexFlag(int mask, boolean value) {
        int n = this.dataTracker.get(VEX_FLAGS).byteValue();
        n = value ? (n |= mask) : (n &= ~mask);
        this.dataTracker.set(VEX_FLAGS, (byte)(n & 0xFF));
    }
    ...
    extends Goal {
        ...
        public void start() {
            ...
            Vec3d _snowman2 = livingEntity.getCameraPosVec(1.0f);
            VexEntity.this.moveControl.moveTo(_snowman2.x, _snowman2.y, _snowman2.z, 1.0);
    ...
    extends MoveControl {
        ...
        public void tick() {
            ...
            if (_snowman2 < VexEntity.this.getBoundingBox().getAverageSideLength()) {
                this.state = MoveControl.State.WAIT;
                VexEntity.this.setVelocity(VexEntity.this.getVelocity().multiply(0.5));
            } else {
                VexEntity.this.setVelocity(VexEntity.this.getVelocity().add(vec3d.multiply(this.speed * 0.05 / _snowman2)));
                if (VexEntity.this.getTarget() == null) {
                    _snowman = VexEntity.this.getVelocity();
                    VexEntity.this.bodyYaw = VexEntity.this.yaw = -((float)MathHelper.atan2(_snowman.x, _snowman.z)) * 57.295776f;
                } else {
                    double d = VexEntity.this.getTarget().getX() - VexEntity.this.getX();
                    _snowman = VexEntity.this.getTarget().getZ() - VexEntity.this.getZ();
                    VexEntity.this.bodyYaw = VexEntity.this.yaw = -((float)MathHelper.atan2(d, _snowman)) * 57.295776f;
                }
            }
        }
    }
