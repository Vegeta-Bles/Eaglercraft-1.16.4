...
implements RangedAttackMob {
        ...
        if (!this.world.isClient && this.isAlive()) {
            ...
            if (this.isDrinking()) {
                if (this.drinkTimeLeft-- <= 0) {
                    ...
                    if (itemStack.getItem() == Items.POTION && (_snowman = PotionUtil.getPotionEffects(itemStack)) != null) {
                        for (StatusEffectInstance statusEffectInstance : _snowman) {
    ...
    public void attack(LivingEntity target, float pullProgress) {
        ...
        double _snowman2 = target.getX() + vec3d.x - this.getX();
        double _snowman3 = target.getEyeY() - (double)1.1f - this.getY();
        double _snowman4 = target.getZ() + vec3d.z - this.getZ();
        float _snowman5 = MathHelper.sqrt(_snowman2 * _snowman2 + _snowman4 * _snowman4);
        Potion _snowman6 = Potions.HARMING;
        if (target instanceof RaiderEntity) {
            _snowman6 = target.getHealth() <= 4.0f ? Potions.HEALING : Potions.REGENERATION;
            this.setTarget(null);
        } else if (_snowman5 >= 8.0f && !target.hasStatusEffect(StatusEffects.SLOWNESS)) {
            _snowman6 = Potions.SLOWNESS;
        } else if (target.getHealth() >= 8.0f && !target.hasStatusEffect(StatusEffects.POISON)) {
            _snowman6 = Potions.POISON;
        } else if (_snowman5 <= 3.0f && !target.hasStatusEffect(StatusEffects.WEAKNESS) && this.random.nextFloat() < 0.25f) {
            _snowman6 = Potions.WEAKNESS;
        }
        PotionEntity _snowman7 = new PotionEntity(this.world, this);
        _snowman7.setItem(PotionUtil.setPotion(new ItemStack(Items.SPLASH_POTION), _snowman6));
        _snowman7.pitch -= -20.0f;
        _snowman7.setVelocity(_snowman2, _snowman3 + (double)(_snowman5 * 0.2f), _snowman4, 0.75f, 8.0f);
        if (!this.isSilent()) {
        ...
        this.world.spawnEntity(_snowman7);
    }
