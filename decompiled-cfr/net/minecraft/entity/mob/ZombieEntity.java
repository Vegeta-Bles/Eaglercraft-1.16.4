...
extends HostileEntity {
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        LivingEntity _snowman2 = this.getTarget();
        if (_snowman2 == null && source.getAttacker() instanceof LivingEntity) {
            _snowman2 = (LivingEntity)source.getAttacker();
        }
        if (_snowman2 != null && this.world.getDifficulty() == Difficulty.HARD && (double)this.random.nextFloat() < this.getAttributeValue(EntityAttributes.ZOMBIE_SPAWN_REINFORCEMENTS) && this.world.getGameRules().getBoolean(GameRules.DO_MOB_SPAWNING)) {
            int n = MathHelper.floor(this.getX());
            _snowman = MathHelper.floor(this.getY());
            _snowman = MathHelper.floor(this.getZ());
            ZombieEntity _snowman3 = new ZombieEntity(this.world);
            for (_snowman = 0; _snowman < 50; ++_snowman) {
                _snowman = n + MathHelper.nextInt(this.random, 7, 40) * MathHelper.nextInt(this.random, -1, 1);
                _snowman = _snowman + MathHelper.nextInt(this.random, 7, 40) * MathHelper.nextInt(this.random, -1, 1);
                _snowman = _snowman + MathHelper.nextInt(this.random, 7, 40) * MathHelper.nextInt(this.random, -1, 1);
                BlockPos blockPos = new BlockPos(_snowman, _snowman, _snowman);
                EntityType<?> _snowman4 = _snowman3.getType();
                SpawnRestriction.Location _snowman5 = SpawnRestriction.getLocation(_snowman4);
                if (!SpawnHelper.canSpawn(_snowman5, this.world, blockPos, _snowman4) || !SpawnRestriction.canSpawn(_snowman4, serverWorld, SpawnReason.REINFORCEMENT, blockPos, this.world.random)) continue;
                _snowman3.updatePosition(_snowman, _snowman, _snowman);
                if (this.world.isPlayerInRange(_snowman, _snowman, _snowman, 7.0) || !this.world.intersectsEntities(_snowman3) || !this.world.isSpaceEmpty(_snowman3) || this.world.containsFluid(_snowman3.getBoundingBox())) continue;
                _snowman3.setTarget(_snowman2);
                _snowman3.initialize(serverWorld, this.world.getLocalDifficulty(_snowman3.getBlockPos()), SpawnReason.REINFORCEMENT, null, null);
                serverWorld.spawnEntityAndPassengers(_snowman3);
                ...
                _snowman3.getAttributeInstance(EntityAttributes.ZOMBIE_SPAWN_REINFORCEMENTS).addPersistentModifier(new EntityAttributeModifier("Zombie reinforcement callee charge", -0.05f, EntityAttributeModifier.Operation.ADDITION));
                break;
            }
        }
        return true;
    ...
    public void onKilledOther(ServerWorld serverWorld, LivingEntity livingEntity) {
        ...
        if ((serverWorld.getDifficulty() == Difficulty.NORMAL || serverWorld.getDifficulty() == Difficulty.HARD) && livingEntity instanceof VillagerEntity) {
            ...
            ZombieVillagerEntity _snowman2 = villagerEntity.method_29243(EntityType.ZOMBIE_VILLAGER, false);
            _snowman2.initialize(serverWorld, serverWorld.getLocalDifficulty(_snowman2.getBlockPos()), SpawnReason.CONVERSION, new ZombieData(false, true), null);
            _snowman2.setVillagerData(villagerEntity.getVillagerData());
            _snowman2.setGossipData((Tag)villagerEntity.getGossip().serialize(NbtOps.INSTANCE).getValue());
            _snowman2.setOfferData(villagerEntity.getOffers().toTag());
            _snowman2.setXp(villagerEntity.getExperience());
            if (!this.isSilent()) {
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
        ...
        if (entityData instanceof ZombieData) {
            object = (ZombieData)entityData;
            if (((ZombieData)object).baby) {
                this.setBaby(true);
                if (((ZombieData)object).field_25607) {
                    List<Entity> list;
                    if ((double)world.getRandom().nextFloat() < 0.05) {
                        list = world.getEntitiesByClass(ChickenEntity.class, this.getBoundingBox().expand(5.0, 3.0, 5.0), EntityPredicates.NOT_MOUNTED);
                        if (!list.isEmpty()) {
                        ...
                        list = EntityType.CHICKEN.create(this.world);
                        ((Entity)((Object)list)).refreshPositionAndAngles(this.getX(), this.getY(), this.getZ(), this.yaw, 0.0f);
                        ((PassiveEntity)((Object)list)).initialize(world, difficulty, SpawnReason.JOCKEY, null, null);
                        ((ChickenEntity)((Object)list)).setHasJockey(true);
                        this.startRiding((Entity)((Object)list));
                        world.spawnEntity((Entity)((Object)list));
                    }
                }
            }
        ...
        if (this.getEquippedStack(EquipmentSlot.HEAD).isEmpty()) {
            ...
            int _snowman2 = ((LocalDate)object).get(ChronoField.DAY_OF_MONTH);
            int _snowman3 = ((LocalDate)object).get(ChronoField.MONTH_OF_YEAR);
    ...
    protected void dropEquipment(DamageSource source, int lootingMultiplier, boolean allowDrops) {
        ...
        if (entity instanceof CreeperEntity && (_snowman = (CreeperEntity)entity).shouldDropHead() && !(_snowman = this.getSkull()).isEmpty()) {
            _snowman.onHeadDropped();
