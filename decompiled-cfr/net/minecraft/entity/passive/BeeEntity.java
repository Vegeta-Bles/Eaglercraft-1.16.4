...
Flutterer {
    ...
    private void startMovingTo(BlockPos pos) {
        ...
        int _snowman4 = (int)vec3d.y - _snowman3.getY();
        if (_snowman4 > 2) {
            _snowman2 = 4;
        } else if (_snowman4 < -2) {
            _snowman2 = -4;
        ...
        int _snowman5 = 6;
        int _snowman6 = 8;
        int _snowman7 = _snowman3.getManhattanDistance(pos);
        if (_snowman7 < 15) {
            _snowman5 = _snowman7 / 2;
            _snowman6 = _snowman7 / 2;
        }
        if ((_snowman = TargetFinder.findGroundTargetTowards(this, _snowman5, _snowman6, _snowman2, vec3d, 0.3141592741012573)) == null) {
            return;
        }
        this.navigation.setRangeMultiplier(0.5f);
        this.navigation.startMovingTo(_snowman.x, _snowman.y, _snowman.z, 1.0);
    }
    ...
    protected EntityNavigation createNavigation(World world) {
        BirdNavigation birdNavigation = new BirdNavigation(this, this, world){
            final /* synthetic */ BeeEntity field_20364;
            {
                this.field_20364 = beeEntity;
            ...
            public void tick() {
                if (PollinateGoal.method_23746(BeeEntity.method_23738(this.field_20364))) {
                    return;
                }
                super.tick();
            }
    ...
    extends NotAngryGoal {
        ...
        public void tick() {
            ...
            for (int i = 1; i <= 2; ++i) {
                ...
                BlockState _snowman2 = BeeEntity.this.world.getBlockState(blockPos);
                Block _snowman3 = _snowman2.getBlock();
                boolean _snowman4 = false;
                IntProperty _snowman5 = null;
                if (!_snowman3.isIn(BlockTags.BEE_GROWABLES)) continue;
                if (_snowman3 instanceof CropBlock) {
                    CropBlock cropBlock = (CropBlock)_snowman3;
                    if (!cropBlock.isMature(_snowman2)) {
                        _snowman4 = true;
                        _snowman5 = cropBlock.getAgeProperty();
                    }
                } else if (_snowman3 instanceof StemBlock) {
                    int n2 = _snowman2.get(StemBlock.AGE);
                    if (n2 < 7) {
                        _snowman4 = true;
                        _snowman5 = StemBlock.AGE;
                    }
                } else if (_snowman3 == Blocks.SWEET_BERRY_BUSH && (n = _snowman2.get(SweetBerryBushBlock.AGE).intValue()) < 3) {
                    _snowman4 = true;
                    _snowman5 = SweetBerryBushBlock.AGE;
                }
                if (!_snowman4) continue;
                BeeEntity.this.world.syncWorldEvent(2005, blockPos, 0);
                BeeEntity.this.world.setBlockState(blockPos, (BlockState)_snowman2.with(_snowman5, _snowman2.get(_snowman5) + 1));
    ...
    extends NotAngryGoal {
        ...
        private List<BlockPos> getNearbyFreeHives() {
            ...
            PointOfInterestStorage _snowman2 = ((ServerWorld)BeeEntity.this.world).getPointOfInterestStorage();
            Stream<PointOfInterest> _snowman3 = _snowman2.getInCircle(pointOfInterestType -> pointOfInterestType == PointOfInterestType.BEEHIVE || pointOfInterestType == PointOfInterestType.BEE_NEST, blockPos3, 20, PointOfInterestStorage.OccupationStatus.ANY);
            return _snowman3.map(PointOfInterest::getPos).filter(blockPos -> BeeEntity.this.doesHiveHaveSpace(blockPos)).sorted(Comparator.comparingDouble(blockPos2 -> blockPos2.getSquaredDistance(blockPos3))).collect(Collectors.toList());
        }
    }
    ...
    extends NotAngryGoal {
        ...
        public void tick() {
            ...
            boolean _snowman2 = BeeEntity.this.getPos().distanceTo(this.nextTarget) <= 0.1;
            boolean _snowman3 = true;
            if (!_snowman2 && this.ticks > 600) {
                BeeEntity.this.flowerPos = null;
                return;
            }
            if (_snowman2) {
                boolean bl = _snowman = BeeEntity.this.random.nextInt(25) == 0;
                if (_snowman) {
        ...
        private Optional<BlockPos> findFlower(Predicate<BlockState> predicate, double searchDistance) {
            ...
            int _snowman3 = 0;
            while ((double)_snowman3 <= searchDistance) {
                int n = 0;
                while ((double)n < searchDistance) {
                    _snowman = 0;
                    while (_snowman <= n) {
                        int n2 = _snowman = _snowman < n && _snowman > -n ? n : 0;
                        while (_snowman <= n) {
                            _snowman2.set(blockPos, _snowman, _snowman3 - 1, _snowman);
                            if (blockPos.isWithinDistance(_snowman2, searchDistance) && predicate.test(BeeEntity.this.world.getBlockState(_snowman2))) {
                                return Optional.of(_snowman2);
                            }
                ...
                _snowman3 = _snowman3 > 0 ? -_snowman3 : 1 - _snowman3;
            }
    ...
    extends Goal {
        ...
        private Vec3d getRandomLocation() {
            if (BeeEntity.this.isHiveValid() && !BeeEntity.this.isWithinDistance(BeeEntity.this.hivePos, 22)) {
                ...
                vec3d = vec3d.subtract(BeeEntity.this.getPos()).normalize();
            } else {
                Vec3d vec3d = BeeEntity.this.getRotationVec(0.0f);
            }
            int n = 8;
            Vec3d _snowman2 = TargetFinder.findAirTarget(BeeEntity.this, 8, 7, vec3d, 1.5707964f, 2, 1);
            if (_snowman2 != null) {
                return _snowman2;
            }
            return TargetFinder.findGroundTarget(BeeEntity.this, 8, 4, -2, vec3d, 1.5707963705062866);
        }
    }
