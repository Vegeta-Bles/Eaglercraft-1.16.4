...
extends TameableEntity {
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
        ...
        if (serverWorld instanceof ServerWorld && serverWorld.getStructureAccessor().getStructureAt(this.getBlockPos(), true, StructureFeature.SWAMP_HUT).hasChildren()) {
            this.setCatType(10);
            this.setPersistent();
        }
        return entityData;
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.isTamed()) {
            if (this.isOwner(player)) {
                if (_snowman2 instanceof DyeItem) {
                    Enum enum_ = ((DyeItem)_snowman2).getColor();
                    ...
                    if (_snowman2.isFood() && this.isBreedingItem(itemStack) && this.getHealth() < this.getMaxHealth()) {
                        this.eat(player, itemStack);
                        this.heal(_snowman2.getFoodComponent().getHunger());
                        return ActionResult.CONSUME;
                    }
                    ActionResult _snowman3 = super.interactMob(player, hand);
                    if (!_snowman3.isAccepted() || this.isBaby()) {
        ...
        if (((ActionResult)(enum_ = super.interactMob(player, hand))).isAccepted()) {
            this.setPersistent();
        }
        return enum_;
    }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if (livingEntity instanceof PlayerEntity) {
                ...
                BlockState _snowman2 = this.cat.world.getBlockState(blockPos);
                if (_snowman2.getBlock().isIn(BlockTags.BEDS)) {
                    this.bedPos = _snowman2.method_28500(BedBlock.FACING).map(direction -> blockPos.offset(direction.getOpposite())).orElseGet(() -> new BlockPos(blockPos));
        ...
        private void dropMorningGifts() {
            ...
            _snowman2.set(this.cat.getBlockPos());
            this.cat.teleport(_snowman2.getX() + random.nextInt(11) - 5, _snowman2.getY() + random.nextInt(5) - 2, _snowman2.getZ() + random.nextInt(11) - 5, false);
            _snowman2.set(this.cat.getBlockPos());
            LootTable _snowman3 = this.cat.world.getServer().getLootManager().getTable(LootTables.CAT_MORNING_GIFT_GAMEPLAY);
            LootContext.Builder _snowman4 = new LootContext.Builder((ServerWorld)this.cat.world).parameter(LootContextParameters.ORIGIN, this.cat.getPos()).parameter(LootContextParameters.THIS_ENTITY, this.cat).random(random);
            List<ItemStack> _snowman5 = _snowman3.generateLoot(_snowman4.build(LootContextTypes.GIFT));
            for (ItemStack itemStack : _snowman5) {
                this.cat.world.spawnEntity(new ItemEntity(this.cat.world, (double)_snowman2.getX() - (double)MathHelper.sin(this.cat.bodyYaw * ((float)Math.PI / 180)), _snowman2.getY(), (double)_snowman2.getZ() + (double)MathHelper.cos(this.cat.bodyYaw * ((float)Math.PI / 180)), itemStack));
            }
        }
