...
extends AnimalEntity {
        ...
        if (!this.world.isClient && this.isAlive() && this.canMoveVoluntarily()) {
            ...
            if (this.canEat(itemStack)) {
                if (this.eatingTime > 600) {
                    object = itemStack.finishUsing(this.world, this);
                    if (!((ItemStack)object).isEmpty()) {
                        this.equipStack(EquipmentSlot.MAINHAND, (ItemStack)object);
                    }
                    this.eatingTime = 0;
    ...
    protected void initEquipment(LocalDifficulty difficulty) {
        if (this.random.nextFloat() < 0.2f) {
            ...
            ItemStack _snowman2 = f < 0.05f ? new ItemStack(Items.EMERALD) : (f < 0.2f ? new ItemStack(Items.EGG) : (f < 0.4f ? (this.random.nextBoolean() ? new ItemStack(Items.RABBIT_FOOT) : new ItemStack(Items.RABBIT_HIDE)) : (f < 0.6f ? new ItemStack(Items.WHEAT) : (f < 0.8f ? new ItemStack(Items.LEATHER) : new ItemStack(Items.FEATHER)))));
            this.equipStack(EquipmentSlot.MAINHAND, _snowman2);
        }
    }
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
        ...
        Type _snowman2 = Type.fromBiome(optional);
        boolean _snowman3 = false;
        if (entityData instanceof FoxData) {
            _snowman2 = ((FoxData)entityData).type;
            if (((FoxData)entityData).getSpawnedCount() >= 2) {
                _snowman3 = true;
            }
        } else {
            entityData = new FoxData(_snowman2);
        }
        this.setType(_snowman2);
        if (_snowman3) {
    ...
    private List<UUID> getTrustedUuids() {
        ...
        arrayList.add(this.dataTracker.get(OWNER).orElse(null));
        arrayList.add(this.dataTracker.get(OTHER_TRUSTED).orElse(null));
        return arrayList;
    }
    ...
    public boolean canPickupItem(ItemStack stack) {
        ...
        ItemStack _snowman2 = this.getEquippedStack(EquipmentSlot.MAINHAND);
        return _snowman2.isEmpty() || this.eatingTime > 0 && item.isFood() && !_snowman2.getItem().isFood();
    }
    ...
    protected void loot(ItemEntity item) {
        ...
        if (this.canPickupItem(itemStack)) {
            ...
            if (n > 1) {
                this.dropItem(itemStack.split(n - 1));
            }
    ...
    public void tick() {
        ...
        if (this.canMoveVoluntarily()) {
            ...
            if (this.isWalking() && this.world.random.nextFloat() < 0.2f) {
                ...
                this.world.syncWorldEvent(2001, blockPos, Block.getRawIdFromState(_snowman2));
            }
        }
    ...
    public static boolean canJumpChase(FoxEntity fox, LivingEntity chasedEntity) {
        ...
        _snowman = chasedEntity.getX() - fox.getX();
        _snowman = d / _snowman;
        int _snowman2 = 6;
        for (int i = 0; i < 6; ++i) {
            double d2 = _snowman == 0.0 ? 0.0 : d * (double)((float)i / 6.0f);
            _snowman = _snowman == 0.0 ? _snowman * (double)((float)i / 6.0f) : d2 / _snowman;
            for (int j = 1; j < 4; ++j) {
                if (fox.world.getBlockState(new BlockPos(fox.getX() + _snowman, fox.getY() + (double)j, fox.getZ() + d2)).getMaterial().isReplaceable()) continue;
                return false;
            }
        }
        return true;
    ...
    extends DiveJumpingGoal {
        ...
        public boolean canStart() {
            ...
            boolean _snowman2 = FoxEntity.canJumpChase(FoxEntity.this, livingEntity);
            if (!_snowman2) {
        ...
        public boolean shouldContinue() {
            ...
            double _snowman2 = FoxEntity.this.getVelocity().y;
            return !(_snowman2 * _snowman2 < (double)0.05f && Math.abs(FoxEntity.this.pitch) < 15.0f && FoxEntity.this.onGround || FoxEntity.this.isWalking());
        }
        ...
        public void start() {
            ...
            Vec3d _snowman2 = new Vec3d(livingEntity.getX() - FoxEntity.this.getX(), livingEntity.getY() - FoxEntity.this.getY(), livingEntity.getZ() - FoxEntity.this.getZ()).normalize();
            FoxEntity.this.setVelocity(FoxEntity.this.getVelocity().add(_snowman2.x * 0.8, 0.9, _snowman2.z * 0.8));
        ...
        public void tick() {
            ...
            if (!FoxEntity.this.isWalking()) {
                ...
                if (vec3d.y * vec3d.y < (double)0.03f && FoxEntity.this.pitch != 0.0f) {
                    ...
                    _snowman = Math.signum(-vec3d.y) * Math.acos(d / vec3d.length()) * 57.2957763671875;
                    FoxEntity.this.pitch = (float)_snowman;
                }
            }
    ...
    extends MoveToTargetPosGoal {
        ...
        protected void eatSweetBerry() {
            ...
            int _snowman2 = blockState.get(SweetBerryBushBlock.AGE);
            blockState.with(SweetBerryBushBlock.AGE, 1);
            int _snowman3 = 1 + FoxEntity.this.world.random.nextInt(2) + (_snowman2 == 3 ? 1 : 0);
            ItemStack _snowman4 = FoxEntity.this.getEquippedStack(EquipmentSlot.MAINHAND);
            if (_snowman4.isEmpty()) {
                FoxEntity.this.equipStack(EquipmentSlot.MAINHAND, new ItemStack(Items.SWEET_BERRIES));
                --_snowman3;
            }
            if (_snowman3 > 0) {
                Block.dropStack(FoxEntity.this.world, this.targetPos, new ItemStack(Items.SWEET_BERRIES, _snowman3));
            }
    ...
    extends FollowTargetGoal<LivingEntity> {
        ...
        public boolean canStart() {
            ...
            for (UUID uUID : FoxEntity.this.getTrustedUuids()) {
                if (uUID == null || !(FoxEntity.this.world instanceof ServerWorld) || !((_snowman = ((ServerWorld)FoxEntity.this.world).getEntity(uUID)) instanceof LivingEntity)) continue;
                this.friend = _snowman = (LivingEntity)_snowman;
                this.offender = _snowman.getAttacker();
                int n = _snowman.getLastAttackedTime();
                return n != this.lastAttackedTime && this.canTrack(this.offender, this.targetPredicate);
            }
            return false;
        }
    ...
    extends AnimalMateGoal {
        ...
        protected void breed() {
            ...
            FoxEntity _snowman2 = (FoxEntity)this.animal.createChild(serverWorld, this.mate);
            if (_snowman2 == null) {
                return;
            }
            ServerPlayerEntity _snowman3 = this.animal.getLovingPlayer();
            ServerPlayerEntity _snowman4 = this.mate.getLovingPlayer();
            ServerPlayerEntity _snowman5 = _snowman3;
            if (_snowman3 != null) {
                _snowman2.addTrustedUuid(_snowman3.getUuid());
            } else {
                _snowman5 = _snowman4;
            }
            if (_snowman4 != null && _snowman3 != _snowman4) {
                _snowman2.addTrustedUuid(_snowman4.getUuid());
            }
            if (_snowman5 != null) {
                _snowman5.incrementStat(Stats.ANIMALS_BRED);
                Criteria.BRED_ANIMALS.trigger(_snowman5, this.animal, this.mate, _snowman2);
            }
            this.animal.setBreedingAge(6000);
            this.mate.setBreedingAge(6000);
            this.animal.resetLoveTicks();
            this.mate.resetLoveTicks();
            _snowman2.setBreedingAge(-24000);
            _snowman2.refreshPositionAndAngles(this.animal.getX(), this.animal.getY(), this.animal.getZ(), 0.0f, 0.0f);
            serverWorld.spawnEntityAndPassengers(_snowman2);
    ...
    extends Goal {
        ...
        public void tick() {
            List<ItemEntity> list = FoxEntity.this.world.getEntitiesByClass(ItemEntity.class, FoxEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0), PICKABLE_DROP_FILTER);
            ItemStack _snowman2 = FoxEntity.this.getEquippedStack(EquipmentSlot.MAINHAND);
            if (_snowman2.isEmpty() && !list.isEmpty()) {
    ...
    public static enum Type {
        ...
        private Type(int id, String key, RegistryKey<Biome> ... registryKeyArray) {
            this.id = id;
            this.key = key;
            this.biomes = Arrays.asList(registryKeyArray);
        }
