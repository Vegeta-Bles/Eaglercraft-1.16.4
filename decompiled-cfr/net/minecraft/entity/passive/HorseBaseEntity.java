...
Saddleable {
    ...
    protected void setHorseFlag(int bitmask, boolean flag) {
        byte by = this.dataTracker.get(HORSE_FLAGS);
        if (flag) {
            this.dataTracker.set(HORSE_FLAGS, (byte)(by | bitmask));
        } else {
            this.dataTracker.set(HORSE_FLAGS, (byte)(by & ~bitmask));
        }
    }
    ...
    public int addTemper(int difference) {
        int n = MathHelper.clamp(this.getTemper() + difference, 0, this.getMaxTemper());
        this.setTemper(n);
        return n;
    }
    ...
    public boolean handleFallDamage(float fallDistance, float damageMultiplier) {
        ...
        if ((n = this.computeFallDamage(fallDistance, damageMultiplier)) <= 0) {
            return false;
        }
    ...
    protected void onChestedStatusChanged() {
        ...
        if (simpleInventory != null) {
            ...
            int n = Math.min(simpleInventory.size(), this.items.size());
            for (_snowman = 0; _snowman < n; ++_snowman) {
                ItemStack itemStack = simpleInventory.getStack(_snowman);
                if (itemStack.isEmpty()) continue;
                this.items.setStack(_snowman, itemStack.copy());
            }
        }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        ...
        BlockSoundGroup _snowman2 = state.getSoundGroup();
        if (blockState.isOf(Blocks.SNOW)) {
            _snowman2 = blockState.getSoundGroup();
        }
        if (this.hasPassengers() && this.playExtraHorseSounds) {
            ++this.soundTicks;
            if (this.soundTicks > 5 && this.soundTicks % 3 == 0) {
                this.playWalkSound(_snowman2);
            } else if (this.soundTicks <= 5) {
                this.playSound(SoundEvents.ENTITY_HORSE_STEP_WOOD, _snowman2.getVolume() * 0.15f, _snowman2.getPitch());
            }
        } else if (_snowman2 == BlockSoundGroup.WOOD) {
            this.playSound(SoundEvents.ENTITY_HORSE_STEP_WOOD, _snowman2.getVolume() * 0.15f, _snowman2.getPitch());
        } else {
            this.playSound(SoundEvents.ENTITY_HORSE_STEP, _snowman2.getVolume() * 0.15f, _snowman2.getPitch());
        }
    }
    ...
    protected boolean receiveFood(PlayerEntity player, ItemStack item) {
        ...
        int _snowman3 = 0;
        int _snowman4 = 0;
        Item _snowman5 = item.getItem();
        if (_snowman5 == Items.WHEAT) {
            _snowman2 = 2.0f;
            _snowman3 = 20;
            _snowman4 = 3;
        } else if (_snowman5 == Items.SUGAR) {
            _snowman2 = 1.0f;
            _snowman3 = 30;
            _snowman4 = 3;
        } else if (_snowman5 == Blocks.HAY_BLOCK.asItem()) {
            _snowman2 = 20.0f;
            _snowman3 = 180;
        } else if (_snowman5 == Items.APPLE) {
            _snowman2 = 3.0f;
            _snowman3 = 60;
            _snowman4 = 3;
        } else if (_snowman5 == Items.GOLDEN_CARROT) {
            _snowman2 = 4.0f;
            _snowman3 = 60;
            _snowman4 = 5;
        ...
        } else if (_snowman5 == Items.GOLDEN_APPLE || _snowman5 == Items.ENCHANTED_GOLDEN_APPLE) {
            _snowman2 = 10.0f;
            _snowman3 = 240;
            _snowman4 = 10;
            if (!this.world.isClient && this.isTame() && this.getBreedingAge() == 0 && !this.isInLove()) {
        ...
        if (this.isBaby() && _snowman3 > 0) {
            this.world.addParticle(ParticleTypes.HAPPY_VILLAGER, this.getParticleX(1.0), this.getRandomBodyY() + 0.5, this.getParticleZ(1.0), 0.0, 0.0, 0.0);
            if (!this.world.isClient) {
                this.growUp(_snowman3);
            }
            bl = true;
        }
        if (_snowman4 > 0 && (bl || !this.isTame()) && this.getTemper() < this.getMaxTemper()) {
            bl = true;
            if (!this.world.isClient) {
                this.addTemper(_snowman4);
            }
        }
        if (bl) {
    ...
    protected void walkToParent() {
        ...
        if (this.isBred() && this.isBaby() && !this.isEatingGrass() && (horseBaseEntity = this.world.getClosestEntity(HorseBaseEntity.class, PARENT_HORSE_PREDICATE, this, this.getX(), this.getY(), this.getZ(), this.getBoundingBox().expand(16.0))) != null && this.squaredDistanceTo(horseBaseEntity) > 4.0) {
            this.navigation.findPathTo(horseBaseEntity, 0);
        }
    }
    ...
    public void travel(Vec3d movementInput) {
        ...
        float _snowman2 = livingEntity.sidewaysSpeed * 0.5f;
        float _snowman3 = livingEntity.forwardSpeed;
        if (_snowman3 <= 0.0f) {
            _snowman3 *= 0.25f;
            this.soundTicks = 0;
        }
        ...
        if (this.jumpStrength > 0.0f && !this.isInAir() && this.onGround) {
            ...
            _snowman = this.hasStatusEffect(StatusEffects.JUMP_BOOST) ? d + (double)((float)(this.getStatusEffect(StatusEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1f) : d;
            Vec3d _snowman4 = this.getVelocity();
            this.setVelocity(_snowman4.x, _snowman, _snowman4.z);
            this.setInAir(true);
            this.velocityDirty = true;
            if (_snowman3 > 0.0f) {
                float f = MathHelper.sin(this.yaw * ((float)Math.PI / 180));
                _snowman = MathHelper.cos(this.yaw * ((float)Math.PI / 180));
                this.setVelocity(this.getVelocity().add(-0.4f * f * this.jumpStrength, 0.0, 0.4f * _snowman * this.jumpStrength));
            }
        ...
        if (this.isLogicalSideForUpdatingMovement()) {
            ...
            super.travel(new Vec3d(_snowman2, movementInput.y, _snowman3));
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        ...
        if (tag.containsUuid("Owner")) {
            _snowman2 = tag.getUuid("Owner");
        } else {
            Object object = tag.getString("Owner");
            _snowman2 = ServerConfigHandler.getPlayerUuidByName(this.getServer(), (String)object);
        }
        if (_snowman2 != null) {
            this.setOwnerUuid(_snowman2);
        }
        if (tag.contains("SaddleItem", 10) && ((ItemStack)(object = ItemStack.fromTag(tag.getCompound("SaddleItem")))).getItem() == Items.SADDLE) {
            this.items.setStack(0, (ItemStack)object);
        }
        this.updateSaddle();
    }
    ...
    protected void setChildAttributes(PassiveEntity mate, HorseBaseEntity child) {
        ...
        _snowman = this.getAttributeBaseValue(EntityAttributes.HORSE_JUMP_STRENGTH) + mate.getAttributeBaseValue(EntityAttributes.HORSE_JUMP_STRENGTH) + this.getChildJumpStrengthBonus();
        child.getAttributeInstance(EntityAttributes.HORSE_JUMP_STRENGTH).setBaseValue(_snowman / 3.0);
        _snowman = this.getAttributeBaseValue(EntityAttributes.GENERIC_MOVEMENT_SPEED) + mate.getAttributeBaseValue(EntityAttributes.GENERIC_MOVEMENT_SPEED) + this.getChildMovementSpeedBonus();
        child.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).setBaseValue(_snowman / 3.0);
    }
    ...
    protected void spawnPlayerReactionParticles(boolean positive) {
        DefaultParticleType defaultParticleType = positive ? ParticleTypes.HEART : ParticleTypes.SMOKE;
        for (int i = 0; i < 7; ++i) {
            double d = this.random.nextGaussian() * 0.02;
            _snowman = this.random.nextGaussian() * 0.02;
            _snowman = this.random.nextGaussian() * 0.02;
            this.world.addParticle(defaultParticleType, this.getParticleX(1.0), this.getRandomBodyY() + 0.5, this.getParticleZ(1.0), d, _snowman, _snowman);
        }
    }
    ...
    public void updatePassengerPosition(Entity passenger) {
        ...
        if (this.lastAngryAnimationProgress > 0.0f) {
            ...
            _snowman = MathHelper.cos(this.bodyYaw * ((float)Math.PI / 180));
            _snowman = 0.7f * this.lastAngryAnimationProgress;
            _snowman = 0.15f * this.lastAngryAnimationProgress;
            passenger.updatePosition(this.getX() + (double)(_snowman * f), this.getY() + this.getMountedHeightOffset() + passenger.getHeightOffset() + (double)_snowman, this.getZ() - (double)(_snowman * _snowman));
    ...
    public boolean equip(int slot, ItemStack item) {
        int n = slot - 400;
        if (n >= 0 && n < 2 && n < this.items.size()) {
            if (n == 0 && item.getItem() != Items.SADDLE) {
                return false;
            }
            if (!(n != 1 || this.hasArmorSlot() && this.isHorseArmor(item))) {
                return false;
            }
            this.items.setStack(n, item);
            this.updateSaddle();
            return true;
        }
        _snowman = slot - 500 + 2;
        if (_snowman >= 2 && _snowman < this.items.size()) {
    ...
    private Vec3d method_27930(Vec3d vec3d, LivingEntity livingEntity) {
        ...
        _snowman = this.getBoundingBox().minY;
        _snowman = this.getZ() + vec3d.z;
        BlockPos.Mutable _snowman2 = new BlockPos.Mutable();
        block0: for (EntityPose entityPose : livingEntity.getPoses()) {
            _snowman2.set(d, _snowman, _snowman);
            double d2 = this.getBoundingBox().maxY + 0.75;
            do {
                _snowman = this.world.getDismountHeight(_snowman2);
                if ((double)_snowman2.getY() + _snowman > d2) continue block0;
                if (Dismounting.canDismountInBlock(_snowman) && Dismounting.canPlaceEntityAt(this.world, livingEntity, (_snowman = livingEntity.getBoundingBox(entityPose)).offset(_snowman = new Vec3d(d, (double)_snowman2.getY() + _snowman, _snowman)))) {
                    livingEntity.setPose(entityPose);
                    return _snowman;
                }
                _snowman2.move(Direction.UP);
            } while ((double)_snowman2.getY() < d2);
        }
        return null;
    }
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        _snowman = this.method_27930(vec3d, passenger);
        if (_snowman != null) {
            return _snowman;
        }
        _snowman = HorseBaseEntity.getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), this.yaw + (passenger.getMainArm() == Arm.LEFT ? 90.0f : -90.0f));
        _snowman = this.method_27930(_snowman, passenger);
        if (_snowman != null) {
            return _snowman;
        }
        return this.getPos();
