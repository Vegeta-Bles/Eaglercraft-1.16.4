...
implements Angerable {
    ...
    public void tickMovement() {
        ...
        if (IronGolemEntity.squaredHorizontalLength(this.getVelocity()) > 2.500000277905201E-7 && this.random.nextInt(5) == 0 && !(blockState = this.world.getBlockState(new BlockPos(_snowman = MathHelper.floor(this.getX()), _snowman = MathHelper.floor(this.getY() - (double)0.2f), _snowman = MathHelper.floor(this.getZ())))).isAir()) {
    ...
    public boolean tryAttack(Entity target) {
        ...
        _snowman = (int)f > 0 ? f / 2.0f + (float)this.random.nextInt((int)f) : f;
        boolean _snowman2 = target.damage(DamageSource.mob(this), _snowman);
        if (_snowman2) {
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        boolean _snowman2 = super.damage(source, amount);
        if (_snowman2 && this.getCrack() != crack) {
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (_snowman2 != Items.IRON_INGOT) {
            return ActionResult.PASS;
        }
        float _snowman3 = this.getHealth();
        this.heal(25.0f);
        if (this.getHealth() == _snowman3) {
            return ActionResult.PASS;
        }
        float _snowman4 = 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f;
        this.playSound(SoundEvents.ENTITY_IRON_GOLEM_REPAIR, 1.0f, _snowman4);
    ...
    public void setPlayerCreated(boolean playerCreated) {
        byte by = this.dataTracker.get(IRON_GOLEM_FLAGS);
        if (playerCreated) {
            this.dataTracker.set(IRON_GOLEM_FLAGS, (byte)(by | 1));
        } else {
            this.dataTracker.set(IRON_GOLEM_FLAGS, (byte)(by & 0xFFFFFFFE));
        }
    }
    ...
    public boolean canSpawn(WorldView world) {
        ...
        BlockState _snowman2 = world.getBlockState(_snowman);
        if (_snowman2.hasSolidTopSurface(world, _snowman, this)) {
            for (int i = 1; i < 3; ++i) {
                BlockPos blockPos2 = blockPos.up(i);
                if (SpawnHelper.isClearForSpawn(world, blockPos2, _snowman = world.getBlockState(blockPos2), _snowman.getFluidState(), EntityType.IRON_GOLEM)) continue;
                return false;
            }
