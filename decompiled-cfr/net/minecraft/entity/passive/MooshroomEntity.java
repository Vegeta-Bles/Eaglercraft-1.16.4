...
implements Shearable {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (itemStack.getItem() == Items.BOWL && !this.isBaby()) {
            ...
            _snowman = ItemUsage.method_30270(itemStack, player, itemStack2, false);
            player.setStackInHand(hand, _snowman);
            SoundEvent _snowman2 = bl ? SoundEvents.ENTITY_MOOSHROOM_SUSPICIOUS_MILK : SoundEvents.ENTITY_MOOSHROOM_MILK;
            this.playSound(_snowman2, 1.0f, 1.0f);
        ...
        if (this.getMooshroomType() == Type.BROWN && itemStack.getItem().isIn(ItemTags.SMALL_FLOWERS)) {
            if (this.stewEffect != null) {
                ...
                Optional<Pair<StatusEffect, Integer>> _snowman3 = this.getStewEffectFrom(itemStack);
                if (!_snowman3.isPresent()) {
                    return ActionResult.PASS;
                }
                Pair<StatusEffect, Integer> _snowman4 = _snowman3.get();
                ...
                this.stewEffect = (StatusEffect)_snowman4.getLeft();
                this.stewEffectDuration = (Integer)_snowman4.getRight();
    ...
    private Optional<Pair<StatusEffect, Integer>> getStewEffectFrom(ItemStack flower) {
        ...
        if (item instanceof BlockItem && (_snowman = ((BlockItem)item).getBlock()) instanceof FlowerBlock) {
            FlowerBlock flowerBlock = (FlowerBlock)_snowman;
    ...
    private Type chooseBabyType(MooshroomEntity mooshroom) {
        ...
        _snowman = type == (_snowman = mooshroom.getMooshroomType()) && this.random.nextInt(1024) == 0 ? (type == Type.BROWN ? Type.RED : Type.BROWN) : (this.random.nextBoolean() ? type : _snowman);
        return _snowman;
    }
