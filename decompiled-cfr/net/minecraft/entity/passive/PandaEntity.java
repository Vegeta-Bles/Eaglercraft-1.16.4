...
extends AnimalEntity {
    ...
    private void setPandaFlag(int mask, boolean value) {
        byte by = this.dataTracker.get(PANDA_FLAGS);
        if (value) {
            this.dataTracker.set(PANDA_FLAGS, (byte)(by | mask));
        } else {
            this.dataTracker.set(PANDA_FLAGS, (byte)(by & ~mask));
        }
    }
    ...
    private void playEatingAnimation() {
        if (this.getEatingTicks() % 5 == 0) {
            ...
            for (int i = 0; i < 6; ++i) {
                ...
                _snowman = new Vec3d(((double)this.random.nextFloat() - 0.5) * 0.8, _snowman2, 1.0 + ((double)this.random.nextFloat() - 0.5) * 0.4);
                _snowman = _snowman.rotateY(-this.bodyYaw * ((float)Math.PI / 180));
                _snowman = _snowman.add(this.getX(), this.getEyeY() + 1.0, this.getZ());
                this.world.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, this.getEquippedStack(EquipmentSlot.MAINHAND)), _snowman.x, _snowman.y, _snowman.z, vec3d.x, vec3d.y + 0.05, vec3d.z);
            }
        }
    }
    ...
    private void updatePlaying() {
        ...
        if (!this.world.isClient) {
            ...
            if (this.playingTicks == 1) {
                ...
                _snowman = this.isBaby() ? 0.1f : 0.2f;
                this.playingJump = new Vec3d(vec3d.x + (double)(-MathHelper.sin(f) * _snowman), 0.0, vec3d.z + (double)(MathHelper.cos(f) * _snowman));
    ...
    private void sneeze() {
        ...
        List<PandaEntity> _snowman2 = this.world.getNonSpectatingEntities(PandaEntity.class, this.getBoundingBox().expand(10.0));
        for (PandaEntity pandaEntity : _snowman2) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.isBreedingItem(itemStack)) {
            ...
            if (this.isBaby()) {
                ...
                this.stop();
                this.setEating(true);
                _snowman = this.getEquippedStack(EquipmentSlot.MAINHAND);
                if (!_snowman.isEmpty() && !player.abilities.creativeMode) {
    ...
    extends Goal {
        ...
        public void stop() {
            ...
            if (!itemStack.isEmpty()) {
                ...
                int n = PandaEntity.this.isLazy() ? PandaEntity.this.random.nextInt(50) + 10 : PandaEntity.this.random.nextInt(150) + 10;
                this.startAge = PandaEntity.this.age + n * 20;
            }
            ...
            for (int i = 0; i < 3; ++i) {
                for (_snowman = 0; _snowman < 8; ++_snowman) {
                    ...
                    while (_snowman <= _snowman) {
                        int n = _snowman = _snowman < _snowman && _snowman > -_snowman ? _snowman : 0;
                        while (_snowman <= _snowman) {
                            _snowman2.set(blockPos, _snowman, i, _snowman);
                            if (this.world.getBlockState(_snowman2).isOf(Blocks.BAMBOO)) {
                                return true;
                            }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if ((double)Math.abs(_snowman) > 0.5) {
                _snowman2 = (int)((float)_snowman2 + _snowman / Math.abs(_snowman));
            }
            if ((double)Math.abs(_snowman) > 0.5) {
                _snowman3 = (int)((float)_snowman3 + _snowman / Math.abs(_snowman));
            }
            if (this.panda.world.getBlockState(this.panda.getBlockPos().add(_snowman2, -1, _snowman3)).isAir()) {
                return true;
            }
