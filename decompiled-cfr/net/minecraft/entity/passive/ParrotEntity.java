...
implements Flutterer {
    ...
    public static boolean imitateNearbyMob(World world, Entity parrot) {
        ...
        if (!list.isEmpty() && !(_snowman = list.get(world.random.nextInt(list.size()))).isSilent()) {
            SoundEvent soundEvent = ParrotEntity.getSound(_snowman.getType());
    ...
    public static SoundEvent getRandomSound(World world, Random random) {
        if (world.getDifficulty() != Difficulty.PEACEFUL && random.nextInt(1000) == 0) {
            ArrayList arrayList = Lists.newArrayList(MOB_SOUNDS.keySet());
            return ParrotEntity.getSound((EntityType)arrayList.get(random.nextInt(arrayList.size())));
        }
