...
extends FishEntity {
    ...
    private void sting(MobEntity mob) {
        ...
        if (mob.damage(DamageSource.mob(this), 1 + n)) {
            mob.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 60 * n, 0));
    ...
    public void onPlayerCollision(PlayerEntity player) {
        int n = this.getPuffState();
        if (player instanceof ServerPlayerEntity && n > 0 && player.damage(DamageSource.mob(this), 1 + n)) {
            if (!this.isSilent()) {
            ...
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 60 * n, 0));
        }
    }
