...
extends AnimalEntity {
    ...
    protected void jump() {
        ...
        if (d > 0.0 && (_snowman = RabbitEntity.squaredHorizontalLength(this.getVelocity())) < 0.01) {
    ...
    public void mobTick() {
        ...
        if (this.onGround) {
            ...
            if (this.getRabbitType() == 99 && this.ticksUntilJump == 0 && (object = this.getTarget()) != null && this.squaredDistanceTo((Entity)object) < 16.0) {
                this.lookTowards(((Entity)object).getX(), ((Entity)object).getZ());
                this.moveControl.moveTo(((Entity)object).getX(), ((Entity)object).getY(), ((Entity)object).getZ(), this.moveControl.getSpeed());
                this.startJump();
                this.lastOnGround = true;
            }
            if (!((RabbitJumpControl)(object = (RabbitJumpControl)this.jumpControl)).isActive()) {
                if (this.moveControl.isMoving() && this.ticksUntilJump == 0) {
                    Path path = this.navigation.getCurrentPath();
                    Vec3d _snowman2 = new Vec3d(this.moveControl.getTargetX(), this.moveControl.getTargetY(), this.moveControl.getTargetZ());
                    if (path != null && !path.isFinished()) {
                        _snowman2 = path.getNodePosition(this);
                    }
                    this.lookTowards(_snowman2.x, _snowman2.z);
                    this.startJump();
                }
            } else if (!((RabbitJumpControl)object).method_27313()) {
                this.method_6611();
            }
        }
    ...
    public RabbitEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        ...
        int _snowman2 = this.chooseType(serverWorld);
        if (this.random.nextInt(20) != 0) {
            _snowman2 = passiveEntity instanceof RabbitEntity && this.random.nextBoolean() ? ((RabbitEntity)passiveEntity).getRabbitType() : this.getRabbitType();
        }
        rabbitEntity.setRabbitType(_snowman2);
        return rabbitEntity;
    }
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
        int n = this.chooseType(world);
        if (entityData instanceof RabbitData) {
            n = ((RabbitData)entityData).type;
        } else {
            entityData = new RabbitData(n);
        }
    ...
    private int chooseType(WorldAccess world) {
        ...
        int _snowman2 = this.random.nextInt(100);
        if (biome.getPrecipitation() == Biome.Precipitation.SNOW) {
            return _snowman2 < 80 ? 1 : 3;
        }
        if (biome.getCategory() == Biome.Category.DESERT) {
            return 4;
        }
        return _snowman2 < 50 ? 0 : (_snowman2 < 90 ? 5 : 2);
    }
    ...
    extends MoveToTargetPosGoal {
        ...
        public void tick() {
            ...
            if (this.hasReached()) {
                ...
                BlockPos _snowman2 = this.targetPos.up();
                BlockState _snowman3 = world.getBlockState(_snowman2);
                Block _snowman4 = _snowman3.getBlock();
                if (this.hasTarget && _snowman4 instanceof CarrotsBlock) {
                    Integer n = _snowman3.get(CarrotsBlock.AGE);
                    if (n == 0) {
                        world.setBlockState(_snowman2, Blocks.AIR.getDefaultState(), 2);
                        world.breakBlock(_snowman2, true, this.rabbit);
                    } else {
                        world.setBlockState(_snowman2, (BlockState)_snowman3.with(CarrotsBlock.AGE, n - 1), 2);
                        world.syncWorldEvent(2001, _snowman2, Block.getRawIdFromState(_snowman3));
                    }
        ...
        protected boolean isTargetPos(WorldView world, BlockPos pos) {
            ...
            if (block == Blocks.FARMLAND && this.wantsCarrots && !this.hasTarget && (block = (_snowman = world.getBlockState(pos = pos.up())).getBlock()) instanceof CarrotsBlock && ((CarrotsBlock)block).isMature(_snowman)) {
                this.hasTarget = true;
                return true;
            }
            return false;
