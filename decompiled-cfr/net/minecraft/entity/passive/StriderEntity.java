...
Saddleable {
    ...
    public double getMountedHeightOffset() {
        ...
        _snowman = this.limbAngle;
        return (double)this.getHeight() - 0.19 + (double)(0.12f * MathHelper.cos(_snowman * 1.5f) * 2.0f * f);
    }
    ...
    public boolean canBeControlledByRider() {
        ...
        PlayerEntity _snowman2 = (PlayerEntity)entity;
        return _snowman2.getMainHandStack().getItem() == Items.WARPED_FUNGUS_ON_A_STICK || _snowman2.getOffHandStack().getItem() == Items.WARPED_FUNGUS_ON_A_STICK;
    }
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        Vec3d[] vec3dArray = new Vec3d[]{StriderEntity.getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.yaw), StriderEntity.getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.yaw - 22.5f), StriderEntity.getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.yaw + 22.5f), StriderEntity.getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.yaw - 45.0f), StriderEntity.getPassengerDismountOffset(this.getWidth(), passenger.getWidth(), passenger.yaw + 45.0f)};
        LinkedHashSet _snowman2 = Sets.newLinkedHashSet();
        double _snowman3 = this.getBoundingBox().maxY;
        double _snowman4 = this.getBoundingBox().minY - 0.5;
        BlockPos.Mutable _snowman5 = new BlockPos.Mutable();
        for (Vec3d vec3d : vec3dArray) {
            _snowman5.set(this.getX() + vec3d.x, _snowman3, this.getZ() + vec3d.z);
            for (double d = _snowman3; d > _snowman4; d -= 1.0) {
                _snowman2.add(_snowman5.toImmutable());
                _snowman5.move(Direction.DOWN);
            }
        }
        for (BlockPos blockPos : _snowman2) {
            if (this.world.getFluidState(blockPos).isIn(FluidTags.LAVA) || !Dismounting.canDismountInBlock(_snowman = this.world.getDismountHeight(blockPos))) continue;
            Vec3d vec3d = Vec3d.ofCenter(blockPos, _snowman);
            for (EntityPose entityPose : passenger.getPoses()) {
                Box box = passenger.getBoundingBox(entityPose);
                if (!Dismounting.canPlaceEntityAt(this.world, passenger, box.offset(vec3d))) continue;
    ...
    public void tick() {
        ...
        _snowman = this.getLandingBlockState();
        boolean _snowman2 = blockState.isIn(BlockTags.STRIDER_WARM_BLOCKS) || _snowman.isIn(BlockTags.STRIDER_WARM_BLOCKS) || this.getFluidHeight(FluidTags.LAVA) > 0.0;
        this.setCold(!_snowman2);
        super.tick();
        this.updateFloating();
        this.checkBlockCollision();
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        ActionResult _snowman2 = super.interactMob(player, hand);
