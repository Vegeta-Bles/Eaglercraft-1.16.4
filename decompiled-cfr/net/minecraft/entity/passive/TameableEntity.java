...
extends AnimalEntity {
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        ...
        if (tag.containsUuid("Owner")) {
            ...
            _snowman2 = ServerConfigHandler.getPlayerUuidByName(this.getServer(), string);
        }
        if (_snowman2 != null) {
            try {
    ...
    protected void showEmoteParticle(boolean positive) {
        DefaultParticleType defaultParticleType = ParticleTypes.HEART;
        if (!positive) {
            defaultParticleType = ParticleTypes.SMOKE;
        }
        for (int i = 0; i < 7; ++i) {
    ...
    public void setTamed(boolean tamed) {
        byte by = this.dataTracker.get(TAMEABLE_FLAGS);
        if (tamed) {
            this.dataTracker.set(TAMEABLE_FLAGS, (byte)(by | 4));
        } else {
            this.dataTracker.set(TAMEABLE_FLAGS, (byte)(by & 0xFFFFFFFB));
        }
        this.onTamedChanged();
    }
    ...
    public void setInSittingPose(boolean inSittingPose) {
        byte by = this.dataTracker.get(TAMEABLE_FLAGS);
        if (inSittingPose) {
            this.dataTracker.set(TAMEABLE_FLAGS, (byte)(by | 1));
        } else {
            this.dataTracker.set(TAMEABLE_FLAGS, (byte)(by & 0xFFFFFFFE));
        }
    }
