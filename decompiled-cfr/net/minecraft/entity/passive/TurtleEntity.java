...
extends AnimalEntity {
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        int n = tag.getInt("HomePosX");
        _snowman = tag.getInt("HomePosY");
        _snowman = tag.getInt("HomePosZ");
        this.setHomePos(new BlockPos(n, _snowman, _snowman));
        super.readCustomDataFromTag(tag);
        this.setHasEgg(tag.getBoolean("HasEgg"));
        _snowman = tag.getInt("TravelPosX");
        _snowman = tag.getInt("TravelPosY");
        _snowman = tag.getInt("TravelPosZ");
        this.setTravelPos(new BlockPos(_snowman, _snowman, _snowman));
    }
    ...
    extends MoveControl {
        ...
        public void tick() {
            ...
            _snowman = this.targetY - this.turtle.getY();
            _snowman = this.targetZ - this.turtle.getZ();
            _snowman = MathHelper.sqrt(d * d + _snowman * _snowman + _snowman * _snowman);
            float _snowman2 = (float)(MathHelper.atan2(_snowman, d) * 57.2957763671875) - 90.0f;
            this.turtle.bodyYaw = this.turtle.yaw = this.changeAngle(this.turtle.yaw, _snowman2, 90.0f);
            float _snowman3 = (float)(this.speed * this.turtle.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED));
            this.turtle.setMovementSpeed(MathHelper.lerp(0.125f, this.turtle.getMovementSpeed(), _snowman3));
            this.turtle.setVelocity(this.turtle.getVelocity().add(0.0, (double)this.turtle.getMovementSpeed() * (_snowman /= _snowman) * 0.1, 0.0));
        }
    }
    ...
    extends AnimalMateGoal {
        ...
        protected void breed() {
            ...
            Random _snowman2 = this.animal.getRandom();
            if (this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
                this.world.spawnEntity(new ExperienceOrbEntity(this.world, this.animal.getX(), this.animal.getY(), this.animal.getZ(), _snowman2.nextInt(7) + 1));
            }
        }
    }
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            boolean _snowman2 = blockPos.isWithinDistance(this.turtle.getPos(), 16.0);
            if (_snowman2) {
                ++this.homeReachingTryTicks;
            }
            if (this.turtle.getNavigation().isIdle()) {
                Vec3d vec3d = Vec3d.ofBottomCenter(blockPos);
                _snowman = TargetFinder.findTargetTowards(this.turtle, 16, 3, vec3d, 0.3141592741012573);
                if (_snowman == null) {
                    _snowman = TargetFinder.findTargetTowards(this.turtle, 8, 7, vec3d);
                }
                if (_snowman != null && !_snowman2 && !this.turtle.world.getBlockState(new BlockPos(_snowman)).isOf(Blocks.WATER)) {
                    _snowman = TargetFinder.findTargetTowards(this.turtle, 16, 5, vec3d);
                }
                if (_snowman == null) {
                    this.noPath = true;
                    return;
                }
                this.turtle.getNavigation().startMovingTo(_snowman.x, _snowman.y, _snowman.z, this.speed);
            }
        }
    }
    ...
    extends Goal {
        ...
        public void start() {
            int n = 512;
            _snowman = 4;
            Random _snowman2 = this.turtle.random;
            _snowman = _snowman2.nextInt(1025) - 512;
            _snowman = _snowman2.nextInt(9) - 4;
            _snowman = _snowman2.nextInt(1025) - 512;
            if ((double)_snowman + this.turtle.getY() > (double)(this.turtle.world.getSeaLevel() - 1)) {
                _snowman = 0;
            }
            BlockPos _snowman3 = new BlockPos((double)_snowman + this.turtle.getX(), (double)_snowman + this.turtle.getY(), (double)_snowman + this.turtle.getZ());
            this.turtle.setTravelPos(_snowman3);
        ...
        public void tick() {
            if (this.turtle.getNavigation().isIdle()) {
                Vec3d vec3d;
                Vec3d vec3d2 = Vec3d.ofBottomCenter(this.turtle.getTravelPos());
                vec3d = TargetFinder.findTargetTowards(this.turtle, 16, 3, vec3d2, 0.3141592741012573);
                if (vec3d == null) {
                    vec3d = TargetFinder.findTargetTowards(this.turtle, 8, 7, vec3d2);
                }
                if (vec3d != null) {
                    int n = MathHelper.floor(vec3d.x);
                    _snowman = MathHelper.floor(vec3d.z);
                    _snowman = 34;
                    if (!this.turtle.world.isRegionLoaded(n - 34, 0, _snowman - 34, n + 34, 0, _snowman + 34)) {
                        vec3d = null;
                    }
                }
                if (vec3d == null) {
