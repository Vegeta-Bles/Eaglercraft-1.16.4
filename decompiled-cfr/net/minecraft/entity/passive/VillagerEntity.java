...
VillagerDataContainer {
    ...
    public boolean shouldRestock() {
        ...
        _snowman = this.world.getTime();
        boolean _snowman2 = _snowman > l;
        _snowman = this.world.getTimeOfDay();
        if (this.lastRestockCheckTime > 0L) {
            _snowman = _snowman / 24000L;
            _snowman = this.lastRestockCheckTime / 24000L;
            _snowman2 |= _snowman > _snowman;
        }
        this.lastRestockCheckTime = _snowman;
        if (_snowman2) {
            this.lastRestockTime = _snowman;
    ...
    private void prepareRecipesFor(PlayerEntity player) {
        int n = this.getReputation(player);
        if (n != 0) {
            for (TradeOffer tradeOffer : this.getOffers()) {
                tradeOffer.increaseSpecialPrice(-MathHelper.floor((float)n * tradeOffer.getPriceMultiplier()));
            }
        }
        if (player.hasStatusEffect(StatusEffects.HERO_OF_THE_VILLAGE)) {
            StatusEffectInstance statusEffectInstance = player.getStatusEffect(StatusEffects.HERO_OF_THE_VILLAGE);
            int _snowman2 = statusEffectInstance.getAmplifier();
            for (TradeOffer tradeOffer : this.getOffers()) {
                double d = 0.3 + 0.0625 * (double)_snowman2;
                int _snowman3 = (int)Math.floor(d * (double)tradeOffer.getOriginalFirstBuyItem().getCount());
                tradeOffer.increaseSpecialPrice(-Math.max(_snowman3, 1));
            }
        }
    }
    ...
    public void writeCustomDataToTag(CompoundTag tag2) {
        ...
        tag2.put("Gossips", (Tag)this.gossip.serialize(NbtOps.INSTANCE).getValue());
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        ...
        if (tag.contains("VillagerData", 10)) {
            object = VillagerData.CODEC.parse(new Dynamic((DynamicOps)NbtOps.INSTANCE, (Object)tag.get("VillagerData")));
            object.resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).ifPresent(this::setVillagerData);
        }
        ...
        object = tag.getList("Gossips", 10);
        this.gossip.deserialize(new Dynamic((DynamicOps)NbtOps.INSTANCE, object));
    ...
    protected void afterUsing(TradeOffer offer) {
        ...
        if (offer.shouldRewardPlayerExperience()) {
            this.world.spawnEntity(new ExperienceOrbEntity(this.world, this.getX(), this.getY() + 0.5, this.getZ(), n));
        }
    }
    ...
    private void notifyDeath(Entity killer) {
        ...
        optional.get().stream().filter(livingEntity -> livingEntity instanceof InteractionObserver).forEach(livingEntity -> _snowman2.handleInteraction(EntityInteraction.VILLAGER_KILLED, killer, (InteractionObserver)((Object)livingEntity)));
    }
    ...
    public void releaseTicketFor(MemoryModuleType<GlobalPos> memoryModuleType) {
        ...
        this.brain.getOptionalMemory(memoryModuleType).ifPresent(globalPos -> {
            ...
            Optional<PointOfInterestType> _snowman3 = _snowman2.getType(globalPos.getPos());
            BiPredicate<VillagerEntity, PointOfInterestType> _snowman4 = POINTS_OF_INTEREST.get(memoryModuleType);
            if (_snowman3.isPresent() && _snowman4.test(this, _snowman3.get())) {
                _snowman2.releaseTicket(globalPos.getPos());
    ...
    private void consumeAvailableFood() {
        ...
        for (int i = 0; i < this.getInventory().size(); ++i) {
            ...
            if (itemStack.isEmpty() || (_snowman = ITEM_FOOD_VALUES.get(itemStack.getItem())) == null) continue;
            for (int j = _snowman = itemStack.getCount(); j > 0; --j) {
                this.foodLevel = (byte)(this.foodLevel + _snowman);
    ...
    private boolean canLevelUp() {
        ...
        return VillagerData.canLevelUp(n) && this.experience >= VillagerData.getUpperLevelExperience(n);
    }
    ...
    public VillagerEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        ...
        VillagerType _snowman2 = d < 0.5 ? VillagerType.forBiome(serverWorld.method_31081(this.getBlockPos())) : (d < 0.75 ? this.getVillagerData().getType() : ((VillagerEntity)passiveEntity).getVillagerData().getType());
        VillagerEntity _snowman3 = new VillagerEntity(EntityType.VILLAGER, serverWorld, _snowman2);
        _snowman3.initialize(serverWorld, serverWorld.getLocalDifficulty(_snowman3.getBlockPos()), SpawnReason.BREEDING, null, null);
        return _snowman3;
    }
    ...
    protected void loot(ItemEntity item) {
        ...
        if (this.canGather(itemStack)) {
            ...
            boolean _snowman2 = simpleInventory.canInsert(itemStack);
            if (!_snowman2) {
                return;
            }
            this.method_29499(item);
            this.sendPickup(item, itemStack.getCount());
            ItemStack _snowman3 = simpleInventory.addStack(itemStack);
            if (_snowman3.isEmpty()) {
                item.remove();
            } else {
                itemStack.setCount(_snowman3.getCount());
            }
        }
    }
    ...
    protected void fillRecipes() {
        ...
        Int2ObjectMap<TradeOffers.Factory[]> _snowman2 = TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(villagerData.getProfession());
        if (_snowman2 == null || _snowman2.isEmpty()) {
            return;
        }
        TradeOffers.Factory[] _snowman3 = (TradeOffers.Factory[])_snowman2.get(villagerData.getLevel());
        if (_snowman3 == null) {
            return;
        }
        TradeOfferList _snowman4 = this.getOffers();
        this.fillRecipesFromPool(_snowman4, _snowman3, 2);
    }
    ...
    public void summonGolem(ServerWorld world, long time, int n) {
        if (!this.canSummonGolem(time)) {
            return;
        }
        Box box = this.getBoundingBox().expand(10.0, 10.0, 10.0);
        List<VillagerEntity> _snowman2 = world.getNonSpectatingEntities(VillagerEntity.class, box);
        List _snowman3 = _snowman2.stream().filter(villagerEntity -> villagerEntity.canSummonGolem(time)).limit(5L).collect(Collectors.toList());
        if (_snowman3.size() < n) {
            return;
        }
        IronGolemEntity _snowman4 = this.spawnIronGolem(world);
        if (_snowman4 == null) {
            return;
        }
        _snowman2.forEach(GolemLastSeenSensor::method_30233);
    }
    ...
    private IronGolemEntity spawnIronGolem(ServerWorld world) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            BlockPos _snowman2 = this.method_30023(blockPos, d, _snowman = (double)(world.random.nextInt(16) - 8));
            if (_snowman2 == null || (_snowman = EntityType.IRON_GOLEM.create(world, null, null, null, _snowman2, SpawnReason.MOB_SUMMONED, false, false)) == null) continue;
            if (_snowman.canSpawn(world, SpawnReason.MOB_SUMMONED) && _snowman.canSpawn(world)) {
    ...
    @Nullable
    private BlockPos method_30023(BlockPos blockPos, double d, double d2) {
        int n = 6;
        BlockPos _snowman2 = blockPos.add(d, 6.0, d2);
        BlockState _snowman3 = this.world.getBlockState(_snowman2);
        for (_snowman = 6; _snowman >= -6; --_snowman) {
            BlockPos blockPos2 = _snowman2;
            BlockState _snowman4 = _snowman3;
            _snowman2 = blockPos2.down();
            _snowman3 = this.world.getBlockState(_snowman2);
            if (!_snowman4.isAir() && !_snowman4.getMaterial().isLiquid() || !_snowman3.getMaterial().blocksLight()) continue;
            return blockPos2;
        }
        return null;
    }
