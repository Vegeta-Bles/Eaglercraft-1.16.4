...
extends MerchantEntity {
    ...
    protected void fillRecipes() {
        TradeOffers.Factory[] factoryArray = (TradeOffers.Factory[])TradeOffers.WANDERING_TRADER_TRADES.get(1);
        _snowman = (TradeOffers.Factory[])TradeOffers.WANDERING_TRADER_TRADES.get(2);
        if (factoryArray == null || _snowman == null) {
            return;
        }
        TradeOfferList _snowman2 = this.getOffers();
        this.fillRecipesFromPool(_snowman2, factoryArray, 5);
        int _snowman3 = this.random.nextInt(_snowman.length);
        TradeOffers.Factory _snowman4 = _snowman[_snowman3];
        TradeOffer _snowman5 = _snowman4.create(this, this.random);
        if (_snowman5 != null) {
            _snowman2.add(_snowman5);
        }
    }
        ...
        public void tick() {
            ...
            if (blockPos != null && WanderingTraderEntity.this.navigation.isIdle()) {
                if (this.isTooFarFrom(blockPos, 10.0)) {
                    ...
                    _snowman = vec3d.multiply(10.0).add(this.trader.getX(), this.trader.getY(), this.trader.getZ());
                    WanderingTraderEntity.this.navigation.startMovingTo(_snowman.x, _snowman.y, _snowman.z, this.speed);
                } else {
