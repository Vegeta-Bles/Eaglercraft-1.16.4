...
implements Angerable {
    ...
    public void tick() {
        ...
        if (this.isWet()) {
            ...
            if (this.shakeProgress > 0.4f) {
                ...
                int _snowman2 = (int)(MathHelper.sin((this.shakeProgress - 0.4f) * (float)Math.PI) * 7.0f);
                Vec3d _snowman3 = this.getVelocity();
                for (int i = 0; i < _snowman2; ++i) {
                    float f2 = (this.random.nextFloat() * 2.0f - 1.0f) * this.getWidth() * 0.5f;
                    _snowman = (this.random.nextFloat() * 2.0f - 1.0f) * this.getWidth() * 0.5f;
                    this.world.addParticle(ParticleTypes.SPLASH, this.getX() + (double)f2, f + 0.8f, this.getZ() + (double)_snowman, _snowman3.x, _snowman3.y, _snowman3.z);
                }
            }
        }
    }
    ...
    public float getShakeAnimationProgress(float tickDelta, float f) {
        _snowman = (MathHelper.lerp(tickDelta, this.lastShakeProgress, this.shakeProgress) + f) / 1.8f;
        if (_snowman < 0.0f) {
            _snowman = 0.0f;
        } else if (_snowman > 1.0f) {
            _snowman = 1.0f;
        }
        return MathHelper.sin(_snowman * (float)Math.PI) * MathHelper.sin(_snowman * (float)Math.PI * 11.0f) * 0.15f * (float)Math.PI;
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.world.isClient) {
            boolean bl = this.isOwner(player) || this.isTamed() || _snowman2 == Items.BONE && !this.isTamed() && !this.hasAngerTime();
        ...
        if (this.isTamed()) {
            ...
            if (_snowman2 instanceof DyeItem) {
                DyeColor dyeColor = ((DyeItem)_snowman2).getColor();
        ...
        if (_snowman2 != Items.BONE || this.hasAngerTime()) return super.interactMob(player, hand);
        if (!player.abilities.creativeMode) {
            itemStack.decrement(1);
        }
        if (this.random.nextInt(3) == 0) {
    ...
    public WolfEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        ...
        UUID _snowman2 = this.getOwnerUuid();
        if (_snowman2 != null) {
            wolfEntity.setOwnerUuid(_snowman2);
