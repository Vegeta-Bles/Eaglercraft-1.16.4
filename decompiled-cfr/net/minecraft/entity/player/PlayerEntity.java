...
extends LivingEntity {
    ...
    public void tick() {
        ...
        int n = 29999999;
        double _snowman2 = MathHelper.clamp(this.getX(), -2.9999999E7, 2.9999999E7);
        double _snowman3 = MathHelper.clamp(this.getZ(), -2.9999999E7, 2.9999999E7);
        if (_snowman2 != this.getX() || _snowman3 != this.getZ()) {
            this.updatePosition(_snowman2, this.getY(), _snowman3);
        }
        ++this.lastAttackedTicks;
        ItemStack _snowman4 = this.getMainHandStack();
        if (!ItemStack.areEqual(this.selectedItem, _snowman4)) {
            if (!ItemStack.areItemsEqual(this.selectedItem, _snowman4)) {
                this.resetLastAttackedTicks();
            }
            this.selectedItem = _snowman4.copy();
        }
    ...
    private void updateCapeAngles() {
        ...
        _snowman = this.getY() - this.capeY;
        _snowman = this.getZ() - this.capeZ;
        _snowman = 10.0;
        if (d > 10.0) {
        ...
        this.capeX += d * 0.25;
        this.capeZ += _snowman * 0.25;
        this.capeY += _snowman * 0.25;
    }
    ...
    protected void updateSize() {
        ...
        _snowman = this.isSpectator() || this.hasVehicle() || this.wouldPoseNotCollide(entityPose) ? entityPose : (this.wouldPoseNotCollide(EntityPose.CROUCHING) ? EntityPose.CROUCHING : EntityPose.SWIMMING);
        this.setPose(_snowman);
    }
    ...
    public void tickRiding() {
        ...
        this.increaseRidingMotionStats(this.getX() - d, this.getY() - _snowman, this.getZ() - _snowman);
    }
    ...
    public void addScore(int score) {
        int n = this.getScore();
        this.dataTracker.set(SCORE, n + score);
    }
    ...
    public ItemEntity dropItem(ItemStack stack, boolean throwRandomly, boolean retainOwnership) {
        ...
        ItemEntity _snowman2 = new ItemEntity(this.world, this.getX(), d, this.getZ(), stack);
        _snowman2.setPickupDelay(40);
        if (retainOwnership) {
            _snowman2.setThrower(this.getUuid());
        }
        if (throwRandomly) {
            float f = this.random.nextFloat() * 0.5f;
            _snowman = this.random.nextFloat() * ((float)Math.PI * 2);
            _snowman2.setVelocity(-MathHelper.sin(_snowman) * f, 0.2f, MathHelper.cos(_snowman) * f);
        } else {
            float f = 0.3f;
            _snowman = MathHelper.sin(this.pitch * ((float)Math.PI / 180));
            _snowman = MathHelper.cos(this.pitch * ((float)Math.PI / 180));
            _snowman = MathHelper.sin(this.yaw * ((float)Math.PI / 180));
            _snowman = MathHelper.cos(this.yaw * ((float)Math.PI / 180));
            _snowman = this.random.nextFloat() * ((float)Math.PI * 2);
            _snowman = 0.02f * this.random.nextFloat();
            _snowman2.setVelocity((double)(-_snowman * _snowman * 0.3f) + Math.cos(_snowman) * (double)_snowman, -_snowman * 0.3f + 0.1f + (this.random.nextFloat() - this.random.nextFloat()) * 0.1f, (double)(_snowman * _snowman * 0.3f) + Math.sin(_snowman) * (double)_snowman);
        }
        return _snowman2;
    }
    ...
    public float getBlockBreakingSpeed(BlockState block) {
        ...
        if (f > 1.0f) {
            int n = EnchantmentHelper.getEfficiency(this);
            ItemStack _snowman2 = this.getMainHandStack();
            if (n > 0 && !_snowman2.isEmpty()) {
                f += (float)(n * n + 1);
            }
        }
    ...
    public boolean shouldDamagePlayer(PlayerEntity player) {
        ...
        _snowman = player.getScoreboardTeam();
        if (abstractTeam == null) {
            return true;
        }
        if (!abstractTeam.isEqual(_snowman)) {
            return true;
        }
    ...
    protected void damageShield(float amount) {
        ...
        if (amount >= 3.0f) {
            int n = 1 + MathHelper.floor(amount);
            Hand _snowman2 = this.getActiveHand();
            this.activeItemStack.damage(n, this, playerEntity -> playerEntity.sendToolBreakStatus(_snowman2));
            if (this.activeItemStack.isEmpty()) {
                if (_snowman2 == Hand.MAIN_HAND) {
    ...
    public ActionResult interact(Entity entity, Hand hand) {
        ...
        ActionResult _snowman2 = entity.interact(this, hand);
        if (_snowman2.isAccepted()) {
            if (this.abilities.creativeMode && itemStack == this.getStackInHand(hand) && itemStack.getCount() < _snowman.getCount()) {
                itemStack.setCount(_snowman.getCount());
            }
            return _snowman2;
        }
        if (!itemStack.isEmpty() && entity instanceof LivingEntity) {
            ...
            if ((_snowman = itemStack.useOnEntity(this, (LivingEntity)entity, hand)).isAccepted()) {
    ...
    protected Vec3d adjustMovementForSneaking(Vec3d movement, MovementType type) {
        if (!this.abilities.flying && (type == MovementType.SELF || type == MovementType.PLAYER) && this.clipAtLedge() && this.method_30263()) {
            ...
            while (_snowman != 0.0 && this.world.isSpaceEmpty(this, this.getBoundingBox().offset(0.0, -this.stepHeight, _snowman))) {
            ...
            while (d != 0.0 && _snowman != 0.0 && this.world.isSpaceEmpty(this, this.getBoundingBox().offset(d, -this.stepHeight, _snowman))) {
            ...
            movement = new Vec3d(d, movement.y, _snowman);
        }
        return movement;
    }
    ...
    public void attack(Entity target) {
        ...
        _snowman = target instanceof LivingEntity ? EnchantmentHelper.getAttackDamage(this.getMainHandStack(), ((LivingEntity)target).getGroup()) : EnchantmentHelper.getAttackDamage(this.getMainHandStack(), EntityGroup.DEFAULT);
        _snowman = this.getAttackCooldownProgress(0.5f);
        _snowman *= _snowman;
        this.resetLastAttackedTicks();
        if ((f *= 0.2f + _snowman * _snowman * 0.8f) > 0.0f || _snowman > 0.0f) {
            boolean bl = _snowman > 0.9f;
            _snowman = false;
            int _snowman2 = 0;
            _snowman2 += EnchantmentHelper.getKnockback(this);
            ...
            bl3 = bl && this.fallDistance > 0.0f && !this.onGround && !this.isClimbing() && !this.isTouchingWater() && !this.hasStatusEffect(StatusEffects.BLINDNESS) && !this.hasVehicle() && target instanceof LivingEntity;
            boolean bl2 = bl3 = bl3 && !this.isSprinting();
            if (bl3) {
                f *= 1.5f;
            }
            f += _snowman;
            _snowman = false;
            double _snowman3 = this.horizontalSpeed - this.prevHorizontalSpeed;
            if (bl && !bl3 && !_snowman && this.onGround && _snowman3 < (double)this.getMovementSpeed() && (_snowman = this.getStackInHand(Hand.MAIN_HAND)).getItem() instanceof SwordItem) {
                _snowman = true;
            }
            float _snowman4 = 0.0f;
            bl4 = false;
            int _snowman5 = EnchantmentHelper.getFireAspect(this);
            if (target instanceof LivingEntity) {
                _snowman4 = ((LivingEntity)target).getHealth();
                if (_snowman5 > 0 && !target.isOnFire()) {
                    bl4 = true;
                    target.setOnFireFor(1);
                }
            }
            Vec3d _snowman6 = target.getVelocity();
            _snowman = target.damage(DamageSource.player(this), f);
            if (_snowman) {
                boolean bl3;
                Object _snowman7;
                if (_snowman2 > 0) {
                    if (target instanceof LivingEntity) {
                        ((LivingEntity)target).takeKnockback((float)_snowman2 * 0.5f, MathHelper.sin(this.yaw * ((float)Math.PI / 180)), -MathHelper.cos(this.yaw * ((float)Math.PI / 180)));
                    } else {
                        target.addVelocity(-MathHelper.sin(this.yaw * ((float)Math.PI / 180)) * (float)_snowman2 * 0.5f, 0.1, MathHelper.cos(this.yaw * ((float)Math.PI / 180)) * (float)_snowman2 * 0.5f);
                    }
                    this.setVelocity(this.getVelocity().multiply(0.6, 1.0, 0.6));
                    this.setSprinting(false);
                }
                if (_snowman) {
                    float f2 = 1.0f + EnchantmentHelper.getSweepingMultiplier(this) * f;
                    _snowman7 = this.world.getNonSpectatingEntities(LivingEntity.class, target.getBoundingBox().expand(1.0, 0.25, 1.0));
                    Iterator<LivingEntity> iterator = _snowman7.iterator();
                    while (iterator.hasNext()) {
                        ...
                        livingEntity.damage(DamageSource.player(this), f2);
                    }
                ...
                ItemStack _snowman8 = this.getMainHandStack();
                _snowman7 = target;
                if (target instanceof EnderDragonPart) {
                    _snowman7 = ((EnderDragonPart)target).owner;
                }
                if (!this.world.isClient && !_snowman8.isEmpty() && _snowman7 instanceof LivingEntity) {
                    _snowman8.postHit((LivingEntity)_snowman7, this);
                    if (_snowman8.isEmpty()) {
                        this.setStackInHand(Hand.MAIN_HAND, ItemStack.EMPTY);
                    }
                }
                if (target instanceof LivingEntity) {
                    float f3 = _snowman4 - ((LivingEntity)target).getHealth();
                    this.increaseStat(Stats.DAMAGE_DEALT, Math.round(f3 * 10.0f));
                    if (_snowman5 > 0) {
                        target.setOnFireFor(_snowman5 * 4);
                    }
                    if (this.world instanceof ServerWorld && f3 > 2.0f) {
    ...
    public void spawnSweepAttackParticles() {
        ...
        if (this.world instanceof ServerWorld) {
            ((ServerWorld)this.world).spawnParticles(ParticleTypes.SWEEP_ATTACK, this.getX() + d, this.getBodyY(0.5), this.getZ() + _snowman, 0, d, 0.0, _snowman, 0.0);
        }
    }
    ...
    public static Optional<Vec3d> findRespawnPosition(ServerWorld world, BlockPos pos, float f2, boolean bl3, boolean bl2) {
        boolean bl3;
        BlockState blockState = world.getBlockState(pos);
        Block _snowman2 = blockState.getBlock();
        if (_snowman2 instanceof RespawnAnchorBlock && blockState.get(RespawnAnchorBlock.CHARGES) > 0 && RespawnAnchorBlock.isNether(world)) {
        ...
        if (_snowman2 instanceof BedBlock && BedBlock.isOverworld(world)) {
            float f2;
            return BedBlock.findWakeUpPosition(EntityType.PLAYER, world, pos, f2);
        }
        if (!bl3) {
            return Optional.empty();
        }
        _snowman = _snowman2.canMobSpawnInside();
        _snowman = world.getBlockState(pos.up()).getBlock().canMobSpawnInside();
    ...
    public void travel(Vec3d movementInput) {
        ...
        if (this.isSwimming() && !this.hasVehicle()) {
            d3 = this.getRotationVector().y;
            double d2 = _snowman = d3 < -0.2 ? 0.085 : 0.06;
            if (d3 <= 0.0 || this.jumping || !this.world.getBlockState(new BlockPos(this.getX(), this.getY() + 1.0 - 0.1, this.getZ())).getFluidState().isEmpty()) {
                Vec3d vec3d = this.getVelocity();
                this.setVelocity(vec3d.add(0.0, (d3 - vec3d.y) * _snowman, 0.0));
            }
        }
        if (this.abilities.flying && !this.hasVehicle()) {
            double d3 = this.getVelocity().y;
            float _snowman2 = this.flyingSpeed;
            this.flyingSpeed = this.abilities.getFlySpeed() * (float)(this.isSprinting() ? 2 : 1);
            super.travel(movementInput);
            Vec3d _snowman3 = this.getVelocity();
            this.setVelocity(_snowman3.x, d3 * 0.6, _snowman3.z);
            this.flyingSpeed = _snowman2;
            this.fallDistance = 0.0f;
            this.setFlag(7, false);
        } else {
            super.travel(movementInput);
        }
        this.increaseTravelMotionStats(this.getX() - d, this.getY() - _snowman, this.getZ() - _snowman);
    }
    ...
    public void increaseTravelMotionStats(double dx, double dy, double dz) {
        ...
        if (this.isSwimming()) {
            int n = Math.round(MathHelper.sqrt(dx * dx + dy * dy + dz * dz) * 100.0f);
            if (n > 0) {
            ...
            int n = Math.round(MathHelper.sqrt(dx * dx + dy * dy + dz * dz) * 100.0f);
            if (n > 0) {
                this.increaseStat(Stats.WALK_UNDER_WATER_ONE_CM, n);
                this.addExhaustion(0.01f * (float)n * 0.01f);
            }
        } else if (this.isTouchingWater()) {
            int n = Math.round(MathHelper.sqrt(dx * dx + dz * dz) * 100.0f);
            if (n > 0) {
                this.increaseStat(Stats.WALK_ON_WATER_ONE_CM, n);
                this.addExhaustion(0.01f * (float)n * 0.01f);
            }
            ...
            if (n > 0) {
                if (this.isSprinting()) {
                    this.increaseStat(Stats.SPRINT_ONE_CM, n);
                    this.addExhaustion(0.1f * (float)n * 0.01f);
                } else if (this.isInSneakingPose()) {
                    this.increaseStat(Stats.CROUCH_ONE_CM, n);
            ...
            int n = Math.round(MathHelper.sqrt(dx * dx + dy * dy + dz * dz) * 100.0f);
            this.increaseStat(Stats.AVIATE_ONE_CM, n);
        } else {
            int n = Math.round(MathHelper.sqrt(dx * dx + dz * dz) * 100.0f);
            if (n > 25) {
    ...
    private void increaseRidingMotionStats(double dx, double dy, double dz) {
        int n;
        if (this.hasVehicle() && (n = Math.round(MathHelper.sqrt(dx * dx + dy * dy + dz * dz) * 100.0f)) > 0) {
            ...
            if (entity instanceof AbstractMinecartEntity) {
                ...
                this.increaseStat(Stats.HORSE_ONE_CM, n);
            } else if (entity instanceof StriderEntity) {
                this.increaseStat(Stats.STRIDER_ONE_CM, n);
            }
        }
    }
    ...
    public boolean canPlaceOn(BlockPos pos, Direction facing, ItemStack stack) {
        ...
        CachedBlockPosition _snowman2 = new CachedBlockPosition(this.world, blockPos, false);
        return stack.canPlaceOn(this.world.getTagManager(), _snowman2);
    }
    ...
    public boolean equip(int slot, ItemStack item) {
        ...
        int _snowman2 = slot - 200;
        if (_snowman2 >= 0 && _snowman2 < this.enderChestInventory.size()) {
            this.enderChestInventory.setStack(_snowman2, item);
            return true;
        }
        return false;
    }
    ...
    public ItemStack getArrowType(ItemStack stack) {
        ...
        ItemStack _snowman2 = RangedWeaponItem.getHeldProjectile(this, predicate);
    ...
    public Vec3d method_30951(float f) {
        ...
        float _snowman2 = MathHelper.lerp(f * 0.5f, this.pitch, this.prevPitch) * ((float)Math.PI / 180);
        float _snowman3 = MathHelper.lerp(f, this.prevBodyYaw, this.bodyYaw) * ((float)Math.PI / 180);
        if (this.isFallFlying() || this.isUsingRiptide()) {
            float _snowman6;
            Vec3d vec3d = this.getRotationVec(f);
            _snowman = this.getVelocity();
            double _snowman4 = Entity.squaredHorizontalLength(_snowman);
            double _snowman5 = Entity.squaredHorizontalLength(vec3d);
            if (_snowman4 > 0.0 && _snowman5 > 0.0) {
                double d2 = (_snowman.x * vec3d.x + _snowman.z * vec3d.z) / Math.sqrt(_snowman4 * _snowman5);
                _snowman = _snowman.x * vec3d.z - _snowman.z * vec3d.x;
                _snowman6 = (float)(Math.signum(_snowman) * Math.acos(d2));
            } else {
                _snowman6 = 0.0f;
            }
            return this.method_30950(f).add(new Vec3d(d, -0.11, 0.85).rotateZ(-_snowman6).rotateX(-_snowman2).rotateY(-_snowman3));
        }
        if (this.isInSwimmingPose()) {
            return this.method_30950(f).add(new Vec3d(d, 0.2, -0.15).rotateX(-_snowman2).rotateY(-_snowman3));
        }
        _snowman = this.getBoundingBox().getYLength() - 1.0;
        _snowman = this.isInSneakingPose() ? -0.2 : 0.07;
        return this.method_30950(f).add(new Vec3d(d, _snowman, _snowman).rotateY(-_snowman3));
    }
