...
extends ExplosiveProjectileEntity {
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (!this.world.isClient) {
            ...
            AreaEffectCloudEntity _snowman2 = new AreaEffectCloudEntity(this.world, this.getX(), this.getY(), this.getZ());
            if (entity instanceof LivingEntity) {
                _snowman2.setOwner((LivingEntity)entity);
            }
            _snowman2.setParticleType(ParticleTypes.DRAGON_BREATH);
            _snowman2.setRadius(3.0f);
            _snowman2.setDuration(600);
            _snowman2.setRadiusGrowth((7.0f - _snowman2.getRadius()) / (float)_snowman2.getDuration());
            _snowman2.addEffect(new StatusEffectInstance(StatusEffects.INSTANT_DAMAGE, 1, 1));
            if (!list.isEmpty()) {
                for (LivingEntity livingEntity : list) {
                    double d = this.squaredDistanceTo(livingEntity);
                    if (!(d < 16.0)) continue;
                    _snowman2.updatePosition(livingEntity.getX(), livingEntity.getY(), livingEntity.getZ());
                    break;
                }
            }
