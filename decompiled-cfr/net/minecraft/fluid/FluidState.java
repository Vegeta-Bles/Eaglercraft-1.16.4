    ...
    public boolean method_15756(BlockView world, BlockPos pos) {
        for (int i = -1; i <= 1; ++i) {
            for (_snowman = -1; _snowman <= 1; ++_snowman) {
                BlockPos blockPos = pos.add(i, 0, _snowman);
                FluidState _snowman2 = world.getFluidState(blockPos);
                if (_snowman2.getFluid().matchesType(this.getFluid()) || world.getBlockState(blockPos).isOpaqueFullCube(world, blockPos)) continue;
                return true;
            }
        }
        return false;
