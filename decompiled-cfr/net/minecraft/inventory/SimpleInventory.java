...
RecipeInputProvider {
    ...
    public ItemStack removeItem(Item item, int count) {
        ...
        for (int i = this.size - 1; i >= 0; --i) {
            ...
            int _snowman2 = count - itemStack.getCount();
            _snowman = itemStack2.split(_snowman2);
            itemStack.increment(_snowman.getCount());
    ...
    private void transfer(ItemStack source, ItemStack target) {
        int n = Math.min(this.getMaxCountPerStack(), target.getMaxCount());
        _snowman = Math.min(source.getCount(), n - target.getCount());
        if (_snowman > 0) {
            target.increment(_snowman);
            source.decrement(_snowman);
            this.markDirty();
        }
    }
