...
implements Vanishable {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (user.fishHook != null) {
            if (!world.isClient) {
                int n = user.fishHook.use(itemStack);
                itemStack.damage(n, user, p -> p.sendToolBreakStatus(hand));
            }
            ...
            if (!world.isClient) {
                int _snowman2 = EnchantmentHelper.getLure(itemStack);
                int _snowman3 = EnchantmentHelper.getLuckOfTheSea(itemStack);
                world.spawnEntity(new FishingBobberEntity(user, world, _snowman3, _snowman2));
            }
