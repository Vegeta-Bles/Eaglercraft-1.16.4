...
extends ItemUsageContext {
    ...
    public Direction[] getPlacementDirections() {
        int n;
        Direction[] directionArray = Direction.getEntityFacingOrder(this.getPlayer());
        if (this.canReplaceExisting) {
            return directionArray;
        }
        Direction _snowman2 = this.getSide();
        for (n = 0; n < directionArray.length && directionArray[n] != _snowman2.getOpposite(); ++n) {
        }
        if (n > 0) {
            System.arraycopy(directionArray, 0, directionArray, 1, n);
            directionArray[0] = _snowman2.getOpposite();
        }
        return directionArray;
    }
}
