...
extends MiningToolItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (context.getSide() != Direction.DOWN) {
            ...
            BlockState _snowman4 = PATH_STATES.get(_snowman3.getBlock());
            BlockState _snowman5 = null;
            if (_snowman4 != null && world.getBlockState(_snowman2.up()).isAir()) {
                world.playSound(playerEntity, _snowman2, SoundEvents.ITEM_SHOVEL_FLATTEN, SoundCategory.BLOCKS, 1.0f, 1.0f);
                _snowman5 = _snowman4;
            } else if (_snowman3.getBlock() instanceof CampfireBlock && _snowman3.get(CampfireBlock.LIT).booleanValue()) {
                if (!world.isClient()) {
                    world.syncWorldEvent(null, 1009, _snowman2, 0);
                }
                CampfireBlock.extinguish(world, _snowman2, _snowman3);
                _snowman5 = (BlockState)_snowman3.with(CampfireBlock.LIT, false);
            }
            if (_snowman5 != null) {
                if (!world.isClient) {
                    world.setBlockState(_snowman2, _snowman5, 11);
