...
public class LootContext {
    ...
    public static class Builder {
        ...
        public LootContext build(LootContextType type) {
            Sets.SetView setView = Sets.difference(this.parameters.keySet(), type.getAllowed());
            if (!setView.isEmpty()) {
                throw new IllegalArgumentException("Parameters not allowed in this parameter set: " + setView);
            }
            _snowman = Sets.difference(type.getRequired(), this.parameters.keySet());
            if (!_snowman.isEmpty()) {
                throw new IllegalArgumentException("Missing required parameters: " + _snowman);
            }
            Random _snowman2 = this.random;
            if (_snowman2 == null) {
                _snowman2 = new Random();
            }
            MinecraftServer _snowman3 = this.world.getServer();
            return new LootContext(_snowman2, this.luck, this.world, _snowman3.getLootManager()::getTable, _snowman3.getPredicateManager()::get, this.parameters, this.drops);
        }
    }
