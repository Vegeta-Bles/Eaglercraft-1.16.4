...
extends LootPoolEntry {
    ...
    private final LootChoice choice = new Choice(this){
        final /* synthetic */ LeafEntry field_1002;
        {
            this.field_1002 = leafEntry;
        }

        public void generateLoot(Consumer<ItemStack> lootConsumer, LootContext context) {
            this.field_1002.generateLoot(LootFunction.apply(LeafEntry.method_435(this.field_1002), lootConsumer, context), context);
        }
    };
    ...
    static /* synthetic */ BiFunction method_435(LeafEntry leafEntry) {
        return leafEntry.compiledFunctions;
    }

    public static abstract class Serializer<T extends LeafEntry>
    extends LootPoolEntry.Serializer<T> {
        @Override
        public void addEntryFields(JsonObject jsonObject, T t, JsonSerializationContext jsonSerializationContext) {
            if (((LeafEntry)t).weight != 1) {
                jsonObject.addProperty("weight", (Number)((LeafEntry)t).weight);
            }
            if (((LeafEntry)t).quality != 0) {
                jsonObject.addProperty("quality", (Number)((LeafEntry)t).quality);
            }
            if (!ArrayUtils.isEmpty((Object[])((LeafEntry)t).functions)) {
                jsonObject.add("functions", jsonSerializationContext.serialize((Object)((LeafEntry)t).functions));
            }
        }

        @Override
        public final T fromJson(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext, LootCondition[] lootConditionArray) {
            int n = JsonHelper.getInt(jsonObject, "weight", 1);
            _snowman = JsonHelper.getInt(jsonObject, "quality", 0);
            LootFunction[] _snowman2 = JsonHelper.deserialize(jsonObject, "functions", new LootFunction[0], jsonDeserializationContext, LootFunction[].class);
            return this.fromJson(jsonObject, jsonDeserializationContext, n, _snowman, lootConditionArray, _snowman2);
        }
