...
extends LeafEntry {
    ...
    private boolean grow(LootContext context, Consumer<LootChoice> lootChoiceExpander) {
        if (this.test(context)) {
            for (Item item : this.name.values()) {
                lootChoiceExpander.accept(new LeafEntry.Choice(this, item){
                    final /* synthetic */ Item field_1007;
                    final /* synthetic */ TagEntry field_1008;
                    {
                        this.field_1008 = tagEntry;
                        this.field_1007 = item;
                        super(tagEntry);
                    }
    ...
    extends LeafEntry.Serializer<TagEntry> {
        ...
        @Override
        protected TagEntry fromJson(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext, int n, int n2, LootCondition[] lootConditionArray, LootFunction[] lootFunctionArray) {
            Identifier identifier = new Identifier(JsonHelper.getString(jsonObject, "name"));
            Tag<Item> _snowman2 = ServerTagManagerHolder.getTagManager().getItems().getTag(identifier);
            if (_snowman2 == null) {
                throw new JsonParseException("Can't find tag: " + identifier);
            }
            boolean _snowman3 = JsonHelper.getBoolean(jsonObject, "expand");
            return new TagEntry(_snowman2, _snowman3, n, n2, lootConditionArray, lootFunctionArray);
        }
