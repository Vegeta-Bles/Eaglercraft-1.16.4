...
extends ConditionalLootFunction {
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        ...
        if (itemStack != null) {
            int n = EnchantmentHelper.getLevel(this.enchantment, itemStack);
            _snowman = this.formula.getValue(context.getRandom(), stack.getCount(), n);
    ...
    extends ConditionalLootFunction.Serializer<ApplyBonusLootFunction> {
        ...
        @Override
        public ApplyBonusLootFunction fromJson(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext, LootCondition[] lootConditionArray) {
            Identifier identifier = new Identifier(JsonHelper.getString(jsonObject, "enchantment"));
            Enchantment _snowman2 = Registry.ENCHANTMENT.getOrEmpty(identifier).orElseThrow(() -> new JsonParseException("Invalid enchantment id: " + identifier));
            _snowman = new Identifier(JsonHelper.getString(jsonObject, "formula"));
            FormulaFactory _snowman3 = (FormulaFactory)FACTORIES.get(_snowman);
            if (_snowman3 == null) {
                throw new JsonParseException("Invalid formula id: " + _snowman);
            }
            Formula _snowman4 = jsonObject.has("parameters") ? _snowman3.deserialize(JsonHelper.getObject(jsonObject, "parameters"), jsonDeserializationContext) : _snowman3.deserialize(new JsonObject(), jsonDeserializationContext);
            return new ApplyBonusLootFunction(lootConditionArray, _snowman2, _snowman4);
        }
    ...
    implements Formula {
        ...
        public int getValue(Random random, int initialCount, int enchantmentLevel) {
            if (enchantmentLevel > 0) {
                int n = random.nextInt(enchantmentLevel + 2) - 1;
                if (n < 0) {
                    n = 0;
                }
                return initialCount * (n + 1);
            }
            return initialCount;
        }
    ...
    implements Formula {
        ...
        public static Formula fromJson(JsonObject json, JsonDeserializationContext context) {
            int n = JsonHelper.getInt(json, "bonusMultiplier");
    ...
    implements Formula {
        ...
        public static Formula fromJson(JsonObject json, JsonDeserializationContext context) {
            int n = JsonHelper.getInt(json, "extra");
            float _snowman2 = JsonHelper.getFloat(json, "probability");
            return new BinomialWithBonusCount(n, _snowman2);
        }
