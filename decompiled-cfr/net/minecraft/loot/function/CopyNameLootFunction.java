...
extends ConditionalLootFunction {
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        Object obj = context.get(this.source.parameter);
        if (obj instanceof Nameable && (_snowman = (Nameable)obj).hasCustomName()) {
            stack.setCustomName(_snowman.getDisplayName());
        }
        return stack;
    }
    ...
    extends ConditionalLootFunction.Serializer<CopyNameLootFunction> {
        ...
        @Override
        public CopyNameLootFunction fromJson(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext, LootCondition[] lootConditionArray) {
            Source source = Source.get(JsonHelper.getString(jsonObject, "source"));
            return new CopyNameLootFunction(lootConditionArray, source);
        }
