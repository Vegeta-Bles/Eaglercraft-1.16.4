...
extends ConditionalLootFunction {
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        ...
        int _snowman2 = random.nextInt(this.effects.size());
        Map.Entry _snowman3 = (Map.Entry)Iterables.get(this.effects.entrySet(), (int)_snowman2);
        StatusEffect _snowman4 = (StatusEffect)_snowman3.getKey();
        int _snowman5 = ((UniformLootTableRange)_snowman3.getValue()).next(random);
        if (!_snowman4.isInstant()) {
            _snowman5 *= 20;
        }
        SuspiciousStewItem.addEffectToStew(stack, _snowman4, _snowman5);
        return stack;
    }
    ...
    extends ConditionalLootFunction.Serializer<SetStewEffectLootFunction> {
        ...
        public void toJson(JsonObject jsonObject, SetStewEffectLootFunction setStewEffectLootFunction, JsonSerializationContext jsonSerializationContext) {
            ...
            if (!setStewEffectLootFunction.effects.isEmpty()) {
                ...
                for (StatusEffect statusEffect : setStewEffectLootFunction.effects.keySet()) {
                    ...
                    Identifier _snowman2 = Registry.STATUS_EFFECT.getId(statusEffect);
                    if (_snowman2 == null) {
                        throw new IllegalArgumentException("Don't know how to serialize mob effect " + statusEffect);
                    }
                    jsonObject2.add("type", (JsonElement)new JsonPrimitive(_snowman2.toString()));
        ...
        @Override
        public SetStewEffectLootFunction fromJson(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext, LootCondition[] lootConditionArray) {
            ...
            if (jsonObject.has("effects")) {
                ...
                for (JsonElement jsonElement : jsonArray) {
                    ...
                    StatusEffect _snowman2 = Registry.STATUS_EFFECT.getOrEmpty(new Identifier(string)).orElseThrow(() -> new JsonSyntaxException("Unknown mob effect '" + string + "'"));
                    UniformLootTableRange _snowman3 = JsonHelper.deserialize(jsonElement.getAsJsonObject(), "duration", jsonDeserializationContext, UniformLootTableRange.class);
                    hashMap.put(_snowman2, _snowman3);
                }
            }
            return new SetStewEffectLootFunction(lootConditionArray, hashMap);
        }
