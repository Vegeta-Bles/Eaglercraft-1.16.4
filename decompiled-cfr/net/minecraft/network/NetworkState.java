...
public enum NetworkState {
    ...
    static {
        ...
        for (NetworkState networkState : NetworkState.values()) {
            int n = networkState.getId();
            if (n < -1 || n > 2) {
                throw new Error("Invalid protocol ID " + Integer.toString(n));
            }
            NetworkState.STATES[n - -1] = networkState;
            networkState.packetHandlers.forEach((networkSide, packetHandler) -> packetHandler.getPacketTypes().forEach(clazz -> {
                if (HANDLER_STATE_MAP.containsKey(clazz) && HANDLER_STATE_MAP.get(clazz) != networkState) {
                    throw new IllegalStateException("Packet " + clazz + " is already assigned to protocol " + (Object)((Object)HANDLER_STATE_MAP.get(clazz)) + " - can't reassign to " + (Object)((Object)networkState));
                }
                HANDLER_STATE_MAP.put((Class<Packet<?>>)clazz, networkState);
            }));
        }
    }
    ...
    static class PacketHandler<T extends PacketListener> {
        ...
        public <P extends Packet<T>> PacketHandler<T> register(Class<P> type, Supplier<P> factory) {
            int n = this.packetFactories.size();
            _snowman = this.packetIds.put(type, n);
            if (_snowman != -1) {
                String string = "Packet " + type + " is already registered to ID " + _snowman;
        ...
        public Integer getId(Class<?> packet) {
            int n = this.packetIds.getInt(packet);
