...
public class DistancePredicate {
    ...
    public boolean test(double x0, double y0, double z0, double x1, double y1, double z1) {
        float f = (float)(x0 - x1);
        _snowman = (float)(y0 - y1);
        _snowman = (float)(z0 - z1);
        if (!(this.x.test(MathHelper.abs(f)) && this.y.test(MathHelper.abs(_snowman)) && this.z.test(MathHelper.abs(_snowman)))) {
            return false;
        }
        if (!this.horizontal.testSqrt(f * f + _snowman * _snowman)) {
            return false;
        }
        return this.absolute.testSqrt(f * f + _snowman * _snowman + _snowman * _snowman);
    }
    ...
    public static DistancePredicate fromJson(@Nullable JsonElement json) {
        ...
        NumberRange.FloatRange _snowman2 = NumberRange.FloatRange.fromJson(jsonObject.get("x"));
        NumberRange.FloatRange _snowman3 = NumberRange.FloatRange.fromJson(jsonObject.get("y"));
        NumberRange.FloatRange _snowman4 = NumberRange.FloatRange.fromJson(jsonObject.get("z"));
        NumberRange.FloatRange _snowman5 = NumberRange.FloatRange.fromJson(jsonObject.get("horizontal"));
        NumberRange.FloatRange _snowman6 = NumberRange.FloatRange.fromJson(jsonObject.get("absolute"));
        return new DistancePredicate(_snowman2, _snowman3, _snowman4, _snowman5, _snowman6);
    }
