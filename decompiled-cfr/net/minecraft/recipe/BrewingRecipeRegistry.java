...
public class BrewingRecipeRegistry {
    ...
    protected static boolean isItemRecipeIngredient(ItemStack stack) {
        int n = ITEM_RECIPES.size();
        for (_snowman = 0; _snowman < n; ++_snowman) {
            if (!((Recipe)ITEM_RECIPES.get(_snowman)).ingredient.test(stack)) continue;
            return true;
        }
        return false;
    }
    ...
    protected static boolean isPotionRecipeIngredient(ItemStack stack) {
        int n = POTION_RECIPES.size();
        for (_snowman = 0; _snowman < n; ++_snowman) {
            if (!((Recipe)POTION_RECIPES.get(_snowman)).ingredient.test(stack)) continue;
            return true;
        }
        return false;
    }
    ...
    public static boolean isBrewable(Potion potion) {
        int n = POTION_RECIPES.size();
        for (_snowman = 0; _snowman < n; ++_snowman) {
            if (((Recipe)POTION_RECIPES.get(_snowman)).output != potion) continue;
            return true;
        }
        return false;
    }
    ...
    protected static boolean hasItemRecipe(ItemStack input, ItemStack ingredient) {
        ...
        int _snowman2 = ITEM_RECIPES.size();
    ...
    protected static boolean hasPotionRecipe(ItemStack input, ItemStack ingredient) {
        ...
        int _snowman2 = POTION_RECIPES.size();
        for (int i = 0; i < _snowman2; ++i) {
    ...
    public static ItemStack craft(ItemStack input, ItemStack ingredient) {
        if (!ingredient.isEmpty()) {
            ...
            Item _snowman2 = ingredient.getItem();
            int _snowman3 = ITEM_RECIPES.size();
            for (n = 0; n < _snowman3; ++n) {
                recipe = ITEM_RECIPES.get(n);
                if (((Recipe)recipe).input != _snowman2 || !((Recipe)recipe).ingredient.test(input)) continue;
                return PotionUtil.setPotion(new ItemStack((ItemConvertible)((Recipe)recipe).output), potion);
            }
            _snowman3 = POTION_RECIPES.size();
            for (n = 0; n < _snowman3; ++n) {
                recipe = POTION_RECIPES.get(n);
                if (((Recipe)recipe).input != potion || !((Recipe)recipe).ingredient.test(input)) continue;
                return PotionUtil.setPotion(new ItemStack(_snowman2), (Potion)((Recipe)recipe).output);
            }
        }
        return ingredient;
    }
