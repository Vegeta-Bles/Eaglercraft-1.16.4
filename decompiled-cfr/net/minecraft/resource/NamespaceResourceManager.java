...
implements ResourceManager {
    ...
    public Resource getResource(Identifier id) throws IOException {
        ...
        Identifier _snowman2 = NamespaceResourceManager.getMetadataPath(id);
        for (int i = this.packList.size() - 1; i >= 0; --i) {
            ResourcePack resourcePack2 = this.packList.get(i);
            if (resourcePack == null && resourcePack2.contains(this.type, _snowman2)) {
                resourcePack = resourcePack2;
            }
            if (!resourcePack2.contains(this.type, id)) continue;
            InputStream _snowman3 = null;
            if (resourcePack != null) {
                _snowman3 = this.open(_snowman2, resourcePack);
            }
            return new ResourceImpl(resourcePack2.getName(), id, this.open(id, resourcePack2), _snowman3);
        }
    ...
    public List<Resource> getAllResources(Identifier id) throws IOException {
        ...
        for (ResourcePack resourcePack : this.packList) {
            if (!resourcePack.contains(this.type, id)) continue;
            InputStream inputStream = resourcePack.contains(this.type, _snowman2) ? this.open(_snowman2, resourcePack) : null;
            arrayList.add(new ResourceImpl(resourcePack.getName(), id, this.open(id, resourcePack), inputStream));
        }
    ...
    public Collection<Identifier> findResources(String resourceType, Predicate<String> pathPredicate) {
        ArrayList arrayList = Lists.newArrayList();
        for (ResourcePack resourcePack : this.packList) {
            arrayList.addAll(resourcePack.findResources(this.type, this.namespace, resourceType, Integer.MAX_VALUE, pathPredicate));
        }
