...
implements AutoCloseable {
    ...
    public ServerResourceManager(CommandManager.RegistrationEnvironment registrationEnvironment, int n) {
        this.commandManager = new CommandManager(registrationEnvironment);
        this.functionLoader = new FunctionLoader(n, this.commandManager.getDispatcher());
    ...
    public static CompletableFuture<ServerResourceManager> reload(List<ResourcePack> list, CommandManager.RegistrationEnvironment registrationEnvironment, int n, Executor executor, Executor executor2) {
        ServerResourceManager serverResourceManager = new ServerResourceManager(registrationEnvironment, n);
        CompletableFuture<Unit> _snowman2 = serverResourceManager.resourceManager.beginReload(executor, executor2, list, field_25334);
        return ((CompletableFuture)_snowman2.whenComplete((unit, throwable) -> {
            if (throwable != null) {
