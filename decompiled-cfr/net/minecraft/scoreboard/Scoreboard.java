...
public class Scoreboard {
    ...
    public boolean playerHasObjective(String playerName, ScoreboardObjective objective) {
        ...
        ScoreboardPlayerScore _snowman2 = map.get(objective);
        return _snowman2 != null;
    }
    ...
    public Collection<ScoreboardPlayerScore> getAllPlayerScores(ScoreboardObjective objective) {
        ...
        for (Map<ScoreboardObjective, ScoreboardPlayerScore> map : this.playerObjectives.values()) {
            ...
            arrayList.add(scoreboardPlayerScore);
        }
        arrayList.sort(ScoreboardPlayerScore.COMPARATOR);
        return arrayList;
    }
    ...
    public Map<ScoreboardObjective, ScoreboardPlayerScore> getPlayerObjectives(String string) {
        HashMap hashMap = this.playerObjectives.get(string);
        if (hashMap == null) {
    ...
    public void removeTeam(Team team2) {
        Team team2;
        this.teams.remove(team2.getName());
        for (String string : team2.getPlayerList()) {
            this.teamsByPlayer.remove(string);
        }
        this.updateRemovedTeam(team2);
    }
    ...
    public static int getDisplaySlotId(String slotName) {
        ...
        if (slotName.startsWith("sidebar.team.") && (formatting = Formatting.byName(_snowman = slotName.substring("sidebar.team.".length()))) != null && formatting.getColorIndex() >= 0) {
    ...
    protected void fromTag(ListTag listTag) {
        for (int i = 0; i < listTag.size(); ++i) {
            ...
            ScoreboardObjective _snowman2 = this.getObjective(compoundTag.getString("Objective"));
            String _snowman3 = compoundTag.getString("Name");
            if (_snowman3.length() > 40) {
                _snowman3 = _snowman3.substring(0, 40);
            }
            ScoreboardPlayerScore _snowman4 = this.getPlayerScore(_snowman3, _snowman2);
            _snowman4.setScore(compoundTag.getInt("Score"));
            if (!compoundTag.contains("Locked")) continue;
            _snowman4.setLocked(compoundTag.getBoolean("Locked"));
        }
    }
}
