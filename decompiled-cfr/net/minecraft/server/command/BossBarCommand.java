...
public class BossBarCommand {
    ...
    private static int setName(ServerCommandSource source, CommandBossBar bossBar, Text name) throws CommandSyntaxException {
        MutableText mutableText = Texts.parse(source, name, null, 0);
        if (bossBar.getName().equals(mutableText)) {
            throw SET_NAME_UNCHANGED_EXCEPTION.create();
        }
        bossBar.setName(mutableText);
    ...
    private static int addBossBar(ServerCommandSource source, Identifier name, Text displayName) throws CommandSyntaxException {
        ...
        CommandBossBar _snowman2 = bossBarManager.add(name, Texts.parse(source, displayName, null, 0));
        source.sendFeedback(new TranslatableText("commands.bossbar.create.success", _snowman2.toHoverableText()), true);
    ...
    public static CommandBossBar getBossBar(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        CommandBossBar _snowman2 = ((ServerCommandSource)context.getSource()).getMinecraftServer().getBossBarManager().get(identifier);
        if (_snowman2 == null) {
