    ...
    private static int execute(ServerCommandSource source, Collection<? extends Entity> targets, Enchantment enchantment, int level) throws CommandSyntaxException {
        ...
        for (Entity entity : targets) {
            if (entity instanceof LivingEntity) {
                ...
                ItemStack _snowman2 = livingEntity.getMainHandStack();
                if (!_snowman2.isEmpty()) {
                    if (enchantment.isAcceptableItem(_snowman2) && EnchantmentHelper.isCompatible(EnchantmentHelper.get(_snowman2).keySet(), enchantment)) {
                        _snowman2.addEnchantment(enchantment, level);
                        ++n;
                        continue;
                    }
                    if (targets.size() != 1) continue;
                    throw FAILED_INCOMPATIBLE_EXCEPTION.create((Object)_snowman2.getItem().getName(_snowman2).getString());
                }
                if (targets.size() != 1) continue;
