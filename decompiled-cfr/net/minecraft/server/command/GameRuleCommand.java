...
public class GameRuleCommand {
    ...
    private static <T extends GameRules.Rule<T>> int executeSet(CommandContext<ServerCommandSource> context, GameRules.Key<T> key) {
        ...
        T _snowman2 = serverCommandSource.getMinecraftServer().getGameRules().get(key);
        ((GameRules.Rule)_snowman2).set(context, "value");
        serverCommandSource.sendFeedback(new TranslatableText("commands.gamerule.set", key.getName(), ((GameRules.Rule)_snowman2).toString()), true);
        return ((GameRules.Rule)_snowman2).getCommandResult();
    }

    private static <T extends GameRules.Rule<T>> int executeQuery(ServerCommandSource source, GameRules.Key<T> key) {
        T t = source.getMinecraftServer().getGameRules().get(key);
        source.sendFeedback(new TranslatableText("commands.gamerule.query", key.getName(), ((GameRules.Rule)t).toString()), false);
        return ((GameRules.Rule)t).getCommandResult();
    }
}
