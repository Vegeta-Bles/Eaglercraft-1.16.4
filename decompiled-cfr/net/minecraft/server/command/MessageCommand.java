...
public class MessageCommand {
    ...
    private static int execute(ServerCommandSource source, Collection<ServerPlayerEntity> targets, Text message) {
        Consumer<Text> _snowman3;
        UUID uUID = source.getEntity() == null ? Util.NIL_UUID : source.getEntity().getUuid();
        Entity _snowman2 = source.getEntity();
        if (_snowman2 instanceof ServerPlayerEntity) {
            ServerPlayerEntity serverPlayerEntity = (ServerPlayerEntity)_snowman2;
            _snowman3 = text2 -> serverPlayerEntity.sendSystemMessage(new TranslatableText("commands.message.display.outgoing", text2, message).formatted(Formatting.GRAY, Formatting.ITALIC), serverPlayerEntity.getUuid());
        } else {
            _snowman3 = text2 -> source.sendFeedback(new TranslatableText("commands.message.display.outgoing", text2, message).formatted(Formatting.GRAY, Formatting.ITALIC), false);
        }
        for (ServerPlayerEntity serverPlayerEntity : targets) {
            _snowman3.accept(serverPlayerEntity.getDisplayName());
