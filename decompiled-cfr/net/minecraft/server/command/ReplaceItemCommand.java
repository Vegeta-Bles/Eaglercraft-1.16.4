...
public class ReplaceItemCommand {
    ...
    private static int executeBlock(ServerCommandSource source, BlockPos pos, int slot, ItemStack item) throws CommandSyntaxException {
        ...
        Inventory _snowman2 = (Inventory)((Object)blockEntity);
        if (slot < 0 || slot >= _snowman2.size()) {
            throw SLOT_INAPPLICABLE_EXCEPTION.create((Object)slot);
        }
        _snowman2.setStack(slot, item);
    ...
    private static int executeEntity(ServerCommandSource source, Collection<? extends Entity> targets, int slot, ItemStack item) throws CommandSyntaxException {
        ArrayList arrayList = Lists.newArrayListWithCapacity((int)targets.size());
        ...
        if (arrayList.size() == 1) {
            source.sendFeedback(new TranslatableText("commands.replaceitem.entity.success.single", ((Entity)arrayList.iterator().next()).getDisplayName(), item.toHoverableText()), true);
        } else {
            source.sendFeedback(new TranslatableText("commands.replaceitem.entity.success.multiple", arrayList.size(), item.toHoverableText()), true);
        }
