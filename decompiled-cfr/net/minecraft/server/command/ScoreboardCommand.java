...
public class ScoreboardCommand {
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> makeRenderTypeArguments() {
        ...
        for (ScoreboardCriterion.RenderType renderType : ScoreboardCriterion.RenderType.values()) {
            literalArgumentBuilder.then(CommandManager.literal(renderType.getName()).executes(commandContext -> ScoreboardCommand.executeModifyRenderType((ServerCommandSource)commandContext.getSource(), ObjectiveArgumentType.getObjective((CommandContext<ServerCommandSource>)commandContext, "objective"), renderType)));
        }
    ...
    private static CompletableFuture<Suggestions> suggestDisabled(ServerCommandSource source, Collection<String> targets, SuggestionsBuilder builder) {
        ...
        ServerScoreboard _snowman2 = source.getMinecraftServer().getScoreboard();
        for (ScoreboardObjective scoreboardObjective : _snowman2.getObjectives()) {
            if (scoreboardObjective.getCriterion() != ScoreboardCriterion.TRIGGER) continue;
            boolean bl = false;
            for (String string : targets) {
                if (_snowman2.playerHasObjective(string, scoreboardObjective) && !_snowman2.getPlayerScore(string, scoreboardObjective).isLocked()) continue;
                bl = true;
                break;
            }
            if (!bl) continue;
    ...
    private static int executeGet(ServerCommandSource source, String target, ScoreboardObjective objective) throws CommandSyntaxException {
        ...
        if (!serverScoreboard.playerHasObjective(target, objective)) {
            throw PLAYERS_GET_NULL_EXCEPTION.create((Object)objective.getName(), (Object)target);
        }
        ScoreboardPlayerScore _snowman2 = serverScoreboard.getPlayerScore(target, objective);
        source.sendFeedback(new TranslatableText("commands.scoreboard.players.get.success", target, _snowman2.getScore(), objective.toHoverableText()), false);
        return _snowman2.getScore();
    }
    ...
    private static int executeOperation(ServerCommandSource source, Collection<String> targets, ScoreboardObjective targetObjective, OperationArgumentType.Operation operation, Collection<String> sources, ScoreboardObjective sourceObjectives) throws CommandSyntaxException {
        ...
        int _snowman2 = 0;
        for (String string : targets) {
            ScoreboardPlayerScore scoreboardPlayerScore = serverScoreboard.getPlayerScore(string, targetObjective);
            for (String string2 : sources) {
                ScoreboardPlayerScore scoreboardPlayerScore2 = serverScoreboard.getPlayerScore(string2, sourceObjectives);
                operation.apply(scoreboardPlayerScore, scoreboardPlayerScore2);
            }
            _snowman2 += scoreboardPlayerScore.getScore();
        }
        if (targets.size() == 1) {
            source.sendFeedback(new TranslatableText("commands.scoreboard.players.operation.success.single", targetObjective.toHoverableText(), targets.iterator().next(), _snowman2), true);
        } else {
    ...
    private static int executeAdd(ServerCommandSource source, Collection<String> targets, ScoreboardObjective objective, int score) {
        ...
        if (targets.size() == 1) {
            source.sendFeedback(new TranslatableText("commands.scoreboard.players.add.success.single", score, objective.toHoverableText(), targets.iterator().next(), _snowman2), true);
        } else {
    ...
    private static int executeRemove(ServerCommandSource source, Collection<String> targets, ScoreboardObjective objective, int score) {
        ...
        if (targets.size() == 1) {
            source.sendFeedback(new TranslatableText("commands.scoreboard.players.remove.success.single", score, objective.toHoverableText(), targets.iterator().next(), _snowman2), true);
        } else {
    ...
    private static int executeClearDisplay(ServerCommandSource source, int slot) throws CommandSyntaxException {
        ServerScoreboard serverScoreboard = source.getMinecraftServer().getScoreboard();
        if (serverScoreboard.getObjectiveForSlot(slot) == null) {
            throw OBJECTIVES_DISPLAY_ALREADY_EMPTY_EXCEPTION.create();
        }
        ((Scoreboard)serverScoreboard).setObjectiveSlot(slot, null);
    ...
    private static int executeSetDisplay(ServerCommandSource source, int slot, ScoreboardObjective objective) throws CommandSyntaxException {
        ServerScoreboard serverScoreboard = source.getMinecraftServer().getScoreboard();
        if (serverScoreboard.getObjectiveForSlot(slot) == objective) {
            throw OBJECTIVES_DISPLAY_ALREADY_SET_EXCEPTION.create();
        }
        ((Scoreboard)serverScoreboard).setObjectiveSlot(slot, objective);
    ...
    private static int executeAddObjective(ServerCommandSource source, String objective, ScoreboardCriterion criteria, Text displayName) throws CommandSyntaxException {
        ServerScoreboard serverScoreboard = source.getMinecraftServer().getScoreboard();
        if (serverScoreboard.getNullableObjective(objective) != null) {
        ...
        serverScoreboard.addObjective(objective, criteria, displayName, criteria.getCriterionType());
        ScoreboardObjective _snowman2 = serverScoreboard.getNullableObjective(objective);
        source.sendFeedback(new TranslatableText("commands.scoreboard.objectives.add.success", _snowman2.toHoverableText()), true);
        return serverScoreboard.getObjectives().size();
    }
