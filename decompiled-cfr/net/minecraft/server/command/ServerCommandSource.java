...
implements CommandSource {
    ...
    public ServerCommandSource withWorld(ServerWorld world) {
        ...
        Vec3d _snowman2 = new Vec3d(this.position.x * d, this.position.y, this.position.z * d);
        return new ServerCommandSource(this.output, _snowman2, this.rotation, world, this.level, this.simpleName, this.name, this.server, this.entity, this.silent, this.resultConsumer, this.entityAnchor);
    }
    ...
    public ServerCommandSource withLookingAt(Vec3d position) throws CommandSyntaxException {
        ...
        double _snowman2 = position.x - vec3d.x;
        double _snowman3 = position.y - vec3d.y;
        double _snowman4 = position.z - vec3d.z;
        double _snowman5 = MathHelper.sqrt(_snowman2 * _snowman2 + _snowman4 * _snowman4);
        float _snowman6 = MathHelper.wrapDegrees((float)(-(MathHelper.atan2(_snowman3, _snowman5) * 57.2957763671875)));
        float _snowman7 = MathHelper.wrapDegrees((float)(MathHelper.atan2(_snowman4, _snowman2) * 57.2957763671875) - 90.0f);
        return this.withRotation(new Vec2f(_snowman6, _snowman7));
    }
    ...
    private void sendToOps(Text message) {
        MutableText mutableText = new TranslatableText("chat.type.admin", this.getDisplayName(), message).formatted(Formatting.GRAY, Formatting.ITALIC);
        ...
        if (this.output != this.server && this.server.getGameRules().getBoolean(GameRules.LOG_ADMIN_COMMANDS)) {
            this.server.sendSystemMessage(mutableText, Util.NIL_UUID);
        }
    }
