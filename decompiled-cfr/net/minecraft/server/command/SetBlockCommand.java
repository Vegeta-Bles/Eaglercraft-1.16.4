...
public class SetBlockCommand {
    ...
    private static int execute(ServerCommandSource source, BlockPos pos, BlockStateArgument block, Mode mode, @Nullable Predicate<CachedBlockPosition> condition) throws CommandSyntaxException {
        ...
        if (mode == Mode.DESTROY) {
            ...
            _snowman3 = !block.getBlockState().isAir() || !serverWorld.getBlockState(pos).isAir();
        } else {
            BlockEntity _snowman2 = serverWorld.getBlockEntity(pos);
            Clearable.clear(_snowman2);
            _snowman3 = true;
        }
        if (_snowman3 && !block.setBlockState(serverWorld, pos, 2)) {
            throw FAILED_EXCEPTION.create();
        }
