...
public class TeamCommand {
    ...
    private static int executeLeave(ServerCommandSource source, Collection<String> members) {
        ...
        for (String string : members) {
            serverScoreboard.clearPlayerTeam(string);
        }
        if (members.size() == 1) {
    ...
    private static int executeJoin(ServerCommandSource source, Team team, Collection<String> members) {
        ServerScoreboard serverScoreboard = source.getMinecraftServer().getScoreboard();
        for (String string : members) {
            ((Scoreboard)serverScoreboard).addPlayerToTeam(string, team);
        }
        if (members.size() == 1) {
    ...
    private static int executeEmpty(ServerCommandSource source, Team team) throws CommandSyntaxException {
        ServerScoreboard serverScoreboard = source.getMinecraftServer().getScoreboard();
        ArrayList _snowman2 = Lists.newArrayList(team.getPlayerList());
        if (_snowman2.isEmpty()) {
            throw EMPTY_UNCHANGED_EXCEPTION.create();
        }
        for (String string : _snowman2) {
            ((Scoreboard)serverScoreboard).removePlayerFromTeam(string, team);
        }
        source.sendFeedback(new TranslatableText("commands.team.empty.success", _snowman2.size(), team.getFormattedName()), true);
    ...
    private static int executeAdd(ServerCommandSource source, String team, Text displayName) throws CommandSyntaxException {
        ServerScoreboard serverScoreboard = source.getMinecraftServer().getScoreboard();
        if (serverScoreboard.getTeam(team) != null) {
            throw ADD_DUPLICATE_EXCEPTION.create();
        }
        if (team.length() > 16) {
            throw ADD_LONG_NAME_EXCEPTION.create((Object)16);
        }
        Team _snowman2 = serverScoreboard.addTeam(team);
        _snowman2.setDisplayName(displayName);
        source.sendFeedback(new TranslatableText("commands.team.add.success", _snowman2.getFormattedName()), true);
        return serverScoreboard.getTeams().size();
    }
