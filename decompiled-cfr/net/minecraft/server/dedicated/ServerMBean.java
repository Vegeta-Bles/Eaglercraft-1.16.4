...
implements DynamicMBean {
    ...
    private ServerMBean(MinecraftServer server) {
        this.server = server;
        MBeanAttributeInfo[] mBeanAttributeInfoArray = (MBeanAttributeInfo[])this.entries.values().stream().map(object -> ((Entry)object).createInfo()).toArray(MBeanAttributeInfo[]::new);
        this.beanInfo = new MBeanInfo(ServerMBean.class.getSimpleName(), "metrics for dedicated server", mBeanAttributeInfoArray, null, null, new MBeanNotificationInfo[0]);
    }
