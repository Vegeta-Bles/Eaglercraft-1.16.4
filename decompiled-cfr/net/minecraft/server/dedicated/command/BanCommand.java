...
public class BanCommand {
    ...
    private static int ban(ServerCommandSource source, Collection<GameProfile> targets, @Nullable Text reason) throws CommandSyntaxException {
        ...
        for (GameProfile gameProfile : targets) {
            ...
            ServerPlayerEntity _snowman3 = source.getMinecraftServer().getPlayerManager().getPlayer(gameProfile.getId());
            if (_snowman3 == null) continue;
            _snowman3.networkHandler.disconnect(new TranslatableText("multiplayer.disconnect.banned"));
        }
        if (_snowman2 == 0) {
