...
implements AutoCloseable {
    ...
    private CompletableFuture<Optional<String>> filterMessage(GameProfile gameProfile, String message, HashIgnorer ignorer, Executor executor) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                boolean _snowman2 = JsonHelper.getBoolean(jsonObject2, "response", false);
                if (_snowman2) {
                    return Optional.of(message);
                }
                String _snowman3 = JsonHelper.getString(jsonObject2, "hashed", null);
                if (_snowman3 == null) {
                    return Optional.empty();
                }
                int _snowman4 = JsonHelper.getArray(jsonObject2, "hashes").size();
                return ignorer.shouldIgnore(_snowman3, _snowman4) ? Optional.empty() : Optional.of(_snowman3);
            }
            catch (Exception exception) {
    ...
    private JsonObject sendJsonRequest(JsonObject payload, URL endpoint) throws IOException {
        ...
        try (InputStream _snowman2 = httpURLConnection.getInputStream();){
            ...
            try {
                jsonObject = Streams.parse((JsonReader)new JsonReader((Reader)new InputStreamReader(_snowman2))).getAsJsonObject();
            }
            catch (Throwable throwable2) {
                try {
                    this.consumeFully(_snowman2);
                    throw throwable2;
                }
    ...
    private HttpURLConnection createConnection(JsonObject payload, URL endpoint) throws IOException {
        ...
        try (OutputStreamWriter _snowman2 = new OutputStreamWriter(httpURLConnection.getOutputStream(), StandardCharsets.UTF_8);
             JsonWriter jsonWriter = new JsonWriter((Writer)_snowman2);){
            Streams.write((JsonElement)payload, (JsonWriter)jsonWriter);
        }
        int _snowman3 = httpURLConnection.getResponseCode();
        if (_snowman3 < 200 || _snowman3 >= 300) {
            throw new FailedHttpRequestException(_snowman3 + " " + httpURLConnection.getResponseMessage());
        }
    ...
    public static interface HashIgnorer {
        public static final HashIgnorer NEVER_IGNORE = (string, n) -> false;
        public static final HashIgnorer IGNORE_IF_MATCHES_ALL = (string, n) -> string.length() == n;
    ...
    implements TextStream {
        ...
        public CompletableFuture<Optional<List<String>>> filterTexts(List<String> texts) {
            List list2 = (List)texts.stream().map(string -> TextFilterer.this.filterMessage(this.gameProfile, string, TextFilterer.this.ignorer, this.executor)).collect(ImmutableList.toImmutableList());
