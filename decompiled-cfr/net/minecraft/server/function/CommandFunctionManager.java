...
public class CommandFunctionManager {
    ...
    public void tick() {
        ...
        if (this.needToRunLoadFunctions) {
            ...
            List<CommandFunction> list = this.field_25333.getTags().getTagOrEmpty(LOAD_FUNCTION).values();
            this.method_29460(list, LOAD_FUNCTION);
        }
    }
    ...
    public int execute(CommandFunction function, ServerCommandSource source) {
        ...
        if (this.executing) {
            if (this.chain.size() + this.pending.size() < n) {
