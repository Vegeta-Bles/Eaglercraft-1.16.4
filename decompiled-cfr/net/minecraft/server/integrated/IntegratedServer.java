...
extends MinecraftServer {
    ...
    public void tick(BooleanSupplier shouldKeepTicking) {
        ...
        Profiler _snowman2 = this.getProfiler();
        if (!bl && this.paused) {
            _snowman2.push("autoSave");
        ...
        int _snowman3 = Math.max(2, this.client.options.viewDistance + -1);
        if (_snowman3 != this.getPlayerManager().getViewDistance()) {
            LOGGER.info("Changing view distance to {}, from {}", (Object)_snowman3, (Object)this.getPlayerManager().getViewDistance());
            this.getPlayerManager().setViewDistance(_snowman3);
        }
    }
    ...
    public boolean openToLan(GameMode gameMode, boolean cheatsAllowed, int port) {
        try {
            ...
            int n = this.getPermissionLevel(this.client.player.getGameProfile());
            this.client.player.setClientPermissionLevel(n);
    ...
    public void stop(boolean bl) {
        this.submitAndJoin(() -> {
            ArrayList arrayList = Lists.newArrayList(this.getPlayerManager().getPlayerList());
