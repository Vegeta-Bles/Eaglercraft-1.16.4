...
implements ServerLoginPacketListener {
    ...
    public void onKey(LoginKeyC2SPacket packet) {
        ...
        try {
            ...
            object = NetworkEncryptionUtils.cipherFromKey(2, this.secretKey);
            Cipher _snowman2 = NetworkEncryptionUtils.cipherFromKey(1, this.secretKey);
            _snowman3 = new BigInteger(NetworkEncryptionUtils.generateServerId("", this.server.getKeyPair().getPublic(), this.secretKey)).toString(16);
            this.state = State.AUTHENTICATING;
            this.connection.setupEncryption((Cipher)object, _snowman2);
        }
        ...
        object = new Thread("User Authenticator #" + authenticatorThreadId.incrementAndGet()){

            @Override
            public void run() {
                GameProfile gameProfile = ServerLoginNetworkHandler.this.profile;
                try {
                    ServerLoginNetworkHandler.this.profile = ServerLoginNetworkHandler.this.server.getSessionService().hasJoinedServer(new GameProfile(null, gameProfile.getName()), _snowman3, this.getClientAddress());
                ...
                catch (AuthenticationUnavailableException _snowman2) {
        ...
        ((Thread)object).setUncaughtExceptionHandler(new UncaughtExceptionLogger(LOGGER));
        ((Thread)object).start();
    }
