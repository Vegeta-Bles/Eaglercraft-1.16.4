...
implements ScreenHandlerListener {
    ...
    private void moveToSpawn(ServerWorld world) {
        ...
        if (world.getDimension().hasSkyLight() && world.getServer().getSaveProperties().getGameMode() != GameMode.ADVENTURE) {
            int n = Math.max(0, this.server.getSpawnRadius(world));
            _snowman = MathHelper.floor(world.getWorldBorder().getDistanceInsideBorder(blockPos.getX(), blockPos.getZ()));
            if (_snowman < n) {
                n = _snowman;
            }
            if (_snowman <= 1) {
            ...
            _snowman = (_snowman = (_snowman = (long)(n * 2 + 1)) * _snowman) > Integer.MAX_VALUE ? Integer.MAX_VALUE : (int)_snowman;
            _snowman = this.calculateSpawnOffsetMultiplier(_snowman);
            _snowman = new Random().nextInt(_snowman);
            for (_snowman = 0; _snowman < _snowman; ++_snowman) {
                _snowman = (_snowman + _snowman * _snowman) % _snowman;
                _snowman = _snowman % (n * 2 + 1);
                _snowman = _snowman / (n * 2 + 1);
                BlockPos blockPos2 = SpawnLocating.findOverworldSpawn(world, blockPos.getX() + _snowman - n, blockPos.getZ() + _snowman - n, false);
    ...
    public void readCustomDataFromTag(CompoundTag tag) {
        ...
        if (tag.contains("SpawnX", 99) && tag.contains("SpawnY", 99) && tag.contains("SpawnZ", 99)) {
            ...
            if (tag.contains("SpawnDimension")) {
                this.spawnPointDimension = World.CODEC.parse((DynamicOps)NbtOps.INSTANCE, (Object)tag.get("SpawnDimension")).resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).orElse(World.OVERWORLD);
            }
        }
    }
    ...
    public void writeCustomDataToTag(CompoundTag tag2) {
        ...
        if (this.enteredNetherPos != null) {
            ...
            ((CompoundTag)object).putDouble("x", this.enteredNetherPos.x);
            ((CompoundTag)object).putDouble("y", this.enteredNetherPos.y);
            ((CompoundTag)object).putDouble("z", this.enteredNetherPos.z);
            tag2.put("enteredNetherPosition", (Tag)object);
        }
        object = this.getRootVehicle();
        Entity _snowman2 = this.getVehicle();
        if (_snowman2 != null && object != this && ((Entity)object).hasPlayerRider()) {
            CompoundTag compoundTag = new CompoundTag();
            _snowman = new CompoundTag();
            ((Entity)object).saveToTag(_snowman);
            compoundTag.putUuid("Attach", _snowman2.getUuid());
            compoundTag.put("Entity", _snowman);
            tag2.put("RootVehicle", compoundTag);
        }
        ...
        if (this.spawnPointPosition != null) {
            ...
            Identifier.CODEC.encodeStart((DynamicOps)NbtOps.INSTANCE, (Object)this.spawnPointDimension.getValue()).resultOrPartial(arg_0 -> ((Logger)LOGGER).error(arg_0)).ifPresent(tag -> tag2.put("SpawnDimension", (Tag)tag));
        }
    }
    ...
    public void setExperiencePoints(int n) {
        ...
        _snowman = (f - 1.0f) / f;
        this.experienceProgress = MathHelper.clamp((float)n / f, 0.0f, _snowman);
    ...
    public void tick() {
        ...
        while (!this.removedEntities.isEmpty()) {
            int n = Math.min(this.removedEntities.size(), Integer.MAX_VALUE);
            int[] _snowman2 = new int[n];
            Iterator<Integer> _snowman3 = this.removedEntities.iterator();
            _snowman = 0;
            while (_snowman3.hasNext() && _snowman < n) {
                _snowman2[_snowman++] = _snowman3.next();
                _snowman3.remove();
            }
            this.networkHandler.sendPacket(new EntitiesDestroyS2CPacket(_snowman2));
        }
    ...
    public void playerTick() {
        try {
            ...
            for (int i = 0; i < this.inventory.size(); ++i) {
                ...
                if (!itemStack.getItem().isNetworkSynced() || (_snowman = ((NetworkSyncedItem)itemStack.getItem()).createSyncPacket(itemStack, this.world, this)) == null) continue;
            ...
            CrashReportSection _snowman2 = crashReport.addElement("Player being ticked");
            this.populateCrashReport(_snowman2);
    ...
    public void onDeath(DamageSource source) {
        ...
        if (bl) {
            Object object = this.getDamageTracker().getDeathMessage();
            this.networkHandler.sendPacket(new CombatEventS2CPacket(this.getDamageTracker(), CombatEventS2CPacket.Type.ENTITY_DIED, (Text)object), (GenericFutureListener<? extends Future<? super Void>>)((GenericFutureListener)arg_0 -> this.method_14223((Text)object, arg_0)));
            AbstractTeam _snowman2 = this.getScoreboardTeam();
            if (_snowman2 == null || _snowman2.getDeathMessageVisibilityRule() == AbstractTeam.VisibilityRule.ALWAYS) {
                this.server.getPlayerManager().broadcastChatMessage((Text)object, MessageType.SYSTEM, Util.NIL_UUID);
            } else if (_snowman2.getDeathMessageVisibilityRule() == AbstractTeam.VisibilityRule.HIDE_FOR_OTHER_TEAMS) {
                this.server.getPlayerManager().sendToTeam(this, (Text)object);
            } else if (_snowman2.getDeathMessageVisibilityRule() == AbstractTeam.VisibilityRule.HIDE_FOR_OWN_TEAM) {
                this.server.getPlayerManager().sendToOtherTeams(this, (Text)object);
            }
        } else {
        ...
        object = this.getPrimeAdversary();
        if (object != null) {
            this.incrementStat(Stats.KILLED_BY.getOrCreateStat(((Entity)object).getType()));
            ((Entity)object).updateKilledAdvancementCriterion(this, this.scoreAmount, source);
            this.onKilledBy((LivingEntity)object);
        }
    ...
    public void updateKilledAdvancementCriterion(Entity killer, int score, DamageSource damageSource) {
        ...
        this.updateScoreboardScore(string, _snowman, ScoreboardCriterion.TEAM_KILLS);
        this.updateScoreboardScore(_snowman, string, ScoreboardCriterion.KILLED_BY_TEAMS);
        Criteria.PLAYER_KILLED_ENTITY.trigger(this, killer, damageSource);
    }

    private void updateScoreboardScore(String playerName, String team, ScoreboardCriterion[] scoreboardCriterionArray) {
        Team team2 = this.getScoreboard().getPlayerTeam(team);
        if (team2 != null && (_snowman = team2.getColor().getColorIndex()) >= 0 && _snowman < scoreboardCriterionArray.length) {
            this.getScoreboard().forEachScore(scoreboardCriterionArray[_snowman], playerName, ScoreboardPlayerScore::incrementScore);
        }
    }
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if (source instanceof EntityDamageSource) {
            ...
            if (entity instanceof PersistentProjectileEntity && (_snowman = (_snowman = (PersistentProjectileEntity)entity).getOwner()) instanceof PlayerEntity && !this.shouldDamagePlayer((PlayerEntity)_snowman)) {
                return false;
            }
        }
    ...
    public Entity moveToWorld(ServerWorld destination) {
        ...
        if (_snowman2 == World.END && destination.getRegistryKey() == World.OVERWORLD) {
            this.detach();
        ...
        WorldProperties _snowman3 = destination.getLevelProperties();
        ...
        this.networkHandler.sendPacket(new DifficultyS2CPacket(_snowman3.getDifficulty(), _snowman3.isDifficultyLocked()));
        PlayerManager _snowman4 = this.server.getPlayerManager();
        _snowman4.sendCommandTree(this);
        serverWorld.removePlayer(this);
        this.removed = false;
        TeleportTarget _snowman5 = this.getTeleportTarget(destination);
        if (_snowman5 != null) {
            serverWorld.getProfiler().push("moving");
            if (_snowman2 == World.OVERWORLD && destination.getRegistryKey() == World.NETHER) {
                this.enteredNetherPos = this.getPos();
            } else if (destination.getRegistryKey() == World.END) {
                this.createEndSpawnPlatform(destination, new BlockPos(_snowman5.position));
            }
            serverWorld.getProfiler().pop();
            serverWorld.getProfiler().push("placing");
            this.setWorld(destination);
            destination.onPlayerChangeDimension(this);
            this.setRotation(_snowman5.yaw, _snowman5.pitch);
            this.refreshPositionAfterTeleport(_snowman5.position.x, _snowman5.position.y, _snowman5.position.z);
            serverWorld.getProfiler().pop();
            ...
            _snowman4.sendWorldInfo(this, destination);
            _snowman4.sendPlayerStatus(this);
        ...
        for (int i = -2; i <= 2; ++i) {
            for (_snowman = -2; _snowman <= 2; ++_snowman) {
                for (_snowman = -1; _snowman < 3; ++_snowman) {
                    BlockState blockState = _snowman == -1 ? Blocks.OBSIDIAN.getDefaultState() : Blocks.AIR.getDefaultState();
                    world.setBlockState(mutable.set(centerPos).move(_snowman, _snowman, i), blockState);
                }
            }
        }
    }
    ...
    protected Optional<class_5459.class_5460> method_30330(ServerWorld serverWorld, BlockPos blockPos, boolean bl) {
        ...
        Direction.Axis _snowman2 = this.world.getBlockState(this.lastNetherPortalPosition).method_28500(NetherPortalBlock.AXIS).orElse(Direction.Axis.X);
        _snowman = serverWorld.getPortalForcer().method_30482(blockPos, _snowman2);
    ...
    private void worldChanged(ServerWorld origin) {
        ...
        _snowman = this.world.getRegistryKey();
        Criteria.CHANGED_DIMENSION.trigger(this, registryKey, _snowman);
        if (registryKey == World.NETHER && _snowman == World.OVERWORLD && this.enteredNetherPos != null) {
            Criteria.NETHER_TRAVEL.trigger(this, this.enteredNetherPos);
        }
        if (_snowman != World.NETHER) {
    ...
    public Either<PlayerEntity.SleepFailureReason, Unit> trySleep(BlockPos pos) {
        ...
        if (!this.isCreative()) {
            double d = 8.0;
            _snowman = 5.0;
            Vec3d _snowman2 = Vec3d.ofBottomCenter(pos);
            List<HostileEntity> _snowman3 = this.world.getEntitiesByClass(HostileEntity.class, new Box(_snowman2.getX() - 8.0, _snowman2.getY() - 5.0, _snowman2.getZ() - 8.0, _snowman2.getX() + 8.0, _snowman2.getY() + 5.0, _snowman2.getZ() + 8.0), hostileEntity -> hostileEntity.isAngryAt(this));
    ...
    public boolean startRiding(Entity entity, boolean force) {
        ...
        _snowman = this.getVehicle();
        if (_snowman != entity2 && this.networkHandler != null) {
            this.networkHandler.requestTeleport(this.getX(), this.getY(), this.getZ(), this.yaw, this.pitch);
        }
        return true;
    }
    ...
    public void stopRiding() {
        ...
        super.stopRiding();
        _snowman = this.getVehicle();
        if (_snowman != entity && this.networkHandler != null) {
            this.networkHandler.requestTeleport(this.getX(), this.getY(), this.getZ(), this.yaw, this.pitch);
        }
    }
    ...
    public void method_14218(float f, float f2, boolean bl, boolean bl2) {
        if (this.hasVehicle()) {
            if (f >= -1.0f && f <= 1.0f) {
    ...
    public void unlockRecipes(Identifier[] ids) {
        ...
        for (Identifier identifier : ids) {
            this.server.getRecipeManager().get(identifier).ifPresent(arrayList::add);
        }
    ...
    public void sendMessage(Text message, MessageType type, UUID senderUuid) {
        this.networkHandler.sendPacket(new GameMessageS2CPacket(message, type, senderUuid), (GenericFutureListener<? extends Future<? super Void>>)((GenericFutureListener)future -> {
            if (!(future.isSuccess() || type != MessageType.GAME_INFO && type != MessageType.SYSTEM)) {
                int n = 256;
                String _snowman2 = message.asTruncatedString(256);
                MutableText _snowman3 = new LiteralText(_snowman2).formatted(Formatting.YELLOW);
                this.networkHandler.sendPacket(new GameMessageS2CPacket(new TranslatableText("multiplayer.message_not_delivered", _snowman3).formatted(Formatting.RED), MessageType.SYSTEM, senderUuid));
            }
        }));
    }
    ...
    public void teleport(ServerWorld targetWorld, double x, double y, double z, float yaw, float pitch) {
        ...
        if (targetWorld == this.world) {
            ...
            ServerWorld serverWorld = this.getServerWorld();
            WorldProperties _snowman2 = targetWorld.getLevelProperties();
            ...
            this.networkHandler.sendPacket(new DifficultyS2CPacket(_snowman2.getDifficulty(), _snowman2.isDifficultyLocked()));
    ...
    public void setSpawnPoint(RegistryKey<World> dimension, @Nullable BlockPos pos, float angle, boolean spawnPointSet, boolean bl) {
        if (pos != null) {
            boolean bl2 = _snowman = pos.equals(this.spawnPointPosition) && dimension.equals(this.spawnPointDimension);
            if (bl && !_snowman) {
    ...
    public ItemEntity dropItem(ItemStack stack, boolean throwRandomly, boolean retainOwnership) {
        ...
        ItemStack _snowman2 = itemEntity.getStack();
        if (retainOwnership) {
            if (!_snowman2.isEmpty()) {
                this.increaseStat(Stats.DROPPED.getOrCreateStat(_snowman2.getItem()), stack.getCount());
            }
    ...
    private /* synthetic */ void method_14223(Text text, Future future) throws Exception {
        if (!future.isSuccess()) {
            int n = 256;
            String _snowman2 = text.asTruncatedString(256);
            TranslatableText _snowman3 = new TranslatableText("death.attack.message_too_long", new LiteralText(_snowman2).formatted(Formatting.YELLOW));
            MutableText _snowman4 = new TranslatableText("death.attack.even_more_magic", this.getDisplayName()).styled(style -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, _snowman3)));
            this.networkHandler.sendPacket(new CombatEventS2CPacket(this.getDamageTracker(), CombatEventS2CPacket.Type.ENTITY_DIED, _snowman4));
        }
    }
}
