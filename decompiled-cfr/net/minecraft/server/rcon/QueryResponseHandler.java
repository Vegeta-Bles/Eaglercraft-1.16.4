...
extends RconBase {
    ...
    public static QueryResponseHandler create(DedicatedServer server) {
        int n = server.getProperties().queryPort;
        if (0 >= n || 65535 < n) {
            field_23963.warn("Invalid query port {} found in server.properties (queries disabled)", (Object)n);
            return null;
        }
        QueryResponseHandler _snowman2 = new QueryResponseHandler(server, n);
        if (!_snowman2.start()) {
            return null;
        }
        return _snowman2;
    ...
    private boolean handle(DatagramPacket packet) throws IOException {
        ...
        int _snowman2 = packet.getLength();
        SocketAddress _snowman3 = packet.getSocketAddress();
        field_23963.debug("Packet len {} [{}]", (Object)_snowman2, (Object)_snowman3);
        if (3 > _snowman2 || -2 != byArray[0] || -3 != byArray[1]) {
            field_23963.debug("Invalid packet [{}]", (Object)_snowman3);
            return false;
        }
        field_23963.debug("Packet '{}' [{}]", (Object)BufferHelper.toHex(byArray[2]), (Object)_snowman3);
        switch (byArray[2]) {
            case 9: {
                this.createQuery(packet);
                field_23963.debug("Challenge [{}]", (Object)_snowman3);
                return true;
            }
            case 0: {
                if (!this.isValidQuery(packet).booleanValue()) {
                    field_23963.debug("Invalid challenge [{}]", (Object)_snowman3);
                    return false;
                }
                if (15 == _snowman2) {
                    this.reply(this.createRulesReply(packet), packet);
                    field_23963.debug("Rules [{}]", (Object)_snowman3);
                    break;
                }
                ...
                field_23963.debug("Status [{}]", (Object)_snowman3);
            }
        }
        return true;
    }
    ...
    private byte[] createRulesReply(DatagramPacket packet) throws IOException {
        ...
        if (l < this.lastResponseTime + 5000L) {
            byte[] byArray = this.data.bytes();
            _snowman = this.getMessageBytes(packet.getSocketAddress());
            byArray[1] = _snowman[0];
            byArray[2] = _snowman[1];
            byArray[3] = _snowman[2];
            byArray[4] = _snowman[3];
            return byArray;
        }
        ...
        this.data.write(0);
        for (String string : _snowman = this.field_23964.getPlayerNames()) {
    ...
    private Boolean isValidQuery(DatagramPacket datagramPacket) {
        ...
        byte[] _snowman2 = datagramPacket.getData();
        return this.queries.get(socketAddress).getId() == BufferHelper.getIntBE(_snowman2, 7, datagramPacket.getLength());
    }
    ...
    static class Query {
        ...
        public Query(DatagramPacket datagramPacket) {
            ...
            this.messageBytes[0] = byArray[3];
            this.messageBytes[1] = byArray[4];
            this.messageBytes[2] = byArray[5];
            this.messageBytes[3] = byArray[6];
