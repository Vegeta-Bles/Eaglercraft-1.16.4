...
implements AutoCloseable {
    ...
    protected void updateChunkStatus(ChunkPos pos) {
        this.enqueue(pos.x, pos.z, () -> 0, Stage.PRE_UPDATE, Util.debugRunnable(() -> {
            ...
            for (n = -1; n < 17; ++n) {
                super.enqueueSectionData(LightType.BLOCK, ChunkSectionPos.from(pos, n), null, true);
                super.enqueueSectionData(LightType.SKY, ChunkSectionPos.from(pos, n), null, true);
            }
            for (n = 0; n < 16; ++n) {
                super.setSectionStatus(ChunkSectionPos.from(pos, n), true);
            }
    ...
    public CompletableFuture<Chunk> light(Chunk chunk, boolean excludeBlocks) {
        ...
        this.enqueue(chunkPos.x, chunkPos.z, Stage.PRE_UPDATE, Util.debugRunnable(() -> {
            ChunkPos chunkPos2;
            ChunkSection[] chunkSectionArray = chunk.getSectionArray();
            for (int i = 0; i < 16; ++i) {
    ...
    private void runTasks() {
        int n;
        Pair pair;
        int n2 = Math.min(this.pendingTasks.size(), this.taskBatchSize);
        ObjectListIterator _snowman2 = this.pendingTasks.iterator();
        for (n = 0; _snowman2.hasNext() && n < n2; ++n) {
            pair = (Pair)_snowman2.next();
