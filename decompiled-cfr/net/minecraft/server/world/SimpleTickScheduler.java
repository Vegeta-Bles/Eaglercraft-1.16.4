...
implements TickScheduler<T> {
    ...
    public static <T> SimpleTickScheduler<T> fromNbt(ListTag ticks, Function<T, Identifier> function, Function<Identifier, T> function2) {
        ...
        for (int i = 0; i < ticks.size(); ++i) {
            ...
            T _snowman2 = function2.apply(new Identifier(compoundTag.getString("i")));
            if (_snowman2 == null) continue;
            BlockPos _snowman3 = new BlockPos(compoundTag.getInt("x"), compoundTag.getInt("y"), compoundTag.getInt("z"));
            arrayList.add(new Tick(_snowman2, _snowman3, compoundTag.getInt("t"), TickPriority.byIndex(compoundTag.getInt("p"))));
        }
        return new SimpleTickScheduler<T>(function, arrayList);
    }
