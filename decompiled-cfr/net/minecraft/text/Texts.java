...
public class Texts {
    ...
    private static Style method_27663(@Nullable ServerCommandSource serverCommandSource, Style style, @Nullable Entity entity, int n) throws CommandSyntaxException {
        HoverEvent hoverEvent = style.getHoverEvent();
        if (hoverEvent != null && (_snowman = hoverEvent.getValue(HoverEvent.Action.SHOW_TEXT)) != null) {
            _snowman = new HoverEvent(HoverEvent.Action.SHOW_TEXT, Texts.parse(serverCommandSource, _snowman, entity, n + 1));
            return style.withHoverEvent(_snowman);
        }
        return style;
    }
    ...
    public static <T extends Comparable<T>> Text joinOrdered(Collection<T> elements, Function<T, Text> transformer) {
        ...
        ArrayList arrayList = Lists.newArrayList(elements);
        arrayList.sort(Comparable::compareTo);
        return Texts.join(arrayList, transformer);
    }
    ...
    public static <T> MutableText join(Collection<T> elements, Function<T, Text> transformer) {
        ...
        for (T t : elements) {
            if (!_snowman2) {
                literalText.append(new LiteralText(", ").formatted(Formatting.GRAY));
            }
            literalText.append(transformer.apply(t));
            _snowman2 = false;
        }
        return literalText;
    }
