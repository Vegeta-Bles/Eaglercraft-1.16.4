...
implements Comparable<Identifier> {
    ...
    protected static String[] split(String id, char delimiter) {
        String[] stringArray = new String[]{"minecraft", id};
        int _snowman2 = id.indexOf(delimiter);
        if (_snowman2 >= 0) {
            stringArray[1] = id.substring(_snowman2 + 1, id.length());
            if (_snowman2 >= 1) {
                stringArray[0] = id.substring(0, _snowman2);
            }
        }
        return stringArray;
    }
    ...
    public int compareTo(Identifier identifier) {
        int n = this.path.compareTo(identifier.path);
        if (n == 0) {
            n = this.namespace.compareTo(identifier.namespace);
        }
        return n;
    }
    ...
    public static Identifier fromCommandInput(StringReader reader) throws CommandSyntaxException {
        ...
        try {
            return new Identifier(_snowman2);
        }
    ...
    public static boolean isValid(String id) {
        String[] stringArray = Identifier.split(id, ':');
        return Identifier.isNamespaceValid(StringUtils.isEmpty((CharSequence)stringArray[0]) ? "minecraft" : stringArray[0]) && Identifier.isPathValid(stringArray[1]);
    }
