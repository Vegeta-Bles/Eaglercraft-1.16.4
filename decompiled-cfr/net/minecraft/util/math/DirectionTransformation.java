...
public enum DirectionTransformation implements StringIdentifiable
{
    ...
    public Direction map(Direction direction) {
        if (this.mappings == null) {
            ...
            for (Direction direction2 : Direction.values()) {
                ...
                Direction.Axis mappedAxis = Direction.Axis.values()[this.axisTransformation.map(axis.ordinal())];
                Direction.AxisDirection mappedAxisDirection = this.shouldFlipDirection(mappedAxis) ? axisDirection.getOpposite() : axisDirection;
                Direction mappedDirection = Direction.from(mappedAxis, mappedAxisDirection);
                this.mappings.put(direction2, mappedDirection);
            }
        }
    ...
    static {
        COMBINATIONS = Util.make(new DirectionTransformation[DirectionTransformation.values().length][DirectionTransformation.values().length], combinations -> {
            Map<Pair<AxisTransformation, BooleanList>, DirectionTransformation> resolved = Arrays
               .stream(DirectionTransformation.values())
               .collect(Collectors.toMap(
                  transformation -> Pair.of(transformation.axisTransformation, transformation.getAxisFlips()),
                  transformation -> transformation
               ));

            for (DirectionTransformation first : DirectionTransformation.values()) {
                for (DirectionTransformation second : DirectionTransformation.values()) {
                    BooleanList firstFlips = first.getAxisFlips();
                    BooleanList secondFlips = second.getAxisFlips();
                    AxisTransformation combinedAxis = second.axisTransformation.prepend(first.axisTransformation);
                    BooleanArrayList combinedFlips = new BooleanArrayList(3);
                    for (int i = 0; i < 3; ++i) {
                        combinedFlips.add(firstFlips.getBoolean(i) ^ secondFlips.getBoolean(first.axisTransformation.map(i)));
                    }
                    combinations[first.ordinal()][second.ordinal()] = resolved.get(Pair.of(combinedAxis, combinedFlips));
                }
            }
        });
        INVERSES = Arrays
           ...
           .map(transformation -> Arrays
               .stream(DirectionTransformation.values())
               .filter(candidate -> transformation.prepend(candidate) == IDENTITY)
               .findAny()
