...
implements ReadableProfiler {
    ...
    public void pop() {
        ...
        _snowman = this.timeList.removeLong(this.timeList.size() - 1);
        this.path.remove(this.path.size() - 1);
        _snowman = l - _snowman;
        LocatedInfo _snowman2 = this.getCurrentInfo();
        _snowman2.time = _snowman2.time + _snowman;
        _snowman2.visits = _snowman2.visits + 1L;
        if (this.checkTimeout && _snowman > TIMEOUT_NANOSECONDS) {
            LOGGER.warn("Something's taking too long! '{}' took aprox {} ms", new Supplier[]{() -> ProfileResult.getHumanReadableName(this.location), () -> (double)_snowman / 1000000.0});
        }
