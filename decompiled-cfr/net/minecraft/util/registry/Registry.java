...
IndexedIterable<T> {
    ...
    public static <T extends MutableRegistry<?>> void validate(MutableRegistry<T> registry) {
        registry.forEach(mutableRegistry2 -> {
            ...
            if (mutableRegistry2 instanceof DefaultedRegistry) {
                ...
                Validate.notNull(mutableRegistry2.get(identifier), (String)("Missing default of DefaultedMappedRegistry: " + identifier), (Object[])new Object[0]);
            }
        });
    }
    ...
    private static <T, R extends MutableRegistry<T>> R create(RegistryKey<? extends Registry<T>> registryKey, R registry, Supplier<T> defaultEntry, Lifecycle lifecycle) {
        ...
        MutableRegistry<MutableRegistry<?>> _snowman2 = ROOT;
        return _snowman2.add(registryKey, registry, lifecycle);
    }
    ...
    public <U> DataResult<Pair<T, U>> decode(DynamicOps<U> dynamicOps, U u) {
        if (dynamicOps.compressMaps()) {
            return dynamicOps.getNumberValue(u).flatMap(number -> {
                ...
                return DataResult.success((Object)object, (Lifecycle)this.getEntryLifecycle(object));
            }).map(object -> Pair.of((Object)object, (Object)dynamicOps.empty()));
        }
        return Identifier.CODEC.decode(dynamicOps, u).flatMap(pair -> {
            T t = this.get((Identifier)pair.getFirst());
            if (t == null) {
                return DataResult.error((String)("Unknown registry key: " + pair.getFirst()));
            }
            return DataResult.success((Object)Pair.of(t, (Object)pair.getSecond()), (Lifecycle)this.getEntryLifecycle(t));
        });
    }

    public <U> DataResult<U> encode(T t, DynamicOps<U> dynamicOps, U u) {
        Identifier identifier = this.getId(t);
        if (identifier == null) {
            return DataResult.error((String)("Unknown registry element " + t));
        }
        if (dynamicOps.compressMaps()) {
            return dynamicOps.mergeToPrimitive(u, dynamicOps.createInt(this.getRawId(t))).setLifecycle(this.lifecycle);
        }
        return dynamicOps.mergeToPrimitive(u, dynamicOps.createString(identifier.toString())).setLifecycle(this.lifecycle);
    }
