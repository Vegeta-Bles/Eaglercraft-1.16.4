...
public interface TaskQueue<T, F> {
    ...
    implements TaskQueue<PrioritizedTask, Runnable> {
        ...
        public Prioritized(int priorityCount) {
            this.queues = IntStream.range(0, priorityCount).mapToObj(n -> Queues.newConcurrentLinkedQueue()).collect(Collectors.toList());
        }
        ...
        public boolean add(PrioritizedTask prioritizedTask) {
            int n = prioritizedTask.getPriority();
            this.queues.get(n).add(prioritizedTask);
            return true;
        }
