...
public class VillagerGossips {
    ...
    private Collection<GossipEntry> pickGossips(Random random, int count) {
        ...
        int[] _snowman2 = new int[list.size()];
        int _snowman3 = 0;
        for (int i = 0; i < list.size(); ++i) {
            GossipEntry gossipEntry = (GossipEntry)list.get(i);
            _snowman2[i] = (_snowman3 += Math.abs(gossipEntry.getValue())) - 1;
        }
        Set set = Sets.newIdentityHashSet();
        for (int i = 0; i < count; ++i) {
            _snowman = random.nextInt(_snowman3);
            _snowman = Arrays.binarySearch(_snowman2, _snowman);
            set.add(list.get(_snowman < 0 ? -_snowman - 1 : _snowman));
        }
        return set;
    }
    ...
    public void shareGossipFrom(VillagerGossips from, Random random, int count) {
        ...
        collection.forEach(gossipEntry -> {
            int n = gossipEntry.value - gossipEntry.type.shareDecrement;
            if (n >= 2) {
                this.getReputationFor(gossipEntry.target).associatedGossip.mergeInt((Object)gossipEntry.type, n, VillagerGossips::max);
            }
        });
    }
    ...
    public void startGossip(UUID target, VillageGossipType type, int value) {
        ...
        reputation.associatedGossip.mergeInt((Object)type, value, (n, n2) -> this.mergeReputation(type, (int)n, (int)n2));
    ...
    public void deserialize(Dynamic<?> dynamic) {
        dynamic.asStream().map(GossipEntry::deserialize).flatMap(dataResult -> Util.stream(dataResult.result())).forEach(gossipEntry -> this.getReputationFor(gossipEntry.target).associatedGossip.put((Object)gossipEntry.type, gossipEntry.value));
    }
    ...
    private int mergeReputation(VillageGossipType type, int left, int right) {
        int n = left + right;
        return n > type.maxValue ? Math.max(type.maxValue, left) : n;
    }
    ...
    static class Reputation {
        ...
        public void decay() {
            ...
            while (objectIterator.hasNext()) {
                ...
                int _snowman2 = entry.getIntValue() - ((VillageGossipType)((Object)entry.getKey())).decay;
                if (_snowman2 < 2) {
                    objectIterator.remove();
                    continue;
                }
                entry.setValue(_snowman2);
            }
        }
        ...
        public void clamp(VillageGossipType gossipType) {
            int n = this.associatedGossip.getInt((Object)gossipType);
            if (n > gossipType.maxValue) {
    ...
    static class GossipEntry {
        ...
        public GossipEntry(UUID uUID, VillageGossipType villageGossipType, int n) {
            this.target = uUID;
        ...
        public <T> Dynamic<T> serialize(DynamicOps<T> dynamicOps) {
            return new Dynamic(dynamicOps, dynamicOps.createMap((Map)ImmutableMap.of((Object)dynamicOps.createString("Target"), DynamicSerializableUuid.CODEC.encodeStart(dynamicOps, (Object)this.target).result().orElseThrow(RuntimeException::new), (Object)dynamicOps.createString("Type"), (Object)dynamicOps.createString(this.type.key), (Object)dynamicOps.createString("Value"), (Object)dynamicOps.createInt(this.value))));
        }

        public static DataResult<GossipEntry> deserialize(Dynamic<?> dynamic) {
            return DataResult.unbox((App)DataResult.instance().group((App)dynamic.get("Target").read(DynamicSerializableUuid.CODEC), (App)dynamic.get("Type").asString().map(VillageGossipType::byKey), (App)dynamic.get("Value").asNumber().map(Number::intValue)).apply((Applicative)DataResult.instance(), GossipEntry::new));
        }
    }
}
