...
implements StructureWorldAccess {
    ...
    public ChunkRegion(ServerWorld world, List<Chunk> chunks) {
        int n = MathHelper.floor(Math.sqrt(chunks.size()));
        if (n * n != chunks.size()) {
            throw Util.throwOrPause(new IllegalStateException("Cache size is not a square."));
        }
        ChunkPos _snowman2 = chunks.get(chunks.size() / 2).getPos();
        this.chunks = chunks;
        this.centerChunkX = _snowman2.x;
        this.centerChunkZ = _snowman2.z;
        this.width = n;
        this.world = world;
    ...
    public Chunk getChunk(int chunkX, int chunkZ, ChunkStatus leastStatus, boolean create) {
        ...
        if (this.isChunkLoaded(chunkX, chunkZ)) {
            int n = chunkX - this.lowerCorner.x;
            _snowman = chunkZ - this.lowerCorner.z;
            _snowman2 = this.chunks.get(n + _snowman * this.width);
            if (_snowman2.getStatus().isAtLeast(leastStatus)) {
                return _snowman2;
            }
        } else {
            _snowman2 = null;
        ...
        if (_snowman2 != null) {
            throw Util.throwOrPause(new RuntimeException(String.format("Chunk is not of correct status. Expecting %s, got %s | %s %s", leastStatus, _snowman2.getStatus(), chunkX, chunkZ)));
        }
    ...
    public BlockEntity getBlockEntity(BlockPos pos) {
        ...
        BlockEntity _snowman2 = chunk.getBlockEntity(pos);
        if (_snowman2 != null) {
            return _snowman2;
        }
        CompoundTag _snowman3 = chunk.getBlockEntityTag(pos);
        BlockState _snowman4 = chunk.getBlockState(pos);
        if (_snowman3 != null) {
            if ("DUMMY".equals(_snowman3.getString("id"))) {
                Block block = _snowman4.getBlock();
                if (!(block instanceof BlockEntityProvider)) {
                    return null;
                }
                _snowman2 = ((BlockEntityProvider)((Object)block)).createBlockEntity(this.world);
            } else {
                _snowman2 = BlockEntity.createFromTag(_snowman4, _snowman3);
            }
            if (_snowman2 != null) {
                chunk.setBlockEntity(pos, _snowman2);
                return _snowman2;
            }
        }
        if (_snowman4.getBlock() instanceof BlockEntityProvider) {
    ...
    public boolean setBlockState(BlockPos pos, BlockState state, int flags, int maxUpdateDepth) {
        Chunk chunk = this.getChunk(pos);
        BlockState _snowman2 = chunk.setBlockState(pos, state, false);
        if (_snowman2 != null) {
            this.world.onBlockChanged(pos, _snowman2, state);
        }
        if ((_snowman = state.getBlock()).hasBlockEntity()) {
            if (chunk.getStatus().getChunkType() == ChunkStatus.ChunkType.field_12807) {
                chunk.setBlockEntity(pos, ((BlockEntityProvider)((Object)_snowman)).createBlockEntity(this));
            } else {
        ...
        } else if (_snowman2 != null && _snowman2.getBlock().hasBlockEntity()) {
