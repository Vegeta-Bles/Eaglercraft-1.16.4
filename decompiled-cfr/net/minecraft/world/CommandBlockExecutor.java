...
implements CommandOutput {
    ...
    public boolean execute(World world) {
        ...
        if (minecraftServer.areCommandBlocksEnabled() && !ChatUtil.isEmpty(this.command)) {
            try {
                this.lastOutput = null;
                ServerCommandSource serverCommandSource = this.getSource().withConsumer((ResultConsumer<ServerCommandSource>)((ResultConsumer)(commandContext, bl, n) -> {
                    if (bl) {
                        ++this.successCount;
                    }
                }));
                ...
                CrashReportSection _snowman2 = crashReport.addElement("Command to be executed");
                _snowman2.add("Command", this::getCommand);
                _snowman2.add("Name", () -> this.getCustomName().getString());
