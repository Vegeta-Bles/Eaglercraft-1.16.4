...
public class ChunkSection {
    ...
    public BlockState setBlockState(int x, int y, int z, BlockState state, boolean lock) {
        ...
        FluidState _snowman3 = state.getFluidState();
        if (!blockState.isAir()) {
    ...
    public void calculateCounts() {
        ...
        this.container.count((blockState, n) -> {
            FluidState fluidState = blockState.getFluidState();
            if (!blockState.isAir()) {
                this.nonEmptyBlockCount = (short)(this.nonEmptyBlockCount + n);
                if (blockState.hasRandomTicks()) {
                    this.randomTickableBlockCount = (short)(this.randomTickableBlockCount + n);
                }
            }
            if (!fluidState.isEmpty()) {
                this.nonEmptyBlockCount = (short)(this.nonEmptyBlockCount + n);
                if (fluidState.hasRandomTicks()) {
                    this.nonEmptyFluidCount = (short)(this.nonEmptyFluidCount + n);
                }
            }
        });
    }
