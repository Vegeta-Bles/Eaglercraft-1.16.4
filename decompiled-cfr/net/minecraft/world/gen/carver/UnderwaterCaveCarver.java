...
extends CaveCarver {
    ...
    protected static boolean carveAtPoint(Carver<?> carver, Chunk chunk, BitSet mask, Random random, BlockPos.Mutable pos, int seaLevel, int mainChunkX, int mainChunkZ, int x, int z, int relativeX, int y, int relativeZ) {
        ...
        mask.set(n);
        pos.set(x, y, z);
        BlockState _snowman2 = chunk.getBlockState(pos);
        if (!carver.canAlwaysCarveBlock(_snowman2)) {
            return false;
        }
        if (y == 10) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            int n2 = x + direction.getOffsetX();
            _snowman = z + direction.getOffsetZ();
            if (n2 >> 4 == mainChunkX && _snowman >> 4 == mainChunkZ && !chunk.getBlockState(pos.set(n2, y, _snowman)).isAir()) continue;
