...
public class StructuresConfig {
    public static final Codec<StructuresConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)StrongholdConfig.CODEC.optionalFieldOf("stronghold").forGetter(config -> Optional.ofNullable(config.stronghold)), (App)Codec.simpleMap(Registry.STRUCTURE_FEATURE, StructureConfig.CODEC, Registry.STRUCTURE_FEATURE).fieldOf("structures").forGetter(config -> config.structures)).apply((Applicative)instance, StructuresConfig::new));
