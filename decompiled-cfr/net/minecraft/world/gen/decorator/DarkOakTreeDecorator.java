...
extends AbstractHeightmapDecorator<NopeDecoratorConfig> {
    ...
    public Stream<BlockPos> getPositions(DecoratorContext decoratorContext, Random random, NopeDecoratorConfig nopeDecoratorConfig, BlockPos blockPos) {
        return IntStream.range(0, 16).mapToObj(n -> {
            _snowman = n / 4;
            _snowman = n % 4;
            _snowman = _snowman * 4 + 1 + random.nextInt(3) + blockPos.getX();
            _snowman = _snowman * 4 + 1 + random.nextInt(3) + blockPos.getZ();
            _snowman = decoratorContext.getTopY(this.getHeightmapType(nopeDecoratorConfig), _snowman, _snowman);
