...
extends Feature<DefaultFeatureConfig> {
    ...
    private void generateNetherWartBlocksInArea(WorldAccess world, Random random, BlockPos pos) {
        ...
        for (int i = 0; i < 200; ++i) {
            ...
            for (Direction direction : DIRECTIONS) {
                BlockState blockState = world.getBlockState(_snowman.set(mutable, direction));
                if (blockState.isOf(Blocks.NETHERRACK) || blockState.isOf(Blocks.NETHER_WART_BLOCK)) {
                    ++n;
                }
                if (n > 1) break;
    ...
    private void generateVinesInArea(WorldAccess world, Random random, BlockPos pos) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (!world.isAir(mutable) || !(_snowman = world.getBlockState((BlockPos)mutable.up())).isOf(Blocks.NETHERRACK) && !_snowman.isOf(Blocks.NETHER_WART_BLOCK)) continue;
            _snowman = MathHelper.nextInt(random, 1, 8);
            ...
            _snowman = 17;
            _snowman = 25;
            WeepingVinesFeature.generateVineColumn(world, random, mutable, _snowman, 17, 25);
        }
    }
