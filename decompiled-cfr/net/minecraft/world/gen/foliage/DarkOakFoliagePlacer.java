...
extends FoliagePlacer {
    ...
    protected void generate(ModifiableTestableWorld world, Random random, TreeFeatureConfig config, int trunkHeight, FoliagePlacer.TreeNode treeNode, int foliageHeight, int radius, Set<BlockPos> leaves, int offset, BlockBox box) {
        ...
        boolean _snowman2 = treeNode.isGiantTrunk();
        if (_snowman2) {
            this.generateSquare(world, random, config, blockPos, radius + 2, leaves, -1, _snowman2, box);
            this.generateSquare(world, random, config, blockPos, radius + 3, leaves, 0, _snowman2, box);
            this.generateSquare(world, random, config, blockPos, radius + 2, leaves, 1, _snowman2, box);
            if (random.nextBoolean()) {
                this.generateSquare(world, random, config, blockPos, radius, leaves, 2, _snowman2, box);
            }
        } else {
            this.generateSquare(world, random, config, blockPos, radius + 2, leaves, -1, _snowman2, box);
            this.generateSquare(world, random, config, blockPos, radius + 1, leaves, 0, _snowman2, box);
        }
    }
