...
extends BlobFoliagePlacer {
    ...
    public LargeOakFoliagePlacer(UniformIntDistribution uniformIntDistribution, UniformIntDistribution uniformIntDistribution2, int n) {
        super(uniformIntDistribution, uniformIntDistribution2, n);
    }
    ...
    protected void generate(ModifiableTestableWorld world, Random random, TreeFeatureConfig config, int trunkHeight, FoliagePlacer.TreeNode treeNode, int foliageHeight, int radius, Set<BlockPos> leaves, int offset, BlockBox box) {
        for (int i = offset; i >= offset - foliageHeight; --i) {
            _snowman = radius + (i == offset || i == offset - foliageHeight ? 0 : 1);
            this.generateSquare(world, random, config, treeNode.getCenter(), _snowman, leaves, i, treeNode.isGiantTrunk(), box);
        }
    }
