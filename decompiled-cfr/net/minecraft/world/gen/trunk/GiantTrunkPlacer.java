...
extends TrunkPlacer {
    ...
    public List<FoliagePlacer.TreeNode> generate(ModifiableTestableWorld world, Random random, int trunkHeight, BlockPos pos, Set<BlockPos> placedStates, BlockBox box, TreeFeatureConfig config) {
        ...
        for (int i = 0; i < trunkHeight; ++i) {
            GiantTrunkPlacer.method_27399(world, random, _snowman2, placedStates, box, config, pos, 0, i, 0);
            if (i >= trunkHeight - 1) continue;
            GiantTrunkPlacer.method_27399(world, random, _snowman2, placedStates, box, config, pos, 1, i, 0);
            GiantTrunkPlacer.method_27399(world, random, _snowman2, placedStates, box, config, pos, 1, i, 1);
            GiantTrunkPlacer.method_27399(world, random, _snowman2, placedStates, box, config, pos, 0, i, 1);
        }
        return ImmutableList.of((Object)new FoliagePlacer.TreeNode(pos.up(trunkHeight), 0, true));
    }

    private static void method_27399(ModifiableTestableWorld modifiableTestableWorld, Random random, BlockPos.Mutable mutable, Set<BlockPos> set, BlockBox blockBox, TreeFeatureConfig treeFeatureConfig, BlockPos blockPos, int n, int n2, int n3) {
        mutable.set(blockPos, n, n2, n3);
