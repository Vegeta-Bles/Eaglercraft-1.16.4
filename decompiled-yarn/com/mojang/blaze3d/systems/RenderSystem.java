...
public class RenderSystem {
   ...
   public static void initGameThread(boolean assertNotRenderThread) {
      ...
      if (gameThread == null && renderThread != null && _snowman != assertNotRenderThread) {
   ...
   public static void replayQueue() {
      ...
      while (!recordingQueue.isEmpty()) {
         RenderCall _snowman = recordingQueue.poll();
         _snowman.execute();
      }
   ...
   public static void limitDisplayFPS(int fps) {
      double _snowman = lastDrawTime + 1.0 / (double)fps;
   ...
   public static void enableScissor(int _snowman, int _snowman, int _snowman, int _snowman) {
      assertThread(RenderSystem::isOnGameThreadOrInit);
      GlStateManager.method_31319();
      GlStateManager.method_31317(_snowman, _snowman, _snowman, _snowman);
   }
   ...
   @Deprecated
   public static void fogMode(int _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.fogMode(_snowman);
   }
   ...
   @Deprecated
   public static void fogi(int _snowman, int _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.fogi(_snowman, _snowman);
   }
   ...
   public static void polygonMode(int _snowman, int _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.polygonMode(_snowman, _snowman);
   }
   ...
   public static void stencilMask(int _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.stencilMask(_snowman);
   }
   ...
   public static void clearStencil(int _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.clearStencil(_snowman);
   }
   ...
   public static void pixelTransfer(int _snowman, float _snowman) {
      GlStateManager.pixelTransfer(_snowman, _snowman);
   }
   ...
   public static int maxSupportedTextureSize() {
      ...
      if (MAX_SUPPORTED_TEXTURE_SIZE == -1) {
         int _snowman = GlStateManager.getInteger(3379);

         for (int _snowmanx = Math.max(32768, _snowman); _snowmanx >= 1024; _snowmanx >>= 1) {
            GlStateManager.texImage2D(32868, 0, 6408, _snowmanx, _snowmanx, 0, 6408, 5121, null);
            int _snowmanxx = GlStateManager.getTexLevelParameter(32868, 0, 4096);
            if (_snowmanxx != 0) {
   ...
   public static void setupLevelDiffuseLighting(Vector3f _snowman, Vector3f _snowman, Matrix4f _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.setupLevelDiffuseLighting(_snowman, _snowman, _snowman);
   }

   public static void setupGuiFlatDiffuseLighting(Vector3f _snowman, Vector3f _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.setupGuiFlatDiffuseLighting(_snowman, _snowman);
   }

   public static void setupGui3DDiffuseLighting(Vector3f _snowman, Vector3f _snowman) {
      assertThread(RenderSystem::isOnGameThread);
      GlStateManager.setupGui3dDiffuseLighting(_snowman, _snowman);
   }
   ...
   @Deprecated
   public static void runAsFancy(Runnable _snowman) {
      boolean _snowmanx = MinecraftClient.isFabulousGraphicsOrBetter();
      if (!_snowmanx) {
         _snowman.run();
      } else {
         GameOptions _snowmanxx = MinecraftClient.getInstance().options;
         GraphicsMode _snowmanxxx = _snowmanxx.graphicsMode;
         _snowmanxx.graphicsMode = GraphicsMode.FANCY;
         _snowman.run();
         _snowmanxx.graphicsMode = _snowmanxxx;
      }
   }
}
