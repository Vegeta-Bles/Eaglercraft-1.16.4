...
public class EntityHurtPlayerCriterion extends AbstractCriterion<EntityHurtPlayerCriterion.Conditions> {
   ...
   public EntityHurtPlayerCriterion.Conditions conditionsFromJson(JsonObject _snowman, EntityPredicate.Extended _snowman, AdvancementEntityPredicateDeserializer _snowman) {
      DamagePredicate _snowmanxxx = DamagePredicate.fromJson(_snowman.get("damage"));
      return new EntityHurtPlayerCriterion.Conditions(_snowman, _snowmanxxx);
   }

   public void trigger(ServerPlayerEntity player, DamageSource source, float dealt, float taken, boolean blocked) {
      this.test(player, _snowmanxxxxx -> _snowmanxxxxx.matches(player, source, dealt, taken, blocked));
   }
