...
public abstract class AbstractBlock {
   ...
   public List<ItemStack> getDroppedStacks(BlockState state, LootContext.Builder builder) {
      Identifier _snowman = this.getLootTableId();
      if (_snowman == LootTables.EMPTY) {
         return Collections.emptyList();
      } else {
         LootContext _snowmanx = builder.parameter(LootContextParameters.BLOCK_STATE, state).build(LootContextTypes.BLOCK);
         ServerWorld _snowmanxx = _snowmanx.getWorld();
         LootTable _snowmanxxx = _snowmanxx.getServer().getLootManager().getTable(_snowman);
         return _snowmanxxx.generateLoot(_snowmanx);
      }
   }
   ...
   public float calcBlockBreakingDelta(BlockState state, PlayerEntity player, BlockView world, BlockPos pos) {
      float _snowman = state.getHardness(world, pos);
      if (_snowman == -1.0F) {
         return 0.0F;
      } else {
         int _snowmanx = player.isUsingEffectiveTool(state) ? 30 : 100;
         return player.getBlockBreakingSpeed(state) / _snowman / (float)_snowmanx;
      }
   }
   ...
   public final Identifier getLootTableId() {
      if (this.lootTableId == null) {
         Identifier _snowman = Registry.BLOCK.getId(this.asBlock());
         this.lootTableId = new Identifier(_snowman.getNamespace(), "blocks/" + _snowman.getPath());
      }
   ...
   public abstract static class AbstractBlockState extends State<Block, BlockState> {
      ...
      protected AbstractBlockState(Block block, ImmutableMap<Property<?>, Comparable<?>> propertyMap, MapCodec<BlockState> _snowman) {
         super(block, propertyMap, _snowman);
         AbstractBlock.Settings _snowmanx = block.settings;
         this.luminance = _snowmanx.luminance.applyAsInt(this.asBlockState());
         this.hasSidedTransparency = block.hasSidedTransparency(this.asBlockState());
         this.isAir = _snowmanx.isAir;
         this.material = _snowmanx.material;
         this.materialColor = _snowmanx.materialColorFactory.apply(this.asBlockState());
         this.hardness = _snowmanx.hardness;
         this.toolRequired = _snowmanx.toolRequired;
         this.opaque = _snowmanx.opaque;
         this.solidBlockPredicate = _snowmanx.solidBlockPredicate;
         this.suffocationPredicate = _snowmanx.suffocationPredicate;
         this.blockVisionPredicate = _snowmanx.blockVisionPredicate;
         this.postProcessPredicate = _snowmanx.postProcessPredicate;
         this.emissiveLightingPredicate = _snowmanx.emissiveLightingPredicate;
      }
      ...
      public boolean isOpaqueFullCube(BlockView world, BlockPos pos) {
         if (this.shapeCache != null) {
            ...
            BlockState _snowman = this.asBlockState();
            return _snowman.isOpaque() ? Block.isShapeFullCube(_snowman.getCullingShape(world, pos)) : false;
         }
      }
      ...
      public Vec3d getModelOffset(BlockView world, BlockPos pos) {
         AbstractBlock.OffsetType _snowman = this.getBlock().getOffsetType();
         if (_snowman == AbstractBlock.OffsetType.NONE) {
            return Vec3d.ZERO;
         } else {
            long _snowmanx = MathHelper.hashCode(pos.getX(), 0, pos.getZ());
            return new Vec3d(
               ((double)((float)(_snowmanx & 15L) / 15.0F) - 0.5) * 0.5,
               _snowman == AbstractBlock.OffsetType.XYZ ? ((double)((float)(_snowmanx >> 4 & 15L) / 15.0F) - 1.0) * 0.2 : 0.0,
      ...
      public final void updateNeighbors(WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
         ...
         for (Direction _snowmanx : AbstractBlock.FACINGS) {
            _snowman.set(pos, _snowmanx);
            BlockState _snowmanxx = world.getBlockState(_snowman);
            BlockState _snowmanxxx = _snowmanxx.getStateForNeighborUpdate(_snowmanx.getOpposite(), this.asBlockState(), world, _snowman, pos);
            Block.replace(_snowmanxx, _snowmanxxx, world, _snowman, flags, maxUpdateDepth);
         }
      }
      ...
      public void scheduledTick(ServerWorld world, BlockPos pos, Random _snowman) {
         this.getBlock().scheduledTick(this.asBlockState(), world, pos, _snowman);
      }

      public void randomTick(ServerWorld world, BlockPos pos, Random _snowman) {
         this.getBlock().randomTick(this.asBlockState(), world, pos, _snowman);
      }
      ...
      public boolean method_27851(Tag<Block> _snowman, Predicate<AbstractBlock.AbstractBlockState> _snowman) {
         return this.getBlock().isIn(_snowman) && _snowman.test(this);
      }
      ...
      static final class ShapeCache {
         ...
         private ShapeCache(BlockState state) {
            ...
            this.translucent = _snowman.isTranslucent(state, EmptyBlockView.INSTANCE, BlockPos.ORIGIN);
            this.lightSubtracted = _snowman.getOpacity(state, EmptyBlockView.INSTANCE, BlockPos.ORIGIN);
            if (!state.isOpaque()) {
               this.extrudedFaces = null;
            } else {
               this.extrudedFaces = new VoxelShape[DIRECTIONS.length];
               VoxelShape _snowmanx = _snowman.getCullingShape(state, EmptyBlockView.INSTANCE, BlockPos.ORIGIN);

               for (Direction _snowmanxx : DIRECTIONS) {
                  this.extrudedFaces[_snowmanxx.ordinal()] = VoxelShapes.extrudeFace(_snowmanx, _snowmanxx);
               }
            }

            this.collisionShape = _snowman.getCollisionShape(state, EmptyBlockView.INSTANCE, BlockPos.ORIGIN, ShapeContext.absent());
            this.exceedsCube = Arrays.stream(Direction.Axis.values())
               .anyMatch(_snowmanx -> this.collisionShape.getMin(_snowmanx) < 0.0 || this.collisionShape.getMax(_snowmanx) > 1.0);
            this.solidSides = new boolean[DIRECTIONS.length * SHAPE_TYPE_LENGTH];

            for (Direction _snowmanx : DIRECTIONS) {
               for (SideShapeType _snowmanxx : SideShapeType.values()) {
                  this.solidSides[indexSolidSide(_snowmanx, _snowmanxx)] = _snowmanxx.matches(state, EmptyBlockView.INSTANCE, BlockPos.ORIGIN, _snowmanx);
               }
            }
   ...
   public static class Settings {
      ...
      public static AbstractBlock.Settings copy(AbstractBlock block) {
         AbstractBlock.Settings _snowman = new AbstractBlock.Settings(block.material, block.settings.materialColorFactory);
         _snowman.material = block.settings.material;
         _snowman.hardness = block.settings.hardness;
         _snowman.resistance = block.settings.resistance;
         _snowman.collidable = block.settings.collidable;
         _snowman.randomTicks = block.settings.randomTicks;
         _snowman.luminance = block.settings.luminance;
         _snowman.materialColorFactory = block.settings.materialColorFactory;
         _snowman.soundGroup = block.settings.soundGroup;
         _snowman.slipperiness = block.settings.slipperiness;
         _snowman.velocityMultiplier = block.settings.velocityMultiplier;
         _snowman.dynamicBounds = block.settings.dynamicBounds;
         _snowman.opaque = block.settings.opaque;
         _snowman.isAir = block.settings.isAir;
         _snowman.toolRequired = block.settings.toolRequired;
         return _snowman;
      }
