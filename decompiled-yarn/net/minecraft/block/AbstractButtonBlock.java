...
public abstract class AbstractButtonBlock extends WallMountedBlock {
      ...
      switch ((WallMountLocation)state.get(FACE)) {
         case FLOOR:
            if (_snowman.getAxis() == Direction.Axis.X) {
               return _snowmanx ? FLOOR_X_PRESSED_SHAPE : FLOOR_X_SHAPE;
            }

            return _snowmanx ? FLOOR_Z_PRESSED_SHAPE : FLOOR_Z_SHAPE;
         case WALL:
            switch (_snowman) {
               case EAST:
                  return _snowmanx ? EAST_PRESSED_SHAPE : EAST_SHAPE;
               case WEST:
                  return _snowmanx ? WEST_PRESSED_SHAPE : WEST_SHAPE;
               case SOUTH:
                  return _snowmanx ? SOUTH_PRESSED_SHAPE : SOUTH_SHAPE;
               case NORTH:
               default:
                  return _snowmanx ? NORTH_PRESSED_SHAPE : NORTH_SHAPE;
            }
         case CEILING:
         default:
            if (_snowman.getAxis() == Direction.Axis.X) {
               return _snowmanx ? CEILING_X_PRESSED_SHAPE : CEILING_X_SHAPE;
            } else {
               return _snowmanx ? CEILING_Z_PRESSED_SHAPE : CEILING_Z_SHAPE;
            }
      }
   }
   ...
   private void tryPowerWithProjectiles(BlockState state, World world, BlockPos pos) {
      List<? extends Entity> _snowman = world.getNonSpectatingEntities(
