...
public abstract class AbstractRailBlock extends Block {
   ...
   public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      RailShape _snowman = state.isOf(this) ? state.get(this.getShapeProperty()) : null;
      return _snowman != null && _snowman.isAscending() ? ASCENDING_SHAPE : STRAIGHT_SHAPE;
   }
   ...
   public void neighborUpdate(BlockState state, World world, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
      if (!world.isClient && world.getBlockState(pos).isOf(this)) {
         RailShape _snowman = state.get(this.getShapeProperty());
         if (shouldDropRail(pos, world, _snowman)) {
   ...
   protected BlockState updateBlockState(World world, BlockPos pos, BlockState state, boolean forceUpdate) {
      if (world.isClient) {
         ...
         RailShape _snowman = state.get(this.getShapeProperty());
         return new RailPlacementHelper(world, pos, state).updateBlockState(world.isReceivingRedstonePower(pos), forceUpdate, _snowman).getBlockState();
      }
   }
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      BlockState _snowman = super.getDefaultState();
      Direction _snowmanx = ctx.getPlayerFacing();
      boolean _snowmanxx = _snowmanx == Direction.EAST || _snowmanx == Direction.WEST;
      return _snowman.with(this.getShapeProperty(), _snowmanxx ? RailShape.EAST_WEST : RailShape.NORTH_SOUTH);
   }
