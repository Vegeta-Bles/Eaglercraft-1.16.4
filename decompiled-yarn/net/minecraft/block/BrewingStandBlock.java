...
public class BrewingStandBlock extends BlockWithEntity {
   ...
   public BrewingStandBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      if (world.isClient) {
         ...
         if (_snowman instanceof BrewingStandBlockEntity) {
            player.openHandledScreen((BrewingStandBlockEntity)_snowman);
   ...
   public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
      if (itemStack.hasCustomName()) {
         BlockEntity _snowman = world.getBlockEntity(pos);
         if (_snowman instanceof BrewingStandBlockEntity) {
            ((BrewingStandBlockEntity)_snowman).setCustomName(itemStack.getName());
         }
      }
   }
   ...
   public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
      double _snowman = (double)pos.getX() + 0.4 + (double)random.nextFloat() * 0.2;
      double _snowmanx = (double)pos.getY() + 0.7 + (double)random.nextFloat() * 0.3;
      double _snowmanxx = (double)pos.getZ() + 0.4 + (double)random.nextFloat() * 0.2;
      world.addParticle(ParticleTypes.SMOKE, _snowman, _snowmanx, _snowmanxx, 0.0, 0.0, 0.0);
   }
   ...
   public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
      if (!state.isOf(newState.getBlock())) {
         BlockEntity _snowman = world.getBlockEntity(pos);
         if (_snowman instanceof BrewingStandBlockEntity) {
            ItemScatterer.spawn(world, pos, (BrewingStandBlockEntity)_snowman);
         }
