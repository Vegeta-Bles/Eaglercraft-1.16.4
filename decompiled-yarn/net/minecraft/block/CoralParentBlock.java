...
public class CoralParentBlock extends Block implements Waterloggable {
   ...
   protected CoralParentBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   protected static boolean isInWater(BlockState state, BlockView world, BlockPos pos) {
      if (state.get(WATERLOGGED)) {
         ...
         for (Direction _snowman : Direction.values()) {
            if (world.getFluidState(pos.offset(_snowman)).isIn(FluidTags.WATER)) {
               return true;
            }
         }
   ...
   public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
      ...
      return world.getBlockState(_snowman).isSideSolidFullSquare(world, _snowman, Direction.UP);
   }
