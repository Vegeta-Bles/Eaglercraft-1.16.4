...
public class DetectorRailBlock extends AbstractRailBlock {
   ...
   public DetectorRailBlock(AbstractBlock.Settings _snowman) {
      super(true, _snowman);
   ...
   private void updatePoweredStatus(World world, BlockPos pos, BlockState state) {
      if (this.canPlaceAt(state, world, pos)) {
         ...
         List<AbstractMinecartEntity> _snowmanxx = this.getCarts(world, pos, AbstractMinecartEntity.class, null);
         if (!_snowmanxx.isEmpty()) {
            _snowmanx = true;
         }

         if (_snowmanx && !_snowman) {
            BlockState _snowmanxxx = state.with(POWERED, Boolean.valueOf(true));
            world.setBlockState(pos, _snowmanxxx, 3);
            this.updateNearbyRails(world, pos, _snowmanxxx, true);
         ...
         if (!_snowmanx && _snowman) {
            BlockState _snowmanxxx = state.with(POWERED, Boolean.valueOf(false));
            world.setBlockState(pos, _snowmanxxx, 3);
            this.updateNearbyRails(world, pos, _snowmanxxx, false);
            world.updateNeighborsAlways(pos, this);
            world.updateNeighborsAlways(pos.down(), this);
            world.scheduleBlockRerenderIfNeeded(pos, state, _snowmanxxx);
         }
   ...
   protected void updateNearbyRails(World world, BlockPos pos, BlockState state, boolean unpowering) {
      RailPlacementHelper _snowman = new RailPlacementHelper(world, pos, state);

      for (BlockPos _snowmanx : _snowman.getNeighbors()) {
         BlockState _snowmanxx = world.getBlockState(_snowmanx);
         _snowmanxx.neighborUpdate(world, _snowmanx, _snowmanxx.getBlock(), pos, false);
      }
   }
   ...
   public int getComparatorOutput(BlockState state, World world, BlockPos pos) {
      if (state.get(POWERED)) {
         List<CommandBlockMinecartEntity> _snowman = this.getCarts(world, pos, CommandBlockMinecartEntity.class, null);
         if (!_snowman.isEmpty()) {
            return _snowman.get(0).getCommandExecutor().getSuccessCount();
         }

         List<AbstractMinecartEntity> _snowmanx = this.getCarts(world, pos, AbstractMinecartEntity.class, EntityPredicates.VALID_INVENTORIES);
         if (!_snowmanx.isEmpty()) {
            return ScreenHandler.calculateComparatorOutput((Inventory)_snowmanx.get(0));
         }
      }
