...
public class FarmlandBlock extends Block {
   ...
   protected FarmlandBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
      ...
      return !_snowman.getMaterial().isSolid() || _snowman.getBlock() instanceof FenceGateBlock || _snowman.getBlock() instanceof PistonExtensionBlock;
   }
   ...
   public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
      int _snowman = state.get(MOISTURE);
      if (!isWaterNearby(world, pos) && !world.hasRain(pos.up())) {
         if (_snowman > 0) {
            world.setBlockState(pos, state.with(MOISTURE, Integer.valueOf(_snowman - 1)), 2);
   ...
   private static boolean hasCrop(BlockView world, BlockPos pos) {
      Block _snowman = world.getBlockState(pos.up()).getBlock();
      return _snowman instanceof CropBlock || _snowman instanceof StemBlock || _snowman instanceof AttachedStemBlock;
   }

   private static boolean isWaterNearby(WorldView world, BlockPos pos) {
      for (BlockPos _snowman : BlockPos.iterate(pos.add(-4, 0, -4), pos.add(4, 1, 4))) {
         if (world.getFluidState(_snowman).isIn(FluidTags.WATER)) {
            return true;
         }
      }
