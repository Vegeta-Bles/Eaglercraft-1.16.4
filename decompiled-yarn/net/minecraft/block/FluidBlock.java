...
public class FluidBlock extends Block implements FluidDrainable {
   ...
   protected FluidBlock(FlowableFluid fluid, AbstractBlock.Settings settings) {
      ...
      for (int _snowman = 1; _snowman < 8; _snowman++) {
         this.statesByLevel.add(fluid.getFlowing(8 - _snowman, false));
      }
   ...
   public FluidState getFluidState(BlockState state) {
      ...
      return this.statesByLevel.get(Math.min(_snowman, 8));
   }
   ...
   private boolean receiveNeighborFluids(World world, BlockPos pos, BlockState state) {
      if (this.fluid.isIn(FluidTags.LAVA)) {
         boolean _snowman = world.getBlockState(pos.down()).isOf(Blocks.SOUL_SOIL);

         for (Direction _snowmanx : Direction.values()) {
            if (_snowmanx != Direction.DOWN) {
               BlockPos _snowmanxx = pos.offset(_snowmanx);
               if (world.getFluidState(_snowmanxx).isIn(FluidTags.WATER)) {
                  Block _snowmanxxx = world.getFluidState(pos).isStill() ? Blocks.OBSIDIAN : Blocks.COBBLESTONE;
                  world.setBlockState(pos, _snowmanxxx.getDefaultState());
                  this.playExtinguishSound(world, pos);
                  return false;
               }

               if (_snowman && world.getBlockState(_snowmanxx).isOf(Blocks.BLUE_ICE)) {
