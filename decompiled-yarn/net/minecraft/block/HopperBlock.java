...
public class HopperBlock extends BlockWithEntity {
   ...
   public HopperBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      Direction _snowman = ctx.getSide().getOpposite();
      return this.getDefaultState().with(FACING, _snowman.getAxis() == Direction.Axis.Y ? Direction.DOWN : _snowman).with(ENABLED, Boolean.valueOf(true));
   }
   ...
   public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
      if (itemStack.hasCustomName()) {
         ...
         if (_snowman instanceof HopperBlockEntity) {
            ((HopperBlockEntity)_snowman).setCustomName(itemStack.getName());
         }
      }
   }
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      if (world.isClient) {
         ...
         BlockEntity _snowman = world.getBlockEntity(pos);
         if (_snowman instanceof HopperBlockEntity) {
            player.openHandledScreen((HopperBlockEntity)_snowman);
   ...
   private void updateEnabled(World world, BlockPos pos, BlockState state) {
      ...
      if (_snowman != state.get(ENABLED)) {
         world.setBlockState(pos, state.with(ENABLED, Boolean.valueOf(_snowman)), 4);
      }
   }
   ...
   public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
      if (!state.isOf(newState.getBlock())) {
         BlockEntity _snowman = world.getBlockEntity(pos);
         if (_snowman instanceof HopperBlockEntity) {
            ItemScatterer.spawn(world, pos, (HopperBlockEntity)_snowman);
   ...
   public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
      BlockEntity _snowman = world.getBlockEntity(pos);
      if (_snowman instanceof HopperBlockEntity) {
         ((HopperBlockEntity)_snowman).onEntityCollided(entity);
      }
   }
