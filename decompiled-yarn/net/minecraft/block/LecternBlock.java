...
public class LecternBlock extends BlockWithEntity {
   ...
   protected LecternBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      ...
      ItemStack _snowmanx = ctx.getStack();
      CompoundTag _snowmanxx = _snowmanx.getTag();
      PlayerEntity _snowmanxxx = ctx.getPlayer();
      boolean _snowmanxxxx = false;
      if (!_snowman.isClient && _snowmanxxx != null && _snowmanxx != null && _snowmanxxx.isCreativeLevelTwoOp() && _snowmanxx.contains("BlockEntityTag")) {
         CompoundTag _snowmanxxxxx = _snowmanxx.getCompound("BlockEntityTag");
         if (_snowmanxxxxx.contains("Book")) {
            _snowmanxxxx = true;
         }
      }

      return this.getDefaultState().with(FACING, ctx.getPlayerFacing().getOpposite()).with(HAS_BOOK, Boolean.valueOf(_snowmanxxxx));
   }
   ...
   private static void putBook(World world, BlockPos pos, BlockState state, ItemStack book) {
      ...
      if (_snowman instanceof LecternBlockEntity) {
         LecternBlockEntity _snowmanx = (LecternBlockEntity)_snowman;
         _snowmanx.setBook(book.split(1));
   ...
   private void dropBook(BlockState state, World world, BlockPos pos) {
      ...
      if (_snowman instanceof LecternBlockEntity) {
         LecternBlockEntity _snowmanx = (LecternBlockEntity)_snowman;
         Direction _snowmanxx = state.get(FACING);
         ItemStack _snowmanxxx = _snowmanx.getBook().copy();
         float _snowmanxxxx = 0.25F * (float)_snowmanxx.getOffsetX();
         float _snowmanxxxxx = 0.25F * (float)_snowmanxx.getOffsetZ();
         ItemEntity _snowmanxxxxxx = new ItemEntity(
            world, (double)pos.getX() + 0.5 + (double)_snowmanxxxx, (double)(pos.getY() + 1), (double)pos.getZ() + 0.5 + (double)_snowmanxxxxx, _snowmanxxx
         );
   ...
   public int getComparatorOutput(BlockState state, World world, BlockPos pos) {
      if (state.get(HAS_BOOK)) {
         BlockEntity _snowman = world.getBlockEntity(pos);
         if (_snowman instanceof LecternBlockEntity) {
            return ((LecternBlockEntity)_snowman).getComparatorOutput();
         }
      }
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      if (state.get(HAS_BOOK)) {
         ...
         return !_snowman.isEmpty() && !_snowman.getItem().isIn(ItemTags.LECTERN_BOOKS) ? ActionResult.CONSUME : ActionResult.PASS;
      }
   }
   ...
   private void openScreen(World world, BlockPos pos, PlayerEntity player) {
      BlockEntity _snowman = world.getBlockEntity(pos);
      if (_snowman instanceof LecternBlockEntity) {
         player.openHandledScreen((LecternBlockEntity)_snowman);
