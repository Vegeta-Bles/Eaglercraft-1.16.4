...
public class ObserverBlock extends FacingBlock {
   ...
   public ObserverBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
      if (!state.isOf(oldState.getBlock())) {
         if (!world.isClient() && state.get(POWERED) && !world.getBlockTickScheduler().isScheduled(pos, this)) {
            BlockState _snowman = state.with(POWERED, Boolean.valueOf(false));
            world.setBlockState(pos, _snowman, 18);
            this.updateNeighbors(world, pos, _snowman);
         }
      }
   }
