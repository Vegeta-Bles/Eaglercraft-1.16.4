...
public class PistonHeadBlock extends FacingBlock {
   ...
   private static VoxelShape[] method_31019(boolean _snowman) {
      return Arrays.stream(Direction.values()).map(_snowmanxx -> getHeadShape(_snowmanxx, _snowman)).toArray(VoxelShape[]::new);
   }

   private static VoxelShape getHeadShape(Direction _snowman, boolean _snowman) {
      switch (_snowman) {
         case DOWN:
         default:
            return VoxelShapes.union(DOWN_HEAD_SHAPE, _snowman ? SHORT_DOWN_ARM_SHAPE : DOWN_ARM_SHAPE);
         case UP:
            return VoxelShapes.union(UP_HEAD_SHAPE, _snowman ? SHORT_UP_ARM_SHAPE : UP_ARM_SHAPE);
         case NORTH:
            return VoxelShapes.union(NORTH_HEAD_SHAPE, _snowman ? SHORT_NORTH_ARM_SHAPE : NORTH_ARM_SHAPE);
         case SOUTH:
            return VoxelShapes.union(SOUTH_HEAD_SHAPE, _snowman ? SHORT_SOUTH_ARM_SHAPE : SOUTH_ARM_SHAPE);
         case WEST:
            return VoxelShapes.union(WEST_HEAD_SHAPE, _snowman ? SHORT_WEST_ARM_SHAPE : WEST_ARM_SHAPE);
         case EAST:
            return VoxelShapes.union(EAST_HEAD_SHAPE, _snowman ? SHORT_EAST_ARM_SHAPE : EAST_ARM_SHAPE);
      }
   }

   public PistonHeadBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      return (state.get(SHORT) ? field_26660 : field_26661)[state.get(FACING).ordinal()];
   }

   private boolean method_26980(BlockState _snowman, BlockState _snowman) {
      Block _snowmanxx = _snowman.get(TYPE) == PistonType.DEFAULT ? Blocks.PISTON : Blocks.STICKY_PISTON;
      return _snowman.isOf(_snowmanxx) && _snowman.get(PistonBlock.EXTENDED) && _snowman.get(FACING) == _snowman.get(FACING);
   }
   ...
   public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
      if (!world.isClient && player.abilities.creativeMode) {
         ...
         if (this.method_26980(state, world.getBlockState(_snowman))) {
            world.breakBlock(_snowman, false);
         }
      }
   ...
   public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
      if (!state.isOf(newState.getBlock())) {
         ...
         BlockPos _snowman = pos.offset(state.get(FACING).getOpposite());
         if (this.method_26980(state, world.getBlockState(_snowman))) {
            world.breakBlock(_snowman, true);
         }
      }
   }
   ...
   public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
      BlockState _snowman = world.getBlockState(pos.offset(state.get(FACING).getOpposite()));
      return this.method_26980(state, _snowman) || _snowman.isOf(Blocks.MOVING_PISTON) && _snowman.get(FACING) == state.get(FACING);
   }
   ...
   public void neighborUpdate(BlockState state, World world, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
      if (state.canPlaceAt(world, pos)) {
         BlockPos _snowman = pos.offset(state.get(FACING).getOpposite());
         world.getBlockState(_snowman).neighborUpdate(world, _snowman, block, fromPos, false);
      }
   }
