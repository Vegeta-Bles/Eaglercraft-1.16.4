...
public class PressurePlateBlock extends AbstractPressurePlateBlock {
   ...
   protected int getRedstoneOutput(World world, BlockPos pos) {
      Box _snowman = BOX.offset(pos);
      List<? extends Entity> _snowmanx;
      switch (this.type) {
         case EVERYTHING:
            _snowmanx = world.getOtherEntities(null, _snowman);
            break;
         case MOBS:
            _snowmanx = world.getNonSpectatingEntities(LivingEntity.class, _snowman);
            break;
         default:
            return 0;
      }
      ...
      if (!_snowmanx.isEmpty()) {
         for (Entity _snowman : _snowmanx) {
            if (!_snowman.canAvoidTraps()) {
               return 15;
            }
         }
      }
