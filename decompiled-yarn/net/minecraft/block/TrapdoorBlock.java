...
public class TrapdoorBlock extends HorizontalFacingBlock implements Waterloggable {
   ...
   protected TrapdoorBlock(AbstractBlock.Settings _snowman) {
      super(_snowman);
   ...
   protected void playToggleSound(@Nullable PlayerEntity player, World world, BlockPos pos, boolean open) {
      if (open) {
         int _snowman = this.material == Material.METAL ? 1037 : 1007;
         world.syncWorldEvent(player, _snowman, pos, 0);
      } else {
         int _snowman = this.material == Material.METAL ? 1036 : 1013;
         world.syncWorldEvent(player, _snowman, pos, 0);
      }
   }
      ...
      if (!world.isClient) {
         ...
         if (_snowman != state.get(POWERED)) {
            if (state.get(OPEN) != _snowman) {
               state = state.with(OPEN, Boolean.valueOf(_snowman));
               this.playToggleSound(null, world, pos, _snowman);
            }

            world.setBlockState(pos, state.with(POWERED, Boolean.valueOf(_snowman)), 2);
            if (state.get(WATERLOGGED)) {
               world.getFluidTickScheduler().schedule(pos, Fluids.WATER, Fluids.WATER.getTickRate(world));
            }
         }
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      ...
      Direction _snowmanxx = ctx.getSide();
      if (!ctx.canReplaceExisting() && _snowmanxx.getAxis().isHorizontal()) {
         _snowman = _snowman.with(FACING, _snowmanxx).with(HALF, ctx.getHitPos().y - (double)ctx.getBlockPos().getY() > 0.5 ? BlockHalf.TOP : BlockHalf.BOTTOM);
      } else {
         _snowman = _snowman.with(FACING, ctx.getPlayerFacing().getOpposite()).with(HALF, _snowmanxx == Direction.UP ? BlockHalf.BOTTOM : BlockHalf.TOP);
      }

      if (ctx.getWorld().isReceivingRedstonePower(ctx.getBlockPos())) {
         _snowman = _snowman.with(OPEN, Boolean.valueOf(true)).with(POWERED, Boolean.valueOf(true));
      }

      return _snowman.with(WATERLOGGED, Boolean.valueOf(_snowmanx.getFluid() == Fluids.WATER));
   }
