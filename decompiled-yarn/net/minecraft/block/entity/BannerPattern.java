...
public enum BannerPattern {
   ...
   public static final int field_24417 = (int)Arrays.stream(VALUES).filter(_snowman -> _snowman.field_24419).count();
   ...
   private BannerPattern(String name, String id, boolean var5) {
      this.name = name;
      this.id = id;
      this.field_24419 = _snowman;
   }

   public Identifier getSpriteId(boolean _snowman) {
      String _snowmanx = _snowman ? "banner" : "shield";
      return new Identifier("entity/" + _snowmanx + "/" + this.getName());
   }
   ...
   public static BannerPattern byId(String id) {
      for (BannerPattern _snowman : values()) {
         if (_snowman.id.equals(id)) {
            return _snowman;
         }
      }
   ...
   public static class Patterns {
      ...
      public ListTag toTag() {
         ...
         for (Pair<BannerPattern, DyeColor> _snowmanx : this.entries) {
            CompoundTag _snowmanxx = new CompoundTag();
            _snowmanxx.putString("Pattern", ((BannerPattern)_snowmanx.getLeft()).id);
            _snowmanxx.putInt("Color", ((DyeColor)_snowmanx.getRight()).getId());
