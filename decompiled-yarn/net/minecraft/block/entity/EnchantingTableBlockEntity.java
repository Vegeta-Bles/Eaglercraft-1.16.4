...
public class EnchantingTableBlockEntity extends BlockEntity implements Nameable, Tickable {
   ...
   public void tick() {
      ...
      PlayerEntity _snowman = this.world.getClosestPlayer((double)this.pos.getX() + 0.5, (double)this.pos.getY() + 0.5, (double)this.pos.getZ() + 0.5, 3.0, false);
      if (_snowman != null) {
         double _snowmanx = _snowman.getX() - ((double)this.pos.getX() + 0.5);
         double _snowmanxx = _snowman.getZ() - ((double)this.pos.getZ() + 0.5);
         this.field_11962 = (float)MathHelper.atan2(_snowmanxx, _snowmanx);
         this.nextPageTurningSpeed += 0.1F;
         if (this.nextPageTurningSpeed < 0.5F || RANDOM.nextInt(40) == 0) {
            float _snowmanxxx = this.field_11969;

            do {
               this.field_11969 = this.field_11969 + (float)(RANDOM.nextInt(4) - RANDOM.nextInt(4));
            } while (_snowmanxxx == this.field_11969);
         }
      } else {
      ...
      float _snowmanx = this.field_11962 - this.field_11964;
      ...
      this.field_11964 += _snowmanx * 0.4F;
      this.nextPageTurningSpeed = MathHelper.clamp(this.nextPageTurningSpeed, 0.0F, 1.0F);
      this.ticks++;
      this.pageAngle = this.nextPageAngle;
      float _snowmanxx = (this.field_11969 - this.nextPageAngle) * 0.4F;
      float _snowmanxxx = 0.2F;
      _snowmanxx = MathHelper.clamp(_snowmanxx, -0.2F, 0.2F);
      this.field_11967 = this.field_11967 + (_snowmanxx - this.field_11967) * 0.9F;
