...
public class LecternBlockEntity extends BlockEntity implements Clearable, NamedScreenHandlerFactory {
   private final Inventory inventory = new Inventory() {
      ...
      public ItemStack removeStack(int slot, int amount) {
         if (slot == 0) {
            ItemStack _snowman = LecternBlockEntity.this.book.split(amount);
      ...
      public ItemStack removeStack(int slot) {
         if (slot == 0) {
            ItemStack _snowman = LecternBlockEntity.this.book;
      ...
      public boolean canPlayerUse(PlayerEntity player) {
         if (LecternBlockEntity.this.world.getBlockEntity(LecternBlockEntity.this.pos) != LecternBlockEntity.this) {
            ...
            return player.squaredDistanceTo(
                     (double)LecternBlockEntity.this.pos.getX() + 0.5,
                     (double)LecternBlockEntity.this.pos.getY() + 0.5,
                     (double)LecternBlockEntity.this.pos.getZ() + 0.5
                  )
                  > 64.0
               ? false
   ...
   public boolean hasBook() {
      Item _snowman = this.book.getItem();
      return _snowman == Items.WRITABLE_BOOK || _snowman == Items.WRITTEN_BOOK;
   }
   ...
   private void setCurrentPage(int currentPage) {
      int _snowman = MathHelper.clamp(currentPage, 0, this.pageCount - 1);
      if (_snowman != this.currentPage) {
         this.currentPage = _snowman;
         this.markDirty();
   ...
   public int getComparatorOutput() {
      float _snowman = this.pageCount > 1 ? (float)this.getCurrentPage() / ((float)this.pageCount - 1.0F) : 1.0F;
      return MathHelper.floor(_snowman * 14.0F) + (this.hasBook() ? 1 : 0);
   }
   ...
   private ServerCommandSource getCommandSource(@Nullable PlayerEntity player) {
      ...
      if (player == null) {
         _snowman = "Lectern";
         _snowmanx = new LiteralText("Lectern");
      } else {
         _snowman = player.getName().getString();
         _snowmanx = player.getDisplayName();
      }

      Vec3d _snowmanxx = Vec3d.ofCenter(this.pos);
      return new ServerCommandSource(CommandOutput.DUMMY, _snowmanxx, Vec2f.ZERO, (ServerWorld)this.world, 2, _snowman, _snowmanx, this.world.getServer(), player);
   }
   ...
   public ScreenHandler createMenu(int _snowman, PlayerInventory _snowman, PlayerEntity _snowman) {
      return new LecternScreenHandler(_snowman, this.inventory, this.propertyDelegate);
   }
