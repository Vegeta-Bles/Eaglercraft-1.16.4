...
public enum Instrument implements StringIdentifiable {
   ...
   public static Instrument fromBlockState(BlockState state) {
      if (state.isOf(Blocks.CLAY)) {
         ...
         if (_snowman == Material.STONE) {
            return BASEDRUM;
         } else if (_snowman == Material.AGGREGATE) {
            return SNARE;
         } else if (_snowman == Material.GLASS) {
            return HAT;
         } else {
            return _snowman != Material.WOOD && _snowman != Material.NETHER_WOOD ? HARP : BASS;
         }
      }
   }
}
