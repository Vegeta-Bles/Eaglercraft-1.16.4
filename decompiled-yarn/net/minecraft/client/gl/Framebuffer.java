...
public class Framebuffer {
   ...
   public void copyDepthFrom(Framebuffer _snowman) {
      RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
      if (GlStateManager.supportsGl30()) {
         GlStateManager.bindFramebuffer(36008, _snowman.fbo);
         GlStateManager.bindFramebuffer(36009, this.fbo);
         GlStateManager.blitFramebuffer(0, 0, _snowman.textureWidth, _snowman.textureHeight, 0, 0, this.textureWidth, this.textureHeight, 256, 9728);
      } else {
         GlStateManager.bindFramebuffer(FramebufferInfo.FRAME_BUFFER, this.fbo);
         int _snowmanx = GlStateManager.getFramebufferDepthAttachment();
         if (_snowmanx != 0) {
            int _snowmanxx = GlStateManager.getActiveBoundTexture();
            GlStateManager.bindTexture(_snowmanx);
            GlStateManager.bindFramebuffer(FramebufferInfo.FRAME_BUFFER, _snowman.fbo);
            GlStateManager.copyTexSubImage2d(3553, 0, 0, 0, 0, 0, Math.min(this.textureWidth, _snowman.textureWidth), Math.min(this.textureHeight, _snowman.textureHeight));
            GlStateManager.bindTexture(_snowmanxx);
         }
      }
   ...
   public void setTexFilter(int _snowman) {
      RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
      this.texFilter = _snowman;
      GlStateManager.bindTexture(this.colorAttachment);
      GlStateManager.texParameter(3553, 10241, _snowman);
      GlStateManager.texParameter(3553, 10240, _snowman);
      GlStateManager.texParameter(3553, 10242, 10496);
      GlStateManager.texParameter(3553, 10243, 10496);
   ...
   public void checkFramebufferStatus() {
      RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
      int _snowman = GlStateManager.checkFramebufferStatus(FramebufferInfo.FRAME_BUFFER);
      if (_snowman != FramebufferInfo.FRAME_BUFFER_COMPLETE) {
         if (_snowman == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_ATTACHMENT) {
            throw new RuntimeException("GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT");
         } else if (_snowman == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_MISSING_ATTACHMENT) {
            throw new RuntimeException("GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT");
         } else if (_snowman == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_DRAW_BUFFER) {
            throw new RuntimeException("GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER");
         } else if (_snowman == FramebufferInfo.FRAME_BUFFER_INCOMPLETE_READ_BUFFER) {
   ...
   public void draw(int width, int height, boolean _snowman) {
      RenderSystem.assertThread(RenderSystem::isOnGameThreadOrInit);
      if (!RenderSystem.isInInitPhase()) {
         RenderSystem.recordRenderCall(() -> this.drawInternal(width, height, _snowman));
      } else {
         this.drawInternal(width, height, _snowman);
      }
   }

   private void drawInternal(int width, int height, boolean _snowman) {
      RenderSystem.assertThread(RenderSystem::isOnRenderThread);
      ...
      this.beginRead();
      float _snowmanx = (float)width;
      float _snowmanxx = (float)height;
      float _snowmanxxx = (float)this.viewportWidth / (float)this.textureWidth;
      float _snowmanxxxx = (float)this.viewportHeight / (float)this.textureHeight;
      Tessellator _snowmanxxxxx = RenderSystem.renderThreadTesselator();
      BufferBuilder _snowmanxxxxxx = _snowmanxxxxx.getBuffer();
      _snowmanxxxxxx.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
      _snowmanxxxxxx.vertex(0.0, (double)_snowmanxx, 0.0).texture(0.0F, 0.0F).color(255, 255, 255, 255).next();
      _snowmanxxxxxx.vertex((double)_snowmanx, (double)_snowmanxx, 0.0).texture(_snowmanxxx, 0.0F).color(255, 255, 255, 255).next();
      _snowmanxxxxxx.vertex((double)_snowmanx, 0.0, 0.0).texture(_snowmanxxx, _snowmanxxxx).color(255, 255, 255, 255).next();
      _snowmanxxxxxx.vertex(0.0, 0.0, 0.0).texture(0.0F, _snowmanxxxx).color(255, 255, 255, 255).next();
      _snowmanxxxxx.draw();
      this.endRead();
   ...
   public void clear(boolean getError) {
      ...
      GlStateManager.clear(_snowman, getError);
      this.endWrite();
   }
