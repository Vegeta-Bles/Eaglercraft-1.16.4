...
public class VertexBuffer implements AutoCloseable {
   ...
   public VertexBuffer(VertexFormat format) {
      this.format = format;
      RenderSystem.glGenBuffers(_snowman -> this.id = _snowman);
   }
   ...
   public CompletableFuture<Void> submitUpload(BufferBuilder buffer) {
      if (!RenderSystem.isOnRenderThread()) {
         return CompletableFuture.runAsync(() -> this.uploadInternal(buffer), _snowman -> RenderSystem.recordRenderCall(_snowman::run));
      } else {
         this.uploadInternal(buffer);
   ...
   private void uploadInternal(BufferBuilder buffer) {
      Pair<BufferBuilder.DrawArrayParameters, ByteBuffer> _snowman = buffer.popData();
      if (this.id != -1) {
         ByteBuffer _snowmanx = (ByteBuffer)_snowman.getSecond();
         this.vertexCount = _snowmanx.remaining() / this.format.getVertexSize();
         this.bind();
