...
public class TeleportToSpecificPlayerSpectatorCommand implements SpectatorMenuCommand {
   ...
   public TeleportToSpecificPlayerSpectatorCommand(GameProfile gameProfile) {
      ...
      Map<Type, MinecraftProfileTexture> _snowmanx = _snowman.getSkinProvider().getTextures(gameProfile);
      if (_snowmanx.containsKey(Type.SKIN)) {
         this.skinId = _snowman.getSkinProvider().loadSkin(_snowmanx.get(Type.SKIN), Type.SKIN);
      } else {
