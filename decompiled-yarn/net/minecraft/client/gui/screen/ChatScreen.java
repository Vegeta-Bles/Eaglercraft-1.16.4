...
public class ChatScreen extends Screen {
   ...
   public void resize(MinecraftClient client, int width, int height) {
      String _snowman = this.chatField.getText();
      this.init(client, width, height);
      this.setText(_snowman);
      this.commandSuggestor.refresh();
   }
   ...
   private void onChatFieldUpdate(String chatText) {
      String _snowman = this.chatField.getText();
      this.commandSuggestor.setWindowActive(!_snowman.equals(this.originalChatText));
      this.commandSuggestor.refresh();
   }
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (this.commandSuggestor.mouseClicked((double)((int)mouseX), (double)((int)mouseY), button)) {
         ...
         if (button == 0) {
            ...
            if (_snowman.mouseClicked(mouseX, mouseY)) {
               return true;
            }

            Style _snowmanx = _snowman.getText(mouseX, mouseY);
            if (_snowmanx != null && this.handleTextClick(_snowmanx)) {
               return true;
            }
         }
   ...
   public void setChatFromHistory(int _snowman) {
      int _snowmanx = this.messageHistorySize + _snowman;
      int _snowmanxx = this.client.inGameHud.getChatHud().getMessageHistory().size();
      _snowmanx = MathHelper.clamp(_snowmanx, 0, _snowmanxx);
      if (_snowmanx != this.messageHistorySize) {
         if (_snowmanx == _snowmanxx) {
            this.messageHistorySize = _snowmanxx;
            this.chatField.setText(this.field_2389);
         } else {
            if (this.messageHistorySize == _snowmanxx) {
               this.field_2389 = this.chatField.getText();
            }

            this.chatField.setText(this.client.inGameHud.getChatHud().getMessageHistory().get(_snowmanx));
            this.commandSuggestor.setWindowActive(false);
            this.messageHistorySize = _snowmanx;
         }
      }
   }
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      Style _snowman = this.client.inGameHud.getChatHud().getText((double)mouseX, (double)mouseY);
      if (_snowman != null && _snowman.getHoverEvent() != null) {
