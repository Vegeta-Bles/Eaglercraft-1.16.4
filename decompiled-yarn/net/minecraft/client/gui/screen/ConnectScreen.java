...
public class ConnectScreen extends Screen {
   ...
   public ConnectScreen(Screen parent, MinecraftClient client, ServerInfo entry) {
      ...
      this.client = client;
      this.parent = parent;
      ServerAddress _snowman = ServerAddress.parse(entry.address);
      client.disconnect();
      client.setCurrentServerEntry(entry);
      this.connect(_snowman.getAddress(), _snowman.getPort());
   }
   ...
   private void connect(String address, int port) {
      ...
      Thread _snowman = new Thread("Server Connector #" + CONNECTOR_THREADS_COUNT.incrementAndGet()) {
         @Override
         public void run() {
            ...
            try {
               ...
               _snowman = InetAddress.getByName(address);
               ConnectScreen.this.connection = ClientConnection.connect(_snowman, port, ConnectScreen.this.client.options.shouldUseNativeTransport());
               ConnectScreen.this.connection
                  .setPacketListener(
                     new ClientLoginNetworkHandler(
                        ConnectScreen.this.connection, ConnectScreen.this.client, ConnectScreen.this.parent, _snowmanx -> ConnectScreen.this.setStatus(_snowmanx)
                     )
                  );
                     ...
                     () -> ConnectScreen.this.client
                           .openScreen(
                              new DisconnectedScreen(
                                 ConnectScreen.this.parent, ScreenTexts.CONNECT_FAILED, new TranslatableText("disconnect.genericReason", "Unknown host")
                              )
                           )
                  );
               ...
               String _snowmanx = _snowman == null ? var5.toString() : var5.toString().replaceAll(_snowman + ":" + port, "");
               ConnectScreen.this.client
                  .execute(
                     () -> ConnectScreen.this.client
                           .openScreen(
                              new DisconnectedScreen(ConnectScreen.this.parent, ScreenTexts.CONNECT_FAILED, new TranslatableText("disconnect.genericReason", _snowman))
                           )
                  );
            }
         }
   ...
   @Override
   protected void init() {
      this.addButton(new ButtonWidget(this.width / 2 - 100, this.height / 4 + 120 + 12, 200, 20, ScreenTexts.CANCEL, _snowman -> {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      if (_snowman - this.narratorTimer > 2000L) {
         this.narratorTimer = _snowman;
