...
public class DirectConnectScreen extends Screen {
   ...
   protected void init() {
      ...
      this.selectServerButton = this.addButton(
         new ButtonWidget(this.width / 2 - 100, this.height / 4 + 96 + 12, 200, 20, new TranslatableText("selectServer.select"), _snowman -> this.saveAndClose())
      );
      this.addButton(new ButtonWidget(this.width / 2 - 100, this.height / 4 + 120 + 12, 200, 20, ScreenTexts.CANCEL, _snowman -> this.callback.accept(false)));
   ...
   private void onAddressFieldChanged() {
      String _snowman = this.addressField.getText();
      this.selectServerButton.active = !_snowman.isEmpty() && _snowman.split(":").length > 0 && _snowman.indexOf(32) == -1;
   }
