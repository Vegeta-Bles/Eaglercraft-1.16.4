...
public class OpenToLanScreen extends Screen {
   ...
   @Override
   protected void init() {
      this.addButton(new ButtonWidget(this.width / 2 - 155, this.height - 28, 150, 20, new TranslatableText("lanServer.start"), _snowman -> {
         this.client.openScreen(null);
         int _snowmanx = NetworkUtils.findLocalPort();
         Text _snowmanxx;
         if (this.client.getServer().openToLan(GameMode.byName(this.gameMode), this.allowCommands, _snowmanx)) {
            _snowmanxx = new TranslatableText("commands.publish.started", _snowmanx);
         } else {
            _snowmanxx = new TranslatableText("commands.publish.failed");
         }

         this.client.inGameHud.getChatHud().addMessage(_snowmanxx);
         this.client.updateWindowTitle();
      }));
      this.addButton(new ButtonWidget(this.width / 2 + 5, this.height - 28, 150, 20, ScreenTexts.CANCEL, _snowman -> this.client.openScreen(this.parent)));
      this.buttonGameMode = this.addButton(new ButtonWidget(this.width / 2 - 155, 100, 150, 20, LiteralText.EMPTY, _snowman -> {
      ...
      this.buttonAllowCommands = this.addButton(new ButtonWidget(this.width / 2 + 5, 100, 150, 20, field_25889, _snowman -> {
