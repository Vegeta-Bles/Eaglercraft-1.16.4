...
public class VideoOptionsScreen extends GameOptionsScreen {
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      int _snowman = this.gameOptions.guiScale;
      if (super.mouseClicked(mouseX, mouseY, button)) {
         if (this.gameOptions.guiScale != _snowman) {
            this.client.onResolutionChanged();
         }

         if (this.warningManager.shouldWarn()) {
            List<StringVisitable> _snowmanx = Lists.newArrayList(new StringVisitable[]{GRAPHICS_WARNING_MESSAGE_TEXT, NEWLINE_TEXT});
            String _snowmanxx = this.warningManager.getRendererWarning();
            if (_snowmanxx != null) {
               _snowmanx.add(NEWLINE_TEXT);
               _snowmanx.add(new TranslatableText("options.graphics.warning.renderer", _snowmanxx).formatted(Formatting.GRAY));
            }

            String _snowmanxxx = this.warningManager.getVendorWarning();
            if (_snowmanxxx != null) {
               _snowmanx.add(NEWLINE_TEXT);
               _snowmanx.add(new TranslatableText("options.graphics.warning.vendor", _snowmanxxx).formatted(Formatting.GRAY));
            }

            String _snowmanxxxx = this.warningManager.getVersionWarning();
            if (_snowmanxxxx != null) {
               _snowmanx.add(NEWLINE_TEXT);
               _snowmanx.add(new TranslatableText("options.graphics.warning.version", _snowmanxxxx).formatted(Formatting.GRAY));
            }

            this.client
               .openScreen(
                  new DialogScreen(GRAPHICS_WARNING_TITLE_TEXT, _snowmanx, ImmutableList.of(new DialogScreen.ChoiceButton(GRAPHICS_WARNING_ACCEPT_TEXT, buttonx -> {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      List<OrderedText> _snowman = getHoveredButtonTooltip(this.list, mouseX, mouseY);
      if (_snowman != null) {
         this.renderOrderedTooltip(matrices, _snowman, mouseX, mouseY);
      }
   }
}
