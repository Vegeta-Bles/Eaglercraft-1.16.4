...
public abstract class AbstractFurnaceScreen<T extends AbstractFurnaceScreenHandler> extends HandledScreen<T> implements RecipeBookProvider {
   ...
   protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
      ...
      if (this.handler.isBurning()) {
         int _snowmanxx = this.handler.getFuelProgress();
         this.drawTexture(matrices, _snowman + 56, _snowmanx + 36 + 12 - _snowmanxx, 176, 12 - _snowmanxx, 14, _snowmanxx + 1);
      }

      int _snowmanxx = this.handler.getCookProgress();
      this.drawTexture(matrices, _snowman + 79, _snowmanx + 34, 176, 14, _snowmanxx + 1, 16);
   }
