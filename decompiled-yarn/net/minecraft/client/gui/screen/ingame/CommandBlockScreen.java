...
public class CommandBlockScreen extends AbstractCommandBlockScreen {
   ...
   protected void init() {
      super.init();
      this.modeButton = this.addButton(new ButtonWidget(this.width / 2 - 50 - 100 - 4, 165, 100, 20, new TranslatableText("advMode.mode.sequence"), _snowman -> {
         this.cycleType();
         this.updateMode();
      }));
      this.conditionalModeButton = this.addButton(
         new ButtonWidget(this.width / 2 - 50, 165, 100, 20, new TranslatableText("advMode.mode.unconditional"), _snowman -> {
            this.conditional = !this.conditional;
            this.updateConditionalMode();
         })
      );
      this.redstoneTriggerButton = this.addButton(
         new ButtonWidget(this.width / 2 + 50 + 4, 165, 100, 20, new TranslatableText("advMode.mode.redstoneTriggered"), _snowman -> {
   ...
   public void updateCommandBlock() {
      CommandBlockExecutor _snowman = this.blockEntity.getCommandExecutor();
      this.consoleCommandTextField.setText(_snowman.getCommand());
      this.trackingOutput = _snowman.isTrackingOutput();
