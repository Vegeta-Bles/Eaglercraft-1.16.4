...
public class HandledScreens {
   ...
   public static <T extends ScreenHandler> void open(@Nullable ScreenHandlerType<T> type, MinecraftClient client, int id, Text title) {
      if (type == null) {
         ...
         HandledScreens.Provider<T, ?> _snowman = getProvider(type);
         if (_snowman == null) {
            LOGGER.warn("Failed to create screen for menu type: {}", Registry.SCREEN_HANDLER.getId(type));
         } else {
            _snowman.open(title, type, client, id);
         }
      }
   }
   ...
   private static <M extends ScreenHandler, U extends Screen & ScreenHandlerProvider<M>> void register(
      ...
      HandledScreens.Provider<?, ?> _snowman = PROVIDERS.put(type, provider);
      if (_snowman != null) {
   ...
   public static boolean validateScreens() {
      ...
      for (ScreenHandlerType<?> _snowmanx : Registry.SCREEN_HANDLER) {
         if (!PROVIDERS.containsKey(_snowmanx)) {
            LOGGER.debug("Menu {} has no matching screen", Registry.SCREEN_HANDLER.getId(_snowmanx));
            _snowman = true;
         }
      }
   ...
   interface Provider<T extends ScreenHandler, U extends Screen & ScreenHandlerProvider<T>> {
      default void open(Text name, ScreenHandlerType<T> type, MinecraftClient client, int id) {
         U _snowman = this.create(type.create(id, client.player.inventory), client.player.inventory, name);
         client.player.currentScreenHandler = _snowman.getScreenHandler();
         client.openScreen(_snowman);
      }
