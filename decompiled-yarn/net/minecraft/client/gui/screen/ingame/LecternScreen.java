...
public class LecternScreen extends BookScreen implements ScreenHandlerProvider<LecternScreenHandler> {
   ...
   protected void addCloseButton() {
      if (this.client.player.canModifyBlocks()) {
         this.addButton(new ButtonWidget(this.width / 2 - 100, 196, 98, 20, ScreenTexts.DONE, _snowman -> this.client.openScreen(null)));
         this.addButton(new ButtonWidget(this.width / 2 + 2, 196, 98, 20, new TranslatableText("lectern.take_book"), _snowman -> this.sendButtonPressPacket(3)));
      } else {
   ...
   private void updatePageProvider() {
      ItemStack _snowman = this.container.getBookItem();
      this.setPageProvider(BookScreen.Contents.create(_snowman));
   }
