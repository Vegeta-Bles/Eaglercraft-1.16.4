...
public class StructureBlockScreen extends Screen {
   ...
   protected void init() {
      ...
      this.buttonDone = this.addButton(new ButtonWidget(this.width / 2 - 4 - 150, 210, 150, 20, ScreenTexts.DONE, _snowman -> this.done()));
      this.buttonCancel = this.addButton(new ButtonWidget(this.width / 2 + 4, 210, 150, 20, ScreenTexts.CANCEL, _snowman -> this.cancel()));
      this.buttonSave = this.addButton(new ButtonWidget(this.width / 2 + 4 + 100, 185, 50, 20, new TranslatableText("structure_block.button.save"), _snowman -> {
         if (this.structureBlock.getMode() == StructureBlockMode.SAVE) {
            this.method_2516(StructureBlockBlockEntity.Action.SAVE_AREA);
            this.client.openScreen(null);
         }
      }));
      this.buttonLoad = this.addButton(new ButtonWidget(this.width / 2 + 4 + 100, 185, 50, 20, new TranslatableText("structure_block.button.load"), _snowman -> {
         if (this.structureBlock.getMode() == StructureBlockMode.LOAD) {
            this.method_2516(StructureBlockBlockEntity.Action.LOAD_AREA);
            this.client.openScreen(null);
         }
      }));
      this.buttonMode = this.addButton(new ButtonWidget(this.width / 2 - 4 - 150, 185, 50, 20, new LiteralText("MODE"), _snowman -> {
         this.structureBlock.cycleMode();
         this.updateMode();
      }));
      this.buttonDetect = this.addButton(
         new ButtonWidget(this.width / 2 + 4 + 100, 120, 50, 20, new TranslatableText("structure_block.button.detect_size"), _snowman -> {
            if (this.structureBlock.getMode() == StructureBlockMode.SAVE) {
      ...
      this.buttonEntities = this.addButton(new ButtonWidget(this.width / 2 + 4 + 100, 160, 50, 20, new LiteralText("ENTITIES"), _snowman -> {
         this.structureBlock.setIgnoreEntities(!this.structureBlock.shouldIgnoreEntities());
         this.updateIgnoreEntitiesButton();
      }));
      this.buttonMirror = this.addButton(new ButtonWidget(this.width / 2 - 20, 185, 40, 20, new LiteralText("MIRROR"), _snowman -> {
      ...
      this.buttonShowAir = this.addButton(new ButtonWidget(this.width / 2 + 4 + 100, 80, 50, 20, new LiteralText("SHOWAIR"), _snowman -> {
         this.structureBlock.setShowAir(!this.structureBlock.shouldShowAir());
         this.updateShowAirButton();
      }));
      this.buttonShowBoundingBox = this.addButton(new ButtonWidget(this.width / 2 + 4 + 100, 80, 50, 20, new LiteralText("SHOWBB"), _snowman -> {
         this.structureBlock.setShowBoundingBox(!this.structureBlock.shouldShowBoundingBox());
         this.updateShowBoundingBoxButton();
      }));
      this.buttonRotate0 = this.addButton(new ButtonWidget(this.width / 2 - 1 - 40 - 1 - 40 - 20, 185, 40, 20, new LiteralText("0"), _snowman -> {
         this.structureBlock.setRotation(BlockRotation.NONE);
         this.updateRotationButton();
      }));
      this.buttonRotate90 = this.addButton(new ButtonWidget(this.width / 2 - 1 - 40 - 20, 185, 40, 20, new LiteralText("90"), _snowman -> {
         this.structureBlock.setRotation(BlockRotation.CLOCKWISE_90);
         this.updateRotationButton();
      }));
      this.buttonRotate180 = this.addButton(new ButtonWidget(this.width / 2 + 1 + 20, 185, 40, 20, new LiteralText("180"), _snowman -> {
         this.structureBlock.setRotation(BlockRotation.CLOCKWISE_180);
         this.updateRotationButton();
      }));
      this.buttonRotate270 = this.addButton(new ButtonWidget(this.width / 2 + 1 + 40 + 1 + 20, 185, 40, 20, new LiteralText("270"), _snowman -> {
      ...
      BlockPos _snowman = this.structureBlock.getOffset();
      ...
      this.inputPosX.setText(Integer.toString(_snowman.getX()));
      ...
      this.inputPosY.setText(Integer.toString(_snowman.getY()));
      ...
      this.inputPosZ.setText(Integer.toString(_snowman.getZ()));
      this.children.add(this.inputPosZ);
      BlockPos _snowmanx = this.structureBlock.getSize();
      ...
      this.inputSizeX.setText(Integer.toString(_snowmanx.getX()));
      ...
      this.inputSizeY.setText(Integer.toString(_snowmanx.getY()));
      ...
      this.inputSizeZ.setText(Integer.toString(_snowmanx.getZ()));
      this.children.add(this.inputSizeZ);
      this.inputIntegrity = new TextFieldWidget(
         this.textRenderer, this.width / 2 - 152, 120, 80, 20, new TranslatableText("structure_block.integrity.integrity")
      );
   ...
   public void resize(MinecraftClient client, int width, int height) {
      String _snowman = this.inputName.getText();
      String _snowmanx = this.inputPosX.getText();
      String _snowmanxx = this.inputPosY.getText();
      String _snowmanxxx = this.inputPosZ.getText();
      String _snowmanxxxx = this.inputSizeX.getText();
      String _snowmanxxxxx = this.inputSizeY.getText();
      String _snowmanxxxxxx = this.inputSizeZ.getText();
      String _snowmanxxxxxxx = this.inputIntegrity.getText();
      String _snowmanxxxxxxxx = this.inputSeed.getText();
      String _snowmanxxxxxxxxx = this.inputMetadata.getText();
      this.init(client, width, height);
      this.inputName.setText(_snowman);
      this.inputPosX.setText(_snowmanx);
      this.inputPosY.setText(_snowmanxx);
      this.inputPosZ.setText(_snowmanxxx);
      this.inputSizeX.setText(_snowmanxxxx);
      this.inputSizeY.setText(_snowmanxxxxx);
      this.inputSizeZ.setText(_snowmanxxxxxx);
      this.inputIntegrity.setText(_snowmanxxxxxxx);
      this.inputSeed.setText(_snowmanxxxxxxxx);
      this.inputMetadata.setText(_snowmanxxxxxxxxx);
   }
   ...
   private void updateMirrorButton() {
      BlockMirror _snowman = this.structureBlock.getMirror();
      switch (_snowman) {
         case NONE:
   ...
   private boolean method_2516(StructureBlockBlockEntity.Action _snowman) {
      BlockPos _snowmanx = new BlockPos(this.parseInt(this.inputPosX.getText()), this.parseInt(this.inputPosY.getText()), this.parseInt(this.inputPosZ.getText()));
      BlockPos _snowmanxx = new BlockPos(this.parseInt(this.inputSizeX.getText()), this.parseInt(this.inputSizeY.getText()), this.parseInt(this.inputSizeZ.getText()));
      float _snowmanxxx = this.parseFloat(this.inputIntegrity.getText());
      long _snowmanxxxx = this.parseLong(this.inputSeed.getText());
      this.client
         .getNetworkHandler()
         .sendPacket(
   ...
   private long parseLong(String _snowman) {
      try {
   ...
   private float parseFloat(String _snowman) {
      try {
   ...
   private int parseInt(String _snowman) {
      try {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      StructureBlockMode _snowman = this.structureBlock.getMode();
      drawCenteredText(matrices, this.textRenderer, this.title, this.width / 2, 10, 16777215);
      if (_snowman != StructureBlockMode.DATA) {
      ...
      if (_snowman == StructureBlockMode.LOAD || _snowman == StructureBlockMode.SAVE) {
      ...
      if (_snowman == StructureBlockMode.SAVE) {
      ...
      if (_snowman == StructureBlockMode.LOAD) {
      ...
      drawTextWithShadow(matrices, this.textRenderer, _snowman.method_30844(), this.width / 2 - 153, 174, 10526880);
