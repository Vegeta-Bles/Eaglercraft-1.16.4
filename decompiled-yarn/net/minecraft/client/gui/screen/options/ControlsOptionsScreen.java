...
public class ControlsOptionsScreen extends GameOptionsScreen {
   ...
   protected void init() {
      ...
      this.resetButton = this.addButton(
         new ButtonWidget(this.width / 2 - 155, this.height - 29, 150, 20, new TranslatableText("controls.resetAll"), button -> {
            for (KeyBinding _snowman : this.gameOptions.keysAll) {
               _snowman.setBoundKey(_snowman.getDefaultKey());
            }
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      for (KeyBinding _snowmanx : this.gameOptions.keysAll) {
         if (!_snowmanx.isDefault()) {
            _snowman = true;
            break;
         }
      }

      this.resetButton.active = _snowman;
