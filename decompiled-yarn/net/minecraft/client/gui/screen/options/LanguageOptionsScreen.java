...
public class LanguageOptionsScreen extends GameOptionsScreen {
   ...
   protected void init() {
      ...
      this.doneButton = this.addButton(new ButtonWidget(this.width / 2 - 155 + 160, this.height - 38, 150, 20, ScreenTexts.DONE, button -> {
         LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry _snowman = this.languageSelectionList.getSelected();
         if (_snowman != null && !_snowman.languageDefinition.getCode().equals(this.languageManager.getLanguage().getCode())) {
            this.languageManager.setLanguage(_snowman.languageDefinition);
            this.gameOptions.language = _snowman.languageDefinition.getCode();
   ...
   class LanguageSelectionListWidget extends AlwaysSelectedEntryListWidget<LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry> {
      public LanguageSelectionListWidget(MinecraftClient client) {
         ...
         for (LanguageDefinition _snowman : LanguageOptionsScreen.this.languageManager.getAllLanguages()) {
            LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry _snowmanx = new LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry(_snowman);
            this.addEntry(_snowmanx);
            if (LanguageOptionsScreen.this.languageManager.getLanguage().getCode().equals(_snowman.getCode())) {
               this.setSelected(_snowmanx);
            }
         }
      ...
      public void setSelected(@Nullable LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry _snowman) {
         super.setSelected(_snowman);
         if (_snowman != null) {
            NarratorManager.INSTANCE.narrate(new TranslatableText("narrator.select", _snowman.languageDefinition).getString());
         }
      }
      ...
      public class LanguageEntry extends AlwaysSelectedEntryListWidget.Entry<LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry> {
         ...
         public void render(
            ...
            String _snowman = this.languageDefinition.toString();
            LanguageOptionsScreen.this.textRenderer
               .drawWithShadow(
                  matrices,
                  _snowman,
                  (float)(LanguageSelectionListWidget.this.width / 2 - LanguageOptionsScreen.this.textRenderer.getWidth(_snowman) / 2),
                  (float)(y + 1),
                  16777215,
                  true
               );
