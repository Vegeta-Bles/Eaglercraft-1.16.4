...
public class OptionsScreen extends Screen {
   ...
   protected void init() {
      ...
      for (Option _snowmanx : OPTIONS) {
         int _snowmanxx = this.width / 2 - 155 + _snowman % 2 * 160;
         int _snowmanxxx = this.height / 6 - 12 + 24 * (_snowman >> 1);
         this.addButton(_snowmanx.createButton(this.client.options, _snowmanxx, _snowmanxxx, 150));
         _snowman++;
      }
      ...
      if (this.client.world != null) {
         ...
         this.difficultyButton = this.addButton(
            new ButtonWidget(
               this.width / 2 - 155 + _snowman % 2 * 160, this.height / 6 - 12 + 24 * (_snowman >> 1), 150, 20, this.getDifficultyButtonText(this.difficulty), button -> {
                        ...
                        .openScreen(
                           new ConfirmScreen(
                              ...
                              new TranslatableText(
                                 ...
                                 new TranslatableText("options.difficulty." + this.client.world.getLevelProperties().getDifficulty().getName())
                              )
                           )
                        )
               )
   ...
   private void refreshResourcePacks(ResourcePackManager resourcePackManager) {
      List<String> _snowman = ImmutableList.copyOf(this.settings.resourcePacks);
      this.settings.resourcePacks.clear();
      this.settings.incompatibleResourcePacks.clear();

      for (ResourcePackProfile _snowmanx : resourcePackManager.getEnabledProfiles()) {
         if (!_snowmanx.isPinned()) {
            this.settings.resourcePacks.add(_snowmanx.getName());
            if (!_snowmanx.getCompatibility().isCompatible()) {
               this.settings.incompatibleResourcePacks.add(_snowmanx.getName());
            }
         }
      }

      this.settings.write();
      List<String> _snowmanxx = ImmutableList.copyOf(this.settings.resourcePacks);
