...
public abstract class AbstractButtonWidget extends DrawableHelper implements Drawable, Element {
   ...
   protected void narrate() {
      if (this.active && this.isHovered() && Util.getMeasuringTimeMs() > this.nextNarration) {
         String _snowman = this.getNarrationMessage().getString();
         if (!_snowman.isEmpty()) {
            NarratorManager.INSTANCE.narrate(_snowman);
            this.nextNarration = Long.MAX_VALUE;
         }
      }
   }
   ...
   public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      _snowman.getTextureManager().bindTexture(WIDGETS_LOCATION);
      RenderSystem.color4f(1.0F, 1.0F, 1.0F, this.alpha);
      int _snowmanxx = this.getYImage(this.isHovered());
      RenderSystem.enableBlend();
      RenderSystem.defaultBlendFunc();
      RenderSystem.enableDepthTest();
      this.drawTexture(matrices, this.x, this.y, 0, 46 + _snowmanxx * 20, this.width / 2, this.height);
      this.drawTexture(matrices, this.x + this.width / 2, this.y, 200 - this.width / 2, 46 + _snowmanxx * 20, this.width / 2, this.height);
      this.renderBg(matrices, _snowman, mouseX, mouseY);
      int _snowmanxxx = this.active ? 16777215 : 10526880;
      drawCenteredText(
         matrices, _snowmanx, this.getMessage(), this.x + this.width / 2, this.y + (this.height - 8) / 2, _snowmanxxx | MathHelper.ceil(this.alpha * 255.0F) << 24
      );
   }
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (this.active && this.visible) {
         if (this.isValidClickButton(button)) {
            boolean _snowman = this.clicked(mouseX, mouseY);
            if (_snowman) {
   ...
   public void setMessage(Text _snowman) {
      if (!Objects.equals(_snowman.getString(), this.message.getString())) {
         this.queueNarration(250);
      }

      this.message = _snowman;
   }
