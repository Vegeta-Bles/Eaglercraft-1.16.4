...
public class KeyBinding implements Comparable<KeyBinding> {
   ...
   private static final Map<String, Integer> categoryOrderMap = Util.make(Maps.newHashMap(), _snowman -> {
      _snowman.put("key.categories.movement", 1);
      _snowman.put("key.categories.gameplay", 2);
      _snowman.put("key.categories.inventory", 3);
      _snowman.put("key.categories.creative", 4);
      _snowman.put("key.categories.multiplayer", 5);
      _snowman.put("key.categories.ui", 6);
      _snowman.put("key.categories.misc", 7);
   });
   ...
   public static void onKeyPressed(InputUtil.Key key) {
      KeyBinding _snowman = keyToBindings.get(key);
      if (_snowman != null) {
         _snowman.timesPressed++;
      }
   }
   ...
   public static void setKeyPressed(InputUtil.Key key, boolean pressed) {
      KeyBinding _snowman = keyToBindings.get(key);
      if (_snowman != null) {
         _snowman.setPressed(pressed);
      }
   }

   public static void updatePressedStates() {
      for (KeyBinding _snowman : keysById.values()) {
         if (_snowman.boundKey.getCategory() == InputUtil.Type.KEYSYM && _snowman.boundKey.getCode() != InputUtil.UNKNOWN_KEY.getCode()) {
            _snowman.setPressed(InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), _snowman.boundKey.getCode()));
         }
      }
   }
   ...
   public static void updateKeysByCode() {
      ...
      for (KeyBinding _snowman : keysById.values()) {
         keyToBindings.put(_snowman.boundKey, _snowman);
      }
   }
   ...
   public int compareTo(KeyBinding _snowman) {
      return this.category.equals(_snowman.category)
         ? I18n.translate(this.translationKey).compareTo(I18n.translate(_snowman.translationKey))
         : categoryOrderMap.get(this.category).compareTo(categoryOrderMap.get(_snowman.category));
   }

   public static Supplier<Text> getLocalizedName(String id) {
      KeyBinding _snowman = keysById.get(id);
      return _snowman == null ? () -> new TranslatableText(id) : _snowman::getBoundKeyLocalizedText;
   }
