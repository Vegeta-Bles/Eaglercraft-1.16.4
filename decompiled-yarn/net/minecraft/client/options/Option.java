...
public abstract class Option {
   public static final DoubleOption BIOME_BLEND_RADIUS = new DoubleOption(
      "options.biomeBlendRadius", 0.0, 7.0, 1.0F, gameOptions -> (double)gameOptions.biomeBlendRadius, (gameOptions, biomeBlendRadius) -> {
         ...
         double _snowman = option.get(gameOptions);
         int _snowmanx = (int)_snowman * 2 + 1;
         return option.getGenericLabel(new TranslatableText("options.biomeBlendRadius." + _snowmanx));
      }
   );
   public static final DoubleOption CHAT_HEIGHT_FOCUSED = new DoubleOption(
      "options.chat.height.focused", 0.0, 1.0, 0.0F, gameOptions -> gameOptions.chatHeightFocused, (gameOptions, chatHeightFocused) -> {
         ...
         double _snowman = option.getRatio(option.get(gameOptions));
         return option.getPixelLabel(ChatHud.getHeight(_snowman));
      }
   );
   ...
   public static final DoubleOption CHAT_OPACITY = new DoubleOption(
      "options.chat.opacity", 0.0, 1.0, 0.0F, gameOptions -> gameOptions.chatOpacity, (gameOptions, chatOpacity) -> {
         ...
         double _snowman = option.getRatio(option.get(gameOptions));
         return option.getPercentLabel(_snowman * 0.9 + 0.1);
      }
   );
   public static final DoubleOption CHAT_SCALE = new DoubleOption(
      "options.chat.scale", 0.0, 1.0, 0.0F, gameOptions -> gameOptions.chatScale, (gameOptions, chatScale) -> {
         ...
         double _snowman = option.getRatio(option.get(gameOptions));
         return (Text)(_snowman == 0.0 ? ScreenTexts.composeToggleText(option.getDisplayPrefix(), false) : option.getPercentLabel(_snowman));
      }
   );
   public static final DoubleOption CHAT_WIDTH = new DoubleOption(
      "options.chat.width", 0.0, 1.0, 0.0F, gameOptions -> gameOptions.chatWidth, (gameOptions, chatWidth) -> {
         ...
         double _snowman = option.getRatio(option.get(gameOptions));
         return option.getPixelLabel(ChatHud.getWidth(_snowman));
      }
   );
   ...
   public static final DoubleOption CHAT_DELAY_INSTANT = new DoubleOption(
      ...
      (gameOptions, option) -> {
         double _snowman = option.get(gameOptions);
         return _snowman <= 0.0 ? new TranslatableText("options.chat.delay_none") : new TranslatableText("options.chat.delay", String.format("%.1f", _snowman));
      }
   );
   public static final DoubleOption FOV = new DoubleOption(
      "options.fov", 30.0, 110.0, 1.0F, gameOptions -> gameOptions.fov, (gameOptions, fov) -> gameOptions.fov = fov, (gameOptions, option) -> {
         ...
         if (_snowman == 70.0) {
            ...
            return _snowman == option.getMax() ? option.getGenericLabel(new TranslatableText("options.fov.max")) : option.getGenericLabel((int)_snowman);
         }
      }
   );
   ...
   public static final DoubleOption FOV_EFFECT_SCALE = new DoubleOption(
      ...
      (gameOptions, option) -> {
         ...
         double _snowman = option.getRatio(option.get(gameOptions));
         return _snowman == 0.0 ? option.getGenericLabel(new TranslatableText("options.fovEffectScale.off")) : option.getPercentLabel(_snowman);
      }
   );
   ...
   public static final DoubleOption DISTORTION_EFFECT_SCALE = new DoubleOption(
      ...
      (gameOptions, option) -> {
         ...
         double _snowman = option.getRatio(option.get(gameOptions));
         return _snowman == 0.0 ? option.getGenericLabel(new TranslatableText("options.screenEffectScale.off")) : option.getPercentLabel(_snowman);
      }
   );
   public static final DoubleOption FRAMERATE_LIMIT = new DoubleOption(
      ...
      (gameOptions, option) -> {
         double _snowman = option.get(gameOptions);
         return _snowman == option.getMax()
            ? option.getGenericLabel(new TranslatableText("options.framerateLimit.max"))
            : option.getGenericLabel(new TranslatableText("options.framerate", (int)_snowman));
      }
   );
   public static final DoubleOption GAMMA = new DoubleOption(
      "options.gamma", 0.0, 1.0, 0.0F, gameOptions -> gameOptions.gamma, (gameOptions, gamma) -> gameOptions.gamma = gamma, (gameOptions, option) -> {
         ...
         if (_snowman == 0.0) {
            ...
            return _snowman == 1.0 ? option.getGenericLabel(new TranslatableText("options.gamma.max")) : option.getPercentAdditionLabel((int)(_snowman * 100.0));
         }
      }
   );
   public static final DoubleOption MIPMAP_LEVELS = new DoubleOption(
      ...
      (gameOptions, option) -> {
         double _snowman = option.get(gameOptions);
         return (Text)(_snowman == 0.0 ? ScreenTexts.composeToggleText(option.getDisplayPrefix(), false) : option.getGenericLabel((int)_snowman));
      }
   );
   public static final DoubleOption MOUSE_WHEEL_SENSITIVITY = new LogarithmicOption(
      ...
      (gameOptions, option) -> {
         double _snowman = option.getRatio(option.get(gameOptions));
         return option.getGenericLabel(new LiteralText(String.format("%.2f", option.getValue(_snowman))));
      }
   );
   public static final BooleanOption RAW_MOUSE_INPUT = new BooleanOption(
      "options.rawMouseInput", gameOptions -> gameOptions.rawMouseInput, (gameOptions, rawMouseInput) -> {
         ...
         Window _snowman = MinecraftClient.getInstance().getWindow();
         if (_snowman != null) {
            _snowman.setRawMouseMotion(rawMouseInput);
         }
      }
   );
   public static final DoubleOption RENDER_DISTANCE = new DoubleOption(
      "options.renderDistance", 2.0, 16.0, 1.0F, gameOptions -> (double)gameOptions.viewDistance, (gameOptions, viewDistance) -> {
         ...
         double _snowman = option.get(gameOptions);
         return option.getGenericLabel(new TranslatableText("options.chunks", (int)_snowman));
      }
   );
   public static final DoubleOption ENTITY_DISTANCE_SCALING = new DoubleOption(
      ...
      (gameOptions, option) -> {
         double _snowman = option.get(gameOptions);
         return option.getPercentLabel(_snowman);
      }
   );
   public static final DoubleOption SENSITIVITY = new DoubleOption(
      ...
      (gameOptions, option) -> {
         ...
         if (_snowman == 0.0) {
            ...
            return _snowman == 1.0 ? option.getGenericLabel(new TranslatableText("options.sensitivity.max")) : option.getPercentLabel(2.0 * _snowman);
         }
      }
   );
   ...
   public static final CyclingOption GRAPHICS = new CyclingOption("options.graphics", (gameOptions, amount) -> {
      ...
      VideoWarningManager _snowmanx = _snowman.getVideoWarningManager();
      if (gameOptions.graphicsMode == GraphicsMode.FANCY && _snowmanx.canWarn()) {
         _snowmanx.scheduleWarning();
      } else {
         gameOptions.graphicsMode = gameOptions.graphicsMode.next();
         if (gameOptions.graphicsMode == GraphicsMode.FABULOUS && (!GlStateManager.supportsGl30() || _snowmanx.hasCancelledAfterWarning())) {
            gameOptions.graphicsMode = GraphicsMode.FAST;
         }

         _snowman.worldRenderer.reload();
      }
   }, (gameOptions, option) -> {
      ...
      MutableText _snowman = new TranslatableText(gameOptions.graphicsMode.getTranslationKey());
      return gameOptions.graphicsMode == GraphicsMode.FABULOUS ? option.getGenericLabel(_snowman.formatted(Formatting.ITALIC)) : option.getGenericLabel(_snowman);
   });
   public static final CyclingOption GUI_SCALE = new CyclingOption(
      ...
      (gameOptions, amount) -> gameOptions.guiScale = Integer.remainderUnsigned(
            gameOptions.guiScale + amount,
            MinecraftClient.getInstance().getWindow().calculateScaleFactor(0, MinecraftClient.getInstance().forcesUnicodeFont()) + 1
         ),
   ...
   public static final CyclingOption CLOUDS = new CyclingOption("options.renderClouds", (gameOptions, amount) -> {
      ...
      if (MinecraftClient.isFabulousGraphicsOrBetter()) {
         Framebuffer _snowman = MinecraftClient.getInstance().worldRenderer.getCloudsFramebuffer();
         if (_snowman != null) {
            _snowman.clear(MinecraftClient.IS_SYSTEM_MAC);
         }
      }
   ...
   public static final CyclingOption TEXT_BACKGROUND = new CyclingOption(
      ...
      (gameOptions, option) -> option.getGenericLabel(
            new TranslatableText(
               gameOptions.backgroundForChatOnly ? "options.accessibility.text_background.chat" : "options.accessibility.text_background.everywhere"
            )
         )
   );
   ...
   public static final BooleanOption AUTO_SUGGESTIONS = new BooleanOption(
      ...
      (gameOptions, autoSuggestions) -> gameOptions.autoSuggestions = autoSuggestions
   );
   public static final BooleanOption field_26924 = new BooleanOption("options.hideMatchedNames", field_26925, _snowman -> _snowman.field_26926, (_snowman, _snowmanx) -> _snowman.field_26926 = _snowmanx);
   ...
   public static final BooleanOption FORCE_UNICODE_FONT = new BooleanOption(
      "options.forceUnicodeFont", gameOptions -> gameOptions.forceUnicodeFont, (gameOptions, forceUnicodeFont) -> {
         ...
         MinecraftClient _snowman = MinecraftClient.getInstance();
         if (_snowman.getWindow() != null) {
            _snowman.initFont(forceUnicodeFont);
         }
      }
   );
   ...
   public static final BooleanOption REDUCED_DEBUG_INFO = new BooleanOption(
      ...
      (gameOptions, reducedDebugInfo) -> gameOptions.reducedDebugInfo = reducedDebugInfo
   );
   ...
   public static final BooleanOption FULLSCREEN = new BooleanOption("options.fullscreen", gameOptions -> gameOptions.fullscreen, (gameOptions, fullscreen) -> {
      ...
      MinecraftClient _snowman = MinecraftClient.getInstance();
      if (_snowman.getWindow() != null && _snowman.getWindow().isFullscreen() != gameOptions.fullscreen) {
         _snowman.getWindow().toggleFullscreen();
         gameOptions.fullscreen = _snowman.getWindow().isFullscreen();
      }
   });
