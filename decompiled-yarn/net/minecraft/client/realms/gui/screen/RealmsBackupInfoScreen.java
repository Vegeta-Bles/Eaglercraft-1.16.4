...
public class RealmsBackupInfoScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addButton(new ButtonWidget(this.width / 2 - 100, this.height / 4 + 120 + 24, 200, 20, ScreenTexts.BACK, _snowman -> this.client.openScreen(this.parent)));
   ...
   private Text checkForSpecificMetadata(String key, String value) {
      String _snowman = key.toLowerCase(Locale.ROOT);
      if (_snowman.contains("game") && _snowman.contains("mode")) {
         return this.gameModeMetadata(value);
      } else {
         return (Text)(_snowman.contains("game") && _snowman.contains("difficulty") ? this.gameDifficultyMetadata(value) : new LiteralText(value));
      }
   }
   ...
   class BackupInfoList extends AlwaysSelectedEntryListWidget<RealmsBackupInfoScreen.BackupInfoListEntry> {
      public BackupInfoList(MinecraftClient var2) {
         super(_snowman, RealmsBackupInfoScreen.this.width, RealmsBackupInfoScreen.this.height, 32, RealmsBackupInfoScreen.this.height - 64, 36);
   ...
   class BackupInfoListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsBackupInfoScreen.BackupInfoListEntry> {
      ...
      public void render(
         ...
         TextRenderer _snowman = RealmsBackupInfoScreen.this.client.textRenderer;
         DrawableHelper.drawStringWithShadow(matrices, _snowman, this.key, x, y, 10526880);
         DrawableHelper.drawTextWithShadow(matrices, _snowman, RealmsBackupInfoScreen.this.checkForSpecificMetadata(this.key, this.value), x, y + 12, 16777215);
      }
   }
}
