...
public class RealmsCreateRealmScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.createButton = this.addButton(
         new ButtonWidget(this.width / 2 - 100, this.height / 4 + 120 + 17, 97, 20, new TranslatableText("mco.create.world"), _snowman -> this.createWorld())
      );
      this.addButton(new ButtonWidget(this.width / 2 + 5, this.height / 4 + 120 + 17, 95, 20, ScreenTexts.CANCEL, _snowman -> this.client.openScreen(this.parent)));
   ...
   public boolean charTyped(char chr, int keyCode) {
      boolean _snowman = super.charTyped(chr, keyCode);
   ...
   public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
      if (keyCode == 256) {
         ...
         boolean _snowman = super.keyPressed(keyCode, scanCode, modifiers);
         this.createButton.active = this.valid();
         return _snowman;
      }
   }
   ...
   private void createWorld() {
      if (this.valid()) {
         RealmsResetWorldScreen _snowman = new RealmsResetWorldScreen(
            this.parent,
            this.server,
         ...
         _snowman.setResetTitle(new TranslatableText("mco.create.world.reset.title"));
         this.client
            .openScreen(
               new RealmsLongRunningMcoTaskScreen(this.parent, new WorldCreationTask(this.server.id, this.nameBox.getText(), this.descriptionBox.getText(), _snowman))
            );
      }
   }
