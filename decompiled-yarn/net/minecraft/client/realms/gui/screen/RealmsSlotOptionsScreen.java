   ...
   public void init() {
      ...
      if (this.worldType == RealmsServer.WorldType.NORMAL) {
         ...
         if (this.worldType == RealmsServer.WorldType.ADVENTUREMAP) {
            _snowman = new TranslatableText("mco.configure.world.edit.subscreen.adventuremap");
         } else if (this.worldType == RealmsServer.WorldType.INSPIRATION) {
            _snowman = new TranslatableText("mco.configure.world.edit.subscreen.inspiration");
         } else {
            _snowman = new TranslatableText("mco.configure.world.edit.subscreen.experience");
         }

         this.toastMessage = new RealmsLabel(_snowman, this.width / 2, 26, 16711680);
         this.pvp = true;
      ...
      this.pvpButton = this.addButton(new ButtonWidget(this.column2_x, row(1), this.column_width, 20, this.pvpTitle(), _snowman -> {
         this.pvp = !this.pvp;
         _snowman.setMessage(this.pvpTitle());
      }));
      this.addButton(new ButtonWidget(this.column1_x, row(3), this.column_width, 20, this.gameModeTitle(), _snowman -> {
         this.gameModeIndex = (this.gameModeIndex + 1) % GAME_MODES.length;
         _snowman.setMessage(this.gameModeTitle());
      }));
      this.spawnAnimalsButton = this.addButton(new ButtonWidget(this.column2_x, row(3), this.column_width, 20, this.spawnAnimalsTitle(), _snowman -> {
         this.spawnAnimals = !this.spawnAnimals;
         _snowman.setMessage(this.spawnAnimalsTitle());
      }));
      this.addButton(new ButtonWidget(this.column1_x, row(5), this.column_width, 20, this.difficultyTitle(), _snowman -> {
         this.difficultyIndex = (this.difficultyIndex + 1) % DIFFICULTIES.length;
         _snowman.setMessage(this.difficultyTitle());
         if (this.worldType == RealmsServer.WorldType.NORMAL) {
            this.spawnMonstersButton.active = this.difficultyIndex != 0;
            this.spawnMonstersButton.setMessage(this.spawnMonstersTitle());
         }
      }));
      this.spawnMonstersButton = this.addButton(new ButtonWidget(this.column2_x, row(5), this.column_width, 20, this.spawnMonstersTitle(), _snowman -> {
         this.spawnMonsters = !this.spawnMonsters;
         _snowman.setMessage(this.spawnMonstersTitle());
      }));
      ...
      this.spawnNPCsButton = this.addButton(new ButtonWidget(this.column2_x, row(7), this.column_width, 20, this.spawnNPCsTitle(), _snowman -> {
         this.spawnNPCs = !this.spawnNPCs;
         _snowman.setMessage(this.spawnNPCsTitle());
      }));
      this.gameModeButton = this.addButton(new ButtonWidget(this.column1_x, row(9), this.column_width, 20, this.forceGameModeTitle(), _snowman -> {
         this.forceGameMode = !this.forceGameMode;
         _snowman.setMessage(this.forceGameModeTitle());
      }));
      this.commandBlocksButton = this.addButton(new ButtonWidget(this.column2_x, row(9), this.column_width, 20, this.commandBlocksTitle(), _snowman -> {
         this.commandBlocks = !this.commandBlocks;
         _snowman.setMessage(this.commandBlocksTitle());
      }));
      if (this.worldType != RealmsServer.WorldType.NORMAL) {
      ...
      this.addButton(
         new ButtonWidget(this.column1_x, row(13), this.column_width, 20, new TranslatableText("mco.configure.world.buttons.done"), _snowman -> this.saveSettings())
      );
      this.addButton(new ButtonWidget(this.column2_x, row(13), this.column_width, 20, ScreenTexts.CANCEL, _snowman -> this.client.openScreen(this.parent)));
