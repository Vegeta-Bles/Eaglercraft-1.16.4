...
public class RealmsTermsScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      int _snowman = this.width / 4 - 2;
      this.addButton(new ButtonWidget(this.width / 4, row(12), _snowman, 20, new TranslatableText("mco.terms.buttons.agree"), _snowmanx -> this.agreedToTos()));
      this.addButton(
         new ButtonWidget(this.width / 2 + 4, row(12), _snowman, 20, new TranslatableText("mco.terms.buttons.disagree"), _snowmanx -> this.client.openScreen(this.parent))
      );
   }
         ...
         this.client
            .openScreen(
               new RealmsLongRunningMcoTaskScreen(
                  this.parent, new RealmsGetServerDetailsTask(this.mainScreen, this.parent, this.realmsServer, new ReentrantLock())
               )
            );
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      int _snowman = this.textRenderer.getWidth(field_26524);
      int _snowmanx = this.width / 2 - 121 + _snowman;
      int _snowmanxx = row(5);
      int _snowmanxxx = _snowmanx + this.textRenderer.getWidth(field_26525) + 1;
      int _snowmanxxxx = _snowmanxx + 1 + 9;
      this.onLink = _snowmanx <= mouseX && mouseX <= _snowmanxxx && _snowmanxx <= mouseY && mouseY <= _snowmanxxxx;
      this.textRenderer.draw(matrices, field_26525, (float)(this.width / 2 - 120 + _snowman), (float)row(5), this.onLink ? 7107012 : 3368635);
