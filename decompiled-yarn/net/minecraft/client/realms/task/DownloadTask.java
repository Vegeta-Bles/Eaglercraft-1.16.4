   ...
   public void run() {
      ...
      while (_snowmanx < 25) {
         try {
            ...
            WorldDownload _snowmanxx = _snowman.download(this.worldId, this.slot);
            pause(1);
            if (this.aborted()) {
               return;
            }

            setScreen(new RealmsDownloadLatestWorldScreen(this.lastScreen, _snowmanxx, this.downloadName, _snowmanxxx -> {
            }));
            return;
