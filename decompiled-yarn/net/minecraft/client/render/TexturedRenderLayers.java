...
public class TexturedRenderLayers {
   ...
   public static final List<SpriteIdentifier> COLORED_SHULKER_BOXES_TEXTURES = Stream.of(
         "white", "orange", "magenta", "light_blue", "yellow", "lime", "pink", "gray", "light_gray", "cyan", "purple", "blue", "brown", "green", "red", "black"
      )
      .map(_snowman -> new SpriteIdentifier(SHULKER_BOXES_ATLAS_TEXTURE, new Identifier("entity/shulker/shulker_" + _snowman)))
      .collect(ImmutableList.toImmutableList());
   public static final Map<SignType, SpriteIdentifier> WOOD_TYPE_TEXTURES = SignType.stream()
      .collect(Collectors.toMap(Function.identity(), TexturedRenderLayers::getSignTextureId));
   public static final SpriteIdentifier[] BED_TEXTURES = Arrays.stream(DyeColor.values())
      .sorted(Comparator.comparingInt(DyeColor::getId))
      .map(_snowman -> new SpriteIdentifier(BEDS_ATLAS_TEXTURE, new Identifier("entity/bed/" + _snowman.getName())))
   ...
   public static void addDefaultTextures(Consumer<SpriteIdentifier> adder) {
      ...
      for (BannerPattern _snowman : BannerPattern.values()) {
         adder.accept(new SpriteIdentifier(BANNER_PATTERNS_ATLAS_TEXTURE, _snowman.getSpriteId(true)));
         adder.accept(new SpriteIdentifier(SHIELD_PATTERNS_ATLAS_TEXTURE, _snowman.getSpriteId(false)));
      }

      WOOD_TYPE_TEXTURES.values().forEach(adder);

      for (SpriteIdentifier _snowman : BED_TEXTURES) {
         adder.accept(_snowman);
      }
