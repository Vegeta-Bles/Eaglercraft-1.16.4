...
public class DebugRenderer {
   ...
   public static Optional<Entity> getTargetedEntity(@Nullable Entity entity, int maxDistance) {
      if (entity == null) {
         ...
         Vec3d _snowman = entity.getCameraPosVec(1.0F);
         Vec3d _snowmanx = entity.getRotationVec(1.0F).multiply((double)maxDistance);
         Vec3d _snowmanxx = _snowman.add(_snowmanx);
         Box _snowmanxxx = entity.getBoundingBox().stretch(_snowmanx).expand(1.0);
         int _snowmanxxxx = maxDistance * maxDistance;
         Predicate<Entity> _snowmanxxxxx = _snowmanxxxxxx -> !_snowmanxxxxxx.isSpectator() && _snowmanxxxxxx.collides();
         EntityHitResult _snowmanxxxxxx = ProjectileUtil.raycast(entity, _snowman, _snowmanxx, _snowmanxxx, _snowmanxxxxx, (double)_snowmanxxxx);
         if (_snowmanxxxxxx == null) {
            return Optional.empty();
         } else {
            return _snowman.squaredDistanceTo(_snowmanxxxxxx.getPos()) > (double)_snowmanxxxx ? Optional.empty() : Optional.of(_snowmanxxxxxx.getEntity());
         }
      }
   }
   ...
   public static void drawBox(BlockPos pos1, BlockPos pos2, float red, float green, float blue, float alpha) {
      ...
      if (_snowman.isReady()) {
         Vec3d _snowmanx = _snowman.getPos().negate();
         Box _snowmanxx = new Box(pos1, pos2).offset(_snowmanx);
         drawBox(_snowmanxx, red, green, blue, alpha);
      }
   }
   ...
   public static void drawBox(BlockPos pos, float expand, float red, float green, float blue, float alpha) {
      ...
      if (_snowman.isReady()) {
         Vec3d _snowmanx = _snowman.getPos().negate();
         Box _snowmanxx = new Box(pos).offset(_snowmanx).expand((double)expand);
         drawBox(_snowmanxx, red, green, blue, alpha);
      }
   }
   ...
   public static void drawBox(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, float red, float green, float blue, float alpha) {
      ...
      WorldRenderer.drawBox(_snowmanx, minX, minY, minZ, maxX, maxY, maxZ, red, green, blue, alpha);
      _snowman.draw();
   }
   ...
   public static void drawString(
      ...
      Camera _snowmanx = _snowman.gameRenderer.getCamera();
      if (_snowmanx.isReady() && _snowman.getEntityRenderDispatcher().gameOptions != null) {
         TextRenderer _snowmanxx = _snowman.textRenderer;
         double _snowmanxxx = _snowmanx.getPos().x;
         double _snowmanxxxx = _snowmanx.getPos().y;
         double _snowmanxxxxx = _snowmanx.getPos().z;
         RenderSystem.pushMatrix();
         RenderSystem.translatef((float)(x - _snowmanxxx), (float)(y - _snowmanxxxx) + 0.07F, (float)(z - _snowmanxxxxx));
         RenderSystem.normal3f(0.0F, 1.0F, 0.0F);
         RenderSystem.multMatrix(new Matrix4f(_snowmanx.getRotation()));
         ...
         float _snowmanxxxxxx = center ? (float)(-_snowmanxx.getWidth(string)) / 2.0F : 0.0F;
         _snowmanxxxxxx -= offset / size;
         RenderSystem.enableAlphaTest();
         VertexConsumerProvider.Immediate _snowmanxxxxxxx = VertexConsumerProvider.immediate(Tessellator.getInstance().getBuffer());
         _snowmanxx.draw(string, _snowmanxxxxxx, 0.0F, color, false, AffineTransformation.identity().getMatrix(), _snowmanxxxxxxx, visibleThroughObjects, 0, 15728880);
