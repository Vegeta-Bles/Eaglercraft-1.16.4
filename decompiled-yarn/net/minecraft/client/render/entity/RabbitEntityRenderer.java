...
public class RabbitEntityRenderer extends MobEntityRenderer<RabbitEntity, RabbitEntityModel<RabbitEntity>> {
   ...
   public RabbitEntityRenderer(EntityRenderDispatcher _snowman) {
      super(_snowman, new RabbitEntityModel<>(), 0.3F);
   }

   public Identifier getTexture(RabbitEntity _snowman) {
      String _snowmanx = Formatting.strip(_snowman.getName().getString());
      if (_snowmanx != null && "Toast".equals(_snowmanx)) {
         return TOAST_TEXTURE;
      } else {
         switch (_snowman.getRabbitType()) {
            case 0:
            default:
