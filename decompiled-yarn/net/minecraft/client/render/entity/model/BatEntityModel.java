...
public class BatEntityModel extends CompositeEntityModel<BatEntity> {
   ...
   public BatEntityModel() {
      ...
      ModelPart _snowman = new ModelPart(this, 24, 0);
      _snowman.addCuboid(-4.0F, -6.0F, -2.0F, 3.0F, 4.0F, 1.0F);
      this.head.addChild(_snowman);
      ModelPart _snowmanx = new ModelPart(this, 24, 0);
      _snowmanx.mirror = true;
      _snowmanx.addCuboid(1.0F, -6.0F, -2.0F, 3.0F, 4.0F, 1.0F);
   ...
   public void setAngles(BatEntity _snowman, float _snowman, float _snowman, float _snowman, float _snowman, float _snowman) {
      if (_snowman.isRoosting()) {
         this.head.pitch = _snowman * (float) (Math.PI / 180.0);
         this.head.yaw = (float) Math.PI - _snowman * (float) (Math.PI / 180.0);
         ...
         this.body.pitch = (float) (Math.PI / 4) + MathHelper.cos(_snowman * 0.1F) * 0.15F;
         this.body.yaw = 0.0F;
         this.rightWing.yaw = MathHelper.cos(_snowman * 1.3F) * (float) Math.PI * 0.25F;
         this.leftWing.yaw = -this.rightWing.yaw;
