...
public class EvokerFangsEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
      float _snowman = limbAngle * 2.0F;
      if (_snowman > 1.0F) {
         _snowman = 1.0F;
      }

      _snowman = 1.0F - _snowman * _snowman * _snowman;
      this.field_3376.roll = (float) Math.PI - _snowman * 0.35F * (float) Math.PI;
      this.field_3375.roll = (float) Math.PI + _snowman * 0.35F * (float) Math.PI;
      this.field_3375.yaw = (float) Math.PI;
      float _snowmanx = (limbAngle + MathHelper.sin(limbAngle * 2.7F)) * 0.6F * 12.0F;
      this.field_3376.pivotY = 24.0F - _snowmanx;
