...
public class IronGolemEntityModel<T extends IronGolemEntity> extends CompositeEntityModel<T> {
   ...
   public void setAngles(T _snowman, float _snowman, float _snowman, float _snowman, float _snowman, float _snowman) {
      ...
      this.rightLeg.pitch = -1.5F * MathHelper.method_24504(_snowman, 13.0F) * _snowman;
      this.leftLeg.pitch = 1.5F * MathHelper.method_24504(_snowman, 13.0F) * _snowman;
      this.rightLeg.yaw = 0.0F;
      this.leftLeg.yaw = 0.0F;
   }

   public void animateModel(T _snowman, float _snowman, float _snowman, float _snowman) {
      int _snowmanxxxx = _snowman.getAttackTicksLeft();
      if (_snowmanxxxx > 0) {
         this.rightArm.pitch = -2.0F + 1.5F * MathHelper.method_24504((float)_snowmanxxxx - _snowman, 10.0F);
         this.leftArm.pitch = -2.0F + 1.5F * MathHelper.method_24504((float)_snowmanxxxx - _snowman, 10.0F);
      } else {
         int _snowmanxxxxx = _snowman.getLookingAtVillagerTicks();
         if (_snowmanxxxxx > 0) {
            this.rightArm.pitch = -0.8F + 0.025F * MathHelper.method_24504((float)_snowmanxxxxx, 70.0F);
            this.leftArm.pitch = 0.0F;
         } else {
            this.rightArm.pitch = (-0.2F + 1.5F * MathHelper.method_24504(_snowman, 13.0F)) * _snowman;
            this.leftArm.pitch = (-0.2F - 1.5F * MathHelper.method_24504(_snowman, 13.0F)) * _snowman;
         }
      }
   }
