...
public class ParrotEntityModel extends CompositeEntityModel<ParrotEntity> {
   ...
   public void setAngles(ParrotEntity _snowman, float _snowman, float _snowman, float _snowman, float _snowman, float _snowman) {
      this.setAngles(getPose(_snowman), _snowman.age, _snowman, _snowman, _snowman, _snowman, _snowman);
   }

   public void animateModel(ParrotEntity _snowman, float _snowman, float _snowman, float _snowman) {
      this.animateModel(getPose(_snowman));
   }
   ...
   public void poseOnShoulder(
      ...
      this.getParts().forEach(_snowmanxxxx -> _snowmanxxxx.render(matrices, vertexConsumer, light, overlay));
   }
   ...
   private void setAngles(ParrotEntityModel.Pose pose, int danceAngle, float limbAngle, float limbDistance, float age, float headYaw, float headPitch) {
      ...
      switch (pose) {
         case SITTING:
            break;
         case PARTY: {
            float _snowman = MathHelper.cos((float)danceAngle);
            float _snowmanx = MathHelper.sin((float)danceAngle);
            this.head.pivotX = _snowman;
            this.head.pivotY = 15.69F + _snowmanx;
            this.head.pitch = 0.0F;
            this.head.yaw = 0.0F;
            this.head.roll = MathHelper.sin((float)danceAngle) * 0.4F;
            this.torso.pivotX = _snowman;
            this.torso.pivotY = 16.5F + _snowmanx;
            this.leftWing.roll = -0.0873F - age;
            this.leftWing.pivotX = 1.5F + _snowman;
            this.leftWing.pivotY = 16.94F + _snowmanx;
            this.rightWing.roll = 0.0873F + age;
            this.rightWing.pivotX = -1.5F + _snowman;
            this.rightWing.pivotY = 16.94F + _snowmanx;
            this.tail.pivotX = _snowman;
            this.tail.pivotY = 21.07F + _snowmanx;
            break;
         }
         case STANDING:
         ...
         case ON_SHOULDER:
         default: {
            float _snowman = age * 0.3F;
            this.head.pivotY = 15.69F + _snowman;
            this.tail.pitch = 1.015F + MathHelper.cos(limbAngle * 0.6662F) * 0.3F * limbDistance;
            this.tail.pivotY = 21.07F + _snowman;
            this.torso.pivotY = 16.5F + _snowman;
            this.leftWing.roll = -0.0873F - age;
            this.leftWing.pivotY = 16.94F + _snowman;
            this.rightWing.roll = 0.0873F + age;
            this.rightWing.pivotY = 16.94F + _snowman;
            this.leftLeg.pivotY = 22.0F + _snowman;
            this.rightLeg.pivotY = 22.0F + _snowman;
         }
      }
   }
