...
public class PhantomEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public PhantomEntityModel() {
      ...
      ModelPart _snowman = new ModelPart(this, 0, 0);
      _snowman.addCuboid(-4.0F, -2.0F, -5.0F, 7.0F, 3.0F, 5.0F);
      _snowman.setPivot(0.0F, 1.0F, -7.0F);
      _snowman.pitch = 0.2F;
      this.body.addChild(_snowman);
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
      float _snowman = ((float)(entity.getEntityId() * 3) + animationProgress) * 0.13F;
      float _snowmanx = 16.0F;
      this.leftWing.roll = MathHelper.cos(_snowman) * 16.0F * (float) (Math.PI / 180.0);
      this.leftWingTip.roll = MathHelper.cos(_snowman) * 16.0F * (float) (Math.PI / 180.0);
      this.rightWing.roll = -this.leftWing.roll;
      this.rightWingTip.roll = -this.leftWingTip.roll;
      this.tail.pitch = -(5.0F + MathHelper.cos(_snowman * 2.0F) * 5.0F) * (float) (Math.PI / 180.0);
      this.lowerTail.pitch = -(5.0F + MathHelper.cos(_snowman * 2.0F) * 5.0F) * (float) (Math.PI / 180.0);
   }
}
