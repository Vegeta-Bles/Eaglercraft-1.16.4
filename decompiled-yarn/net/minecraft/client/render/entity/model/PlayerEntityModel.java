...
public class PlayerEntityModel<T extends LivingEntity> extends BipedEntityModel<T> {
   ...
   public void setAngles(T _snowman, float _snowman, float _snowman, float _snowman, float _snowman, float _snowman) {
      ...
      if (_snowman.getEquippedStack(EquipmentSlot.CHEST).isEmpty()) {
   ...
   public void setArmAngle(Arm arm, MatrixStack matrices) {
      ...
      if (this.thinArms) {
         float _snowmanx = 0.5F * (float)(arm == Arm.RIGHT ? 1 : -1);
         _snowman.pivotX += _snowmanx;
         _snowman.rotate(matrices);
         _snowman.pivotX -= _snowmanx;
      } else {
         _snowman.rotate(matrices);
      }
