...
public class RabbitEntityModel<T extends RabbitEntity> extends EntityModel<T> {
   ...
   public void render(MatrixStack matrices, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float alpha) {
      if (this.child) {
         ...
         ImmutableList.of(this.head, this.leftEar, this.rightEar, this.nose)
            .forEach(_snowmanxxxxxxxx -> _snowmanxxxxxxxx.render(matrices, vertices, light, overlay, red, green, blue, alpha));
         matrices.pop();
         matrices.push();
   ...
   public void setAngles(T _snowman, float _snowman, float _snowman, float _snowman, float _snowman, float _snowman) {
      ...
      this.nose.pitch = _snowman * (float) (Math.PI / 180.0);
      this.head.pitch = _snowman * (float) (Math.PI / 180.0);
      this.rightEar.pitch = _snowman * (float) (Math.PI / 180.0);
      this.leftEar.pitch = _snowman * (float) (Math.PI / 180.0);
      this.nose.yaw = _snowman * (float) (Math.PI / 180.0);
      ...
      this.field_3531 = MathHelper.sin(_snowman.getJumpProgress(_snowmanxxxxxx) * (float) Math.PI);
   ...
   public void animateModel(T _snowman, float _snowman, float _snowman, float _snowman) {
      ...
      this.field_3531 = MathHelper.sin(_snowman.getJumpProgress(_snowman) * (float) Math.PI);
   }
}
