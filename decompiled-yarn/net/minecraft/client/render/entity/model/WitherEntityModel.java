...
public class WitherEntityModel<T extends WitherEntity> extends CompositeEntityModel<T> {
   ...
   public WitherEntityModel(float scale) {
      ...
      _snowman.addAll(Arrays.asList(this.heads));
      _snowman.addAll(Arrays.asList(this.bodySegments));
   ...
   public void setAngles(T _snowman, float _snowman, float _snowman, float _snowman, float _snowman, float _snowman) {
      float _snowmanxxxxxx = MathHelper.cos(_snowman * 0.1F);
      this.bodySegments[1].pitch = (0.065F + 0.05F * _snowmanxxxxxx) * (float) Math.PI;
      this.bodySegments[2]
         .setPivot(-2.0F, 6.9F + MathHelper.cos(this.bodySegments[1].pitch) * 10.0F, -0.5F + MathHelper.sin(this.bodySegments[1].pitch) * 10.0F);
      this.bodySegments[2].pitch = (0.265F + 0.1F * _snowmanxxxxxx) * (float) Math.PI;
      this.heads[0].yaw = _snowman * (float) (Math.PI / 180.0);
      this.heads[0].pitch = _snowman * (float) (Math.PI / 180.0);
   }
   ...
   public void animateModel(T _snowman, float _snowman, float _snowman, float _snowman) {
      for (int _snowmanxxxx = 1; _snowmanxxxx < 3; _snowmanxxxx++) {
         this.heads[_snowmanxxxx].yaw = (_snowman.getHeadYaw(_snowmanxxxx - 1) - _snowman.bodyYaw) * (float) (Math.PI / 180.0);
         this.heads[_snowmanxxxx].pitch = _snowman.getHeadPitch(_snowmanxxxx - 1) * (float) (Math.PI / 180.0);
      }
   }
}
