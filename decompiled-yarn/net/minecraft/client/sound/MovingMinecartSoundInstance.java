...
public class MovingMinecartSoundInstance extends MovingSoundInstance {
   ...
   public void tick() {
      if (this.minecart.removed) {
         ...
         float _snowman = MathHelper.sqrt(Entity.squaredHorizontalLength(this.minecart.getVelocity()));
         if ((double)_snowman >= 0.01) {
            this.distance = MathHelper.clamp(this.distance + 0.0025F, 0.0F, 1.0F);
            this.volume = MathHelper.lerp(MathHelper.clamp(_snowman, 0.0F, 0.5F), 0.0F, 0.7F);
         } else {
            this.distance = 0.0F;
            this.volume = 0.0F;
         }
