...
public class RecipeToast implements Toast {
   ...
   public Toast.Visibility draw(MatrixStack matrices, ToastManager manager, long startTime) {
      ...
      if (this.recipes.isEmpty()) {
         ...
         Recipe<?> _snowman = this.recipes.get((int)(startTime / Math.max(1L, 5000L / (long)this.recipes.size()) % (long)this.recipes.size()));
         ItemStack _snowmanx = _snowman.getRecipeKindIcon();
         RenderSystem.pushMatrix();
         RenderSystem.scalef(0.6F, 0.6F, 1.0F);
         manager.getGame().getItemRenderer().renderInGui(_snowmanx, 3, 3);
         RenderSystem.popMatrix();
         manager.getGame().getItemRenderer().renderInGui(_snowman.getOutput(), 8, 8);
   ...
   public static void show(ToastManager manager, Recipe<?> recipes) {
      RecipeToast _snowman = manager.getToast(RecipeToast.class, TYPE);
      if (_snowman == null) {
         manager.add(new RecipeToast(recipes));
      } else {
         _snowman.addRecipes(recipes);
      }
   }
}
