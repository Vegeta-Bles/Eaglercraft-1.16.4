...
public class CraftPlanksTutorialStepHandler implements TutorialStepHandler {
   ...
   public void tick() {
      ...
      if (this.manager.getGameMode() != GameMode.SURVIVAL) {
         ...
         if (this.ticks == 1) {
            ...
            if (_snowman != null) {
               if (_snowman.inventory.contains(ItemTags.PLANKS)) {
                  this.manager.setStep(TutorialStep.NONE);
                  return;
               }

               if (hasCrafted(_snowman, ItemTags.PLANKS)) {
                  this.manager.setStep(TutorialStep.NONE);
                  return;
               }
            }
   ...
   public void onSlotUpdate(ItemStack stack) {
      ...
      if (ItemTags.PLANKS.contains(_snowman)) {
         this.manager.setStep(TutorialStep.NONE);
      }
   }
   ...
   public static boolean hasCrafted(ClientPlayerEntity player, Tag<Item> tag) {
      for (Item _snowman : tag.values()) {
         if (player.getStatHandler().getStat(Stats.CRAFTED.getOrCreateStat(_snowman)) > 0) {
            return true;
         }
      }
