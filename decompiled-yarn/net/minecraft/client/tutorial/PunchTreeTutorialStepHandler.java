...
public class PunchTreeTutorialStepHandler implements TutorialStepHandler {
   ...
   public void tick() {
      ...
      if (this.manager.getGameMode() != GameMode.SURVIVAL) {
         ...
         if (this.ticks == 1) {
            ...
            if (_snowman != null) {
               if (_snowman.inventory.contains(ItemTags.LOGS)) {
                  this.manager.setStep(TutorialStep.CRAFT_PLANKS);
                  return;
               }

               if (FindTreeTutorialStepHandler.hasBrokenTreeBlocks(_snowman)) {
                  this.manager.setStep(TutorialStep.CRAFT_PLANKS);
                  return;
               }
            }
   ...
   @Override
   public void onBlockAttacked(ClientWorld client, BlockPos pos, BlockState state, float _snowman) {
      boolean _snowmanx = state.isIn(BlockTags.LOGS);
      if (_snowmanx && _snowman > 0.0F) {
         if (this.toast != null) {
            this.toast.setProgress(_snowman);
         }
