...
public class InputUtil {
   ...
   public static InputUtil.Key fromTranslationKey(String translationKey) {
      if (InputUtil.Key.KEYS.containsKey(translationKey)) {
         ...
         for (InputUtil.Type _snowman : InputUtil.Type.values()) {
            if (translationKey.startsWith(_snowman.name)) {
               String _snowmanx = translationKey.substring(_snowman.name.length() + 1);
               return _snowman.createFromCode(Integer.parseInt(_snowmanx));
            }
         }
   ...
   public static void setMouseCallbacks(
      ...
      GLFW.glfwSetDropCallback(handle, _snowman);
   }

   public static void setCursorParameters(long handler, int _snowman, double _snowman, double _snowman) {
      GLFW.glfwSetCursorPos(handler, _snowman, _snowman);
      GLFW.glfwSetInputMode(handler, 208897, _snowman);
   }
   ...
   static {
      ...
      GLFW_RAW_MOUSE_MOTION_SUPPORTED_HANDLE = _snowmanxx;
      GLFW_RAW_MOUSE_MOTION = _snowmanxxx;
   ...
   public static final class Key {
      ...
      public boolean equals(Object other) {
         if (this == other) {
            ...
            InputUtil.Key _snowman = (InputUtil.Key)other;
            return this.code == _snowman.code && this.type == _snowman.type;
         } else {
            return false;
         }
      }
   ...
   public static enum Type {
      KEYSYM("key.keyboard", (_snowman, _snowmanx) -> {
         String _snowmanxx = GLFW.glfwGetKeyName(_snowman, -1);
         return (Text)(_snowmanxx != null ? new LiteralText(_snowmanxx) : new TranslatableText(_snowmanx));
      }),
      SCANCODE("scancode", (_snowman, _snowmanx) -> {
         String _snowmanxx = GLFW.glfwGetKeyName(-1, _snowman);
         return (Text)(_snowmanxx != null ? new LiteralText(_snowmanxx) : new TranslatableText(_snowmanx));
      }),
      MOUSE("key.mouse", (_snowman, _snowmanx) -> Language.getInstance().hasTranslation(_snowmanx) ? new TranslatableText(_snowmanx) : new TranslatableText("key.mouse", _snowman + 1));
      ...
      private static void mapKey(InputUtil.Type type, String translationKey, int keyCode) {
         InputUtil.Key _snowman = new InputUtil.Key(translationKey, type, keyCode);
         type.map.put(keyCode, _snowman);
      }
      ...
      public InputUtil.Key createFromCode(int code) {
         return (InputUtil.Key)this.map.computeIfAbsent(code, codex -> {
            ...
            String _snowmanx = this.name + "." + _snowman;
            return new InputUtil.Key(_snowmanx, this, codex);
         });
      }
