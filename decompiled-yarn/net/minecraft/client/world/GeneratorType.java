...
public abstract class GeneratorType {
   ...
   public static final GeneratorType AMPLIFIED = new GeneratorType("amplified") {
      ...
      protected ChunkGenerator getChunkGenerator(Registry<Biome> biomeRegistry, Registry<ChunkGeneratorSettings> chunkGeneratorSettingsRegistry, long seed) {
         return new NoiseChunkGenerator(
            new VanillaLayeredBiomeSource(seed, false, false, biomeRegistry),
            seed,
            () -> chunkGeneratorSettingsRegistry.getOrThrow(ChunkGeneratorSettings.AMPLIFIED)
         );
      }
   };
   ...
   private static final GeneratorType SINGLE_BIOME_CAVES = new GeneratorType("single_biome_caves") {
      ...
      public GeneratorOptions createDefaultOptions(DynamicRegistryManager.Impl registryManager, long seed, boolean generateStructures, boolean bonusChest) {
         Registry<Biome> _snowman = registryManager.get(Registry.BIOME_KEY);
         Registry<DimensionType> _snowmanx = registryManager.get(Registry.DIMENSION_TYPE_KEY);
         Registry<ChunkGeneratorSettings> _snowmanxx = registryManager.get(Registry.NOISE_SETTINGS_WORLDGEN);
         return new GeneratorOptions(
            seed,
            generateStructures,
            bonusChest,
            GeneratorOptions.method_29962(
               DimensionType.createDefaultDimensionOptions(_snowmanx, _snowman, _snowmanxx, seed),
               () -> _snowman.getOrThrow(DimensionType.OVERWORLD_CAVES_REGISTRY_KEY),
               this.getChunkGenerator(_snowman, _snowmanxx, seed)
            )
         );
      }
      ...
      protected ChunkGenerator getChunkGenerator(Registry<Biome> biomeRegistry, Registry<ChunkGeneratorSettings> chunkGeneratorSettingsRegistry, long seed) {
         return new NoiseChunkGenerator(
            new FixedBiomeSource(biomeRegistry.getOrThrow(BiomeKeys.PLAINS)),
            seed,
            () -> chunkGeneratorSettingsRegistry.getOrThrow(ChunkGeneratorSettings.CAVES)
         );
      }
   };
   ...
   protected static final Map<Optional<GeneratorType>, GeneratorType.ScreenProvider> SCREEN_PROVIDERS = ImmutableMap.of(
      ...
      (GeneratorType.ScreenProvider)(screen, generatorOptions) -> {
         ChunkGenerator _snowman = generatorOptions.getChunkGenerator();
                  ...
                  .setGeneratorOptions(
                     new GeneratorOptions(
                        ...
                        GeneratorOptions.method_28608(
                           screen.moreOptionsDialog.method_29700().get(Registry.DIMENSION_TYPE_KEY),
                           generatorOptions.getDimensions(),
                           new FlatChunkGenerator(config)
                        )
                     )
                  ),
            _snowman instanceof FlatChunkGenerator
               ? ((FlatChunkGenerator)_snowman).getConfig()
   ...
   private static GeneratorOptions createFixedBiomeOptions(
      ...
      BiomeSource _snowman = new FixedBiomeSource(biome);
      Registry<DimensionType> _snowmanx = registryManager.get(Registry.DIMENSION_TYPE_KEY);
      Registry<ChunkGeneratorSettings> _snowmanxx = registryManager.get(Registry.NOISE_SETTINGS_WORLDGEN);
      Supplier<ChunkGeneratorSettings> _snowmanxxx;
      if (type == SINGLE_BIOME_CAVES) {
         _snowmanxxx = () -> _snowman.getOrThrow(ChunkGeneratorSettings.CAVES);
      } else if (type == SINGLE_BIOME_FLOATING_ISLANDS) {
         _snowmanxxx = () -> _snowman.getOrThrow(ChunkGeneratorSettings.FLOATING_ISLANDS);
      } else {
         _snowmanxxx = () -> _snowman.getOrThrow(ChunkGeneratorSettings.OVERWORLD);
      }
      ...
      return new GeneratorOptions(
         ...
         generatorOptions.shouldGenerateStructures(),
         generatorOptions.hasBonusChest(),
         GeneratorOptions.method_28608(_snowmanx, generatorOptions.getDimensions(), new NoiseChunkGenerator(_snowman, generatorOptions.getSeed(), _snowmanxxx))
      );
   }

   private static Biome getFirstBiome(DynamicRegistryManager registryManager, GeneratorOptions options) {
      return options.getChunkGenerator()
         .getBiomeSource()
         .getBiomes()
         .stream()
         .findFirst()
   ...
   public static Optional<GeneratorType> method_29078(GeneratorOptions _snowman) {
      ChunkGenerator _snowmanx = _snowman.getChunkGenerator();
      if (_snowmanx instanceof FlatChunkGenerator) {
         return Optional.of(FLAT);
      } else {
         return _snowmanx instanceof DebugChunkGenerator ? Optional.of(DEBUG_ALL_BLOCK_STATES) : Optional.empty();
      }
   }
   ...
   public GeneratorOptions createDefaultOptions(DynamicRegistryManager.Impl registryManager, long seed, boolean generateStructures, boolean bonusChest) {
      ...
      return new GeneratorOptions(
         ...
         generateStructures,
         bonusChest,
         GeneratorOptions.method_28608(_snowmanx, DimensionType.createDefaultDimensionOptions(_snowmanx, _snowman, _snowmanxx, seed), this.getChunkGenerator(_snowman, _snowmanxx, seed))
      );
   }
