...
public class BlockDataObject implements DataCommandObject {
   ...
   public static final Function<String, DataCommand.ObjectType> TYPE_FACTORY = _snowman -> new DataCommand.ObjectType() {
         ...
         public DataCommandObject getObject(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
            BlockPos _snowman = BlockPosArgumentType.getLoadedBlockPos(context, _snowman + "Pos");
            BlockEntity _snowmanx = ((ServerCommandSource)context.getSource()).getWorld().getBlockEntity(_snowman);
            if (_snowmanx == null) {
               throw BlockDataObject.INVALID_BLOCK_EXCEPTION.create();
            } else {
               return new BlockDataObject(_snowmanx, _snowman);
            }
         }
         ...
         public ArgumentBuilder<ServerCommandSource, ?> addArgumentsToBuilder(
            ...
            return argument.then(
               CommandManager.literal("block").then((ArgumentBuilder)argumentAdder.apply(CommandManager.argument(_snowman + "Pos", BlockPosArgumentType.blockPos())))
            );
         }
      };
   ...
   public void setTag(CompoundTag tag) {
      ...
      BlockState _snowman = this.blockEntity.getWorld().getBlockState(this.pos);
      this.blockEntity.fromTag(_snowman, tag);
      this.blockEntity.markDirty();
      this.blockEntity.getWorld().updateListeners(this.pos, _snowman, _snowman, 3);
   }
   ...
   public Text feedbackGet(NbtPathArgumentType.NbtPath _snowman, double scale, int result) {
      return new TranslatableText(
         "commands.data.block.get", _snowman, this.pos.getX(), this.pos.getY(), this.pos.getZ(), String.format(Locale.ROOT, "%.2f", scale), result
      );
   }
}
