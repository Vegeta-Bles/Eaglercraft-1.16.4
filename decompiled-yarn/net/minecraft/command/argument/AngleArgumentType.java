...
public class AngleArgumentType implements ArgumentType<AngleArgumentType.Angle> {
   ...
   public AngleArgumentType.Angle parse(StringReader _snowman) throws CommandSyntaxException {
      if (!_snowman.canRead()) {
         throw INCOMPLETE_ANGLE_EXCEPTION.createWithContext(_snowman);
      } else {
         boolean _snowmanx = CoordinateArgument.isRelative(_snowman);
         float _snowmanxx = _snowman.canRead() && _snowman.peek() != ' ' ? _snowman.readFloat() : 0.0F;
         return new AngleArgumentType.Angle(_snowmanxx, _snowmanx);
      }
   }
