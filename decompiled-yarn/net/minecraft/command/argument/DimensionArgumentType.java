...
public class DimensionArgumentType implements ArgumentType<Identifier> {
   private static final Collection<String> EXAMPLES = Stream.of(World.OVERWORLD, World.NETHER).map(_snowman -> _snowman.getValue().toString()).collect(Collectors.toList());
   private static final DynamicCommandExceptionType INVALID_DIMENSION_EXCEPTION = new DynamicCommandExceptionType(
      _snowman -> new TranslatableText("argument.dimension.invalid", _snowman)
   );
   ...
   public static ServerWorld getDimensionArgument(CommandContext<ServerCommandSource> context, String name) throws CommandSyntaxException {
      Identifier _snowman = (Identifier)context.getArgument(name, Identifier.class);
      RegistryKey<World> _snowmanx = RegistryKey.of(Registry.DIMENSION, _snowman);
      ServerWorld _snowmanxx = ((ServerCommandSource)context.getSource()).getMinecraftServer().getWorld(_snowmanx);
      if (_snowmanxx == null) {
         throw INVALID_DIMENSION_EXCEPTION.create(_snowman);
      } else {
         return _snowmanxx;
      }
   }
