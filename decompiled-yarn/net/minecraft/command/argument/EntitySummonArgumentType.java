...
public class EntitySummonArgumentType implements ArgumentType<Identifier> {
   ...
   public static final DynamicCommandExceptionType NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType(_snowman -> new TranslatableText("entity.notFound", _snowman));
   ...
   private static Identifier validate(Identifier _snowman) throws CommandSyntaxException {
      Registry.ENTITY_TYPE.getOrEmpty(_snowman).filter(EntityType::isSummonable).orElseThrow(() -> NOT_FOUND_EXCEPTION.create(_snowman));
      return _snowman;
   }

   public Identifier parse(StringReader _snowman) throws CommandSyntaxException {
      return validate(Identifier.fromCommandInput(_snowman));
   }
