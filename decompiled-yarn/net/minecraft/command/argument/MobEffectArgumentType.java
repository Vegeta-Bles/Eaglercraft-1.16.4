...
public class MobEffectArgumentType implements ArgumentType<StatusEffect> {
   ...
   public static final DynamicCommandExceptionType INVALID_EFFECT_EXCEPTION = new DynamicCommandExceptionType(
      _snowman -> new TranslatableText("effect.effectNotFound", _snowman)
   );
   ...
   public static StatusEffect getMobEffect(CommandContext<ServerCommandSource> _snowman, String _snowman) throws CommandSyntaxException {
      return (StatusEffect)_snowman.getArgument(_snowman, StatusEffect.class);
   }

   public StatusEffect parse(StringReader _snowman) throws CommandSyntaxException {
      Identifier _snowmanx = Identifier.fromCommandInput(_snowman);
      return Registry.STATUS_EFFECT.getOrEmpty(_snowmanx).orElseThrow(() -> INVALID_EFFECT_EXCEPTION.create(_snowman));
   }
