...
public class ParticleArgumentType implements ArgumentType<ParticleEffect> {
   ...
   public static final DynamicCommandExceptionType UNKNOWN_PARTICLE_EXCEPTION = new DynamicCommandExceptionType(
      _snowman -> new TranslatableText("particle.notFound", _snowman)
   );

   public ParticleArgumentType() {
   }
   ...
   public ParticleEffect parse(StringReader _snowman) throws CommandSyntaxException {
      return readParameters(_snowman);
   }
   ...
   public static ParticleEffect readParameters(StringReader reader) throws CommandSyntaxException {
      Identifier _snowman = Identifier.fromCommandInput(reader);
      ParticleType<?> _snowmanx = Registry.PARTICLE_TYPE.getOrEmpty(_snowman).orElseThrow(() -> UNKNOWN_PARTICLE_EXCEPTION.create(_snowman));
      return readParameters(reader, (ParticleType<ParticleEffect>)_snowmanx);
   }
