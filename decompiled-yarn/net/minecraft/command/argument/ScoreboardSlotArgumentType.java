...
public class ScoreboardSlotArgumentType implements ArgumentType<Integer> {
   ...
   public static final DynamicCommandExceptionType INVALID_SLOT_EXCEPTION = new DynamicCommandExceptionType(
      _snowman -> new TranslatableText("argument.scoreboardDisplaySlot.invalid", _snowman)
   );
   ...
   public Integer parse(StringReader _snowman) throws CommandSyntaxException {
      ...
      int _snowmanxx = Scoreboard.getDisplaySlotId(_snowmanx);
      if (_snowmanxx == -1) {
         throw INVALID_SLOT_EXCEPTION.create(_snowmanx);
      } else {
         return _snowmanxx;
      }
   }
