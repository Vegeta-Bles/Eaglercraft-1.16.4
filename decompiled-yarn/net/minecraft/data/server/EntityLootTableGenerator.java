...
public class EntityLootTableGenerator implements Consumer<BiConsumer<Identifier, LootTable.Builder>> {
   ...
   private static LootTable.Builder createForSheep(ItemConvertible _snowman) {
      return LootTable.builder()
         .pool(LootPool.builder().rolls(ConstantLootTableRange.create(1)).with(ItemEntry.builder(_snowman)))
   ...
   public void accept(BiConsumer<Identifier, LootTable.Builder> _snowman) {
         ...
         LootTable.builder()
            ...
            .pool(
               LootPool.builder()
                  ...
                  .conditionally(
                     EntityPropertiesLootCondition.builder(LootContext.EntityTarget.KILLER, EntityPredicate.Builder.create().type(EntityTypeTags.SKELETONS))
                  )
            )
      );
      this.register(
      ...
      for (EntityType<?> _snowmanxx : Registry.ENTITY_TYPE) {
         Identifier _snowmanxxx = _snowmanxx.getLootTableId();
         if (!ENTITY_TYPES_IN_MISC_GROUP_TO_CHECK.contains(_snowmanxx) && _snowmanxx.getSpawnGroup() == SpawnGroup.MISC) {
            if (_snowmanxxx != LootTables.EMPTY && this.lootTables.remove(_snowmanxxx) != null) {
               throw new IllegalStateException(
                  String.format("Weird loottable '%s' for '%s', not a LivingEntity so should not have loot", _snowmanxxx, Registry.ENTITY_TYPE.getId(_snowmanxx))
               );
            }
         } else if (_snowmanxxx != LootTables.EMPTY && _snowmanx.add(_snowmanxxx)) {
            LootTable.Builder _snowmanxxxx = this.lootTables.remove(_snowmanxxx);
            if (_snowmanxxxx == null) {
               throw new IllegalStateException(String.format("Missing loottable '%s' for '%s'", _snowmanxxx, Registry.ENTITY_TYPE.getId(_snowmanxx)));
            }

            _snowman.accept(_snowmanxxx, _snowmanxxxx);
         }
      }

      this.lootTables.forEach(_snowman::accept);
   }

   private void register(EntityType<?> _snowman, LootTable.Builder _snowman) {
      this.register(_snowman.getLootTableId(), _snowman);
   }

   private void register(Identifier _snowman, LootTable.Builder _snowman) {
      this.lootTables.put(_snowman, _snowman);
   }
}
