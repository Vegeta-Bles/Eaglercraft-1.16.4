...
public class ItemTagsProvider extends AbstractTagProvider<Item> {
   ...
   public ItemTagsProvider(DataGenerator _snowman, BlockTagsProvider _snowman) {
      super(_snowman, Registry.ITEM);
      this.field_23783 = _snowman::method_27169;
   }
   ...
   protected void copy(Tag.Identified<Block> _snowman, Tag.Identified<Item> _snowman) {
      Tag.Builder _snowmanxx = this.method_27169(_snowman);
      Tag.Builder _snowmanxxx = this.field_23783.apply(_snowman);
      _snowmanxxx.streamEntries().forEach(_snowmanxx::add);
   }

   @Override
   protected Path getOutput(Identifier _snowman) {
      return this.root.getOutput().resolve("data/" + _snowman.getNamespace() + "/tags/items/" + _snowman.getPath() + ".json");
   }
