...
public class ShapedRecipeJsonFactory {
   ...
   public void offerTo(Consumer<RecipeJsonProvider> exporter, String recipeIdStr) {
      Identifier _snowman = Registry.ITEM.getId(this.output);
      if (new Identifier(recipeIdStr).equals(_snowman)) {
   ...
   private void validate(Identifier recipeId) {
      if (this.pattern.isEmpty()) {
         ...
         Set<Character> _snowman = Sets.newHashSet(this.inputs.keySet());
         _snowman.remove(' ');

         for (String _snowmanx : this.pattern) {
            for (int _snowmanxx = 0; _snowmanxx < _snowmanx.length(); _snowmanxx++) {
               char _snowmanxxx = _snowmanx.charAt(_snowmanxx);
               if (!this.inputs.containsKey(_snowmanxxx) && _snowmanxxx != ' ') {
                  throw new IllegalStateException("Pattern in recipe " + recipeId + " uses undefined symbol '" + _snowmanxxx + "'");
               }
   ...
   class ShapedRecipeJsonProvider implements RecipeJsonProvider {
      ...
      public void serialize(JsonObject json) {
         ...
         json.add("pattern", _snowman);
         JsonObject _snowmanx = new JsonObject();

         for (Entry<Character, Ingredient> _snowmanxx : this.inputs.entrySet()) {
            _snowmanx.add(String.valueOf(_snowmanxx.getKey()), _snowmanxx.getValue().toJson());
         }

         json.add("key", _snowmanx);
         JsonObject _snowmanxx = new JsonObject();
         _snowmanxx.addProperty("item", Registry.ITEM.getId(this.output).toString());
         if (this.resultCount > 1) {
            _snowmanxx.addProperty("count", this.resultCount);
         }

         json.add("result", _snowmanxx);
      }
