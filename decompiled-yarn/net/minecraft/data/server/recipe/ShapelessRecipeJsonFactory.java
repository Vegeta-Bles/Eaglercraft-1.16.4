...
public class ShapelessRecipeJsonFactory {
   ...
   public static class ShapelessRecipeJsonProvider implements RecipeJsonProvider {
      ...
      public void serialize(JsonObject json) {
         ...
         for (Ingredient _snowmanx : this.inputs) {
            _snowman.add(_snowmanx.toJson());
         }

         json.add("ingredients", _snowman);
         JsonObject _snowmanx = new JsonObject();
         _snowmanx.addProperty("item", Registry.ITEM.getId(this.output).toString());
         if (this.count > 1) {
            _snowmanx.addProperty("count", this.count);
         }

         json.add("result", _snowmanx);
      }
