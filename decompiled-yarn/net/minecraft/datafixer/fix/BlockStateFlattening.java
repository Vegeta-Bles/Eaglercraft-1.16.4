...
public class BlockStateFlattening {
   ...
   private static final Object2IntMap<Dynamic<?>> OLD_STATE_TO_ID = (Object2IntMap<Dynamic<?>>)DataFixUtils.make(
      new Object2IntOpenHashMap(), _snowman -> _snowman.defaultReturnValue(-1)
   );
   ...
   private static void putStates(int oldId, String newStateStr, String... oldStateStrings) {
      Dynamic<?> _snowman = parseState(newStateStr);
      field_24645[oldId] = _snowman;
      int _snowmanx = oldId >> 4;
      if (field_24646[_snowmanx] == null) {
         field_24646[_snowmanx] = _snowman;
      }

      for (String _snowmanxx : oldStateStrings) {
         Dynamic<?> _snowmanxxx = parseState(_snowmanxx);
         String _snowmanxxxx = _snowmanxxx.get("Name").asString("");
         OLD_BLOCK_TO_ID.putIfAbsent(_snowmanxxxx, oldId);
         OLD_STATE_TO_ID.put(_snowmanxxx, oldId);
      }
   }

   private static void fillEmptyStates() {
      for (int _snowman = 0; _snowman < field_24645.length; _snowman++) {
         if (field_24645[_snowman] == null) {
            field_24645[_snowman] = field_24646[_snowman >> 4];
         }
      }
   }

   public static Dynamic<?> lookupState(Dynamic<?> _snowman) {
      int _snowmanx = OLD_STATE_TO_ID.getInt(_snowman);
      if (_snowmanx >= 0 && _snowmanx < field_24645.length) {
         Dynamic<?> _snowmanxx = field_24645[_snowmanx];
         return _snowmanxx == null ? _snowman : _snowmanxx;
      } else {
         return _snowman;
      }
   }
   ...
   public static String lookupBlock(String oldBlockName) {
      int _snowman = OLD_BLOCK_TO_ID.getInt(oldBlockName);
      if (_snowman >= 0 && _snowman < field_24645.length) {
         Dynamic<?> _snowmanx = field_24645[_snowman];
         return _snowmanx == null ? oldBlockName : _snowmanx.get("Name").asString("");
      } else {
         return oldBlockName;
      }
   }
   ...
   public static String lookupStateBlock(int stateId) {
      if (stateId >= 0 && stateId < field_24645.length) {
         Dynamic<?> _snowman = field_24645[stateId];
         return _snowman == null ? "minecraft:air" : _snowman.get("Name").asString("");
      } else {
   ...
   public static Dynamic<?> lookupState(int stateId) {
      Dynamic<?> _snowman = null;
      if (stateId >= 0 && stateId < field_24645.length) {
         _snowman = field_24645[stateId];
      }

      return _snowman == null ? field_24645[0] : _snowman;
   }
