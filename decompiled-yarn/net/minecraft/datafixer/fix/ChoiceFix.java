...
public abstract class ChoiceFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      OpticFinder<?> _snowman = DSL.namedChoice(this.choiceName, this.getInputSchema().getChoiceType(this.type, this.choiceName));
      return this.fixTypeEverywhereTyped(
         this.name,
         this.getInputSchema().getType(this.type),
         this.getOutputSchema().getType(this.type),
         _snowmanx -> _snowmanx.updateTyped(_snowman, this.getOutputSchema().getChoiceType(this.type, this.choiceName), this::transform)
      );
   }
