...
public class ChunkStructuresTemplateRenameFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      OpticFinder<?> _snowmanx = _snowman.findField("Children");
      return this.fixTypeEverywhereTyped(
         "ChunkStructuresTemplateRenameFix",
         _snowman,
         _snowmanxx -> _snowmanxx.updateTyped(_snowman, _snowmanxxx -> _snowmanxxx.update(DSL.remainderFinder(), _snowmanxxxx -> this.method_15671((Dynamic<?>)_snowmanx.get(DSL.remainderFinder()), _snowmanxxxx)))
      );
   }

   private Dynamic<?> method_15671(Dynamic<?> _snowman, Dynamic<?> _snowman) {
      String _snowmanxx = _snowman.get("id").asString("");
      if (structures.containsKey(_snowmanxx)) {
         Pair<String, ImmutableMap<String, String>> _snowmanxxx = (Pair<String, ImmutableMap<String, String>>)structures.get(_snowmanxx);
         if (((String)_snowmanxxx.getFirst()).equals(_snowman.get("id").asString(""))) {
            String _snowmanxxxx = _snowman.get("Template").asString("");
            _snowman = _snowman.set("Template", _snowman.createString((String)((ImmutableMap)_snowmanxxx.getSecond()).getOrDefault(_snowmanxxxx, _snowmanxxxx)));
         }
      }
