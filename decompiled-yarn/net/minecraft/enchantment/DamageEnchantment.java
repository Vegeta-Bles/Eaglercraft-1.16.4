...
public class DamageEnchantment extends Enchantment {
   ...
   public void onTargetDamaged(LivingEntity user, Entity target, int level) {
      if (target instanceof LivingEntity) {
         LivingEntity _snowman = (LivingEntity)target;
         if (this.typeIndex == 2 && _snowman.getGroup() == EntityGroup.ARTHROPOD) {
            int _snowmanx = 20 + user.getRandom().nextInt(10 * level);
            _snowman.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, _snowmanx, 3));
         }
      }
   }
}
