...
public abstract class Enchantment {
   ...
   public Map<EquipmentSlot, ItemStack> getEquipment(LivingEntity entity) {
      Map<EquipmentSlot, ItemStack> _snowman = Maps.newEnumMap(EquipmentSlot.class);

      for (EquipmentSlot _snowmanx : this.slotTypes) {
         ItemStack _snowmanxx = entity.getEquippedStack(_snowmanx);
   ...
   public Text getName(int level) {
      MutableText _snowman = new TranslatableText(this.getTranslationKey());
      if (this.isCursed()) {
         _snowman.formatted(Formatting.RED);
      } else {
         _snowman.formatted(Formatting.GRAY);
      }

      if (level != 1 || this.getMaxLevel() != 1) {
         _snowman.append(" ").append(new TranslatableText("enchantment.level." + level));
      }
