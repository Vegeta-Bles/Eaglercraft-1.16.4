...
public class ProtectionEnchantment extends Enchantment {
   ...
   public ProtectionEnchantment(Enchantment.Rarity weight, ProtectionEnchantment.Type _snowman, EquipmentSlot... _snowman) {
      super(weight, _snowman == ProtectionEnchantment.Type.FALL ? EnchantmentTarget.ARMOR_FEET : EnchantmentTarget.ARMOR, _snowman);
      this.protectionType = _snowman;
   }
   ...
   public boolean canAccept(Enchantment other) {
      if (other instanceof ProtectionEnchantment) {
         ProtectionEnchantment _snowman = (ProtectionEnchantment)other;
         return this.protectionType == _snowman.protectionType
            ? false
            : this.protectionType == ProtectionEnchantment.Type.FALL || _snowman.protectionType == ProtectionEnchantment.Type.FALL;
      } else {
   ...
   public static int transformFireDuration(LivingEntity entity, int duration) {
      int _snowman = EnchantmentHelper.getEquipmentLevel(Enchantments.FIRE_PROTECTION, entity);
      if (_snowman > 0) {
         duration -= MathHelper.floor((float)duration * (float)_snowman * 0.15F);
      }
   ...
   public static double transformExplosionKnockback(LivingEntity entity, double velocity) {
      int _snowman = EnchantmentHelper.getEquipmentLevel(Enchantments.BLAST_PROTECTION, entity);
      if (_snowman > 0) {
         velocity -= (double)MathHelper.floor(velocity * (double)((float)_snowman * 0.15F));
      }
