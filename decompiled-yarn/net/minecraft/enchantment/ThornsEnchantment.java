...
public class ThornsEnchantment extends Enchantment {
   ...
   public void onUserDamaged(LivingEntity user, Entity attacker, int level) {
      Random _snowman = user.getRandom();
      Entry<EquipmentSlot, ItemStack> _snowmanx = EnchantmentHelper.chooseEquipmentWith(Enchantments.THORNS, user);
      if (shouldDamageAttacker(level, _snowman)) {
         if (attacker != null) {
            attacker.damage(DamageSource.thorns(user), (float)getDamageAmount(level, _snowman));
         }

         if (_snowmanx != null) {
            _snowmanx.getValue().damage(2, user, _snowmanxx -> _snowmanxx.sendEquipmentBreakStatus(_snowman.getKey()));
         }
      }
   }
