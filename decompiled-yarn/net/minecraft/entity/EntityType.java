...
public class EntityType<T extends Entity> {
   ...
   public static final EntityType<ChestMinecartEntity> CHEST_MINECART = register(
      "chest_minecart",
      EntityType.Builder.<ChestMinecartEntity>create(ChestMinecartEntity::new, SpawnGroup.MISC).setDimensions(0.98F, 0.7F).maxTrackingRange(8)
   );
   ...
   public static final EntityType<ZombifiedPiglinEntity> ZOMBIFIED_PIGLIN = register(
      "zombified_piglin",
      EntityType.Builder.create(ZombifiedPiglinEntity::new, SpawnGroup.MONSTER).makeFireImmune().setDimensions(0.6F, 1.95F).maxTrackingRange(8)
   );
   ...
   public EntityType(
      ...
      ImmutableSet<Block> spawnBlocks,
      ...
      this.canSpawnInside = spawnBlocks;
   ...
   public Entity spawnFromItemStack(
      ServerWorld _snowman, @Nullable ItemStack stack, @Nullable PlayerEntity player, BlockPos pos, SpawnReason spawnReason, boolean alignPosition, boolean invertY
   ) {
      return this.spawn(
         _snowman,
   ...
   public T spawn(
      ...
      T _snowmanx = this.create(_snowman, itemTag, name, player, pos, spawnReason, alignPosition, invertY);
      if (_snowmanx != null) {
   ...
   public T create(
      ...
      T _snowmanx = this.create(_snowman);
      if (_snowmanx == null) {
         return null;
      } else {
         double _snowmanxx;
         if (alignPosition) {
            _snowmanx.updatePosition((double)pos.getX() + 0.5, (double)(pos.getY() + 1), (double)pos.getZ() + 0.5);
            _snowmanxx = getOriginY(_snowman, pos, invertY, _snowmanx.getBoundingBox());
         } else {
            _snowmanxx = 0.0;
         }

         _snowmanx.refreshPositionAndAngles(
            (double)pos.getX() + 0.5, (double)pos.getY() + _snowmanxx, (double)pos.getZ() + 0.5, MathHelper.wrapDegrees(_snowman.random.nextFloat() * 360.0F), 0.0F
         );
         if (_snowmanx instanceof MobEntity) {
            MobEntity _snowmanxxx = (MobEntity)_snowmanx;
            _snowmanxxx.headYaw = _snowmanxxx.yaw;
            _snowmanxxx.bodyYaw = _snowmanxxx.yaw;
            _snowmanxxx.initialize(_snowman, _snowman.getLocalDifficulty(_snowmanxxx.getBlockPos()), spawnReason, null, itemTag);
            _snowmanxxx.playAmbientSound();
         }

         if (name != null && _snowmanx instanceof LivingEntity) {
            _snowmanx.setCustomName(name);
         }

         loadFromEntityTag(_snowman, player, _snowmanx, itemTag);
         return _snowmanx;
      }
   }

   protected static double getOriginY(WorldView world, BlockPos pos, boolean invertY, Box boundingBox) {
      Box _snowman = new Box(pos);
      if (invertY) {
         _snowman = _snowman.stretch(0.0, -1.0, 0.0);
      }

      Stream<VoxelShape> _snowmanx = world.getCollisions(null, _snowman, _snowmanxx -> true);
      return 1.0 + VoxelShapes.calculateMaxOffset(Direction.Axis.Y, boundingBox, _snowmanx, invertY ? -2.0 : -1.0);
   }
   ...
   public static void loadFromEntityTag(World world, @Nullable PlayerEntity player, @Nullable Entity entity, @Nullable CompoundTag itemTag) {
      if (itemTag != null && itemTag.contains("EntityTag", 10)) {
         ...
         if (_snowman != null && entity != null) {
            if (world.isClient || !entity.entityDataRequiresOperator() || player != null && _snowman.getPlayerManager().isOperator(player.getGameProfile())) {
               CompoundTag _snowmanx = entity.toTag(new CompoundTag());
               UUID _snowmanxx = entity.getUuid();
               _snowmanx.copyFrom(itemTag.getCompound("EntityTag"));
               entity.setUuid(_snowmanxx);
               entity.fromTag(_snowmanx);
            }
         }
      }
   }
   ...
   public Identifier getLootTableId() {
      if (this.lootTableId == null) {
         Identifier _snowman = Registry.ENTITY_TYPE.getId(this);
         this.lootTableId = new Identifier(_snowman.getNamespace(), "entities/" + _snowman.getPath());
      }
   ...
   public static Optional<Entity> getEntityFromTag(CompoundTag tag, World world) {
      return Util.ifPresentOrElse(
         fromTag(tag).map(_snowmanx -> _snowmanx.create(world)), _snowmanx -> _snowmanx.fromTag(tag), () -> LOGGER.warn("Skipping Entity with id {}", tag.getString("id"))
      );
   }
   ...
   public Box createSimpleBoundingBox(double feetX, double feetY, double feetZ) {
      float _snowman = this.getWidth() / 2.0F;
      return new Box(feetX - (double)_snowman, feetY, feetZ - (double)_snowman, feetX + (double)_snowman, feetY + (double)this.getHeight(), feetZ + (double)_snowman);
   }

   public boolean isInvalidSpawn(BlockState _snowman) {
      if (this.canSpawnInside.contains(_snowman.getBlock())) {
         return false;
      } else {
         return this.fireImmune || !_snowman.isIn(BlockTags.FIRE) && !_snowman.isOf(Blocks.MAGMA_BLOCK) && !CampfireBlock.isLitCampfire(_snowman) && !_snowman.isOf(Blocks.LAVA)
            ? _snowman.isOf(Blocks.WITHER_ROSE) || _snowman.isOf(Blocks.SWEET_BERRY_BUSH) || _snowman.isOf(Blocks.CACTUS)
            : true;
      }
   }
   ...
   public static Entity loadEntityWithPassengers(CompoundTag compoundTag, World world, Function<Entity, Entity> entityProcessor) {
      return loadEntityFromTag(compoundTag, world).map(entityProcessor).map(_snowmanxxx -> {
         if (compoundTag.contains("Passengers", 9)) {
            ListTag _snowmanxxxx = compoundTag.getList("Passengers", 10);

            for (int _snowmanx = 0; _snowmanx < _snowmanxxxx.size(); _snowmanx++) {
               Entity _snowmanxx = loadEntityWithPassengers(_snowmanxxxx.getCompound(_snowmanx), world, entityProcessor);
               if (_snowmanxx != null) {
                  _snowmanxx.startRiding(_snowmanxxx, true);
               }
            }
         }

         return (Entity)_snowmanxxx;
      }).orElse(null);
   }
   ...
   public static class Builder<T extends Entity> {
      ...
      public static <T extends Entity> EntityType.Builder<T> create(SpawnGroup spawnGroup) {
         return new EntityType.Builder<>((_snowman, _snowmanx) -> null, spawnGroup);
      }
      ...
      public EntityType.Builder<T> allowSpawningInside(Block... _snowman) {
         this.canSpawnInside = ImmutableSet.copyOf(_snowman);
         return this;
      }
