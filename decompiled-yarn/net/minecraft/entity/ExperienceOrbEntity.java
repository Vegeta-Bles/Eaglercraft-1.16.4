...
public class ExperienceOrbEntity extends Entity {
   ...
   public ExperienceOrbEntity(EntityType<? extends ExperienceOrbEntity> _snowman, World _snowman) {
   ...
   public void tick() {
      ...
      if (this.target != null) {
         Vec3d _snowmanx = new Vec3d(
            this.target.getX() - this.getX(),
            this.target.getY() + (double)this.target.getStandingEyeHeight() / 2.0 - this.getY(),
            this.target.getZ() - this.getZ()
         );
         double _snowmanxx = _snowmanx.lengthSquared();
         if (_snowmanxx < 64.0) {
            double _snowmanxxx = 1.0 - Math.sqrt(_snowmanxx) / 8.0;
            this.setVelocity(this.getVelocity().add(_snowmanx.normalize().multiply(_snowmanxxx * _snowmanxxx * 0.1)));
         }
      }
      ...
      if (this.onGround) {
         _snowmanx = this.world.getBlockState(new BlockPos(this.getX(), this.getY() - 1.0, this.getZ())).getBlock().getSlipperiness() * 0.98F;
      }

      this.setVelocity(this.getVelocity().multiply((double)_snowmanx, 0.98, (double)_snowmanx));
      if (this.onGround) {
   ...
   private void applyWaterMovement() {
      ...
      this.setVelocity(_snowman.x * 0.99F, Math.min(_snowman.y + 5.0E-4F, 0.06F), _snowman.z * 0.99F);
   }
   ...
   public void onPlayerCollision(PlayerEntity player) {
      if (!this.world.isClient) {
         if (this.pickupDelay == 0 && player.experiencePickUpDelay == 0) {
            ...
            Entry<EquipmentSlot, ItemStack> _snowman = EnchantmentHelper.chooseEquipmentWith(Enchantments.MENDING, player, ItemStack::isDamaged);
            if (_snowman != null) {
               ItemStack _snowmanx = _snowman.getValue();
               if (!_snowmanx.isEmpty() && _snowmanx.isDamaged()) {
                  int _snowmanxx = Math.min(this.getMendingRepairAmount(this.amount), _snowmanx.getDamage());
                  this.amount = this.amount - this.getMendingRepairCost(_snowmanxx);
                  _snowmanx.setDamage(_snowmanx.getDamage() - _snowmanxx);
               }
            }
