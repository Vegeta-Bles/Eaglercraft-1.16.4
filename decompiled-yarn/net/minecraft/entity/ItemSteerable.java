...
public interface ItemSteerable {
   ...
   default boolean travel(MobEntity entity, SaddledComponent saddledEntity, Vec3d movementInput) {
      if (!entity.isAlive()) {
         ...
         Entity _snowman = entity.getPassengerList().isEmpty() ? null : entity.getPassengerList().get(0);
         if (entity.hasPassengers() && entity.canBeControlledByRider() && _snowman instanceof PlayerEntity) {
            entity.yaw = _snowman.yaw;
            entity.prevYaw = entity.yaw;
            entity.pitch = _snowman.pitch * 0.5F;
            ...
            if (entity.isLogicalSideForUpdatingMovement()) {
               float _snowmanx = this.getSaddledSpeed();
               if (saddledEntity.boosted) {
                  _snowmanx += _snowmanx * 1.15F * MathHelper.sin((float)saddledEntity.field_23216 / (float)saddledEntity.currentBoostTime * (float) Math.PI);
               }

               entity.setMovementSpeed(_snowmanx);
