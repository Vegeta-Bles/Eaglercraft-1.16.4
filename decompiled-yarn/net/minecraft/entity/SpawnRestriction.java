...
public class SpawnRestriction {
   ...
   private static <T extends MobEntity> void register(
      ...
      SpawnRestriction.Entry _snowman = RESTRICTIONS.put(type, new SpawnRestriction.Entry(heightmapType, location, predicate));
      if (_snowman != null) {
   ...
   public static SpawnRestriction.Location getLocation(EntityType<?> type) {
      SpawnRestriction.Entry _snowman = RESTRICTIONS.get(type);
      return _snowman == null ? SpawnRestriction.Location.NO_RESTRICTIONS : _snowman.location;
   }

   public static Heightmap.Type getHeightmapType(@Nullable EntityType<?> type) {
      SpawnRestriction.Entry _snowman = RESTRICTIONS.get(type);
      return _snowman == null ? Heightmap.Type.MOTION_BLOCKING_NO_LEAVES : _snowman.heightmapType;
   }

   public static <T extends Entity> boolean canSpawn(EntityType<T> type, ServerWorldAccess _snowman, SpawnReason spawnReason, BlockPos pos, Random random) {
      SpawnRestriction.Entry _snowmanx = RESTRICTIONS.get(type);
      return _snowmanx == null || _snowmanx.predicate.test(type, _snowman, spawnReason, pos, random);
   }
   ...
   public interface SpawnPredicate<T extends Entity> {
      boolean test(EntityType<T> type, ServerWorldAccess var2, SpawnReason spawnReason, BlockPos pos, Random random);
   }
}
