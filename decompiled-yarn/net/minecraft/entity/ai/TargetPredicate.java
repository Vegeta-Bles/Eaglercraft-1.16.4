      ...
      if (baseEntity == targetEntity) {
         ...
         if (baseEntity != null) {
            ...
            if (this.baseMaxDistance > 0.0) {
               double _snowman = this.useDistanceScalingFactor ? targetEntity.getAttackDistanceScalingFactor(baseEntity) : 1.0;
               double _snowmanx = Math.max(this.baseMaxDistance * _snowman, 2.0);
               double _snowmanxx = baseEntity.squaredDistanceTo(targetEntity.getX(), targetEntity.getY(), targetEntity.getZ());
               if (_snowmanxx > _snowmanx * _snowmanx) {
                  return false;
               }
            }
