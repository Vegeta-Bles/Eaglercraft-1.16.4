...
public class VillagerHostilesSensor extends Sensor<LivingEntity> {
   ...
   private Optional<LivingEntity> getNearestHostile(LivingEntity entity) {
      return this.getVisibleMobs(entity)
         .flatMap(
            _snowmanx -> _snowmanx.stream()
                  .filter(this::isHostile)
                  .filter(_snowmanxx -> this.isCloseEnoughForDanger(entity, _snowmanxx))
                  .min((_snowmanxxx, _snowmanxx) -> this.compareDistances(entity, _snowmanxxx, _snowmanxx))
         );
   }
   ...
   private boolean isCloseEnoughForDanger(LivingEntity entity, LivingEntity hostile) {
      float _snowman = (Float)SQUARED_DISTANCES_FOR_DANGER.get(hostile.getType());
      return hostile.squaredDistanceTo(entity) <= (double)(_snowman * _snowman);
   }
