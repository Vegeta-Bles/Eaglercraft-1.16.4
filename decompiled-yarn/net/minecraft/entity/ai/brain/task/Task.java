...
public abstract class Task<E extends LivingEntity> {
   ...
   public final boolean tryStarting(ServerWorld world, E entity, long time) {
      if (this.hasRequiredMemoryState(entity) && this.shouldRun(world, entity)) {
         ...
         int _snowman = this.minRunTime + world.getRandom().nextInt(this.maxRunTime + 1 - this.minRunTime);
         this.endTime = time + (long)_snowman;
   ...
   private boolean hasRequiredMemoryState(E _snowman) {
      for (Entry<MemoryModuleType<?>, MemoryModuleState> _snowmanx : this.requiredMemoryStates.entrySet()) {
         MemoryModuleType<?> _snowmanxx = _snowmanx.getKey();
         MemoryModuleState _snowmanxxx = _snowmanx.getValue();
         if (!_snowman.getBrain().isMemoryInState(_snowmanxx, _snowmanxxx)) {
            return false;
         }
      }
