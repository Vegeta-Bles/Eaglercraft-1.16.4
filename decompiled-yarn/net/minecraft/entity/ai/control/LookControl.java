...
public class LookControl {
   ...
   protected float getTargetPitch() {
      double _snowman = this.lookX - this.entity.getX();
      double _snowmanx = this.lookY - this.entity.getEyeY();
      double _snowmanxx = this.lookZ - this.entity.getZ();
      double _snowmanxxx = (double)MathHelper.sqrt(_snowman * _snowman + _snowmanxx * _snowmanxx);
      return (float)(-(MathHelper.atan2(_snowmanx, _snowmanxxx) * 180.0F / (float)Math.PI));
   }

   protected float getTargetYaw() {
      double _snowman = this.lookX - this.entity.getX();
      double _snowmanx = this.lookZ - this.entity.getZ();
      return (float)(MathHelper.atan2(_snowmanx, _snowman) * 180.0F / (float)Math.PI) - 90.0F;
   }

   protected float changeAngle(float from, float to, float max) {
      float _snowman = MathHelper.subtractAngles(from, to);
      float _snowmanx = MathHelper.clamp(_snowman, -max, max);
      return from + _snowmanx;
   }
