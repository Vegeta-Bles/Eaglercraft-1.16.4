...
public class AnimalMateGoal extends Goal {
   ...
   public AnimalMateGoal(AnimalEntity animal, double chance) {
      this(animal, chance, (Class<? extends AnimalEntity>)animal.getClass());
   }
   ...
   private AnimalEntity findMate() {
      List<AnimalEntity> _snowman = this.world.getTargets(this.entityClass, VALID_MATE_PREDICATE, this.animal, this.animal.getBoundingBox().expand(8.0));
      double _snowmanx = Double.MAX_VALUE;
      AnimalEntity _snowmanxx = null;

      for (AnimalEntity _snowmanxxx : _snowman) {
         if (this.animal.canBreedWith(_snowmanxxx) && this.animal.squaredDistanceTo(_snowmanxxx) < _snowmanx) {
            _snowmanxx = _snowmanxxx;
            _snowmanx = this.animal.squaredDistanceTo(_snowmanxxx);
         }
      }
