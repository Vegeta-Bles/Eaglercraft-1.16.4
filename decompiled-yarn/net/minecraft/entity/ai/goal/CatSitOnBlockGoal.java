...
public class CatSitOnBlockGoal extends MoveToTargetPosGoal {
   ...
   protected boolean isTargetPos(WorldView world, BlockPos pos) {
      if (!world.isAir(pos.up())) {
         ...
         if (_snowman.isOf(Blocks.CHEST)) {
            return ChestBlockEntity.getPlayersLookingInChestCount(world, pos) < 1;
         } else {
            return _snowman.isOf(Blocks.FURNACE) && _snowman.get(FurnaceBlock.LIT)
               ? true
               : _snowman.method_27851(BlockTags.BEDS, _snowmanx -> _snowmanx.method_28500(BedBlock.PART).map(_snowmanxx -> _snowmanxx != BedPart.HEAD).orElse(true));
         }
      }
   }
}
