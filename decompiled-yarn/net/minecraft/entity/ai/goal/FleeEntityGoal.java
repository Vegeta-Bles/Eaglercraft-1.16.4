...
public class FleeEntityGoal<T extends LivingEntity> extends Goal {
   ...
   public FleeEntityGoal(PathAwareEntity mob, Class<T> fleeFromType, float distance, double slowSpeed, double fastSpeed) {
      this(mob, fleeFromType, _snowman -> true, distance, slowSpeed, fastSpeed, EntityPredicates.EXCEPT_CREATIVE_OR_SPECTATOR::test);
   }
   ...
   public FleeEntityGoal(
      ...
      this(fleeingEntity, classToFleeFrom, _snowman -> true, fleeDistance, fleeSlowSpeed, fleeFastSpeed, inclusionSelector);
   }
   ...
   public boolean canStart() {
      ...
      if (this.targetEntity == null) {
         ...
         Vec3d _snowman = TargetFinder.findTargetAwayFrom(this.mob, 16, 7, this.targetEntity.getPos());
         if (_snowman == null) {
            return false;
         } else if (this.targetEntity.squaredDistanceTo(_snowman.x, _snowman.y, _snowman.z) < this.targetEntity.squaredDistanceTo(this.mob)) {
            return false;
         } else {
            this.fleePath = this.fleeingEntityNavigation.findPathTo(_snowman.x, _snowman.y, _snowman.z, 0);
