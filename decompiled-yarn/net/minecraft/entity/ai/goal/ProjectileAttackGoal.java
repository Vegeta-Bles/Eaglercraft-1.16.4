...
public class ProjectileAttackGoal extends Goal {
   ...
   @Override
   public void tick() {
      double _snowman = this.mob.squaredDistanceTo(this.target.getX(), this.target.getY(), this.target.getZ());
      boolean _snowmanx = this.mob.getVisibilityCache().canSee(this.target);
      if (_snowmanx) {
      ...
      if (!(_snowman > (double)this.squaredMaxShootRange) && this.seenTargetTicks >= 5) {
      ...
      if (--this.updateCountdownTicks == 0) {
         ...
         float _snowmanxx = MathHelper.sqrt(_snowman) / this.maxShootRange;
         float var5 = MathHelper.clamp(_snowmanxx, 0.1F, 1.0F);
         this.owner.attack(this.target, var5);
         this.updateCountdownTicks = MathHelper.floor(_snowmanxx * (float)(this.maxIntervalTicks - this.minIntervalTicks) + (float)this.minIntervalTicks);
