...
public abstract class TrackTargetGoal extends Goal {
   ...
   public boolean shouldContinue() {
      ...
      if (_snowman == null) {
         ...
         AbstractTeam _snowmanx = this.mob.getScoreboardTeam();
         AbstractTeam _snowmanxx = _snowman.getScoreboardTeam();
         if (_snowmanx != null && _snowmanxx == _snowmanx) {
            return false;
         } else {
            double _snowmanxxx = this.getFollowRange();
            if (this.mob.squaredDistanceTo(_snowman) > _snowmanxxx * _snowmanxxx) {
               return false;
            } else {
               if (this.checkVisibility) {
                  if (this.mob.getVisibilityCache().canSee(_snowman)) {
               ...
               if (_snowman instanceof PlayerEntity && ((PlayerEntity)_snowman).abilities.invulnerable) {
                  return false;
               } else {
                  this.mob.setTarget(_snowman);
                  return true;
               }
            }
         }
   ...
   private boolean canNavigateToEntity(LivingEntity entity) {
      ...
      Path _snowman = this.mob.getNavigation().findPathTo(entity, 0);
      if (_snowman == null) {
         return false;
      } else {
         PathNode _snowmanx = _snowman.getEnd();
         if (_snowmanx == null) {
            return false;
         } else {
            int _snowmanxx = _snowmanx.x - MathHelper.floor(entity.getX());
            int _snowmanxxx = _snowmanx.z - MathHelper.floor(entity.getZ());
