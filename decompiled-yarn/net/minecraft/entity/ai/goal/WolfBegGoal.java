...
public class WolfBegGoal extends Goal {
   ...
   public WolfBegGoal(WolfEntity wolf, float begDistance) {
      ...
      this.validPlayerPredicate = new TargetPredicate()
         .setBaseMaxDistance((double)begDistance)
         .includeInvulnerable()
         .includeTeammates()
   ...
   public boolean shouldContinue() {
      if (!this.begFrom.isAlive()) {
         ...
         return this.wolf.squaredDistanceTo(this.begFrom) > (double)(this.begDistance * this.begDistance)
            ? false
   ...
   private boolean isAttractive(PlayerEntity player) {
      for (Hand _snowman : Hand.values()) {
         ...
         if (this.wolf.isTamed() && _snowmanx.getItem() == Items.BONE) {
            return true;
         }

         if (this.wolf.isBreedingItem(_snowmanx)) {
            return true;
         }
      }
