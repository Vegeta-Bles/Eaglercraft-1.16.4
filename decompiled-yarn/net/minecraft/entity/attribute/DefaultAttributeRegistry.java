...
public class DefaultAttributeRegistry {
   ...
   public static void checkMissing() {
      Registry.ENTITY_TYPE
         .stream()
         .filter(_snowman -> _snowman.getSpawnGroup() != SpawnGroup.MISC)
         .filter(_snowman -> !hasDefinitionFor((EntityType<?>)_snowman))
         .map(Registry.ENTITY_TYPE::getId)
         .forEach(_snowman -> {
            if (SharedConstants.isDevelopment) {
               throw new IllegalStateException("Entity " + _snowman + " has no attributes");
            } else {
               LOGGER.error("Entity {} has no attributes", _snowman);
            }
         });
   }
}
