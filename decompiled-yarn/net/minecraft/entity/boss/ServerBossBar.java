...
public class ServerBossBar extends BossBar {
   ...
   private void sendPacket(BossBarS2CPacket.Type type) {
      if (this.visible) {
         BossBarS2CPacket _snowman = new BossBarS2CPacket(type, this);
   ...
   public void clearPlayers() {
      if (!this.players.isEmpty()) {
         for (ServerPlayerEntity _snowman : Lists.newArrayList(this.players)) {
            this.removePlayer(_snowman);
         }
      }
   }
   ...
   public void setVisible(boolean visible) {
      if (visible != this.visible) {
         ...
         for (ServerPlayerEntity _snowman : this.players) {
            _snowman.networkHandler.sendPacket(new BossBarS2CPacket(visible ? BossBarS2CPacket.Type.ADD : BossBarS2CPacket.Type.REMOVE, this));
         }
      }
   }
