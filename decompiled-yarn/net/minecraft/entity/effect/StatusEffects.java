...
public class StatusEffects {
   ...
   public static final StatusEffect HASTE = register(
      ...
      new StatusEffect(StatusEffectType.BENEFICIAL, 14270531)
         .addAttributeModifier(
            EntityAttributes.GENERIC_ATTACK_SPEED, "AF8B6E3F-3328-4C0A-AA36-5BA2BB9DBEF3", 0.1F, EntityAttributeModifier.Operation.MULTIPLY_TOTAL
         )
   );
   ...
   public static final StatusEffect BAD_OMEN = register(31, "bad_omen", new StatusEffect(StatusEffectType.NEUTRAL, 745784) {
      ...
      public void applyUpdateEffect(LivingEntity entity, int amplifier) {
         if (entity instanceof ServerPlayerEntity && !entity.isSpectator()) {
            ...
            ServerWorld _snowmanx = _snowman.getServerWorld();
            if (_snowmanx.getDifficulty() == Difficulty.PEACEFUL) {
               return;
            }

            if (_snowmanx.isNearOccupiedPointOfInterest(entity.getBlockPos())) {
               _snowmanx.getRaidManager().startRaid(_snowman);
            }
         }
      }
   });
