...
public class IllusionerEntity extends SpellcastingIllagerEntity implements RangedAttackMob {
   ...
   public IllusionerEntity(EntityType<? extends IllusionerEntity> _snowman, World _snowman) {
      super(_snowman, _snowman);
      this.experiencePoints = 5;
      this.field_7297 = new Vec3d[2][4];

      for (int _snowmanxx = 0; _snowmanxx < 4; _snowmanxx++) {
         this.field_7297[0][_snowmanxx] = Vec3d.ZERO;
         this.field_7297[1][_snowmanxx] = Vec3d.ZERO;
      }
   }
   ...
   public void tickMovement() {
      ...
      if (this.world.isClient && this.isInvisible()) {
         ...
         if (this.hurtTime == 1 || this.age % 1200 == 0) {
            ...
            for (int _snowmanxx = 0; _snowmanxx < 4; _snowmanxx++) {
               this.field_7297[0][_snowmanxx] = this.field_7297[1][_snowmanxx];
               this.field_7297[1][_snowmanxx] = new Vec3d(
            ...
            for (int _snowman = 0; _snowman < 4; _snowman++) {
               this.field_7297[0][_snowman] = this.field_7297[1][_snowman];
               this.field_7297[1][_snowman] = new Vec3d(0.0, 0.0, 0.0);
            }
         }
      }
   }
   ...
   public Vec3d[] method_7065(float _snowman) {
      if (this.field_7296 <= 0) {
         return this.field_7297[1];
      } else {
         double _snowmanx = (double)(((float)this.field_7296 - _snowman) / 3.0F);
         _snowmanx = Math.pow(_snowmanx, 0.25);
         Vec3d[] _snowmanxx = new Vec3d[4];

         for (int _snowmanxxx = 0; _snowmanxxx < 4; _snowmanxxx++) {
            _snowmanxx[_snowmanxxx] = this.field_7297[1][_snowmanxxx].multiply(1.0 - _snowmanx).add(this.field_7297[0][_snowmanxxx].multiply(_snowmanx));
         }
   ...
   public void attack(LivingEntity target, float pullProgress) {
      ...
      PersistentProjectileEntity _snowmanx = ProjectileUtil.createArrowProjectile(this, _snowman, pullProgress);
