...
public class PiglinEntity extends AbstractPiglinEntity implements CrossbowUser {
   ...
   public PiglinEntity(EntityType<? extends AbstractPiglinEntity> _snowman, World _snowman) {
   ...
   protected Brain<?> deserializeBrain(Dynamic<?> _snowman) {
      return PiglinBrain.create(this, this.createBrainProfile().deserialize(_snowman));
   }
   ...
   public ActionResult interactMob(PlayerEntity player, Hand hand) {
      ActionResult _snowman = super.interactMob(player, hand);
      if (_snowman.isAccepted()) {
         return _snowman;
      } else if (!this.world.isClient) {
         return PiglinBrain.playerInteract(this, player, hand);
      } else {
         boolean _snowmanx = PiglinBrain.isWillingToTrade(this, player.getStackInHand(hand)) && this.getActivity() != PiglinActivity.ADMIRING_ITEM;
         return _snowmanx ? ActionResult.SUCCESS : ActionResult.PASS;
      }
   }
   ...
   public void setBaby(boolean baby) {
      ...
      if (!this.world.isClient) {
         ...
         _snowman.removeModifier(BABY_SPEED_BOOST);
         if (baby) {
            _snowman.addTemporaryModifier(BABY_SPEED_BOOST);
         }
      }
   }
   ...
   public boolean damage(DamageSource source, float amount) {
      ...
      if (this.world.isClient) {
         ...
         if (_snowman && source.getAttacker() instanceof LivingEntity) {
   ...
   protected boolean method_24846(ItemStack stack) {
      ...
      return this.prefersNewEquipment(stack, _snowmanx);
   }
   ...
   protected boolean prefersNewEquipment(ItemStack newStack, ItemStack oldStack) {
      if (EnchantmentHelper.hasBindingCurse(oldStack)) {
         ...
         boolean _snowman = PiglinBrain.isGoldenItem(newStack.getItem()) || newStack.getItem() == Items.CROSSBOW;
         boolean _snowmanx = PiglinBrain.isGoldenItem(oldStack.getItem()) || oldStack.getItem() == Items.CROSSBOW;
   ...
   private Entity method_26089(Entity _snowman, int _snowman) {
      List<Entity> _snowmanxx = _snowman.getPassengerList();
      return _snowman != 1 && !_snowmanxx.isEmpty() ? this.method_26089(_snowmanxx.get(0), _snowman - 1) : _snowman;
   }

   @Override
   protected SoundEvent getAmbientSound() {
      return this.world.isClient ? null : PiglinBrain.method_30091(this).orElse(null);
   }
