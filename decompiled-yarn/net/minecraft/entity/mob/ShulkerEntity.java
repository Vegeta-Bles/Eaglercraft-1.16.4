...
public class ShulkerEntity extends GolemEntity implements Monster {
   ...
   public ShulkerEntity(EntityType<? extends ShulkerEntity> _snowman, World _snowman) {
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      ...
      if (tag.contains("APX")) {
         int _snowman = tag.getInt("APX");
         int _snowmanx = tag.getInt("APY");
         int _snowmanxx = tag.getInt("APZ");
         this.dataTracker.set(ATTACHED_BLOCK, Optional.of(new BlockPos(_snowman, _snowmanx, _snowmanxx)));
      } else {
   ...
   public void writeCustomDataToTag(CompoundTag tag) {
      ...
      BlockPos _snowman = this.getAttachedBlock();
      if (_snowman != null) {
         tag.putInt("APX", _snowman.getX());
         tag.putInt("APY", _snowman.getY());
         tag.putInt("APZ", _snowman.getZ());
      }
   }
   ...
   public void tick() {
      super.tick();
      BlockPos _snowman = this.dataTracker.get(ATTACHED_BLOCK).orElse(null);
      if (_snowman == null && !this.world.isClient) {
         _snowman = this.getBlockPos();
         this.dataTracker.set(ATTACHED_BLOCK, Optional.of(_snowman));
      }

      if (this.hasVehicle()) {
         _snowman = null;
         float _snowmanx = this.getVehicle().yaw;
         this.yaw = _snowmanx;
         this.bodyYaw = _snowmanx;
         this.prevBodyYaw = _snowmanx;
         this.teleportLerpTimer = 0;
      } else if (!this.world.isClient) {
         BlockState _snowmanx = this.world.getBlockState(_snowman);
         if (!_snowmanx.isAir()) {
            if (_snowmanx.isOf(Blocks.MOVING_PISTON)) {
               Direction _snowmanxx = _snowmanx.get(PistonBlock.FACING);
               if (this.world.isAir(_snowman.offset(_snowmanxx))) {
                  _snowman = _snowman.offset(_snowmanxx);
                  this.dataTracker.set(ATTACHED_BLOCK, Optional.of(_snowman));
               } else {
                  this.tryTeleport();
               }
            } else if (_snowmanx.isOf(Blocks.PISTON_HEAD)) {
               Direction _snowmanxx = _snowmanx.get(PistonHeadBlock.FACING);
               if (this.world.isAir(_snowman.offset(_snowmanxx))) {
                  _snowman = _snowman.offset(_snowmanxx);
                  this.dataTracker.set(ATTACHED_BLOCK, Optional.of(_snowman));
               } else {
         ...
         Direction _snowmanxx = this.getAttachedFace();
         if (!this.canStay(_snowman, _snowmanxx)) {
            Direction _snowmanxxx = this.findAttachSide(_snowman);
            if (_snowmanxxx != null) {
               this.dataTracker.set(ATTACHED_FACE, _snowmanxxx);
            } else {
               this.tryTeleport();
            }
         }
      ...
      float _snowmanxx = (float)this.getPeekAmount() * 0.01F;
      this.prevOpenProgress = this.openProgress;
      if (this.openProgress > _snowmanxx) {
         this.openProgress = MathHelper.clamp(this.openProgress - 0.05F, _snowmanxx, 1.0F);
      } else if (this.openProgress < _snowmanxx) {
         this.openProgress = MathHelper.clamp(this.openProgress + 0.05F, 0.0F, _snowmanxx);
      }
      ...
      if (_snowman != null) {
         if (this.world.isClient) {
            if (this.teleportLerpTimer > 0 && this.prevAttachedBlock != null) {
               ...
               this.prevAttachedBlock = _snowman;
            }
         }

         this.resetPosition((double)_snowman.getX() + 0.5, (double)_snowman.getY(), (double)_snowman.getZ() + 0.5);
         double _snowmanxxx = 0.5 - (double)MathHelper.sin((0.5F + this.openProgress) * (float) Math.PI) * 0.5;
         double _snowmanxxxx = 0.5 - (double)MathHelper.sin((0.5F + this.prevOpenProgress) * (float) Math.PI) * 0.5;
         Direction _snowmanxxxxx = this.getAttachedFace().getOpposite();
         this.setBoundingBox(
            new Box(this.getX() - 0.5, this.getY(), this.getZ() - 0.5, this.getX() + 0.5, this.getY() + 1.0, this.getZ() + 0.5)
               .stretch((double)_snowmanxxxxx.getOffsetX() * _snowmanxxx, (double)_snowmanxxxxx.getOffsetY() * _snowmanxxx, (double)_snowmanxxxxx.getOffsetZ() * _snowmanxxx)
         );
         double _snowmanxxxxxx = _snowmanxxx - _snowmanxxxx;
         if (_snowmanxxxxxx > 0.0) {
            List<Entity> _snowmanxxxxxxx = this.world.getOtherEntities(this, this.getBoundingBox());
            if (!_snowmanxxxxxxx.isEmpty()) {
               for (Entity _snowmanxxxxxxxx : _snowmanxxxxxxx) {
                  if (!(_snowmanxxxxxxxx instanceof ShulkerEntity) && !_snowmanxxxxxxxx.noClip) {
                     _snowmanxxxxxxxx.move(
                        MovementType.SHULKER,
                        new Vec3d(_snowmanxxxxxx * (double)_snowmanxxxxx.getOffsetX(), _snowmanxxxxxx * (double)_snowmanxxxxx.getOffsetY(), _snowmanxxxxxx * (double)_snowmanxxxxx.getOffsetZ())
                     );
                  }
               }
            }
   ...
   public void updatePosition(double x, double y, double z) {
      ...
      if (this.dataTracker != null && this.age != 0) {
         Optional<BlockPos> _snowman = this.dataTracker.get(ATTACHED_BLOCK);
         Optional<BlockPos> _snowmanx = Optional.of(new BlockPos(x, y, z));
         if (!_snowmanx.equals(_snowman)) {
            this.dataTracker.set(ATTACHED_BLOCK, _snowmanx);
   ...
   protected boolean tryTeleport() {
      if (!this.isAiDisabled() && this.isAlive()) {
         ...
         for (int _snowmanx = 0; _snowmanx < 5; _snowmanx++) {
            BlockPos _snowmanxx = _snowman.add(8 - this.random.nextInt(17), 8 - this.random.nextInt(17), 8 - this.random.nextInt(17));
            if (_snowmanxx.getY() > 0 && this.world.isAir(_snowmanxx) && this.world.getWorldBorder().contains(_snowmanxx) && this.world.isSpaceEmpty(this, new Box(_snowmanxx))) {
               Direction _snowmanxxx = this.findAttachSide(_snowmanxx);
               if (_snowmanxxx != null) {
                  this.dataTracker.set(ATTACHED_FACE, _snowmanxxx);
                  this.playSound(SoundEvents.ENTITY_SHULKER_TELEPORT, 1.0F, 1.0F);
                  this.dataTracker.set(ATTACHED_BLOCK, Optional.of(_snowmanxx));
                  this.dataTracker.set(PEEK_AMOUNT, (byte)0);
                  this.setTarget(null);
                  return true;
               }
   ...
   public DyeColor getColor() {
      ...
      return _snowman != 16 && _snowman <= 15 ? DyeColor.byId(_snowman) : null;
   }
   ...
   class SearchForPlayerGoal extends FollowTargetGoal<PlayerEntity> {
      ...
      protected Box getSearchBox(double distance) {
         Direction _snowman = ((ShulkerEntity)this.mob).getAttachedFace();
         if (_snowman.getAxis() == Direction.Axis.X) {
            return this.mob.getBoundingBox().expand(4.0, distance, distance);
         } else {
            return _snowman.getAxis() == Direction.Axis.Z
   ...
   class ShootBulletGoal extends Goal {
      ...
      @Override
      public boolean canStart() {
         LivingEntity _snowman = ShulkerEntity.this.getTarget();
         return _snowman != null && _snowman.isAlive() ? ShulkerEntity.this.world.getDifficulty() != Difficulty.PEACEFUL : false;
      }
      ...
      public void tick() {
         if (ShulkerEntity.this.world.getDifficulty() != Difficulty.PEACEFUL) {
            this.counter--;
            LivingEntity _snowman = ShulkerEntity.this.getTarget();
            ShulkerEntity.this.getLookControl().lookAt(_snowman, 180.0F, 180.0F);
            double _snowmanx = ShulkerEntity.this.squaredDistanceTo(_snowman);
            if (_snowmanx < 400.0) {
               if (this.counter <= 0) {
                  this.counter = 20 + ShulkerEntity.this.random.nextInt(10) * 20 / 2;
                  ShulkerEntity.this.world
                     .spawnEntity(new ShulkerBulletEntity(ShulkerEntity.this.world, ShulkerEntity.this, _snowman, ShulkerEntity.this.getAttachedFace().getAxis()));
