...
public abstract class SpellcastingIllagerEntity extends IllagerEntity {
   ...
   protected SpellcastingIllagerEntity(EntityType<? extends SpellcastingIllagerEntity> _snowman, World _snowman) {
   ...
   public void tick() {
      ...
      if (this.world.isClient && this.isSpellcasting()) {
         SpellcastingIllagerEntity.Spell _snowman = this.getSpell();
         double _snowmanx = _snowman.particleVelocity[0];
         double _snowmanxx = _snowman.particleVelocity[1];
         double _snowmanxxx = _snowman.particleVelocity[2];
         float _snowmanxxxx = this.bodyYaw * (float) (Math.PI / 180.0) + MathHelper.cos((float)this.age * 0.6662F) * 0.25F;
         float _snowmanxxxxx = MathHelper.cos(_snowmanxxxx);
         float _snowmanxxxxxx = MathHelper.sin(_snowmanxxxx);
         this.world
            .addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() + (double)_snowmanxxxxx * 0.6, this.getY() + 1.8, this.getZ() + (double)_snowmanxxxxxx * 0.6, _snowmanx, _snowmanxx, _snowmanxxx);
         this.world
            .addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() - (double)_snowmanxxxxx * 0.6, this.getY() + 1.8, this.getZ() - (double)_snowmanxxxxxx * 0.6, _snowmanx, _snowmanxx, _snowmanxxx);
      }
   }
   ...
   public abstract class CastSpellGoal extends Goal {
      ...
      public boolean canStart() {
         LivingEntity _snowman = SpellcastingIllagerEntity.this.getTarget();
      ...
      public boolean shouldContinue() {
         LivingEntity _snowman = SpellcastingIllagerEntity.this.getTarget();
         return _snowman != null && _snowman.isAlive() && this.spellCooldown > 0;
      }
      ...
      public void start() {
         ...
         SoundEvent _snowman = this.getSoundPrepare();
         if (_snowman != null) {
            SpellcastingIllagerEntity.this.playSound(_snowman, 1.0F, 1.0F);
         }
   ...
   public static enum Spell {
      ...
      public static SpellcastingIllagerEntity.Spell byId(int id) {
         for (SpellcastingIllagerEntity.Spell _snowman : values()) {
            if (id == _snowman.id) {
               return _snowman;
            }
         }
