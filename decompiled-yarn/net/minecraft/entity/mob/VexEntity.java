...
public class VexEntity extends HostileEntity {
   ...
   public VexEntity(EntityType<? extends VexEntity> _snowman, World _snowman) {
   ...
   private boolean areFlagsSet(int mask) {
      int _snowman = this.dataTracker.get(VEX_FLAGS);
      return (_snowman & mask) != 0;
   }
   ...
   private void setVexFlag(int mask, boolean value) {
      ...
      this.dataTracker.set(VEX_FLAGS, (byte)(_snowman & 0xFF));
   }
   ...
   class ChargeTargetGoal extends Goal {
      ...
      @Override
      public void start() {
         LivingEntity _snowman = VexEntity.this.getTarget();
         Vec3d _snowmanx = _snowman.getCameraPosVec(1.0F);
         VexEntity.this.moveControl.moveTo(_snowmanx.x, _snowmanx.y, _snowmanx.z, 1.0);
      ...
      public void tick() {
         LivingEntity _snowman = VexEntity.this.getTarget();
         if (VexEntity.this.getBoundingBox().intersects(_snowman.getBoundingBox())) {
            VexEntity.this.tryAttack(_snowman);
            VexEntity.this.setCharging(false);
         } else {
            double _snowmanx = VexEntity.this.squaredDistanceTo(_snowman);
            if (_snowmanx < 9.0) {
               Vec3d _snowmanxx = _snowman.getCameraPosVec(1.0F);
               VexEntity.this.moveControl.moveTo(_snowmanxx.x, _snowmanxx.y, _snowmanxx.z, 1.0);
            }
         }
      }
   }
   ...
   class LookAtTargetGoal extends Goal {
      ...
      @Override
      public void tick() {
         BlockPos _snowman = VexEntity.this.getBounds();
         if (_snowman == null) {
            _snowman = VexEntity.this.getBlockPos();
         }

         for (int _snowmanx = 0; _snowmanx < 3; _snowmanx++) {
            BlockPos _snowmanxx = _snowman.add(VexEntity.this.random.nextInt(15) - 7, VexEntity.this.random.nextInt(11) - 5, VexEntity.this.random.nextInt(15) - 7);
            if (VexEntity.this.world.isAir(_snowmanxx)) {
               VexEntity.this.moveControl.moveTo((double)_snowmanxx.getX() + 0.5, (double)_snowmanxx.getY() + 0.5, (double)_snowmanxx.getZ() + 0.5, 0.25);
               if (VexEntity.this.getTarget() == null) {
                  VexEntity.this.getLookControl().lookAt((double)_snowmanxx.getX() + 0.5, (double)_snowmanxx.getY() + 0.5, (double)_snowmanxx.getZ() + 0.5, 180.0F, 20.0F);
               }
               break;
            }
         }
   ...
   class VexMoveControl extends MoveControl {
      ...
      public void tick() {
         if (this.state == MoveControl.State.MOVE_TO) {
            Vec3d _snowman = new Vec3d(this.targetX - VexEntity.this.getX(), this.targetY - VexEntity.this.getY(), this.targetZ - VexEntity.this.getZ());
            double _snowmanx = _snowman.length();
            if (_snowmanx < VexEntity.this.getBoundingBox().getAverageSideLength()) {
               this.state = MoveControl.State.WAIT;
               VexEntity.this.setVelocity(VexEntity.this.getVelocity().multiply(0.5));
            } else {
               VexEntity.this.setVelocity(VexEntity.this.getVelocity().add(_snowman.multiply(this.speed * 0.05 / _snowmanx)));
               if (VexEntity.this.getTarget() == null) {
                  Vec3d _snowmanxx = VexEntity.this.getVelocity();
                  VexEntity.this.yaw = -((float)MathHelper.atan2(_snowmanxx.x, _snowmanxx.z)) * (180.0F / (float)Math.PI);
                  VexEntity.this.bodyYaw = VexEntity.this.yaw;
               } else {
                  double _snowmanxx = VexEntity.this.getTarget().getX() - VexEntity.this.getX();
                  double _snowmanxxx = VexEntity.this.getTarget().getZ() - VexEntity.this.getZ();
                  VexEntity.this.yaw = -((float)MathHelper.atan2(_snowmanxx, _snowmanxxx)) * (180.0F / (float)Math.PI);
                  VexEntity.this.bodyYaw = VexEntity.this.yaw;
               }
            }
         }
