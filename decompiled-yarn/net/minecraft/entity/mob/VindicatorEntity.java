...
public class VindicatorEntity extends IllagerEntity {
   private static final Predicate<Difficulty> DIFFICULTY_ALLOWS_DOOR_BREAKING_PREDICATE = _snowman -> _snowman == Difficulty.NORMAL || _snowman == Difficulty.HARD;
   private boolean johnny;

   public VindicatorEntity(EntityType<? extends VindicatorEntity> _snowman, World _snowman) {
   ...
   protected void mobTick() {
      if (!this.isAiDisabled() && class_5493.method_30955(this)) {
         boolean _snowman = ((ServerWorld)this.world).hasRaidAt(this.getBlockPos());
         ((MobNavigation)this.getNavigation()).setCanPathThroughDoors(_snowman);
      }
   ...
   public EntityData initialize(
      ...
      EntityData _snowman = super.initialize(world, difficulty, spawnReason, entityData, entityTag);
   ...
   public void addBonusForWave(int wave, boolean unused) {
      ItemStack _snowman = new ItemStack(Items.IRON_AXE);
      Raid _snowmanx = this.getRaid();
      int _snowmanxx = 1;
      if (wave > _snowmanx.getMaxWaves(Difficulty.NORMAL)) {
         _snowmanxx = 2;
      }

      boolean _snowmanxxx = this.random.nextFloat() <= _snowmanx.getEnchantmentChance();
      if (_snowmanxxx) {
         Map<Enchantment, Integer> _snowmanxxxx = Maps.newHashMap();
         _snowmanxxxx.put(Enchantments.SHARPNESS, _snowmanxx);
         EnchantmentHelper.set(_snowmanxxxx, _snowman);
      }
   ...
   class AttackGoal extends MeleeAttackGoal {
      ...
      protected double getSquaredMaxAttackDistance(LivingEntity entity) {
         if (this.mob.getVehicle() instanceof RavagerEntity) {
            float _snowman = this.mob.getVehicle().getWidth() - 0.1F;
   ...
   static class BreakDoorGoal extends net.minecraft.entity.ai.goal.BreakDoorGoal {
      public BreakDoorGoal(MobEntity _snowman) {
         super(_snowman, 6, VindicatorEntity.DIFFICULTY_ALLOWS_DOOR_BREAKING_PREDICATE);
         this.setControls(EnumSet.of(Goal.Control.MOVE));
      }

      @Override
      public boolean shouldContinue() {
         VindicatorEntity _snowman = (VindicatorEntity)this.mob;
         return _snowman.hasActiveRaid() && super.shouldContinue();
      }

      @Override
      public boolean canStart() {
         VindicatorEntity _snowman = (VindicatorEntity)this.mob;
         return _snowman.hasActiveRaid() && _snowman.random.nextInt(10) == 0 && super.canStart();
      }
