...
public class ZoglinEntity extends HostileEntity implements Monster, Hoglin {
   ...
   public ZoglinEntity(EntityType<? extends ZoglinEntity> _snowman, World _snowman) {
   ...
   protected Brain<?> deserializeBrain(Dynamic<?> _snowman) {
      Brain<ZoglinEntity> _snowmanx = this.createBrainProfile().deserialize(_snowman);
      method_26928(_snowmanx);
      method_26929(_snowmanx);
      method_26930(_snowmanx);
      _snowmanx.setCoreActivities(ImmutableSet.of(Activity.CORE));
      _snowmanx.setDefaultActivity(Activity.IDLE);
      _snowmanx.resetPossibleActivities();
      return _snowmanx;
   }

   private static void method_26928(Brain<ZoglinEntity> _snowman) {
      _snowman.setTaskList(Activity.CORE, 0, ImmutableList.of(new LookAroundTask(45, 90), new WanderAroundTask()));
   }

   private static void method_26929(Brain<ZoglinEntity> _snowman) {
   ...
   private static void method_26930(Brain<ZoglinEntity> _snowman) {
      _snowman.setTaskList(
         Activity.FIGHT,
         10,
         ImmutableList.of(
   ...
   private static boolean method_26936(LivingEntity _snowman) {
      EntityType<?> _snowmanx = _snowman.getType();
      return _snowmanx != EntityType.ZOGLIN && _snowmanx != EntityType.CREEPER && EntityPredicates.EXCEPT_CREATIVE_SPECTATOR_OR_PEACEFUL.test(_snowman);
   }
   ...
   public boolean damage(DamageSource source, float amount) {
      boolean _snowman = super.damage(source, amount);
      if (this.world.isClient) {
         return false;
      } else if (_snowman && source.getAttacker() instanceof LivingEntity) {
         LivingEntity _snowmanx = (LivingEntity)source.getAttacker();
         if (EntityPredicates.EXCEPT_CREATIVE_SPECTATOR_OR_PEACEFUL.test(_snowmanx) && !LookTargetUtil.isNewTargetTooFar(this, _snowmanx, 4.0)) {
            this.method_26938(_snowmanx);
         }
   ...
   private void method_26938(LivingEntity _snowman) {
      this.brain.forget(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE);
      this.brain.remember(MemoryModuleType.ATTACK_TARGET, _snowman, 200L);
   }
   ...
   protected void method_26931() {
      Activity _snowman = this.brain.getFirstPossibleNonCoreActivity().orElse(null);
      this.brain.resetPossibleActivities(ImmutableList.of(Activity.FIGHT, Activity.IDLE));
      Activity _snowmanx = this.brain.getFirstPossibleNonCoreActivity().orElse(null);
      if (_snowmanx == Activity.FIGHT && _snowman != Activity.FIGHT) {
         this.playAngrySound();
      }
