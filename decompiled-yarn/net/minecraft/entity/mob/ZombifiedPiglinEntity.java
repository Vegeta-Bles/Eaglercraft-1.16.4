...
public class ZombifiedPiglinEntity extends ZombieEntity implements Angerable {
   ...
   public ZombifiedPiglinEntity(EntityType<? extends ZombifiedPiglinEntity> _snowman, World _snowman) {
   ...
   protected void mobTick() {
      ...
      if (this.hasAngerTime()) {
         if (!this.isBaby() && !_snowman.hasModifier(ATTACKING_SPEED_BOOST)) {
            _snowman.addTemporaryModifier(ATTACKING_SPEED_BOOST);
         }

         this.method_30080();
      } else if (_snowman.hasModifier(ATTACKING_SPEED_BOOST)) {
   ...
   private void method_29942() {
      double _snowman = this.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE);
      Box _snowmanx = Box.method_29968(this.getPos()).expand(_snowman, 10.0, _snowman);
      this.world
         .getEntitiesIncludingUngeneratedChunks(ZombifiedPiglinEntity.class, _snowmanx)
         .stream()
         .filter(_snowmanxx -> _snowmanxx != this)
         .filter(_snowmanxx -> _snowmanxx.getTarget() == null)
         .filter(_snowmanxx -> !_snowmanxx.isTeammate(this.getTarget()))
         .forEach(_snowmanxx -> _snowmanxx.setTarget(this.getTarget()));
   }
