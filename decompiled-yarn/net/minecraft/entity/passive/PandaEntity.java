...
public class PandaEntity extends AnimalEntity {
   ...
   private static final Predicate<ItemEntity> IS_FOOD = _snowman -> {
      Item _snowmanx = _snowman.getStack().getItem();
      return (_snowmanx == Blocks.BAMBOO.asItem() || _snowmanx == Blocks.CAKE.asItem()) && _snowman.isAlive() && !_snowman.cannotPickup();
   };

   public PandaEntity(EntityType<? extends PandaEntity> _snowman, World _snowman) {
   ...
   private void setPandaFlag(int mask, boolean value) {
      byte _snowman = this.dataTracker.get(PANDA_FLAGS);
      if (value) {
         this.dataTracker.set(PANDA_FLAGS, (byte)(_snowman | mask));
      } else {
         this.dataTracker.set(PANDA_FLAGS, (byte)(_snowman & ~mask));
      }
   }
   ...
   public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
      PandaEntity _snowman = EntityType.PANDA.create(world);
      if (entity instanceof PandaEntity) {
         _snowman.initGenes(this, (PandaEntity)entity);
      }

      _snowman.resetAttributes();
      return _snowman;
   }
   ...
   private void playEatingAnimation() {
      if (this.getEatingTicks() % 5 == 0) {
         this.playSound(
            SoundEvents.ENTITY_PANDA_EAT, 0.5F + 0.5F * (float)this.random.nextInt(2), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F
         );

         for (int _snowman = 0; _snowman < 6; _snowman++) {
            Vec3d _snowmanx = new Vec3d(((double)this.random.nextFloat() - 0.5) * 0.1, Math.random() * 0.1 + 0.1, ((double)this.random.nextFloat() - 0.5) * 0.1);
            ...
            Vec3d _snowmanxxx = new Vec3d(((double)this.random.nextFloat() - 0.5) * 0.8, _snowmanxx, 1.0 + ((double)this.random.nextFloat() - 0.5) * 0.4);
            _snowmanxxx = _snowmanxxx.rotateY(-this.bodyYaw * (float) (Math.PI / 180.0));
            _snowmanxxx = _snowmanxxx.add(this.getX(), this.getEyeY() + 1.0, this.getZ());
            this.world
               .addParticle(
      ...
      if (this.playingTicks > 32) {
         ...
         if (!this.world.isClient) {
            ...
            if (this.playingTicks == 1) {
               float _snowmanx = this.yaw * (float) (Math.PI / 180.0);
               float _snowmanxx = this.isBaby() ? 0.1F : 0.2F;
               this.playingJump = new Vec3d(_snowman.x + (double)(-MathHelper.sin(_snowmanx) * _snowmanxx), 0.0, _snowman.z + (double)(MathHelper.cos(_snowmanx) * _snowmanxx));
               ...
               this.setVelocity(this.playingJump.x, _snowman.y, this.playingJump.z);
            } else {
               this.setVelocity(0.0, this.onGround ? 0.27 : _snowman.y, 0.0);
            }
         }
      }
   }
   ...
   private void sneeze() {
      ...
      for (PandaEntity _snowmanx : this.world.getNonSpectatingEntities(PandaEntity.class, this.getBoundingBox().expand(10.0))) {
         if (!_snowmanx.isBaby() && _snowmanx.onGround && !_snowmanx.isTouchingWater() && _snowmanx.isIdle()) {
            _snowmanx.jump();
         }
      }
   ...
   public ActionResult interactMob(PlayerEntity player, Hand hand) {
      ...
      if (this.isScaredByThunderstorm()) {
         ...
         if (this.isBaby()) {
            ...
            if (!_snowmanx.isEmpty() && !player.abilities.creativeMode) {
               this.dropStack(_snowmanx);
            }

            this.equipStack(EquipmentSlot.MAINHAND, new ItemStack(_snowman.getItem(), 1));
            this.eat(player, _snowman);
         }
   ...
   public static enum Gene {
      ...
      private static final PandaEntity.Gene[] VALUES = Arrays.stream(values())
      ...
      public static PandaEntity.Gene byName(String name) {
         for (PandaEntity.Gene _snowman : values()) {
      ...
      public static PandaEntity.Gene createRandom(Random random) {
         int _snowman = random.nextInt(16);
         if (_snowman == 0) {
            return LAZY;
         } else if (_snowman == 1) {
            return WORRIED;
            ...
            return _snowman < 11 ? BROWN : NORMAL;
         }
      }
   }
   ...
   class PandaMateGoal extends AnimalMateGoal {
         ...
         if (!super.canStart() || this.panda.getAskForBambooTicks() != 0) {
            ...
            if (this.nextAskPlayerForBambooAge <= this.panda.age) {
               ...
               if (this.panda.canMoveVoluntarily()) {
                  PlayerEntity _snowman = this.world.getClosestPlayer(PandaEntity.ASK_FOR_BAMBOO_TARGET, this.panda);
                  this.panda.lookAtPlayerGoal.setTarget(_snowman);
               }
            }
      ...
      private boolean isBambooClose() {
         BlockPos _snowman = this.panda.getBlockPos();
            ...
            for (int _snowmanxxx = 0; _snowmanxxx < 8; _snowmanxxx++) {
               for (int _snowmanxxxx = 0; _snowmanxxxx <= _snowmanxxx; _snowmanxxxx = _snowmanxxxx > 0 ? -_snowmanxxxx : 1 - _snowmanxxxx) {
                  for (int _snowmanxxxxx = _snowmanxxxx < _snowmanxxx && _snowmanxxxx > -_snowmanxxx ? _snowmanxxx : 0; _snowmanxxxxx <= _snowmanxxx; _snowmanxxxxx = _snowmanxxxxx > 0 ? -_snowmanxxxxx : 1 - _snowmanxxxxx) {
                     _snowmanx.set(_snowman, _snowmanxxxx, _snowmanxx, _snowmanxxxxx);
                     if (this.world.getBlockState(_snowmanx).isOf(Blocks.BAMBOO)) {
                        return true;
                     }
                  }
               }
   ...
   class PickUpFoodGoal extends Goal {
      ...
      public boolean canStart() {
         if (this.startAge <= PandaEntity.this.age
            ...
            List<ItemEntity> _snowman = PandaEntity.this.world
               .getEntitiesByClass(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(6.0, 6.0, 6.0), PandaEntity.IS_FOOD);
            return !_snowman.isEmpty() || !PandaEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty();
         } else {
            return false;
         }
      }
      ...
      public void start() {
         ...
         if (!_snowman.isEmpty() && PandaEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty()) {
            PandaEntity.this.getNavigation().startMovingTo(_snowman.get(0), 1.2F);
      ...
      @Override
      public void stop() {
         ItemStack _snowman = PandaEntity.this.getEquippedStack(EquipmentSlot.MAINHAND);
         if (!_snowman.isEmpty()) {
            PandaEntity.this.dropStack(_snowman);
            PandaEntity.this.equipStack(EquipmentSlot.MAINHAND, ItemStack.EMPTY);
            int _snowmanx = PandaEntity.this.isLazy() ? PandaEntity.this.random.nextInt(50) + 10 : PandaEntity.this.random.nextInt(150) + 10;
            this.startAge = PandaEntity.this.age + _snowmanx * 20;
         }
      ...
      public boolean canStart() {
         if ((this.panda.isBaby() || this.panda.isPlayful()) && this.panda.onGround) {
            if (!this.panda.isIdle()) {
               ...
               float _snowman = this.panda.yaw * (float) (Math.PI / 180.0);
               int _snowmanx = 0;
               int _snowmanxx = 0;
               float _snowmanxxx = -MathHelper.sin(_snowman);
               ...
               if (this.panda.world.getBlockState(this.panda.getBlockPos().add(_snowmanx, -1, _snowmanxx)).isAir()) {
                  return true;
               } else {
