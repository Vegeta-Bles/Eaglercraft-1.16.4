...
public class PufferfishEntity extends FishEntity {
   ...
   private static final Predicate<LivingEntity> BLOW_UP_FILTER = _snowman -> {
      if (_snowman == null) {
         return false;
      } else {
         return !(_snowman instanceof PlayerEntity) || !_snowman.isSpectator() && !((PlayerEntity)_snowman).isCreative() ? _snowman.getGroup() != EntityGroup.AQUATIC : false;
      }
   };

   public PufferfishEntity(EntityType<? extends PufferfishEntity> _snowman, World _snowman) {
   ...
   public void tickMovement() {
      ...
      if (this.isAlive() && this.getPuffState() > 0) {
         for (MobEntity _snowman : this.world.getEntitiesByClass(MobEntity.class, this.getBoundingBox().expand(0.3), BLOW_UP_FILTER)) {
            if (_snowman.isAlive()) {
               this.sting(_snowman);
            }
         }
   ...
   private void sting(MobEntity mob) {
      int _snowman = this.getPuffState();
      if (mob.damage(DamageSource.mob(this), (float)(1 + _snowman))) {
         mob.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 60 * _snowman, 0));
   ...
   public void onPlayerCollision(PlayerEntity player) {
      int _snowman = this.getPuffState();
      if (player instanceof ServerPlayerEntity && _snowman > 0 && player.damage(DamageSource.mob(this), (float)(1 + _snowman))) {
         if (!this.isSilent()) {
         ...
         player.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 60 * _snowman, 0));
      }
   }
   ...
   static class InflateGoal extends Goal {
      ...
      public boolean canStart() {
         List<LivingEntity> _snowman = this.pufferfish
            .world
