...
public class RabbitEntity extends AnimalEntity {
   ...
   public RabbitEntity(EntityType<? extends RabbitEntity> _snowman, World _snowman) {
   ...
   protected float getJumpVelocity() {
      if (!this.horizontalCollision && (!this.moveControl.isMoving() || !(this.moveControl.getTargetY() > this.getY() + 0.5))) {
         Path _snowman = this.navigation.getCurrentPath();
         if (_snowman != null && !_snowman.isFinished()) {
            Vec3d _snowmanx = _snowman.getNodePosition(this);
            if (_snowmanx.y > this.getY() + 0.5) {
               return 0.5F;
            }
         }
   ...
   protected void jump() {
      super.jump();
      double _snowman = this.moveControl.getSpeed();
      if (_snowman > 0.0) {
         double _snowmanx = squaredHorizontalLength(this.getVelocity());
         if (_snowmanx < 0.01) {
   ...
   public void mobTick() {
      ...
      if (this.onGround) {
         ...
         if (this.getRabbitType() == 99 && this.ticksUntilJump == 0) {
            ...
            if (_snowman != null && this.squaredDistanceTo(_snowman) < 16.0) {
               this.lookTowards(_snowman.getX(), _snowman.getZ());
               this.moveControl.moveTo(_snowman.getX(), _snowman.getY(), _snowman.getZ(), this.moveControl.getSpeed());
               this.startJump();
               this.lastOnGround = true;
            }
         }

         RabbitEntity.RabbitJumpControl _snowman = (RabbitEntity.RabbitJumpControl)this.jumpControl;
         if (!_snowman.isActive()) {
            if (this.moveControl.isMoving() && this.ticksUntilJump == 0) {
               Path _snowmanx = this.navigation.getCurrentPath();
               Vec3d _snowmanxx = new Vec3d(this.moveControl.getTargetX(), this.moveControl.getTargetY(), this.moveControl.getTargetZ());
               if (_snowmanx != null && !_snowmanx.isFinished()) {
                  _snowmanxx = _snowmanx.getNodePosition(this);
               }

               this.lookTowards(_snowmanxx.x, _snowmanxx.z);
               this.startJump();
            }
         } else if (!_snowman.method_27313()) {
            this.method_6611();
         }
      }
   ...
   public RabbitEntity createChild(ServerWorld _snowman, PassiveEntity _snowman) {
      RabbitEntity _snowmanxx = EntityType.RABBIT.create(_snowman);
      int _snowmanxxx = this.chooseType(_snowman);
      if (this.random.nextInt(20) != 0) {
         if (_snowman instanceof RabbitEntity && this.random.nextBoolean()) {
            _snowmanxxx = ((RabbitEntity)_snowman).getRabbitType();
         } else {
            _snowmanxxx = this.getRabbitType();
         }
      }

      _snowmanxx.setRabbitType(_snowmanxxx);
      return _snowmanxx;
   }
   ...
   public EntityData initialize(
      ...
      int _snowman = this.chooseType(world);
      if (entityData instanceof RabbitEntity.RabbitData) {
         _snowman = ((RabbitEntity.RabbitData)entityData).type;
      } else {
         entityData = new RabbitEntity.RabbitData(_snowman);
      }

      this.setRabbitType(_snowman);
      return super.initialize(world, difficulty, spawnReason, entityData, entityTag);
   }

   private int chooseType(WorldAccess world) {
      Biome _snowman = world.getBiome(this.getBlockPos());
      int _snowmanx = this.random.nextInt(100);
      if (_snowman.getPrecipitation() == Biome.Precipitation.SNOW) {
         return _snowmanx < 80 ? 1 : 3;
      } else if (_snowman.getCategory() == Biome.Category.DESERT) {
         return 4;
      } else {
   ...
   public static boolean canSpawn(EntityType<RabbitEntity> entity, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random _snowman) {
      BlockState _snowmanx = world.getBlockState(pos.down());
      return (_snowmanx.isOf(Blocks.GRASS_BLOCK) || _snowmanx.isOf(Blocks.SNOW) || _snowmanx.isOf(Blocks.SAND)) && world.getBaseLightLevel(pos, 0) > 8;
   }
   ...
   static class EatCarrotCropGoal extends MoveToTargetPosGoal {
      ...
      public void tick() {
         ...
         if (this.hasReached()) {
            World _snowman = this.rabbit.world;
            BlockPos _snowmanx = this.targetPos.up();
            BlockState _snowmanxx = _snowman.getBlockState(_snowmanx);
            Block _snowmanxxx = _snowmanxx.getBlock();
            if (this.hasTarget && _snowmanxxx instanceof CarrotsBlock) {
               Integer _snowmanxxxx = _snowmanxx.get(CarrotsBlock.AGE);
               if (_snowmanxxxx == 0) {
                  _snowman.setBlockState(_snowmanx, Blocks.AIR.getDefaultState(), 2);
                  _snowman.breakBlock(_snowmanx, true, this.rabbit);
               } else {
                  _snowman.setBlockState(_snowmanx, _snowmanxx.with(CarrotsBlock.AGE, Integer.valueOf(_snowmanxxxx - 1)), 2);
                  _snowman.syncWorldEvent(2001, _snowmanx, Block.getRawIdFromState(_snowmanxx));
               }
      ...
      protected boolean isTargetPos(WorldView world, BlockPos pos) {
         Block _snowman = world.getBlockState(pos).getBlock();
         if (_snowman == Blocks.FARMLAND && this.wantsCarrots && !this.hasTarget) {
            pos = pos.up();
            BlockState _snowmanx = world.getBlockState(pos);
            _snowman = _snowmanx.getBlock();
            if (_snowman instanceof CarrotsBlock && ((CarrotsBlock)_snowman).isMature(_snowmanx)) {
               this.hasTarget = true;
               return true;
            }
         }
   ...
   public class RabbitJumpControl extends JumpControl {
      ...
      public void method_27311(boolean _snowman) {
         this.field_24091 = _snowman;
      }
