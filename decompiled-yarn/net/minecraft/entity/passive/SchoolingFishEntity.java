...
public abstract class SchoolingFishEntity extends FishEntity {
   ...
   public SchoolingFishEntity(EntityType<? extends SchoolingFishEntity> _snowman, World _snowman) {
   ...
   public void tick() {
      ...
      if (this.hasOtherFishInGroup() && this.world.random.nextInt(200) == 1) {
         List<FishEntity> _snowman = this.world.getNonSpectatingEntities((Class<? extends FishEntity>)this.getClass(), this.getBoundingBox().expand(8.0, 8.0, 8.0));
         if (_snowman.size() <= 1) {
            this.groupSize = 1;
         }
      }
   ...
   public void pullInOtherFish(Stream<SchoolingFishEntity> fish) {
      fish.limit((long)(this.getMaxGroupSize() - this.groupSize)).filter(_snowman -> _snowman != this).forEach(_snowman -> _snowman.joinGroupOf(this));
   }
