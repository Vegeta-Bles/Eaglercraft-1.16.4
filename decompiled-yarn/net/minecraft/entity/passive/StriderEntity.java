...
public class StriderEntity extends AnimalEntity implements ItemSteerable, Saddleable {
   ...
   public StriderEntity(EntityType<? extends StriderEntity> _snowman, World _snowman) {
   ...
   public static boolean canSpawn(EntityType<StriderEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
      ...
      } while (world.getFluidState(_snowman).isIn(FluidTags.LAVA));

      return world.getBlockState(_snowman).isAir();
   }
   ...
   public double getMountedHeightOffset() {
      float _snowman = Math.min(0.25F, this.limbDistance);
      float _snowmanx = this.limbAngle;
      return (double)this.getHeight() - 0.19 + (double)(0.12F * MathHelper.cos(_snowmanx * 1.5F) * 2.0F * _snowman);
   }
   ...
   public boolean canBeControlledByRider() {
      ...
      if (!(_snowman instanceof PlayerEntity)) {
         ...
         return _snowmanx.getMainHandStack().getItem() == Items.WARPED_FUNGUS_ON_A_STICK || _snowmanx.getOffHandStack().getItem() == Items.WARPED_FUNGUS_ON_A_STICK;
      }
   }
   ...
   public Vec3d updatePassengerForDismount(LivingEntity passenger) {
      ...
      Set<BlockPos> _snowmanx = Sets.newLinkedHashSet();
      double _snowmanxx = this.getBoundingBox().maxY;
      double _snowmanxxx = this.getBoundingBox().minY - 0.5;
      BlockPos.Mutable _snowmanxxxx = new BlockPos.Mutable();

      for (Vec3d _snowmanxxxxx : _snowman) {
         _snowmanxxxx.set(this.getX() + _snowmanxxxxx.x, _snowmanxx, this.getZ() + _snowmanxxxxx.z);

         for (double _snowmanxxxxxx = _snowmanxx; _snowmanxxxxxx > _snowmanxxx; _snowmanxxxxxx--) {
            _snowmanx.add(_snowmanxxxx.toImmutable());
            _snowmanxxxx.move(Direction.DOWN);
         }
      }

      for (BlockPos _snowmanxxxxx : _snowmanx) {
         if (!this.world.getFluidState(_snowmanxxxxx).isIn(FluidTags.LAVA)) {
            double _snowmanxxxxxx = this.world.getDismountHeight(_snowmanxxxxx);
            if (Dismounting.canDismountInBlock(_snowmanxxxxxx)) {
               Vec3d _snowmanxxxxxxx = Vec3d.ofCenter(_snowmanxxxxx, _snowmanxxxxxx);
               UnmodifiableIterator var14 = passenger.getPoses().iterator();

               while (var14.hasNext()) {
                  EntityPose _snowmanxxxxxxxx = (EntityPose)var14.next();
                  Box _snowmanxxxxxxxxx = passenger.getBoundingBox(_snowmanxxxxxxxx);
                  if (Dismounting.canPlaceEntityAt(this.world, passenger, _snowmanxxxxxxxxx.offset(_snowmanxxxxxxx))) {
                     passenger.setPose(_snowmanxxxxxxxx);
   ...
   public void tick() {
      ...
      BlockState _snowmanx = this.getLandingBlockState();
      boolean _snowmanxx = _snowman.isIn(BlockTags.STRIDER_WARM_BLOCKS) || _snowmanx.isIn(BlockTags.STRIDER_WARM_BLOCKS) || this.getFluidHeight(FluidTags.LAVA) > 0.0;
      this.setCold(!_snowmanxx);
      super.tick();
      this.updateFloating();
      this.checkBlockCollision();
   }
   ...
   private void updateFloating() {
      if (this.isInLava()) {
         ShapeContext _snowman = ShapeContext.of(this);
         if (_snowman.isAbove(FluidBlock.COLLISION_SHAPE, this.getBlockPos(), true) && !this.world.getFluidState(this.getBlockPos().up()).isIn(FluidTags.LAVA)) {
            this.onGround = true;
         } else {
   ...
   public StriderEntity createChild(ServerWorld _snowman, PassiveEntity _snowman) {
      return EntityType.STRIDER.create(_snowman);
   }
   ...
   public EntityData initialize(
      ...
      if (this.isBaby()) {
         ...
         if (this.random.nextInt(30) == 0) {
            MobEntity _snowman = EntityType.ZOMBIFIED_PIGLIN.create(world.toServerWorld());
            var7 = this.method_30336(world, difficulty, _snowman, new ZombieEntity.ZombieData(ZombieEntity.method_29936(this.random), false));
            _snowman.equipStack(EquipmentSlot.MAINHAND, new ItemStack(Items.WARPED_FUNGUS_ON_A_STICK));
            this.saddle(null);
         } else if (this.random.nextInt(10) == 0) {
            PassiveEntity _snowman = EntityType.STRIDER.create(world.toServerWorld());
            _snowman.setBreedingAge(-24000);
            var7 = this.method_30336(world, difficulty, _snowman, null);
         } else {
   ...
   private EntityData method_30336(ServerWorldAccess _snowman, LocalDifficulty _snowman, MobEntity _snowman, @Nullable EntityData _snowman) {
      _snowman.refreshPositionAndAngles(this.getX(), this.getY(), this.getZ(), this.yaw, 0.0F);
      _snowman.initialize(_snowman, _snowman, SpawnReason.JOCKEY, _snowman, null);
      _snowman.startRiding(this, true);
