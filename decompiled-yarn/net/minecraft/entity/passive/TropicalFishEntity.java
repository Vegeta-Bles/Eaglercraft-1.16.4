...
public class TropicalFishEntity extends SchoolingFishEntity {
   ...
   public TropicalFishEntity(EntityType<? extends TropicalFishEntity> _snowman, World _snowman) {
   ...
   public static String getTranslationKey(int variant) {
      int _snowman = getShape(variant);
      int _snowmanx = getPattern(variant);
      return "entity.minecraft.tropical_fish.type." + TropicalFishEntity.Variety.getTranslateKey(_snowman, _snowmanx);
   }
   ...
   protected void copyDataToStack(ItemStack stack) {
      ...
      _snowman.putInt("BucketVariantTag", this.getVariant());
   }
   ...
   public EntityData initialize(
      ...
      if (entityTag != null && entityTag.contains("BucketVariantTag", 3)) {
         ...
         if (entityData instanceof TropicalFishEntity.TropicalFishData) {
            TropicalFishEntity.TropicalFishData _snowmanxxxx = (TropicalFishEntity.TropicalFishData)entityData;
            _snowman = _snowmanxxxx.shape;
            _snowmanx = _snowmanxxxx.pattern;
            _snowmanxx = _snowmanxxxx.baseColor;
            _snowmanxxx = _snowmanxxxx.patternColor;
         } else if ((double)this.random.nextFloat() < 0.9) {
            int _snowmanxxxx = Util.getRandom(COMMON_VARIANTS, this.random);
            ...
            entityData = new TropicalFishEntity.TropicalFishData(this, _snowman, _snowmanx, _snowmanxx, _snowmanxxx);
         } else {
            this.commonSpawn = false;
            _snowman = this.random.nextInt(2);
            _snowmanx = this.random.nextInt(6);
            _snowmanxx = this.random.nextInt(15);
            _snowmanxxx = this.random.nextInt(15);
         }

         this.setVariant(_snowman | _snowmanx << 8 | _snowmanxx << 16 | _snowmanxxx << 24);
         return entityData;
      }
   }
