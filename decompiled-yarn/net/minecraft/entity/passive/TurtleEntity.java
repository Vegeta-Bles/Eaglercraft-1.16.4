...
public class TurtleEntity extends AnimalEntity {
   ...
   public static final Predicate<LivingEntity> BABY_TURTLE_ON_LAND_FILTER = _snowman -> _snowman.isBaby() && !_snowman.isTouchingWater();

   public TurtleEntity(EntityType<? extends TurtleEntity> _snowman, World _snowman) {
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      int _snowman = tag.getInt("HomePosX");
      int _snowmanx = tag.getInt("HomePosY");
      int _snowmanxx = tag.getInt("HomePosZ");
      this.setHomePos(new BlockPos(_snowman, _snowmanx, _snowmanxx));
      super.readCustomDataFromTag(tag);
      this.setHasEgg(tag.getBoolean("HasEgg"));
      int _snowmanxxx = tag.getInt("TravelPosX");
      int _snowmanxxxx = tag.getInt("TravelPosY");
      int _snowmanxxxxx = tag.getInt("TravelPosZ");
      this.setTravelPos(new BlockPos(_snowmanxxx, _snowmanxxxx, _snowmanxxxxx));
   }
   ...
   protected void playStepSound(BlockPos pos, BlockState state) {
      SoundEvent _snowman = this.isBaby() ? SoundEvents.ENTITY_TURTLE_SHAMBLE_BABY : SoundEvents.ENTITY_TURTLE_SHAMBLE;
      this.playSound(_snowman, 0.15F, 1.0F);
   }
   ...
   public void tickMovement() {
      ...
      if (this.isAlive() && this.isDiggingSand() && this.sandDiggingCounter >= 1 && this.sandDiggingCounter % 5 == 0) {
         ...
         if (TurtleEggBlock.isSand(this.world, _snowman)) {
            this.world.syncWorldEvent(2001, _snowman, Block.getRawIdFromState(Blocks.SAND.getDefaultState()));
         }
      }
   }
   ...
   static class GoHomeGoal extends Goal {
      ...
      @Override
      public void tick() {
         BlockPos _snowman = this.turtle.getHomePos();
         boolean _snowmanx = _snowman.isWithinDistance(this.turtle.getPos(), 16.0);
         if (_snowmanx) {
            this.homeReachingTryTicks++;
         }

         if (this.turtle.getNavigation().isIdle()) {
            Vec3d _snowmanxx = Vec3d.ofBottomCenter(_snowman);
            Vec3d _snowmanxxx = TargetFinder.findTargetTowards(this.turtle, 16, 3, _snowmanxx, (float) (Math.PI / 10));
            if (_snowmanxxx == null) {
               _snowmanxxx = TargetFinder.findTargetTowards(this.turtle, 8, 7, _snowmanxx);
            }

            if (_snowmanxxx != null && !_snowmanx && !this.turtle.world.getBlockState(new BlockPos(_snowmanxxx)).isOf(Blocks.WATER)) {
               _snowmanxxx = TargetFinder.findTargetTowards(this.turtle, 16, 5, _snowmanxx);
            }
            ...
            this.turtle.getNavigation().startMovingTo(_snowmanxxx.x, _snowmanxxx.y, _snowmanxxx.z, this.speed);
         }
      }
   }
   ...
   static class LayEggGoal extends MoveToTargetPosGoal {
      ...
      public void tick() {
         super.tick();
         BlockPos _snowman = this.turtle.getBlockPos();
         if (!this.turtle.isTouchingWater() && this.hasReached()) {
            if (this.turtle.sandDiggingCounter < 1) {
               ...
               World _snowmanx = this.turtle.world;
               _snowmanx.playSound(null, _snowman, SoundEvents.ENTITY_TURTLE_LAY_EGG, SoundCategory.BLOCKS, 0.3F, 0.9F + _snowmanx.random.nextFloat() * 0.2F);
               _snowmanx.setBlockState(
   ...
   static class MateGoal extends AnimalMateGoal {
      ...
      protected void breed() {
         ServerPlayerEntity _snowman = this.animal.getLovingPlayer();
         if (_snowman == null && this.mate.getLovingPlayer() != null) {
            _snowman = this.mate.getLovingPlayer();
         }

         if (_snowman != null) {
            _snowman.incrementStat(Stats.ANIMALS_BRED);
            Criteria.BRED_ANIMALS.trigger(_snowman, this.animal, this.mate, null);
         }

         this.turtle.setHasEgg(true);
         this.animal.resetLoveTicks();
         this.mate.resetLoveTicks();
         Random _snowmanx = this.animal.getRandom();
         if (this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
            this.world.spawnEntity(new ExperienceOrbEntity(this.world, this.animal.getX(), this.animal.getY(), this.animal.getZ(), _snowmanx.nextInt(7) + 1));
         }
      }
   }
   ...
   static class TravelGoal extends Goal {
      ...
      public void start() {
         int _snowman = 512;
         int _snowmanx = 4;
         Random _snowmanxx = this.turtle.random;
         ...
         if ((double)_snowmanxxxx + this.turtle.getY() > (double)(this.turtle.world.getSeaLevel() - 1)) {
            _snowmanxxxx = 0;
         }

         BlockPos _snowmanxxxxxx = new BlockPos((double)_snowmanxxx + this.turtle.getX(), (double)_snowmanxxxx + this.turtle.getY(), (double)_snowmanxxxxx + this.turtle.getZ());
         this.turtle.setTravelPos(_snowmanxxxxxx);
      ...
      public void tick() {
         if (this.turtle.getNavigation().isIdle()) {
            Vec3d _snowman = Vec3d.ofBottomCenter(this.turtle.getTravelPos());
            Vec3d _snowmanx = TargetFinder.findTargetTowards(this.turtle, 16, 3, _snowman, (float) (Math.PI / 10));
            if (_snowmanx == null) {
               _snowmanx = TargetFinder.findTargetTowards(this.turtle, 8, 7, _snowman);
            }

            if (_snowmanx != null) {
               int _snowmanxx = MathHelper.floor(_snowmanx.x);
               int _snowmanxxx = MathHelper.floor(_snowmanx.z);
               int _snowmanxxxx = 34;
               if (!this.turtle.world.isRegionLoaded(_snowmanxx - 34, 0, _snowmanxxx - 34, _snowmanxx + 34, 0, _snowmanxxx + 34)) {
                  _snowmanx = null;
               }
            }
            ...
            this.turtle.getNavigation().startMovingTo(_snowmanx.x, _snowmanx.y, _snowmanx.z, this.speed);
         }
      }
   ...
   static class TurtleEscapeDangerGoal extends EscapeDangerGoal {
      ...
      public boolean canStart() {
         if (this.mob.getAttacker() == null && !this.mob.isOnFire()) {
            ...
            BlockPos _snowman = this.locateClosestWater(this.mob.world, this.mob, 7, 4);
            if (_snowman != null) {
   ...
   static class TurtleMoveControl extends MoveControl {
      ...
      public void tick() {
         ...
         if (this.state == MoveControl.State.MOVE_TO && !this.turtle.getNavigation().isIdle()) {
            double _snowman = this.targetX - this.turtle.getX();
            double _snowmanx = this.targetY - this.turtle.getY();
            double _snowmanxx = this.targetZ - this.turtle.getZ();
            double _snowmanxxx = (double)MathHelper.sqrt(_snowman * _snowman + _snowmanx * _snowmanx + _snowmanxx * _snowmanxx);
            _snowmanx /= _snowmanxxx;
            float _snowmanxxxx = (float)(MathHelper.atan2(_snowmanxx, _snowman) * 180.0F / (float)Math.PI) - 90.0F;
            this.turtle.yaw = this.changeAngle(this.turtle.yaw, _snowmanxxxx, 90.0F);
            this.turtle.bodyYaw = this.turtle.yaw;
            float _snowmanxxxxx = (float)(this.speed * this.turtle.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED));
            this.turtle.setMovementSpeed(MathHelper.lerp(0.125F, this.turtle.getMovementSpeed(), _snowmanxxxxx));
            this.turtle.setVelocity(this.turtle.getVelocity().add(0.0, (double)this.turtle.getMovementSpeed() * _snowmanx * 0.1, 0.0));
         } else {
   ...
   static class TurtleSwimNavigation extends SwimNavigation {
      ...
      public boolean isValidPosition(BlockPos pos) {
         if (this.entity instanceof TurtleEntity) {
            TurtleEntity _snowman = (TurtleEntity)this.entity;
            if (_snowman.isActivelyTravelling()) {
