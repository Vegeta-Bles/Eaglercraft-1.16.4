...
public class HungerManager {
   ...
   public void add(int food, float _snowman) {
      this.foodLevel = Math.min(food + this.foodLevel, 20);
      this.foodSaturationLevel = Math.min(this.foodSaturationLevel + (float)food * _snowman * 2.0F, (float)this.foodLevel);
   }

   public void eat(Item item, ItemStack stack) {
      if (item.isFood()) {
         FoodComponent _snowman = item.getFoodComponent();
         this.add(_snowman.getHunger(), _snowman.getSaturationModifier());
      }
   }

   public void update(PlayerEntity player) {
      Difficulty _snowman = player.world.getDifficulty();
      this.prevFoodLevel = this.foodLevel;
      if (this.exhaustion > 4.0F) {
         this.exhaustion -= 4.0F;
         if (this.foodSaturationLevel > 0.0F) {
            this.foodSaturationLevel = Math.max(this.foodSaturationLevel - 1.0F, 0.0F);
         } else if (_snowman != Difficulty.PEACEFUL) {
            this.foodLevel = Math.max(this.foodLevel - 1, 0);
         }
      }

      boolean _snowmanx = player.world.getGameRules().getBoolean(GameRules.NATURAL_REGENERATION);
      if (_snowmanx && this.foodSaturationLevel > 0.0F && player.canFoodHeal() && this.foodLevel >= 20) {
         this.foodStarvationTimer++;
         if (this.foodStarvationTimer >= 10) {
            float _snowmanxx = Math.min(this.foodSaturationLevel, 6.0F);
            player.heal(_snowmanxx / 6.0F);
            this.addExhaustion(_snowmanxx);
            this.foodStarvationTimer = 0;
         }
      } else if (_snowmanx && this.foodLevel >= 18 && player.canFoodHeal()) {
         this.foodStarvationTimer++;
         ...
         this.foodStarvationTimer++;
         if (this.foodStarvationTimer >= 80) {
            if (player.getHealth() > 10.0F || _snowman == Difficulty.HARD || player.getHealth() > 1.0F && _snowman == Difficulty.NORMAL) {
