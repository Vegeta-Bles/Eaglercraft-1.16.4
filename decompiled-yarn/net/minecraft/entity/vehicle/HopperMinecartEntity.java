...
public class HopperMinecartEntity extends StorageMinecartEntity implements Hopper {
   ...
   public HopperMinecartEntity(EntityType<? extends HopperMinecartEntity> _snowman, World _snowman) {
   ...
   public void onActivatorRail(int x, int y, int z, boolean powered) {
      boolean _snowman = !powered;
      if (_snowman != this.isEnabled()) {
         this.setEnabled(_snowman);
      }
   }
   ...
   public void tick() {
      ...
      if (!this.world.isClient && this.isAlive() && this.isEnabled()) {
         ...
         if (_snowman.equals(this.currentBlockPos)) {
   ...
   public boolean canOperate() {
      if (HopperBlockEntity.extract(this)) {
         ...
         List<ItemEntity> _snowman = this.world.getEntitiesByClass(ItemEntity.class, this.getBoundingBox().expand(0.25, 0.0, 0.25), EntityPredicates.VALID_ENTITY);
         if (!_snowman.isEmpty()) {
            HopperBlockEntity.extract(this, _snowman.get(0));
         }
