...
public class TntMinecartEntity extends AbstractMinecartEntity {
   ...
   public TntMinecartEntity(EntityType<? extends TntMinecartEntity> _snowman, World _snowman) {
   ...
   public void tick() {
      ...
      if (this.horizontalCollision) {
         double _snowman = squaredHorizontalLength(this.getVelocity());
         if (_snowman >= 0.01F) {
            this.explode(_snowman);
         }
      }
   }
   ...
   public boolean damage(DamageSource source, float amount) {
      ...
      if (_snowman instanceof PersistentProjectileEntity) {
         PersistentProjectileEntity _snowmanx = (PersistentProjectileEntity)_snowman;
         if (_snowmanx.isOnFire()) {
            this.explode(_snowmanx.getVelocity().lengthSquared());
         }
      }
   ...
   public void dropItems(DamageSource damageSource) {
      double _snowman = squaredHorizontalLength(this.getVelocity());
      if (!damageSource.isFire() && !damageSource.isExplosive() && !(_snowman >= 0.01F)) {
   ...
   protected void explode(double velocity) {
      if (!this.world.isClient) {
         double _snowman = Math.sqrt(velocity);
         if (_snowman > 5.0) {
            _snowman = 5.0;
         }

         this.world
            .createExplosion(this, this.getX(), this.getY(), this.getZ(), (float)(4.0 + this.random.nextDouble() * 1.5 * _snowman), Explosion.DestructionType.BREAK);
         this.remove();
      }
   }
   ...
   public boolean handleFallDamage(float fallDistance, float damageMultiplier) {
      if (fallDistance >= 3.0F) {
         float _snowman = fallDistance / 10.0F;
         this.explode((double)(_snowman * _snowman));
      }
