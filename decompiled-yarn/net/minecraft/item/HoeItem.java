...
public class HoeItem extends MiningToolItem {
   ...
   public ActionResult useOnBlock(ItemUsageContext context) {
      ...
      if (context.getSide() != Direction.DOWN && _snowman.getBlockState(_snowmanx.up()).isAir()) {
         BlockState _snowmanxx = TILLED_BLOCKS.get(_snowman.getBlockState(_snowmanx).getBlock());
         if (_snowmanxx != null) {
            PlayerEntity _snowmanxxx = context.getPlayer();
            _snowman.playSound(_snowmanxxx, _snowmanx, SoundEvents.ITEM_HOE_TILL, SoundCategory.BLOCKS, 1.0F, 1.0F);
            if (!_snowman.isClient) {
               _snowman.setBlockState(_snowmanx, _snowmanxx, 11);
               if (_snowmanxxx != null) {
