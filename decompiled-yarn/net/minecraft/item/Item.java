...
public class Item implements ItemConvertible {
   ...
   protected static BlockHitResult raycast(World world, PlayerEntity player, RaycastContext.FluidHandling fluidHandling) {
      float _snowman = player.pitch;
      float _snowmanx = player.yaw;
      Vec3d _snowmanxx = player.getCameraPosVec(1.0F);
      float _snowmanxxx = MathHelper.cos(-_snowmanx * (float) (Math.PI / 180.0) - (float) Math.PI);
      float _snowmanxxxx = MathHelper.sin(-_snowmanx * (float) (Math.PI / 180.0) - (float) Math.PI);
      float _snowmanxxxxx = -MathHelper.cos(-_snowman * (float) (Math.PI / 180.0));
      float _snowmanxxxxxx = MathHelper.sin(-_snowman * (float) (Math.PI / 180.0));
      ...
      Vec3d _snowmanxxxxxxxxxx = _snowmanxx.add((double)_snowmanxxxxxxx * 5.0, (double)_snowmanxxxxxx * 5.0, (double)_snowmanxxxxxxxx * 5.0);
      return world.raycast(new RaycastContext(_snowmanxx, _snowmanxxxxxxxxxx, RaycastContext.ShapeType.OUTLINE, fluidHandling, player));
   }
   ...
   protected boolean isIn(ItemGroup group) {
      ItemGroup _snowman = this.getGroup();
      return _snowman != null && (group == ItemGroup.SEARCH || group == _snowman);
   }
