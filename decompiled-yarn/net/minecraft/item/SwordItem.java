...
public class SwordItem extends ToolItem implements Vanishable {
   ...
   public SwordItem(ToolMaterial _snowman, int attackDamage, float attackSpeed, Item.Settings settings) {
      super(_snowman, settings);
      this.attackDamage = (float)attackDamage + _snowman.getAttackDamage();
   ...
   public float getMiningSpeedMultiplier(ItemStack stack, BlockState state) {
      if (state.isOf(Blocks.COBWEB)) {
         ...
         return _snowman != Material.PLANT && _snowman != Material.REPLACEABLE_PLANT && _snowman != Material.UNUSED_PLANT && !state.isIn(BlockTags.LEAVES) && _snowman != Material.GOURD
            ? 1.0F
            : 1.5F;
      }
   }
