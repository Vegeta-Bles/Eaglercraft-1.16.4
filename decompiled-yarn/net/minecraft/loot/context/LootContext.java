...
public class LootContext {
   ...
   public void drop(Identifier id, Consumer<ItemStack> lootConsumer) {
      LootContext.Dropper _snowman = this.drops.get(id);
      if (_snowman != null) {
         _snowman.add(this, lootConsumer);
      }
   }
   ...
   public static class Builder {
      ...
      public LootContext.Builder putDrop(Identifier id, LootContext.Dropper value) {
         LootContext.Dropper _snowman = this.drops.put(id, value);
         if (_snowman != null) {
      ...
      public <T> T get(LootContextParameter<T> parameter) {
         T _snowman = (T)this.parameters.get(parameter);
         if (_snowman == null) {
      ...
      public LootContext build(LootContextType type) {
         Set<LootContextParameter<?>> _snowman = Sets.difference(this.parameters.keySet(), type.getAllowed());
         if (!_snowman.isEmpty()) {
            throw new IllegalArgumentException("Parameters not allowed in this parameter set: " + _snowman);
         } else {
            Set<LootContextParameter<?>> _snowmanx = Sets.difference(type.getRequired(), this.parameters.keySet());
            if (!_snowmanx.isEmpty()) {
               throw new IllegalArgumentException("Missing required parameters: " + _snowmanx);
            } else {
               Random _snowmanxx = this.random;
               if (_snowmanxx == null) {
                  _snowmanxx = new Random();
               }

               MinecraftServer _snowmanxxx = this.world.getServer();
               return new LootContext(_snowmanxx, this.luck, this.world, _snowmanxxx.getLootManager()::getTable, _snowmanxxx.getPredicateManager()::get, this.parameters, this.drops);
            }
         }
      }
   }
   ...
   public interface Dropper {
      void add(LootContext context, Consumer<ItemStack> var2);
   }
   ...
   public static enum EntityTarget {
      ...
      public static LootContext.EntityTarget fromString(String type) {
         for (LootContext.EntityTarget _snowman : values()) {
            if (_snowman.type.equals(type)) {
               return _snowman;
            }
         }
      ...
      public static class Serializer extends TypeAdapter<LootContext.EntityTarget> {
         ...
         public void write(JsonWriter _snowman, LootContext.EntityTarget _snowman) throws IOException {
            _snowman.value(_snowman.type);
         }

         public LootContext.EntityTarget read(JsonReader _snowman) throws IOException {
            return LootContext.EntityTarget.fromString(_snowman.nextString());
         }
      }
   }
}
