...
public class LootContextType {
   ...
   public void validate(LootTableReporter reporter, LootContextAware parameterConsumer) {
      Set<LootContextParameter<?>> _snowman = parameterConsumer.getRequiredParameters();
      Set<LootContextParameter<?>> _snowmanx = Sets.difference(_snowman, this.allowed);
      if (!_snowmanx.isEmpty()) {
         reporter.report("Parameters " + _snowmanx + " are not provided in this context");
      }
   }
