...
public class LootFunctionTypes {
   ...
   private static LootFunctionType register(String id, JsonSerializer<? extends LootFunction> _snowman) {
      return Registry.register(Registry.LOOT_FUNCTION_TYPE, new Identifier(id), new LootFunctionType(_snowman));
   }
   ...
   public static BiFunction<ItemStack, LootContext, ItemStack> join(BiFunction<ItemStack, LootContext, ItemStack>[] lootFunctions) {
      switch (lootFunctions.length) {
         ...
         case 2:
            BiFunction<ItemStack, LootContext, ItemStack> _snowman = lootFunctions[0];
            BiFunction<ItemStack, LootContext, ItemStack> _snowmanx = lootFunctions[1];
            return (stack, context) -> _snowman.apply(_snowman.apply(stack, context), context);
         default:
            return (stack, context) -> {
               for (BiFunction<ItemStack, LootContext, ItemStack> _snowmanxx : lootFunctions) {
                  stack = _snowmanxx.apply(stack, context);
               }
