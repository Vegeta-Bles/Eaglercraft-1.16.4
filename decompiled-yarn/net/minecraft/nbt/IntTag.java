...
public class IntTag extends AbstractNumberTag {
   public static final TagReader<IntTag> READER = new TagReader<IntTag>() {
      public IntTag read(DataInput _snowman, int _snowman, PositionTracker _snowman) throws IOException {
         _snowman.add(96L);
         return IntTag.of(_snowman.readInt());
      }
   ...
   static class Cache {
      ...
      static {
         for (int _snowman = 0; _snowman < VALUES.length; _snowman++) {
            VALUES[_snowman] = new IntTag(-128 + _snowman);
         }
      }
   }
}
