...
public enum NetworkState {
   HANDSHAKING(
      ...
      createPacketHandlerInitializer()
         .setup(
            ...
            new NetworkState.PacketHandler<ServerHandshakePacketListener>().register(HandshakeC2SPacket.class, HandshakeC2SPacket::new)
         )
   ),
   PLAY(
      0,
      createPacketHandlerInitializer()
         .setup(
            ...
            new NetworkState.PacketHandler<ClientPlayPacketListener>()
         ...
         .setup(
            NetworkSide.SERVERBOUND,
            new NetworkState.PacketHandler<ServerPlayPacketListener>()
   ...
   LOGIN(
      ...
      createPacketHandlerInitializer()
         .setup(
            NetworkSide.CLIENTBOUND,
            new NetworkState.PacketHandler<ClientLoginPacketListener>()
         ...
         .setup(
            NetworkSide.SERVERBOUND,
            new NetworkState.PacketHandler<ServerLoginPacketListener>()
   ...
   private NetworkState(int id, NetworkState.PacketHandlerInitializer var4) {
      this.stateId = id;
      this.packetHandlers = _snowman.packetHandlers;
   }
   ...
   public Packet<?> getPacketHandler(NetworkSide side, int packetId) {
      return this.packetHandlers.get(side).createPacket(packetId);
   }
   ...
   static {
      for (NetworkState _snowman : values()) {
         int _snowmanx = _snowman.getId();
         if (_snowmanx < -1 || _snowmanx > 2) {
            throw new Error("Invalid protocol ID " + Integer.toString(_snowmanx));
         }

         STATES[_snowmanx - -1] = _snowman;
         _snowman.packetHandlers
            .forEach(
               (_snowmanxx, _snowmanxxx) -> _snowmanxxx.getPacketTypes()
                     .forEach(
                        _snowmanxxxx -> {
                           if (HANDLER_STATE_MAP.containsKey(_snowmanxxxx) && HANDLER_STATE_MAP.get(_snowmanxxxx) != _snowman) {
                              throw new IllegalStateException(
                                 "Packet " + _snowmanxxxx + " is already assigned to protocol " + HANDLER_STATE_MAP.get(_snowmanxxxx) + " - can't reassign to " + _snowman
                              );
                           } else {
                              HANDLER_STATE_MAP.put(_snowmanxxxx, _snowman);
                           }
                        }
                     )
            );
   ...
   static class PacketHandler<T extends PacketListener> {
      private final Object2IntMap<Class<? extends Packet<T>>> packetIds = Util.make(new Object2IntOpenHashMap(), _snowman -> _snowman.defaultReturnValue(-1));
      ...
      public <P extends Packet<T>> NetworkState.PacketHandler<T> register(Class<P> type, Supplier<P> factory) {
         int _snowman = this.packetFactories.size();
         int _snowmanx = this.packetIds.put(type, _snowman);
         if (_snowmanx != -1) {
            String _snowmanxx = "Packet " + type + " is already registered to ID " + _snowmanx;
            LogManager.getLogger().fatal(_snowmanxx);
            throw new IllegalArgumentException(_snowmanxx);
         } else {
      ...
      public Integer getId(Class<?> packet) {
         int _snowman = this.packetIds.getInt(packet);
         return _snowman == -1 ? null : _snowman;
      }

      @Nullable
      public Packet<?> createPacket(int id) {
         Supplier<? extends Packet<T>> _snowman = this.packetFactories.get(id);
         return (Packet<?>)(_snowman != null ? _snowman.get() : null);
      }
   ...
   static class PacketHandlerInitializer {
      ...
      public <T extends PacketListener> NetworkState.PacketHandlerInitializer setup(NetworkSide side, NetworkState.PacketHandler<T> _snowman) {
         this.packetHandlers.put(side, _snowman);
         return this;
      }
   }
}
