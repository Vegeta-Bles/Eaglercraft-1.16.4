...
public class EntityS2CPacket implements Packet<ClientPlayPacketListener> {
   ...
   public Vec3d calculateDeltaPosition(Vec3d orig) {
      double _snowman = this.deltaX == 0 ? orig.x : decodePacketCoordinate(encodePacketCoordinate(orig.x) + (long)this.deltaX);
      double _snowmanx = this.deltaY == 0 ? orig.y : decodePacketCoordinate(encodePacketCoordinate(orig.y) + (long)this.deltaY);
      double _snowmanxx = this.deltaZ == 0 ? orig.z : decodePacketCoordinate(encodePacketCoordinate(orig.z) + (long)this.deltaZ);
