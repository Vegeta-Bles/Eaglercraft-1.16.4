...
public class LightPredicate {
   ...
   public JsonElement toJson() {
      if (this == ANY) {
         ...
         _snowman.add("light", this.range.toJson());
         return _snowman;
      }
   }
   ...
   public static LightPredicate fromJson(@Nullable JsonElement json) {
      if (json != null && !json.isJsonNull()) {
         JsonObject _snowman = JsonHelper.asObject(json, "light");
         NumberRange.IntRange _snowmanx = NumberRange.IntRange.fromJson(_snowman.get("light"));
         return new LightPredicate(_snowmanx);
      } else {
         return ANY;
      }
   }
