...
public class NbtPredicate {
   ...
   public static NbtPredicate fromJson(@Nullable JsonElement json) {
      if (json != null && !json.isJsonNull()) {
         ...
         try {
            _snowman = StringNbtReader.parse(JsonHelper.asString(json, "nbt"));
         } catch (CommandSyntaxException var3) {
            throw new JsonSyntaxException("Invalid nbt tag: " + var3.getMessage());
         }

         return new NbtPredicate(_snowman);
      } else {
         return ANY;
      }
   }
   ...
   public static CompoundTag entityToTag(Entity entity) {
      CompoundTag _snowman = entity.toTag(new CompoundTag());
      if (entity instanceof PlayerEntity) {
         ItemStack _snowmanx = ((PlayerEntity)entity).inventory.getMainHandStack();
         if (!_snowmanx.isEmpty()) {
            _snowman.put("SelectedItem", _snowmanx.toTag(new CompoundTag()));
         }
      }
