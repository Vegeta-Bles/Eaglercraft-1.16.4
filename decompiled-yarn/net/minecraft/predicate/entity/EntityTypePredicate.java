...
public abstract class EntityTypePredicate {
   ...
   public static EntityTypePredicate fromJson(@Nullable JsonElement json) {
      if (json != null && !json.isJsonNull()) {
         String _snowman = JsonHelper.asString(json, "type");
         if (_snowman.startsWith("#")) {
            Identifier _snowmanx = new Identifier(_snowman.substring(1));
            return new EntityTypePredicate.Tagged(ServerTagManagerHolder.getTagManager().getEntityTypes().getTagOrEmpty(_snowmanx));
         } else {
            Identifier _snowmanx = new Identifier(_snowman);
            EntityType<?> _snowmanxx = Registry.ENTITY_TYPE
               .getOrEmpty(_snowmanx)
               .orElseThrow(
                  () -> new JsonSyntaxException("Unknown entity type '" + _snowman + "', valid types are: " + COMMA_JOINER.join(Registry.ENTITY_TYPE.getIds()))
               );
            return new EntityTypePredicate.Single(_snowmanxx);
         }
      } else {
         return ANY;
      }
