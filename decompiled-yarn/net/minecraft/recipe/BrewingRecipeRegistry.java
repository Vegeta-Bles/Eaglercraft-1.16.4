...
public class BrewingRecipeRegistry {
   ...
   private static final Predicate<ItemStack> POTION_TYPE_PREDICATE = _snowman -> {
      for (Ingredient _snowmanx : POTION_TYPES) {
   ...
   protected static boolean isItemRecipeIngredient(ItemStack stack) {
      ...
      for (int _snowmanx = ITEM_RECIPES.size(); _snowman < _snowmanx; _snowman++) {
         if (ITEM_RECIPES.get(_snowman).ingredient.test(stack)) {
            return true;
         }
      }
   ...
   protected static boolean isPotionRecipeIngredient(ItemStack stack) {
      ...
      for (int _snowmanx = POTION_RECIPES.size(); _snowman < _snowmanx; _snowman++) {
         if (POTION_RECIPES.get(_snowman).ingredient.test(stack)) {
            return true;
         }
      }
   ...
   public static boolean isBrewable(Potion potion) {
      ...
      for (int _snowmanx = POTION_RECIPES.size(); _snowman < _snowmanx; _snowman++) {
         if (POTION_RECIPES.get(_snowman).output == potion) {
            return true;
         }
      }
   ...
   protected static boolean hasItemRecipe(ItemStack input, ItemStack ingredient) {
      Item _snowman = input.getItem();
      int _snowmanx = 0;

      for (int _snowmanxx = ITEM_RECIPES.size(); _snowmanx < _snowmanxx; _snowmanx++) {
         BrewingRecipeRegistry.Recipe<Item> _snowmanxxx = ITEM_RECIPES.get(_snowmanx);
         if (_snowmanxxx.input == _snowman && _snowmanxxx.ingredient.test(ingredient)) {
            return true;
         }
      }
   ...
   protected static boolean hasPotionRecipe(ItemStack input, ItemStack ingredient) {
      Potion _snowman = PotionUtil.getPotion(input);
      int _snowmanx = 0;

      for (int _snowmanxx = POTION_RECIPES.size(); _snowmanx < _snowmanxx; _snowmanx++) {
         BrewingRecipeRegistry.Recipe<Potion> _snowmanxxx = POTION_RECIPES.get(_snowmanx);
         if (_snowmanxxx.input == _snowman && _snowmanxxx.ingredient.test(ingredient)) {
            return true;
         }
      }
   ...
   public static ItemStack craft(ItemStack input, ItemStack ingredient) {
      if (!ingredient.isEmpty()) {
         Potion _snowman = PotionUtil.getPotion(ingredient);
         Item _snowmanx = ingredient.getItem();
         int _snowmanxx = 0;

         for (int _snowmanxxx = ITEM_RECIPES.size(); _snowmanxx < _snowmanxxx; _snowmanxx++) {
            BrewingRecipeRegistry.Recipe<Item> _snowmanxxxx = ITEM_RECIPES.get(_snowmanxx);
            if (_snowmanxxxx.input == _snowmanx && _snowmanxxxx.ingredient.test(input)) {
               return PotionUtil.setPotion(new ItemStack(_snowmanxxxx.output), _snowman);
            }
         }

         _snowmanxx = 0;

         for (int _snowmanxxxx = POTION_RECIPES.size(); _snowmanxx < _snowmanxxxx; _snowmanxx++) {
            BrewingRecipeRegistry.Recipe<Potion> _snowmanxxxxx = POTION_RECIPES.get(_snowmanxx);
            if (_snowmanxxxxx.input == _snowman && _snowmanxxxxx.ingredient.test(input)) {
               return PotionUtil.setPotion(new ItemStack(_snowmanx), _snowmanxxxxx.output);
            }
         }
      }
   ...
   private static void registerPotionRecipe(Potion input, Item _snowman, Potion output) {
      POTION_RECIPES.add(new BrewingRecipeRegistry.Recipe<>(input, Ingredient.ofItems(_snowman), output));
   }
   ...
   static class Recipe<T> {
      ...
      public Recipe(T _snowman, Ingredient _snowman, T _snowman) {
         this.input = _snowman;
         this.ingredient = _snowman;
         this.output = _snowman;
      }
   }
}
