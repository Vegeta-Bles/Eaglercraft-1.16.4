...
public class ScoreboardCriterion {
   ...
   protected ScoreboardCriterion(String name, boolean readOnly, ScoreboardCriterion.RenderType _snowman) {
      this.name = name;
      this.readOnly = readOnly;
      this.criterionType = _snowman;
   ...
   public static Optional<ScoreboardCriterion> createStatCriterion(String name) {
      if (OBJECTIVES.containsKey(name)) {
         ...
         return _snowman < 0
            ? Optional.empty()
            : Registry.STAT_TYPE
               .getOrEmpty(Identifier.splitOn(name.substring(0, _snowman), '.'))
               .flatMap(_snowmanxx -> createStatCriterion((StatType<?>)_snowmanxx, Identifier.splitOn(name.substring(_snowman + 1), '.')));
      }
   }
   ...
   public static enum RenderType {
      ...
      static {
         Builder<String, ScoreboardCriterion.RenderType> _snowman = ImmutableMap.builder();

         for (ScoreboardCriterion.RenderType _snowmanx : values()) {
            _snowman.put(_snowmanx.name, _snowmanx);
         }

         CRITERION_TYPES = _snowman.build();
      }
   }
}
