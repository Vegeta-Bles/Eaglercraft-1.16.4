...
public abstract class BanEntry<T> extends ServerConfigEntry<T> {
   ...
   protected BanEntry(T key, JsonObject json) {
      ...
      Date _snowman;
      try {
         _snowman = json.has("created") ? DATE_FORMAT.parse(json.get("created").getAsString()) : new Date();
      } catch (ParseException var7) {
         _snowman = new Date();
      }

      this.creationDate = _snowman;
      this.source = json.has("source") ? json.get("source").getAsString() : "(Unknown)";

      Date _snowmanx;
      try {
         _snowmanx = json.has("expires") ? DATE_FORMAT.parse(json.get("expires").getAsString()) : null;
      } catch (ParseException var6) {
         _snowmanx = null;
      }

      this.expiryDate = _snowmanx;
