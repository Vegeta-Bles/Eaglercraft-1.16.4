...
public class RecipeCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("recipe").requires(_snowman -> _snowman.hasPermissionLevel(2)))
               .then(
                                 ...
                                 CommandManager.argument("recipe", IdentifierArgumentType.identifier())
                                    ...
                                    .executes(
                                       _snowman -> executeGive(
                                             ...
                                             Collections.singleton(IdentifierArgumentType.getRecipeArgument(_snowman, "recipe"))
                                          )
                                    )
                              ))
                           .then(
                              CommandManager.literal("*")
                                 .executes(
                                    _snowman -> executeGive(
                                          (ServerCommandSource)_snowman.getSource(),
                                          EntityArgumentType.getPlayers(_snowman, "targets"),
                                          ((ServerCommandSource)_snowman.getSource()).getMinecraftServer().getRecipeManager().values()
                                       )
                                 )
                           )
                     )
   ...
   private static int executeGive(ServerCommandSource source, Collection<ServerPlayerEntity> targets, Collection<Recipe<?>> recipes) throws CommandSyntaxException {
      ...
      for (ServerPlayerEntity _snowmanx : targets) {
         _snowman += _snowmanx.unlockRecipes(recipes);
      }
   ...
   private static int executeTake(ServerCommandSource source, Collection<ServerPlayerEntity> targets, Collection<Recipe<?>> recipes) throws CommandSyntaxException {
      ...
      for (ServerPlayerEntity _snowmanx : targets) {
         _snowman += _snowmanx.lockRecipes(recipes);
      }
