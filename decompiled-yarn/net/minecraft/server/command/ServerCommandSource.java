...
public class ServerCommandSource implements CommandSource {
   ...
   public ServerCommandSource mergeConsumers(ResultConsumer<ServerCommandSource> consumer, BinaryOperator<ResultConsumer<ServerCommandSource>> _snowman) {
      ResultConsumer<ServerCommandSource> _snowmanx = _snowman.apply(this.resultConsumer, consumer);
      return this.withConsumer(_snowmanx);
   }
   ...
   public ServerCommandSource withWorld(ServerWorld world) {
      if (world == this.world) {
         ...
         double _snowman = DimensionType.method_31109(this.world.getDimension(), world.getDimension());
         Vec3d _snowmanx = new Vec3d(this.position.x * _snowman, this.position.y, this.position.z * _snowman);
   ...
   public ServerCommandSource withLookingAt(Vec3d position) throws CommandSyntaxException {
      Vec3d _snowman = this.entityAnchor.positionAt(this);
      double _snowmanx = position.x - _snowman.x;
      double _snowmanxx = position.y - _snowman.y;
      double _snowmanxxx = position.z - _snowman.z;
      double _snowmanxxxx = (double)MathHelper.sqrt(_snowmanx * _snowmanx + _snowmanxxx * _snowmanxxx);
      float _snowmanxxxxx = MathHelper.wrapDegrees((float)(-(MathHelper.atan2(_snowmanxx, _snowmanxxxx) * 180.0F / (float)Math.PI)));
      float _snowmanxxxxxx = MathHelper.wrapDegrees((float)(MathHelper.atan2(_snowmanxxx, _snowmanx) * 180.0F / (float)Math.PI) - 90.0F);
      return this.withRotation(new Vec2f(_snowmanxxxxx, _snowmanxxxxxx));
   }
   ...
   private void sendToOps(Text message) {
      Text _snowman = new TranslatableText("chat.type.admin", this.getDisplayName(), message).formatted(new Formatting[]{Formatting.GRAY, Formatting.ITALIC});
      if (this.server.getGameRules().getBoolean(GameRules.SEND_COMMAND_FEEDBACK)) {
         for (ServerPlayerEntity _snowmanx : this.server.getPlayerManager().getPlayerList()) {
            if (_snowmanx != this.output && this.server.getPlayerManager().isOperator(_snowmanx.getGameProfile())) {
               _snowmanx.sendSystemMessage(_snowman, Util.NIL_UUID);
            }
         }
      }

      if (this.output != this.server && this.server.getGameRules().getBoolean(GameRules.LOG_ADMIN_COMMANDS)) {
         this.server.sendSystemMessage(_snowman, Util.NIL_UUID);
      }
   }
