...
public class SpectateCommand {
   ...
   private static final DynamicCommandExceptionType NOT_SPECTATOR_EXCEPTION = new DynamicCommandExceptionType(
      _snowman -> new TranslatableText("commands.spectate.not_spectator", _snowman)
   );

   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("spectate").requires(_snowman -> _snowman.hasPermissionLevel(2)))
               .executes(_snowman -> execute((ServerCommandSource)_snowman.getSource(), null, ((ServerCommandSource)_snowman.getSource()).getPlayer())))
            .then(
               ((RequiredArgumentBuilder)CommandManager.argument("target", EntityArgumentType.entity())
                     .executes(
                        _snowman -> execute(
                              (ServerCommandSource)_snowman.getSource(), EntityArgumentType.getEntity(_snowman, "target"), ((ServerCommandSource)_snowman.getSource()).getPlayer()
                           )
                     ))
                  .then(
                     CommandManager.argument("player", EntityArgumentType.player())
                        .executes(
                           _snowman -> execute(
                                 (ServerCommandSource)_snowman.getSource(), EntityArgumentType.getEntity(_snowman, "target"), EntityArgumentType.getPlayer(_snowman, "player")
                              )
                        )
                  )
            )
