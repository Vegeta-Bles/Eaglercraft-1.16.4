...
public final class ServerMBean implements DynamicMBean {
   ...
   private final Map<String, ServerMBean.Entry> entries = Stream.of(
      ...
      .collect(Collectors.toMap(_snowman -> _snowman.name, Function.identity()));

   private ServerMBean(MinecraftServer server) {
      this.server = server;
      MBeanAttributeInfo[] _snowman = this.entries.values().stream().map(_snowmanx -> _snowmanx.createInfo()).toArray(MBeanAttributeInfo[]::new);
      this.beanInfo = new MBeanInfo(ServerMBean.class.getSimpleName(), "metrics for dedicated server", _snowman, null, null, new MBeanNotificationInfo[0]);
   }
   ...
   public Object getAttribute(String attribute) {
      ServerMBean.Entry _snowman = this.entries.get(attribute);
      return _snowman == null ? null : _snowman.getter.get();
   }
   ...
   public AttributeList getAttributes(String[] attributes) {
      List<Attribute> _snowman = Arrays.stream(attributes)
         .map(this.entries::get)
         .filter(Objects::nonNull)
         .map(_snowmanx -> new Attribute(_snowmanx.name, _snowmanx.getter.get()))
