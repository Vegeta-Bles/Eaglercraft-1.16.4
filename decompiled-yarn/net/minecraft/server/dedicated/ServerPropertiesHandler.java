...
public class ServerPropertiesHandler extends AbstractPropertiesHandler<ServerPropertiesHandler> {
   ...
   public final int maxBuildHeight = this.transformedParseInt("max-build-height", _snowmanxx -> MathHelper.clamp((_snowmanxx + 8) / 16 * 16, 64, 256), 256);
   ...
   public ServerPropertiesHandler(Properties _snowman, DynamicRegistryManager _snowman) {
      super(_snowman);
      ...
      this.maxWorldSize = this.transformedParseInt("max-world-size", _snowmanxx -> MathHelper.clamp(_snowmanxx, 1, 29999984), 29999984);
      ...
      this.entityBroadcastRangePercentage = this.transformedParseInt("entity-broadcast-range-percentage", _snowmanxx -> MathHelper.clamp(_snowmanxx, 10, 1000), 100);
      ...
      this.generatorOptions = GeneratorOptions.fromProperties(_snowman, _snowman);
   }

   public static ServerPropertiesHandler load(DynamicRegistryManager _snowman, Path _snowman) {
      return new ServerPropertiesHandler(loadProperties(_snowman), _snowman);
   }

   protected ServerPropertiesHandler create(DynamicRegistryManager _snowman, Properties _snowman) {
      return new ServerPropertiesHandler(_snowman, _snowman);
   }
}
