...
public class DebugInfoSender {
   ...
   public static void addGameTestMarker(ServerWorld world, BlockPos pos, String message, int color, int duration) {
      PacketByteBuf _snowman = new PacketByteBuf(Unpooled.buffer());
      _snowman.writeBlockPos(pos);
      _snowman.writeInt(color);
      _snowman.writeString(message);
      _snowman.writeInt(duration);
      sendToAll(world, _snowman, CustomPayloadS2CPacket.DEBUG_GAME_TEST_ADD_MARKER);
   }

   public static void clearGameTestMarkers(ServerWorld world) {
      PacketByteBuf _snowman = new PacketByteBuf(Unpooled.buffer());
      sendToAll(world, _snowman, CustomPayloadS2CPacket.DEBUG_GAME_TEST_CLEAR);
   }
   ...
   private static void method_24819(ServerWorld _snowman, BlockPos _snowman) {
   }
   ...
   public static void sendStructureStart(StructureWorldAccess _snowman, StructureStart<?> structureStart) {
   }
   ...
   private static void sendToAll(ServerWorld world, PacketByteBuf buf, Identifier channel) {
      Packet<?> _snowman = new CustomPayloadS2CPacket(channel, buf);

      for (PlayerEntity _snowmanx : world.toServerWorld().getPlayers()) {
         ((ServerPlayerEntity)_snowmanx).networkHandler.sendPacket(_snowman);
      }
   }
}
