         ...
         case LOGIN:
            ...
            if (packet.getProtocolVersion() != SharedConstants.getGameVersion().getProtocolVersion()) {
               ...
               if (packet.getProtocolVersion() < 754) {
                  _snowman = new TranslatableText("multiplayer.disconnect.outdated_client", SharedConstants.getGameVersion().getName());
               } else {
                  _snowman = new TranslatableText("multiplayer.disconnect.incompatible", SharedConstants.getGameVersion().getName());
               }

               this.connection.send(new LoginDisconnectS2CPacket(_snowman));
               this.connection.disconnect(_snowman);
            } else {
