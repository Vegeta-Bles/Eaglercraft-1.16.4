...
public class ServerLoginNetworkHandler implements ServerLoginPacketListener {
   ...
   public void tick() {
      if (this.state == ServerLoginNetworkHandler.State.READY_TO_ACCEPT) {
         ...
         ServerPlayerEntity _snowman = this.server.getPlayerManager().getPlayer(this.profile.getId());
         if (_snowman == null) {
   ...
   public void acceptPlayer() {
      ...
      Text _snowman = this.server.getPlayerManager().checkCanJoin(this.connection.getAddress(), this.profile);
      if (_snowman != null) {
         this.disconnect(_snowman);
      } else {
         ...
         if (this.server.getNetworkCompressionThreshold() >= 0 && !this.connection.isLocal()) {
            this.connection
               .send(
                  ...
                  (ChannelFutureListener)_snowmanx -> this.connection.setCompressionThreshold(this.server.getNetworkCompressionThreshold())
               );
         }

         this.connection.send(new LoginSuccessS2CPacket(this.profile));
         ServerPlayerEntity _snowmanx = this.server.getPlayerManager().getPlayer(this.profile.getId());
         if (_snowmanx != null) {
   ...
   public void onKey(LoginKeyC2SPacket packet) {
      ...
      PrivateKey _snowman = this.server.getKeyPair().getPrivate();

      final String _snowmanx;
      try {
         if (!Arrays.equals(this.nonce, packet.decryptNonce(_snowman))) {
            throw new IllegalStateException("Protocol error");
         }

         this.secretKey = packet.decryptSecretKey(_snowman);
         Cipher _snowmanxx = NetworkEncryptionUtils.cipherFromKey(2, this.secretKey);
         Cipher _snowmanxxx = NetworkEncryptionUtils.cipherFromKey(1, this.secretKey);
         _snowmanx = new BigInteger(NetworkEncryptionUtils.generateServerId("", this.server.getKeyPair().getPublic(), this.secretKey)).toString(16);
         this.state = ServerLoginNetworkHandler.State.AUTHENTICATING;
         this.connection.setupEncryption(_snowmanxx, _snowmanxxx);
      } catch (NetworkEncryptionException var6) {
         throw new IllegalStateException("Protocol error", var6);
      }

      Thread _snowmanxx = new Thread("User Authenticator #" + authenticatorThreadId.incrementAndGet()) {
         @Override
         public void run() {
            GameProfile _snowman = ServerLoginNetworkHandler.this.profile;

            try {
               ServerLoginNetworkHandler.this.profile = ServerLoginNetworkHandler.this.server
                  .getSessionService()
                  .hasJoinedServer(new GameProfile(null, _snowman.getName()), _snowman, this.getClientAddress());
               if (ServerLoginNetworkHandler.this.profile != null) {
                  ...
                  ServerLoginNetworkHandler.this.profile = ServerLoginNetworkHandler.this.toOfflineProfile(_snowman);
                  ServerLoginNetworkHandler.this.state = ServerLoginNetworkHandler.State.READY_TO_ACCEPT;
               } else {
                  ServerLoginNetworkHandler.this.disconnect(new TranslatableText("multiplayer.disconnect.unverified_username"));
                  ServerLoginNetworkHandler.LOGGER.error("Username '{}' tried to join with an invalid session", _snowman.getName());
               }
         ...
         private InetAddress getClientAddress() {
            SocketAddress _snowman = ServerLoginNetworkHandler.this.connection.getAddress();
            return ServerLoginNetworkHandler.this.server.shouldPreventProxyConnections() && _snowman instanceof InetSocketAddress
               ? ((InetSocketAddress)_snowman).getAddress()
               : null;
         }
      };
      _snowmanxx.setUncaughtExceptionHandler(new UncaughtExceptionLogger(LOGGER));
      _snowmanxx.start();
   }
   ...
   protected GameProfile toOfflineProfile(GameProfile profile) {
      UUID _snowman = PlayerEntity.getOfflinePlayerUuid(profile.getName());
      return new GameProfile(_snowman, profile.getName());
   }
