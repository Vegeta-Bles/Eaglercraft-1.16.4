...
public class ChunkTaskPrioritySystem implements AutoCloseable, ChunkHolder.LevelUpdateListener {
                  ...
                  () -> {
                     ...
                     yield.send(
                        MessageListener.create(
                           ...
                           _snowmanxxxxx -> this.enqueueChunk(executor, _snowmanxxxxx.taskFunction, _snowmanxxxxx.pos, _snowmanxxxxx.lastLevelUpdatedToProvider, addBlocker)
                        )
                     );
                  }
               )
      ...
      return this.controlActor
         .<MessageListener<ChunkTaskPrioritySystem.UnblockingMessage>>ask(
            yield -> new TaskQueue.PrioritizedTask(
                  ...
                  () -> yield.send(
                        MessageListener.create(
                           "chunk priority sorter around " + executor.getName(), _snowmanxxx -> this.removeChunk(executor, _snowmanxxx.pos, _snowmanxxx.callback, _snowmanxxx.removeTask)
                        )
                     )
               )
         )
   ...
   public void updateLevel(ChunkPos pos, IntSupplier levelGetter, int targetLevel, IntConsumer levelSetter) {
      this.controlActor.send(new TaskQueue.PrioritizedTask(0, () -> {
         int _snowmanxxxx = levelGetter.getAsInt();
         this.queues.values().forEach(queue -> queue.updateLevel(_snowman, pos, targetLevel));
   ...
   private <T> void removeChunk(MessageListener<T> actor, long chunkPos, Runnable callback, boolean clearTask) {
      this.controlActor.send(new TaskQueue.PrioritizedTask(1, () -> {
         LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> _snowmanxxxx = this.getQueue(actor);
         _snowmanxxxx.remove(chunkPos, clearTask);
         if (this.idleActors.remove(actor)) {
            this.enqueueExecution(_snowmanxxxx, actor);
         }
   ...
   private <T> void enqueueChunk(
      ...
      this.controlActor.send(new TaskQueue.PrioritizedTask(2, () -> {
         LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> _snowmanxxxxx = this.getQueue(actor);
         int _snowmanx = lastLevelUpdatedToProvider.getAsInt();
         _snowmanxxxxx.add(Optional.of(task), chunkPos, _snowmanx);
         if (addBlocker) {
            _snowmanxxxxx.add(Optional.empty(), chunkPos, _snowmanx);
         }

         if (this.idleActors.remove(actor)) {
            this.enqueueExecution(_snowmanxxxxx, actor);
         }
      }));
   }
   ...
   private <T> void enqueueExecution(LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> queue, MessageListener<T> actor) {
      this.controlActor.send(new TaskQueue.PrioritizedTask(3, () -> {
         Stream<Either<Function<MessageListener<Unit>, T>, Runnable>> _snowmanxx = queue.poll();
         if (_snowmanxx == null) {
            this.idleActors.add(actor);
         } else {
            Util.combine(_snowmanxx.<CompletableFuture>map(executeOrAddBlocking -> (CompletableFuture)executeOrAddBlocking.map(actor::ask, addBlocking -> {
                  addBlocking.run();
                  return CompletableFuture.completedFuture(Unit.INSTANCE);
               })).collect(Collectors.toList())).thenAccept(_snowmanxx -> this.enqueueExecution(queue, actor));
         }
      }));
   }

   private <T> LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> getQueue(MessageListener<T> actor) {
      LevelPrioritizedQueue<? extends Function<MessageListener<Unit>, ?>> _snowman = this.queues.get(actor);
      if (_snowman == null) {
         throw (IllegalArgumentException)Util.throwOrPause(new IllegalArgumentException("No queue for: " + actor));
      } else {
         return (LevelPrioritizedQueue<Function<MessageListener<Unit>, T>>)_snowman;
      }
   }
   ...
   public String getDebugString() {
      return this.queues
            ...
            .stream()
            .map(
               _snowman -> _snowman.getKey().getName()
                     + "=["
                     + _snowman.getValue().getBlockingChunks().stream().map(_snowmanx -> _snowmanx + ":" + new ChunkPos(_snowmanx)).collect(Collectors.joining(","))
                     + "]"
            )
