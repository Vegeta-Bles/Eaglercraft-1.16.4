...
public abstract class Property<T extends Comparable<T>> {
   ...
   public Property.Value<T> createValue(State<?, ?> _snowman) {
      return new Property.Value<>(this, _snowman.get(this));
   }
   ...
   public boolean equals(Object _snowman) {
      if (this == _snowman) {
         return true;
      } else if (!(_snowman instanceof Property)) {
         return false;
      } else {
         Property<?> _snowmanx = (Property<?>)_snowman;
         return this.type.equals(_snowmanx.type) && this.name.equals(_snowmanx.name);
      }
   }
   ...
   public static final class Value<T extends Comparable<T>> {
      ...
      private Value(Property<T> _snowman, T value) {
         if (!_snowman.getValues().contains(value)) {
            throw new IllegalArgumentException("Value " + value + " does not belong to property " + _snowman);
         } else {
      ...
      public boolean equals(Object _snowman) {
         if (this == _snowman) {
            return true;
         } else if (!(_snowman instanceof Property.Value)) {
            return false;
         } else {
            Property.Value<?> _snowmanx = (Property.Value<?>)_snowman;
            return this.property == _snowmanx.property && this.value.equals(_snowmanx.value);
         }
      }

      @Override
      public int hashCode() {
         int _snowman = this.property.hashCode();
         return 31 * _snowman + this.value.hashCode();
      }
   }
}
