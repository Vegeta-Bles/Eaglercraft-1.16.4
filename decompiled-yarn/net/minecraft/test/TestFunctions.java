...
public class TestFunctions {
   ...
   public static Collection<TestFunction> getTestFunctions(String testClass) {
      return TEST_FUNCTIONS.stream().filter(_snowmanx -> isInClass(_snowmanx, testClass)).collect(Collectors.toList());
   }
   ...
   public static Optional<TestFunction> getTestFunction(String structurePath) {
      return getTestFunctions().stream().filter(_snowmanx -> _snowmanx.getStructurePath().equalsIgnoreCase(structurePath)).findFirst();
   }

   public static TestFunction getTestFunctionOrThrow(String structurePath) {
      Optional<TestFunction> _snowman = getTestFunction(structurePath);
   ...
   public static void method_29404(TestFunction _snowman) {
      field_25302.add(_snowman);
   }
