...
public final class TextColor {
   private static final Map<Formatting, TextColor> FORMATTING_TO_COLOR = Stream.of(Formatting.values())
      ...
      .collect(ImmutableMap.toImmutableMap(Function.identity(), _snowman -> new TextColor(_snowman.getColorValue(), _snowman.getName())));
   private static final Map<String, TextColor> BY_NAME = FORMATTING_TO_COLOR.values()
      .stream()
      .collect(ImmutableMap.toImmutableMap(_snowman -> _snowman.name, Function.identity()));
   ...
   public boolean equals(Object _snowman) {
      if (this == _snowman) {
         ...
         TextColor _snowmanx = (TextColor)_snowman;
         return this.rgb == _snowmanx.rgb;
      } else {
         return false;
      }
   }
   ...
   public static TextColor parse(String name) {
      if (name.startsWith("#")) {
         try {
            int _snowman = Integer.parseInt(name.substring(1), 16);
            return fromRgb(_snowman);
