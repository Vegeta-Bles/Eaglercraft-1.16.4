...
public enum Formatting {
   ...
   public static Formatting byColorIndex(int colorIndex) {
      if (colorIndex < 0) {
         ...
         for (Formatting _snowman : values()) {
            if (_snowman.getColorIndex() == colorIndex) {
               return _snowman;
            }
         }
   ...
   public static Formatting byCode(char code) {
      char _snowman = Character.toString(code).toLowerCase(Locale.ROOT).charAt(0);

      for (Formatting _snowmanx : values()) {
         if (_snowmanx.code == _snowman) {
            return _snowmanx;
         }
      }
   ...
   public static Collection<String> getNames(boolean colors, boolean modifiers) {
      ...
      for (Formatting _snowmanx : values()) {
         if ((!_snowmanx.isColor() || colors) && (!_snowmanx.isModifier() || modifiers)) {
            _snowman.add(_snowmanx.getName());
         }
      }
