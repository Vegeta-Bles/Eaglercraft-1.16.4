...
public class JsonHelper {
   ...
   public static Item asItem(JsonElement element, String name) {
      if (element.isJsonPrimitive()) {
         String _snowman = element.getAsString();
         return Registry.ITEM
            .getOrEmpty(new Identifier(_snowman))
            .orElseThrow(() -> new JsonSyntaxException("Expected " + name + " to be an item, was unknown string '" + _snowman + "'"));
      } else {
   ...
   public static String getType(JsonElement element) {
      String _snowman = StringUtils.abbreviateMiddle(String.valueOf(element), "...", 10);
      if (element == null) {
         return "null (missing)";
         ...
         if (element.isJsonPrimitive()) {
            JsonPrimitive _snowmanx = element.getAsJsonPrimitive();
   ...
   @Nullable
   public static <T> T deserialize(Gson _snowman, Reader _snowman, Class<T> type, boolean lenient) {
      try {
         JsonReader _snowmanxx = new JsonReader(_snowman);
         _snowmanxx.setLenient(lenient);
         return (T)_snowman.getAdapter(type).read(_snowmanxx);
   ...
   @Nullable
   public static <T> T deserialize(Gson _snowman, Reader _snowman, TypeToken<T> _snowman, boolean lenient) {
      try {
         JsonReader _snowmanxxx = new JsonReader(_snowman);
         _snowmanxxx.setLenient(lenient);
         return (T)_snowman.getAdapter(_snowman).read(_snowmanxxx);
      } catch (IOException var5) {
         throw new JsonParseException(var5);
      }
   }
   ...
   @Nullable
   public static <T> T deserialize(Gson _snowman, String content, TypeToken<T> _snowman, boolean lenient) {
      return deserialize(_snowman, new StringReader(content), _snowman, lenient);
   }

   @Nullable
   public static <T> T deserialize(Gson _snowman, String content, Class<T> _snowman, boolean lenient) {
      return deserialize(_snowman, new StringReader(content), _snowman, lenient);
   }

   @Nullable
   public static <T> T deserialize(Gson _snowman, Reader _snowman, TypeToken<T> _snowman) {
      return deserialize(_snowman, _snowman, _snowman, false);
   }

   @Nullable
   public static <T> T deserialize(Gson _snowman, String content, TypeToken<T> _snowman) {
      return deserialize(_snowman, content, _snowman, false);
   }

   @Nullable
   public static <T> T deserialize(Gson _snowman, Reader _snowman, Class<T> _snowman) {
      return deserialize(_snowman, _snowman, _snowman, false);
   }

   @Nullable
   public static <T> T deserialize(Gson _snowman, String content, Class<T> _snowman) {
      return deserialize(_snowman, content, _snowman, false);
   }
   ...
   public static JsonObject deserialize(Reader _snowman, boolean lenient) {
      return deserialize(GSON, _snowman, JsonObject.class, lenient);
   }
   ...
   public static JsonObject deserialize(Reader _snowman) {
      return deserialize(_snowman, false);
   }
}
