...
public class TickDurationMonitor {
   ...
   public void endTick() {
      if (this.profiler != DummyProfiler.INSTANCE) {
         ProfileResult _snowman = this.profiler.getResult();
         this.profiler = DummyProfiler.INSTANCE;
         if (_snowman.getTimeSpan() >= this.overtime) {
            File _snowmanx = new File(this.tickResultsDirectory, "tick-results-" + new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss").format(new Date()) + ".txt");
            _snowman.save(_snowmanx);
            LOGGER.info("Recorded long tick -- wrote info to: {}", _snowmanx.getAbsolutePath());
         }
      }
   }
