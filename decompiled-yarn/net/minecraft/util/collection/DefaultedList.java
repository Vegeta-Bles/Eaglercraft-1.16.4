...
public class DefaultedList<E> extends AbstractList<E> {
   ...
   public static <E> DefaultedList<E> ofSize(int size, E defaultValue) {
      ...
      Object[] _snowman = new Object[size];
      Arrays.fill(_snowman, defaultValue);
      return new DefaultedList<>(Arrays.asList((E[])_snowman), defaultValue);
   }
   ...
   public void clear() {
      if (this.initialElement == null) {
         ...
         for (int _snowman = 0; _snowman < this.size(); _snowman++) {
            this.set(_snowman, this.initialElement);
         }
      }
   }
}
