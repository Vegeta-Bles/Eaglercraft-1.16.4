...
public class WeightedList<U> {
   ...
   public WeightedList<U> shuffle(Random random) {
      ...
      this.entries.sort(Comparator.comparingDouble(object -> object.getShuffledOrder()));
      return this;
   }
      ...
      public static <E> Codec<WeightedList.Entry<E>> createCodec(Codec<E> codec) {
         return new Codec<WeightedList.Entry<E>>() {
            public <T> DataResult<Pair<WeightedList.Entry<E>, T>> decode(DynamicOps<T> ops, T object) {
               Dynamic<T> _snowman = new Dynamic(ops, object);
               return _snowman.get("data")
                  .flatMap(codec::parse)
                  .map(objectx -> new WeightedList.Entry(objectx, _snowman.get("weight").asInt(1)))
                  .map(entry -> Pair.of(entry, ops.empty()));
            }

            public <T> DataResult<T> encode(WeightedList.Entry<E> _snowman, DynamicOps<T> _snowman, T _snowman) {
               return _snowman.mapBuilder().add("weight", _snowman.createInt(_snowman.weight)).add("data", codec.encodeStart(_snowman, _snowman.item)).build(_snowman);
            }
         };
      }
   }
