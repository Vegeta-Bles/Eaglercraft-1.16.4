...
public final class DynamicSerializableUuid {
   public static final Codec<UUID> CODEC = Codec.INT_STREAM
      .comapFlatMap(_snowman -> Util.toIntArray(_snowman, 4).map(DynamicSerializableUuid::toUuid), _snowman -> Arrays.stream(toIntArray(_snowman)));
   ...
   public static int[] toIntArray(UUID uuid) {
      long _snowman = uuid.getMostSignificantBits();
      long _snowmanx = uuid.getLeastSignificantBits();
      return toIntArray(_snowman, _snowmanx);
   }
