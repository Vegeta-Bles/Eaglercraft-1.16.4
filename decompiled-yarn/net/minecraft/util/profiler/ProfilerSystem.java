...
public class ProfilerSystem implements ReadableProfiler {
   ...
   public void pop() {
      if (!this.tickStarted) {
         ...
         long _snowmanx = this.timeList.removeLong(this.timeList.size() - 1);
         this.path.remove(this.path.size() - 1);
         long _snowmanxx = _snowman - _snowmanx;
         ProfilerSystem.LocatedInfo _snowmanxxx = this.getCurrentInfo();
         _snowmanxxx.time = _snowmanxxx.time + _snowmanxx;
         _snowmanxxx.visits = _snowmanxxx.visits + 1L;
         if (this.checkTimeout && _snowmanxx > TIMEOUT_NANOSECONDS) {
            LOGGER.warn(
               "Something's taking too long! '{}' took aprox {} ms",
               new org.apache.logging.log4j.util.Supplier[]{() -> ProfileResult.getHumanReadableName(this.location), () -> (double)_snowman / 1000000.0}
            );
         }
   ...
   private ProfilerSystem.LocatedInfo getCurrentInfo() {
      if (this.currentInfo == null) {
         this.currentInfo = this.locationInfos.computeIfAbsent(this.location, _snowman -> new ProfilerSystem.LocatedInfo());
      }
