...
public abstract class Registry<T> implements Codec<T>, Keyable, IndexedIterable<T> {
   ...
   public static <T extends MutableRegistry<?>> void validate(MutableRegistry<T> registry) {
      registry.forEach(_snowmanx -> {
         if (_snowmanx.getIds().isEmpty()) {
            LOGGER.error("Registry '{}' was empty after loading", registry.getId((T)_snowmanx));
            if (SharedConstants.isDevelopment) {
               throw new IllegalStateException("Registry: '" + registry.getId((T)_snowmanx) + "' is empty, not allowed, fix me!");
            }
         }

         if (_snowmanx instanceof DefaultedRegistry) {
            Identifier _snowmanxx = ((DefaultedRegistry)_snowmanx).getDefaultId();
            Validate.notNull(_snowmanx.get(_snowmanxx), "Missing default of DefaultedMappedRegistry: " + _snowmanxx, new Object[0]);
         }
      });
   }

   private static <T> Registry<T> create(RegistryKey<? extends Registry<T>> registryRef, Supplier<T> defaultEntry) {
      return create(registryRef, Lifecycle.experimental(), defaultEntry);
   }

   private static <T> DefaultedRegistry<T> create(RegistryKey<? extends Registry<T>> registryRef, String defaultId, Supplier<T> defaultEntry) {
      return create(registryRef, defaultId, Lifecycle.experimental(), defaultEntry);
   }

   private static <T> Registry<T> create(RegistryKey<? extends Registry<T>> registryRef, Lifecycle lifecycle, Supplier<T> defaultEntry) {
      return create(registryRef, new SimpleRegistry<>(registryRef, lifecycle), defaultEntry, lifecycle);
   }

   private static <T> DefaultedRegistry<T> create(RegistryKey<? extends Registry<T>> registryRef, String defaultId, Lifecycle lifecycle, Supplier<T> defaultEntry) {
      return create(registryRef, new DefaultedRegistry<>(defaultId, registryRef, lifecycle), defaultEntry, lifecycle);
   }

   private static <T, R extends MutableRegistry<T>> R create(RegistryKey<? extends Registry<T>> registryRef, R registry, Supplier<T> defaultEntry, Lifecycle lifecycle) {
      Identifier identifier = registryRef.getValue();
      DEFAULT_ENTRIES.put(identifier, defaultEntry);
      MutableRegistry<R> rootRegistry = ROOT;
      return rootRegistry.add((RegistryKey<R>)registryRef, registry, lifecycle);
   }
   ...
   public <U> DataResult<Pair<T, U>> decode(DynamicOps<U> ops, U input) {
      return ops.compressMaps()
         ? ops.getNumberValue(input).flatMap(value -> {
            T entry = this.get(value.intValue());
            return entry == null ? DataResult.error("Unknown registry id: " + value) : DataResult.success(entry, this.getEntryLifecycle(entry));
         }).map(entry -> Pair.of(entry, ops.empty()))
         : Identifier.CODEC
            .decode(ops, input)
            .flatMap(
               result -> {
                  T entry = this.get((Identifier)result.getFirst());
                  return entry == null
                     ? DataResult.error("Unknown registry key: " + result.getFirst())
                     : DataResult.success(Pair.of(entry, result.getSecond()), this.getEntryLifecycle(entry));
               }
            );
   }

   public <U> DataResult<U> encode(T entry, DynamicOps<U> ops, U prefix) {
      Identifier id = this.getId(entry);
      if (id == null) {
         return DataResult.error("Unknown registry element " + entry);
      } else {
         return ops.compressMaps()
            ? ops.mergeToPrimitive(prefix, ops.createInt(this.getRawId(entry))).setLifecycle(this.lifecycle)
            : ops.mergeToPrimitive(prefix, ops.createString(id.toString())).setLifecycle(this.lifecycle);
      }
   }

   public <U> Stream<U> keys(DynamicOps<U> ops) {
      return this.getIds().stream().map(identifier -> (U)ops.createString(identifier.toString()));
   }
   ...
   static {
      ...
      DEFAULT_ENTRIES.forEach((_snowman, _snowmanx) -> {
