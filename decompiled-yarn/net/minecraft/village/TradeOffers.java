...
public class TradeOffers {
         ...
         map.put(
            ...
            copyToFastUtilMap(
               ImmutableMap.of(
                  ...
                  3,
                  new TradeOffers.Factory[]{
                     new TradeOffers.SellItemFactory(Items.COOKIE, 3, 18, 10), new TradeOffers.BuyForOneEmeraldFactory(Blocks.MELON, 4, 12, 20)
                  },
                  4,
                  new TradeOffers.Factory[]{
         ...
         map.put(
            ...
            copyToFastUtilMap(
               ImmutableMap.of(
                  ...
                  2,
                  new TradeOffers.Factory[]{
                     new TradeOffers.BuyForOneEmeraldFactory(Items.FLINT, 26, 12, 10), new TradeOffers.SellItemFactory(Items.BOW, 2, 1, 5)
                  },
                  3,
                  new TradeOffers.Factory[]{
                     new TradeOffers.BuyForOneEmeraldFactory(Items.STRING, 14, 16, 20), new TradeOffers.SellItemFactory(Items.CROSSBOW, 3, 1, 10)
                  },
                  4,
                  new TradeOffers.Factory[]{
         ...
         map.put(
            ...
            copyToFastUtilMap(
               ImmutableMap.of(
                  1,
                  new TradeOffers.Factory[]{
                     new TradeOffers.BuyForOneEmeraldFactory(Items.PAPER, 24, 16, 2), new TradeOffers.SellItemFactory(Items.MAP, 7, 1, 1)
                  },
                  2,
                  new TradeOffers.Factory[]{
   ...
   static class BuyForOneEmeraldFactory implements TradeOffers.Factory {
      ...
      public TradeOffer create(Entity entity, Random random) {
         ItemStack _snowman = new ItemStack(this.buy, this.price);
         return new TradeOffer(_snowman, new ItemStack(Items.EMERALD), this.maxUses, this.experience, this.multiplier);
      }
   }
   ...
   static class EnchantBookFactory implements TradeOffers.Factory {
      ...
      @Override
      public TradeOffer create(Entity entity, Random random) {
         List<Enchantment> _snowman = Registry.ENCHANTMENT.stream().filter(Enchantment::isAvailableForEnchantedBookOffer).collect(Collectors.toList());
         Enchantment _snowmanx = _snowman.get(random.nextInt(_snowman.size()));
         int _snowmanxx = MathHelper.nextInt(random, _snowmanx.getMinLevel(), _snowmanx.getMaxLevel());
         ItemStack _snowmanxxx = EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry(_snowmanx, _snowmanxx));
         int _snowmanxxxx = 2 + random.nextInt(5 + _snowmanxx * 10) + 3 * _snowmanxx;
         if (_snowmanx.isTreasure()) {
            _snowmanxxxx *= 2;
         }
         ...
         return new TradeOffer(new ItemStack(Items.EMERALD, _snowmanxxxx), new ItemStack(Items.BOOK), _snowmanxxx, 12, this.experience, 0.2F);
      }
   }
   ...
   static class SellDyedArmorFactory implements TradeOffers.Factory {
      ...
      @Override
      public TradeOffer create(Entity entity, Random random) {
         ItemStack _snowman = new ItemStack(Items.EMERALD, this.price);
         ItemStack _snowmanx = new ItemStack(this.sell);
         if (this.sell instanceof DyeableArmorItem) {
            List<DyeItem> _snowmanxx = Lists.newArrayList();
            _snowmanxx.add(getDye(random));
            ...
            _snowmanx = DyeableItem.blendAndSetColor(_snowmanx, _snowmanxx);
         }

         return new TradeOffer(_snowman, _snowmanx, this.maxUses, this.experience, 0.2F);
      }

      private static DyeItem getDye(Random _snowman) {
         return DyeItem.byColor(DyeColor.byId(_snowman.nextInt(16)));
      }
   }
   ...
   static class SellEnchantedToolFactory implements TradeOffers.Factory {
      ...
      @Override
      public TradeOffer create(Entity entity, Random random) {
         int _snowman = 5 + random.nextInt(15);
         ItemStack _snowmanx = EnchantmentHelper.enchant(random, new ItemStack(this.tool.getItem()), _snowman, false);
         int _snowmanxx = Math.min(this.basePrice + _snowman, 64);
         ItemStack _snowmanxxx = new ItemStack(Items.EMERALD, _snowmanxx);
         return new TradeOffer(_snowmanxxx, _snowmanx, this.maxUses, this.experience, this.multiplier);
      }
   }
   ...
   static class SellMapFactory implements TradeOffers.Factory {
      ...
      public TradeOffer create(Entity entity, Random random) {
         if (!(entity.world instanceof ServerWorld)) {
            ...
            ServerWorld _snowman = (ServerWorld)entity.world;
            BlockPos _snowmanx = _snowman.locateStructure(this.structure, entity.getBlockPos(), 100, true);
            if (_snowmanx != null) {
               ItemStack _snowmanxx = FilledMapItem.createMap(_snowman, _snowmanx.getX(), _snowmanx.getZ(), (byte)2, true, true);
               FilledMapItem.fillExplorationMap(_snowman, _snowmanxx);
               MapState.addDecorationsTag(_snowmanxx, _snowmanx, "+", this.iconType);
               _snowmanxx.setCustomName(new TranslatableText("filled_map." + this.structure.getName().toLowerCase(Locale.ROOT)));
               return new TradeOffer(new ItemStack(Items.EMERALD, this.price), new ItemStack(Items.COMPASS), _snowmanxx, this.maxUses, this.experience, 0.2F);
            } else {
               return null;
            }
         }
   ...
   static class SellPotionHoldingItemFactory implements TradeOffers.Factory {
      ...
      public TradeOffer create(Entity entity, Random random) {
         ItemStack _snowman = new ItemStack(Items.EMERALD, this.price);
         List<Potion> _snowmanx = Registry.POTION
            .stream()
            .filter(_snowmanxx -> !_snowmanxx.getEffects().isEmpty() && BrewingRecipeRegistry.isBrewable(_snowmanxx))
            .collect(Collectors.toList());
         Potion _snowmanxx = _snowmanx.get(random.nextInt(_snowmanx.size()));
         ItemStack _snowmanxxx = PotionUtil.setPotion(new ItemStack(this.sell.getItem(), this.sellCount), _snowmanxx);
         return new TradeOffer(_snowman, new ItemStack(this.secondBuy, this.secondCount), _snowmanxxx, this.maxUses, this.experience, this.priceMultiplier);
      }
   }
   ...
   static class SellSuspiciousStewFactory implements TradeOffers.Factory {
      ...
      @Override
      public TradeOffer create(Entity entity, Random random) {
         ItemStack _snowman = new ItemStack(Items.SUSPICIOUS_STEW, 1);
         SuspiciousStewItem.addEffectToStew(_snowman, this.effect, this.duration);
         return new TradeOffer(new ItemStack(Items.EMERALD, 1), _snowman, 12, this.experience, this.multiplier);
      }
   }
   ...
   static class TypeAwareBuyForOneEmeraldFactory implements TradeOffers.Factory {
      ...
      public TypeAwareBuyForOneEmeraldFactory(int count, int maxUses, int experience, Map<VillagerType, Item> map) {
         Registry.VILLAGER_TYPE.stream().filter(_snowmanx -> !map.containsKey(_snowmanx)).findAny().ifPresent(_snowman -> {
            throw new IllegalStateException("Missing trade for villager type: " + Registry.VILLAGER_TYPE.getId(_snowman));
         });
         this.map = map;
         this.count = count;
         this.maxUses = maxUses;
      ...
      public TradeOffer create(Entity entity, Random random) {
         if (entity instanceof VillagerDataContainer) {
            ItemStack _snowman = new ItemStack(this.map.get(((VillagerDataContainer)entity).getVillagerData().getType()), this.count);
            return new TradeOffer(_snowman, new ItemStack(Items.EMERALD), this.maxUses, this.experience, 0.05F);
         } else {
            return null;
         }
      }
