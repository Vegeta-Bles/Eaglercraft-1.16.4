...
public class ChunkRegion implements StructureWorldAccess {
   ...
   public ChunkRegion(ServerWorld world, List<Chunk> chunks) {
      int _snowman = MathHelper.floor(Math.sqrt((double)chunks.size()));
      if (_snowman * _snowman != chunks.size()) {
         throw (IllegalStateException)Util.throwOrPause(new IllegalStateException("Cache size is not a square."));
      } else {
         ChunkPos _snowmanx = chunks.get(chunks.size() / 2).getPos();
         this.chunks = chunks;
         this.centerChunkX = _snowmanx.x;
         this.centerChunkZ = _snowmanx.z;
         this.width = _snowman;
         this.world = world;
   ...
   public Chunk getChunk(int chunkX, int chunkZ, ChunkStatus leastStatus, boolean create) {
      ...
      if (this.isChunkLoaded(chunkX, chunkZ)) {
         int _snowmanx = chunkX - this.lowerCorner.x;
         int _snowmanxx = chunkZ - this.lowerCorner.z;
         _snowman = this.chunks.get(_snowmanx + _snowmanxx * this.width);
         if (_snowman.getStatus().isAtLeast(leastStatus)) {
            return _snowman;
         }
      } else {
         _snowman = null;
      ...
      if (!create) {
         ...
         if (_snowman != null) {
            throw (RuntimeException)Util.throwOrPause(
               new RuntimeException(String.format("Chunk is not of correct status. Expecting %s, got %s | %s %s", leastStatus, _snowman.getStatus(), chunkX, chunkZ))
            );
         } else {
            throw (RuntimeException)Util.throwOrPause(
               new RuntimeException(String.format("We are asking a region for a chunk out of bound | %s %s", chunkX, chunkZ))
            );
         }
      }
   }
   ...
   public boolean breakBlock(BlockPos pos, boolean drop, @Nullable Entity breakingEntity, int maxUpdateDepth) {
      ...
      if (_snowman.isAir()) {
         ...
         if (drop) {
            BlockEntity _snowmanx = _snowman.getBlock().hasBlockEntity() ? this.getBlockEntity(pos) : null;
            Block.dropStacks(_snowman, this.world, pos, _snowmanx, breakingEntity, ItemStack.EMPTY);
         }
   ...
   public BlockEntity getBlockEntity(BlockPos pos) {
      Chunk _snowman = this.getChunk(pos);
      BlockEntity _snowmanx = _snowman.getBlockEntity(pos);
      if (_snowmanx != null) {
         return _snowmanx;
      } else {
         CompoundTag _snowmanxx = _snowman.getBlockEntityTag(pos);
         BlockState _snowmanxxx = _snowman.getBlockState(pos);
         if (_snowmanxx != null) {
            if ("DUMMY".equals(_snowmanxx.getString("id"))) {
               Block _snowmanxxxx = _snowmanxxx.getBlock();
               if (!(_snowmanxxxx instanceof BlockEntityProvider)) {
                  return null;
               }

               _snowmanx = ((BlockEntityProvider)_snowmanxxxx).createBlockEntity(this.world);
            } else {
               _snowmanx = BlockEntity.createFromTag(_snowmanxxx, _snowmanxx);
            }

            if (_snowmanx != null) {
               _snowman.setBlockEntity(pos, _snowmanx);
               return _snowmanx;
            }
         }

         if (_snowmanxxx.getBlock() instanceof BlockEntityProvider) {
   ...
   public boolean setBlockState(BlockPos pos, BlockState state, int flags, int maxUpdateDepth) {
      Chunk _snowman = this.getChunk(pos);
      BlockState _snowmanx = _snowman.setBlockState(pos, state, false);
      if (_snowmanx != null) {
         this.world.onBlockChanged(pos, _snowmanx, state);
      }

      Block _snowmanxx = state.getBlock();
      if (_snowmanxx.hasBlockEntity()) {
         if (_snowman.getStatus().getChunkType() == ChunkStatus.ChunkType.field_12807) {
            _snowman.setBlockEntity(pos, ((BlockEntityProvider)_snowmanxx).createBlockEntity(this));
         } else {
            ...
            _snowmanxxx.putString("id", "DUMMY");
            _snowman.addPendingBlockEntityTag(_snowmanxxx);
         }
      } else if (_snowmanx != null && _snowmanx.getBlock().hasBlockEntity()) {
         _snowman.removeBlockEntity(pos);
      }
   ...
   public boolean spawnEntity(Entity entity) {
      ...
      this.getChunk(_snowman, _snowmanx).addEntity(entity);
      return true;
   }
