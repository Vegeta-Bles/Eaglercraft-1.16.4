...
public abstract class CommandBlockExecutor implements CommandOutput {
   ...
   public boolean execute(World world) {
      if (world.isClient || world.getTime() == this.lastExecution) {
         ...
         this.successCount = 0;
         MinecraftServer _snowman = this.getWorld().getServer();
         if (_snowman.areCommandBlocksEnabled() && !ChatUtil.isEmpty(this.command)) {
            try {
               this.lastOutput = null;
               ServerCommandSource _snowmanx = this.getSource().withConsumer((_snowmanxx, _snowmanxxx, _snowmanxxxx) -> {
                  if (_snowmanxxx) {
                     this.successCount++;
                  }
               });
               _snowman.getCommandManager().execute(_snowmanx, this.command);
            } catch (Throwable var6) {
               CrashReport _snowmanxx = CrashReport.create(var6, "Executing command block");
               CrashReportSection _snowmanxxx = _snowmanxx.addElement("Command to be executed");
               _snowmanxxx.add("Command", this::getCommand);
               _snowmanxxx.add("Name", () -> this.getCustomName().getString());
