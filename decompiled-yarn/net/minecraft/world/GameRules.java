...
public class GameRules {
   ...
   public static final GameRules.Key<GameRules.BooleanRule> REDUCED_DEBUG_INFO = register(
      "reducedDebugInfo", GameRules.Category.MISC, GameRules.BooleanRule.create(false, (server, rule) -> {
         byte _snowman = (byte)(rule.get() ? 22 : 23);

         for (ServerPlayerEntity _snowmanx : server.getPlayerManager().getPlayerList()) {
            _snowmanx.networkHandler.sendPacket(new EntityStatusS2CPacket(_snowmanx, _snowman));
         }
      })
   );
   ...
   public static final GameRules.Key<GameRules.BooleanRule> DO_IMMEDIATE_RESPAWN = register(
      "doImmediateRespawn", GameRules.Category.PLAYER, GameRules.BooleanRule.create(false, (server, rule) -> {
         for (ServerPlayerEntity _snowman : server.getPlayerManager().getPlayerList()) {
            _snowman.networkHandler.sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.IMMEDIATE_RESPAWN, rule.get() ? 1.0F : 0.0F));
         }
      })
   );
   ...
   private static <T extends GameRules.Rule<T>> GameRules.Key<T> register(String name, GameRules.Category category, GameRules.Type<T> type) {
      GameRules.Key<T> _snowman = new GameRules.Key<>(name, category);
      GameRules.Type<?> _snowmanx = RULE_TYPES.put(_snowman, type);
      if (_snowmanx != null) {
   ...
   public GameRules(DynamicLike<?> _snowman) {
      this();
      this.load(_snowman);
   }

   public GameRules() {
      this.rules = RULE_TYPES.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, e -> ((GameRules.Type)e.getValue()).createRule()));
   }
   ...
   public CompoundTag toNbt() {
      ...
      this.rules.forEach((key, rule) -> _snowman.putString(key.name, rule.serialize()));
      return _snowman;
   }

   private void load(DynamicLike<?> _snowman) {
      this.rules.forEach((key, rule) -> _snowman.get(key.name).asString().result().ifPresent(rule::deserialize));
   }

   public GameRules copy() {
      return new GameRules(this.rules.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, _snowman -> ((GameRules.Rule)_snowman.getValue()).copy())));
   }

   public static void accept(GameRules.Visitor visitor) {
      RULE_TYPES.forEach((key, type) -> accept(visitor, (GameRules.Key<?>)key, (GameRules.Type<?>)type));
   }
   ...
   private <T extends GameRules.Rule<T>> void setValue(GameRules.Key<T> key, GameRules rules, @Nullable MinecraftServer server) {
      T _snowman = rules.get(key);
      this.<T>get(key).setValue(_snowman, server);
   }
   ...
   public static class BooleanRule extends GameRules.Rule<GameRules.BooleanRule> {
      ...
      private static GameRules.Type<GameRules.BooleanRule> create(boolean initialValue, BiConsumer<MinecraftServer, GameRules.BooleanRule> changeCallback) {
         return new GameRules.Type<>(
            BoolArgumentType::bool, type -> new GameRules.BooleanRule(type, initialValue), changeCallback, GameRules.Visitor::visitBoolean
         );
      }
      ...
      public void setValue(GameRules.BooleanRule _snowman, @Nullable MinecraftServer _snowman) {
         this.value = _snowman.value;
         this.changed(_snowman);
      }
   }
   ...
   public static class IntRule extends GameRules.Rule<GameRules.IntRule> {
      ...
      private static GameRules.Type<GameRules.IntRule> create(int initialValue, BiConsumer<MinecraftServer, GameRules.IntRule> changeCallback) {
         return new GameRules.Type<>(
            IntegerArgumentType::integer, type -> new GameRules.IntRule(type, initialValue), changeCallback, GameRules.Visitor::visitInt
         );
      }
      ...
      public void setValue(GameRules.IntRule _snowman, @Nullable MinecraftServer _snowman) {
         this.value = _snowman.value;
         this.changed(_snowman);
      }
   }
