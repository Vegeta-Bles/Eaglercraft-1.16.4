...
public interface SaveProperties {
   ...
   default void populateCrashReport(CrashReportSection reportSection) {
      reportSection.add("Known server brands", () -> String.join(", ", this.getServerBrands()));
      reportSection.add("Level was modded", () -> Boolean.toString(this.isModded()));
      reportSection.add("Level storage version", () -> {
         int _snowman = this.getVersion();
         return String.format("0x%05X - %s", _snowman, this.getFormatName(_snowman));
      });
   }
   ...
   CompoundTag cloneWorldTag(DynamicRegistryManager var1, @Nullable CompoundTag var2);
