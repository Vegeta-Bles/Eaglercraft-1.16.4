...
public class BiomeEffects {
   public static final Codec<BiomeEffects> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
               Codec.INT.fieldOf("fog_color").forGetter(_snowmanx -> _snowmanx.fogColor),
               Codec.INT.fieldOf("water_color").forGetter(_snowmanx -> _snowmanx.waterColor),
               Codec.INT.fieldOf("water_fog_color").forGetter(_snowmanx -> _snowmanx.waterFogColor),
               Codec.INT.fieldOf("sky_color").forGetter(_snowmanx -> _snowmanx.skyColor),
               Codec.INT.optionalFieldOf("foliage_color").forGetter(_snowmanx -> _snowmanx.foliageColor),
               Codec.INT.optionalFieldOf("grass_color").forGetter(_snowmanx -> _snowmanx.grassColor),
               BiomeEffects.GrassColorModifier.CODEC
                  .optionalFieldOf("grass_color_modifier", BiomeEffects.GrassColorModifier.NONE)
                  .forGetter(_snowmanx -> _snowmanx.grassColorModifier),
               BiomeParticleConfig.CODEC.optionalFieldOf("particle").forGetter(_snowmanx -> _snowmanx.particleConfig),
               SoundEvent.CODEC.optionalFieldOf("ambient_sound").forGetter(_snowmanx -> _snowmanx.loopSound),
               BiomeMoodSound.CODEC.optionalFieldOf("mood_sound").forGetter(_snowmanx -> _snowmanx.moodSound),
               BiomeAdditionsSound.CODEC.optionalFieldOf("additions_sound").forGetter(_snowmanx -> _snowmanx.additionsSound),
               MusicSound.CODEC.optionalFieldOf("music").forGetter(_snowmanx -> _snowmanx.music)
            )
            .apply(_snowman, BiomeEffects::new)
   );
   ...
   public static enum GrassColorModifier implements StringIdentifiable {
      ...
      SWAMP("swamp") {
         @Override
         public int getModifiedGrassColor(double x, double z, int color) {
            double _snowman = Biome.FOLIAGE_NOISE.sample(x * 0.0225, z * 0.0225, false);
      ...
      private static final Map<String, BiomeEffects.GrassColorModifier> BY_NAME = Arrays.stream(values())
         .collect(Collectors.toMap(BiomeEffects.GrassColorModifier::getName, grassColorModifier -> (BiomeEffects.GrassColorModifier)grassColorModifier));
