...
public class GenerationSettings {
   ...
   public static final MapCodec<GenerationSettings> CODEC = RecordCodecBuilder.mapCodec(
      _snowman -> _snowman.group(
               ConfiguredSurfaceBuilder.REGISTRY_CODEC.fieldOf("surface_builder").forGetter(_snowmanx -> _snowmanx.surfaceBuilder),
               Codec.simpleMap(
   ...
   private GenerationSettings(
      ...
      this.flowerFeatures = features.stream()
         ...
         .filter(_snowman -> _snowman.feature == Feature.FLOWER)
   ...
   public boolean hasStructureFeature(StructureFeature<?> structureFeature) {
      return this.structureFeatures.stream().anyMatch(_snowmanx -> _snowmanx.get().feature == structureFeature);
   }
   ...
   public ConfiguredStructureFeature<?, ?> method_30978(ConfiguredStructureFeature<?, ?> _snowman) {
      return (ConfiguredStructureFeature<?, ?>)DataFixUtils.orElse(
         this.structureFeatures.stream().map(Supplier::get).filter(_snowmanxx -> _snowmanxx.feature == _snowman.feature).findAny(), _snowman
      );
   }
   ...
   public static class Builder {
      ...
      public <C extends CarverConfig> GenerationSettings.Builder carver(GenerationStep.Carver carverStep, ConfiguredCarver<C> carver) {
         this.carvers.computeIfAbsent(carverStep, _snowman -> Lists.newArrayList()).add(() -> carver);
         return this;
      }
      ...
      public GenerationSettings build() {
         return new GenerationSettings(
            ...
            this.carvers.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, _snowman -> ImmutableList.copyOf((Collection)_snowman.getValue()))),
