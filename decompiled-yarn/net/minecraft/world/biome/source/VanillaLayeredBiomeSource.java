...
public class VanillaLayeredBiomeSource extends BiomeSource {
   public static final Codec<VanillaLayeredBiomeSource> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
               ...
               Codec.BOOL.optionalFieldOf("legacy_biome_init_layer", false, Lifecycle.stable()).forGetter(_snowmanx -> _snowmanx.legacyBiomeInitLayer),
               Codec.BOOL.fieldOf("large_biomes").orElse(false).stable().forGetter(_snowmanx -> _snowmanx.largeBiomes),
               RegistryLookupCodec.of(Registry.BIOME_KEY).forGetter(_snowmanx -> _snowmanx.biomeRegistry)
            )
            .apply(_snowman, _snowman.stable(VanillaLayeredBiomeSource::new))
   );
   ...
   public VanillaLayeredBiomeSource(long seed, boolean legacyBiomeInitLayer, boolean largeBiomes, Registry<Biome> biomeRegistry) {
      super(BIOMES.stream().map(_snowmanx -> () -> biomeRegistry.getOrThrow((RegistryKey<Biome>)_snowmanx)));
      this.seed = seed;
