...
public interface Chunk extends BlockView, StructureHolder {
   ...
   default ChunkSection getHighestNonEmptySection() {
      ChunkSection[] _snowman = this.getSectionArray();

      for (int _snowmanx = _snowman.length - 1; _snowmanx >= 0; _snowmanx--) {
         ChunkSection _snowmanxx = _snowman[_snowmanx];
   ...
   default int getHighestNonEmptySectionYOffset() {
      ChunkSection _snowman = this.getHighestNonEmptySection();
      return _snowman == null ? 0 : _snowman.getYOffset();
   }
   ...
   default boolean areSectionsEmptyBetween(int lowerHeight, int upperHeight) {
      ...
      for (int _snowman = lowerHeight; _snowman <= upperHeight; _snowman += 16) {
         if (!ChunkSection.isEmpty(this.getSectionArray()[_snowman >> 4])) {
            return false;
         }
      }
   ...
   default void markBlockForPostProcessing(short _snowman, int _snowman) {
      getList(this.getPostProcessingLists(), _snowman).add(_snowman);
   }
