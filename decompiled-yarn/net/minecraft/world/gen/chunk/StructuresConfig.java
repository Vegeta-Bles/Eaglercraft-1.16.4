...
public class StructuresConfig {
   public static final Codec<StructuresConfig> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
            ...
            .apply(_snowman, StructuresConfig::new)
   );
   ...
   static {
      for (StructureFeature<?> _snowman : Registry.STRUCTURE_FEATURE) {
         if (!DEFAULT_STRUCTURES.containsKey(_snowman)) {
            throw new IllegalStateException("Structure feature without default settings: " + Registry.STRUCTURE_FEATURE.getId(_snowman));
         }
      }
