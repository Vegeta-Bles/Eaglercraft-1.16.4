...
public class BasaltColumnsFeatureConfig implements FeatureConfig {
   public static final Codec<BasaltColumnsFeatureConfig> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
               UniformIntDistribution.createValidatedCodec(0, 2, 1).fieldOf("reach").forGetter(_snowmanx -> _snowmanx.reach),
               UniformIntDistribution.createValidatedCodec(1, 5, 5).fieldOf("height").forGetter(_snowmanx -> _snowmanx.height)
            )
            .apply(_snowman, BasaltColumnsFeatureConfig::new)
   );
