...
public class EndGatewayFeatureConfig implements FeatureConfig {
   public static final Codec<EndGatewayFeatureConfig> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(BlockPos.CODEC.optionalFieldOf("exit").forGetter(_snowmanx -> _snowmanx.exitPos), Codec.BOOL.fieldOf("exact").forGetter(_snowmanx -> _snowmanx.exact))
            .apply(_snowman, EndGatewayFeatureConfig::new)
   );
