...
public class OreFeatureConfig implements FeatureConfig {
   public static final Codec<OreFeatureConfig> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
               RuleTest.field_25012.fieldOf("target").forGetter(_snowmanx -> _snowmanx.target),
               BlockState.CODEC.fieldOf("state").forGetter(_snowmanx -> _snowmanx.state),
               Codec.intRange(0, 64).fieldOf("size").forGetter(_snowmanx -> _snowmanx.size)
            )
            .apply(_snowman, OreFeatureConfig::new)
   );
