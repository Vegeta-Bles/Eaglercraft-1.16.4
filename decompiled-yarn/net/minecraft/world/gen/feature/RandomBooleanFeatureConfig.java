...
public class RandomBooleanFeatureConfig implements FeatureConfig {
   public static final Codec<RandomBooleanFeatureConfig> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
               ConfiguredFeature.REGISTRY_CODEC.fieldOf("feature_true").forGetter(_snowmanx -> _snowmanx.featureTrue),
               ConfiguredFeature.REGISTRY_CODEC.fieldOf("feature_false").forGetter(_snowmanx -> _snowmanx.featureFalse)
            )
            .apply(_snowman, RandomBooleanFeatureConfig::new)
   );
