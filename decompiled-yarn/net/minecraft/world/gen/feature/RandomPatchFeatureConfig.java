...
public class RandomPatchFeatureConfig implements FeatureConfig {
   public static final Codec<RandomPatchFeatureConfig> CODEC = RecordCodecBuilder.create(
      _snowman -> _snowman.group(
               BlockStateProvider.TYPE_CODEC.fieldOf("state_provider").forGetter(_snowmanx -> _snowmanx.stateProvider),
               BlockPlacer.TYPE_CODEC.fieldOf("block_placer").forGetter(_snowmanx -> _snowmanx.blockPlacer),
               BlockState.CODEC.listOf().fieldOf("whitelist").forGetter(_snowmanx -> _snowmanx.whitelist.stream().map(Block::getDefaultState).collect(Collectors.toList())),
               BlockState.CODEC.listOf().fieldOf("blacklist").forGetter(_snowmanx -> ImmutableList.copyOf(_snowmanx.blacklist)),
               Codec.INT.fieldOf("tries").orElse(128).forGetter(_snowmanx -> _snowmanx.tries),
               Codec.INT.fieldOf("xspread").orElse(7).forGetter(_snowmanx -> _snowmanx.spreadX),
               Codec.INT.fieldOf("yspread").orElse(3).forGetter(_snowmanx -> _snowmanx.spreadY),
               Codec.INT.fieldOf("zspread").orElse(7).forGetter(_snowmanx -> _snowmanx.spreadZ),
               Codec.BOOL.fieldOf("can_replace").orElse(false).forGetter(_snowmanx -> _snowmanx.canReplace),
               Codec.BOOL.fieldOf("project").orElse(true).forGetter(_snowmanx -> _snowmanx.project),
               Codec.BOOL.fieldOf("need_water").orElse(false).forGetter(_snowmanx -> _snowmanx.needsWater)
            )
            .apply(_snowman, RandomPatchFeatureConfig::new)
   );
