...
public abstract class FoliagePlacer {
   ...
   protected static <P extends FoliagePlacer> P2<Mu<P>, UniformIntDistribution, UniformIntDistribution> fillFoliagePlacerFields(Instance<P> _snowman) {
      return _snowman.group(
         UniformIntDistribution.createValidatedCodec(0, 8, 8).fieldOf("radius").forGetter(_snowmanx -> _snowmanx.radius),
         UniformIntDistribution.createValidatedCodec(0, 8, 8).fieldOf("offset").forGetter(_snowmanx -> _snowmanx.offset)
      );
   }
   ...
   protected boolean isPositionInvalid(Random random, int dx, int y, int dz, int radius, boolean giantTrunk) {
      ...
      int _snowmanx;
      if (giantTrunk) {
         _snowman = Math.min(Math.abs(dx), Math.abs(dx - 1));
         _snowmanx = Math.min(Math.abs(dz), Math.abs(dz - 1));
      } else {
         _snowman = Math.abs(dx);
         _snowmanx = Math.abs(dz);
      }

      return this.isInvalidForLeaves(random, _snowman, y, _snowmanx, radius, giantTrunk);
   }
   ...
   protected void generateSquare(
      ...
      int _snowman = giantTrunk ? 1 : 0;
      BlockPos.Mutable _snowmanx = new BlockPos.Mutable();

      for (int _snowmanxx = -radius; _snowmanxx <= radius + _snowman; _snowmanxx++) {
         for (int _snowmanxxx = -radius; _snowmanxxx <= radius + _snowman; _snowmanxxx++) {
            if (!this.isPositionInvalid(random, _snowmanxx, y, _snowmanxxx, radius, giantTrunk)) {
               _snowmanx.set(pos, _snowmanxx, y, _snowmanxxx);
               if (TreeFeature.canReplace(world, _snowmanx)) {
                  world.setBlockState(_snowmanx, config.leavesProvider.getBlockState(random, _snowmanx), 19);
                  box.encompass(new BlockBox(_snowmanx, _snowmanx));
                  leaves.add(_snowmanx.toImmutable());
               }
            }
         }
      }
