...
public class PineFoliagePlacer extends FoliagePlacer {
   public static final Codec<PineFoliagePlacer> CODEC = RecordCodecBuilder.create(
      _snowman -> fillFoliagePlacerFields(_snowman)
            .and(UniformIntDistribution.createValidatedCodec(0, 16, 8).fieldOf("height").forGetter(_snowmanx -> _snowmanx.height))
            .apply(_snowman, PineFoliagePlacer::new)
   );
   ...
   protected void generate(
      ...
      for (int _snowmanx = offset; _snowmanx >= offset - foliageHeight; _snowmanx--) {
         this.generateSquare(world, random, config, treeNode.getCenter(), _snowman, leaves, _snowmanx, treeNode.isGiantTrunk(), box);
         if (_snowman >= 1 && _snowmanx == offset - foliageHeight + 1) {
            _snowman--;
         } else if (_snowman < radius + treeNode.getFoliageRadius()) {
            _snowman++;
         }
      }
   }
