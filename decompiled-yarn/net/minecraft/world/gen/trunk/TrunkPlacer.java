...
public abstract class TrunkPlacer {
   ...
   protected static <P extends TrunkPlacer> P3<Mu<P>, Integer, Integer, Integer> method_28904(Instance<P> _snowman) {
      return _snowman.group(
         Codec.intRange(0, 32).fieldOf("base_height").forGetter(_snowmanx -> _snowmanx.baseHeight),
         Codec.intRange(0, 24).fieldOf("height_rand_a").forGetter(_snowmanx -> _snowmanx.firstRandomHeight),
         Codec.intRange(0, 24).fieldOf("height_rand_b").forGetter(_snowmanx -> _snowmanx.secondRandomHeight)
      );
   }
   ...
   private static boolean canGenerate(TestableWorld world, BlockPos pos) {
      return world.testBlockState(pos, state -> {
         ...
         return Feature.isSoil(_snowman) && !state.isOf(Blocks.GRASS_BLOCK) && !state.isOf(Blocks.MYCELIUM);
      });
   }
