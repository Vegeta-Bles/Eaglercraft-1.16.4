...
public class LevelProperties implements ServerWorldProperties, SaveProperties {
   ...
   public static LevelProperties readProperties(
      ...
      long _snowman = dynamic.get("Time").asLong(0L);
      CompoundTag _snowmanx = (CompoundTag)dynamic.get("DragonFight")
         .result()
      ...
      return new LevelProperties(
         ...
         _snowman,
         dynamic.get("DayTime").asLong(_snowman),
         ...
         dynamic.get("ServerBrands").asStream().flatMap(_snowmanxx -> Util.stream(_snowmanxx.asString().result())).collect(Collectors.toCollection(Sets::newLinkedHashSet)),
   ...
   @Override
   public CompoundTag cloneWorldTag(DynamicRegistryManager _snowman, @Nullable CompoundTag _snowman) {
      this.loadPlayerData();
      if (_snowman == null) {
         _snowman = this.playerData;
      }

      CompoundTag _snowmanxx = new CompoundTag();
      this.updateProperties(_snowman, _snowmanxx, _snowman);
      return _snowmanxx;
   }

   private void updateProperties(DynamicRegistryManager _snowman, CompoundTag _snowman, @Nullable CompoundTag _snowman) {
      ListTag _snowmanxxx = new ListTag();
      this.serverBrands.stream().map(StringTag::of).forEach(_snowmanxxx::add);
      _snowman.put("ServerBrands", _snowmanxxx);
      _snowman.putBoolean("WasModded", this.modded);
      CompoundTag _snowmanxxxx = new CompoundTag();
      _snowmanxxxx.putString("Name", SharedConstants.getGameVersion().getName());
      _snowmanxxxx.putInt("Id", SharedConstants.getGameVersion().getWorldVersion());
      _snowmanxxxx.putBoolean("Snapshot", !SharedConstants.getGameVersion().isStable());
      _snowman.put("Version", _snowmanxxxx);
      _snowman.putInt("DataVersion", SharedConstants.getGameVersion().getWorldVersion());
      RegistryReadingOps<Tag> _snowmanxxxxx = RegistryReadingOps.of(NbtOps.INSTANCE, _snowman);
      GeneratorOptions.CODEC
         .encodeStart(_snowmanxxxxx, this.generatorOptions)
         .resultOrPartial(Util.method_29188("WorldGenSettings: ", LOGGER::error))
         .ifPresent(_snowmanxxxxxx -> _snowman.put("WorldGenSettings", _snowmanxxxxxx));
      _snowman.putInt("GameType", this.levelInfo.getGameMode().getId());
      _snowman.putInt("SpawnX", this.spawnX);
      _snowman.putInt("SpawnY", this.spawnY);
      _snowman.putInt("SpawnZ", this.spawnZ);
      _snowman.putFloat("SpawnAngle", this.spawnAngle);
      _snowman.putLong("Time", this.time);
      _snowman.putLong("DayTime", this.timeOfDay);
      _snowman.putLong("LastPlayed", Util.getEpochTimeMs());
      _snowman.putString("LevelName", this.levelInfo.getLevelName());
      _snowman.putInt("version", 19133);
      _snowman.putInt("clearWeatherTime", this.clearWeatherTime);
      _snowman.putInt("rainTime", this.rainTime);
      _snowman.putBoolean("raining", this.raining);
      _snowman.putInt("thunderTime", this.thunderTime);
      _snowman.putBoolean("thundering", this.thundering);
      _snowman.putBoolean("hardcore", this.levelInfo.isHardcore());
      _snowman.putBoolean("allowCommands", this.levelInfo.areCommandsAllowed());
      _snowman.putBoolean("initialized", this.initialized);
      this.worldBorder.toTag(_snowman);
      _snowman.putByte("Difficulty", (byte)this.levelInfo.getDifficulty().getId());
      _snowman.putBoolean("DifficultyLocked", this.difficultyLocked);
      _snowman.put("GameRules", this.levelInfo.getGameRules().toNbt());
      _snowman.put("DragonFight", this.dragonFight);
      if (_snowman != null) {
         _snowman.put("Player", _snowman);
      }

      DataPackSettings.CODEC.encodeStart(NbtOps.INSTANCE, this.levelInfo.getDataPackSettings()).result().ifPresent(_snowmanxxxxxx -> _snowman.put("DataPacks", _snowmanxxxxxx));
      if (this.customBossEvents != null) {
         _snowman.put("CustomBossEvents", this.customBossEvents);
      }

      _snowman.put("ScheduledEvents", this.scheduledEvents.toTag());
      _snowman.putInt("WanderingTraderSpawnDelay", this.wanderingTraderSpawnDelay);
      _snowman.putInt("WanderingTraderSpawnChance", this.wanderingTraderSpawnChance);
      if (this.wanderingTraderId != null) {
         _snowman.putUuid("WanderingTraderId", this.wanderingTraderId);
      }
   }
