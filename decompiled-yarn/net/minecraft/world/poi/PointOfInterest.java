   ...
   public static Codec<PointOfInterest> createCodec(Runnable updateListener) {
      return RecordCodecBuilder.create(
         _snowmanx -> _snowmanx.group(
                  BlockPos.CODEC.fieldOf("pos").forGetter(_snowmanxx -> _snowmanxx.pos),
                  Registry.POINT_OF_INTEREST_TYPE.fieldOf("type").forGetter(_snowmanxx -> _snowmanxx.type),
                  Codec.INT.fieldOf("free_tickets").orElse(0).forGetter(_snowmanxx -> _snowmanxx.freeTickets),
                  RecordCodecBuilder.point(updateListener)
               )
               .apply(_snowmanx, PointOfInterest::new)
      );
   }
