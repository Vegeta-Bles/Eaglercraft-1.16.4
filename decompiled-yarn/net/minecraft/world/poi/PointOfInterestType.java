...
public class PointOfInterestType {
   private static final Supplier<Set<PointOfInterestType>> VILLAGER_WORKSTATIONS = Suppliers.memoize(
      () -> Registry.VILLAGER_PROFESSION.stream().map(VillagerProfession::getWorkStation).collect(Collectors.toSet())
   );
   public static final Predicate<PointOfInterestType> IS_USED_BY_PROFESSION = _snowman -> VILLAGER_WORKSTATIONS.get().contains(_snowman);
   public static final Predicate<PointOfInterestType> ALWAYS_TRUE = _snowman -> true;
   private static final Set<BlockState> BED_STATES = ImmutableList.of(
         Blocks.RED_BED,
         Blocks.BLACK_BED,
         Blocks.BLUE_BED,
         Blocks.BROWN_BED,
      ...
      .filter(_snowman -> _snowman.get(BedBlock.PART) == BedPart.HEAD)
   ...
   private PointOfInterestType(String id, Set<BlockState> blockStates, int ticketCount, int searchDistance) {
      ...
      this.blockStates = ImmutableSet.copyOf(blockStates);
      this.ticketCount = ticketCount;
      this.completionCondition = _snowman -> _snowman == this;
      this.searchDistance = searchDistance;
   }
   ...
   private static PointOfInterestType register(
      ...
      return setup(
         Registry.register(
            ...
            new Identifier(id),
            new PointOfInterestType(id, workStationStates, ticketCount, completionCondition, searchDistance)
         )
      );
   }

   private static PointOfInterestType setup(PointOfInterestType _snowman) {
      _snowman.blockStates.forEach(_snowmanxxx -> {
         PointOfInterestType _snowmanxx = BLOCK_STATE_TO_POINT_OF_INTEREST_TYPE.put(_snowmanxxx, _snowman);
         if (_snowmanxx != null) {
            throw (IllegalStateException)Util.throwOrPause(new IllegalStateException(String.format("%s is defined in too many tags", _snowmanxxx)));
         }
      });
      return _snowman;
   }
