...
public class ajh extends DataFix {
   ...
   private static final Set<String> b = a.keySet().stream().map(var0 -> var0.substring(0, var0.indexOf(46))).collect(Collectors.toSet());
   ...
   public ajh(Schema var1, boolean var2) {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", _snowman, var2x -> {
         Optional<Pair<String, String>> _snowmanxxx = var2x.getOptional(_snowman);
         if (!_snowmanxxx.isPresent()) {
            return var2x;
         } else {
            Typed<?> _snowmanx = var2x;
            Dynamic<?> _snowmanxx = (Dynamic<?>)var2x.get(DSL.remainderFinder());
            int _snowmanxxx = _snowmanxx.get("Damage").asInt(0);
            String _snowmanxxxx = a((String)_snowmanxxx.get().getSecond(), _snowmanxxx);
            if (_snowmanxxxx != null) {
               _snowmanx = var2x.set(_snowman, Pair.of(akn.r.typeName(), _snowmanxxxx));
            }

            if (c.contains(_snowmanxxx.get().getSecond())) {
               Typed<?> _snowmanxxxxx = var2x.getOrCreateTyped(_snowman);
               Dynamic<?> _snowmanxxxxxx = (Dynamic<?>)_snowmanxxxxx.get(DSL.remainderFinder());
               _snowmanxxxxxx = _snowmanxxxxxx.set("Damage", _snowmanxxxxxx.createInt(_snowmanxxx));
               _snowmanx = _snowmanx.set(_snowman, _snowmanxxxxx.set(DSL.remainderFinder(), _snowmanxxxxxx));
            }

            return _snowmanx.set(DSL.remainderFinder(), _snowmanxx.remove("Damage"));
         }
      });
   }

   @Nullable
   public static String a(@Nullable String var0, int var1) {
      if (b.contains(_snowman)) {
         String _snowman = a.get(_snowman + '.' + _snowman);
         return _snowman == null ? a.get(_snowman + ".0") : _snowman;
      } else {
         return null;
      }
   }
