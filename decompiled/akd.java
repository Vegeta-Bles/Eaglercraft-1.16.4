...
public class akd extends DataFix {
   ...
   public akd(Schema var1, boolean var2) {
   ...
   public TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped(
         ...
         var0 -> var0.update(DSL.remainderFinder(), var0x -> var0x.getMapValues().map(var1 -> var0x.createMap(var1.entrySet().stream().map(var0xx -> {
                     if (((Dynamic)var0xx.getKey()).asString("").startsWith("key_")) {
                        int _snowman = Integer.parseInt(((Dynamic)var0xx.getValue()).asString(""));
                        if (_snowman < 0) {
                           int _snowmanx = _snowman + 100;
                           String _snowmanxx;
                           if (_snowmanx == 0) {
                              _snowmanxx = "key.mouse.left";
                           } else if (_snowmanx == 1) {
                              _snowmanxx = "key.mouse.right";
                           } else if (_snowmanx == 2) {
                              _snowmanxx = "key.mouse.middle";
                           } else {
                              _snowmanxx = "key.mouse." + (_snowmanx + 1);
                           }

                           return Pair.of(var0xx.getKey(), ((Dynamic)var0xx.getValue()).createString(_snowmanxx));
                        } else {
                           String _snowmanx = (String)a.getOrDefault(_snowman, "key.unknown");
                           return Pair.of(var0xx.getKey(), ((Dynamic)var0xx.getValue()).createString(_snowmanx));
                        }
                     } else {
                        return Pair.of(var0xx.getKey(), var0xx.getValue());
                     }
