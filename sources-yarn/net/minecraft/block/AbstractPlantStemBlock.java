...
public abstract class AbstractPlantStemBlock extends AbstractPlantPartBlock implements Fertilizable {
   ...
   public BlockState getRandomGrowthState(WorldAccess arg) {
      return this.getDefaultState().with(AGE, Integer.valueOf(arg.getRandom().nextInt(25)));
   }
   ...
   public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
      if (state.get(AGE) < 25 && random.nextDouble() < this.growthChance) {
         ...
         if (this.chooseStemState(world.getBlockState(lv))) {
            world.setBlockState(lv, state.cycle(AGE));
         }
      }
   }
   ...
   public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
      ...
      for (int k = 0; k < j && this.chooseStemState(world.getBlockState(lv)); k++) {
         world.setBlockState(lv, state.with(AGE, Integer.valueOf(i)));
