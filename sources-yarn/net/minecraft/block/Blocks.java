...
public class Blocks {
   ...
   private static BedBlock createBedBlock(DyeColor color) {
      return new BedBlock(
         color,
         AbstractBlock.Settings.of(Material.WOOL, arg2 -> arg2.get(BedBlock.PART) == BedPart.FOOT ? color.getMaterialColor() : MaterialColor.WEB)
            .sounds(BlockSoundGroup.WOOD)
            .strength(0.2F)
            .nonOpaque()
      );
   ...
   private static PillarBlock createLogBlock(MaterialColor topMaterialColor, MaterialColor sideMaterialColor) {
      return new PillarBlock(
         AbstractBlock.Settings.of(Material.WOOD, arg3 -> arg3.get(PillarBlock.AXIS) == Direction.Axis.Y ? topMaterialColor : sideMaterialColor)
            .strength(2.0F)
            .sounds(BlockSoundGroup.WOOD)
      );
   }
   ...
   private static Block createNetherStemBlock(MaterialColor materialColor) {
      return new PillarBlock(AbstractBlock.Settings.of(Material.NETHER_WOOD, arg2 -> materialColor).strength(2.0F).sounds(BlockSoundGroup.NETHER_STEM));
   }
   ...
   private static ShulkerBoxBlock createShulkerBoxBlock(DyeColor color, AbstractBlock.Settings settings) {
      AbstractBlock.ContextPredicate lv = (arg, arg2, arg3) -> {
         BlockEntity lvx = arg2.getBlockEntity(arg3);
         if (!(lvx instanceof ShulkerBoxBlockEntity)) {
            return true;
         } else {
            ShulkerBoxBlockEntity lv2 = (ShulkerBoxBlockEntity)lvx;
            return lv2.suffocates();
         }
      };
      return new ShulkerBoxBlock(color, settings.strength(2.0F).dynamicBounds().nonOpaque().suffocates(lv).blockVision(lv));
   }

   private static PistonBlock createPistonBlock(boolean sticky) {
      AbstractBlock.ContextPredicate lv = (arg, arg2, arg3) -> !arg.get(PistonBlock.EXTENDED);
      return new PistonBlock(sticky, AbstractBlock.Settings.of(Material.PISTON).strength(1.5F).solidBlock(Blocks::never).suffocates(lv).blockVision(lv));
   }
