...
public class CampfireBlock extends BlockWithEntity implements Waterloggable {
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      ...
      if (lv instanceof CampfireBlockEntity) {
         ...
         Optional<CampfireCookingRecipe> optional = lv2.getRecipeFor(lv3);
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      ...
      return this.getDefaultState()
         ...
         .with(SIGNAL_FIRE, Boolean.valueOf(this.doesBlockCauseSignalFire(lv.getBlockState(lv2.down()))))
   ...
   public void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
      if (!world.isClient && projectile.isOnFire()) {
         ...
         boolean bl = lv == null || lv instanceof PlayerEntity || world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING);
         if (bl && !state.get(LIT) && !state.get(WATERLOGGED)) {
            BlockPos lv2 = hit.getBlockPos();
            world.setBlockState(lv2, state.with(Properties.LIT, Boolean.valueOf(true)), 11);
         }
      }
   }
   ...
   public static boolean method_30035(BlockState arg) {
      return arg.method_27851(BlockTags.CAMPFIRES, argx -> argx.contains(Properties.WATERLOGGED) && argx.contains(Properties.LIT))
         && !arg.get(Properties.WATERLOGGED)
         && !arg.get(Properties.LIT);
   }
}
