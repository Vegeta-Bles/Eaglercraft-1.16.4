   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      ...
      if (lv.isEmpty()) {
         ...
         if (lv2 == Items.WATER_BUCKET) {
            ...
            return ActionResult.success(world.isClient);
         } else if (lv2 == Items.GLASS_BOTTLE) {
            ...
            return ActionResult.success(world.isClient);
         } else if (lv2 == Items.POTION && PotionUtil.getPotion(lv) == Potions.WATER) {
            if (i < 3 && !world.isClient) {
               if (!player.abilities.creativeMode) {
            ...
            if (i > 0 && lv2 instanceof DyeableItem) {
               DyeableItem lv5 = (DyeableItem)lv2;
               if (lv5.hasColor(lv) && !world.isClient) {
                  lv5.removeColor(lv);
                  this.setLevel(world, pos, state, i - 1);
            ...
            if (i > 0 && lv2 instanceof BannerItem) {
               ...
               return ActionResult.success(world.isClient);
            } else if (i > 0 && lv2 instanceof BlockItem) {
   ...
   public void rainTick(World world, BlockPos pos) {
      if (world.random.nextInt(20) == 1) {
         ...
         if (!(f < 0.15F)) {
            ...
            if (lv.get(LEVEL) < 3) {
               world.setBlockState(pos, lv.cycle(LEVEL), 2);
            }
         }
      }
   }
