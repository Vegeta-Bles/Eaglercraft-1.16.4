...
public class ConnectingBlock extends Block {
   ...
   private VoxelShape[] generateFacingsToShapeMap(float radius) {
      float g = 0.5F - radius;
      float h = 0.5F + radius;
      VoxelShape lv = Block.createCuboidShape(
      ...
      for (int i = 0; i < FACINGS.length; i++) {
         ...
         lvs[i] = VoxelShapes.cuboid(
            0.5 + Math.min((double)(-radius), (double)lv2.getOffsetX() * 0.5),
            0.5 + Math.min((double)(-radius), (double)lv2.getOffsetY() * 0.5),
            0.5 + Math.min((double)(-radius), (double)lv2.getOffsetZ() * 0.5),
            0.5 + Math.max((double)radius, (double)lv2.getOffsetX() * 0.5),
            0.5 + Math.max((double)radius, (double)lv2.getOffsetY() * 0.5),
            0.5 + Math.max((double)radius, (double)lv2.getOffsetZ() * 0.5)
         );
      }
