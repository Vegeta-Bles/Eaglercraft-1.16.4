...
public class FireBlock extends AbstractFireBlock {
   ...
   public FireBlock(AbstractBlock.Settings arg) {
      ...
      this.field_26658 = ImmutableMap.copyOf(
         this.stateManager.getStates().stream().filter(argx -> argx.get(AGE) == 0).collect(Collectors.toMap(Function.identity(), FireBlock::method_31016))
      );
   }

   private static VoxelShape method_31016(BlockState arg) {
      ...
      if (arg.get(NORTH)) {
         lv = VoxelShapes.union(lv, field_26656);
      }

      if (arg.get(SOUTH)) {
         lv = VoxelShapes.union(lv, field_26657);
      }

      if (arg.get(EAST)) {
         lv = VoxelShapes.union(lv, field_26655);
      }

      if (arg.get(WEST)) {
         lv = VoxelShapes.union(lv, field_26654);
      }
   ...
   protected BlockState getStateForPosition(BlockView world, BlockPos pos) {
      ...
      if (!this.isFlammable(lv2) && !lv2.isSideSolidFullSquare(world, lv, Direction.UP)) {
         ...
         for (Direction lv4 : Direction.values()) {
            ...
            if (lv5 != null) {
               lv3 = lv3.with(lv5, Boolean.valueOf(this.isFlammable(world.getBlockState(pos.offset(lv4)))));
            }
         }
   ...
   private BlockState method_24855(WorldAccess arg, BlockPos arg2, int i) {
      ...
      return lv.isOf(Blocks.FIRE) ? lv.with(AGE, Integer.valueOf(i)) : lv;
   }
