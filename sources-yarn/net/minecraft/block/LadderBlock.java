...
public class LadderBlock extends Block implements Waterloggable {
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      if (!ctx.canReplaceExisting()) {
         BlockState lv = ctx.getWorld().getBlockState(ctx.getBlockPos().offset(ctx.getSide().getOpposite()));
      ...
      FluidState lv5 = ctx.getWorld().getFluidState(ctx.getBlockPos());

      for (Direction lv6 : ctx.getPlacementDirections()) {
