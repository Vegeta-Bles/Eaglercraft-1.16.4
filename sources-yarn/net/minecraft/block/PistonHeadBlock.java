...
public class PistonHeadBlock extends FacingBlock {
   ...
   private static VoxelShape[] method_31019(boolean bl) {
      return Arrays.stream(Direction.values()).map(arg -> getHeadShape(arg, bl)).toArray(VoxelShape[]::new);
   }

   private static VoxelShape getHeadShape(Direction arg, boolean bl) {
      switch (arg) {
         case DOWN:
         default:
   ...
   private boolean method_26980(BlockState arg, BlockState arg2) {
      ...
      return arg2.isOf(lv) && arg2.get(PistonBlock.EXTENDED) && arg2.get(FACING) == arg.get(FACING);
   }
