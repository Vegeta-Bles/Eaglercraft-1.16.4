...
public abstract class SpreadableBlock extends SnowyBlock {
   ...
   private static boolean canSpread(BlockState state, WorldView arg2, BlockPos pos) {
      BlockPos lv = pos.up();
      return canSurvive(state, arg2, pos) && !arg2.getFluidState(lv).isIn(FluidTags.WATER);
   }
         ...
         if (world.getLightLevel(pos.up()) >= 9) {
            ...
            for (int i = 0; i < 4; i++) {
               ...
               if (world.getBlockState(lv2).isOf(Blocks.DIRT) && canSpread(lv, world, lv2)) {
                  world.setBlockState(lv2, lv.with(SNOWY, Boolean.valueOf(world.getBlockState(lv2.up()).isOf(Blocks.SNOW))));
               }
            }
         }
      }
