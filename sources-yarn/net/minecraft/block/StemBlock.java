      ...
      if (world.getBaseLightLevel(pos, 0) >= 9) {
         ...
         if (random.nextInt((int)(25.0F / f) + 1) == 0) {
            ...
            if (i < 7) {
               ...
               if (world.getBlockState(lv2).isAir()
                  && (
                     lv3.isOf(Blocks.FARMLAND)
                        || lv3.isOf(Blocks.DIRT)
                        || lv3.isOf(Blocks.COARSE_DIRT)
                        || lv3.isOf(Blocks.PODZOL)
                        || lv3.isOf(Blocks.GRASS_BLOCK)
                  )) {
                  world.setBlockState(lv2, this.gourdBlock.getDefaultState());
                  world.setBlockState(pos, this.gourdBlock.getAttachedStem().getDefaultState().with(HorizontalFacingBlock.FACING, lv));
               }
            }
         }
      }
