...
public class TrapdoorBlock extends HorizontalFacingBlock implements Waterloggable {
      ...
      if (!world.isClient) {
         ...
         if (bl2 != state.get(POWERED)) {
            if (state.get(OPEN) != bl2) {
               state = state.with(OPEN, Boolean.valueOf(bl2));
               this.playToggleSound(null, world, pos, bl2);
            }

            world.setBlockState(pos, state.with(POWERED, Boolean.valueOf(bl2)), 2);
            if (state.get(WATERLOGGED)) {
               world.getFluidTickScheduler().schedule(pos, Fluids.WATER, Fluids.WATER.getTickRate(world));
            }
         }
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      ...
      if (!ctx.canReplaceExisting() && lv3.getAxis().isHorizontal()) {
         lv = lv.with(FACING, lv3).with(HALF, ctx.getHitPos().y - (double)ctx.getBlockPos().getY() > 0.5 ? BlockHalf.TOP : BlockHalf.BOTTOM);
      } else {
         lv = lv.with(FACING, ctx.getPlayerFacing().getOpposite()).with(HALF, lv3 == Direction.UP ? BlockHalf.BOTTOM : BlockHalf.TOP);
      }

      if (ctx.getWorld().isReceivingRedstonePower(ctx.getBlockPos())) {
         lv = lv.with(OPEN, Boolean.valueOf(true)).with(POWERED, Boolean.valueOf(true));
      }
