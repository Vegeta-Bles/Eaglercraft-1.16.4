   ...
   public void update(World world, BlockPos pos, BlockState state, boolean beingRemoved, boolean bl2, int i, @Nullable BlockState arg4) {
      ...
      for (int k = 1; k < 42; k++) {
         ...
         if (!lv3.isOf(Blocks.TRIPWIRE) && k != i) {
            ...
            boolean bl7 = !lv3.get(TripwireBlock.DISARMED);
      ...
      bl5 &= j > 1;
      bl6 &= bl5;
      BlockState lv4 = this.getDefaultState().with(ATTACHED, Boolean.valueOf(bl5)).with(POWERED, Boolean.valueOf(bl6));
      if (j > 0) {
         BlockPos lv5 = pos.offset(lv, j);
         Direction lv6 = lv.getOpposite();
         world.setBlockState(lv5, lv4.with(FACING, lv6), 3);
      ...
      if (!beingRemoved) {
         world.setBlockState(pos, lv4.with(FACING, lv), 3);
         if (bl2) {
      ...
      if (bl3 != bl5) {
         for (int l = 1; l < j; l++) {
            ...
            if (lv8 != null) {
               world.setBlockState(lv7, lv8.with(ATTACHED, Boolean.valueOf(bl5)), 3);
               if (!world.getBlockState(lv7).isAir()) {
               }
            }
         }
      }
