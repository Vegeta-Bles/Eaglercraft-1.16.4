...
public class BellBlockEntity extends BlockEntity implements Tickable {
   ...
   private void notifyMemoriesOfBell() {
      ...
      if (!this.world.isClient) {
         for (LivingEntity lv3 : this.hearingEntities) {
            if (lv3.isAlive() && !lv3.removed && lv.isWithinDistance(lv3.getPos(), 32.0)) {
   ...
   private boolean raidersHearBell() {
      ...
      for (LivingEntity lv2 : this.hearingEntities) {
         if (lv2.isAlive() && !lv2.removed && lv.isWithinDistance(lv2.getPos(), 32.0) && lv2.getType().isIn(EntityTypeTags.RAIDERS)) {
            return true;
         }
      }
            ...
            .forEach(
               arg3 -> {
                  ...
                  float g = MathHelper.sqrt(
                     (arg3.getX() - (double)lv.getX()) * (arg3.getX() - (double)lv.getX())
                        + (arg3.getZ() - (double)lv.getZ()) * (arg3.getZ() - (double)lv.getZ())
                  );
                  ...
                  for (int k = 0; k < j; k++) {
                     ...
                     world.addParticle(ParticleTypes.ENTITY_EFFECT, d, (double)((float)lv.getY() + 0.5F), e, h, m, n);
                  }
               }
            );
      }
