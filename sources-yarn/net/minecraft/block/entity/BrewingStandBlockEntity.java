...
public class BrewingStandBlockEntity extends LockableContainerBlockEntity implements SidedInventory, Tickable {
      ...
      if (!this.world.isClient) {
         ...
         if (!Arrays.equals(bls, this.slotsEmptyLastTick)) {
            ...
            for (int i = 0; i < BrewingStandBlock.BOTTLE_PROPERTIES.length; i++) {
               lv3 = lv3.with(BrewingStandBlock.BOTTLE_PROPERTIES[i], Boolean.valueOf(bls[i]));
            }
   ...
   public boolean isValid(int slot, ItemStack stack) {
      if (slot == 3) {
         ...
         return slot == 4
            ? lv == Items.BLAZE_POWDER
            : (lv == Items.POTION || lv == Items.SPLASH_POTION || lv == Items.LINGERING_POTION || lv == Items.GLASS_BOTTLE) && this.getStack(slot).isEmpty();
      }
   }
