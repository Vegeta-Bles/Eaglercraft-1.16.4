      ...
      for (int i = 0; i < this.itemsBeingCooked.size(); i++) {
         ...
         if (!lv.isEmpty()) {
            ...
            if (this.cookingTimes[i] >= this.cookingTotalTimes[i]) {
               Inventory lv2 = new SimpleInventory(lv);
               ItemStack lv3 = this.world
                  .getRecipeManager()
                  .getFirstMatch(RecipeType.CAMPFIRE_COOKING, lv2, this.world)
                  .map(arg2 -> arg2.craft(lv2))
                  .orElse(lv);
      ...
      if (lv != null) {
         ...
         for (int k = 0; k < this.itemsBeingCooked.size(); k++) {
            if (!this.itemsBeingCooked.get(k).isEmpty() && random.nextFloat() < 0.2F) {
               ...
               float f = 0.3125F;
               double d = (double)lv2.getX()
                  + 0.5
                  - (double)((float)lv3.getOffsetX() * 0.3125F)
                  + (double)((float)lv3.rotateYClockwise().getOffsetX() * 0.3125F);
               double e = (double)lv2.getY() + 0.5;
               double g = (double)lv2.getZ()
                  + 0.5
                  - (double)((float)lv3.getOffsetZ() * 0.3125F)
                  + (double)((float)lv3.rotateYClockwise().getOffsetZ() * 0.3125F);
