...
public class ConduitBlockEntity extends BlockEntity implements Tickable {
   ...
   private void givePlayersEffects() {
      ...
      int l = this.pos.getY();
      int m = this.pos.getZ();
      Box lv = new Box((double)k, (double)l, (double)m, (double)(k + 1), (double)(l + 1), (double)(m + 1))
         .expand((double)j)
      ...
      if (!list.isEmpty()) {
         for (PlayerEntity lv2 : list) {
            if (this.pos.isWithinDistance(lv2.getBlockPos(), (double)j) && lv2.isTouchingWaterOrRain()) {
   ...
   private void attackHostileEntity() {
      ...
      if (i < 42) {
         ...
         List<LivingEntity> list = this.world
            .getEntitiesByClass(LivingEntity.class, this.getAttackZone(), arg -> arg instanceof Monster && arg.isTouchingWaterOrRain());
         if (!list.isEmpty()) {
   ...
   private LivingEntity findTargetEntity() {
      List<LivingEntity> list = this.world.getEntitiesByClass(LivingEntity.class, this.getAttackZone(), arg -> arg.getUuid().equals(this.targetUuid));
   ...
   private void spawnNautilusParticles() {
      ...
      for (BlockPos lv2 : this.activatingBlocks) {
         if (random.nextInt(50) == 0) {
            ...
            Vec3d lv4 = new Vec3d((double)f, (double)g, (double)h).add((double)lv3.getX(), (double)lv3.getY(), (double)lv3.getZ());
      ...
      if (this.targetEntity != null) {
         ...
         Vec3d lv6 = new Vec3d((double)i, (double)j, (double)k);
