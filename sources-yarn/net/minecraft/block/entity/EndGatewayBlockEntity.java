...
public class EndGatewayBlockEntity extends EndPortalBlockEntity implements Tickable {
   ...
   public static boolean method_30276(Entity arg) {
      return EntityPredicates.EXCEPT_SPECTATOR.test(arg) && !arg.getRootVehicle().hasNetherPortalCooldown();
   }
      ...
      if (this.world instanceof ServerWorld && !this.needsCooldownBeforeTeleporting()) {
         ...
         if (this.exitPortalPos != null) {
            ...
            if (arg instanceof EnderPearlEntity) {
               ...
               lv3 = arg.getRootVehicle();
            }

            lv3.resetNetherPortalCooldown();
            lv3.teleport((double)lv.getX() + 0.5, (double)lv.getY(), (double)lv.getZ() + 0.5);
         }
   ...
   private BlockPos findBestPortalExitPos() {
      BlockPos lv = findExitPortalPos(this.world, this.exitPortalPos.add(0, 2, 0), 5, false);
      LOGGER.debug("Best exit position for portal at {} is {}", this.exitPortalPos, lv);
      return lv.up();
   }

   private void createPortal(ServerWorld world) {
      Vec3d lv = new Vec3d((double)this.getPos().getX(), 0.0, (double)this.getPos().getZ()).normalize();
      Vec3d lv2 = lv.multiply(1024.0);

      for (int i = 16; getChunk(world, lv2).getHighestNonEmptySectionYOffset() > 0 && i-- > 0; lv2 = lv2.add(lv.multiply(-16.0))) {
         LOGGER.debug("Skipping backwards past nonempty chunk at {}", lv2);
      }

      for (int var6 = 16; getChunk(world, lv2).getHighestNonEmptySectionYOffset() == 0 && var6-- > 0; lv2 = lv2.add(lv.multiply(16.0))) {
         LOGGER.debug("Skipping forward past empty chunk at {}", lv2);
      }

      LOGGER.debug("Found chunk at {}", lv2);
      WorldChunk lv3 = getChunk(world, lv2);
      this.exitPortalPos = findPortalPosition(lv3);
      if (this.exitPortalPos == null) {
         ...
         for (int k = -searchRadius; k <= searchRadius; k++) {
            if (j != 0 || k != 0 || bl) {
               for (int l = 255; l > (lv == null ? 0 : lv.getY()); l--) {
                  ...
                  if (lv3.isFullCube(world, lv2) && (bl || !lv3.isOf(Blocks.BEDROCK))) {
                     lv = lv2;
                     break;
                  }
               }
   ...
   private static BlockPos findPortalPosition(WorldChunk chunk) {
      ...
      for (BlockPos lv5 : BlockPos.iterate(lv2, lv3)) {
         ...
         if (lv6.isOf(Blocks.END_STONE) && !chunk.getBlockState(lv7).isFullCube(chunk, lv7) && !chunk.getBlockState(lv8).isFullCube(chunk, lv8)) {
