...
public class ShulkerBoxBlockEntity extends LootableContainerBlockEntity implements SidedInventory, Tickable {
   ...
   private Box getCollisionBox(Direction facing) {
      ...
      return this.getBoundingBox(facing).shrink((double)lv.getOffsetX(), (double)lv.getOffsetY(), (double)lv.getOffsetZ());
   }
         ...
         if (!list.isEmpty()) {
            for (int i = 0; i < list.size(); i++) {
               ...
               if (lv4.getPistonBehavior() != PistonBehavior.IGNORE) {
                  ...
                  switch (lv2.getAxis()) {
                     case X:
                        if (lv2.getDirection() == Direction.AxisDirection.POSITIVE) {
                           d = lv3.maxX - lv5.minX;
                        } else {
                           d = lv5.maxX - lv3.minX;
                        }
