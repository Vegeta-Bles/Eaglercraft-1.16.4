...
public class BlockPattern {
         ...
         for (Direction lv2 : Direction.values()) {
            for (Direction lv3 : Direction.values()) {
               if (lv3 != lv2 && lv3 != lv2.getOpposite()) {
                  BlockPattern.Result lv4 = this.testTransform(lv, lv2, lv3, loadingCache);
                  if (lv4 != null) {
                     return lv4;
                  }
               }
   ...
   protected static BlockPos translate(BlockPos pos, Direction forwards, Direction up, int offsetLeft, int offsetDown, int offsetForwards) {
      if (forwards != up && forwards != up.getOpposite()) {
         ...
         return pos.add(
            lv2.getX() * -offsetDown + lv3.getX() * offsetLeft + lv.getX() * offsetForwards,
            lv2.getY() * -offsetDown + lv3.getY() * offsetLeft + lv.getY() * offsetForwards,
            lv2.getZ() * -offsetDown + lv3.getZ() * offsetLeft + lv.getZ() * offsetForwards
         );
      } else {
