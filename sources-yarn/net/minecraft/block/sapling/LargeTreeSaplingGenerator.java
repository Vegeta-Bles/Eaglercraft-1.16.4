...
public abstract class LargeTreeSaplingGenerator extends SaplingGenerator {
   ...
   public boolean generate(ServerWorld arg, ChunkGenerator arg2, BlockPos arg3, BlockState arg4, Random random) {
      for (int i = 0; i >= -1; i--) {
         for (int j = 0; j >= -1; j--) {
            if (canGenerateLargeTree(arg4, arg, arg3, i, j)) {
   ...
   public static boolean canGenerateLargeTree(BlockState state, BlockView world, BlockPos pos, int x, int z) {
      ...
      return lv == world.getBlockState(pos.add(x, 0, z)).getBlock()
         && lv == world.getBlockState(pos.add(x + 1, 0, z)).getBlock()
         && lv == world.getBlockState(pos.add(x, 0, z + 1)).getBlock()
         && lv == world.getBlockState(pos.add(x + 1, 0, z + 1)).getBlock();
   }
}
