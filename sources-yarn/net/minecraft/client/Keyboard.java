...
public class Keyboard {
               ...
               if (bl) {
                  if (bl2) {
                     this.client.player.networkHandler.getDataQueryHandler().queryEntityNbt(lv6.getEntityId(), arg3 -> {
                        this.copyEntity(lv7, lv6.getPos(), arg3);
   ...
   public void onKey(long window, int key, int scancode, int k, int m) {
      if (window == this.client.getWindow().getHandle()) {
         ...
         ParentElement lv = this.client.currentScreen;
         if (k == 1 && (!(this.client.currentScreen instanceof ControlsOptionsScreen) || ((ControlsOptionsScreen)lv).time <= Util.getMeasuringTimeMs() - 20L)) {
            ...
            if (this.client.options.keyScreenshot.matchesKey(key, scancode)) {
               ...
               ScreenshotUtils.saveScreenshot(
                  ...
                  arg -> this.client.execute(() -> this.client.inGameHud.getChatHud().addMessage(arg))
               );
               return;
            }
         }
         ...
         if (k != 0 && key == 66 && Screen.hasControlDown() && bl) {
            ...
            if (lv instanceof NarratorOptionsScreen) {
               ((NarratorOptionsScreen)lv).updateNarratorButtonText();
            }
         }
