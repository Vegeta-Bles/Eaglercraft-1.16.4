...
public class GlShader {
   ...
   public static GlShader createFromResource(GlShader.Type type, String name, InputStream sourceCode, String string2) throws IOException {
      RenderSystem.assertThread(RenderSystem::isOnRenderThread);
      String string3 = TextureUtil.readAllToString(sourceCode);
      if (string3 == null) {
         throw new IOException("Could not load program " + type.getName());
      } else {
         int i = GlStateManager.createShader(type.getGlType());
         GlStateManager.shaderSource(i, string3);
         ...
         if (GlStateManager.getShader(i, 35713) == 0) {
            ...
            throw new IOException("Couldn't compile " + type.getName() + " program (" + string2 + ", " + name + ") : " + string4);
         } else {
            GlShader lv = new GlShader(type, i, name);
