...
public class PlayerListHud extends DrawableHelper {
   ...
   public Text getPlayerName(PlayerListEntry arg) {
      return arg.getDisplayName() != null
         ? this.method_27538(arg, arg.getDisplayName().shallowCopy())
         : this.method_27538(arg, Team.modifyText(arg.getScoreboardTeam(), new LiteralText(arg.getProfile().getName())));
   }
   ...
   public void render(MatrixStack arg, int i, Scoreboard arg2, @Nullable ScoreboardObjective arg3) {
      ...
      List<PlayerListEntry> list = ENTRY_ORDERING.sortedCopy(lv.getPlayerList());
      int j = 0;
      int k = 0;
      ...
      for (PlayerListEntry lv2 : list) {
         ...
         j = Math.max(j, l);
         if (arg3 != null && arg3.getRenderType() != ScoreboardCriterion.RenderType.HEARTS) {
      ...
      if (this.header != null) {
         ...
         for (OrderedText lv3 : list2) {
            v = Math.max(v, this.client.textRenderer.getWidth(lv3));
         }
      }
      ...
      if (this.footer != null) {
         ...
         for (OrderedText lv4 : list3) {
            v = Math.max(v, this.client.textRenderer.getWidth(lv4));
         }
      }

      if (list2 != null) {
         fill(arg, i / 2 - v / 2 - 1, u - 1, i / 2 + v / 2 + 1, u + list2.size() * 9, Integer.MIN_VALUE);

         for (OrderedText lv5 : list2) {
      ...
      fill(arg, i / 2 - v / 2 - 1, u - 1, i / 2 + v / 2 + 1, u + n * 9, Integer.MIN_VALUE);
      ...
      for (int y = 0; y < m; y++) {
         ...
         if (y < list.size()) {
            ...
            if (bl) {
               ...
               int ad = 8 + (bl2 ? 8 : 0);
               int ae = 8 * (bl2 ? -1 : 1);
               DrawableHelper.drawTexture(arg, ab, ac, 8, 8, 8.0F, (float)ad, 8, ae, 64, 64);
               if (lv7 != null && lv7.isPartVisible(PlayerModelPart.HAT)) {
                  int af = 8 + (bl2 ? 8 : 0);
                  int ag = 8 * (bl2 ? -1 : 1);
                  DrawableHelper.drawTexture(arg, ab, ac, 8, 8, 40.0F, (float)af, 8, ag, 64, 64);
               }
            ...
            this.client
               .textRenderer
               .drawWithShadow(arg, this.getPlayerName(lv6), (float)ab, (float)ac, lv6.getGameMode() == GameMode.SPECTATOR ? -1862270977 : -1);
            if (arg3 != null && lv6.getGameMode() != GameMode.SPECTATOR) {
               int ah = ab + j + 1;
               int ai = ah + p;
               if (ai - ah > 5) {
      ...
      if (list3 != null) {
         u += n * 9 + 1;
         fill(arg, i / 2 - v / 2 - 1, u - 1, i / 2 + v / 2 + 1, u + list3.size() * 9, Integer.MIN_VALUE);
   ...
   protected void renderLatencyIcon(MatrixStack arg, int i, int j, int k, PlayerListEntry arg2) {
      ...
      this.drawTexture(arg, j + i - 11, k, 0, 176 + m * 8, 10, 8);
      this.setZOffset(this.getZOffset() - 100);
   }
      ...
      if (arg.getRenderType() == ScoreboardCriterion.RenderType.HEARTS) {
         ...
         if (this.showTime == arg2.method_2976()) {
            if (l < arg2.method_2973()) {
               arg2.method_2978(m);
               arg2.method_2975((long)(this.inGameHud.getTicks() + 20));
            } else if (l > arg2.method_2973()) {
               arg2.method_2978(m);
               arg2.method_2975((long)(this.inGameHud.getTicks() + 10));
            }
         }
         ...
         int o = Math.max(MathHelper.ceil((float)(l / 2)), Math.max(MathHelper.ceil((float)(arg2.method_2960() / 2)), 10));
         boolean bl = arg2.method_2961() > (long)this.inGameHud.getTicks() && (arg2.method_2961() - (long)this.inGameHud.getTicks()) / 3L % 2L == 1L;
         if (n > 0) {
         ...
         this.client.textRenderer.drawWithShadow(arg3, string3, (float)(k - this.client.textRenderer.getWidth(string3)), (float)i, 16777215);
      }
   }
   ...
   static class EntryOrderComparator implements Comparator<PlayerListEntry> {
      ...
      public int compare(PlayerListEntry arg, PlayerListEntry arg2) {
         ...
         return ComparisonChain.start()
            .compareTrueFirst(arg.getGameMode() != GameMode.SPECTATOR, arg2.getGameMode() != GameMode.SPECTATOR)
            .compare(lv != null ? lv.getName() : "", lv2 != null ? lv2.getName() : "")
            .compare(arg.getProfile().getName(), arg2.getProfile().getName(), String::compareToIgnoreCase)
            .result();
      }
   }
}
