...
public class SpectatorHud extends DrawableHelper implements SpectatorMenuCloseCallback {
   ...
   protected void renderSpectatorMenu(MatrixStack arg, float f, int i, int j, SpectatorMenuState arg2) {
      ...
      for (int k = 0; k < 9; k++) {
         this.renderSpectatorCommand(arg, k, this.client.getWindow().getScaledWidth() / 2 - 90 + k * 20 + 2, (float)(j + 3), f, arg2.getCommand(k));
      }
   ...
   private void renderSpectatorCommand(MatrixStack arg, int i, int j, float f, float g, SpectatorMenuCommand arg2) {
      ...
      if (arg2 != SpectatorMenu.BLANK_COMMAND) {
         ...
         if (k > 3 && arg2.isEnabled()) {
            ...
            this.client
               .textRenderer
               .drawWithShadow(arg, lv, (float)(j + 19 - 2 - this.client.textRenderer.getWidth(lv)), f + 6.0F + 3.0F, 16777215 + (k << 24));
         }
      }
   }
   ...
   public void render(MatrixStack arg) {
      ...
      if (i > 3 && this.spectatorMenu != null) {
         ...
         if (lv2 != null) {
            ...
            RenderSystem.enableBlend();
            RenderSystem.defaultBlendFunc();
            this.client.textRenderer.drawWithShadow(arg, lv2, (float)j, (float)k, 16777215 + (i << 24));
            RenderSystem.disableBlend();
            RenderSystem.popMatrix();
         }
      }
