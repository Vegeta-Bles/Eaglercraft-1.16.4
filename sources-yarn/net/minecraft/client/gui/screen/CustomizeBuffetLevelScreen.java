...
public class CustomizeBuffetLevelScreen extends Screen {
   ...
   public CustomizeBuffetLevelScreen(Screen arg, DynamicRegistryManager arg2, Consumer<Biome> consumer, Biome arg3) {
      ...
      this.field_25888 = arg2.get(Registry.BIOME_KEY);
   }
   ...
   protected void init() {
      ...
      this.biomeSelectionList
         .setSelected(this.biomeSelectionList.children().stream().filter(arg -> Objects.equals(arg.field_24564, this.field_25040)).findFirst().orElse(null));
   }
   ...
   class BuffetBiomesListWidget extends AlwaysSelectedEntryListWidget<CustomizeBuffetLevelScreen.BuffetBiomesListWidget.BuffetBiomeItem> {
      private BuffetBiomesListWidget() {
         ...
         CustomizeBuffetLevelScreen.this.field_25888
            .getEntries()
            .stream()
            .sorted(Comparator.comparing(entry -> entry.getKey().getValue().toString()))
            .forEach(entry -> this.addEntry(new CustomizeBuffetLevelScreen.BuffetBiomesListWidget.BuffetBiomeItem(entry.getValue())));
      }
      ...
      public void setSelected(@Nullable CustomizeBuffetLevelScreen.BuffetBiomesListWidget.BuffetBiomeItem arg) {
         super.setSelected(arg);
         if (arg != null) {
            CustomizeBuffetLevelScreen.this.field_25040 = arg.field_24564;
            NarratorManager.INSTANCE
               .narrate(new TranslatableText("narrator.select", CustomizeBuffetLevelScreen.this.field_25888.getId(arg.field_24564)).getString());
         }
      ...
      class BuffetBiomeItem extends AlwaysSelectedEntryListWidget.Entry<CustomizeBuffetLevelScreen.BuffetBiomesListWidget.BuffetBiomeItem> {
         ...
         public BuffetBiomeItem(Biome arg2) {
            ...
            Identifier lv = CustomizeBuffetLevelScreen.this.field_25888.getId(arg2);
            String string = "biome." + lv.getNamespace() + "." + lv.getPath();
            if (Language.getInstance().hasTranslation(string)) {
               this.field_26536 = new TranslatableText(string);
            } else {
               this.field_26536 = new LiteralText(lv.toString());
            }
         }
