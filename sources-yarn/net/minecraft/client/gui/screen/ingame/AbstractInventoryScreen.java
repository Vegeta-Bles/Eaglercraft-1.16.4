...
public abstract class AbstractInventoryScreen<T extends ScreenHandler> extends HandledScreen<T> {
   ...
   private void drawStatusEffectSprites(MatrixStack arg, int i, int j, Iterable<StatusEffectInstance> iterable) {
      ...
      for (StatusEffectInstance lv2 : iterable) {
         ...
         drawSprite(arg, i + 6, k + 7, this.getZOffset(), 18, 18, lv4);
         k += j;
      }
   }
   ...
   private void drawStatusEffectDescriptions(MatrixStack arg, int i, int j, Iterable<StatusEffectInstance> iterable) {
      ...
      for (StatusEffectInstance lv : iterable) {
         String string = I18n.translate(lv.getEffectType().getTranslationKey());
         if (lv.getAmplifier() >= 1 && lv.getAmplifier() <= 9) {
            string = string + ' ' + I18n.translate("enchantment.level." + (lv.getAmplifier() + 1));
         }

         this.textRenderer.drawWithShadow(arg, string, (float)(i + 10 + 18), (float)(k + 6), 16777215);
         String string2 = StatusEffectUtil.durationToString(lv, 1.0F);
         this.textRenderer.drawWithShadow(arg, string2, (float)(i + 10 + 18), (float)(k + 6 + 10), 8355711);
         k += j;
      }
   }
}
