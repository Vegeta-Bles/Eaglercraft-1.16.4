...
public class CartographyTableScreen extends HandledScreen<CartographyTableScreenHandler> {
   ...
   protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
      ...
      Item lv = this.handler.getSlot(1).getStack().getItem();
      ...
      if (lv2.getItem() == Items.FILLED_MAP) {
         lv3 = FilledMapItem.getMapState(lv2, this.client.world);
         if (lv3 != null) {
            if (lv3.locked) {
               bl4 = true;
               if (bl2 || bl3) {
      ...
      this.drawMap(matrices, lv3, bl, bl2, bl3, bl4);
   }
   ...
   private void drawMap(@Nullable MapState state, int x, int y, float size) {
      if (state != null) {
         ...
         this.client.gameRenderer.getMapRenderer().draw(new MatrixStack(), lv, state, true, 15728880);
         lv.draw();
         RenderSystem.popMatrix();
      }
   }
