...
public class CreativeInventoryScreen extends AbstractInventoryScreen<CreativeInventoryScreen.CreativeScreenHandler> {
      ...
      if (slot == null && selectedTab != ItemGroup.INVENTORY.getIndex() && actionType != SlotActionType.QUICK_CRAFT) {
         ...
         if (slot == this.deleteItemSlot && bl) {
            ...
            if (slot == this.deleteItemSlot) {
               ...
               this.client.interactionManager.clickCreativeStack(lv2, ((CreativeInventoryScreen.CreativeSlot)slot).slot.id);
            ...
            ItemStack lv9 = slot == null ? ItemStack.EMPTY : this.handler.getSlot(slot.id).getStack();
            ...
            if (ScreenHandler.unpackQuickCraftStage(clickData) == 2) {
               for (int l = 0; l < 9; l++) {
                  this.client.interactionManager.clickCreativeStack(this.handler.getSlot(45 + l).getStack(), 36 + l);
               }
            } else if (slot != null) {
               ItemStack lv10 = this.handler.getSlot(slot.id).getStack();
               this.client.interactionManager.clickCreativeStack(lv10, slot.id - this.handler.slots.size() + 9 + 36);
               int m = 45 + clickData;
               if (actionType == SlotActionType.SWAP) {
                  this.client.interactionManager.clickCreativeStack(lv9, m - this.handler.slots.size() + 9 + 36);
   ...
   private void search() {
      ...
      if (string.isEmpty()) {
         for (Item lv : Registry.ITEM) {
            lv.appendStacks(ItemGroup.SEARCH, this.handler.itemList);
         }
      } else {
         Searchable<ItemStack> lv2;
         if (string.startsWith("#")) {
            string = string.substring(1);
            lv2 = this.client.getSearchableContainer(SearchManager.ITEM_TAG);
            this.searchForTags(string);
         } else {
            lv2 = this.client.getSearchableContainer(SearchManager.ITEM_TOOLTIP);
         }

         this.handler.itemList.addAll(lv2.findAll(string.toLowerCase(Locale.ROOT)));
      }
   ...
   private void searchForTags(String string) {
      ...
      if (i == -1) {
         predicate = arg -> arg.getPath().contains(string);
      } else {
         String string2 = string.substring(0, i).trim();
         String string3 = string.substring(i + 1).trim();
         predicate = arg -> arg.getNamespace().contains(string2) && arg.getPath().contains(string3);
      }

      TagGroup<Item> lv = ItemTags.getTagGroup();
      lv.getTagIds().stream().filter(predicate).forEach(arg2 -> {
         Tag var10000 = this.searchResultTags.put(arg2, lv.getTag(arg2));
      });
   }
   ...
   protected void drawForeground(MatrixStack matrices, int mouseX, int mouseY) {
      ...
      if (lv.shouldRenderName()) {
         RenderSystem.disableBlend();
         this.textRenderer.draw(matrices, lv.getTranslationKey(), 8.0F, 6.0F, 4210752);
      }
   }
   ...
   private void setSelectedTab(ItemGroup group) {
            ...
            if (lv2.isEmpty()) {
               for (int k = 0; k < 9; k++) {
                  if (k == j) {
                     ...
                     this.handler.itemList.add(lv3);
                  } else {
                     this.handler.itemList.add(ItemStack.EMPTY);
                  }
               }
            } else {
               this.handler.itemList.addAll(lv2);
            }
         }
      ...
      if (group == ItemGroup.INVENTORY) {
         ScreenHandler lv6 = this.client.player.playerScreenHandler;
         if (this.slots == null) {
         ...
         for (int l = 0; l < lv6.slots.size(); l++) {
            ...
            Slot lv7 = new CreativeInventoryScreen.CreativeSlot(lv6.slots.get(l), l, p, q);
            this.handler.slots.add(lv7);
         }
   ...
   protected void renderTooltip(MatrixStack matrices, ItemStack stack, int x, int y) {
      if (selectedTab == ItemGroup.SEARCH.getIndex()) {
         ...
         if (lv2 != null) {
            list2.add(1, lv2.getTranslationKey().shallowCopy().formatted(Formatting.BLUE));
         }
   ...
   protected boolean renderTabTooltipIfHovered(MatrixStack arg, ItemGroup arg2, int i, int j) {
      ...
      if (this.isPointWithinBounds(l + 3, m + 3, 23, 27, (double)i, (double)j)) {
         this.renderTooltip(arg, arg2.getTranslationKey(), i, j);
         return true;
      } else {
         return false;
      }
   ...
   protected void renderTabIcon(MatrixStack arg, ItemGroup arg2) {
      ...
      this.itemRenderer.renderInGuiWithOverrides(lv, l, m);
