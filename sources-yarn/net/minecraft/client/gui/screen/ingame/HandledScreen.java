...
public abstract class HandledScreen<T extends ScreenHandler> extends Screen implements ScreenHandlerProvider<T> {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      for (int o = 0; o < this.handler.slots.size(); o++) {
         Slot lv = this.handler.slots.get(o);
         if (lv.doDrawHoveringEffect()) {
            this.drawSlot(matrices, lv);
         }

         if (this.isPointOverSlot(lv, (double)mouseX, (double)mouseY) && lv.doDrawHoveringEffect()) {
            this.focusedSlot = lv;
            RenderSystem.disableDepthTest();
            int p = lv.x;
            int q = lv.y;
      ...
      ItemStack lv3 = this.touchDragStack.isEmpty() ? lv2.getCursorStack() : this.touchDragStack;
      if (!lv3.isEmpty()) {
         int r = 8;
   ...
   private void drawSlot(MatrixStack matrices, Slot slot) {
      ...
      if (slot == this.touchDragSlotStart && !this.touchDragStack.isEmpty() && this.touchIsRightClickDrag && !lv.isEmpty()) {
         ...
         if (ScreenHandler.canInsertItemIntoSlot(slot, lv2, true) && this.handler.canInsertIntoSlot(slot)) {
            ...
            int k = Math.min(lv.getMaxCount(), slot.getMaxItemCount(lv));
            if (lv.getCount() > k) {
      ...
      if (lv.isEmpty() && slot.doDrawHoveringEffect()) {
         ...
         if (pair != null) {
            ...
            drawSprite(matrices, i, j, this.getZOffset(), 16, 16, lv3);
            bl2 = true;
         }
      }
      ...
      if (!lv.isEmpty() && this.cursorDragging) {
         if (this.heldButtonType == 2) {
            ...
            for (Slot lv2 : this.cursorDragSlots) {
               ...
               int j = Math.min(lv3.getMaxCount(), lv2.getMaxItemCount(lv3));
               if (lv3.getCount() > j) {
                  lv3.setCount(j);
               }

               this.draggedStackRemainder = this.draggedStackRemainder - (lv3.getCount() - i);
            }
         }
      }
   }
   ...
   private Slot getSlotAt(double xPosition, double yPosition) {
      for (int i = 0; i < this.handler.slots.size(); i++) {
         Slot lv = this.handler.slots.get(i);
         if (this.isPointOverSlot(lv, xPosition, yPosition) && lv.doDrawHoveringEffect()) {
            return lv;
         }
      }
   ...
   public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
      ...
      if (this.touchDragSlotStart != null && this.client.options.touchscreen) {
         ...
         && lv != null
         && !lv2.isEmpty()
         && (lv2.getCount() > this.cursorDragSlots.size() || this.heldButtonType == 2)
         && ScreenHandler.canInsertItemIntoSlot(lv, lv2, true)
         && lv.canInsert(lv2)
         && this.handler.canInsertIntoSlot(lv)) {
      ...
      if (this.doubleClicking && lv != null && button == 0 && this.handler.canInsertIntoSlot(ItemStack.EMPTY, lv)) {
         if (hasShiftDown()) {
            if (!this.quickMovingStack.isEmpty()) {
               for (Slot lv2 : this.handler.slots) {
                  if (lv2 != null
                     && lv2.canTakeItems(this.client.player)
                     && lv2.hasStack()
                     && lv2.inventory == lv.inventory
                     && ScreenHandler.canInsertItemIntoSlot(lv2, this.quickMovingStack, true)) {
   ...
   protected boolean handleHotbarKeyPressed(int keyCode, int scanCode) {
      if (this.client.player.inventory.getCursorStack().isEmpty() && this.focusedSlot != null) {
         ...
         for (int k = 0; k < 9; k++) {
            if (this.client.options.keysHotbar[k].matchesKey(keyCode, scanCode)) {
