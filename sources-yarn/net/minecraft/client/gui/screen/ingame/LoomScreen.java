...
public class LoomScreen extends HandledScreen<LoomScreenHandler> {
   ...
   protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
      ...
      if (this.field_21841 != null && !this.hasTooManyPatterns) {
         ...
         matrices.push();
         matrices.translate((double)(k + 139), (double)(l + 52), 0.0);
         ...
         BannerBlockEntityRenderer.method_29999(
            matrices, lv5, 15728880, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, this.field_21841
         );
         matrices.pop();
         lv5.draw();
      ...
      if (this.canApplyDyePattern) {
         ...
         int x = this.handler.getSelectedPattern();
   ...
   private void method_22692(int i, int j, int k) {
      ...
      lv4.push();
      lv4.translate((double)((float)j + 0.5F), (double)(k + 16), 0.0);
      ...
      BannerBlockEntityRenderer.method_29999(lv4, lv5, 15728880, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, list);
      lv4.pop();
      lv5.draw();
   }
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      if (this.canApplyDyePattern) {
         ...
         for (int m = this.firstPatternButtonId; m < l; m++) {
            ...
            if (f >= 0.0 && g >= 0.0 && f < 14.0 && g < 14.0 && this.handler.onButtonClick(this.client.player, m)) {
               MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_LOOM_SELECT_PATTERN, 1.0F));
               this.client.interactionManager.clickButton(this.handler.syncId, m);
               return true;
            }
         }
   ...
   private void onInventoryChanged() {
      ItemStack lv = this.handler.getOutputSlot().getStack();
      if (lv.isEmpty()) {
         this.field_21841 = null;
      } else {
         this.field_21841 = BannerBlockEntity.method_24280(((BannerItem)lv.getItem()).getColor(), BannerBlockEntity.getPatternListTag(lv));
      }
      ...
      this.hasTooManyPatterns = lv5.contains("Patterns", 9) && !lv2.isEmpty() && lv5.getList("Patterns", 10).size() >= 6;
