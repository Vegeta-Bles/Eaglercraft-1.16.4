...
public class SocialInteractionsPlayerListEntry extends ElementListWidget.Entry<SocialInteractionsPlayerListEntry> {
   ...
   public SocialInteractionsPlayerListEntry(MinecraftClient client, SocialInteractionsScreen parent, UUID uuid, String name, Supplier<Identifier> supplier) {
      ...
      SocialInteractionsManager lv = client.getSocialInteractionsManager();
      if (!client.player.getGameProfile().getId().equals(uuid) && !lv.isPlayerBlocked(uuid)) {
         this.hideButton = new TexturedButtonWidget(0, 0, 20, 20, 0, 38, 20, SocialInteractionsScreen.SOCIAL_INTERACTIONS_TEXTURE, 256, 256, arg2 -> {
            lv.hidePlayer(uuid);
            ...
            if (this.field_26864 >= 10.0F) {
               parent.method_31354(() -> method_31328(parent, arg4, this.hideTooltip, i, j));
            }
         ...
         this.showButton = new TexturedButtonWidget(0, 0, 20, 20, 20, 38, 20, SocialInteractionsScreen.SOCIAL_INTERACTIONS_TEXTURE, 256, 256, arg2 -> {
            ...
            this.field_26864 = this.field_26864 + client.getLastFrameDuration();
            if (this.field_26864 >= 10.0F) {
               parent.method_31354(() -> method_31328(parent, arg4, this.showTooltip, i, j));
            }
         ...
         this.showButton.visible = lv.isPlayerHidden(uuid);
   ...
   public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
      ...
      int r = p + 24 + 4;
      Text lv = this.method_31390();
      int s;
      if (lv == LiteralText.EMPTY) {
         ...
         this.client.textRenderer.draw(matrices, lv, (float)r, (float)(s + 12), field_26903);
      }

      this.client.getTextureManager().bindTexture(this.field_26904.get());
      DrawableHelper.drawTexture(matrices, p, q, 24, 24, 8.0F, 8.0F, 8, 8, 64, 64);
      RenderSystem.enableBlend();
      DrawableHelper.drawTexture(matrices, p, q, 24, 24, 40.0F, 8.0F, 8, 8, 64, 64);
      RenderSystem.disableBlend();
      this.client.textRenderer.draw(matrices, this.name, (float)r, (float)s, field_26853);
      if (this.field_26859) {
         DrawableHelper.fill(matrices, p, q, p + 24, q + 24, field_26850);
      }

      if (this.hideButton != null && this.showButton != null) {
         float g = this.field_26864;
   ...
   private void method_31329(boolean bl, Text arg) {
      this.showButton.visible = bl;
      this.hideButton.visible = !bl;
      this.client.inGameHud.getChatHud().addMessage(arg);
      NarratorManager.INSTANCE.narrate(arg.getString());
   }

   private MutableText method_31389(MutableText arg) {
      Text lv = this.method_31390();
      return lv == LiteralText.EMPTY
         ? new LiteralText(this.name).append(", ").append(arg)
         : new LiteralText(this.name).append(", ").append(lv).append(", ").append(arg);
   }
   ...
   private static void method_31328(SocialInteractionsScreen arg, MatrixStack arg2, List<OrderedText> list, int i, int j) {
      arg.renderOrderedTooltip(arg2, list, i, j);
      arg.method_31354(null);
   }
}
