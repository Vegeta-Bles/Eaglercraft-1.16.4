...
public class SocialInteractionsPlayerListWidget extends ElementListWidget<SocialInteractionsPlayerListEntry> {
   ...
   public void method_31393(Collection<UUID> collection, double d) {
      ...
      for (UUID uUID : collection) {
         PlayerListEntry lv = this.minecraftClient.player.networkHandler.getPlayerListEntry(uUID);
         if (lv != null) {
            this.players
               .add(
                  new SocialInteractionsPlayerListEntry(
                     this.minecraftClient, this.parent, lv.getProfile().getId(), lv.getProfile().getName(), lv::getSkinTexture
                  )
               );
         }
      }
      ...
      this.method_31349();
      this.players.sort((arg, arg2) -> arg.getName().compareToIgnoreCase(arg2.getName()));
   ...
   public void method_31345(PlayerListEntry arg, SocialInteractionsScreen.Tab arg2) {
      UUID uUID = arg.getProfile().getId();

      for (SocialInteractionsPlayerListEntry lv : this.players) {
         if (lv.getUuid().equals(uUID)) {
            lv.method_31335(false);
            return;
         }
      }

      if ((arg2 == SocialInteractionsScreen.Tab.ALL || this.minecraftClient.getSocialInteractionsManager().method_31391(uUID))
         && (Strings.isNullOrEmpty(this.currentSearch) || arg.getProfile().getName().toLowerCase(Locale.ROOT).contains(this.currentSearch))) {
         SocialInteractionsPlayerListEntry lv2 = new SocialInteractionsPlayerListEntry(
            this.minecraftClient, this.parent, arg.getProfile().getId(), arg.getProfile().getName(), arg::getSkinTexture
         );
         this.addEntry(lv2);
