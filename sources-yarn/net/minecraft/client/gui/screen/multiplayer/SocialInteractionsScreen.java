...
public class SocialInteractionsScreen extends Screen {
   ...
   protected void init() {
      ...
      this.allTabButton = this.addButton(new ButtonWidget(j, 45, i, 20, ALL_TAB_TITLE, arg -> this.setCurrentTab(SocialInteractionsScreen.Tab.ALL)));
      this.hiddenTabButton = this.addButton(
         new ButtonWidget((j + k - i) / 2 + 1, 45, i, 20, HIDDEN_TAB_TITLE, arg -> this.setCurrentTab(SocialInteractionsScreen.Tab.HIDDEN))
      );
      this.field_26913 = this.addButton(new ButtonWidget(k - i + 1, 45, i, 20, field_26915, arg -> this.setCurrentTab(SocialInteractionsScreen.Tab.BLOCKED)));
      this.field_26914 = this.addButton(new ButtonWidget(n, m, l, 20, field_26919, arg -> this.client.openScreen(new ConfirmChatLinkScreen(bl -> {
            if (bl) {
   ...
   private void setCurrentTab(SocialInteractionsScreen.Tab currentTab) {
      ...
      switch (currentTab) {
         ...
         case BLOCKED:
            ...
            collection = this.client.player.networkHandler.getPlayerUuids().stream().filter(lv::isPlayerBlocked).collect(Collectors.toSet());
            break;
         default:
   ...
   private void method_31350(MinecraftClient arg) {
      int i = arg.getNetworkHandler().getPlayerList().size();
      if (this.playerCount != i) {
         String string = "";
         ServerInfo lv = arg.getCurrentServerEntry();
         if (arg.isInSingleplayer()) {
            string = arg.getServer().getServerMotd();
         } else if (lv != null) {
            string = lv.name;
         }
   ...
   public void method_31353(PlayerListEntry arg) {
      this.playerList.method_31345(arg, this.currentTab);
   }
