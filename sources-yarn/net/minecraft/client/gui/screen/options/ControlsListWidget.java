...
public class ControlsListWidget extends ElementListWidget<ControlsListWidget.Entry> {
   ...
   public ControlsListWidget(ControlsOptionsScreen parent, MinecraftClient client) {
      ...
      this.parent = parent;
      KeyBinding[] lvs = (KeyBinding[])ArrayUtils.clone(client.options.keysAll);
      Arrays.sort((Object[])lvs);
      String string = null;

      for (KeyBinding lv : lvs) {
         ...
         Text lv2 = new TranslatableText(lv.getTranslationKey());
         int i = client.textRenderer.getWidth(lv2);
         if (i > this.maxKeyNameLength) {
            this.maxKeyNameLength = i;
         }

         this.addEntry(new ControlsListWidget.KeyBindingEntry(lv, lv2));
      }
   }
   ...
   public class KeyBindingEntry extends ControlsListWidget.Entry {
      ...
      public void render(
         ...
         boolean bl2 = ControlsListWidget.this.parent.focusedBinding == this.binding;
         ControlsListWidget.this.client
            .textRenderer
         ...
         if (!this.binding.isUnbound()) {
            for (KeyBinding lv : ControlsListWidget.this.client.options.keysAll) {
               if (lv != this.binding && this.binding.equals(lv)) {
                  bl3 = true;
                  break;
               }
            }
