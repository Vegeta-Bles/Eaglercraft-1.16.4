...
public class SkinOptionsScreen extends GameOptionsScreen {
   ...
   protected void init() {
      ...
      for (PlayerModelPart lv : PlayerModelPart.values()) {
         this.addButton(
            new ButtonWidget(
               this.width / 2 - 155 + i % 2 * 160, this.height / 6 + 24 * (i >> 1), 150, 20, this.getPlayerModelPartDisplayString(lv), button -> {
                  this.gameOptions.togglePlayerModelPart(lv);
                  button.setMessage(this.getPlayerModelPartDisplayString(lv));
               }
            )
         );
         i++;
