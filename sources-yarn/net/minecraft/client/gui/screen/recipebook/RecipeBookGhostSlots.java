...
public class RecipeBookGhostSlots {
   ...
   public void draw(MatrixStack arg, MinecraftClient arg2, int i, int j, boolean bl, float f) {
      ...
      for (int k = 0; k < this.slots.size(); k++) {
         RecipeBookGhostSlots.GhostInputSlot lv = this.slots.get(k);
         int l = lv.getX() + i;
         int m = lv.getY() + j;
         if (k == 0 && bl) {
            DrawableHelper.fill(arg, l - 4, m - 4, l + 20, m + 20, 822018048);
         } else {
            DrawableHelper.fill(arg, l, m, l + 16, m + 16, 822018048);
         }
         ...
         DrawableHelper.fill(arg, l, m, l + 16, m + 16, 822083583);
