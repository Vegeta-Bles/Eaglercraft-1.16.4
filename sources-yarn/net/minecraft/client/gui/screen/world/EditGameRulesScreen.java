...
public class EditGameRulesScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addButton(
         new ButtonWidget(this.width / 2 - 155 + 160, this.height - 29, 150, 20, ScreenTexts.CANCEL, arg -> this.ruleSaver.accept(Optional.empty()))
      );
      this.doneButton = this.addButton(
         new ButtonWidget(this.width / 2 - 155, this.height - 29, 150, 20, ScreenTexts.DONE, arg -> this.ruleSaver.accept(Optional.of(this.gameRules)))
      );
   }
   ...
   public class BooleanRuleWidget extends EditGameRulesScreen.NamedRuleWidget {
      ...
      public BooleanRuleWidget(final Text name, List<OrderedText> description, final String ruleName, final GameRules.BooleanRule arg3) {
         ...
         this.toggleButton = new ButtonWidget(10, 5, 44, 20, ScreenTexts.getToggleText(arg3.get()), arg2 -> {
            ...
            protected MutableText getNarrationMessage() {
               return ScreenTexts.composeToggleText(name, arg3.get()).append("\n").append(ruleName);
            }
         };
   ...
   public class RuleListWidget extends ElementListWidget<EditGameRulesScreen.AbstractRuleWidget> {
      public RuleListWidget(final GameRules arg2) {
         ...
         GameRules.accept(new GameRules.Visitor() {
            ...
            public void visitBoolean(GameRules.Key<GameRules.BooleanRule> key, GameRules.Type<GameRules.BooleanRule> type) {
               this.createRuleWidget(key, (arg, list, string, arg2xx) -> EditGameRulesScreen.this.new BooleanRuleWidget(arg, list, string, arg2xx));
            }

            @Override
            public void visitInt(GameRules.Key<GameRules.IntRule> key, GameRules.Type<GameRules.IntRule> type) {
               this.createRuleWidget(key, (arg, list, string, arg2xx) -> EditGameRulesScreen.this.new IntRuleWidget(arg, list, string, arg2xx));
            }
            ...
            private <T extends GameRules.Rule<T>> void createRuleWidget(GameRules.Key<T> key, EditGameRulesScreen.RuleWidgetFactory<T> widgetFactory) {
               Text lv = new TranslatableText(key.getTranslationKey());
               Text lv2 = new LiteralText(key.getName()).formatted(Formatting.YELLOW);
               T lv3 = arg2.get(key);
               String string = lv3.serialize();
               Text lv4 = new TranslatableText("editGamerule.default", new LiteralText(string)).formatted(Formatting.GRAY);
               String string2 = key.getTranslationKey() + ".description";
               List<OrderedText> list;
               String string3;
               if (I18n.hasTranslation(string2)) {
                  Builder<OrderedText> builder = ImmutableList.builder().add(lv2.asOrderedText());
                  Text lv5 = new TranslatableText(string2);
                  EditGameRulesScreen.this.textRenderer.wrapLines(lv5, 150).forEach(builder::add);
               ...
               map.computeIfAbsent(key.getCategory(), arg -> Maps.newHashMap()).put(key, widgetFactory.create(lv, list, string3, lv3));
            }
         });
         map.entrySet()
            .stream()
               ...
               entry -> {
                  this.addEntry(
                     EditGameRulesScreen.this.new RuleCategoryWidget(
                        new TranslatableText(entry.getKey().getCategory()).formatted(new Formatting[]{Formatting.BOLD, Formatting.YELLOW})
                     )
                  );
                  entry.getValue()
                     .entrySet()
      ...
      public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
         ...
         if (this.isMouseOver((double)mouseX, (double)mouseY)) {
            EditGameRulesScreen.AbstractRuleWidget lv = this.getEntryAtPosition((double)mouseX, (double)mouseY);
            if (lv != null) {
