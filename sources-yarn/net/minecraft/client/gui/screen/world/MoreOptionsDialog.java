...
public class MoreOptionsDialog implements TickableElement, Drawable {
   ...
   public void method_28092(final CreateWorldScreen parent, MinecraftClient client, TextRenderer textRenderer) {
         ...
         new ButtonWidget(
            ...
            arg2 -> {
               while (this.field_25049.isPresent()) {
                  ...
                  GeneratorType lv = GeneratorType.VALUES.get(ix);
                  this.field_25049 = Optional.of(lv);
                  this.generatorOptions = lv.createDefaultOptions(
      ...
      this.customizeTypeButton = parent.addButton(new ButtonWidget(j, 120, 150, 20, new TranslatableText("selectWorld.customizeType"), arg3 -> {
         GeneratorType.ScreenProvider lv = GeneratorType.SCREEN_PROVIDERS.get(this.field_25049);
         if (lv != null) {
      ...
      this.field_25048 = parent.addButton(
         new ButtonWidget(
            ...
            arg3 -> {
               ...
               String string = TinyFileDialogs.tinyfd_openFileDialog(lv.getString(), null, null, null, false);
               if (string != null) {
                  DynamicRegistryManager.Impl lv2 = DynamicRegistryManager.create();
                  ResourcePackManager lv3 = new ResourcePackManager(
                     new VanillaDataPackProvider(), new FileResourcePackProvider(parent.method_29693().toFile(), ResourcePackSource.PACK_SOURCE_WORLD)
                  );

                  ServerResourceManager lv4;
                  try {
                     MinecraftServer.loadDataPacks(lv3, parent.field_25479, false);
                     CompletableFuture<ServerResourceManager> completableFuture = ServerResourceManager.reload(
                        lv3.createResourcePacks(), CommandManager.RegistrationEnvironment.INTEGRATED, 2, Util.getMainWorkerExecutor(), client
                     );
                     client.runTasks(completableFuture::isDone);
                     lv4 = completableFuture.get();
                     ...
                     Text lv5 = new TranslatableText("selectWorld.import_worldgen_settings.failure");
                     Text lv6 = new LiteralText(var25.getMessage());
                     client.getToastManager().add(SystemToast.create(client, SystemToast.Type.WORLD_GEN_SETTINGS_TRANSFER, lv5, lv6));
                     lv3.close();
                     return;
                  }

                  RegistryOps<JsonElement> lv8 = RegistryOps.of(JsonOps.INSTANCE, lv4.getResourceManager(), lv2);
                  JsonParser jsonParser = new JsonParser();

                  DataResult<GeneratorOptions> dataResult;
                  try (BufferedReader bufferedReader = Files.newBufferedReader(Paths.get(string))) {
                     JsonElement jsonElement = jsonParser.parse(bufferedReader);
                     dataResult = GeneratorOptions.CODEC.parse(lv8, jsonElement);
   ...
   void method_31132(ServerResourceManager arg) {
      DynamicRegistryManager.Impl lv = DynamicRegistryManager.create();
      RegistryReadingOps<JsonElement> lv2 = RegistryReadingOps.of(JsonOps.INSTANCE, this.field_25483);
      RegistryOps<JsonElement> lv3 = RegistryOps.of(JsonOps.INSTANCE, arg.getResourceManager(), lv);
      DataResult<GeneratorOptions> dataResult = GeneratorOptions.CODEC
         .encodeStart(lv2, this.generatorOptions)
         .flatMap(jsonElement -> GeneratorOptions.CODEC.parse(lv3, jsonElement));
      dataResult.resultOrPartial(Util.method_29188("Error parsing worldgen settings after loading data packs: ", field_25046::error)).ifPresent(arg2 -> {
         this.generatorOptions = arg2;
         this.field_25483 = lv;
      });
   }
