...
public class ButtonListWidget extends ElementListWidget<ButtonListWidget.ButtonEntry> {
   ...
   @Nullable
   public AbstractButtonWidget getButtonFor(Option arg) {
      for (ButtonListWidget.ButtonEntry lv : this.children()) {
         for (AbstractButtonWidget lv2 : lv.buttons) {
            if (lv2 instanceof OptionButtonWidget && ((OptionButtonWidget)lv2).getOption() == arg) {
               return lv2;
            }
         }
      }
   ...
   public static class ButtonEntry extends ElementListWidget.Entry<ButtonListWidget.ButtonEntry> {
      ...
      public static ButtonListWidget.ButtonEntry create(GameOptions options, int width, Option firstOption, @Nullable Option secondOption) {
         ...
         return secondOption == null
            ? new ButtonListWidget.ButtonEntry(ImmutableList.of(lv))
            : new ButtonListWidget.ButtonEntry(ImmutableList.of(lv, secondOption.createButton(options, width / 2 - 155 + 160, 0, 150)));
      }
