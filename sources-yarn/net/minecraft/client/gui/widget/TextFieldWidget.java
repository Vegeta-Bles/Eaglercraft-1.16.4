...
public class TextFieldWidget extends AbstractButtonWidget implements Drawable, Element {
   ...
   public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      if (this.isVisible()) {
         if (this.hasBorder()) {
            ...
            fill(matrices, this.x - 1, this.y - 1, this.x + this.width + 1, this.y + this.height + 1, k);
         ...
         if (!string.isEmpty()) {
            ...
            q = this.textRenderer.drawWithShadow(matrices, this.renderTextProvider.apply(string2, this.firstCharacterIndex), (float)o, (float)p, l);
         }
   ...
   private void drawSelectionHighlight(int x1, int y1, int x2, int y2) {
      ...
      lv2.vertex((double)x1, (double)y2, 0.0).next();
      lv2.vertex((double)x2, (double)y2, 0.0).next();
      lv2.vertex((double)x2, (double)y1, 0.0).next();
      lv2.vertex((double)x1, (double)y1, 0.0).next();
      lv.draw();
