...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
   ...
   private void pushOutOfBlocks(double x, double e) {
      BlockPos lv = new BlockPos(x, this.getY(), e);
   ...
   private boolean wouldCollideAt(BlockPos arg) {
      Box lv = this.getBoundingBox();
      Box lv2 = new Box((double)arg.getX(), lv.minY, (double)arg.getZ(), (double)arg.getX() + 1.0, lv.maxY, (double)arg.getZ() + 1.0).contract(1.0E-7);
      return !this.world.isBlockSpaceEmpty(this, lv2, (argx, arg2) -> argx.shouldSuffocate(this.world, arg2));
   }
   ...
   public boolean hasJumpingMount() {
      ...
      return this.hasVehicle() && lv instanceof JumpingMount && ((JumpingMount)lv).canJump();
   }
   ...
   public void tickMovement() {
      ...
      if (this.input.jumping && !bl8 && !bl && !this.abilities.flying && !this.hasVehicle() && !this.isClimbing()) {
         ...
         if (lv.getItem() == Items.ELYTRA && ElytraItem.isUsable(lv) && this.checkFallFlying()) {
      ...
      if (this.hasJumpingMount()) {
         JumpingMount lv2 = (JumpingMount)this.getVehicle();
         ...
         if (!(o < -0.15F)) {
            ...
            if (lv9.getCollisionShape(this.world, lv8, lv7).isEmpty()) {
               ...
               if (lv10.getCollisionShape(this.world, lv8, lv7).isEmpty()) {
                  ...
                  Vec3d lv12 = lv2.add(lv5.multiply((double)r));
                  float s = this.getWidth();
                  float t = this.getHeight();
                  Box lv13 = new Box(lv, lv12.add(0.0, (double)t, 0.0)).expand((double)s, 0.0, (double)s);
                  Vec3d lv11 = lv.add(0.0, 0.51F, 0.0);
                  lv12 = lv12.add(0.0, 0.51F, 0.0);
                  Vec3d lv14 = lv5.crossProduct(new Vec3d(0.0, 1.0, 0.0));
                  Vec3d lv15 = lv14.multiply((double)(s * 0.5F));
                  Vec3d lv16 = lv11.subtract(lv15);
                  Vec3d lv17 = lv12.subtract(lv15);
                  Vec3d lv18 = lv11.add(lv15);
                  Vec3d lv19 = lv12.add(lv15);
                  Iterator<Box> iterator = this.world.getCollisions(this, lv13, arg -> true).flatMap(arg -> arg.getBoundingBoxes().stream()).iterator();
                  float u = Float.MIN_VALUE;

                  while (iterator.hasNext()) {
                     Box lv20 = iterator.next();
                     if (lv20.intersects(lv16, lv17) || lv20.intersects(lv18, lv19)) {
                        ...
                        for (int v = 1; (float)v < q; v++) {
                           ...
                           if (!(lv25 = lv24.getCollisionShape(this.world, lv23, lv7)).isEmpty()) {
                              u = (float)lv25.getMax(Direction.Axis.Y) + (float)lv23.getY();
   ...
   public float getUnderwaterVisibility() {
      if (!this.isSubmergedIn(FluidTags.WATER)) {
         ...
         if ((float)this.underwaterVisibilityTicks >= 600.0F) {
            ...
            float i = (float)this.underwaterVisibilityTicks < 100.0F
               ? 0.0F
   ...
   public Vec3d method_30951(float f) {
      if (this.client.options.getPerspective().isFirstPerson()) {
         ...
         return lv.rotateX(-h).rotateY(-g).add(this.getCameraPosVec(f));
      } else {
