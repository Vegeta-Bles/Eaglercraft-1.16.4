...
public class GameOptions {
   ...
   public void load() {
      try {
         ...
         CompoundTag lv2 = this.update(lv);
         if (!lv2.contains("graphicsMode") && lv2.contains("fancyGraphics")) {
            if ("true".equals(lv2.getString("fancyGraphics"))) {
         ...
         for (String string : lv2.getKeys()) {
            String string2 = lv2.getString(string);
            ...
            try {
               ...
               for (KeyBinding lv3 : this.keysAll) {
                  if (string.equals("key_" + lv3.getTranslationKey())) {
                     lv3.setBoundKey(InputUtil.fromTranslationKey(string2));
                  }
               }

               for (SoundCategory lv4 : SoundCategory.values()) {
                  if (string.equals("soundCategory_" + lv4.getName())) {
                     this.soundVolumeLevels.put(lv4, parseFloat(string2));
                  }
               }

               for (PlayerModelPart lv5 : PlayerModelPart.values()) {
                  if (string.equals("modelPart_" + lv5.getName())) {
                     this.setPlayerModelPart(lv5, "true".equals(string2));
                  }
               }
   ...
   public void write() {
      try (PrintWriter printWriter = new PrintWriter(new OutputStreamWriter(new FileOutputStream(this.optionsFile), StandardCharsets.UTF_8))) {
         ...
         for (KeyBinding lv : this.keysAll) {
            printWriter.println("key_" + lv.getTranslationKey() + ":" + lv.getBoundKeyTranslationKey());
         }

         for (SoundCategory lv2 : SoundCategory.values()) {
            printWriter.println("soundCategory_" + lv2.getName() + ":" + this.getSoundVolume(lv2));
         }

         for (PlayerModelPart lv3 : PlayerModelPart.values()) {
            printWriter.println("modelPart_" + lv3.getName() + ":" + this.enabledPlayerModelParts.contains(lv3));
         }
