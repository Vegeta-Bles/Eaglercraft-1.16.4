...
public abstract class Option {
   ...
   public static final BooleanOption RAW_MOUSE_INPUT = new BooleanOption(
      "options.rawMouseInput", gameOptions -> gameOptions.rawMouseInput, (gameOptions, rawMouseInput) -> {
         ...
         if (lv != null) {
            lv.setRawMouseMotion(rawMouseInput);
         }
      }
   );
   ...
   public static final CyclingOption GRAPHICS = new CyclingOption("options.graphics", (gameOptions, amount) -> {
      ...
      if (gameOptions.graphicsMode == GraphicsMode.FANCY && lv2.canWarn()) {
         lv2.scheduleWarning();
      } else {
         gameOptions.graphicsMode = gameOptions.graphicsMode.next();
         if (gameOptions.graphicsMode == GraphicsMode.FABULOUS && (!GlStateManager.supportsGl30() || lv2.hasCancelledAfterWarning())) {
      ...
      MutableText lv = new TranslatableText(gameOptions.graphicsMode.getTranslationKey());
      return gameOptions.graphicsMode == GraphicsMode.FABULOUS ? option.getGenericLabel(lv.formatted(Formatting.ITALIC)) : option.getGenericLabel(lv);
   });
   ...
   public static final BooleanOption field_26924 = new BooleanOption(
      "options.hideMatchedNames", field_26925, arg -> arg.field_26926, (arg, boolean_) -> arg.field_26926 = boolean_
   );
   ...
   public static final BooleanOption FULLSCREEN = new BooleanOption("options.fullscreen", gameOptions -> gameOptions.fullscreen, (gameOptions, fullscreen) -> {
      ...
      MinecraftClient lv = MinecraftClient.getInstance();
      if (lv.getWindow() != null && lv.getWindow().isFullscreen() != gameOptions.fullscreen) {
         lv.getWindow().toggleFullscreen();
         gameOptions.fullscreen = lv.getWindow().isFullscreen();
      }
   });
