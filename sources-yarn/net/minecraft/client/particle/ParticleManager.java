...
public class ParticleManager implements ResourceReloadListener {
   ...
   private <T extends ParticleEffect> void registerFactory(ParticleType<T> arg, ParticleManager.SpriteAwareFactory<T> arg2) {
      ParticleManager.SimpleSpriteProvider lv = new ParticleManager.SimpleSpriteProvider();
   ...
   public CompletableFuture<Void> reload(
      ...
      CompletableFuture<?>[] completableFutures = Registry.PARTICLE_TYPE
         .getIds()
         .stream()
         .map(arg2 -> CompletableFuture.runAsync(() -> this.loadTextureList(manager, arg2, map), prepareExecutor))
         .toArray(CompletableFuture[]::new);
      return CompletableFuture.allOf(completableFutures)
         .thenApplyAsync(void_ -> {
            prepareProfiler.startTick();
            prepareProfiler.push("stitching");
            SpriteAtlasTexture.Data lv = this.particleAtlasTexture.stitch(manager, map.values().stream().flatMap(Collection::stream), prepareProfiler, 0);
            prepareProfiler.pop();
         ...
         .thenAcceptAsync(
            arg2 -> {
               ...
               this.particleAtlasTexture.upload(arg2);
               ...
               map.forEach(
                  (arg2x, list) -> {
                     ImmutableList<Sprite> immutableList = list.isEmpty()
                        ? ImmutableList.of(lv)
                        : list.stream().map(this.particleAtlasTexture::getSprite).collect(ImmutableList.toImmutableList());
                     this.spriteAwareFactories.get(arg2x).setSprites(immutableList);
                  }
               );
               applyProfiler.pop();
   ...
   private void loadTextureList(ResourceManager resourceManager, Identifier id, Map<Identifier, List<Identifier>> result) {
      Identifier lv = new Identifier(id.getNamespace(), "particles/" + id.getPath() + ".json");

      try (
         Resource lv2 = resourceManager.getResource(lv);
         Reader reader = new InputStreamReader(lv2.getInputStream(), Charsets.UTF_8);
      ) {
         ...
         if (list == null) {
            ...
            result.put(id, list.stream().map(arg -> new Identifier(arg.getNamespace(), "particle/" + arg.getPath())).collect(Collectors.toList()));
         }
   ...
   private <T extends ParticleEffect> Particle createParticle(T parameters, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
      ParticleFactory<T> lv = (ParticleFactory<T>)this.factories.get(Registry.PARTICLE_TYPE.getRawId(parameters.getType()));
   ...
   public void tick() {
      ...
      if (!this.newParticles.isEmpty()) {
         while ((lv2 = this.newParticles.poll()) != null) {
            this.particles.computeIfAbsent(lv2.getType(), arg -> EvictingQueue.create(16384)).add(lv2);
         }
      }
   }
   ...
   private void tickParticle(Particle particle) {
      try {
         ...
         lv2.add("Particle Type", particle.getType()::toString);
   ...
   public void renderParticles(MatrixStack arg, VertexConsumerProvider.Immediate arg2, LightmapTextureManager arg3, Camera arg4, float f) {
      ...
      for (ParticleTextureSheet lv : PARTICLE_TEXTURE_SHEETS) {
         Iterable<Particle> iterable = this.particles.get(lv);
         if (iterable != null) {
            ...
            for (Particle lv4 : iterable) {
               try {
                  ...
                  CrashReport lv5 = CrashReport.create(var16, "Rendering Particle");
                     ...
                     for (int r = 0; r < o; r++) {
                        ...
                        this.addParticle(
                           new BlockDustParticle(
                                 this.world, (double)pos.getX() + v, (double)pos.getY() + w, (double)pos.getZ() + x, s - 0.5, t - 0.5, u - 0.5, state
                              )
                              .setBlockPos(pos)
                        );
                     }
   ...
   public void addBlockBreakingParticles(BlockPos pos, Direction direction) {
      ...
      if (lv.getRenderType() != BlockRenderType.INVISIBLE) {
         ...
         double d = (double)i + this.random.nextDouble() * (lv2.maxX - lv2.minX - 0.2F) + 0.1F + lv2.minX;
         double e = (double)j + this.random.nextDouble() * (lv2.maxY - lv2.minY - 0.2F) + 0.1F + lv2.minY;
         double g = (double)k + this.random.nextDouble() * (lv2.maxZ - lv2.minZ - 0.2F) + 0.1F + lv2.minZ;
         if (direction == Direction.DOWN) {
            e = (double)j + lv2.minY - 0.1F;
         }

         if (direction == Direction.UP) {
            e = (double)j + lv2.maxY + 0.1F;
         }

         if (direction == Direction.NORTH) {
            g = (double)k + lv2.minZ - 0.1F;
         }

         if (direction == Direction.SOUTH) {
            g = (double)k + lv2.maxZ + 0.1F;
         }

         if (direction == Direction.WEST) {
            d = (double)i + lv2.minX - 0.1F;
         }

         if (direction == Direction.EAST) {
            d = (double)i + lv2.maxX + 0.1F;
         }

         this.addParticle(new BlockDustParticle(this.world, d, e, g, 0.0, 0.0, 0.0, lv).setBlockPos(pos).move(0.2F).scale(0.6F));
      }
   }
