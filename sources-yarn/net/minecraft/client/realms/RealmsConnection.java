...
public class RealmsConnection {
   ...
   public void connect(final RealmsServer arg, final String string, final int i) {
      ...
      (new Thread("Realms-connect-task") {
            ...
            public void run() {
               ...
               try {
                  ...
                  RealmsConnection.this.connection = ClientConnection.connect(inetAddress, i, lv.options.shouldUseNativeTransport());
                  if (RealmsConnection.this.aborted) {
                     return;
                  }

                  RealmsConnection.this.connection
                     .setPacketListener(new ClientLoginNetworkHandler(RealmsConnection.this.connection, lv, RealmsConnection.this.onlineScreen, argx -> {
                     }));
                  if (RealmsConnection.this.aborted) {
                     return;
                  }
                  ...
                  RealmsConnection.this.connection.send(new LoginHelloC2SPacket(lv.getSession().getProfile()));
                  lv.setCurrentServerEntry(arg.method_31403(string));
                  ...
                  DisconnectedRealmsScreen lv = new DisconnectedRealmsScreen(
                     RealmsConnection.this.onlineScreen,
                     ScreenTexts.CONNECT_FAILED,
                     new TranslatableText("disconnect.genericReason", "Unknown host '" + string + "'")
                  );
                  lv.execute(() -> lv.openScreen(lv));
                  ...
                  DisconnectedRealmsScreen lv2 = new DisconnectedRealmsScreen(
