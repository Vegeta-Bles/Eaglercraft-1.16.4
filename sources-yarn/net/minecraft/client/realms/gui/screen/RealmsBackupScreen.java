...
public class RealmsBackupScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      (new Thread("Realms-fetch-backups") {
         ...
         public void run() {
            ...
            try {
               List<Backup> list = lv.backupsFor(RealmsBackupScreen.this.serverData.id).backups;
               RealmsBackupScreen.this.client.execute(() -> {
                  ...
                  for (Backup lvx : RealmsBackupScreen.this.backups) {
                     RealmsBackupScreen.this.backupObjectSelectionList.addEntry(lvx);
                  }
      ...
      this.downloadButton = this.addButton(
         new ButtonWidget(this.width - 135, row(1), 120, 20, new TranslatableText("mco.backup.button.download"), arg -> this.downloadClicked())
      );
      this.restoreButton = this.addButton(
         new ButtonWidget(this.width - 135, row(3), 120, 20, new TranslatableText("mco.backup.button.restore"), arg -> this.restoreClicked(this.selectedBackup))
      );
      this.changesButton = this.addButton(new ButtonWidget(this.width - 135, row(5), 120, 20, new TranslatableText("mco.backup.changes.tooltip"), arg -> {
   ...
   private void generateChangeList() {
      if (this.backups.size() > 1) {
         for (int i = 0; i < this.backups.size() - 1; i++) {
            ...
            if (!lv.metadata.isEmpty() && !lv2.metadata.isEmpty()) {
               for (String string : lv.metadata.keySet()) {
                  if (!string.contains("Uploaded") && lv2.metadata.containsKey(string)) {
                     if (!lv.metadata.get(string).equals(lv2.metadata.get(string))) {
   ...
   private void restoreClicked(int selectedBackup) {
      if (selectedBackup >= 0 && selectedBackup < this.backups.size() && !this.serverData.expired) {
         ...
         Date date = this.backups.get(selectedBackup).lastModifiedDate;
         String string = DateFormat.getDateTimeInstance(3, 3).format(date);
         String string2 = RealmsUtil.method_25282(date);
         Text lv = new TranslatableText("mco.configure.world.restore.question.line1", string, string2);
         Text lv2 = new TranslatableText("mco.configure.world.restore.question.line2");
   ...
   private void downloadClicked() {
      Text lv = new TranslatableText("mco.configure.world.restore.download.question.line1");
      Text lv2 = new TranslatableText("mco.configure.world.restore.download.question.line2");
      this.client.openScreen(new RealmsLongConfirmationScreen(bl -> {
         if (bl) {
   ...
   class BackupObjectSelectionList extends RealmsObjectSelectionList<RealmsBackupScreen.BackupObjectSelectionListEntry> {
      ...
      public void setSelected(@Nullable RealmsBackupScreen.BackupObjectSelectionListEntry arg) {
   ...
   class BackupObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsBackupScreen.BackupObjectSelectionListEntry> {
      ...
      private void renderBackupItem(MatrixStack arg, Backup arg2, int i, int j, int k, int l) {
         int m = arg2.isUploadedVersion() ? -8388737 : 16777215;
         ...
         RealmsBackupScreen.this.textRenderer.draw(arg, this.getMediumDatePresentation(arg2.lastModifiedDate), (float)(i + 40), (float)(j + 12), 5000268);
