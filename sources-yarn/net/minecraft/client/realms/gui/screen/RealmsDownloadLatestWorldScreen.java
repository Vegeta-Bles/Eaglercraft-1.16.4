...
public class RealmsDownloadLatestWorldScreen extends RealmsScreen {
   ...
   private void checkDownloadSize() {
      if (!this.finished) {
         if (!this.checked && this.getContentLength(this.worldDownload.downloadLink) >= 5368709120L) {
            Text lv = new TranslatableText("mco.download.confirmation.line1", SizeUnit.getUserFriendlyString(5368709120L));
            Text lv2 = new TranslatableText("mco.download.confirmation.line2");
   ...
   private void drawDots(MatrixStack arg) {
      ...
      this.textRenderer.draw(arg, DOTS[this.dotIndex % DOTS.length], (float)(this.width / 2 + i / 2 + 5), 50.0F, 16777215);
   }
