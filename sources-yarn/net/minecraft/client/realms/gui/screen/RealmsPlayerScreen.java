...
public class RealmsPlayerScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addButton(
         new ButtonWidget(
            ...
            arg -> this.client.openScreen(new RealmsInviteScreen(this.parent, this, this.serverData))
         )
      );
      this.removeButton = this.addButton(
         new ButtonWidget(
            ...
            arg -> this.uninvite(this.player)
         )
      );
      this.opdeopButton = this.addButton(
         new ButtonWidget(this.column2_x, row(9), this.column_width + 10, 20, new TranslatableText("mco.configure.world.invites.ops.tooltip"), arg -> {
      ...
      this.addButton(
         new ButtonWidget(
            this.column2_x + this.column_width / 2 + 2, row(12), this.column_width / 2 + 10 - 2, 20, ScreenTexts.BACK, arg -> this.backButtonClicked()
         )
      );
   ...
   private void uninvite(int index) {
      ...
      if (index >= 0 && index < this.serverData.players.size()) {
         ...
         RealmsConfirmScreen lv2 = new RealmsConfirmScreen(bl -> {
            if (bl) {
               RealmsClient lvx = RealmsClient.createRealmsClient();

               try {
                  lvx.uninvite(this.serverData.id, this.selectedInvited);
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      lv2.vertex(0.0, (double)this.height, 0.0).texture(0.0F, (float)(this.height - k) / 32.0F + 0.0F).color(64, 64, 64, 255).next();
      lv2.vertex((double)this.width, (double)this.height, 0.0)
         .texture((float)this.width / 32.0F, (float)(this.height - k) / 32.0F + 0.0F)
         .color(64, 64, 64, 255)
         .next();
      lv2.vertex((double)this.width, (double)k, 0.0).texture((float)this.width / 32.0F, 0.0F).color(64, 64, 64, 255).next();
      lv2.vertex(0.0, (double)k, 0.0).texture(0.0F, 0.0F).color(64, 64, 64, 255).next();
      lv.draw();
   ...
   protected void renderMousehoverTooltip(MatrixStack matrices, @Nullable Text arg2, int mouseX, int mouseY) {
      if (arg2 != null) {
         int k = mouseX + 12;
         int l = mouseY - 12;
         int m = this.textRenderer.getWidth(arg2);
         this.fillGradient(matrices, k - 3, l - 3, k + m + 3, l + 8 + 3, -1073741824, -1073741824);
         this.textRenderer.drawWithShadow(matrices, arg2, (float)k, (float)l, 16777215);
      }
   }
   ...
   class InvitedObjectSelectionList extends RealmsObjectSelectionList<RealmsPlayerScreen.InvitedObjectSelectionListEntry> {
      ...
      public void setSelected(@Nullable RealmsPlayerScreen.InvitedObjectSelectionListEntry arg) {
   ...
   class InvitedObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsPlayerScreen.InvitedObjectSelectionListEntry> {
      ...
      private void renderInvitedItem(MatrixStack matrices, PlayerInfo playerInfo, int x, int y, int mouseX, int mouseY) {
         ...
         RealmsPlayerScreen.this.textRenderer.draw(matrices, playerInfo.getName(), (float)(RealmsPlayerScreen.this.column1_x + 3 + 12), (float)(y + 1), m);
