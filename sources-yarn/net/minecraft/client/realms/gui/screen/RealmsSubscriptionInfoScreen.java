...
public class RealmsSubscriptionInfoScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addButton(new ButtonWidget(this.width / 2 - 100, row(12), 200, 20, ScreenTexts.BACK, arg -> this.client.openScreen(this.parent)));
      if (this.serverData.expired) {
         this.addButton(new ButtonWidget(this.width / 2 - 100, row(10), 200, 20, new TranslatableText("mco.configure.world.delete.button"), arg -> {
            Text lv = new TranslatableText("mco.configure.world.delete.question.line1");
            Text lv2 = new TranslatableText("mco.configure.world.delete.question.line2");
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      this.textRenderer.draw(matrices, subscriptionStartLabelText, (float)k, (float)row(0), 10526880);
      this.textRenderer.draw(matrices, this.startDate, (float)k, (float)row(1), 16777215);
      if (this.type == Subscription.SubscriptionType.NORMAL) {
         this.textRenderer.draw(matrices, timeLeftLabelText, (float)k, (float)row(3), 10526880);
      } else if (this.type == Subscription.SubscriptionType.RECURRING) {
         this.textRenderer.draw(matrices, daysLeftLabelText, (float)k, (float)row(3), 10526880);
      }
   ...
   private Text daysLeftPresentation(int daysLeft) {
      if (daysLeft < 0 && this.serverData.expired) {
         ...
         if (j > 0) {
            lv.append(Integer.toString(j)).append(" ");
            if (j == 1) {
               lv.append(field_26519);
            } else {
               lv.append(field_26520);
            }
         }
         ...
         if (k > 0) {
            ...
            lv.append(Integer.toString(k)).append(" ");
            if (k == 1) {
               lv.append(field_26521);
            } else {
               lv.append(field_26522);
            }
         }
