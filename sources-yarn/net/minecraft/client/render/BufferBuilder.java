...
public class BufferBuilder extends FixedColorVertexConsumer implements BufferVertexConsumer {
   ...
   public void sortQuads(float cameraX, float cameraY, float cameraZ) {
      ...
      for (int j = 0; j < i; j++) {
         fs[j] = getDistanceSq(
            floatBuffer, cameraX, cameraY, cameraZ, this.format.getVertexSizeInteger(), this.buildStart / 4 + j * this.format.getVertexSize()
         );
      }
   ...
   private void method_22628(FloatBuffer floatBuffer, int i) {
      ...
      ((Buffer)floatBuffer).limit(this.buildStart / 4 + (i + 1) * j);
      ((Buffer)floatBuffer).position(this.buildStart / 4 + i * j);
   }
   ...
   private static float getDistanceSq(FloatBuffer buffer, float x, float y, float z, int i, int j) {
      ...
      float xx = (l + o + r + u) * 0.25F - y;
      float yx = (m + p + s + v) * 0.25F - z;
      return w * w + xx * xx + yx * yx;
   }

   public void restoreState(BufferBuilder.State arg) {
      ((Buffer)arg.buffer).clear();
   ...
   public void vertex(
      ...
      if (this.colorFixed) {
         ...
         this.putShort(t + 0, (short)(light & 65535));
         this.putShort(t + 2, (short)(light >> 16 & 65535));
         this.putByte(t + 4, BufferVertexConsumer.method_24212(normalX));
         this.putByte(t + 5, BufferVertexConsumer.method_24212(normalY));
         this.putByte(t + 6, BufferVertexConsumer.method_24212(normalZ));
   ...
   public Pair<BufferBuilder.DrawArrayParameters, ByteBuffer> popData() {
      BufferBuilder.DrawArrayParameters lv = this.parameters.get(this.lastParameterIndex++);
      ((Buffer)this.buffer).position(this.nextDrawStart);
      this.nextDrawStart = this.nextDrawStart + lv.getCount() * lv.getVertexFormat().getVertexSize();
