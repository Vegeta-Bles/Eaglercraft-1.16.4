...
public interface BufferVertexConsumer extends VertexConsumer {
   ...
   default VertexConsumer color(int red, int green, int blue, int alpha) {
      ...
      if (lv.getType() != VertexFormatElement.Type.COLOR) {
         return this;
      } else if (lv.getFormat() != VertexFormatElement.Format.UBYTE) {
         throw new IllegalStateException();
      } else {
   ...
   default VertexConsumer texture(short u, short v, int index) {
      ...
      if (lv.getType() != VertexFormatElement.Type.UV || lv.getIndex() != index) {
         return this;
      } else if (lv.getFormat() != VertexFormatElement.Format.SHORT) {
         throw new IllegalStateException();
      } else {
         this.putShort(0, u);
         this.putShort(2, v);
   ...
   default VertexConsumer normal(float x, float y, float z) {
      ...
      if (lv.getType() != VertexFormatElement.Type.NORMAL) {
         return this;
      } else if (lv.getFormat() != VertexFormatElement.Format.BYTE) {
         throw new IllegalStateException();
      } else {
