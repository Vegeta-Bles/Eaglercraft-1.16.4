...
public class Frustum {
   ...
   private void transform(Matrix4f function, int x, int y, int z, int index) {
      Vector4f lv = new Vector4f((float)x, (float)y, (float)z, 1.0F);
   ...
   private boolean isAnyCornerVisible(float x1, float y1, float z1, float x2, float y2, float z2) {
      for (int l = 0; l < 6; l++) {
         ...
         if (!(lv.dotProduct(new Vector4f(x1, y1, z1, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x2, y1, z1, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x1, y2, z1, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x2, y2, z1, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x1, y1, z2, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x2, y1, z2, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x1, y2, z2, 1.0F)) > 0.0F)
            && !(lv.dotProduct(new Vector4f(x2, y2, z2, 1.0F)) > 0.0F)) {
            return false;
         }
      }
