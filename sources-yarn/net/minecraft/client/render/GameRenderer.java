...
public class GameRenderer implements SynchronousResourceReloadListener, AutoCloseable {
   ...
   public GameRenderer(MinecraftClient client, ResourceManager arg2, BufferBuilderStorage arg3) {
      this.client = client;
   ...
   private void loadShader(Identifier arg) {
      ...
      try {
         ...
         LOGGER.warn("Failed to load shader: {}", arg, var3);
         this.forcedShaderIndex = SHADER_COUNT;
         this.shadersEnabled = false;
      } catch (JsonSyntaxException var4) {
         LOGGER.warn("Failed to parse shader: {}", arg, var4);
         this.forcedShaderIndex = SHADER_COUNT;
         this.shadersEnabled = false;
      }
   }
   ...
   public void updateTargetedEntity(float tickDelta) {
      ...
      if (lv != null) {
         if (this.client.world != null) {
            ...
            EntityHitResult lv6 = ProjectileUtil.raycast(lv, lv2, lv4, lv5, arg -> !arg.isSpectator() && arg.collides(), e);
            if (lv6 != null) {
   ...
   private void bobView(MatrixStack arg, float f) {
      if (this.client.getCameraEntity() instanceof PlayerEntity) {
         ...
         arg.translate((double)(MathHelper.sin(h * (float) Math.PI) * i * 0.5F), (double)(-Math.abs(MathHelper.cos(h * (float) Math.PI) * i)), 0.0);
   ...
   public Matrix4f getBasicProjectionMatrix(Camera arg, float f, boolean bl) {
      ...
      if (this.zoom != 1.0F) {
         lv.translate((double)this.zoomX, (double)(-this.zoomY), 0.0);
   ...
   public void render(float tickDelta, long startTime, boolean tick) {
      ...
      if (!this.client.skipGameRender) {
         ...
         RenderSystem.loadIdentity();
         RenderSystem.ortho(
            0.0, (double)lv.getFramebufferWidth() / lv.getScaleFactor(), (double)lv.getFramebufferHeight() / lv.getScaleFactor(), 0.0, 1000.0, 3000.0
         );
         RenderSystem.matrixMode(5888);
         RenderSystem.loadIdentity();
         ...
         if (tick && this.client.world != null) {
            ...
            if (this.client.player != null) {
               ...
               if (g > 0.0F && this.client.player.hasStatusEffect(StatusEffects.NAUSEA) && this.client.options.distortionEffectScale < 1.0F) {
                  this.method_31136(g * (1.0F - this.client.options.distortionEffectScale));
               }
            }
            ...
            if (!this.client.options.hudHidden || this.client.currentScreen != null) {
               ...
               this.client.inGameHud.render(lv2, tickDelta);
               RenderSystem.clear(256, MinecraftClient.IS_SYSTEM_MAC);
            }
         ...
         if (this.client.overlay != null) {
            try {
               ...
               CrashReport lv3 = CrashReport.create(var13, "Rendering overlay");
            ...
            try {
               ...
               CrashReport lv5 = CrashReport.create(var12, "Rendering screen");
   ...
   private void updateWorldIcon() {
      if (this.client.worldRenderer.getCompletedChunkCount() > 10
         ...
         NativeImage lv = ScreenshotUtils.takeScreenshot(
         ...
         Util.getIoWorkerExecutor().execute(() -> {
            ...
            try (NativeImage lvx = new NativeImage(64, 64, false)) {
               lv.resizeSubRectTo(k, l, i, j, lvx);
               lvx.writeFile(this.client.getServer().getIconFile());
   ...
   public void renderWorld(float tickDelta, long limitTime, MatrixStack matrix) {
      ...
      MatrixStack lv2 = new MatrixStack();
      lv2.peek().getModel().multiply(this.getBasicProjectionMatrix(lv, tickDelta, true));
      this.bobViewWhenHurt(lv2, tickDelta);
      if (this.client.options.bobView) {
      ...
      if (g > 0.0F) {
         ...
         lv2.multiply(lv3.getDegreesQuaternion(((float)this.ticks + tickDelta) * (float)i));
      ...
      matrix.multiply(Vector3f.POSITIVE_X.getDegreesQuaternion(lv.getPitch()));
      matrix.multiply(Vector3f.POSITIVE_Y.getDegreesQuaternion(lv.getYaw() + 180.0F));
      this.client.worldRenderer.render(matrix, tickDelta, limitTime, bl, lv, this, this.lightmapTextureManager, lv4);
      this.client.getProfiler().swap("hand");
      if (this.renderHand) {
         RenderSystem.clear(256, MinecraftClient.IS_SYSTEM_MAC);
         this.renderHand(matrix, lv, tickDelta);
      }
   ...
   private void renderFloatingItem(int scaledWidth, int scaledHeight, float tickDelta) {
      if (this.floatingItem != null && this.floatingItemTimeLeft > 0) {
         ...
         lv.push();
         lv.translate(
            (double)((float)(scaledWidth / 2) + o * MathHelper.abs(MathHelper.sin(n * 2.0F))),
            (double)((float)(scaledHeight / 2) + p * MathHelper.abs(MathHelper.sin(n * 2.0F))),
            -50.0
         );
         ...
         this.client.getItemRenderer().renderItem(this.floatingItem, ModelTransformation.Mode.FIXED, 15728880, OverlayTexture.DEFAULT_UV, lv, lv2);
         lv.pop();
         lv2.draw();
