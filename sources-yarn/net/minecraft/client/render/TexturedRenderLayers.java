...
public class TexturedRenderLayers {
   ...
   public static final SpriteIdentifier[] BED_TEXTURES = Arrays.stream(DyeColor.values())
      ...
      .map(arg -> new SpriteIdentifier(BEDS_ATLAS_TEXTURE, new Identifier("entity/bed/" + arg.getName())))
   ...
   public static void addDefaultTextures(Consumer<SpriteIdentifier> adder) {
      ...
      for (BannerPattern lv : BannerPattern.values()) {
         adder.accept(new SpriteIdentifier(BANNER_PATTERNS_ATLAS_TEXTURE, lv.getSpriteId(true)));
         adder.accept(new SpriteIdentifier(SHIELD_PATTERNS_ATLAS_TEXTURE, lv.getSpriteId(false)));
      }

      WOOD_TYPE_TEXTURES.values().forEach(adder);

      for (SpriteIdentifier lv2 : BED_TEXTURES) {
         adder.accept(lv2);
      }
