...
public class ChunkBuilder {
   ...
   private void scheduleRunTasks() {
      if (!this.threadBuffers.isEmpty()) {
         ChunkBuilder.BuiltChunk.Task lv = this.rebuildQueue.poll();
         if (lv != null) {
            BlockBufferBuilderStorage lv2 = this.threadBuffers.poll();
            this.queuedTaskCount = this.rebuildQueue.size();
            this.bufferCount = this.threadBuffers.size();
            CompletableFuture.runAsync(() -> {
            }, this.executor).thenCompose(void_ -> lv.run(lv2)).whenComplete((arg2, throwable) -> {
               if (throwable != null) {
                  CrashReport lvx = CrashReport.create(throwable, "Batching chunks");
                  MinecraftClient.getInstance().setCrashReport(MinecraftClient.getInstance().addDetailsToCrashReport(lvx));
               } else {
                  this.mailbox.send(() -> {
                     if (arg2 == ChunkBuilder.Result.SUCCESSFUL) {
                        lv2.clear();
                     } else {
                        lv2.reset();
                     }
   ...
   public boolean upload() {
      boolean bl;
      Runnable runnable;
      for (bl = false; (runnable = this.uploadQueue.poll()) != null; bl = true) {
         runnable.run();
      }
   ...
   public class BuiltChunk {
      ...
      private final Map<RenderLayer, VertexBuffer> buffers = RenderLayer.getBlockLayers()
         .stream()
         .collect(Collectors.toMap(argx -> argx, argx -> new VertexBuffer(VertexFormats.POSITION_COLOR_TEXTURE_LIGHT_NORMAL)));
      ...
      public boolean scheduleSort(RenderLayer layer, ChunkBuilder chunkRenderer) {
         ChunkBuilder.ChunkData lv = this.getData();
         ...
         if (!lv.initializedLayers.contains(layer)) {
            ...
            this.sortTask = new ChunkBuilder.BuiltChunk.SortTask(this.getSquaredCameraDistance(), lv);
      ...
      public ChunkBuilder.BuiltChunk.Task createRebuildTask() {
         ...
         this.rebuildTask = new ChunkBuilder.BuiltChunk.RebuildTask(this.getSquaredCameraDistance(), lv2);
         return this.rebuildTask;
      }

      public void scheduleRebuild(ChunkBuilder chunkRenderer) {
         ChunkBuilder.BuiltChunk.Task lv = this.createRebuildTask();
         chunkRenderer.send(lv);
      }

      private void setNoCullingBlockEntities(Set<BlockEntity> noCullingBlockEntities) {
         Set<BlockEntity> set2 = Sets.newHashSet(noCullingBlockEntities);
      ...
      class RebuildTask extends ChunkBuilder.BuiltChunk.Task {
         ...
         public RebuildTask(double d, ChunkRendererRegion arg2) {
            super(d);
            this.region = arg2;
         }
         ...
         public CompletableFuture<ChunkBuilder.Result> run(BlockBufferBuilderStorage buffers) {
            if (this.cancelled.get()) {
               ...
               float g = (float)lv.y;
               float h = (float)lv.z;
               ChunkBuilder.ChunkData lv2 = new ChunkBuilder.ChunkData();
               ...
               if (this.cancelled.get()) {
                  ...
                  lv2.initializedLayers.forEach(arg2 -> list.add(ChunkBuilder.this.scheduleUpload(buffers.get(arg2), BuiltChunk.this.getBuffer(arg2))));
         ...
         private Set<BlockEntity> render(float cameraX, float cameraY, float cameraZ, ChunkBuilder.ChunkData data, BlockBufferBuilderStorage buffers) {
            ...
            if (lv4 != null) {
               ...
               for (BlockPos lv7 : BlockPos.iterate(lv, lv2)) {
                  ...
                  FluidState lv11 = lv4.getFluidState(lv7);
                  if (!lv11.isEmpty()) {
                  ...
                  if (lv8.getRenderType() != BlockRenderType.INVISIBLE) {
                     ...
                     lv5.push();
                     lv5.translate((double)(lv7.getX() & 15), (double)(lv7.getY() & 15), (double)(lv7.getZ() & 15));
      ...
      class SortTask extends ChunkBuilder.BuiltChunk.Task {
         ...
         public SortTask(double d, ChunkBuilder.ChunkData arg2) {
            super(d);
            this.data = arg2;
         }
      ...
      abstract class Task implements Comparable<ChunkBuilder.BuiltChunk.Task> {
         ...
         public int compareTo(ChunkBuilder.BuiltChunk.Task arg) {
