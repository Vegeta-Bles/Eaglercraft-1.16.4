...
public class CaveDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      for (Entry<BlockPos, BlockPos> entry : this.field_4507.entrySet()) {
         ...
         float j = this.field_4508.get(lv4);
         if (lv.isWithinDistance(lv4, 160.0)) {
            WorldRenderer.drawBox(
               lv3,
               (double)((float)lv4.getX() + 0.5F) - cameraX - (double)j,
               (double)((float)lv4.getY() + 0.5F) - cameraY - (double)j,
               (double)((float)lv4.getZ() + 0.5F) - cameraZ - (double)j,
               (double)((float)lv4.getX() + 0.5F) - cameraX + (double)j,
               (double)((float)lv4.getY() + 0.5F) - cameraY + (double)j,
               (double)((float)lv4.getZ() + 0.5F) - cameraZ + (double)j,
               g,
      ...
      for (BlockPos lv6 : this.field_4506) {
         if (lv.isWithinDistance(lv6, 160.0)) {
            WorldRenderer.drawBox(
               lv3,
               (double)lv6.getX() - cameraX,
               (double)lv6.getY() - cameraY,
               (double)lv6.getZ() - cameraZ,
               (double)((float)lv6.getX() + 1.0F) - cameraX,
               (double)((float)lv6.getY() + 1.0F) - cameraY,
               (double)((float)lv6.getZ() + 1.0F) - cameraZ,
               1.0F,
               1.0F,
               1.0F,
               1.0F
