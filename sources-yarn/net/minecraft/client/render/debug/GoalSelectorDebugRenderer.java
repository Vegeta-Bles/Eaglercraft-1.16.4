   ...
   public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      this.goalSelectors.forEach((integer, list) -> {
         for (int i = 0; i < list.size(); i++) {
            GoalSelectorDebugRenderer.GoalSelector lvx = list.get(i);
            if (lv2.isWithinDistance(lvx.pos, 160.0)) {
               double d = (double)lvx.pos.getX() + 0.5;
               double e = (double)lvx.pos.getY() + 2.0 + (double)i * 0.25;
               double f = (double)lvx.pos.getZ() + 0.5;
               int j = lvx.field_18785 ? -16711936 : -3355444;
               DebugRenderer.drawString(lvx.name, d, e, f, j);
            }
         }
      });
