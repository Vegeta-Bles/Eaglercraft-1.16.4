...
public class VillageDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void addPointOfInterest(VillageDebugRenderer.PointOfInterest arg) {
   ...
   public void setFreeTicketCount(BlockPos pos, int freeTicketCount) {
      VillageDebugRenderer.PointOfInterest lv = this.pointsOfInterest.get(pos);
      if (lv == null) {
   ...
   private void method_24805() {
      this.brains.entrySet().removeIf(entry -> {
         Entity lv = this.client.world.getEntityById(entry.getValue().field_18924);
   ...
   private static void drawString(String string, VillageDebugRenderer.PointOfInterest pointOfInterest, int offsetY, int color) {
      ...
      drawString(string, lv, offsetY, color);
   }
   ...
   private Set<String> method_29385(VillageDebugRenderer.PointOfInterest arg) {
   ...
   private Collection<UUID> getBrains(BlockPos pointOfInterest) {
      return this.brains
         .values()
         .stream()
         .filter(arg2 -> arg2.isPointOfInterest(pointOfInterest))
         .map(VillageDebugRenderer.Brain::getUuid)
         .collect(Collectors.toSet());
   }

   private Collection<UUID> method_29386(BlockPos arg) {
      return this.brains.values().stream().filter(arg2 -> arg2.method_29388(arg)).map(VillageDebugRenderer.Brain::getUuid).collect(Collectors.toSet());
   }

   private Map<BlockPos, List<String>> getGhostPointsOfInterest() {
      Map<BlockPos, List<String>> map = Maps.newHashMap();

      for (VillageDebugRenderer.Brain lv : this.brains.values()) {
