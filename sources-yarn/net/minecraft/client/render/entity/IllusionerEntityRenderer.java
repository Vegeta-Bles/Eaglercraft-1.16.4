   ...
   public void render(IllusionerEntity arg, float f, float g, MatrixStack arg2, VertexConsumerProvider arg3, int i) {
      if (arg.isInvisible()) {
         ...
         for (int j = 0; j < lvs.length; j++) {
            arg2.push();
            arg2.translate(
               lvs[j].x + (double)MathHelper.cos((float)j + h * 0.5F) * 0.025,
               lvs[j].y + (double)MathHelper.cos((float)j + h * 0.75F) * 0.0125,
               lvs[j].z + (double)MathHelper.cos((float)j + h * 0.7F) * 0.025
            );
