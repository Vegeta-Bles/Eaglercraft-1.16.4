...
public class EndermiteEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public EndermiteEntityModel() {
      ...
      for (int i = 0; i < this.field_3368.length; i++) {
         ...
         this.field_3368[i]
            .addCuboid(
               (float)field_3366[i][0] * -0.5F,
               0.0F,
               (float)field_3366[i][2] * -0.5F,
               (float)field_3366[i][0],
               (float)field_3366[i][1],
               (float)field_3366[i][2]
            );
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
      for (int k = 0; k < this.field_3368.length; k++) {
         this.field_3368[k].yaw = MathHelper.cos(animationProgress * 0.9F + (float)k * 0.15F * (float) Math.PI)
            * (float) Math.PI
            * 0.01F
            * (float)(1 + Math.abs(k - 2));
         this.field_3368[k].pivotX = MathHelper.sin(animationProgress * 0.9F + (float)k * 0.15F * (float) Math.PI)
            * (float) Math.PI
            * 0.1F
            * (float)Math.abs(k - 2);
      }
