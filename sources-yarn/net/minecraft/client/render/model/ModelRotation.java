...
public enum ModelRotation implements ModelBakeSettings {
   ...
   private static final Map<Integer, ModelRotation> BY_INDEX = Arrays.stream(values()).collect(Collectors.toMap(arg -> arg.index, arg -> (ModelRotation)arg));
   ...
   private ModelRotation(int x, int y) {
      ...
      Quaternion lv = new Quaternion(new Vector3f(0.0F, 1.0F, 0.0F), (float)(-y), true);
      lv.hamiltonProduct(new Quaternion(new Vector3f(1.0F, 0.0F, 0.0F), (float)(-x), true));
