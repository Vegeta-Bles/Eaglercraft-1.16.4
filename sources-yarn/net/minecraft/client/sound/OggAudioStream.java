...
public class OggAudioStream implements AudioStream {
   ...
   public OggAudioStream(InputStream inputStream) throws IOException {
      ...
      try {
         ...
         ((Buffer)this.buffer).position(this.buffer.position() + intBuffer.get(0));
   ...
   private boolean readHeader() throws IOException {
      ...
      if (j == 0) {
         ...
         if (k == -1) {
            ...
            ((Buffer)this.buffer).limit(i + k);
            ((Buffer)this.buffer).position(i);
            this.buffer.put(bs, 0, k);
            ((Buffer)this.buffer).position(l);
            return true;
         }
      }
   }
   ...
   private boolean readOggFile(OggAudioStream.ChannelList arg) throws IOException {
      if (this.pointer == 0L) {
         return false;
      } else {
         MemoryStack memoryStack = MemoryStack.stackPush();
         ...
         try {
            ...
            while (true) {
               ...
               ((Buffer)this.buffer).position(this.buffer.position() + i);
   ...
   private void readChannels(FloatBuffer floatBuffer, OggAudioStream.ChannelList arg) {
      while (floatBuffer.hasRemaining()) {
         arg.addChannel(floatBuffer.get());
      }
   }

   private void readChannels(FloatBuffer floatBuffer, FloatBuffer floatBuffer2, OggAudioStream.ChannelList arg) {
   ...
   public ByteBuffer getBuffer(int size) throws IOException {
      OggAudioStream.ChannelList lv = new OggAudioStream.ChannelList(size + 8192);
   ...
   public ByteBuffer getBuffer() throws IOException {
      OggAudioStream.ChannelList lv = new OggAudioStream.ChannelList(16384);
