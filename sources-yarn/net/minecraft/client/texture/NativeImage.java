...
public final class NativeImage implements AutoCloseable {
   ...
   public static NativeImage read(@Nullable NativeImage.Format arg, InputStream inputStream) throws IOException {
   ...
   public static NativeImage read(@Nullable NativeImage.Format arg, ByteBuffer byteBuffer) throws IOException {
      if (arg != null && !arg.isWriteable()) {
         ...
         try {
            ...
            ByteBuffer byteBuffer2 = STBImage.stbi_load_from_memory(byteBuffer, intBuffer, intBuffer2, intBuffer3, arg == null ? 0 : arg.channelCount);
            if (byteBuffer2 == null) {
               throw new IOException("Could not load image: " + STBImage.stbi_failure_reason());
            }

            var8 = new NativeImage(
               arg == null ? NativeImage.Format.getFormat(intBuffer3.get(0)) : arg,
               intBuffer.get(0),
               intBuffer2.get(0),
               true,
   ...
   private boolean write(WritableByteChannel writableByteChannel) throws IOException {
      NativeImage.WriteCallback lv = new NativeImage.WriteCallback(writableByteChannel);
   ...
   public void resizeSubRectTo(int x, int y, int width, int height, NativeImage targetImage) {
      ...
      if (targetImage.getFormat() != this.format) {
         ...
         STBImageResize.nstbir_resize_uint8(
            this.pointer + (long)((x + y * this.getWidth()) * m),
            width,
            height,
            this.getWidth() * m,
            targetImage.pointer,
