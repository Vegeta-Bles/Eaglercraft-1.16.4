...
public class Sprite implements AutoCloseable {
   ...
   protected Sprite(SpriteAtlasTexture arg, Sprite.Info arg2, int maxLevel, int atlasWidth, int atlasHeight, int x, int y, NativeImage arg3) {
      this.atlas = arg;
      ...
      if (lv.getFrameCount() > 0) {
         int r = lv.getFrameIndexSet().stream().max(Integer::compareTo).get() + 1;
         ...
         for (int w = 0; w < q; w++) {
            for (int xx = 0; xx < p; xx++) {
               ...
               this.frameXs[yx] = xx;
               this.frameYs[yx] = w;
               list.add(new AnimationFrameResourceMetadata(yx, -1));
            }
         }
      ...
      this.info = new Sprite.Info(arg2.id, n, o, lv);
      ...
      try {
         try {
            ...
            CrashReport lv2 = CrashReport.create(var19, "Generating mipmaps for frame");
         ...
         CrashReport lv4 = CrashReport.create(var20, "Applying mipmap");
      ...
      if (lv.shouldInterpolate()) {
         this.interpolation = new Sprite.Interpolation(arg2, maxLevel);
      } else {
   ...
   final class Interpolation implements AutoCloseable {
      ...
      private Interpolation(Sprite.Info arg2, int mipmap) {
