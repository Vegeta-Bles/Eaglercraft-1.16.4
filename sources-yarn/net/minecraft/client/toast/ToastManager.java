...
public class ToastManager extends DrawableHelper {
   ...
   public void draw(MatrixStack matrices) {
      if (!this.client.options.hudHidden) {
         for (int i = 0; i < this.visibleEntries.length; i++) {
            ToastManager.Entry<?> lv = this.visibleEntries[i];
            ...
            if (this.visibleEntries[i] == null && !this.toastQueue.isEmpty()) {
               this.visibleEntries[i] = new ToastManager.Entry(this.toastQueue.removeFirst());
            }
         }
      }
   }
   ...
   public <T extends Toast> T getToast(Class<? extends T> toastClass, Object type) {
      for (ToastManager.Entry<?> lv : this.visibleEntries) {
         if (lv != null && toastClass.isAssignableFrom(lv.getInstance().getClass()) && lv.getInstance().getType().equals(type)) {
            return (T)lv.getInstance();
         }
      }

      for (Toast lv2 : this.toastQueue) {
         if (toastClass.isAssignableFrom(lv2.getClass()) && lv2.getType().equals(type)) {
            return (T)lv2;
         }
      }
   ...
   class Entry<T extends Toast> {
      ...
      public boolean draw(int x, int y, MatrixStack matrices) {
         ...
         RenderSystem.translatef(
            (float)x - (float)this.instance.getWidth() * this.getDisappearProgress(l), (float)(y * this.instance.getHeight()), (float)(800 + y)
         );
         Toast.Visibility lv = this.instance.draw(matrices, ToastManager.this, l - this.field_2242);
