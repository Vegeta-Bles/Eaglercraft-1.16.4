...
public class FindTreeTutorialStepHandler implements TutorialStepHandler {
         ...
         if (this.ticks == 1) {
            ...
            if (lv != null) {
               for (Block lv2 : TREE_BLOCKS) {
                  if (lv.inventory.contains(new ItemStack(lv2))) {
                     this.manager.setStep(TutorialStep.CRAFT_PLANKS);
                     return;
                  }
               }

               if (hasBrokenTreeBlocks(lv)) {
                  this.manager.setStep(TutorialStep.CRAFT_PLANKS);
                  return;
               }
            }
   ...
   public static boolean hasBrokenTreeBlocks(ClientPlayerEntity player) {
      for (Block lv : TREE_BLOCKS) {
         if (player.getStatHandler().getStat(Stats.MINED.getOrCreateStat(lv)) > 0) {
            return true;
         }
      }
