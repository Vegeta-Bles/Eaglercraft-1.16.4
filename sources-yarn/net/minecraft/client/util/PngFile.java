...
public class PngFile {
   ...
   public PngFile(String name, InputStream in) throws IOException {
      ...
      try (PngFile.Reader lv = createReader(in)) {
         STBIReadCallback sTBIReadCallback = STBIReadCallback.create(lv::read);
         Throwable var8 = null;

         try {
            STBISkipCallback sTBISkipCallback = STBISkipCallback.create(lv::skip);
   ...
   static class ChannelReader extends PngFile.Reader {
      ...
      private void readToBuffer(int size) throws IOException {
         ...
         ((Buffer)byteBuffer).position(this.bufferPosition);
