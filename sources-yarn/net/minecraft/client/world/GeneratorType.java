...
public abstract class GeneratorType {
   ...
   private static final GeneratorType SINGLE_BIOME_CAVES = new GeneratorType("single_biome_caves") {
      ...
      public GeneratorOptions createDefaultOptions(DynamicRegistryManager.Impl registryManager, long seed, boolean generateStructures, boolean bonusChest) {
         Registry<Biome> lv = registryManager.get(Registry.BIOME_KEY);
         Registry<DimensionType> lv2 = registryManager.get(Registry.DIMENSION_TYPE_KEY);
         Registry<ChunkGeneratorSettings> lv3 = registryManager.get(Registry.NOISE_SETTINGS_WORLDGEN);
         return new GeneratorOptions(
            seed,
            generateStructures,
            bonusChest,
            GeneratorOptions.method_29962(
               DimensionType.createDefaultDimensionOptions(lv2, lv, lv3, seed),
               () -> lv2.getOrThrow(DimensionType.OVERWORLD_CAVES_REGISTRY_KEY),
               this.getChunkGenerator(lv, lv3, seed)
            )
         );
      }
   ...
   protected static final Map<Optional<GeneratorType>, GeneratorType.ScreenProvider> SCREEN_PROVIDERS = ImmutableMap.of(
      ...
      (GeneratorType.ScreenProvider)(screen, generatorOptions) -> {
         ChunkGenerator lv = generatorOptions.getChunkGenerator();
         return new CustomizeFlatLevelScreen(
            ...
            lv instanceof FlatChunkGenerator
               ? ((FlatChunkGenerator)lv).getConfig()
   ...
   private static GeneratorOptions createFixedBiomeOptions(
      ...
      BiomeSource lv = new FixedBiomeSource(biome);
      Registry<DimensionType> lv2 = registryManager.get(Registry.DIMENSION_TYPE_KEY);
      Registry<ChunkGeneratorSettings> lv3 = registryManager.get(Registry.NOISE_SETTINGS_WORLDGEN);
      Supplier<ChunkGeneratorSettings> supplier;
      if (type == SINGLE_BIOME_CAVES) {
         supplier = () -> lv3.getOrThrow(ChunkGeneratorSettings.CAVES);
      } else if (type == SINGLE_BIOME_FLOATING_ISLANDS) {
         supplier = () -> lv3.getOrThrow(ChunkGeneratorSettings.FLOATING_ISLANDS);
      } else {
         supplier = () -> lv3.getOrThrow(ChunkGeneratorSettings.OVERWORLD);
      }
      ...
      return new GeneratorOptions(
         ...
         generatorOptions.shouldGenerateStructures(),
         generatorOptions.hasBonusChest(),
         GeneratorOptions.method_28608(lv2, generatorOptions.getDimensions(), new NoiseChunkGenerator(lv, generatorOptions.getSeed(), supplier))
      );
   }
   ...
   public GeneratorOptions createDefaultOptions(DynamicRegistryManager.Impl registryManager, long seed, boolean generateStructures, boolean bonusChest) {
      ...
      return new GeneratorOptions(
         ...
         generateStructures,
         bonusChest,
         GeneratorOptions.method_28608(lv2, DimensionType.createDefaultDimensionOptions(lv2, lv, lv3, seed), this.getChunkGenerator(lv, lv3, seed))
      );
   }
