...
public interface CommandSource {
   ...
   static <T> void forEachMatching(Iterable<T> candidates, String string, Function<T, Identifier> identifier, Consumer<T> action) {
      ...
      for (T object : candidates) {
         ...
         } else if (method_27136(string, lv.getNamespace()) || lv.getNamespace().equals("minecraft") && method_27136(string, lv.getPath())) {
            action.accept(object);
         }
      }
   }
   ...
   static CompletableFuture<Suggestions> suggestIdentifiers(Iterable<Identifier> candidates, SuggestionsBuilder builder, String string) {
      ...
      forEachMatching(candidates, string2, string, arg -> arg, arg -> builder.suggest(string + arg));
   ...
   static CompletableFuture<Suggestions> suggestIdentifiers(Iterable<Identifier> candidates, SuggestionsBuilder builder) {
      ...
      forEachMatching(candidates, string, arg -> arg, arg -> builder.suggest(arg.toString()));
      return builder.buildFuture();
   }
   ...
   static CompletableFuture<Suggestions> suggestPositions(
      ...
      if (Strings.isNullOrEmpty(string)) {
         for (CommandSource.RelativePosition lv : candidates) {
         ...
         if (strings.length == 1) {
            for (CommandSource.RelativePosition lv2 : candidates) {
            ...
            for (CommandSource.RelativePosition lv3 : candidates) {
   ...
   static CompletableFuture<Suggestions> suggestColumnPositions(
      ...
      List<String> list = Lists.newArrayList();
      if (Strings.isNullOrEmpty(string)) {
         for (CommandSource.RelativePosition lv : collection) {
         ...
         String[] strings = string.split(" ");
         if (strings.length == 1) {
            for (CommandSource.RelativePosition lv2 : collection) {
