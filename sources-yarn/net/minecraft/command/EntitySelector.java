...
public class EntitySelector {
   ...
   public List<? extends Entity> getEntities(ServerCommandSource arg) throws CommandSyntaxException {
      ...
      if (!this.includesNonPlayers) {
         ...
         return (List<? extends Entity>)(lv == null ? Collections.emptyList() : Lists.newArrayList(new ServerPlayerEntity[]{lv}));
         ...
         if (this.senderOnly) {
            return (List<? extends Entity>)(arg.getEntity() != null && predicate.test(arg.getEntity())
               ? Lists.newArrayList(new Entity[]{arg.getEntity()})
   ...
   private void appendEntitiesFromWorld(List<Entity> list, ServerWorld arg, Vec3d arg2, Predicate<Entity> predicate) {
      if (this.box != null) {
         list.addAll(arg.getEntitiesByType((EntityType<? extends Entity>)this.type, this.box.offset(arg2), predicate));
      } else {
   ...
   public List<ServerPlayerEntity> getPlayers(ServerCommandSource arg) throws CommandSyntaxException {
      ...
      if (this.playerName != null) {
         ServerPlayerEntity lv = arg.getMinecraftServer().getPlayerManager().getPlayer(this.playerName);
         return (List<ServerPlayerEntity>)(lv == null ? Collections.emptyList() : Lists.newArrayList(new ServerPlayerEntity[]{lv}));
      } else if (this.uuid != null) {
         ServerPlayerEntity lv2 = arg.getMinecraftServer().getPlayerManager().getPlayer(this.uuid);
         return (List<ServerPlayerEntity>)(lv2 == null ? Collections.emptyList() : Lists.newArrayList(new ServerPlayerEntity[]{lv2}));
      } else {
         Vec3d lv3 = this.positionOffset.apply(arg.getPosition());
         Predicate<Entity> predicate = this.getPositionPredicate(lv3);
         if (this.senderOnly) {
            if (arg.getEntity() instanceof ServerPlayerEntity) {
               ServerPlayerEntity lv4 = (ServerPlayerEntity)arg.getEntity();
               if (predicate.test(lv4)) {
                  return Lists.newArrayList(new ServerPlayerEntity[]{lv4});
               }
            }
            ...
            if (this.isLocalWorldOnly()) {
               list = arg.getWorld().getPlayers(predicate::test);
            } else {
            ...
            return this.getEntities(lv3, list);
         }
      }
   }
