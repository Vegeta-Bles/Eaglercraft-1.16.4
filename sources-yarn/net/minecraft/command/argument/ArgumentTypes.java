...
public class ArgumentTypes {
   ...
   public static <T extends ArgumentType<?>> void register(String id, Class<T> class_, ArgumentSerializer<T> arg) {
      ...
      } else if (idMap.containsKey(lv)) {
         throw new IllegalArgumentException("'" + lv + "' is already a registered serializer!");
      } else {
         ArgumentTypes.Entry<T> lv2 = new ArgumentTypes.Entry<>(class_, arg, lv);
   ...
   @Nullable
   private static ArgumentTypes.Entry<?> byId(Identifier arg) {
   ...
   public static <T extends ArgumentType<?>> void toPacket(PacketByteBuf arg, T argumentType) {
      ArgumentTypes.Entry<T> lv = (ArgumentTypes.Entry<T>)byClass(argumentType);
      if (lv == null) {
   ...
   public static ArgumentType<?> fromPacket(PacketByteBuf buf) {
      ...
      ArgumentTypes.Entry<?> lv2 = byId(lv);
      if (lv2 == null) {
         LOGGER.error("Could not deserialize {}", lv);
         return null;
      } else {
         return lv2.serializer.fromPacket(buf);
      }
   }
   ...
   private static <T> void getAllArgumentTypes(CommandNode<T> node, Set<ArgumentType<?>> argumentTypes, Set<CommandNode<T>> ignoredNodes) {
      if (ignoredNodes.add(node)) {
         ...
         if (commandNode2 != null) {
            getAllArgumentTypes(commandNode2, argumentTypes, ignoredNodes);
         }
      }
   }
