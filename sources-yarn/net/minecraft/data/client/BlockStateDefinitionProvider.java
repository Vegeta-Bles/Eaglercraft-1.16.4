...
public class BlockStateDefinitionProvider implements DataProvider {
   ...
   public void run(DataCache cache) {
      ...
      BiConsumer<Identifier, Supplier<JsonElement>> biConsumer = (arg, supplier) -> {
         Supplier<JsonElement> supplier2 = map2.put(arg, supplier);
         if (supplier2 != null) {
      ...
      List<Block> list = Registry.BLOCK.stream().filter(arg -> !map.containsKey(arg)).collect(Collectors.toList());
      if (!list.isEmpty()) {
   ...
   private <T> void writeJsons(DataCache cache, Path root, Map<T, ? extends Supplier<JsonElement>> jsons, BiFunction<Path, T, Path> locator) {
      jsons.forEach((object, supplier) -> {
         Path path2 = locator.apply(root, (T)object);
