...
public class BiomeListProvider implements DataProvider {
   ...
   public void run(DataCache cache) {
      ...
      for (Entry<RegistryKey<Biome>, Biome> entry : BuiltinRegistries.BIOME.getEntries()) {
         Path path2 = getPath(path, entry.getKey().getValue());
         ...
         try {
            Optional<JsonElement> optional = function.apply(() -> lv).result();
            if (optional.isPresent()) {
               DataProvider.writeToPath(GSON, cache, optional.get(), path2);
            } else {
