...
public class EntityLootTableGenerator implements Consumer<BiConsumer<Identifier, LootTable.Builder>> {
   ...
   private static LootTable.Builder createForSheep(ItemConvertible arg) {
      return LootTable.builder()
         .pool(LootPool.builder().rolls(ConstantLootTableRange.create(1)).with(ItemEntry.builder(arg)))
   ...
   public void accept(BiConsumer<Identifier, LootTable.Builder> biConsumer) {
      ...
      for (EntityType<?> lv : Registry.ENTITY_TYPE) {
         ...
         if (!ENTITY_TYPES_IN_MISC_GROUP_TO_CHECK.contains(lv) && lv.getSpawnGroup() == SpawnGroup.MISC) {
            if (lv2 != LootTables.EMPTY && this.lootTables.remove(lv2) != null) {
               throw new IllegalStateException(
                  String.format("Weird loottable '%s' for '%s', not a LivingEntity so should not have loot", lv2, Registry.ENTITY_TYPE.getId(lv))
               );
            }
         } else if (lv2 != LootTables.EMPTY && set.add(lv2)) {
            LootTable.Builder lv3 = this.lootTables.remove(lv2);
            if (lv3 == null) {
               throw new IllegalStateException(String.format("Missing loottable '%s' for '%s'", lv2, Registry.ENTITY_TYPE.getId(lv)));
            }

            biConsumer.accept(lv2, lv3);
         }
      }
   ...
   private void register(EntityType<?> arg, LootTable.Builder arg2) {
      this.register(arg.getLootTableId(), arg2);
   }

   private void register(Identifier arg, LootTable.Builder arg2) {
      this.lootTables.put(arg, arg2);
   }
}
