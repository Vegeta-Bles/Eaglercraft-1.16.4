...
public class HusbandryTabAdvancementGenerator implements Consumer<Consumer<Advancement>> {
   ...
   public void accept(Consumer<Advancement> consumer) {
      ...
      Advancement lv5 = this.requireListedFishCaught(Advancement.Task.create())
         .parent(lv)
         .criteriaMerger(CriterionMerger.OR)
         .display(
            Items.FISHING_ROD,
   ...
   private Advancement.Task requireListedFishCaught(Advancement.Task arg) {
      for (Item lv : FISH_ITEMS) {
         arg.criterion(
            Registry.ITEM.getId(lv).getPath(),
            FishingRodHookedCriterion.Conditions.create(ItemPredicate.ANY, EntityPredicate.ANY, ItemPredicate.Builder.create().item(lv).build())
         );
      }
   ...
   private Advancement.Task requireAllCatsTamed(Advancement.Task arg) {
      CatEntity.TEXTURES
         .forEach((integer, arg2) -> arg.criterion(arg2.getPath(), TameAnimalCriterion.Conditions.create(EntityPredicate.Builder.create().type(arg2).build())));
      return arg;
   }
}
