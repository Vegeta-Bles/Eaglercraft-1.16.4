                  ...
                  typedx -> {
                     ...
                     if (bl) {
                        ...
                        if (optional.isPresent()) {
                           ...
                           dynamic = dynamic.set(
                              "ToBeTicked",
                              dynamic.createList(list.stream().map(shortList -> dynamic2.createList(shortList.stream().map(dynamic2::createShort))))
                           );
                        }
                     ...
                     return (Typed)((Pair)type4.readTyped(dynamic3).result().orElseThrow(() -> new IllegalStateException("Could not read the new chunk")))
                        .getFirst();
                  }
               )
         ),
