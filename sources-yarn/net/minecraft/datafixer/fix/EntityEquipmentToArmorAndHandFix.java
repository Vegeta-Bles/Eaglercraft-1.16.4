         ...
         typed -> {
            ...
            if (optional.isPresent()) {
               ...
               IS object = (IS)((Pair)type.read(dynamic.emptyMap())
                  ...
                  .getFirst();
               if (!list.isEmpty()) {
                  List<IS> handItems = Lists.newArrayListWithCapacity(2);
                  handItems.add(list.get(0));
                  handItems.add(object);
                  either = Either.left(handItems);
               }

               if (list.size() > 1) {
                  List<IS> armorItems = Lists.newArrayListWithCapacity(4);
                  for (int index = 0; index < 4; index++) {
                     armorItems.add(object);
                  }

                  for (int i = 1; i < Math.min(list.size(), 5); i++) {
                     armorItems.set(i - 1, list.get(i));
                  }

                  either2 = Either.left(armorItems);
               }
            }
