...
public class ItemLoreToTextFix extends DataFix {
                     ...
                     dynamic -> dynamic.update(
                           ...
                           dynamicx -> dynamicx.update(
                                 ...
                                 dynamicxx -> (Dynamic)DataFixUtils.orElse(
                                       dynamicxx.asStreamOpt().map(ItemLoreToTextFix::fixLoreTags).map(dynamicxx::createList).result(), dynamicxx
                                    )
                              )
                        )
                  )
   ...
   private static <T> Stream<Dynamic<T>> fixLoreTags(Stream<Dynamic<T>> tags) {
      return tags.map(
         dynamic -> (Dynamic<T>)DataFixUtils.orElse(dynamic.asString().map(ItemLoreToTextFix::componentize).map(dynamic::createString).result(), dynamic)
      );
   }
