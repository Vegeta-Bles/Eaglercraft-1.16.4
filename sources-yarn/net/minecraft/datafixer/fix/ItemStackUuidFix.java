   ...
   private Dynamic<?> method_26297(Dynamic<?> dynamic) {
      return dynamic.update(
         ...
         dynamic2 -> dynamic.createList(
               dynamic2.asStream().map((Dynamic<?> dynamicxx) -> updateRegularMostLeast(dynamicxx, "UUID", "UUID").orElse(dynamicxx))
            )
      );
   }

   private Dynamic<?> method_26298(Dynamic<?> dynamic) {
      return dynamic.update("SkullOwner", dynamicx -> updateStringUuid(dynamicx, "Id", "Id").orElse(dynamicx));
   }
}
