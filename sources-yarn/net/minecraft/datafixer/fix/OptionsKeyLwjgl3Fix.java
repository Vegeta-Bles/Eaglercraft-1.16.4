      ...
      return this.fixTypeEverywhereTyped(
         ...
         typed -> typed.update(
               ...
               dynamic -> dynamic.getMapValues().map(map -> {
                        Map<Dynamic<?>, Dynamic<?>> remapped = map.entrySet()
                           .stream()
                           .map(entry -> {
                              Dynamic<?> key = entry.getKey();
                              Dynamic<?> value = entry.getValue();
                              if (key.asString("").startsWith("key_")) {
                                 int i = Integer.parseInt(value.asString(""));
                                 if (i < 0) {
                                    int j = i + 100;
                                    String string = j == 0
                                       ? "key.mouse.left"
                                       : (j == 1 ? "key.mouse.right" : (j == 2 ? "key.mouse.middle" : "key.mouse." + (j + 1)));
                                    return Pair.of(key, value.createString(string));
                                 } else {
                                    String string5 = (String)NUMERICAL_KEY_IDS_TO_KEY_NAMES.getOrDefault(i, "key.unknown");
                                    return Pair.of(key, value.createString(string5));
                                 }
                              }
                        ...
                        Map<Dynamic<?>, Dynamic<?>> castMap = remapped;
                        return dynamic.createMap((Map)castMap);
                     })
                     .result()
                     .orElse(dynamic)
            )
