   ...
   private <T> Dynamic<T> method_23299(Dynamic<T> dynamic) {
      return dynamic.update(
         ...
         dynamicx -> dynamicx.updateMapValues(
               pair -> pair.mapSecond(
                     dynamicxx -> dynamicxx.update("Records", dynamicxxx -> (Dynamic)DataFixUtils.orElse(this.method_23304(dynamicxxx), dynamicxxx))
                  )
            )
      );
   }
            ...
            stream -> dynamic.createList(
                  stream.map(
                     dynamicxx -> dynamicxx.update(
                           "type",
                           dynamicxxx -> (Dynamic)DataFixUtils.orElse(
                                 dynamicxxx.asString().map(this::rename).map(dynamicxxx::createString).result(), dynamicxxx
                              )
                        )
                  )
               )
