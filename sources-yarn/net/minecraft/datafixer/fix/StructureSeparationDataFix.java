...
public class StructureSeparationDataFix extends DataFix {
   ...
   private static <T> Dynamic<T> method_28271(Dynamic<T> dynamic) {
      ...
      Optional<String> optional2 = dynamic.get("legacy_custom_options")
         .asString()
         .result()
         .map(Optional::of)
      ...
      if (optional.equals(Optional.of("customized"))) {
         ...
         switch (bl6) {
                              ...
                              .orElseGet(
                                 () -> dynamic.createList(
                                       Stream.of(
                                          dynamic.createMap(
                                             ImmutableMap.of(
                                                dynamic.createString("height"),
                                                dynamic.createInt(1),
                                                dynamic.createString("block"),
                                                dynamic.createString("minecraft:bedrock")
                                             )
                                          ),
                                          dynamic.createMap(
                                             ImmutableMap.of(
                                                ...
                                                dynamic.createInt(2),
                                                dynamic.createString("block"),
                                                dynamic.createString("minecraft:dirt")
                                             )
                                          ),
                                          dynamic.createMap(
                                             ImmutableMap.of(
                                                ...
                                                dynamic.createInt(1),
                                                dynamic.createString("block"),
                                                dynamic.createString("minecraft:grass_block")
                                             )
                                          )
                                       )
                                    )
            ...
            case "buffet":
               ...
               Dynamic<T> dynamic9 = optionalDynamic2.get("biome_source")
                  .result()
               ...
               if (dynamic9.get("type").asString().result().equals(Optional.of("minecraft:fixed"))) {
                  String string = dynamic9.get("options")
                     .get("biomes")
                     .asStream()
                     .findFirst()
            ...
            default:
               ...
               dynamic2 = method_28268(
                  l, dynamic, dynamic.createString(bl4 ? "minecraft:amplified" : "minecraft:overworld"), method_28272(dynamic, l, bl3, bl5)
               );
         }
      }
      ...
      Builder<T, T> builder = ImmutableMap.<T, T>builder();
            ...
            dynamicOps.createMap(
               ImmutableMap.of(
                  ...
                  method_28268(
                        ...
                        dynamic.createMap(
                           ImmutableMap.of(
                              dynamic.createString("type"), dynamic.createString("minecraft:the_end"), dynamic.createString("seed"), dynamic.createLong(l)
                           )
                        )
                     )
                     .getValue()
   ...
   private static <T> Map<Dynamic<T>, Dynamic<T>> method_28275(DynamicOps<T> dynamicOps, OptionalDynamic<T> optionalDynamic) {
                                       ...
                                       switch (string2) {
                                          ...
                                          case "separation":
                                             if ("oceanmonument".equals(string)) {
                                                StructureSeparationDataFix.Information lv = map.getOrDefault(
                                                ...
                                                map.put("minecraft:monument", new StructureSeparationDataFix.Information(i, lv.separation, lv.salt));
                                             }
      ...
      builder.put(
         ...
         optionalDynamic.createMap(
            map.entrySet()
               .stream()
               .collect(Collectors.toMap(entry -> optionalDynamic.createString(entry.getKey()), entry -> entry.getValue().method_28288(dynamicOps)))
         )
      );
   ...
   private static void method_28281(Map<String, StructureSeparationDataFix.Information> map, String string, String string2, int i) {
      StructureSeparationDataFix.Information lv = map.getOrDefault(string, (StructureSeparationDataFix.Information)STRUCTURE_SPACING.get(string));
      int j = method_28280(string2, lv.spacing, i);
      map.put(string, new StructureSeparationDataFix.Information(j, lv.separation, lv.salt));
   }
   ...
   static final class Information {
      public static final Codec<StructureSeparationDataFix.Information> CODEC = RecordCodecBuilder.create(
         instance -> instance.group(
                  Codec.INT.fieldOf("spacing").forGetter(arg -> arg.spacing),
                  Codec.INT.fieldOf("separation").forGetter(arg -> arg.separation),
                  Codec.INT.fieldOf("salt").forGetter(arg -> arg.salt)
               )
