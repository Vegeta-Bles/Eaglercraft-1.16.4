   ...
   private Dynamic<?> method_26060(Dynamic<?> dynamic) {
      return dynamic.update(
         ...
         dynamicx -> dynamicx.updateMapValues(
               pair -> pair.mapSecond(
                     dynamicxx -> dynamicxx.update("DragonFight", dynamicxxx -> updateRegularMostLeast(dynamicxxx, "DragonUUID", "Dragon").orElse(dynamicxxx))
                  )
            )
      );
   }
               ...
               pair -> pair.mapSecond(
                     dynamicxx -> dynamicxx.update(
                           ...
                           dynamic2 -> dynamicxx.createList(
                                 dynamic2.asStream().map(dynamicxxxx -> createArrayFromCompoundUuid(dynamicxxxx).orElseGet(() -> {
