...
public class EntityType<T extends Entity> {
   ...
   public Entity spawnFromItemStack(
      ServerWorld arg, @Nullable ItemStack stack, @Nullable PlayerEntity player, BlockPos pos, SpawnReason spawnReason, boolean alignPosition, boolean invertY
   ) {
      return this.spawn(
         arg,
   ...
   public T create(
      ...
      if (lv == null) {
         ...
         double d;
         if (alignPosition) {
            lv.updatePosition((double)pos.getX() + 0.5, (double)(pos.getY() + 1), (double)pos.getZ() + 0.5);
            d = getOriginY(arg, pos, invertY, lv.getBoundingBox());
         } else {
            d = 0.0;
         }

         lv.refreshPositionAndAngles(
            (double)pos.getX() + 0.5, (double)pos.getY() + d, (double)pos.getZ() + 0.5, MathHelper.wrapDegrees(arg.random.nextFloat() * 360.0F), 0.0F
         );
         ...
         loadFromEntityTag(arg, player, lv, itemTag);
         return lv;
      }
   }
   ...
   public static Optional<Entity> getEntityFromTag(CompoundTag tag, World world) {
      return Util.ifPresentOrElse(
         fromTag(tag).map(arg2 -> arg2.create(world)), arg2 -> arg2.fromTag(tag), () -> LOGGER.warn("Skipping Entity with id {}", tag.getString("id"))
      );
   }
   ...
   public boolean isInvalidSpawn(BlockState arg) {
      if (this.canSpawnInside.contains(arg.getBlock())) {
         return false;
      } else {
         return this.fireImmune || !arg.isIn(BlockTags.FIRE) && !arg.isOf(Blocks.MAGMA_BLOCK) && !CampfireBlock.isLitCampfire(arg) && !arg.isOf(Blocks.LAVA)
            ? arg.isOf(Blocks.WITHER_ROSE) || arg.isOf(Blocks.SWEET_BERRY_BUSH) || arg.isOf(Blocks.CACTUS)
            : true;
      }
   }
   ...
   public static Entity loadEntityWithPassengers(CompoundTag compoundTag, World world, Function<Entity, Entity> entityProcessor) {
      return loadEntityFromTag(compoundTag, world).map(entityProcessor).map(arg3 -> {
   ...
   public static class Builder<T extends Entity> {
      ...
      public static <T extends Entity> EntityType.Builder<T> create(SpawnGroup spawnGroup) {
         return new EntityType.Builder<>((arg, arg2) -> null, spawnGroup);
      }
      ...
      public EntityType.Builder<T> allowSpawningInside(Block... args) {
         this.canSpawnInside = ImmutableSet.copyOf(args);
         return this;
      }
