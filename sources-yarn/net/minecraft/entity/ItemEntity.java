...
public class ItemEntity extends Entity {
   ...
   private void applyBuoyancy() {
      Vec3d lv = this.getVelocity();
      this.setVelocity(lv.x * 0.99F, lv.y + (double)(lv.y < 0.06F ? 5.0E-4F : 0.0F), lv.z * 0.99F);
   }

   private void method_24348() {
      Vec3d lv = this.getVelocity();
      this.setVelocity(lv.x * 0.95F, lv.y + (double)(lv.y < 0.06F ? 5.0E-4F : 0.0F), lv.z * 0.95F);
   }

   private void tryMerge() {
      if (this.canMerge()) {
         for (ItemEntity lv : this.world
            .getEntitiesByClass(ItemEntity.class, this.getBoundingBox().expand(0.5, 0.0, 0.5), arg -> arg != this && arg.canMerge())) {
            if (lv.canMerge()) {
               this.tryMerge(lv);
               if (this.removed) {
                  break;
   ...
   private void tryMerge(ItemEntity other) {
      ...
      if (Objects.equals(this.getOwner(), other.getOwner()) && canMerge(lv, lv2)) {
   ...
   public Text getName() {
      ...
      return (Text)(lv != null ? lv : new TranslatableText(this.getStack().getTranslationKey()));
   }
