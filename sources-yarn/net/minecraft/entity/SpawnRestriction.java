...
public class SpawnRestriction {
   ...
   private static <T extends MobEntity> void register(
      ...
      SpawnRestriction.Entry lv = RESTRICTIONS.put(type, new SpawnRestriction.Entry(heightmapType, location, predicate));
      if (lv != null) {
   ...
   public static SpawnRestriction.Location getLocation(EntityType<?> type) {
      SpawnRestriction.Entry lv = RESTRICTIONS.get(type);
   ...
   public static <T extends Entity> boolean canSpawn(EntityType<T> type, ServerWorldAccess arg2, SpawnReason spawnReason, BlockPos pos, Random random) {
      SpawnRestriction.Entry lv = RESTRICTIONS.get(type);
      return lv == null || lv.test(type, arg2, spawnReason, pos, random);
   }
   ...
   static class Entry {
      ...
      private <T extends Entity> boolean test(EntityType<T> type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
         return ((SpawnRestriction.SpawnPredicate<T>)this.predicate).test(type, world, spawnReason, pos, random);
      }
   }
   ...
   public interface SpawnPredicate<T extends Entity> {
      boolean test(EntityType<T> type, ServerWorldAccess arg2, SpawnReason spawnReason, BlockPos pos, Random random);
   }
}
