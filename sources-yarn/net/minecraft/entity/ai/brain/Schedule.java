...
public class Schedule {
   ...
   protected List<ScheduleRule> getOtherRules(Activity activity) {
      return this.scheduleRules.entrySet().stream().filter(entry -> entry.getKey() != activity).map(Entry::getValue).collect(Collectors.toList());
   }
   ...
   public Activity getActivityForTime(int time) {
      return this.scheduleRules
         .entrySet()
         .stream()
         .max(Comparator.comparingDouble(entry -> (double)entry.getValue().getPriority(time)))
         .map(Entry::getKey)
