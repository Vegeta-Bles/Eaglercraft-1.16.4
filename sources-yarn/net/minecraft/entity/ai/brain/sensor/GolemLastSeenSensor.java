...
public class GolemLastSeenSensor extends Sensor<LivingEntity> {
   ...
   public static void senseIronGolem(LivingEntity arg) {
      Optional<List<LivingEntity>> optional = arg.getBrain().getOptionalMemory(MemoryModuleType.MOBS);
      if (optional.isPresent()) {
         boolean bl = optional.get().stream().anyMatch(argx -> argx.getType().equals(EntityType.IRON_GOLEM));
         if (bl) {
            method_30233(arg);
         }
      }
