...
public class HoglinSpecificSensor extends Sensor<HoglinEntity> {
   ...
   protected void sense(ServerWorld arg, HoglinEntity arg2) {
      ...
      for (LivingEntity lv2 : lv.getOptionalMemory(MemoryModuleType.VISIBLE_MOBS).orElse(Lists.newArrayList())) {
         ...
         if (lv2 instanceof HoglinEntity && !lv2.isBaby()) {
   ...
   private Optional<BlockPos> findNearestWarpedFungus(ServerWorld world, HoglinEntity hoglin) {
      return BlockPos.findClosest(hoglin.getBlockPos(), 8, 4, arg2 -> world.getBlockState(arg2).isIn(BlockTags.HOGLIN_REPELLENTS));
   }
}
