...
public class NearestBedSensor extends Sensor<MobEntity> {
   ...
   protected void sense(ServerWorld arg, MobEntity arg2) {
      if (arg2.isBaby()) {
         ...
         Stream<BlockPos> stream = lv.getPositions(
            PointOfInterestType.HOME.getCompletionCondition(), predicate, arg2.getBlockPos(), 48, PointOfInterestStorage.OccupationStatus.ANY
         );
         Path lv2 = arg2.getNavigation().findPathToAny(stream, PointOfInterestType.HOME.getSearchDistance());
         if (lv2 != null && lv2.reachesTarget()) {
            BlockPos lv3 = lv2.getTarget();
            Optional<PointOfInterestType> optional = lv.getType(lv3);
            if (optional.isPresent()) {
               arg2.getBrain().remember(MemoryModuleType.NEAREST_BED, lv3);
            }
         } else if (this.tries < 5) {
