...
public class AdmireItemTask<E extends PiglinEntity> extends Task<E> {
   ...
   protected boolean shouldRun(ServerWorld arg, E arg2) {
      ItemEntity lv = arg2.getBrain().getOptionalMemory(MemoryModuleType.NEAREST_VISIBLE_WANTED_ITEM).get();
      return PiglinBrain.isGoldenItem(lv.getStack().getItem());
   }

   protected void run(ServerWorld arg, E arg2, long l) {
      arg2.getBrain().remember(MemoryModuleType.ADMIRING_ITEM, true, (long)this.duration);
   }
}
