...
public class FindEntityTask<E extends LivingEntity, T extends LivingEntity> extends Task<E> {
   ...
   public static <T extends LivingEntity> FindEntityTask<LivingEntity, T> create(
      ...
      return new FindEntityTask<>(entityType, maxDistance, arg -> true, arg -> true, targetModule, speed, completionRange);
   }
   ...
   private boolean method_24582(E arg) {
      List<LivingEntity> list = arg.getBrain().getOptionalMemory(MemoryModuleType.VISIBLE_MOBS).get();
      return list.stream().anyMatch(this::method_24583);
   }

   private boolean method_24583(LivingEntity arg) {
      return this.entityType.equals(arg.getType()) && this.predicate.test((T)arg);
   }

   @Override
   protected void run(ServerWorld world, E entity, long time) {
      Brain<?> lv = entity.getBrain();
      lv.getOptionalMemory(MemoryModuleType.VISIBLE_MOBS)
         .ifPresent(
            list -> list.stream()
                  .filter(arg -> this.entityType.equals(arg.getType()))
                  .map(arg -> (LivingEntity)arg)
                  .filter(arg2 -> arg2.squaredDistanceTo(entity) <= (double)this.maxSquaredDistance)
                  .filter(arg2 -> this.predicate.test((T)arg2))
                  .findFirst()
                  .ifPresent(arg2 -> {
                     lv.remember(this.targetModule, (T)arg2);
                     lv.remember(MemoryModuleType.LOOK_TARGET, new EntityLookTarget(arg2, true));
                     lv.remember(MemoryModuleType.WALK_TARGET, new WalkTarget(new EntityLookTarget(arg2, false), this.speed, this.completionRange));
                  })
         );
   }
}
