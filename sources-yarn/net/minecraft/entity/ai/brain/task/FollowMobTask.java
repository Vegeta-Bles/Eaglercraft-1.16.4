...
public class FollowMobTask extends Task<LivingEntity> {
   ...
   public FollowMobTask(SpawnGroup group, float maxDistance) {
      this(arg2 -> group.equals(arg2.getType().getSpawnGroup()), maxDistance);
   }

   public FollowMobTask(EntityType<?> type, float maxDistance) {
      this(arg2 -> type.equals(arg2.getType()), maxDistance);
   }

   public FollowMobTask(float maxDistance) {
      this(arg -> true, maxDistance);
   }
      ...
      lv.getOptionalMemory(MemoryModuleType.VISIBLE_MOBS)
         .ifPresent(
            list -> list.stream()
                  ...
                  .filter(arg2 -> arg2.squaredDistanceTo(entity) <= (double)this.maxDistanceSquared)
                  .findFirst()
                  .ifPresent(arg2 -> lv.remember(MemoryModuleType.LOOK_TARGET, new EntityLookTarget(arg2, true)))
         );
   }
}
