...
public class GatherItemsVillagerTask extends Task<VillagerEntity> {
   ...
   private static Set<Item> getGatherableItems(VillagerEntity arg, VillagerEntity arg2) {
      ImmutableSet<Item> immutableSet = arg2.getVillagerData().getProfession().getGatherableItems();
      ImmutableSet<Item> immutableSet2 = arg.getVillagerData().getProfession().getGatherableItems();
      return immutableSet.stream().filter(argx -> !immutableSet2.contains(argx)).collect(Collectors.toSet());
   }
