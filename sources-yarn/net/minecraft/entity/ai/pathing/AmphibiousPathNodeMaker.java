...
public class AmphibiousPathNodeMaker extends LandPathNodeMaker {
   ...
   public int getSuccessors(PathNode[] successors, PathNode node) {
      ...
      boolean bl4 = lv3 == null || lv3.type == PathNodeType.OPEN || lv3.penalty != 0.0F;
      if (bl && bl4) {
         PathNode lv8 = this.getPathNode(node.x - 1, node.y, node.z - 1, 1, d);
         if (lv8 != null && !lv8.visited) {
            successors[i++] = lv8;
         }
      }

      if (bl && bl3) {
         PathNode lv9 = this.getPathNode(node.x + 1, node.y, node.z - 1, 1, d);
         if (lv9 != null && !lv9.visited) {
            successors[i++] = lv9;
         }
      }

      if (bl2 && bl4) {
         PathNode lv10 = this.getPathNode(node.x - 1, node.y, node.z + 1, 1, d);
         if (lv10 != null && !lv10.visited) {
            successors[i++] = lv10;
         }
      }

      if (bl2 && bl3) {
         PathNode lv11 = this.getPathNode(node.x + 1, node.y, node.z + 1, 1, d);
         if (lv11 != null && !lv11.visited) {
            successors[i++] = lv11;
         }
      }
   ...
   private PathNode getPathNode(int x, int y, int z, int maxYStep, double prevFeetY) {
      ...
      if (e - prevFeetY > 1.125) {
         ...
         PathNodeType lv3 = this.getNodeType(
         ...
         if (lv3 != PathNodeType.WATER && lv3 != PathNodeType.WALKABLE) {
            ...
            if (lv3 == PathNodeType.OPEN) {
               ...
               if (!this.entity.world.isSpaceEmpty(this.entity, lv4)) {
                  return null;
               }

               PathNodeType lv5 = this.getNodeType(
   ...
   public PathNodeType getDefaultNodeType(BlockView world, int x, int y, int z) {
      ...
      if (lv2 == PathNodeType.WATER) {
         ...
         if (lv2 == PathNodeType.OPEN && y >= 1) {
            ...
            PathNodeType lv6 = getCommonNodeType(world, lv.set(x, y - 1, z));
            if (lv6 != PathNodeType.WALKABLE && lv6 != PathNodeType.OPEN && lv6 != PathNodeType.LAVA) {
               lv2 = PathNodeType.WALKABLE;
            } else {
               lv2 = PathNodeType.OPEN;
            }
