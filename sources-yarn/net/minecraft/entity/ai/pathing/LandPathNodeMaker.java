...
public class LandPathNodeMaker extends PathNodeMaker {
   ...
   public PathNode getStart() {
      ...
      if (!this.entity.canWalkOnFluid(lv2.getFluidState().getFluid())) {
         if (this.canSwim() && this.entity.isTouchingWater()) {
            while (true) {
               if (lv2.getBlock() != Blocks.WATER && lv2.getFluidState() != Fluids.WATER.getStill(false)) {
                  i--;
                  break;
               }
            ...
            while (
               (
                     this.cachedWorld.getBlockState(lv3).isAir()
                        || this.cachedWorld.getBlockState(lv3).canPathfindThrough(this.cachedWorld, lv3, NavigationType.LAND)
                  )
                  && lv3.getY() > 0
            ) {
               lv3 = lv3.down();
      ...
      if (this.entity.getPathfindingPenalty(lv5) < 0.0F) {
         ...
         if (this.method_27139(lv.set(lv6.minX, (double)i, lv6.minZ))
            || this.method_27139(lv.set(lv6.minX, (double)i, lv6.maxZ))
            || this.method_27139(lv.set(lv6.maxX, (double)i, lv6.minZ))
            || this.method_27139(lv.set(lv6.maxX, (double)i, lv6.maxZ))) {
   ...
   private boolean method_29579(PathNode arg, @Nullable PathNode arg2, @Nullable PathNode arg3, @Nullable PathNode arg4) {
      if (arg4 == null || arg3 == null || arg2 == null) {
         return false;
      } else if (arg4.visited) {
         return false;
      } else if (arg3.y > arg.y || arg2.y > arg.y) {
         return false;
      } else if (arg2.type != PathNodeType.WALKABLE_DOOR && arg3.type != PathNodeType.WALKABLE_DOOR && arg4.type != PathNodeType.WALKABLE_DOOR) {
   ...
   private boolean method_29578(PathNode arg) {
      ...
      lv = lv.multiply((double)(1.0F / (float)i));

      for (int j = 1; j <= i; j++) {
         lv2 = lv2.offset(lv);
         if (this.method_29304(lv2)) {
            return false;
         }
      }
   ...
   private PathNode getPathNode(int x, int y, int z, int maxYStep, double prevFeetY, Direction direction, PathNodeType arg2) {
      ...
      double e = getFeetY(this.cachedWorld, lv2.set(x, y, z));
      if (e - prevFeetY > 1.125) {
         ...
         if (lv3 == PathNodeType.WALKABLE) {
            ...
            if ((lv == null || lv.penalty < 0.0F)
               && maxYStep > 0
               && lv3 != PathNodeType.FENCE
               && lv3 != PathNodeType.UNPASSABLE_RAIL
               && lv3 != PathNodeType.TRAPDOOR) {
               lv = this.getPathNode(x, y + 1, z, maxYStep - 1, prevFeetY, direction, arg2);
               if (lv != null && (lv.type == PathNodeType.OPEN || lv.type == PathNodeType.WALKABLE) && this.entity.getWidth() < 1.0F) {
                  ...
                  Box lv4 = new Box(
                     h - g,
                     getFeetY(this.cachedWorld, lv2.set(h, (double)(y + 1), m)) + 0.001,
                     m - g,
                     h + g,
                     (double)this.entity.getHeight() + getFeetY(this.cachedWorld, lv2.set((double)lv.x, (double)lv.y, (double)lv.z)) - 0.002,
                     m + g
                  );
                  if (this.method_29304(lv4)) {
                     lv = null;
                  }
               }
            }
   ...
   private boolean method_29304(Box arg) {
      return (Boolean)this.field_25191.computeIfAbsent(arg, arg2 -> !this.cachedWorld.isSpaceEmpty(this.entity, arg));
   }
   ...
   public PathNodeType getNodeType(
      ...
      if (enumSet.contains(PathNodeType.FENCE)) {
         ...
         return lv == PathNodeType.OPEN && mob.getPathfindingPenalty(lv3) == 0.0F && sizeX <= 1 ? PathNodeType.OPEN : lv3;
      }
   }
   ...
   public static PathNodeType getLandNodeType(BlockView arg, BlockPos.Mutable arg2) {
      ...
      int j = arg2.getY();
      int k = arg2.getZ();
      PathNodeType lv = getCommonNodeType(arg, arg2);
      if (lv == PathNodeType.OPEN && j >= 1) {
         PathNodeType lv2 = getCommonNodeType(arg, arg2.set(i, j - 1, k));
         lv = lv2 != PathNodeType.WALKABLE && lv2 != PathNodeType.OPEN && lv2 != PathNodeType.WATER && lv2 != PathNodeType.LAVA
      ...
      if (lv == PathNodeType.WALKABLE) {
         lv = getNodeTypeFromNeighbors(arg, arg2.set(i, j, k), lv);
      }
         ...
         for (int m = -1; m <= 1; m++) {
            for (int n = -1; n <= 1; n++) {
               if (l != 0 || n != 0) {
                  ...
                  if (arg.getFluidState(arg2).isIn(FluidTags.WATER)) {
   ...
   protected static PathNodeType getCommonNodeType(BlockView arg, BlockPos arg2) {
      ...
      if (lv.isAir()) {
      ...
      } else if (lv.isIn(BlockTags.TRAPDOORS) || lv.isOf(Blocks.LILY_PAD)) {
         return PathNodeType.TRAPDOOR;
      } else if (lv.isOf(Blocks.CACTUS)) {
         return PathNodeType.DAMAGE_CACTUS;
      } else if (lv.isOf(Blocks.SWEET_BERRY_BUSH)) {
         return PathNodeType.DAMAGE_OTHER;
      } else if (lv.isOf(Blocks.HONEY_BLOCK)) {
         ...
         } else if (lv4.isIn(FluidTags.LAVA)) {
            return PathNodeType.LAVA;
         } else if (method_27138(lv)) {
            return PathNodeType.DAMAGE_FIRE;
         } else if (DoorBlock.isWoodenDoor(lv) && !lv.get(DoorBlock.OPEN)) {
            return PathNodeType.DOOR_WOOD_CLOSED;
         } else if (lv2 instanceof DoorBlock && lv3 == Material.METAL && !lv.get(DoorBlock.OPEN)) {
            return PathNodeType.DOOR_IRON_CLOSED;
         } else if (lv2 instanceof DoorBlock && lv.get(DoorBlock.OPEN)) {
            return PathNodeType.DOOR_OPEN;
         } else if (lv2 instanceof AbstractRailBlock) {
            return PathNodeType.RAIL;
         } else if (lv2 instanceof LeavesBlock) {
            return PathNodeType.LEAVES;
         } else if (!lv2.isIn(BlockTags.FENCES) && !lv2.isIn(BlockTags.WALLS) && (!(lv2 instanceof FenceGateBlock) || lv.get(FenceGateBlock.OPEN))) {
            return !lv.canPathfindThrough(arg, arg2, NavigationType.LAND) ? PathNodeType.BLOCKED : PathNodeType.OPEN;
         } else {
            return PathNodeType.FENCE;
         }
      }
