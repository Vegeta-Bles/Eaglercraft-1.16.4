...
public class PathNodeNavigator {
   ...
   public Path findPathToAny(ChunkCache world, MobEntity mob, Set<BlockPos> positions, float followRange, int distance, float rangeMultiplier) {
      ...
      Map<TargetPathNode, BlockPos> map = positions.stream()
         .collect(Collectors.toMap(arg -> this.pathNodeMaker.getNode((double)arg.getX(), (double)arg.getY(), (double)arg.getZ()), Function.identity()));
   ...
   private Path findPathToAny(PathNode startNode, Map<TargetPathNode, BlockPos> positions, float followRange, int distance, float rangeMultiplier) {
      ...
      while (!this.minHeap.isEmpty()) {
         ...
         for (TargetPathNode lv2 : set) {
            if (lv.getManhattanDistance(lv2) <= (float)distance) {
               lv2.markReached();
               set3.add(lv2);
            }
         }
      ...
      Optional<Path> optional = !set3.isEmpty()
         ? set3.stream().map(arg -> this.createPath(arg.getNearestNode(), positions.get(arg), true)).min(Comparator.comparingInt(Path::getLength))
         : set.stream()
            .map(arg -> this.createPath(arg.getNearestNode(), positions.get(arg), false))
