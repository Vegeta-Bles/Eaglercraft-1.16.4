...
public class SwimNavigation extends EntityNavigation {
   ...
   protected void continueFollowingPath() {
      if (this.currentPath != null) {
         ...
         int i = 6;
         Vec3d lv3 = Vec3d.ofBottomCenter(this.currentPath.method_31032());
         if (Math.abs(this.entity.getX() - lv3.x) < (double)g
            && Math.abs(this.entity.getZ() - lv3.z) < (double)g
         ...
         for (int j = Math.min(this.currentPath.getCurrentNodeIndex() + 6, this.currentPath.getLength() - 1); j > this.currentPath.getCurrentNodeIndex(); j--) {
            ...
            if (!(lv3.squaredDistanceTo(lv) > 36.0) && this.canPathDirectlyThrough(lv, lv3, 0, 0, 0)) {
   ...
   protected boolean canPathDirectlyThrough(Vec3d origin, Vec3d target, int sizeX, int sizeY, int sizeZ) {
      ...
      return this.world.raycast(new RaycastContext(origin, lv, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this.entity)).getType()
         == HitResult.Type.MISS;
   }
