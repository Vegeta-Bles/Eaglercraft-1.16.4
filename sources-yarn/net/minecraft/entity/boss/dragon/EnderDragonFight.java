...
public class EnderDragonFight {
   ...
   public EnderDragonFight(ServerWorld world, long l, CompoundTag arg2) {
      this.world = world;
   ...
   private BlockPattern.Result findEndPortal() {
      ...
      for (int l = k; l >= 0; l--) {
         BlockPattern.Result lv5 = this.endPortalPattern
   ...
   private void countAliveCrystals() {
      ...
      for (EndSpikeFeature.Spike lv : EndSpikeFeature.getSpikes(this.world)) {
         this.endCrystalsAlive = this.endCrystalsAlive + this.world.getNonSpectatingEntities(EndCrystalEntity.class, lv.getBoundingBox()).size();
      }
   ...
   private void generateEndGateway(BlockPos arg) {
      ...
      ConfiguredFeatures.END_GATEWAY_DELAYED.generate(this.world, this.world.getChunkManager().getChunkGenerator(), new Random(), arg);
   }
   ...
   public void resetEndCrystals() {
      for (EndSpikeFeature.Spike lv : EndSpikeFeature.getSpikes(this.world)) {
         for (EndCrystalEntity lv2 : this.world.getNonSpectatingEntities(EndCrystalEntity.class, lv.getBoundingBox())) {
