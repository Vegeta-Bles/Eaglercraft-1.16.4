...
public class StatusEffectInstance implements Comparable<StatusEffectInstance> {
   ...
   public boolean equals(Object o) {
      if (this == o) {
         ...
         return this.duration == lv.duration
            && this.amplifier == lv.amplifier
            && this.splash == lv.splash
            && this.ambient == lv.ambient
            && this.type.equals(lv.type);
      }
   }
   ...
   public static StatusEffectInstance fromTag(CompoundTag tag) {
      ...
      return lv == null ? null : fromTag(lv, tag);
   }
   ...
   private static StatusEffectInstance fromTag(StatusEffect type, CompoundTag tag) {
      ...
      if (tag.contains("HiddenEffect", 10)) {
         lv = fromTag(type, tag.getCompound("HiddenEffect"));
      }

      return new StatusEffectInstance(type, j, i < 0 ? 0 : i, bl, bl2, bl3, lv);
   }
   ...
   public int compareTo(StatusEffectInstance arg) {
      ...
      return (this.getDuration() <= 32147 || arg.getDuration() <= 32147) && (!this.isAmbient() || !arg.isAmbient())
         ? ComparisonChain.start()
            ...
            .compare(this.getEffectType().getColor(), arg.getEffectType().getColor())
            .result()
         : ComparisonChain.start().compare(this.isAmbient(), arg.isAmbient()).compare(this.getEffectType().getColor(), arg.getEffectType().getColor()).result();
   }
}
