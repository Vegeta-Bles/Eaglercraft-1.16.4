...
public class EndermanEntity extends HostileEntity implements Angerable {
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      ...
      if (tag.contains("carriedBlockState", 10)) {
         lv = NbtHelper.toBlockState(tag.getCompound("carriedBlockState"));
         if (lv.isAir()) {
            lv = null;
         }
      }
   ...
   static class PickUpBlockGoal extends Goal {
      ...
      public void tick() {
         ...
         BlockHitResult lv7 = lv.raycast(new RaycastContext(lv5, lv6, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, this.enderman));
         boolean bl = lv7.getBlockPos().equals(lv2);
         if (lv4.isIn(BlockTags.ENDERMAN_HOLDABLE) && bl) {
            lv.removeBlock(lv2, false);
            this.enderman.setCarriedBlock(lv3.getBlock().getDefaultState());
         }
      }
   }
   ...
   static class PlaceBlockGoal extends Goal {
      ...
      public void tick() {
         ...
         if (lv6 != null) {
            lv6 = Block.postProcessState(lv6, this.enderman.world, lv2);
            if (this.canPlaceOn(lv, lv2, lv6, lv3, lv5, lv4)) {
      ...
      private boolean canPlaceOn(World arg, BlockPos posAbove, BlockState carriedState, BlockState stateAbove, BlockState state, BlockPos pos) {
         return stateAbove.isAir()
            && !state.isAir()
            && !state.isOf(Blocks.BEDROCK)
            && state.isFullCube(arg, pos)
            && carriedState.canPlaceAt(arg, posAbove)
            && arg.getOtherEntities(this.enderman, Box.method_29968(Vec3d.of(posAbove))).isEmpty();
      }
   }
