...
public class EvokerEntity extends SpellcastingIllagerEntity {
      ...
      protected void castSpell() {
         ...
         if (EvokerEntity.this.squaredDistanceTo(lv) < 9.0) {
            for (int i = 0; i < 5; i++) {
               ...
               this.conjureFangs(
                  EvokerEntity.this.getX() + (double)MathHelper.cos(g) * 1.5, EvokerEntity.this.getZ() + (double)MathHelper.sin(g) * 1.5, d, e, g, 0
               );
            }
            ...
            for (int j = 0; j < 8; j++) {
               ...
               this.conjureFangs(
                  EvokerEntity.this.getX() + (double)MathHelper.cos(h) * 2.5, EvokerEntity.this.getZ() + (double)MathHelper.sin(h) * 2.5, d, e, h, 3
               );
            }
         } else {
            for (int k = 0; k < 16; k++) {
   ...
   public class WololoGoal extends SpellcastingIllagerEntity.CastSpellGoal {
      private final TargetPredicate convertibleSheepPredicate = new TargetPredicate()
         .setBaseMaxDistance(16.0)
         .includeInvulnerable()
         .setPredicate(argx -> ((SheepEntity)argx).getColor() == DyeColor.BLUE);
