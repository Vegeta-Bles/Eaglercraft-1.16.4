...
public class HoglinBrain {
   ...
   private static void addCoreTasks(Brain<HoglinEntity> brain) {
      brain.setTaskList(Activity.CORE, 0, tasks(new LookAroundTask(45, 90), new WanderAroundTask()));
   }
   ...
   private static void avoidEnemy(HoglinEntity hoglin, LivingEntity target) {
      ...
      LivingEntity lv = LookTargetUtil.getCloserEntity(hoglin, lv2.getOptionalMemory(MemoryModuleType.AVOID_TARGET), target);
      lv = LookTargetUtil.getCloserEntity(hoglin, lv2.getOptionalMemory(MemoryModuleType.ATTACK_TARGET), lv);
      avoid(hoglin, lv);
   }
   ...
   private static boolean method_25947(HoglinEntity arg) {
      return arg.isAdult() && !hasMoreHoglinsAround(arg);
   }
   ...
   private static void setAttackTargetIfCloser(HoglinEntity hoglin, LivingEntity targetCandidate) {
      if (!isNearPlayer(hoglin)) {
         ...
         LivingEntity lv = LookTargetUtil.getCloserEntity(hoglin, optional, targetCandidate);
         setAttackTarget(hoglin, lv);
      }
   }

   public static Optional<SoundEvent> method_30083(HoglinEntity arg) {
      return arg.getBrain().getFirstPossibleNonCoreActivity().map(arg2 -> method_30082(arg, arg2));
   }
   ...
   private static SoundEvent method_30082(HoglinEntity arg, Activity arg2) {
      ...
      } else if (arg2 == Activity.FIGHT) {
         return SoundEvents.ENTITY_HOGLIN_ANGRY;
      } else {
         return method_30085(arg) ? SoundEvents.ENTITY_HOGLIN_RETREAT : SoundEvents.ENTITY_HOGLIN_AMBIENT;
      }
   }
   ...
   @SafeVarargs
   private static ImmutableList<Task<? super HoglinEntity>> tasks(Task<? super HoglinEntity>... tasks) {
