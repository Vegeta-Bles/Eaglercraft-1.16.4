...
public abstract class HostileEntity extends PathAwareEntity implements Monster {
   ...
   public static boolean isSpawnDark(ServerWorldAccess arg, BlockPos pos, Random random) {
      if (arg.getLightLevel(LightType.SKY, pos) > random.nextInt(32)) {
         return false;
      } else {
         int i = arg.toServerWorld().isThundering() ? arg.getLightLevel(pos, 10) : arg.getLightLevel(pos);
         return i <= random.nextInt(8);
      }
   }

   public static boolean canSpawnInDark(EntityType<? extends HostileEntity> type, ServerWorldAccess arg2, SpawnReason spawnReason, BlockPos pos, Random random) {
      return arg2.getDifficulty() != Difficulty.PEACEFUL && isSpawnDark(arg2, pos, random) && canMobSpawn(type, arg2, spawnReason, pos, random);
   }
