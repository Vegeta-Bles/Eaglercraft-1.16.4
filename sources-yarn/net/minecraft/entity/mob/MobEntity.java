...
public abstract class MobEntity extends LivingEntity {
   ...
   public float getPathfindingPenalty(PathNodeType nodeType) {
      ...
      if (this.getVehicle() instanceof MobEntity && ((MobEntity)this.getVehicle()).movesIndependently()) {
         lv = (MobEntity)this.getVehicle();
      } else {
         lv = this;
      }

      Float float_ = lv.pathfindingPenalties.get(nodeType);
   ...
   public boolean method_29244(PathNodeType arg) {
      return arg != PathNodeType.DANGER_FIRE && arg != PathNodeType.DANGER_CACTUS && arg != PathNodeType.DANGER_OTHER && arg != PathNodeType.WALKABLE_DOOR;
   }
   ...
   public void writeCustomDataToTag(CompoundTag tag) {
      ...
      for (float f : this.armorDropChances) {
         lv7.add(FloatTag.of(f));
      }

      tag.put("ArmorDropChances", lv7);
      ListTag lv8 = new ListTag();

      for (float g : this.handDropChances) {
         lv8.add(FloatTag.of(g));
      }

      tag.put("HandDropChances", lv8);
   ...
   public void tickMovement() {
      ...
      if (!this.world.isClient && this.canPickUpLoot() && this.isAlive() && !this.dead && this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
         for (ItemEntity lv : this.world.getNonSpectatingEntities(ItemEntity.class, this.getBoundingBox().expand(1.0, 0.0, 1.0))) {
            if (!lv.removed && !lv.getStack().isEmpty() && !lv.cannotPickup() && this.canGather(lv.getStack())) {
               this.loot(lv);
            }
         }
      }
   ...
   public boolean tryEquip(ItemStack equipment) {
      EquipmentSlot lv = getPreferredEquipmentSlot(equipment);
      ...
      if (oldStack.isEmpty()) {
         ...
         if (!(oldStack.getItem() instanceof SwordItem)) {
            ...
            return lv.getAttackDamage() != lv2.getAttackDamage()
               ? lv.getAttackDamage() > lv2.getAttackDamage()
         ...
         if (EnchantmentHelper.hasBindingCurse(oldStack)) {
            ...
            if (lv3.getProtection() != lv4.getProtection()) {
               ...
               return lv3.method_26353() != lv4.method_26353() ? lv3.method_26353() > lv4.method_26353() : this.prefersNewDamageableItem(newStack, oldStack);
            }
         }
      } else {
   ...
   protected void dropEquipment(DamageSource source, int lootingMultiplier, boolean allowDrops) {
      ...
      for (EquipmentSlot lv : EquipmentSlot.values()) {
         ...
         boolean bl2 = f > 1.0F;
         if (!lv2.isEmpty()
            && !EnchantmentHelper.hasVanishingCurse(lv2)
            && (allowDrops || bl2)
   ...
   public static EquipmentSlot getPreferredEquipmentSlot(ItemStack stack) {
      ...
      if (lv != Blocks.CARVED_PUMPKIN.asItem() && (!(lv instanceof BlockItem) || !(((BlockItem)lv).getBlock() instanceof AbstractSkullBlock))) {
         ...
         } else if (lv == Items.ELYTRA) {
            return EquipmentSlot.CHEST;
         } else {
            return lv == Items.SHIELD ? EquipmentSlot.OFFHAND : EquipmentSlot.MAINHAND;
         }
      } else {
   ...
   protected void method_30758(float f, EquipmentSlot arg) {
      ItemStack lv = this.getEquippedStack(arg);
      if (!lv.isEmpty() && this.random.nextFloat() < 0.5F * f) {
         this.equipStack(arg, EnchantmentHelper.enchant(this.random, lv, (int)(5.0F + f * (float)this.random.nextInt(18)), false));
      }
   }
   ...
   public final ActionResult interact(PlayerEntity player, Hand hand) {
      if (!this.isAlive()) {
         ...
         if (lv.isAccepted()) {
            ...
            return lv.isAccepted() ? lv : super.interact(player, hand);
         }
      }
   }

   private ActionResult method_29506(PlayerEntity arg, Hand arg2) {
      ...
      if (lv.getItem() == Items.LEAD && this.canBeLeashedBy(arg)) {
         ...
         if (lv.getItem() instanceof SpawnEggItem) {
            if (this.world instanceof ServerWorld) {
               ...
               Optional<MobEntity> optional = lv3.spawnBaby(
                  arg, this, (EntityType<? extends MobEntity>)this.getType(), (ServerWorld)this.world, this.getPos(), lv
               );
               optional.ifPresent(arg2x -> this.onPlayerSpawnedChild(arg, arg2x));
   ...
   @Nullable
   public <T extends MobEntity> T method_29243(EntityType<T> arg, boolean bl) {
      if (this.removed) {
         return null;
      } else {
         T lv = (T)arg.create(this.world);
   ...
   public boolean equip(int slot, ItemStack item) {
      ...
      if (!item.isEmpty() && !canEquipmentSlotContain(lv, item) && lv != EquipmentSlot.HEAD) {
         return false;
      } else {
   ...
   public static boolean canEquipmentSlotContain(EquipmentSlot slot, ItemStack item) {
      EquipmentSlot lv = getPreferredEquipmentSlot(item);
   ...
   public boolean tryAttack(Entity target) {
      ...
      if (bl) {
         if (g > 0.0F && target instanceof LivingEntity) {
            ((LivingEntity)target)
               .takeKnockback(
                  g * 0.5F, (double)MathHelper.sin(this.yaw * (float) (Math.PI / 180.0)), (double)(-MathHelper.cos(this.yaw * (float) (Math.PI / 180.0)))
               );
   ...
   protected boolean isAffectedByDaylight() {
      if (this.world.isDay() && !this.world.isClient) {
         ...
         BlockPos lv = this.getVehicle() instanceof BoatEntity
