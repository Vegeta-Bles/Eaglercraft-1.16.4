...
public class RavagerEntity extends RaiderEntity {
      ...
      if (this.isAlive()) {
         ...
         if (this.horizontalCollision && this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
            ...
            for (BlockPos lv2 : BlockPos.iterate(
               MathHelper.floor(lv.minX),
               MathHelper.floor(lv.minY),
               MathHelper.floor(lv.minZ),
               MathHelper.floor(lv.maxX),
               MathHelper.floor(lv.maxY),
               MathHelper.floor(lv.maxZ)
            )) {
   ...
   private void roar() {
      if (this.isAlive()) {
         for (Entity lv : this.world.getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(4.0), IS_NOT_RAVAGER)) {
