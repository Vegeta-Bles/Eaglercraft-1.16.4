...
public class ShulkerEntity extends GolemEntity implements Monster {
   ...
   public void tick() {
      ...
      if (this.hasVehicle()) {
         ...
         Direction lv5 = this.getAttachedFace();
         if (!this.canStay(lv, lv5)) {
      ...
      if (lv != null) {
         ...
         this.resetPosition((double)lv.getX() + 0.5, (double)lv.getY(), (double)lv.getZ() + 0.5);
         ...
         this.setBoundingBox(
            new Box(this.getX() - 0.5, this.getY(), this.getZ() - 0.5, this.getX() + 0.5, this.getY() + 1.0, this.getZ() + 0.5)
               .stretch((double)lv7.getOffsetX() * d, (double)lv7.getOffsetY() * d, (double)lv7.getOffsetZ() * d)
         );
         double h = d - e;
         if (h > 0.0) {
            ...
            if (!list.isEmpty()) {
               for (Entity lv8 : list) {
                  if (!(lv8 instanceof ShulkerEntity) && !lv8.noClip) {
   ...
   protected boolean tryTeleport() {
      if (!this.isAiDisabled() && this.isAlive()) {
         ...
         for (int i = 0; i < 5; i++) {
            ...
            if (lv2.getY() > 0 && this.world.isAir(lv2) && this.world.getWorldBorder().contains(lv2) && this.world.isSpaceEmpty(this, new Box(lv2))) {
   ...
   class SearchForPlayerGoal extends FollowTargetGoal<PlayerEntity> {
      ...
      protected Box getSearchBox(double distance) {
         ...
         if (lv.getAxis() == Direction.Axis.X) {
            ...
            return lv.getAxis() == Direction.Axis.Z
   ...
   class ShootBulletGoal extends Goal {
      ...
      public boolean canStart() {
         ...
         return lv != null && lv.isAlive() ? ShulkerEntity.this.world.getDifficulty() != Difficulty.PEACEFUL : false;
      }
            ...
            if (d < 400.0) {
               if (this.counter <= 0) {
                  ...
                  ShulkerEntity.this.world
                     .spawnEntity(new ShulkerBulletEntity(ShulkerEntity.this.world, ShulkerEntity.this, lv, ShulkerEntity.this.getAttachedFace().getAxis()));
