...
public class ZombifiedPiglinEntity extends ZombieEntity implements Angerable {
   ...
   private void method_29942() {
      ...
      this.world
         .getEntitiesIncludingUngeneratedChunks(ZombifiedPiglinEntity.class, lv)
         .stream()
         .filter(arg -> arg != this)
         .filter(arg -> arg.getTarget() == null)
         .filter(arg -> !arg.isTeammate(this.getTarget()))
         .forEach(arg -> arg.setTarget(this.getTarget()));
   }
