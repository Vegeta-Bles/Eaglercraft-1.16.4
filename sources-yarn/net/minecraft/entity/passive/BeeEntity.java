...
public class BeeEntity extends AnimalEntity implements Angerable, Flutterer {
   ...
   private void startMovingTo(BlockPos pos) {
      ...
      Vec3d lv3 = TargetFinder.findGroundTargetTowards(this, k, l, i, lv, (float) (Math.PI / 10));
      if (lv3 != null) {
   ...
   private boolean doesHiveHaveSpace(BlockPos pos) {
      ...
      return lv instanceof BeehiveBlockEntity ? !((BeehiveBlockEntity)lv).isFullOfBees() : false;
   }
   ...
   class BeeWanderAroundGoal extends Goal {
      ...
      private Vec3d getRandomLocation() {
         ...
         int i = 8;
         Vec3d lv4 = TargetFinder.findAirTarget(BeeEntity.this, 8, 7, lv2, (float) (Math.PI / 2), 2, 1);
         return lv4 != null ? lv4 : TargetFinder.findGroundTarget(BeeEntity.this, 8, 4, -2, lv2, (float) (Math.PI / 2));
      }
   }
   ...
   class FindHiveGoal extends BeeEntity.NotAngryGoal {
      ...
      public void start() {
         ...
         if (!list.isEmpty()) {
            for (BlockPos lv : list) {
               if (!BeeEntity.this.moveToHiveGoal.isPossibleHive(lv)) {
      ...
      private List<BlockPos> getNearbyFreeHives() {
         ...
         Stream<PointOfInterest> stream = lv2.getInCircle(
            arg -> arg == PointOfInterestType.BEEHIVE || arg == PointOfInterestType.BEE_NEST, lv, 20, PointOfInterestStorage.OccupationStatus.ANY
         );
         return stream.map(PointOfInterest::getPos)
            .filter(arg2 -> BeeEntity.this.doesHiveHaveSpace(arg2))
            .sorted(Comparator.comparingDouble(arg2 -> arg2.getSquaredDistance(lv)))
         ...
         if (BeeEntity.this.random.nextInt(30) == 0) {
            for (int i = 1; i <= 2; i++) {
               ...
               if (lv3.isIn(BlockTags.BEE_GROWABLES)) {
                  ...
                  } else if (lv3 == Blocks.SWEET_BERRY_BUSH) {
                     int k = lv2.get(SweetBerryBushBlock.AGE);
                     if (k < 3) {
                        bl = true;
                  ...
                  if (bl) {
                     ...
                     BeeEntity.this.world.setBlockState(lv, lv2.with(lv4, Integer.valueOf(lv2.get(lv4) + 1)));
   ...
   class PollinateGoal extends BeeEntity.NotAngryGoal {
      private final Predicate<BlockState> flowerPredicate = argx -> {
         if (argx.isIn(BlockTags.TALL_FLOWERS)) {
            return argx.isOf(Blocks.SUNFLOWER) ? argx.get(TallPlantBlock.HALF) == DoubleBlockHalf.UPPER : true;
         } else {
