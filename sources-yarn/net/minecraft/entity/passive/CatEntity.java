...
public class CatEntity extends TameableEntity {
   ...
   public EntityData initialize(
      ...
      World lv = world.toServerWorld();
      if (lv instanceof ServerWorld
         && ((ServerWorld)lv).getStructureAccessor().getStructureAt(this.getBlockPos(), true, StructureFeature.SWAMP_HUT).hasChildren()) {
         this.setCatType(10);
         this.setPersistent();
      }
   ...
   static class SleepWithOwnerGoal extends Goal {
      ...
      private boolean method_16098() {
         for (CatEntity lv : this.cat.world.getNonSpectatingEntities(CatEntity.class, new Box(this.bedPos).expand(2.0))) {
            if (lv != this.cat && (lv.isSleepingWithOwner() || lv.isHeadDown())) {
               return true;
            }
         }
      ...
      private void dropMorningGifts() {
         ...
         this.cat
            .teleport(
               (double)(lv.getX() + random.nextInt(11) - 5), (double)(lv.getY() + random.nextInt(5) - 2), (double)(lv.getZ() + random.nextInt(11) - 5), false
            );
         lv.set(this.cat.getBlockPos());
         LootTable lv2 = this.cat.world.getServer().getLootManager().getTable(LootTables.CAT_MORNING_GIFT_GAMEPLAY);
         LootContext.Builder lv3 = new LootContext.Builder((ServerWorld)this.cat.world)
            .parameter(LootContextParameters.ORIGIN, this.cat.getPos())
            .parameter(LootContextParameters.THIS_ENTITY, this.cat)
            .random(random);

         for (ItemStack lv4 : lv2.generateLoot(lv3.build(LootContextTypes.GIFT))) {
            this.cat
               .world
               .spawnEntity(
                  new ItemEntity(
                     this.cat.world,
                     (double)lv.getX() - (double)MathHelper.sin(this.cat.bodyYaw * (float) (Math.PI / 180.0)),
                     (double)lv.getY(),
                     (double)lv.getZ() + (double)MathHelper.cos(this.cat.bodyYaw * (float) (Math.PI / 180.0)),
                     lv4
                  )
               );
         }
