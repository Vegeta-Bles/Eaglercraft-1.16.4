...
public class FoxEntity extends AnimalEntity {
   ...
   private static final Predicate<Entity> NOTICEABLE_PLAYER_FILTER = arg -> !arg.isSneaky() && EntityPredicates.EXCEPT_CREATIVE_OR_SPECTATOR.test(arg);
   ...
   protected void initGoals() {
      this.followChickenAndRabbitGoal = new FollowTargetGoal<>(
         this, AnimalEntity.class, 10, false, false, arg -> arg instanceof ChickenEntity || arg instanceof RabbitEntity
      );
      ...
      this.followFishGoal = new FollowTargetGoal<>(this, FishEntity.class, 20, false, false, arg -> arg instanceof SchoolingFishEntity);
      ...
      this.goalSelector
         .add(
            4,
            new FleeEntityGoal<>(
               this, PlayerEntity.class, 16.0F, 1.6, 1.4, arg -> NOTICEABLE_PLAYER_FILTER.test(arg) && !this.canTrust(arg.getUuid()) && !this.isAggressive()
            )
         );
      this.goalSelector.add(4, new FleeEntityGoal<>(this, WolfEntity.class, 8.0F, 1.6, 1.4, arg -> !((WolfEntity)arg).isTamed() && !this.isAggressive()));
      this.goalSelector.add(4, new FleeEntityGoal<>(this, PolarBearEntity.class, 8.0F, 1.6, 1.4, arg -> !this.isAggressive()));
      ...
      this.targetSelector
         .add(
            3,
            new FoxEntity.DefendFriendGoal(LivingEntity.class, false, false, arg -> JUST_ATTACKED_SOMETHING_FILTER.test(arg) && !this.canTrust(arg.getUuid()))
         );
   }
   ...
   protected void initEquipment(LocalDifficulty difficulty) {
      if (this.random.nextFloat() < 0.2F) {
         ...
         if (f < 0.05F) {
            ...
            lv = this.random.nextBoolean() ? new ItemStack(Items.RABBIT_FOOT) : new ItemStack(Items.RABBIT_HIDE);
         } else if (f < 0.6F) {
   ...
   public void handleStatus(byte status) {
      if (status == 45) {
         ...
         if (!lv.isEmpty()) {
            for (int i = 0; i < 8; i++) {
               Vec3d lv2 = new Vec3d(((double)this.random.nextFloat() - 0.5) * 0.1, Math.random() * 0.1 + 0.1, 0.0)
                  .rotateX(-this.pitch * (float) (Math.PI / 180.0))
                  .rotateY(-this.yaw * (float) (Math.PI / 180.0));
               this.world
                  .addParticle(
                     new ItemStackParticleEffect(ParticleTypes.ITEM, lv),
   ...
   public EntityData initialize(
      ...
      FoxEntity.Type lv = FoxEntity.Type.fromBiome(optional);
      boolean bl = false;
      if (entityData instanceof FoxEntity.FoxData) {
         lv = ((FoxEntity.FoxData)entityData).type;
         if (((FoxEntity.FoxData)entityData).getSpawnedCount() >= 2) {
            bl = true;
         }
      } else {
         entityData = new FoxEntity.FoxData(lv);
      }
   ...
   public boolean canEquip(ItemStack stack) {
      ...
      return !this.getEquippedStack(lv).isEmpty() ? false : lv == EquipmentSlot.MAINHAND && super.canEquip(stack);
   }
      ...
      public boolean canStart() {
         if (FoxEntity.this.isSleeping() || this.mob.getTarget() != null) {
            ...
            return FoxEntity.this.world.isDay()
               && FoxEntity.this.world.isSkyVisible(lv)
               && !((ServerWorld)FoxEntity.this.world).isNearOccupiedPointOfInterest(lv)
      ...
      protected void eatSweetBerry() {
         if (FoxEntity.this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
            ...
            if (lv.isOf(Blocks.SWEET_BERRY_BUSH)) {
               ...
               FoxEntity.this.world.setBlockState(this.targetPos, lv.with(SweetBerryBushBlock.AGE, Integer.valueOf(1)), 2);
            }
         }
      }
   ...
   public class JumpChasingGoal extends DiveJumpingGoal {
      ...
      public void tick() {
         ...
         if (!FoxEntity.this.isWalking()) {
            ...
            if (lv2.y * lv2.y < 0.03F && FoxEntity.this.pitch != 0.0F) {
               ...
               double e = Math.signum(-lv2.y) * Math.acos(d / lv2.length()) * 180.0F / (float)Math.PI;
   ...
   class MoveToHuntGoal extends Goal {
      ...
      public boolean canStart() {
         if (FoxEntity.this.isSleeping()) {
            ...
            return lv != null
               && lv.isAlive()
               && FoxEntity.CHICKEN_AND_RABBIT_FILTER.test(lv)
               && FoxEntity.this.squaredDistanceTo(lv) > 36.0
               && !FoxEntity.this.isInSneakingPose()
      ...
      public void stop() {
         ...
         if (lv != null && FoxEntity.canJumpChase(FoxEntity.this, lv)) {
            ...
            FoxEntity.this.getNavigation().stop();
            FoxEntity.this.getLookControl().lookAt(lv, (float)FoxEntity.this.getBodyYawSpeed(), (float)FoxEntity.this.getLookPitchSpeed());
         } else {
            FoxEntity.this.setRollingHead(false);
            FoxEntity.this.setCrouching(false);
         }
   ...
   public static enum Type {
      ...
      private static final Map<String, FoxEntity.Type> NAME_TYPE_MAP = Arrays.stream(values())
         .collect(Collectors.toMap(FoxEntity.Type::getKey, arg -> (FoxEntity.Type)arg));
      private final int id;
      private final String key;
      private final List<RegistryKey<Biome>> biomes;

      private Type(int id, String key, RegistryKey<Biome>... args) {
         this.id = id;
         this.key = key;
   ...
   public class WorriableEntityFilter implements Predicate<LivingEntity> {
      ...
      public boolean test(LivingEntity arg) {
         if (arg instanceof FoxEntity) {
            ...
            return true;
         } else if (arg instanceof TameableEntity) {
            return !((TameableEntity)arg).isTamed();
         } else if (!(arg instanceof PlayerEntity) || !arg.isSpectator() && !((PlayerEntity)arg).isCreative()) {
            return FoxEntity.this.canTrust(arg.getUuid()) ? false : !arg.isSleeping() && !arg.isSneaky();
         } else {
            return false;
         }
      }
