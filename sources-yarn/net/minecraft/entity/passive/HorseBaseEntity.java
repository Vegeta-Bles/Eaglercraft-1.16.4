...
public abstract class HorseBaseEntity extends AnimalEntity implements InventoryChangedListener, JumpingMount, Saddleable {
      ...
      if (i <= 0) {
         ...
         if (this.hasPassengers()) {
            for (Entity lv : this.getPassengersDeep()) {
               lv.damage(DamageSource.FALL, (float)i);
            }
         }
      ...
      if (this.isAlive()) {
         if (this.hasPassengers() && this.canBeControlledByRider() && this.isSaddled()) {
            ...
            if (this.jumpStrength > 0.0F && !this.isInAir() && this.onGround) {
               ...
               this.setVelocity(lv2.x, e, lv2.z);
               this.setInAir(true);
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      ...
      if (tag.contains("SaddleItem", 10)) {
         ItemStack lv = ItemStack.fromTag(tag.getCompound("SaddleItem"));
         if (lv.getItem() == Items.SADDLE) {
   ...
   public boolean equip(int slot, ItemStack item) {
      ...
      if (j >= 0 && j < 2 && j < this.items.size()) {
         if (j == 0 && item.getItem() != Items.SADDLE) {
            return false;
         } else if (j != 1 || this.hasArmorSlot() && this.isHorseArmor(item)) {
   ...
   private Vec3d method_27930(Vec3d arg, LivingEntity arg2) {
      ...
      UnmodifiableIterator var10 = arg2.getPoses().iterator();
      ...
      while (var10.hasNext()) {
         ...
         do {
            ...
            if (Dismounting.canDismountInBlock(h)) {
               Box lv3 = arg2.getBoundingBox(lv2);
               Vec3d lv4 = new Vec3d(d, (double)lv.getY() + h, f);
               if (Dismounting.canPlaceEntityAt(this.world, arg2, lv3.offset(lv4))) {
                  arg2.setPose(lv2);
                  return lv4;
               }
            }
   ...
   public Vec3d updatePassengerForDismount(LivingEntity passenger) {
      Vec3d lv = getPassengerDismountOffset(
      ...
      if (lv2 != null) {
         ...
         Vec3d lv3 = getPassengerDismountOffset(
