...
public class HorseEntity extends HorseBaseEntity {
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      ...
      if (tag.contains("ArmorItem", 10)) {
         ItemStack lv = ItemStack.fromTag(tag.getCompound("ArmorItem"));
            ...
            if (i != 0) {
               this.getAttributeInstance(EntityAttributes.GENERIC_ARMOR)
                  .addTemporaryModifier(
                     new EntityAttributeModifier(HORSE_ARMOR_BONUS_ID, "Horse armor bonus", (double)i, EntityAttributeModifier.Operation.ADDITION)
                  );
            }
         }
      }
   ...
   public ActionResult interactMob(PlayerEntity player, Hand hand) {
      ...
      if (!lv.isEmpty()) {
         ...
         boolean bl = !this.isBaby() && !this.isSaddled() && lv.getItem() == Items.SADDLE;
         if (this.isHorseArmor(lv) || bl) {
            this.openInventory(player);
            return ActionResult.success(this.world.isClient);
         }
      }
   ...
   public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
      ...
      if (entity instanceof DonkeyEntity) {
         lv = EntityType.MULE.create(world);
      } else {
         HorseEntity lv2 = (HorseEntity)entity;
         lv = EntityType.HORSE.create(world);
         ...
         if (i < 4) {
            ...
            lv4 = Util.getRandom(HorseColor.values(), this.random);
         }
         ...
         if (j < 2) {
            ...
            lv7 = Util.getRandom(HorseMarking.values(), this.random);
         }

         ((HorseEntity)lv).setVariant(lv4, lv7);
      }
   ...
   public EntityData initialize(
      ...
      if (entityData instanceof HorseEntity.HorseData) {
         lv = ((HorseEntity.HorseData)entityData).color;
      } else {
         lv = Util.getRandom(HorseColor.values(), this.random);
         entityData = new HorseEntity.HorseData(lv);
      }

      this.setVariant(lv, Util.getRandom(HorseMarking.values(), this.random));
