...
public class MooshroomEntity extends CowEntity implements Shearable {
   ...
   public ActionResult interactMob(PlayerEntity player, Hand hand) {
      ...
      if (lv.getItem() == Items.BOWL && !this.isBaby()) {
         ...
         ItemStack lv4 = ItemUsage.method_30270(lv, player, lv2, false);
         player.setStackInHand(hand, lv4);
         SoundEvent lv5;
         if (bl) {
            lv5 = SoundEvents.ENTITY_MOOSHROOM_SUSPICIOUS_MILK;
         } else {
      ...
      } else if (lv.getItem() == Items.SHEARS && this.isShearable()) {
         ...
         return ActionResult.success(this.world.isClient);
      } else if (this.getMooshroomType() == MooshroomEntity.Type.BROWN && lv.getItem().isIn(ItemTags.SMALL_FLOWERS)) {
         if (this.stewEffect != null) {
            for (int i = 0; i < 2; i++) {
               this.world
                  .addParticle(
   ...
   private Optional<Pair<StatusEffect, Integer>> getStewEffectFrom(ItemStack flower) {
      ...
      if (lv instanceof BlockItem) {
         ...
         if (lv2 instanceof FlowerBlock) {
            FlowerBlock lv3 = (FlowerBlock)lv2;
            return Optional.of(Pair.of(lv3.getEffectInStew(), lv3.getEffectInStewDuration()));
         }
      }
   ...
   private MooshroomEntity.Type chooseBabyType(MooshroomEntity mooshroom) {
      MooshroomEntity.Type lv = this.getMooshroomType();
      MooshroomEntity.Type lv2 = mooshroom.getMooshroomType();
      MooshroomEntity.Type lv3;
   ...
   public static enum Type {
      ...
      private static MooshroomEntity.Type fromName(String name) {
         for (MooshroomEntity.Type lv : values()) {
