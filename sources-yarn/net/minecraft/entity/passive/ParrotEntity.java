...
public class ParrotEntity extends TameableShoulderEntity implements Flutterer {
   ...
   public static boolean imitateNearbyMob(World world, Entity parrot) {
      if (parrot.isAlive() && !parrot.isSilent() && world.random.nextInt(2) == 0) {
         ...
         if (!list.isEmpty()) {
            MobEntity lv = list.get(world.random.nextInt(list.size()));
            if (!lv.isSilent()) {
               SoundEvent lv2 = getSound(lv.getType());
               world.playSound(null, parrot.getX(), parrot.getY(), parrot.getZ(), lv2, parrot.getSoundCategory(), 0.7F, getSoundPitch(world.random));
               return true;
            }
         }
   ...
   public static boolean canSpawn(EntityType<ParrotEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
      ...
      return (lv.isIn(BlockTags.LEAVES) || lv.isOf(Blocks.GRASS_BLOCK) || lv.isIn(BlockTags.LOGS) || lv.isOf(Blocks.AIR))
