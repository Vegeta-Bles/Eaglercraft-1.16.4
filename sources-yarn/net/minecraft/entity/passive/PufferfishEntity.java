...
public class PufferfishEntity extends FishEntity {
   ...
   private static final Predicate<LivingEntity> BLOW_UP_FILTER = arg -> {
      if (arg == null) {
         ...
         return !(arg instanceof PlayerEntity) || !arg.isSpectator() && !((PlayerEntity)arg).isCreative() ? arg.getGroup() != EntityGroup.AQUATIC : false;
      }
   };
   ...
   public void tickMovement() {
      ...
      if (this.isAlive() && this.getPuffState() > 0) {
         for (MobEntity lv : this.world.getEntitiesByClass(MobEntity.class, this.getBoundingBox().expand(0.3), BLOW_UP_FILTER)) {
            if (lv.isAlive()) {
               this.sting(lv);
            }
         }
