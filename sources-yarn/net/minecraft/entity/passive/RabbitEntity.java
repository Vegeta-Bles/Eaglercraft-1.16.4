...
public class RabbitEntity extends AnimalEntity {
   ...
   public void mobTick() {
      ...
      if (this.onGround) {
         ...
         RabbitEntity.RabbitJumpControl lv2 = (RabbitEntity.RabbitJumpControl)this.jumpControl;
         if (!lv2.isActive()) {
   ...
   public RabbitEntity createChild(ServerWorld arg, PassiveEntity arg2) {
      ...
      if (this.random.nextInt(20) != 0) {
         if (arg2 instanceof RabbitEntity && this.random.nextBoolean()) {
            i = ((RabbitEntity)arg2).getRabbitType();
         } else {
   ...
   private int chooseType(WorldAccess world) {
      Biome lv = world.getBiome(this.getBlockPos());
      int i = this.random.nextInt(100);
      if (lv.getPrecipitation() == Biome.Precipitation.SNOW) {
         return i < 80 ? 1 : 3;
      } else if (lv.getCategory() == Biome.Category.DESERT) {
         return 4;
      } else {
   ...
   public static boolean canSpawn(EntityType<RabbitEntity> entity, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
      ...
      return (lv.isOf(Blocks.GRASS_BLOCK) || lv.isOf(Blocks.SNOW) || lv.isOf(Blocks.SAND)) && world.getBaseLightLevel(pos, 0) > 8;
   }
   ...
   static class EatCarrotCropGoal extends MoveToTargetPosGoal {
         ...
         if (this.hasReached()) {
            ...
            if (this.hasTarget && lv4 instanceof CarrotsBlock) {
               ...
               if (integer == 0) {
                  ...
                  lv.setBlockState(lv2, lv3.with(CarrotsBlock.AGE, Integer.valueOf(integer - 1)), 2);
      ...
      protected boolean isTargetPos(WorldView world, BlockPos pos) {
         Block lv = world.getBlockState(pos).getBlock();
         if (lv == Blocks.FARMLAND && this.wantsCarrots && !this.hasTarget) {
            pos = pos.up();
