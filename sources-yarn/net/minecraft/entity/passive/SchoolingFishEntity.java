...
public abstract class SchoolingFishEntity extends FishEntity {
   ...
   public void tick() {
      ...
      if (this.hasOtherFishInGroup() && this.world.random.nextInt(200) == 1) {
         List<FishEntity> list = this.world.getNonSpectatingEntities((Class<? extends FishEntity>)this.getClass(), this.getBoundingBox().expand(8.0, 8.0, 8.0));
         if (list.size() <= 1) {
            this.groupSize = 1;
         }
      }
   ...
   public void pullInOtherFish(Stream<SchoolingFishEntity> fish) {
      fish.limit((long)(this.getMaxGroupSize() - this.groupSize)).filter(arg -> arg != this).forEach(arg -> arg.joinGroupOf(this));
   }
