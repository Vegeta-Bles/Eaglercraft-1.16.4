...
public class SheepEntity extends AnimalEntity implements Shearable {
   ...
   private static final Map<DyeColor, float[]> COLORS = Util.make(Maps.newEnumMap(DyeColor.class), map -> {
      for (DyeColor dyeColor : DyeColor.values()) {
         map.put(dyeColor, getDyedColor(dyeColor));
      }
   });
   ...
   private DyeColor getChildColor(AnimalEntity firstParent, AnimalEntity secondParent) {
      ...
      return this.world
         .getRecipeManager()
         .getFirstMatch(RecipeType.CRAFTING, lv3, this.world)
         .map(arg2 -> arg2.craft(lv3))
         .map(ItemStack::getItem)
         .filter(DyeItem.class::isInstance)
         .map(DyeItem.class::cast)
         .map(DyeItem::getColor)
         .orElseGet(() -> this.world.random.nextBoolean() ? lv : lv2);
   }
