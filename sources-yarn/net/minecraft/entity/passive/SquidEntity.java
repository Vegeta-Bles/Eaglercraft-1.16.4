...
public class SquidEntity extends WaterCreatureEntity {
   ...
   public void tickMovement() {
      ...
      if (this.isInsideWaterOrBubbleColumn()) {
         ...
         this.bodyYaw = this.bodyYaw + (-((float)MathHelper.atan2(lv.x, lv.z)) * (180.0F / (float)Math.PI) - this.bodyYaw) * 0.1F;
         this.yaw = this.bodyYaw;
         this.rollAngle = (float)((double)this.rollAngle + Math.PI * (double)this.turningSpeed * 1.5);
         this.tiltAngle = this.tiltAngle + (-((float)MathHelper.atan2((double)g, lv.y)) * (180.0F / (float)Math.PI) - this.tiltAngle) * 0.1F;
      } else {
   ...
   private Vec3d applyBodyRotations(Vec3d shootVector) {
      Vec3d lv = shootVector.rotateX(this.prevTiltAngle * (float) (Math.PI / 180.0));
      return lv.rotateY(-this.prevBodyYaw * (float) (Math.PI / 180.0));
   }
   ...
   class EscapeAttackerGoal extends Goal {
      ...
      public boolean canStart() {
         ...
         return SquidEntity.this.isTouchingWater() && lv != null ? SquidEntity.this.squaredDistanceTo(lv) < 100.0 : false;
      }
      ...
      public void tick() {
         ...
         if (lv != null) {
            ...
            BlockState lv3 = SquidEntity.this.world
