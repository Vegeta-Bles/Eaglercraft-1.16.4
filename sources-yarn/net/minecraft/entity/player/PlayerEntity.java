...
public abstract class PlayerEntity extends LivingEntity {
   ...
   protected void updateSize() {
      if (this.wouldPoseNotCollide(EntityPose.SWIMMING)) {
         ...
         if (this.isSpectator() || this.hasVehicle() || this.wouldPoseNotCollide(lv)) {
            ...
            lv7 = EntityPose.CROUCHING;
         } else {
            lv7 = EntityPose.SWIMMING;
         }
   ...
   public ItemEntity dropItem(ItemStack stack, boolean throwRandomly, boolean retainOwnership) {
      if (stack.isEmpty()) {
         ...
         if (throwRandomly) {
            ...
            lv.setVelocity((double)(-MathHelper.sin(g) * f), 0.2F, (double)(MathHelper.cos(g) * f));
         } else {
            float h = 0.3F;
            ...
            if (f > 0.0F || g > 0.0F) {
               ...
               if (bl6) {
                  ...
                  if (bl4) {
                     ...
                     for (LivingEntity lv3 : this.world.getNonSpectatingEntities(LivingEntity.class, target.getBoundingBox().expand(1.0, 0.25, 1.0))) {
                        if (lv3 != this
                           && lv3 != target
                           && !this.isTeammate(lv3)
                           && (!(lv3 instanceof ArmorStandEntity) || !((ArmorStandEntity)lv3).isMarker())
                           && this.squaredDistanceTo(lv3) < 9.0) {
                           lv3.takeKnockback(
                              0.4F,
                              (double)MathHelper.sin(this.yaw * (float) (Math.PI / 180.0)),
                              (double)(-MathHelper.cos(this.yaw * (float) (Math.PI / 180.0)))
   ...
   public static Optional<Vec3d> findRespawnPosition(ServerWorld world, BlockPos pos, float f, boolean bl, boolean bl2) {
      ...
      if (lv2 instanceof RespawnAnchorBlock && lv.get(RespawnAnchorBlock.CHARGES) > 0 && RespawnAnchorBlock.isNether(world)) {
         ...
         if (!bl2 && optional.isPresent()) {
            world.setBlockState(pos, lv.with(RespawnAnchorBlock.CHARGES, Integer.valueOf(lv.get(RespawnAnchorBlock.CHARGES) - 1)), 3);
         }

         return optional;
      } else if (lv2 instanceof BedBlock && BedBlock.isOverworld(world)) {
   ...
   private void dropShoulderEntity(CompoundTag entityNbt) {
      if (!this.world.isClient && !entityNbt.isEmpty()) {
         EntityType.getEntityFromTag(entityNbt, this.world).ifPresent(arg -> {
            ...
            arg.updatePosition(this.getX(), this.getY() + 0.7F, this.getZ());
            ((ServerWorld)this.world).tryLoadEntity(arg);
         });
      }
   }
         ...
         if (slot == 98) {
            ...
            if (!item.isEmpty()) {
               if (!(item.getItem() instanceof ArmorItem) && !(item.getItem() instanceof ElytraItem)) {
                  if (lv != EquipmentSlot.HEAD) {
                     return false;
                  }
               } else if (MobEntity.getPreferredEquipmentSlot(item) != lv) {
                  return false;
               }
            }

            this.inventory.setStack(lv.getEntitySlotId() + this.inventory.main.size(), item);
            return true;
         }
      }
   }
   ...
   public Vec3d method_30951(float f) {
      ...
      if (this.isFallFlying() || this.isUsingRiptide()) {
         Vec3d lv = this.getRotationVec(f);
         Vec3d lv2 = this.getVelocity();
         double e = Entity.squaredHorizontalLength(lv2);
         double i = Entity.squaredHorizontalLength(lv);
         float l;
         if (e > 0.0 && i > 0.0) {
