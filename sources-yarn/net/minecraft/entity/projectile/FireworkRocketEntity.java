...
public class FireworkRocketEntity extends ProjectileEntity implements FlyingItemEntity {
   ...
   public void tick() {
      ...
      if (this.wasShotByEntity()) {
         ...
         if (this.shooter != null) {
            if (this.shooter.isFallFlying()) {
               ...
               this.shooter
                  .setVelocity(
                     lv2.add(lv.x * 0.1 + (lv.x * 1.5 - lv2.x) * 0.5, lv.y * 0.1 + (lv.y * 1.5 - lv2.y) * 0.5, lv.z * 0.1 + (lv.z * 1.5 - lv2.z) * 0.5)
                  );
            }
      ...
      HitResult lv4 = ProjectileUtil.getCollision(this, this::method_26958);
      if (!this.noClip) {
   ...
   private void explode() {
      ...
      if (f > 0.0F) {
         ...
         for (LivingEntity lv5 : this.world.getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(5.0))) {
            if (lv5 != this.shooter && !(this.squaredDistanceTo(lv5) > 25.0)) {
               boolean bl = false;
