...
public abstract class PersistentProjectileEntity extends ProjectileEntity {
   ...
   public void tick() {
      ...
      if (this.prevPitch == 0.0F && this.prevYaw == 0.0F) {
         ...
         this.pitch = (float)(MathHelper.atan2(lv.y, (double)f) * 180.0F / (float)Math.PI);
      ...
      if (this.inGround && !bl) {
         ...
         Vec3d lv8 = lv7.add(lv);
         HitResult lv9 = this.world.raycast(new RaycastContext(lv7, lv8, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this));
         ...
         while (!this.removed) {
            ...
            if (lv10 == null || this.getPierceLevel() <= 0) {
               break;
            }
         ...
         if (this.isCritical()) {
            for (int i = 0; i < 4; i++) {
               this.world
                  .addParticle(
                     ParticleTypes.CRIT,
                     this.getX() + d * (double)i / 4.0,
                     this.getY() + e * (double)i / 4.0,
                     this.getZ() + g * (double)i / 4.0,
                     -d,
                     -e + 0.2,
                     -g
                  );
         ...
         float l = MathHelper.sqrt(squaredHorizontalLength(lv));
         if (bl) {
         ...
         if (!this.hasNoGravity() && !bl) {
            ...
            this.setVelocity(lv13.x, lv13.y - 0.05F, lv13.z);
         }
   ...
   private void method_26352() {
      ...
      Vec3d lv = this.getVelocity();
      this.setVelocity(
         lv.multiply((double)(this.random.nextFloat() * 0.2F), (double)(this.random.nextFloat() * 0.2F), (double)(this.random.nextFloat() * 0.2F))
      );
      this.life = 0;
   }
   ...
   protected void onEntityHit(EntityHitResult entityHitResult) {
      ...
      DamageSource lv3;
      if (lv2 == null) {
         lv3 = DamageSource.arrow(this, this);
      } else {
         lv3 = DamageSource.arrow(this, lv2);
      ...
      if (lv.damage(lv3, (float)i)) {
         ...
         if (lv instanceof LivingEntity) {
            ...
            if (this.punch > 0) {
               Vec3d lv6 = this.getVelocity().multiply(1.0, 0.0, 1.0).normalize().multiply((double)this.punch * 0.6);
               if (lv6.lengthSquared() > 0.0) {
