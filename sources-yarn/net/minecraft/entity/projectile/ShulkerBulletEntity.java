...
public class ShulkerBulletEntity extends ProjectileEntity {
   ...
   private void method_7486(@Nullable Direction.Axis arg) {
      ...
      if (this.target == null) {
         ...
         lv = new BlockPos(this.target.getX(), this.target.getY() + d, this.target.getZ());
      }
      ...
      if (!lv.isWithinDistance(this.getPos(), 2.0)) {
         ...
         if (arg != Direction.Axis.X) {
            if (lv4.getX() < lv.getX() && this.world.isAir(lv4.east())) {
               list.add(Direction.EAST);
            } else if (lv4.getX() > lv.getX() && this.world.isAir(lv4.west())) {
               list.add(Direction.WEST);
            }
         }

         if (arg != Direction.Axis.Y) {
            if (lv4.getY() < lv.getY() && this.world.isAir(lv4.up())) {
               list.add(Direction.UP);
            } else if (lv4.getY() > lv.getY() && this.world.isAir(lv4.down())) {
               list.add(Direction.DOWN);
            }
         }

         if (arg != Direction.Axis.Z) {
            if (lv4.getZ() < lv.getZ() && this.world.isAir(lv4.south())) {
               list.add(Direction.SOUTH);
            } else if (lv4.getZ() > lv.getZ() && this.world.isAir(lv4.north())) {
         ...
         if (list.isEmpty()) {
            ...
            lv3 = list.get(this.random.nextInt(list.size()));
         }
