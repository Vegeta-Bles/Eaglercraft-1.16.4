...
public class BoatEntity extends Entity {
   ...
   public static boolean method_30959(Entity arg, Entity arg2) {
      return (arg2.isCollidable() || arg2.isPushable()) && !arg.isConnectedThroughVehicle(arg2);
   }
   ...
   public void tick() {
         ...
         if (this.isPaddleMoving(i)) {
            if (!this.isSilent()
               ...
               if (lv != null) {
                  ...
                  this.world
                     .playSound(null, this.getX() + d, this.getY(), this.getZ() + e, lv, this.getSoundCategory(), 1.0F, 0.8F + 0.4F * this.random.nextFloat());
               }
            }
      ...
      if (!list.isEmpty()) {
         ...
         for (int j = 0; j < list.size(); j++) {
            ...
            if (!lv3.hasPassenger(this)) {
               if (bl
                  && this.getPassengerList().size() < 2
                  && !lv3.hasVehicle()
                  && lv3.getWidth() < this.getWidth()
   ...
   private BoatEntity.Location checkLocation() {
      BoatEntity.Location lv = this.getUnderWaterLocation();
      if (lv != null) {
   ...
   private void updatePaddles() {
      if (this.hasPassengers()) {
         ...
         this.setVelocity(
            this.getVelocity()
               .add(
                  (double)(MathHelper.sin(-this.yaw * (float) (Math.PI / 180.0)) * f), 0.0, (double)(MathHelper.cos(this.yaw * (float) (Math.PI / 180.0)) * f)
               )
         );
   ...
   public void updatePassengerPosition(Entity passenger) {
      if (this.hasPassenger(passenger)) {
         ...
         Vec3d lv = new Vec3d((double)f, 0.0, 0.0).rotateY(-this.yaw * (float) (Math.PI / 180.0) - (float) (Math.PI / 2));
   ...
   public Vec3d updatePassengerForDismount(LivingEntity passenger) {
      Vec3d lv = getPassengerDismountOffset((double)(this.getWidth() * MathHelper.SQUARE_ROOT_OF_TWO), (double)passenger.getWidth(), this.yaw);
      double d = this.getX() + lv.x;
      double e = this.getZ() + lv.z;
      BlockPos lv2 = new BlockPos(d, this.getBoundingBox().maxY, e);
      ...
      if (!this.world.isWater(lv3)) {
         ...
         while (var13.hasNext()) {
            EntityPose lv4 = (EntityPose)var13.next();
   ...
   public static enum Type {
      ...
      public static BoatEntity.Type getType(String string) {
         ...
         for (int i = 0; i < lvs.length; i++) {
            if (lvs[i].getName().equals(string)) {
               return lvs[i];
            }
         }
