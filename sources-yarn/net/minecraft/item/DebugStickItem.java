      ...
      if (player.isCreativeLevelTwoOp()) {
         ...
         if (collection.isEmpty()) {
            ...
            if (update) {
               ...
               sendMessage(player, new TranslatableText(this.getTranslationKey() + ".update", lv4.getName(), getValueString(lv5, lv4)));
            } else {
               lv4 = cycle(collection, lv4, player.shouldCancelInteraction());
               String string3 = lv4.getName();
               lv3.putString(string, string3);
               sendMessage(player, new TranslatableText(this.getTranslationKey() + ".select", string3, getValueString(state, lv4)));
            }
         }
      }
   }
