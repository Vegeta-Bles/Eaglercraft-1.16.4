...
public class EnderEyeItem extends Item {
   ...
   public ActionResult useOnBlock(ItemUsageContext context) {
      ...
      if (!lv3.isOf(Blocks.END_PORTAL_FRAME) || lv3.get(EndPortalFrameBlock.EYE)) {
         return ActionResult.PASS;
      } else if (lv.isClient) {
         return ActionResult.SUCCESS;
      } else {
         BlockState lv4 = lv3.with(EndPortalFrameBlock.EYE, Boolean.valueOf(true));
   ...
   public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
      ...
      if (lv2.getType() == HitResult.Type.BLOCK && world.getBlockState(((BlockHitResult)lv2).getBlockPos()).isOf(Blocks.END_PORTAL_FRAME)) {
         ...
         if (world instanceof ServerWorld) {
            BlockPos lv3 = ((ServerWorld)world)
               .getChunkManager()
               .getChunkGenerator()
