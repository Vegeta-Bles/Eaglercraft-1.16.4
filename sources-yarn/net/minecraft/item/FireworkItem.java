...
public class FireworkItem extends Item {
   ...
   public ActionResult useOnBlock(ItemUsageContext context) {
      ...
      if (!lv.isClient) {
         ...
         FireworkRocketEntity lv5 = new FireworkRocketEntity(
            lv,
            context.getPlayer(),
            lv3.x + (double)lv4.getOffsetX() * 0.15,
            lv3.y + (double)lv4.getOffsetY() * 0.15,
            lv3.z + (double)lv4.getOffsetZ() * 0.15,
            lv2
         );
         lv.spawnEntity(lv5);
         lv2.decrement(1);
         ...
         if (lv.contains("Flight", 99)) {
            tooltip.add(
               new TranslatableText("item.minecraft.firework_rocket.flight")
                  .append(" ")
                  .append(String.valueOf(lv.getByte("Flight")))
                  .formatted(Formatting.GRAY)
            );
         }

         ListTag lv2 = lv.getList("Explosions", 10);
         if (!lv2.isEmpty()) {
            for (int i = 0; i < lv2.size(); i++) {
               ...
               if (!list2.isEmpty()) {
                  for (int j = 1; j < list2.size(); j++) {
                     list2.set(j, new LiteralText("  ").append(list2.get(j)).formatted(Formatting.GRAY));
                  }
