...
public final class ItemStack {
   public static final Codec<ItemStack> CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
               Registry.ITEM.fieldOf("id").forGetter(arg -> arg.item),
               Codec.INT.fieldOf("Count").forGetter(arg -> arg.count),
               CompoundTag.CODEC.optionalFieldOf("tag").forGetter(arg -> Optional.ofNullable(arg.tag))
            )
   ...
   private ItemStack(ItemConvertible arg, int count, Optional<CompoundTag> optional) {
      this(arg, count);
   ...
   public List<Text> getTooltip(@Nullable PlayerEntity player, TooltipContext context) {
      ...
      if (this.hasTag()) {
         ...
         if (this.tag.contains("display", 10)) {
            ...
            if (isSectionHidden(i, ItemStack.TooltipSection.DYE) && lv2.contains("color", 99)) {
         ...
         for (EquipmentSlot lv5 : EquipmentSlot.values()) {
            ...
            if (!multimap.isEmpty()) {
               ...
               for (Entry<EntityAttribute, EntityAttributeModifier> entry : multimap.entries()) {
                  ...
                  double e;
                  if (lv6.getOperation() == EntityAttributeModifier.Operation.MULTIPLY_BASE
                     || lv6.getOperation() == EntityAttributeModifier.Operation.MULTIPLY_TOTAL) {
                     e = d * 100.0;
                  } else if (entry.getKey().equals(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE)) {
                     e = d * 10.0;
                  } else {
                     e = d;
                  }
                        ...
                        new LiteralText(" ")
                           .append(
                              new TranslatableText(
                                 "attribute.modifier.equals." + lv6.getOperation().getId(),
      ...
      if (this.hasTag()) {
         ...
         if (isSectionHidden(i, ItemStack.TooltipSection.CAN_DESTROY) && this.tag.contains("CanDestroy", 9)) {
            ListTag lv7 = this.tag.getList("CanDestroy", 8);
            if (!lv7.isEmpty()) {
               list.add(LiteralText.EMPTY);
   ...
   @Environment(EnvType.CLIENT)
   private static boolean isSectionHidden(int flags, ItemStack.TooltipSection arg) {
      return (flags & arg.getFlag()) == 0;
   }
   ...
   public void addHideFlag(ItemStack.TooltipSection arg) {
      CompoundTag lv = this.getOrCreateTag();
      lv.putInt("HideFlags", lv.getInt("HideFlags") | arg.getFlag());
   }
   ...
   private static Collection<Text> parseBlockTag(String tag) {
      try {
         ...
         if (bl || bl2) {
            if (bl) {
               return Lists.newArrayList(new Text[]{lv2.getBlock().getName().formatted(Formatting.DARK_GRAY)});
            }

            Tag<Block> lv4 = BlockTags.getTagGroup().getTag(lv3);
            if (lv4 != null) {
               Collection<Block> collection = lv4.values();
   ...
   public void addEnchantment(Enchantment enchantment, int level) {
      ...
      lv2.putShort("lvl", (short)((byte)level));
      lv.add(lv2);
   }
   ...
   public Multimap<EntityAttribute, EntityAttributeModifier> getAttributeModifiers(EquipmentSlot arg) {
      ...
      if (this.hasTag() && this.tag.contains("AttributeModifiers", 9)) {
         ...
         for (int i = 0; i < lv.size(); i++) {
            ...
            if (!lv2.contains("Slot", 8) || lv2.getString("Slot").equals(arg.getName())) {
               Optional<EntityAttribute> optional = Registry.ATTRIBUTE.getOrEmpty(Identifier.tryParse(lv2.getString("AttributeName")));
               if (optional.isPresent()) {
                  EntityAttributeModifier lv3 = EntityAttributeModifier.fromTag(lv2);
         ...
         multimap = this.getItem().getAttributeModifiers(arg);
      }
   ...
   public Text toHoverableText() {
      ...
      if (!this.empty) {
         lv2.formatted(this.getRarity().formatting)
