...
public class MapState extends PersistentState {
   ...
   public void fromTag(CompoundTag tag) {
      ...
      for (int i = 0; i < lv.size(); i++) {
         ...
         this.addIcon(lv2.getIconType(), null, lv2.getKey(), (double)lv2.getPos().getX(), (double)lv2.getPos().getZ(), 180.0, lv2.getName());
      }
      ...
      for (int j = 0; j < lv3.size(); j++) {
         ...
         this.addIcon(
            MapIcon.Type.FRAME, null, "frame-" + lv4.getEntityId(), (double)lv4.getPos().getX(), (double)lv4.getPos().getZ(), (double)lv4.getRotation(), null
         );
      }
   }
   ...
   public CompoundTag toTag(CompoundTag tag) {
      Identifier.CODEC
         ...
         .resultOrPartial(field_25019::error)
         .ifPresent(arg2 -> tag.put("dimension", arg2));
   ...
   public void update(PlayerEntity player, ItemStack stack) {
      if (!this.updateTrackersByPlayer.containsKey(player)) {
         MapState.PlayerUpdateTracker lv = new MapState.PlayerUpdateTracker(player);
      ...
      for (int i = 0; i < this.updateTrackers.size(); i++) {
         MapState.PlayerUpdateTracker lv2 = this.updateTrackers.get(i);
         ...
         if (!lv2.player.removed && (lv2.player.inventory.contains(stack) || stack.isInFrame())) {
            if (!stack.isInFrame() && lv2.player.world.getRegistryKey() == this.dimension && this.showIcons) {
               this.addIcon(MapIcon.Type.PLAYER, lv2.player.world, string, lv2.player.getX(), lv2.player.getZ(), (double)lv2.player.yaw, null);
            }
         } else {
      ...
      if (stack.isInFrame() && this.showIcons) {
         ...
         this.addIcon(
            MapIcon.Type.FRAME,
            player.world,
            "frame-" + lv3.getEntityId(),
            (double)lv4.getX(),
            (double)lv4.getZ(),
            (double)(lv3.getHorizontalFacing().getHorizontal() * 90),
            null
         );
         this.frames.put(lv6.getKey(), lv6);
      }

      CompoundTag lv7 = stack.getTag();
      if (lv7 != null && lv7.contains("Decorations", 9)) {
         ...
         for (int j = 0; j < lv8.size(); j++) {
            ...
            if (!this.icons.containsKey(lv9.getString("id"))) {
               this.addIcon(
                  MapIcon.Type.byId(lv9.getByte("type")), player.world, lv9.getString("id"), lv9.getDouble("x"), lv9.getDouble("z"), lv9.getDouble("rot"), null
               );
            }
         }
      }
   ...
   public static void addDecorationsTag(ItemStack stack, BlockPos pos, String id, MapIcon.Type type) {
      ...
      if (stack.hasTag() && stack.getTag().contains("Decorations", 9)) {
         lv = stack.getTag().getList("Decorations", 10);
      } else {
         lv = new ListTag();
         stack.putSubTag("Decorations", lv);
      }

      CompoundTag lv3 = new CompoundTag();
      lv3.putByte("type", type.getId());
      lv3.putString("id", id);
      lv3.putDouble("x", (double)pos.getX());
      lv3.putDouble("z", (double)pos.getZ());
   ...
   private void addIcon(MapIcon.Type type, @Nullable WorldAccess world, String key, double x, double z, double rotation, @Nullable Text text) {
      ...
      this.icons.put(key, new MapIcon(type, b, c, k, text));
   }

   @Nullable
   public Packet<?> getPlayerMarkerPacket(ItemStack map, BlockView world, PlayerEntity pos) {
      MapState.PlayerUpdateTracker lv = this.updateTrackersByPlayer.get(pos);
      return lv == null ? null : lv.getPacket(map);
   }

   public void markDirty(int x, int z) {
      this.markDirty();

      for (MapState.PlayerUpdateTracker lv : this.updateTrackers) {
         lv.markDirty(x, z);
      }
   }

   public MapState.PlayerUpdateTracker getPlayerSyncData(PlayerEntity player) {
      MapState.PlayerUpdateTracker lv = this.updateTrackersByPlayer.get(player);
      if (lv == null) {
         lv = new MapState.PlayerUpdateTracker(player);
         this.updateTrackersByPlayer.put(player, lv);
         this.updateTrackers.add(lv);
      }
