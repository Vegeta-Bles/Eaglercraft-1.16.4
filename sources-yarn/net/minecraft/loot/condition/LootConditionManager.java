...
public class LootConditionManager extends JsonDataLoader {
   ...
   protected void apply(Map<Identifier, JsonElement> map, ResourceManager arg, Profiler arg2) {
      ...
      map.forEach((argx, jsonElement) -> {
         try {
            if (jsonElement.isJsonArray()) {
               ...
               builder.put(argx, new LootConditionManager.AndCondition(lvs));
            } else {
               LootCondition lvx = (LootCondition)GSON.fromJson(jsonElement, LootCondition.class);
      ...
      LootTableReporter lv = new LootTableReporter(LootContextTypes.GENERIC, map2::get, argx -> null);
      map2.forEach((arg2x, arg3) -> arg3.validate(lv.withCondition("{" + arg2x + "}", arg2x)));
