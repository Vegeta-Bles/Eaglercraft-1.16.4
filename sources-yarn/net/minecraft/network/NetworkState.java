...
public enum NetworkState {
   ...
   private NetworkState(int id, NetworkState.PacketHandlerInitializer arg) {
      this.stateId = id;
   ...
   static {
      for (NetworkState lv : values()) {
         ...
         lv.packetHandlers
            .forEach(
               (arg2, arg3) -> arg3.getPacketTypes()
                     .forEach(
                        class_ -> {
                           if (HANDLER_STATE_MAP.containsKey(class_) && HANDLER_STATE_MAP.get(class_) != lv) {
                              throw new IllegalStateException(
                                 "Packet " + class_ + " is already assigned to protocol " + HANDLER_STATE_MAP.get(class_) + " - can't reassign to " + lv
                              );
                           } else {
                              HANDLER_STATE_MAP.put((Class<? extends Packet<?>>)class_, lv);
                           }
                        }
                     )
            );
   ...
   static class PacketHandler<T extends PacketListener> {
      ...
      public Packet<?> createPacket(int id) {
         Supplier<? extends Packet<T>> supplier = this.packetFactories.get(id);
         return (Packet<?>)(supplier != null ? supplier.get() : null);
      }
   ...
   static class PacketHandlerInitializer {
      ...
      public <T extends PacketListener> NetworkState.PacketHandlerInitializer setup(NetworkSide side, NetworkState.PacketHandler<T> arg2) {
         this.packetHandlers.put(side, arg2);
         return this;
      }
   }
}
