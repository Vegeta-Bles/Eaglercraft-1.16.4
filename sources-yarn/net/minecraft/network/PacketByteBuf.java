...
public class PacketByteBuf extends ByteBuf {
   ...
   public <T> T decode(Codec<T> codec) throws IOException {
      ...
      DataResult<T> dataResult = codec.parse(NbtOps.INSTANCE, lv);
      if (dataResult.error().isPresent()) {
         throw new IOException("Failed to decode: " + ((PartialResult)dataResult.error().get()).message() + " " + lv);
      } else {
   ...
   public <T extends Enum<T>> T readEnumConstant(Class<T> class_) {
      return class_.getEnumConstants()[this.readVarInt()];
   }
   ...
   public PacketByteBuf writeCompoundTag(@Nullable CompoundTag arg) {
      if (arg == null) {
         ...
         try {
            NbtIo.write(arg, new ByteBufOutputStream(this));
   ...
   public CompoundTag method_30616(PositionTracker arg) {
      ...
      if (b == 0) {
         ...
         try {
            return NbtIo.read(new ByteBufInputStream(this), arg);
