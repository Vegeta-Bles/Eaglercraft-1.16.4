...
public class EntityAttributesS2CPacket implements Packet<ClientPlayPacketListener> {
   ...
   public EntityAttributesS2CPacket(int entityId, Collection<EntityAttributeInstance> attributes) {
      ...
      for (EntityAttributeInstance lv : attributes) {
         this.entries.add(new EntityAttributesS2CPacket.Entry(lv.getAttribute(), lv.getBaseValue(), lv.getModifiers()));
      }
   }
         ...
         for (int l = 0; l < k; l++) {
            ...
            list.add(
               new EntityAttributeModifier(
                  uUID, "Unknown synced attribute modifier", buf.readDouble(), EntityAttributeModifier.Operation.fromId(buf.readByte())
               )
            );
         }

         this.entries.add(new EntityAttributesS2CPacket.Entry(lv2, d, list));
      }
   }
   ...
   public void write(PacketByteBuf buf) throws IOException {
      ...
      for (EntityAttributesS2CPacket.Entry lv : this.entries) {
