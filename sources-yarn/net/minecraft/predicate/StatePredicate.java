...
public class StatePredicate {
   ...
   private static StatePredicate.Condition createPredicate(String key, JsonElement json) {
      if (json.isJsonPrimitive()) {
         ...
         return new StatePredicate.ExactValueCondition(key, string2);
      } else {
         ...
         return (StatePredicate.Condition)(string3 != null && string3.equals(string4)
            ? new StatePredicate.ExactValueCondition(key, string3)
            : new StatePredicate.RangedValueCondition(key, string3, string4));
      }
   }
   ...
   public <S extends State<?, S>> boolean test(StateManager<?, S> stateManager, S container) {
      for (StatePredicate.Condition lv : this.conditions) {
         if (!lv.test(stateManager, container)) {
            return false;
         }
      }
   ...
   public static StatePredicate fromJson(@Nullable JsonElement json) {
      if (json != null && !json.isJsonNull()) {
         ...
         for (Entry<String, JsonElement> entry : jsonObject.entrySet()) {
            list.add(createPredicate(entry.getKey(), entry.getValue()));
         }
