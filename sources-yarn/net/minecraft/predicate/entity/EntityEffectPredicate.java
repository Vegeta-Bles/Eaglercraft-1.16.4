...
public class EntityEffectPredicate {
   ...
   public static EntityEffectPredicate fromJson(@Nullable JsonElement json) {
      if (json != null && !json.isJsonNull()) {
         ...
         for (Entry<String, JsonElement> entry : jsonObject.entrySet()) {
            Identifier lv = new Identifier(entry.getKey());
            StatusEffect lv2 = Registry.STATUS_EFFECT.getOrEmpty(lv).orElseThrow(() -> new JsonSyntaxException("Unknown effect '" + lv + "'"));
            EntityEffectPredicate.EffectData lv3 = EntityEffectPredicate.EffectData.fromJson(JsonHelper.asObject(entry.getValue(), entry.getKey()));
            map.put(lv2, lv3);
         }
   ...
   public static class EffectData {
      ...
      public static EntityEffectPredicate.EffectData fromJson(JsonObject json) {
         ...
         return new EntityEffectPredicate.EffectData(lv, lv2, boolean_, boolean2);
      }
   }
}
