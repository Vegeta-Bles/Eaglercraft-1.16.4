   ...
   public static EntityTypePredicate fromJson(@Nullable JsonElement json) {
      if (json != null && !json.isJsonNull()) {
         ...
         if (string.startsWith("#")) {
            ...
            return new EntityTypePredicate.Tagged(ServerTagManagerHolder.getTagManager().getEntityTypes().getTagOrEmpty(lv));
         } else {
            Identifier lv2 = new Identifier(string);
            EntityType<?> lv3 = Registry.ENTITY_TYPE
               .getOrEmpty(lv2)
               .orElseThrow(
                  () -> new JsonSyntaxException("Unknown entity type '" + lv2 + "', valid types are: " + COMMA_JOINER.join(Registry.ENTITY_TYPE.getIds()))
               );
            return new EntityTypePredicate.Single(lv3);
         }
      } else {
         return ANY;
      }
