...
public class BrewingRecipeRegistry {
   ...
   protected static boolean isItemRecipeIngredient(ItemStack stack) {
      ...
      for (int j = ITEM_RECIPES.size(); i < j; i++) {
         if (ITEM_RECIPES.get(i).ingredient.test(stack)) {
            return true;
         }
      }
   ...
   protected static boolean isPotionRecipeIngredient(ItemStack stack) {
      ...
      for (int j = POTION_RECIPES.size(); i < j; i++) {
         if (POTION_RECIPES.get(i).ingredient.test(stack)) {
            return true;
         }
      }
   ...
   protected static boolean hasItemRecipe(ItemStack input, ItemStack ingredient) {
      ...
      for (int j = ITEM_RECIPES.size(); i < j; i++) {
         BrewingRecipeRegistry.Recipe<Item> lv2 = ITEM_RECIPES.get(i);
   ...
   protected static boolean hasPotionRecipe(ItemStack input, ItemStack ingredient) {
      ...
      for (int j = POTION_RECIPES.size(); i < j; i++) {
         BrewingRecipeRegistry.Recipe<Potion> lv2 = POTION_RECIPES.get(i);
         if (lv2.input == lv && lv2.ingredient.test(ingredient)) {
            return true;
         }
      }
   ...
   public static ItemStack craft(ItemStack input, ItemStack ingredient) {
      if (!ingredient.isEmpty()) {
         ...
         for (int j = ITEM_RECIPES.size(); i < j; i++) {
            BrewingRecipeRegistry.Recipe<Item> lv3 = ITEM_RECIPES.get(i);
            if (lv3.input == lv2 && lv3.ingredient.test(input)) {
               return PotionUtil.setPotion(new ItemStack(lv3.output), lv);
            }
         }
         ...
         for (int l = POTION_RECIPES.size(); i < l; i++) {
            BrewingRecipeRegistry.Recipe<Potion> lv4 = POTION_RECIPES.get(i);
            if (lv4.input == lv && lv4.ingredient.test(input)) {
               return PotionUtil.setPotion(new ItemStack(lv2), lv4.output);
            }
         }
      }
   ...
   private static void registerPotionRecipe(Potion input, Item arg2, Potion output) {
      POTION_RECIPES.add(new BrewingRecipeRegistry.Recipe<>(input, Ingredient.ofItems(arg2), output));
   }
