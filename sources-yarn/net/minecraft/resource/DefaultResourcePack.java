...
public class DefaultResourcePack implements ResourcePack {
   ...
   public Collection<Identifier> findResources(ResourceType type, String namespace, String prefix, int maxDepth, Predicate<String> pathFilter) {
      ...
      try {
         ...
         if ("file".equals(uRI2.getScheme())) {
            ...
            Path path2 = typeToFileSystem.get(type).getPath("/" + type.getDirectory());
   ...
   private static void getIdentifiers(
      ...
      try (Stream<Path> stream = Files.walk(path2.resolve(searchLocation), maxDepth)) {
         stream.filter(pathx -> !pathx.endsWith(".mcmeta") && Files.isRegularFile(pathx) && predicate.test(pathx.getFileName().toString()))
