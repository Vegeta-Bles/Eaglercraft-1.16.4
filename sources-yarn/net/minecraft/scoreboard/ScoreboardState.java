...
public class ScoreboardState extends PersistentState {
   ...
   protected void deserializeTeams(ListTag arg) {
      for (int i = 0; i < arg.size(); i++) {
         ...
         if (lv.contains("MemberNamePrefix", 8)) {
            Text lv4 = Text.Serializer.fromJson(lv.getString("MemberNamePrefix"));
            if (lv4 != null) {
               lv2.setPrefix(lv4);
            }
         }

         if (lv.contains("MemberNameSuffix", 8)) {
            Text lv5 = Text.Serializer.fromJson(lv.getString("MemberNameSuffix"));
            if (lv5 != null) {
               lv2.setSuffix(lv5);
            }
         }

         if (lv.contains("NameTagVisibility", 8)) {
            AbstractTeam.VisibilityRule lv6 = AbstractTeam.VisibilityRule.getRule(lv.getString("NameTagVisibility"));
            if (lv6 != null) {
               lv2.setNameTagVisibilityRule(lv6);
            }
         }

         if (lv.contains("DeathMessageVisibility", 8)) {
            AbstractTeam.VisibilityRule lv7 = AbstractTeam.VisibilityRule.getRule(lv.getString("DeathMessageVisibility"));
            if (lv7 != null) {
               lv2.setDeathMessageVisibilityRule(lv7);
            }
         }

         if (lv.contains("CollisionRule", 8)) {
            AbstractTeam.CollisionRule lv8 = AbstractTeam.CollisionRule.getRule(lv.getString("CollisionRule"));
            if (lv8 != null) {
   ...
   protected void deserializeObjectives(ListTag arg) {
      for (int i = 0; i < arg.size(); i++) {
         ...
         ScoreboardCriterion.createStatCriterion(lv.getString("CriteriaName")).ifPresent(arg2 -> {
            ...
            this.scoreboard.addObjective(string, arg2, lvx, lv2);
         });
      }
   }
   ...
   protected ListTag serializeTeams() {
      ...
      for (Team lv2 : this.scoreboard.getTeams()) {
   ...
   protected ListTag serializeObjectives() {
      ...
      for (ScoreboardObjective lv2 : this.scoreboard.getObjectives()) {
