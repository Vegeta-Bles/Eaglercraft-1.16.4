...
public abstract class ScreenHandler {
   ...
   protected static boolean canUse(ScreenHandlerContext context, PlayerEntity player, Block block) {
      return context.run(
         (arg3, arg4) -> !arg3.getBlockState(arg4).isOf(block)
               ? false
               : player.squaredDistanceTo((double)arg4.getX() + 0.5, (double)arg4.getY() + 0.5, (double)arg4.getZ() + 0.5) <= 64.0,
         true
      );
   }
   ...
   public ItemStack transferSlot(PlayerEntity player, int index) {
      ...
      return lv != null ? lv.getStack() : ItemStack.EMPTY;
   }
   ...
   private ItemStack method_30010(int i, int j, SlotActionType arg, PlayerEntity arg2) {
      ...
      if (arg == SlotActionType.QUICK_CRAFT) {
         ...
         if ((k != 1 || this.quickCraftButton != 2) && k != this.quickCraftButton) {
            ...
            if (shouldQuickCraftContinue(this.quickCraftStage, arg2)) {
            ...
            if (lv3 != null
               && canInsertItemIntoSlot(lv3, lv4, true)
               && lv3.canInsert(lv4)
               && (this.quickCraftStage == 2 || lv4.getCount() > this.quickCraftSlots.size())
               && this.canInsertIntoSlot(lv3)) {
            ...
            if (!this.quickCraftSlots.isEmpty()) {
               ...
               for (Slot lv6 : this.quickCraftSlots) {
                  ...
                  if (lv6 != null
                     && canInsertItemIntoSlot(lv6, lv7, true)
                     && lv6.canInsert(lv7)
                     && (this.quickCraftStage == 2 || lv7.getCount() >= this.quickCraftSlots.size())
                     && this.canInsertIntoSlot(lv6)) {
                     ItemStack lv8 = lv5.copy();
                     int m = lv6.hasStack() ? lv6.getStack().getCount() : 0;
                     calculateStackSize(this.quickCraftSlots, this.quickCraftStage, lv8, m);
                     int n = Math.min(lv8.getMaxCount(), lv6.getMaxItemCount(lv8));
                     if (lv8.getCount() > n) {
                        lv8.setCount(n);
                     }
         ...
         if (i == -999) {
            ...
            for (ItemStack lv10 = this.transferSlot(arg2, i);
               !lv10.isEmpty() && ItemStack.areItemsEqualIgnoreDamage(lv9.getStack(), lv10);
            ...
            if (lv11 != null) {
               ...
               if (lv12.isEmpty()) {
                  if (!lv13.isEmpty() && lv11.canInsert(lv13)) {
                     int o = j == 0 ? lv13.getCount() : 1;
                     if (o > lv11.getMaxItemCount(lv13)) {
                        o = lv11.getMaxItemCount(lv13);
                     }
                  ...
                  if (lv13.isEmpty()) {
                     ...
                     if (canStacksCombine(lv12, lv13)) {
                        int q = j == 0 ? lv13.getCount() : 1;
                        if (q > lv11.getMaxItemCount(lv13) - lv12.getCount()) {
                           q = lv11.getMaxItemCount(lv13) - lv12.getCount();
                        }
                        ...
                        lv13.decrement(q);
                        lv12.increment(q);
                     } else if (lv13.getCount() <= lv11.getMaxItemCount(lv13)) {
                        lv11.setStack(lv13);
                        lv2.setCursorStack(lv12);
                     }
                  } else if (lv13.getMaxCount() > 1 && canStacksCombine(lv12, lv13) && !lv12.isEmpty()) {
                     int r = lv12.getCount();
                     if (r + lv13.getCount() <= lv13.getMaxCount()) {
                        lv13.increment(r);
         ...
         if (!lv15.isEmpty() || !lv16.isEmpty()) {
            if (lv15.isEmpty()) {
               ...
               int t = lv14.getMaxItemCount(lv15);
               if (lv15.getCount() > t) {
         ...
         if (!lv22.isEmpty() && (lv21 == null || !lv21.hasStack() || !lv21.canTakeItems(arg2))) {
            ...
            for (int w = 0; w < 2; w++) {
               for (int x = u; x >= 0 && x < this.slots.size() && lv22.getCount() < lv22.getMaxCount(); x += v) {
                  ...
                  if (lv23.hasStack() && canInsertItemIntoSlot(lv23, lv22, true) && lv23.canTakeItems(arg2) && this.canInsertIntoSlot(lv22, lv23)) {
                     ItemStack lv24 = lv23.getStack();
                     if (w != 0 || lv24.getCount() != lv24.getMaxCount()) {
   ...
   protected boolean insertItem(ItemStack stack, int startIndex, int endIndex, boolean fromLast) {
      ...
      if (stack.isStackable()) {
         while (!stack.isEmpty() && (fromLast ? k >= startIndex : k < endIndex)) {
            ...
            if (!lv2.isEmpty() && canStacksCombine(stack, lv2)) {
      ...
      if (!stack.isEmpty()) {
         ...
         while (fromLast ? k >= startIndex : k < endIndex) {
            ...
            if (lv4.isEmpty() && lv3.canInsert(stack)) {
               if (stack.getCount() > lv3.getMaxItemCount()) {
                  lv3.setStack(stack.split(lv3.getMaxItemCount()));
               } else {
