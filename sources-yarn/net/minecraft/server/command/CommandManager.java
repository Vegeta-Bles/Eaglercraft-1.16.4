...
public class CommandManager {
      ...
      this.dispatcher
         .findAmbiguities(
            (commandNode, commandNode2, commandNode3, collection) -> LOGGER.warn(
                  ...
                  this.dispatcher.getPath(commandNode2),
                  this.dispatcher.getPath(commandNode3),
                  collection
               )
         );
   ...
   public int execute(ServerCommandSource commandSource, String command) {
      ...
      try {
         ...
         if (var14.getInput() != null && var14.getCursor() >= 0) {
            ...
            MutableText lv2 = new LiteralText("")
               .formatted(Formatting.GRAY)
               .styled(arg -> arg.withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, command)));
            if (i > 10) {
               lv2.append("...");
            }

            lv2.append(var14.getInput().substring(Math.max(0, i - 10), i));
            if (i < var14.getInput().length()) {
               Text lv3 = new LiteralText(var14.getInput().substring(i)).formatted(new Formatting[]{Formatting.RED, Formatting.UNDERLINE});
               lv2.append(lv3);
            }

            lv2.append(new TranslatableText("command.context.here").formatted(new Formatting[]{Formatting.RED, Formatting.ITALIC}));
            commandSource.sendError(lv2);
         }

         return 0;
      } catch (Exception var15) {
         MutableText lv4 = new LiteralText(var15.getMessage() == null ? var15.getClass().getName() : var15.getMessage());
   ...
   private void makeTreeForSource(
      ...
      for (CommandNode<ServerCommandSource> commandNode3 : tree.getChildren()) {
         if (commandNode3.canUse(source)) {
            ArgumentBuilder<?, ?> baseBuilder = commandNode3.createBuilder();
            ArgumentBuilder<CommandSource, ?> argumentBuilder = (ArgumentBuilder<CommandSource, ?>)baseBuilder;
   ...
   public static void checkMissing() {
      ...
      Set<ArgumentType<?>> set2 = set.stream().filter(argumentType -> !ArgumentTypes.hasClass((ArgumentType<?>)argumentType)).collect(Collectors.toSet());
      if (!set2.isEmpty()) {
         LOGGER.warn(
