...
public class DatapackCommand {
   ...
   private static final SuggestionProvider<ServerCommandSource> DISABLED_CONTAINERS_SUGGESTION_PROVIDER = (commandContext, suggestionsBuilder) -> {
      ...
      return CommandSource.suggestMatching(
         lv.getNames().stream().filter(string -> !collection.contains(string)).map(StringArgumentType::escapeIfRequired), suggestionsBuilder
      );
   };
                           ...
                           ((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument(
                                          ...
                                          .executes(
                                             commandContext -> executeEnable(
                                                   ...
                                                   (list, arg) -> arg.getInitialPosition().insert(list, arg, argx -> argx, false)
                                                )
                                          ))
                                       .then(
                                                ...
                                                CommandManager.argument("existing", StringArgumentType.string())
                                                   ...
                                                   .executes(
                                                      commandContext -> executeEnable(
                                                            (ServerCommandSource)commandContext.getSource(),
                                                            getPackContainer(commandContext, "name", true),
                                                            (list, arg) -> list.add(list.indexOf(getPackContainer(commandContext, "existing", false)) + 1, arg)
                                                         )
                                                   )
                                             )
                                       ))
                                             ...
                                             CommandManager.argument("existing", StringArgumentType.string())
                                                ...
                                                .executes(
                                                   commandContext -> executeEnable(
                                                         (ServerCommandSource)commandContext.getSource(),
                                                         getPackContainer(commandContext, "name", true),
                                                         (list, arg) -> list.add(list.indexOf(getPackContainer(commandContext, "existing", false)), arg)
                                                      )
                                                )
                                          )
                                    ))
                                 .then(
                                    CommandManager.literal("last")
                                       .executes(
                                          commandContext -> executeEnable(
                                                (ServerCommandSource)commandContext.getSource(), getPackContainer(commandContext, "name", true), List::add
                                             )
                                       )
                                 ))
                              .then(
                  ...
                  CommandManager.literal("disable")
                     .then(
                        CommandManager.argument("name", StringArgumentType.string())
                           .suggests(ENABLED_CONTAINERS_SUGGESTION_PROVIDER)
                           .executes(
                              commandContext -> executeDisable((ServerCommandSource)commandContext.getSource(), getPackContainer(commandContext, "name", false))
                           )
                     )
               ))
            .then(
               ((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("list")
                     ...
                     .then(
                        CommandManager.literal("available").executes(commandContext -> executeListAvailable((ServerCommandSource)commandContext.getSource()))
                     ))
   ...
   private static int executeEnable(ServerCommandSource source, ResourcePackProfile container, DatapackCommand.PackAdder packAdder) throws CommandSyntaxException {
      ...
      List<ResourcePackProfile> list = Lists.newArrayList(lv.getEnabledProfiles());
   ...
   private static int executeListAvailable(ServerCommandSource source) {
      ResourcePackManager lv = source.getMinecraftServer().getDataPackManager();
      lv.scanPacks();
      Collection<? extends ResourcePackProfile> collection = lv.getEnabledProfiles();
      Collection<? extends ResourcePackProfile> collection2 = lv.getProfiles();
      List<ResourcePackProfile> list = collection2.stream().filter(arg -> !collection.contains(arg)).collect(Collectors.toList());
      if (list.isEmpty()) {
         source.sendFeedback(new TranslatableText("commands.datapack.list.available.none"), false);
      } else {
         source.sendFeedback(
            new TranslatableText("commands.datapack.list.available.success", list.size(), Texts.join(list, arg -> arg.getInformationText(false))), false
         );
      }
   ...
   private static int executeListEnabled(ServerCommandSource source) {
      ...
      if (collection.isEmpty()) {
         ...
         source.sendFeedback(
            new TranslatableText("commands.datapack.list.enabled.success", collection.size(), Texts.join(collection, arg -> arg.getInformationText(true))),
            false
         );
      }
   ...
   interface PackAdder {
      void apply(List<ResourcePackProfile> list, ResourcePackProfile arg) throws CommandSyntaxException;
   }
}
