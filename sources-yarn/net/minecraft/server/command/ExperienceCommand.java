...
public class ExperienceCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
                              ...
                              CommandManager.literal("points")
                                 .executes(
                                    commandContext -> executeQuery(
                                          (ServerCommandSource)commandContext.getSource(),
                                          EntityArgumentType.getPlayer(commandContext, "targets"),
                                          ExperienceCommand.Component.POINTS
                                       )
                                 )
                           ))
      ...
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("xp").requires(arg -> arg.hasPermissionLevel(2))).redirect(literalCommandNode)
      );
   }
   ...
   static enum Component {
      POINTS("points", PlayerEntity::addExperience, (arg, integer) -> {
         if (integer >= arg.getNextLevelExperience()) {
            ...
            arg.setExperiencePoints(integer);
            return true;
         }
      ...
      LEVELS("levels", ServerPlayerEntity::addExperienceLevels, (arg, integer) -> {
         arg.setExperienceLevel(integer);
         return true;
