...
public class GameModeCommand {
         ...
         if (lv != GameMode.NOT_SET) {
            literalArgumentBuilder.then(
               ((LiteralArgumentBuilder)CommandManager.literal(lv.getName())
                     .executes(
                        commandContext -> execute(commandContext, Collections.singleton(((ServerCommandSource)commandContext.getSource()).getPlayer()), lv)
                     ))
                  .then(
                     CommandManager.argument("target", EntityArgumentType.players())
                        .executes(commandContext -> execute(commandContext, EntityArgumentType.getPlayers(commandContext, "target"), lv))
                  )
            );
         }
      }
   ...
   private static void setGameMode(ServerCommandSource source, ServerPlayerEntity player, GameMode gameMode) {
      Text lv = new TranslatableText("gameMode." + gameMode.getName());
   ...
   private static int execute(CommandContext<ServerCommandSource> context, Collection<ServerPlayerEntity> targets, GameMode gameMode) {
      ...
      for (ServerPlayerEntity lv : targets) {
         if (lv.interactionManager.getGameMode() != gameMode) {
            lv.setGameMode(gameMode);
            setGameMode((ServerCommandSource)context.getSource(), lv, gameMode);
            i++;
         }
      }
