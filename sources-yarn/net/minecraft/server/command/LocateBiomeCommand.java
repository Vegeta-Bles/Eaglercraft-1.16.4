...
public class LocateBiomeCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("locatebiome").requires(arg -> arg.hasPermissionLevel(2)))
            .then(
               CommandManager.argument("biome", IdentifierArgumentType.identifier())
                  .suggests(SuggestionProviders.ALL_BIOMES)
                  .executes(
                     commandContext -> execute(
                           (ServerCommandSource)commandContext.getSource(), (Identifier)commandContext.getArgument("biome", Identifier.class)
                        )
                  )
            )
      );
   ...
   private static int execute(ServerCommandSource source, Identifier arg2) throws CommandSyntaxException {
      Biome lv = source.getMinecraftServer().getRegistryManager().get(Registry.BIOME_KEY).getOrEmpty(arg2).orElseThrow(() -> INVALID_EXCEPTION.create(arg2));
