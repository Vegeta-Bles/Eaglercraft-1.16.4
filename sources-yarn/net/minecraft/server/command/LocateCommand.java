...
public class LocateCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      ...
      for (Entry<String, StructureFeature<?>> entry : StructureFeature.STRUCTURES.entrySet()) {
         literalArgumentBuilder = (LiteralArgumentBuilder<ServerCommandSource>)literalArgumentBuilder.then(
            CommandManager.literal(entry.getKey()).executes(commandContext -> execute((ServerCommandSource)commandContext.getSource(), entry.getValue()))
         );
      }
   ...
   private static int execute(ServerCommandSource source, StructureFeature<?> arg2) throws CommandSyntaxException {
      ...
      if (lv2 == null) {
         ...
         return sendCoordinates(source, arg2.getName(), lv, lv2, "commands.locate.success");
      }
   }
   ...
   public static int sendCoordinates(ServerCommandSource source, String structure, BlockPos sourcePos, BlockPos structurePos, String successMessage) {
      ...
      Text lv = Texts.bracketed(new TranslatableText("chat.coordinates", structurePos.getX(), "~", structurePos.getZ()))
         .styled(
            arg2 -> arg2.withColor(Formatting.GREEN)
