   ...
   private static int executeEntity(ServerCommandSource source, Collection<? extends Entity> targets, int slot, ItemStack item) throws CommandSyntaxException {
      ...
      for (Entity lv : targets) {
         if (lv instanceof ServerPlayerEntity) {
            ((ServerPlayerEntity)lv).playerScreenHandler.sendContentUpdates();
         }

         if (lv.equip(slot, item.copy())) {
            list.add(lv);
            if (lv instanceof ServerPlayerEntity) {
               ((ServerPlayerEntity)lv).playerScreenHandler.sendContentUpdates();
            }
      ...
      if (list.isEmpty()) {
         ...
         if (list.size() == 1) {
            source.sendFeedback(
               new TranslatableText("commands.replaceitem.entity.success.single", list.iterator().next().getDisplayName(), item.toHoverableText()), true
            );
         } else {
