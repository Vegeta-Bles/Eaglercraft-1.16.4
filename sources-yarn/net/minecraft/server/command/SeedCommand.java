...
public class SeedCommand {
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher, boolean dedicated) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("seed").requires(arg -> !dedicated || arg.hasPermissionLevel(2)))
            .executes(
               commandContext -> {
                  long l = ((ServerCommandSource)commandContext.getSource()).getWorld().getSeed();
                  Text lv = Texts.bracketed(
                     new LiteralText(String.valueOf(l))
                        .styled(
                           arg -> arg.withColor(Formatting.GREEN)
