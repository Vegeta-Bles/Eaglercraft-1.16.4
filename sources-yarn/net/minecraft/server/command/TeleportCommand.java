...
public class TeleportCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
                                                         ...
                                                         .executes(
                                                            commandContext -> execute(
                                                                  ...
                                                                  new TeleportCommand.LookTarget(
                                                                     EntityArgumentType.getEntity(commandContext, "facingEntity"),
                                                         ...
                                                         CommandManager.argument("facingAnchor", EntityAnchorArgumentType.entityAnchor())
                                                            .executes(
                                                               commandContext -> execute(
                                                                     ...
                                                                     new TeleportCommand.LookTarget(
                                                                        EntityArgumentType.getEntity(commandContext, "facingEntity"),
                                                                        EntityAnchorArgumentType.getEntityAnchor(commandContext, "facingAnchor")
                                                                     )
                                                                  )
                                                            )
                                                      )
      ...
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("tp").requires(arg -> arg.hasPermissionLevel(2))).redirect(literalCommandNode)
      );
   }
   ...
   private static int execute(
      ...
      for (Entity lv3 : targets) {
         if (rotation == null) {
            teleport(source, lv3, world, lv.x, lv.y, lv.z, set, lv3.yaw, lv3.pitch, facingLocation);
         } else {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(
            new TranslatableText("commands.teleport.success.location.single", targets.iterator().next().getDisplayName(), lv.x, lv.y, lv.z), true
         );
      } else {
   ...
   private static void teleport(
      ...
      if (!World.isValid(lv)) {
         ...
         if (target instanceof ServerPlayerEntity) {
            ...
            world.getChunkManager().addTicket(ChunkTicketType.POST_TELEPORT, lv2, 1, target.getEntityId());
            target.stopRiding();
