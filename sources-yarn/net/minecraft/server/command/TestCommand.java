...
public class TestCommand {
         ...
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal(
                                          ...
                                          .then(
                                             CommandManager.literal("runthis")
                                                .executes(commandContext -> executeRunThis((ServerCommandSource)commandContext.getSource()))
                                          ))
                                       .then(
                                          CommandManager.literal("runthese")
                                             .executes(commandContext -> executeRunThese((ServerCommandSource)commandContext.getSource()))
                                       ))
                                    .then(
                              ...
                              CommandManager.literal("export")
                                 .then(
                                    CommandManager.argument("testName", StringArgumentType.word())
                                       .executes(
                                          commandContext -> executeExport(
                                                (ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName")
                                             )
                                       )
                                 )
                           ))
                  ...
                  .then(
                     ((LiteralArgumentBuilder)CommandManager.literal("pos")
                           .executes(commandContext -> executePos((ServerCommandSource)commandContext.getSource(), "pos")))
                        .then(
                           CommandManager.argument("var", StringArgumentType.word())
                              .executes(
                                 commandContext -> executePos(
                                       (ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "var")
                                    )
                              )
                        )
                  ))
                        ...
                        ((RequiredArgumentBuilder)CommandManager.argument("testName", StringArgumentType.word())
                              .executes(
                                 commandContext -> executeCreate(
                                       (ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"), 5, 5, 5
                                    )
                              ))
                           .then(
               ...
               ((LiteralArgumentBuilder)CommandManager.literal("clearall")
                  ...
                  .then(
                     CommandManager.argument("radius", IntegerArgumentType.integer())
                        .executes(
                           commandContext -> executeClearAll(
                                 (ServerCommandSource)commandContext.getSource(), IntegerArgumentType.getInteger(commandContext, "radius")
                              )
                        )
                  )
            )
   ...
   private static int executePos(ServerCommandSource source, String variableName) throws CommandSyntaxException {
      BlockHitResult lv = (BlockHitResult)source.getPlayer().raycast(10.0, 1.0F, false);
      BlockPos lv2 = lv.getBlockPos();
      ServerWorld lv3 = source.getWorld();
      Optional<BlockPos> optional = StructureTestUtil.findContainingStructureBlock(lv2, 15, lv3);
      ...
      if (!optional.isPresent()) {
         ...
         Text lv6 = new LiteralText(string2)
            .setStyle(
               Style.EMPTY
                  .withBold(true)
                  .withColor(Formatting.GREEN)
                  .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to copy to clipboard")))
                  .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, "final BlockPos " + variableName + " = new BlockPos(" + string2 + ");"))
            );
   ...
   private static int executeRunThese(ServerCommandSource source) {
      BlockPos lv = new BlockPos(source.getPosition());
      ServerWorld lv2 = source.getWorld();
      Collection<BlockPos> collection = StructureTestUtil.findStructureBlocks(lv, 200, lv2);
      if (collection.isEmpty()) {
         ...
         collection.forEach(arg3 -> run(lv2, arg3, lv3));
         return 1;
      }
   }
   ...
   private static void run(ServerWorld world, BlockPos pos, @Nullable TestSet tests) {
      ...
      if (tests != null) {
         tests.add(lv3);
         lv3.addListener(new TestCommand.Listener(world, tests));
      }
   ...
   private static int method_29411(ServerCommandSource arg, boolean bl, int i, int j) {
      ...
      if (bl) {
         collection = TestFunctions.method_29405().stream().filter(TestFunction::isRequired).collect(Collectors.toList());
      } else {
         collection = TestFunctions.method_29405();
      }

      if (collection.isEmpty()) {
         sendMessage(arg, "No failed tests to rerun");
         return 0;
      } else {
         TestUtil.clearDebugMarkers(arg.getWorld());
         sendMessage(arg, "Rerunning " + collection.size() + " failed tests (" + (bl ? "only required tests" : "including optional tests") + ")");
         run(arg, collection, i, j);
         return 1;
      }
   }
   ...
   private static void run(ServerCommandSource source, Collection<TestFunction> testFunctions, int i, int j) {
      ...
      lv5.addListener(new TestCommand.Listener(lv3, lv5));
   ...
   private static int method_29413(ServerCommandSource arg) {
      ...
      if (lv3 == null) {
         ...
         return executeExport(arg, string);
      }
   }
   ...
   private static int executeImport(ServerCommandSource arg, String structure) {
      ...
      try {
         ...
         sendMessage(arg, "Imported to " + path2.toAbsolutePath());
         return 0;
   ...
   private static void sendMessage(ServerWorld world, String message, Formatting formatting) {
      world.getPlayers(arg -> true).forEach(arg2 -> arg2.sendSystemMessage(new LiteralText(formatting + message), Util.NIL_UUID));
   }
