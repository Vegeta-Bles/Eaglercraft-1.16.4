...
public final class ServerMBean implements DynamicMBean {
   ...
   private final Map<String, ServerMBean.Entry> entries = Stream.of(
      ...
      .collect(Collectors.toMap(arg -> arg.name, Function.identity()));

   private ServerMBean(MinecraftServer server) {
      this.server = server;
      MBeanAttributeInfo[] mBeanAttributeInfos = this.entries.values().stream().map(object -> object.createInfo()).toArray(MBeanAttributeInfo[]::new);
   ...
   public Object getAttribute(String attribute) {
      ServerMBean.Entry lv = this.entries.get(attribute);
   ...
   public AttributeList getAttributes(String[] attributes) {
      List<Attribute> list = Arrays.stream(attributes)
         ...
         .map(arg -> new Attribute(arg.name, arg.getter.get()))
