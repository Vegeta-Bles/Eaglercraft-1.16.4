...
public class ServerPropertiesHandler extends AbstractPropertiesHandler<ServerPropertiesHandler> {
   ...
   public ServerPropertiesHandler(Properties properties, DynamicRegistryManager arg) {
      super(properties);
      ...
      this.generatorOptions = GeneratorOptions.fromProperties(arg, properties);
   }

   public static ServerPropertiesHandler load(DynamicRegistryManager arg, Path path) {
      return new ServerPropertiesHandler(loadProperties(path), arg);
   }

   protected ServerPropertiesHandler create(DynamicRegistryManager arg, Properties properties) {
      return new ServerPropertiesHandler(properties, arg);
   }
}
