...
public class OpCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("op").requires(arg -> arg.hasPermissionLevel(3)))
            .then(
                  ...
                  .suggests(
                     (commandContext, suggestionsBuilder) -> {
                        ...
                        return CommandSource.suggestMatching(
                           lv.getPlayerList().stream().filter(arg2 -> !lv.isOperator(arg2.getGameProfile())).map(arg -> arg.getGameProfile().getName()),
                           suggestionsBuilder
                        );
                     }
                  )
