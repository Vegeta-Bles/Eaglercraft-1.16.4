...
public class CommandFunction {
   ...
   public static CommandFunction create(Identifier id, CommandDispatcher<ServerCommandSource> commandDispatcher, ServerCommandSource arg2, List<String> list) {
      ...
      for (int i = 0; i < list.size(); i++) {
         ...
         if (stringReader.canRead() && stringReader.peek() != '#') {
            if (stringReader.peek() == '/') {
               ...
               if (stringReader.peek() == '/') {
                  throw new IllegalArgumentException(
                     "Unknown or invalid command '" + string + "' on line " + j + " (if you intended to make a comment, use '#' not '//')"
                  );
               }
            ...
            try {
               ParseResults<ServerCommandSource> parseResults = commandDispatcher.parse(stringReader, arg2);
      ...
      return new CommandFunction(id, list2.toArray(new CommandFunction.Element[0]));
   }
   ...
   public static class FunctionElement implements CommandFunction.Element {
      ...
      public FunctionElement(CommandFunction arg) {
         this.function = new CommandFunction.LazyContainer(arg);
      }
      ...
      public void execute(CommandFunctionManager manager, ServerCommandSource source, ArrayDeque<CommandFunctionManager.Entry> stack, int maxChainLength) {
         this.function.get(manager).ifPresent(arg3 -> {
            CommandFunction.Element[] lvs = arg3.getElements();
   ...
   public static class LazyContainer {
      ...
      @Nullable
      public Identifier getId() {
         return this.function.<Identifier>map(arg -> arg.id).orElse(this.id);
      }
   }
}
