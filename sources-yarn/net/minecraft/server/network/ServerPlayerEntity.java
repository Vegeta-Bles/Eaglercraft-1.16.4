...
public class ServerPlayerEntity extends PlayerEntity implements ScreenHandlerListener {
   ...
   public void writeCustomDataToTag(CompoundTag tag) {
      ...
      if (this.spawnPointPosition != null) {
         ...
         Identifier.CODEC
            ...
            .ifPresent(arg2 -> tag.put("SpawnDimension", arg2));
      }
   }
   ...
   public void onDeath(DamageSource source) {
      ...
      if (bl) {
         ...
         this.networkHandler
            .sendPacket(
               new CombatEventS2CPacket(this.getDamageTracker(), CombatEventS2CPacket.Type.ENTITY_DIED, lv),
               future -> {
                  if (!future.isSuccess()) {
                     int i = 256;
                     String string = lv.asTruncatedString(256);
                     Text lvx = new TranslatableText("death.attack.message_too_long", new LiteralText(string).formatted(Formatting.YELLOW));
                     Text lv2x = new TranslatableText("death.attack.even_more_magic", this.getDisplayName())
                        .styled(arg2 -> arg2.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, lv)));
                     this.networkHandler.sendPacket(new CombatEventS2CPacket(this.getDamageTracker(), CombatEventS2CPacket.Type.ENTITY_DIED, lv2x));
                  }
               }
            );
   ...
   private void forgiveMobAnger() {
      ...
      this.world
         .getEntitiesIncludingUngeneratedChunks(MobEntity.class, lv)
         .stream()
         ...
         if (!bl && this.joinInvulnerabilityTicks > 0 && source != DamageSource.OUT_OF_WORLD) {
            ...
            if (source instanceof EntityDamageSource) {
               ...
               if (lv instanceof PersistentProjectileEntity) {
                  ...
                  Entity lv3 = lv2.getOwner();
                  if (lv3 instanceof PlayerEntity && !this.shouldDamagePlayer((PlayerEntity)lv3)) {
                     return false;
                  }
               }
            }
   ...
   private void sendBlockEntityUpdate(BlockEntity blockEntity) {
      if (blockEntity != null) {
         BlockEntityUpdateS2CPacket lv = blockEntity.toUpdatePacket();
         if (lv != null) {
   ...
   public Either<PlayerEntity.SleepFailureReason, Unit> trySleep(BlockPos pos) {
      ...
      if (this.isSleeping() || !this.isAlive()) {
         ...
         if (this.world.isDay()) {
            ...
            Either<PlayerEntity.SleepFailureReason, Unit> either = super.trySleep(pos).ifRight(arg -> {
         ...
         .sendPacket(
            ...
            future -> {
               if (!future.isSuccess() && (type == MessageType.GAME_INFO || type == MessageType.SYSTEM)) {
                  ...
                  Text lv = new LiteralText(string).formatted(Formatting.YELLOW);
                  this.networkHandler
                     .sendPacket(
                        new GameMessageS2CPacket(
                           new TranslatableText("multiplayer.message_not_delivered", lv).formatted(Formatting.RED), MessageType.SYSTEM, senderUuid
                        )
                     );
               }
            }
