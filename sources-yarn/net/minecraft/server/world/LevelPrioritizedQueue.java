...
public class LevelPrioritizedQueue<T> {
   ...
   private final List<Long2ObjectLinkedOpenHashMap<List<Optional<T>>>> levelToPosToElements = IntStream.range(0, LEVEL_COUNT)
      .mapToObj(i -> new Long2ObjectLinkedOpenHashMap<List<Optional<T>>>())
   ...
   protected void updateLevel(int fromLevel, ChunkPos pos, int toLevel) {
      if (fromLevel < LEVEL_COUNT) {
         ...
         if (list != null && !list.isEmpty()) {
            this.levelToPosToElements.get(toLevel).computeIfAbsent(pos.toLong(), l -> Lists.<Optional<T>>newArrayList()).addAll(list);
   ...
   protected void add(Optional<T> element, long pos, int level) {
      this.levelToPosToElements.get(level).computeIfAbsent(pos, l -> Lists.<Optional<T>>newArrayList()).add(element);
   ...
   public Stream<Either<T, Runnable>> poll() {
      if (this.blockingChunks.size() >= this.maxBlocking) {
         ...
         return list.stream()
            .map(optional -> optional.map(value -> Either.<T, Runnable>left(value)).orElseGet(() -> Either.right(this.createBlockingAdder(l))));
      }
   }
