...
public class ServerChunkManager extends ChunkManager {
   ...
   public ServerChunkManager(
      ...
      this.world = arg;
      this.mainThreadExecutor = new ServerChunkManager.MainThreadExecutor(arg);
      ...
      this.threadedAnvilChunkStorage = new ThreadedAnvilChunkStorage(
         arg, arg2, dataFixer, structureManager, workerExecutor, this.mainThreadExecutor, this, this.getChunkGenerator(), arg5, supplier, viewDistance, bl
      );
   ...
   public Chunk getChunk(int x, int z, ChunkStatus leastStatus, boolean create) {
      if (Thread.currentThread() != this.serverThread) {
         ...
         Chunk lv3 = (Chunk)completableFuture.join().map(arg -> arg, arg -> {
            if (create) {
               throw (IllegalStateException)Util.throwOrPause(new IllegalStateException("Chunk not there when requested: " + arg));
            } else {
               return null;
            }
         });
   ...
   public WorldChunk getWorldChunk(int chunkX, int chunkZ) {
      if (Thread.currentThread() != this.serverThread) {
         ...
         if (lv2 == null) {
            ...
            Either<Chunk, ChunkHolder.Unloaded> either = lv2.getValidFutureFor(ChunkStatus.FULL).getNow(null);
            if (either == null) {
               return null;
            } else {
               Chunk lv3 = (Chunk)either.left().orElse(null);
               if (lv3 != null) {
   ...
   public CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> getChunkFutureSyncOnMainThread(int chunkX, int chunkZ, ChunkStatus leastStatus, boolean create) {
      ...
      if (bl2) {
         ...
         completableFuture = CompletableFuture.<CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>>>supplyAsync(
               () -> this.getChunkFuture(chunkX, chunkZ, leastStatus, create), this.mainThreadExecutor
            )
            .thenCompose(completableFuturex -> (CompletionStage<Either<Chunk, ChunkHolder.Unloaded>>)completableFuturex);
      }
   ...
   public BlockView getChunk(int chunkX, int chunkZ) {
      ...
      if (lv == null) {
         ...
         while (true) {
            ChunkStatus lv2 = CHUNK_STATUSES.get(k);
            Optional<Chunk> optional = lv.getFutureFor(lv2).getNow(ChunkHolder.UNLOADED_CHUNK).left();
   ...
   private void tickChunks() {
      ...
      if (!bl) {
         ...
         SpawnHelper.Info lv2 = SpawnHelper.setupSpawn(j, this.world.iterateEntities(), this::ifChunkLoaded);
         this.spawnEntry = lv2;
         this.world.getProfiler().pop();
         List<ChunkHolder> list = Lists.newArrayList(this.threadedAnvilChunkStorage.entryIterator());
         Collections.shuffle(list);
         list.forEach(arg2 -> {
            Optional<WorldChunk> optional = arg2.getTickingFuture().getNow(ChunkHolder.UNLOADED_WORLD_CHUNK).left();
            if (optional.isPresent()) {
               this.world.getProfiler().push("broadcast");
               arg2.flushUpdates(optional.get());
               this.world.getProfiler().pop();
               Optional<WorldChunk> optional2 = arg2.getEntityTickingFuture().getNow(ChunkHolder.UNLOADED_WORLD_CHUNK).left();
               if (optional2.isPresent()) {
                  WorldChunk lvx = optional2.get();
                  ChunkPos lv2x = arg2.getPos();
                  if (!this.threadedAnvilChunkStorage.isTooFarFromPlayersToSpawnMobs(lv2x)) {
                     lvx.setInhabitedTime(lvx.getInhabitedTime() + m);
                     if (bl2 && (this.spawnMonsters || this.spawnAnimals) && this.world.getWorldBorder().contains(lvx.getPos())) {
                        SpawnHelper.spawn(this.world, lvx, lv2, this.spawnAnimals, this.spawnMonsters, bl3);
                     }

                     this.world.tickChunk(lvx, i);
                  }
               }
            }
         });
   ...
   public String getChunkLoadingDebugInfo(ChunkPos arg) {
      return this.threadedAnvilChunkStorage.getChunkLoadingDebugInfo(arg);
   }
