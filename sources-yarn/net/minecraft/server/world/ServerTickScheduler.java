...
public class ServerTickScheduler<T> implements TickScheduler<T> {
   ...
   public void tick() {
      ...
      if (i != this.scheduledTickActions.size()) {
         ...
         while (i > 0 && iterator.hasNext()) {
            ...
            if (lv2.time > this.world.getTime()) {
               break;
            }

            if (lv.shouldTickBlock(lv2.pos)) {
               iterator.remove();
         ...
         while ((lv3 = this.currentTickActions.poll()) != null) {
            if (lv.shouldTickBlock(lv3.pos)) {
               try {
                  ...
                  CrashReport lv4 = CrashReport.create(var8, "Exception while ticking");
   ...
   private List<ScheduledTick<T>> transferTicksInBounds(@Nullable List<ScheduledTick<T>> dst, Collection<ScheduledTick<T>> src, BlockBox bounds, boolean move) {
      ...
      while (iterator.hasNext()) {
         ...
         BlockPos lv2 = lv.pos;
         if (lv2.getX() >= bounds.minX && lv2.getX() < bounds.maxX && lv2.getZ() >= bounds.minZ && lv2.getZ() < bounds.maxZ) {
            if (move) {
               iterator.remove();
            }
   ...
   public void copyScheduledTicks(BlockBox box, BlockPos offset) {
      for (ScheduledTick<T> lv : this.getScheduledTicks(box, false, false)) {
         if (box.contains(lv.pos)) {
            BlockPos lv2 = lv.pos.add(offset);
            T object = lv.getObject();
            this.addScheduledTick(new ScheduledTick<>(lv2, object, lv.time, lv.priority));
         }
      }
   }
