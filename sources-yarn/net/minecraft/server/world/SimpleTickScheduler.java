...
public class SimpleTickScheduler<T> implements TickScheduler<T> {
   ...
   public SimpleTickScheduler(Function<T, Identifier> identifierProvider, List<ScheduledTick<T>> scheduledTicks, long startTime) {
      this(
         ...
         scheduledTicks.stream()
            .map(arg -> new SimpleTickScheduler.Tick(arg.getObject(), arg.pos, (int)(arg.time - startTime), arg.priority))
   ...
   public ListTag toNbt() {
      ...
      for (SimpleTickScheduler.Tick<T> lv2 : this.scheduledTicks) {
         CompoundTag lv3 = new CompoundTag();
         lv3.putString("i", this.identifierProvider.apply(lv2.object).toString());
   ...
   public static <T> SimpleTickScheduler<T> fromNbt(ListTag ticks, Function<T, Identifier> function, Function<Identifier, T> function2) {
      ...
      for (int i = 0; i < ticks.size(); i++) {
         ...
         if (object != null) {
            ...
            list.add(new SimpleTickScheduler.Tick<>(object, lv2, lv.getInt("t"), TickPriority.byIndex(lv.getInt("p"))));
         }
      }
   ...
   public void scheduleTo(TickScheduler<T> scheduler) {
      this.scheduledTicks.forEach(arg2 -> scheduler.schedule(arg2.pos, arg2.object, arg2.delay, arg2.priority));
   }
