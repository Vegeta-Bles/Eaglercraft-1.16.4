...
public class OceanRuinGenerator {
   ...
   private static void method_14825(
      ...
      List<BlockPos> list2 = getRoomPositions(random, lv3.getX(), lv3.getZ());
      int k = MathHelper.nextInt(random, 4, 8);

      for (int l = 0; l < k; l++) {
         if (!list2.isEmpty()) {
            int m = random.nextInt(list2.size());
            BlockPos lv4 = list2.remove(m);
            int n = lv4.getX();
            int o = lv4.getZ();
            BlockRotation lv5 = BlockRotation.random(random);
            BlockPos lv6 = Structure.transformAround(new BlockPos(5, 0, 6), BlockMirror.NONE, lv5, BlockPos.ORIGIN).add(n, 0, o);
            BlockBox lv7 = BlockBox.create(n, 0, o, lv6.getX(), 0, lv6.getZ());
            if (!lv7.intersects(lv2)) {
               method_14822(manager, lv4, lv5, pieces, random, config, false, 0.8F);
            }
         }
      }
   }
   ...
   private static void method_14822(
      ...
      if (config.biomeType == OceanRuinFeature.BiomeType.WARM) {
         ...
         pieces.add(new OceanRuinGenerator.Piece(manager, lv, pos, rotation, integrity, config.biomeType, large));
         ...
         pieces.add(new OceanRuinGenerator.Piece(manager, lvs[i], pos, rotation, integrity, config.biomeType, large));
         pieces.add(new OceanRuinGenerator.Piece(manager, lvs2[i], pos, rotation, 0.7F, config.biomeType, large));
         pieces.add(new OceanRuinGenerator.Piece(manager, lvs3[i], pos, rotation, 0.5F, config.biomeType, large));
      }
   }
   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      protected void handleMetadata(String metadata, BlockPos pos, ServerWorldAccess arg2, Random random, BlockBox boundingBox) {
         if ("chest".equals(metadata)) {
            arg2.setBlockState(
               pos, Blocks.CHEST.getDefaultState().with(ChestBlock.WATERLOGGED, Boolean.valueOf(arg2.getFluidState(pos).isIn(FluidTags.WATER))), 2
            );
            ...
            DrownedEntity lv2 = EntityType.DROWNED.create(arg2.toServerWorld());
            lv2.setPersistent();
            ...
            arg2.spawnEntityAndPassengers(lv2);
      ...
      public boolean generate(
         ...
         BlockPos lv = Structure.transformAround(
      ...
      private int method_14829(BlockPos arg, BlockView arg2, BlockPos arg3) {
         ...
         for (BlockPos lv : BlockPos.iterate(arg, arg3)) {
            ...
            for (FluidState lv4 = arg2.getFluidState(lv2);
               (lv3.isAir() || lv4.isIn(FluidTags.WATER) || lv3.getBlock().isIn(BlockTags.ICE)) && o > 1;
