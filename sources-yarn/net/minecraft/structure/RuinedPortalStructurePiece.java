...
public class RuinedPortalStructurePiece extends SimpleStructurePiece {
   ...
   protected void toNbt(CompoundTag tag) {
      ...
      RuinedPortalStructurePiece.Properties.CODEC
         ...
         .ifPresent(arg2 -> tag.put("Properties", arg2));
   }

   private void processProperties(Structure structure, BlockPos center) {
      BlockIgnoreStructureProcessor lv = this.properties.airPocket
         ...
         if (this.properties.vines || this.properties.overgrown) {
            BlockPos.stream(this.getBoundingBox()).forEach(arg2 -> {
               if (this.properties.vines) {
                  this.generateVines(random, arg, arg2);
               }

               if (this.properties.overgrown) {
                  this.generateOvergrownLeaves(random, arg, arg2);
               }
            });
         }
      ...
      if (!lv.isAir() && !lv.isOf(Blocks.VINE)) {
         ...
         if (lv4.isAir()) {
            if (Block.isFaceFullSquare(lv.getCollisionShape(world, pos), lv2)) {
               ...
               world.setBlockState(lv3, Blocks.VINE.getDefaultState().with(lv5, Boolean.valueOf(true)), 3);
            }
         }
      }
   }
         ...
         for (int p = j - k; p <= j + k; p++) {
            ...
            if (r < k) {
               ...
               if (random.nextDouble() < (double)f) {
                  ...
                  lv2.set(o, t, p);
                  if (Math.abs(t - this.boundingBox.minY) <= 3 && this.canFillNetherrack(world, lv2)) {
   ...
   private boolean canFillNetherrack(WorldAccess world, BlockPos pos) {
      BlockState lv = world.getBlockState(pos);
      return !lv.isOf(Blocks.AIR)
         && !lv.isOf(Blocks.OBSIDIAN)
         && !lv.isOf(Blocks.CHEST)
         && (this.verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.IN_NETHER || !lv.isOf(Blocks.LAVA));
   }
   ...
   public static class Properties {
      public static final Codec<RuinedPortalStructurePiece.Properties> CODEC = RecordCodecBuilder.create(
         instance -> instance.group(
                  Codec.BOOL.fieldOf("cold").forGetter(arg -> arg.cold),
                  Codec.FLOAT.fieldOf("mossiness").forGetter(arg -> arg.mossiness),
                  Codec.BOOL.fieldOf("air_pocket").forGetter(arg -> arg.airPocket),
                  Codec.BOOL.fieldOf("overgrown").forGetter(arg -> arg.overgrown),
                  Codec.BOOL.fieldOf("vines").forGetter(arg -> arg.vines),
                  Codec.BOOL.fieldOf("replace_with_blackstone").forGetter(arg -> arg.replaceWithBlackstone)
               )
   ...
   public static enum VerticalPlacement {
      ...
      private static final Map<String, RuinedPortalStructurePiece.VerticalPlacement> VERTICAL_PLACEMENTS = Arrays.stream(values())
         .collect(Collectors.toMap(RuinedPortalStructurePiece.VerticalPlacement::getId, arg -> (RuinedPortalStructurePiece.VerticalPlacement)arg));
