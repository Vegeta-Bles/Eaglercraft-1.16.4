...
public abstract class StructureStart<C extends FeatureConfig> {
   ...
   public void generateStructure(
      ...
      synchronized (this.children) {
         if (!this.children.isEmpty()) {
            BlockBox lv = this.children.get(0).boundingBox;
            ...
            while (iterator.hasNext()) {
               ...
               if (lv4.getBoundingBox().intersects(box) && !lv4.generate(world, structureAccessor, chunkGenerator, random, box, chunkPos, lv3)) {
                  iterator.remove();
               }
            }
   ...
   protected void randomUpwardTranslation(int seaLevel, Random random, int minSeaLevelDistance) {
      int k = seaLevel - minSeaLevelDistance;
   ...
   protected void randomUpwardTranslation(Random random, int minY, int maxY) {
      int k = maxY - minY + 1 - this.boundingBox.getBlockCountY();
      int l;
      if (k > 1) {
         l = minY + random.nextInt(k);
      } else {
         l = minY;
      }
