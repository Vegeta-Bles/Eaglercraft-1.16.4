...
public class SwampHutGenerator extends StructurePieceWithDimensions {
   ...
   public boolean generate(
      ...
      if (!this.method_14839(arg, boundingBox, 0)) {
         ...
         BlockState lv = Blocks.SPRUCE_STAIRS.getDefaultState().with(StairsBlock.FACING, Direction.NORTH);
         BlockState lv2 = Blocks.SPRUCE_STAIRS.getDefaultState().with(StairsBlock.FACING, Direction.EAST);
         BlockState lv3 = Blocks.SPRUCE_STAIRS.getDefaultState().with(StairsBlock.FACING, Direction.WEST);
         BlockState lv4 = Blocks.SPRUCE_STAIRS.getDefaultState().with(StairsBlock.FACING, Direction.SOUTH);
         ...
         this.addBlock(arg, lv.with(StairsBlock.SHAPE, StairShape.OUTER_RIGHT), 0, 4, 1, boundingBox);
         this.addBlock(arg, lv.with(StairsBlock.SHAPE, StairShape.OUTER_LEFT), 6, 4, 1, boundingBox);
         this.addBlock(arg, lv4.with(StairsBlock.SHAPE, StairShape.OUTER_LEFT), 0, 4, 8, boundingBox);
         this.addBlock(arg, lv4.with(StairsBlock.SHAPE, StairShape.OUTER_RIGHT), 6, 4, 8, boundingBox);

         for (int i = 2; i <= 7; i += 5) {
            for (int j = 1; j <= 5; j += 4) {
               this.fillDownwards(arg, Blocks.OAK_LOG.getDefaultState(), j, -1, i, boundingBox);
            }
         }
         ...
         if (!this.hasWitch) {
            ...
            if (boundingBox.contains(new BlockPos(k, l, m))) {
               this.hasWitch = true;
               WitchEntity lv5 = EntityType.WITCH.create(arg.toServerWorld());
               lv5.setPersistent();
               lv5.refreshPositionAndAngles((double)k + 0.5, (double)l, (double)m + 0.5, 0.0F, 0.0F);
               lv5.initialize(arg, arg.getLocalDifficulty(new BlockPos(k, l, m)), SpawnReason.STRUCTURE, null, null);
               arg.spawnEntityAndPassengers(lv5);
            }
         }
   ...
   private void method_16181(ServerWorldAccess arg, BlockBox arg2) {
      if (!this.hasCat) {
         int i = this.applyXTransform(2, 5);
         int j = this.applyYTransform(2);
         int k = this.applyZTransform(2, 5);
         if (arg2.contains(new BlockPos(i, j, k))) {
            this.hasCat = true;
            CatEntity lv = EntityType.CAT.create(arg.toServerWorld());
            lv.setPersistent();
            lv.refreshPositionAndAngles((double)i + 0.5, (double)j, (double)k + 0.5, 0.0F, 0.0F);
