...
public class TestFunctions {
   ...
   public static Collection<TestFunction> getTestFunctions(String testClass) {
      return TEST_FUNCTIONS.stream().filter(arg -> isInClass(arg, testClass)).collect(Collectors.toList());
   }
   ...
   public static Optional<TestFunction> getTestFunction(String structurePath) {
      return getTestFunctions().stream().filter(arg -> arg.getStructurePath().equalsIgnoreCase(structurePath)).findFirst();
   }
