...
public class TestUtil {
   ...
   public static Collection<GameTest> runTestBatches(
      Collection<GameTestBatch> batches, BlockPos pos, BlockRotation arg2, ServerWorld arg3, TestManager arg4, int i
   ) {
   ...
   public static Collection<GameTest> runTestFunctions(
      Collection<TestFunction> testFunctions, BlockPos pos, BlockRotation arg2, ServerWorld arg3, TestManager arg4, int i
   ) {
      return runTestBatches(createBatches(testFunctions), pos, arg2, arg3, arg4, i);
   }
   ...
   public static Collection<GameTestBatch> createBatches(Collection<TestFunction> testFunctions) {
      ...
      testFunctions.forEach(arg -> {
         String string = arg.getBatchId();
         Collection<TestFunction> collection = map.computeIfAbsent(string, stringx -> Lists.newArrayList());
         collection.add(arg);
      });
      return map.keySet()
         .stream()
         .flatMap(
            string -> {
               Collection<TestFunction> collection = map.get(string);
   ...
   private static void handleTestFail(GameTest test) {
      ...
      if (throwable instanceof PositionedException) {
         ...
         addDebugMarker(test.getWorld(), lv.getPos(), lv.getDebugMessage());
      }
   ...
   private static void createLectern(GameTest test, String message) {
      ...
      ItemStack lv6 = createBook(test.getStructurePath(), test.isRequired(), message);
   ...
   public static void clearTests(ServerWorld world, BlockPos pos, TestManager testManager, int radius) {
      ...
      BlockPos.stream(lv, lv2).filter(arg2 -> world.getBlockState(arg2).isOf(Blocks.STRUCTURE_BLOCK)).forEach(arg2 -> {
         StructureBlockBlockEntity lvx = (StructureBlockBlockEntity)world.getBlockEntity(arg2);
         BlockPos lv2x = lvx.getPos();
         BlockBox lv3 = StructureTestUtil.method_29410(lvx);
         StructureTestUtil.clearArea(lv3, lv2x.getY(), world);
      });
   }
}
