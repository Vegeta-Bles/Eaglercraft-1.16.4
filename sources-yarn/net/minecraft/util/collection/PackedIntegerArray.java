...
public class PackedIntegerArray {
   ...
   public PackedIntegerArray(int elementBits, int size, @Nullable long[] storage) {
      ...
      if (storage != null) {
         if (storage.length != l) {
            throw (RuntimeException)Util.throwOrPause(new RuntimeException("Invalid length given for storage, got: " + storage.length + " but expected: " + l));
         }
   ...
   public int setAndGetOldValue(int index, int value) {
      ...
      this.storage[k] = l & ~(this.maxValue << m) | ((long)value & this.maxValue) << m;
      return n;
   }
