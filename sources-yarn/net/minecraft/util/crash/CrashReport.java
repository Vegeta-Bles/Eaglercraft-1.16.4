...
public class CrashReport {
   ...
   public void addStackTrace(StringBuilder stringBuilder) {
      ...
      for (CrashReportSection lv : this.otherSections) {
         lv.addStackTrace(stringBuilder);
         stringBuilder.append("\n\n");
      }
   ...
   public CrashReportSection addElement(String name, int ignoredStackTraceCallCount) {
      ...
      if (this.hasStackTrace) {
         ...
         this.hasStackTrace = lv.method_584(stackTraceElement, stackTraceElement2);
         if (j > 0 && !this.otherSections.isEmpty()) {
            CrashReportSection lv2 = this.otherSections.get(this.otherSections.size() - 1);
            lv2.trimStackTraceEnd(j);
