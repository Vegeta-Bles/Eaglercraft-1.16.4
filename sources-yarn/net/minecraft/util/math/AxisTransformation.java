...
public enum AxisTransformation {
   ...
   private static final AxisTransformation[][] COMBINATIONS = Util.make(new AxisTransformation[values().length][values().length], args -> {
      for (AxisTransformation lv : values()) {
         for (AxisTransformation lv2 : values()) {
            int[] is = new int[3];
            ...
            AxisTransformation lv3 = Arrays.stream(values()).filter(arg -> Arrays.equals(arg.mappings, is)).findFirst().get();
