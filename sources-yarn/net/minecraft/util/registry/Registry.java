...
public abstract class Registry<T> implements Codec<T>, Keyable, IndexedIterable<T> {
   ...
   public static <T extends MutableRegistry<?>> void validate(MutableRegistry<T> registry) {
      registry.forEach(arg2 -> {
         if (arg2.getIds().isEmpty()) {
            LOGGER.error("Registry '{}' was empty after loading", registry.getId((T)arg2));
            if (SharedConstants.isDevelopment) {
               throw new IllegalStateException("Registry: '" + registry.getId((T)arg2) + "' is empty, not allowed, fix me!");
            }
         }

         if (arg2 instanceof DefaultedRegistry) {
            Identifier lv = ((DefaultedRegistry)arg2).getDefaultId();
            Validate.notNull(arg2.get(lv), "Missing default of DefaultedMappedRegistry: " + lv, new Object[0]);
         }
      });
   }

   private static <T> Registry<T> create(RegistryKey<? extends Registry<T>> arg, Supplier<T> defaultEntry) {
      return create(arg, Lifecycle.experimental(), defaultEntry);
   }

   private static <T> DefaultedRegistry<T> create(RegistryKey<? extends Registry<T>> arg, String defaultId, Supplier<T> defaultEntry) {
      return create(arg, defaultId, Lifecycle.experimental(), defaultEntry);
   }

   private static <T> Registry<T> create(RegistryKey<? extends Registry<T>> arg, Lifecycle lifecycle, Supplier<T> defaultEntry) {
      return create(arg, new SimpleRegistry<>(arg, lifecycle), defaultEntry, lifecycle);
   }
   ...
   private static <T> DefaultedRegistry<T> create(RegistryKey<? extends Registry<T>> arg, String defaultId, Lifecycle lifecycle, Supplier<T> defaultEntry) {
      return create(arg, new DefaultedRegistry<>(defaultId, arg, lifecycle), defaultEntry, lifecycle);
   }

   private static <T, R extends MutableRegistry<T>> R create(RegistryKey<? extends Registry<T>> arg, R registry, Supplier<T> defaultEntry, Lifecycle lifecycle) {
      Identifier lv = arg.getValue();
      DEFAULT_ENTRIES.put(lv, defaultEntry);
      @SuppressWarnings("unchecked")
      MutableRegistry<R> root = (MutableRegistry<R>)(MutableRegistry<?>)ROOT;
      return root.add((RegistryKey<R>)arg, registry, lifecycle);
   }
   ...
   public <U> DataResult<U> encode(T object, DynamicOps<U> dynamicOps, U object2) {
      ...
      if (lv == null) {
         ...
         return dynamicOps.compressMaps()
            ...
            : dynamicOps.mergeToPrimitive(object2, dynamicOps.createString(lv.toString())).setLifecycle(this.lifecycle);
      }
   }

   public <U> Stream<U> keys(DynamicOps<U> dynamicOps) {
      return this.getIds().stream().map(arg -> (U)dynamicOps.createString(arg.toString()));
   }
   ...
   public static <V, T extends V> T register(Registry<V> registry, Identifier id, T entry) {
      return asMutable(registry).add(RegistryKey.of(registry.registryKey, id), entry, Lifecycle.stable());
   }

   public static <V, T extends V> T register(Registry<V> registry, int rawId, String id, T entry) {
      return asMutable(registry).set(rawId, RegistryKey.of(registry.registryKey, new Identifier(id)), entry, Lifecycle.stable());
   }

   private static <V> MutableRegistry<V> asMutable(Registry<V> registry) {
      if (registry instanceof MutableRegistry) {
         return (MutableRegistry<V>)registry;
      } else {
         throw new IllegalStateException("Registry is not mutable: " + registry);
      }
   }

   static {
      BuiltinRegistries.init();
      DEFAULT_ENTRIES.forEach((arg, supplier) -> {
