...
public abstract class VoxelSet {
   ...
   @Environment(EnvType.CLIENT)
   public void forEachEdge(VoxelSet.PositionBiConsumer arg, boolean bl) {
   ...
   @Environment(EnvType.CLIENT)
   private void forEachEdge(VoxelSet.PositionBiConsumer arg, AxisCycleDirection direction, boolean bl) {
            ...
            for (int o = 0; o <= k; o++) {
               ...
               if (p == 1 || p == 3 || p == 2 && (q & 1) == 0) {
                  if (bl) {
                     ...
                     arg.consume(
                        lv.choose(l, m, o, Direction.Axis.X),
                        lv.choose(l, m, o, Direction.Axis.Y),
                        lv.choose(l, m, o, Direction.Axis.Z),
                        lv.choose(l, m, o + 1, Direction.Axis.X),
                        lv.choose(l, m, o + 1, Direction.Axis.Y),
                        lv.choose(l, m, o + 1, Direction.Axis.Z)
                     );
                  }
               } else if (n != -1) {
                  arg.consume(
                     lv.choose(l, m, n, Direction.Axis.X),
                     lv.choose(l, m, n, Direction.Axis.Y),
                     lv.choose(l, m, n, Direction.Axis.Z),
                     lv.choose(l, m, o, Direction.Axis.X),
                     lv.choose(l, m, o, Direction.Axis.Y),
                     lv.choose(l, m, o, Direction.Axis.Z)
                  );
   ...
   protected boolean isColumnFull(int minZ, int maxZ, int x, int y) {
      for (int m = minZ; m < maxZ; m++) {
         if (!this.inBoundsAndContains(x, y, m)) {
            return false;
         }
      }
   ...
   protected boolean isRectangleFull(int minX, int maxX, int minZ, int maxZ, int y) {
      for (int n = minX; n < maxX; n++) {
         if (!this.isColumnFull(minZ, maxZ, n, y)) {
            return false;
         }
      }
   ...
   public void forEachBox(VoxelSet.PositionBiConsumer consumer, boolean largest) {
      VoxelSet lv = new BitSetVoxelSet(this);
         ...
         for (int j = 0; j <= this.ySize; j++) {
            ...
            for (int l = 0; l <= this.zSize; l++) {
               if (lv.inBoundsAndContains(i, j, l)) {
                  ...
                  int o = j;
                  int p = j;
                  lv.setColumn(k, l, i, j, false);

                  while (lv.isColumnFull(k, l, m - 1, o)) {
                     lv.setColumn(k, l, m - 1, o, false);
                     m--;
                  }

                  while (lv.isColumnFull(k, l, n + 1, o)) {
                     lv.setColumn(k, l, n + 1, o, false);
                     n++;
                  }

                  while (lv.isRectangleFull(m, n + 1, k, l, o - 1)) {
                     for (int q = m; q <= n; q++) {
                        lv.setColumn(k, l, q, o - 1, false);
                     }
   ...
   public void forEachDirection(VoxelSet.PositionConsumer arg) {
   ...
   private void forEachDirection(VoxelSet.PositionConsumer arg, AxisCycleDirection direction) {
      AxisCycleDirection lv = direction.opposite();
