...
public abstract class VoxelShape {
   ...
   public VoxelShape simplify() {
      ...
      this.forEachBox((d, e, f, g, h, i) -> lvs[0] = VoxelShapes.combine(lvs[0], VoxelShapes.cuboid(d, e, f, g, h, i), BooleanBiFunction.OR));
      return lvs[0];
   }
   ...
   public void forEachBox(VoxelShapes.BoxConsumer arg) {
      ...
      this.voxels
         .forEachBox(
            (i, j, k, l, m, n) -> arg.consume(
                  doubleList.getDouble(i),
                  doubleList2.getDouble(j),
                  doubleList3.getDouble(k),
                  doubleList.getDouble(l),
                  doubleList2.getDouble(m),
                  doubleList3.getDouble(n)
               ),
            true
         );
   }
   ...
   public double getEndingCoord(Direction.Axis arg, double from, double to) {
      ...
      int k = this.voxels.getEndingAxisCoord(arg, i, j);
      return k <= 0 ? Double.NEGATIVE_INFINITY : this.getPointPosition(arg, k);
   }
   ...
   protected int getCoordIndex(Direction.Axis arg, double coord) {
      return MathHelper.binarySearch(0, this.voxels.getSize(arg) + 1, i -> {
         if (i < 0) {
            ...
            return i > this.voxels.getSize(arg) ? true : coord < this.getPointPosition(arg, i);
         }
      }) - 1;
   }
   ...
   public BlockHitResult raycast(Vec3d start, Vec3d end, BlockPos pos) {
      if (this.isEmpty()) {
         ...
         if (lv.lengthSquared() < 1.0E-7) {
            ...
            return this.contains(lv2.x - (double)pos.getX(), lv2.y - (double)pos.getY(), lv2.z - (double)pos.getZ())
               ? new BlockHitResult(lv2, Direction.getFacing(lv.x, lv.y, lv.z).getOpposite(), pos, true)
   ...
   public VoxelShape getFace(Direction facing) {
      if (!this.isEmpty() && this != VoxelShapes.fullCube()) {
         ...
         VoxelShape lv2 = this.getUncachedFace(facing);
         this.shapeCache[facing.ordinal()] = lv2;
         return lv2;
      } else {
         return this;
      }
      ...
      if (this.isEmpty()) {
         ...
         if (maxDist > 0.0) {
            ...
            for (int s = i - 1; s >= 0; s--) {
               for (int t = k; t < l; t++) {
                  for (int u = m; u < n; u++) {
                     if (this.voxels.inBoundsAndContains(lv, s, t, u)) {
