...
public class TradeOffers {
   ...
   static class SellPotionHoldingItemFactory implements TradeOffers.Factory {
      ...
      public TradeOffer create(Entity entity, Random random) {
         ItemStack lv = new ItemStack(Items.EMERALD, this.price);
         List<Potion> list = Registry.POTION
            .stream()
            .filter(arg -> !arg.getEffects().isEmpty() && BrewingRecipeRegistry.isBrewable(arg))
            .collect(Collectors.toList());
         Potion lv2 = list.get(random.nextInt(list.size()));
   ...
   static class TypeAwareBuyForOneEmeraldFactory implements TradeOffers.Factory {
      ...
      public TypeAwareBuyForOneEmeraldFactory(int count, int maxUses, int experience, Map<VillagerType, Item> map) {
         Registry.VILLAGER_TYPE.stream().filter(arg -> !map.containsKey(arg)).findAny().ifPresent(arg -> {
            throw new IllegalStateException("Missing trade for villager type: " + Registry.VILLAGER_TYPE.getId(arg));
         });
         this.map = map;
         this.count = count;
         this.maxUses = maxUses;
      ...
      public TradeOffer create(Entity entity, Random random) {
         if (entity instanceof VillagerDataContainer) {
            ItemStack lv = new ItemStack(this.map.get(((VillagerDataContainer)entity).getVillagerData().getType()), this.count);
