   ...
   private boolean spawn(ServerWorld world) {
      for (PlayerEntity lv : world.getPlayers()) {
         if (!lv.isSpectator()) {
            ...
            if (world.isNearOccupiedPointOfInterest(lv2) && world.getBiome(lv2).getCategory() != Biome.Category.MUSHROOM) {
               for (int i = 0; i < 10; i++) {
   ...
   private void trySpawnZombie(ServerWorld world) {
      ...
      if (lv != null) {
         ...
         try {
            ...
            field_26390.warn("Failed to create zombie for village siege at {}", lv, var5);
            return;
         }
