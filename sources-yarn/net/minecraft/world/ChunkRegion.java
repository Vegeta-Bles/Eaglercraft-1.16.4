      ...
      if (!create) {
         ...
         if (lv != null) {
            throw (RuntimeException)Util.throwOrPause(
               new RuntimeException(String.format("Chunk is not of correct status. Expecting %s, got %s | %s %s", leastStatus, lv.getStatus(), chunkX, chunkZ))
            );
         } else {
            throw (RuntimeException)Util.throwOrPause(
      ...
      if (lv2 != null) {
         ...
         if (lv3 != null) {
            if ("DUMMY".equals(lv3.getString("id"))) {
               ...
               lv2 = ((BlockEntityProvider)lv5).createBlockEntity(this.world);
            } else {
   ...
   public boolean setBlockState(BlockPos pos, BlockState state, int flags, int maxUpdateDepth) {
      ...
      if (lv3.hasBlockEntity()) {
         if (lv.getStatus().getChunkType() == ChunkStatus.ChunkType.field_12807) {
            lv.setBlockEntity(pos, ((BlockEntityProvider)lv3).createBlockEntity(this));
         } else {
