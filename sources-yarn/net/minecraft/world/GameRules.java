...
public class GameRules {
   ...
   private static <T extends GameRules.Rule<T>> GameRules.Key<T> register(String name, GameRules.Category category, GameRules.Type<T> type) {
      GameRules.Key<T> lv = new GameRules.Key<>(name, category);
      GameRules.Type<?> lv2 = RULE_TYPES.put(lv, type);
      if (lv2 != null) {
   ...
   public GameRules copy() {
      return new GameRules(
         this.rules.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, entry -> ((GameRules.Rule)entry.getValue()).copy()))
      );
   }
   ...
   private static <T extends GameRules.Rule<T>> void accept(GameRules.Visitor consumer, GameRules.Key<?> key, GameRules.Type<?> type) {
      GameRules.Key<T> castKey = (GameRules.Key<T>)key;
      GameRules.Type<T> castType = (GameRules.Type<T>)type;
      consumer.visit(castKey, castType);
      castType.accept(consumer, castKey);
   }
   ...
   private <T extends GameRules.Rule<T>> void setValue(GameRules.Key<T> key, GameRules rules, @Nullable MinecraftServer server) {
      T lv = rules.get(key);
      this.<T>get(key).setValue(lv, server);
   }
   ...
   public static class BooleanRule extends GameRules.Rule<GameRules.BooleanRule> {
      ...
      @Environment(EnvType.CLIENT)
      public void setValue(GameRules.BooleanRule arg, @Nullable MinecraftServer minecraftServer) {
   ...
   public static class IntRule extends GameRules.Rule<GameRules.IntRule> {
      ...
      @Environment(EnvType.CLIENT)
      public void setValue(GameRules.IntRule arg, @Nullable MinecraftServer minecraftServer) {
         this.value = arg.value;
         this.changed(minecraftServer);
      }
   }
