...
public abstract class MobSpawnerLogic {
   ...
   private Identifier getEntityId() {
      ...
      try {
         ...
         LOGGER.warn("Invalid entity id '{}' at spawner {}:[{},{},{}]", string, this.getWorld().getRegistryKey().getValue(), lv2.getX(), lv2.getY(), lv2.getZ());
         return null;
      }
   }
   ...
   private boolean isPlayerInRange() {
      ...
      return this.getWorld().isPlayerInRange((double)lv.getX() + 0.5, (double)lv.getY() + 0.5, (double)lv.getZ() + 0.5, (double)this.requiredPlayerRange);
   }
      ...
      if (!this.isPlayerInRange()) {
         ...
         if (!(lv instanceof ServerWorld)) {
            ...
            for (int i = 0; i < this.spawnCount; i++) {
               ...
               double k = j >= 3 ? lv4.getDouble(2) : (double)lv2.getZ() + (lv.random.nextDouble() - lv.random.nextDouble()) * (double)this.spawnRange + 0.5;
               if (lv.isSpaceEmpty(optional.get().createSimpleBoundingBox(g, h, k))) {
                  ServerWorld lv5 = (ServerWorld)lv;
                  if (SpawnRestriction.canSpawn(optional.get(), lv5, SpawnReason.SPAWNER, new BlockPos(g, h, k), lv.getRandom())) {
                     ...
                     int l = lv.getNonSpectatingEntities(
                           lv6.getClass(),
                           new Box(
                                 (double)lv2.getX(),
                                 (double)lv2.getY(),
                     ...
                     if (lv6 instanceof MobEntity) {
                        ...
                        if (!lv7.canSpawn(lv, SpawnReason.SPAWNER) || !lv7.canSpawn(lv)) {
                           continue;
                        }
                     ...
                     if (!lv5.shouldCreateNewEntityWithPassenger(lv6)) {
                        this.updateSpawns();
                        return;
                     }
