...
public class PortalForcer {
   ...
   public Optional<class_5459.class_5460> method_30483(BlockPos arg, boolean bl) {
      PointOfInterestStorage lv = this.world.getPointOfInterestStorage();
      int i = bl ? 16 : 128;
      lv.preloadChunks(this.world, arg, i);
      Optional<PointOfInterest> optional = lv.getInSquare(
            argx -> argx == PointOfInterestType.NETHER_PORTAL, arg, i, PointOfInterestStorage.OccupationStatus.ANY
         )
         .sorted(Comparator.<PointOfInterest>comparingDouble(arg2 -> arg2.getPos().getSquaredDistance(arg)).thenComparingInt(argx -> argx.getPos().getY()))
         .filter(argx -> this.world.getBlockState(argx.getPos()).contains(Properties.HORIZONTAL_AXIS))
         .findFirst();
      return optional.map(argx -> {
         BlockPos lvx = argx.getPos();
         this.world.getChunkManager().addTicket(ChunkTicketType.PORTAL, new ChunkPos(lvx), 3, lvx);
         BlockState lv2 = this.world.getBlockState(lvx);
         return class_5459.method_30574(lvx, lv2.get(Properties.HORIZONTAL_AXIS), 21, Direction.Axis.Y, 21, arg2 -> this.world.getBlockState(arg2) == lv2);
      });
   }
   ...
   public Optional<class_5459.class_5460> method_30482(BlockPos arg, Direction.Axis arg2) {
      ...
      for (BlockPos.Mutable lv6 : BlockPos.method_30512(arg, 16, Direction.EAST, Direction.SOUTH)) {
         ...
         int k = 1;
         if (lv4.contains(lv6) && lv4.contains(lv6.move(lv, 1))) {
            lv6.move(lv.getOpposite(), 1);

            for (int l = j; l >= 0; l--) {
               lv6.setY(l);
               if (this.world.isAir(lv6)) {
                  int m = l;
                  ...
                  if (l + 4 <= i) {
                     ...
                     if (n <= 0 || n >= 3) {
                        lv6.setY(l);
                        if (this.method_30481(lv6, lv5, lv, 0)) {
      ...
      BlockState lv9 = Blocks.NETHER_PORTAL.getDefaultState().with(NetherPortalBlock.AXIS, arg2);
