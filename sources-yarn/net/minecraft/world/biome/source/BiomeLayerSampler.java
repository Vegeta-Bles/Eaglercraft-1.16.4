...
public class BiomeLayerSampler {
   ...
   public Biome sample(Registry<Biome> arg, int i, int j) {
      int k = this.sampler.sample(i, j);
      RegistryKey<Biome> lv = BuiltinBiomes.fromRawId(k);
      if (lv == null) {
         throw new IllegalStateException("Unknown biome id emitted by layers: " + k);
      } else {
         Biome lv2 = arg.get(lv);
         if (lv2 == null) {
            if (SharedConstants.isDevelopment) {
               ...
               return arg.get(BuiltinBiomes.fromRawId(0));
            }
         } else {
            return lv2;
         }
