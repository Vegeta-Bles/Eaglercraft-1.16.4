...
public class VanillaLayeredBiomeSource extends BiomeSource {
   public static final Codec<VanillaLayeredBiomeSource> CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
               ...
               Codec.BOOL.optionalFieldOf("legacy_biome_init_layer", false, Lifecycle.stable()).forGetter(arg -> arg.legacyBiomeInitLayer),
               Codec.BOOL.fieldOf("large_biomes").orElse(false).stable().forGetter(arg -> arg.largeBiomes),
               RegistryLookupCodec.of(Registry.BIOME_KEY).forGetter(arg -> arg.biomeRegistry)
            )
   ...
   public VanillaLayeredBiomeSource(long seed, boolean legacyBiomeInitLayer, boolean largeBiomes, Registry<Biome> biomeRegistry) {
      super(BIOMES.stream().map(arg2 -> () -> biomeRegistry.getOrThrow((RegistryKey<Biome>)arg2)));
      this.seed = seed;
