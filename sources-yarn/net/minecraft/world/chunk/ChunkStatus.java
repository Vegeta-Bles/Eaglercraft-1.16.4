...
public class ChunkStatus {
   ...
   public static final ChunkStatus NOISE = register(
      "noise", BIOMES, 8, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.field_12808, (world, generator, surroundingChunks, chunk) -> {
         ChunkRegion lv = new ChunkRegion(world, surroundingChunks);
         generator.populateNoise(lv, world.getStructureAccessor().forRegion(lv), chunk);
      }
   );
   ...
   public static final ChunkStatus FEATURES = register(
      ...
      (status, world, generator, structureManager, lightingProvider, function, surroundingChunks, chunk) -> {
         ...
         if (!chunk.getStatus().isAtLeast(status)) {
            ...
            generator.generateFeatures(lv2, world.getStructureAccessor().forRegion(lv2));
            lv.setStatus(status);
         }
   ...
   public static final ChunkStatus FULL = register(
      ...
      (targetStatus, world, generator, structureManager, lightingProvider, function, surroundingChunks, chunk) -> function.apply(chunk),
      (status, world, structureManager, lightingProvider, function, chunk) -> function.apply(chunk)
   );
