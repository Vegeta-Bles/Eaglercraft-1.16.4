...
public class ProtoChunk implements Chunk {
   ...
   public BlockState getBlockState(BlockPos pos) {
      ...
      if (World.isOutOfBuildLimitVertically(i)) {
         ...
         return ChunkSection.isEmpty(lv) ? Blocks.AIR.getDefaultState() : lv.getBlockState(pos.getX() & 15, i & 15, pos.getZ() & 15);
      }
   }
   ...
   public FluidState getFluidState(BlockPos pos) {
      ...
      if (World.isOutOfBuildLimitVertically(i)) {
         ...
         ChunkSection lv = this.getSectionArray()[i >> 4];
         return ChunkSection.isEmpty(lv) ? Fluids.EMPTY.getDefaultState() : lv.getFluidState(pos.getX() & 15, i & 15, pos.getZ() & 15);
      }
   }
   ...
   public ShortList[] getLightSourcesBySection() {
      ...
      for (BlockPos lv : this.lightSources) {
         Chunk.getList(shortLists, lv.getY() >> 4).add(getPackedSectionRelative(lv));
      }
      ...
      if (j >= 0 && j < 256) {
         if (this.sections[j >> 4] == WorldChunk.EMPTY_SECTION && state.isOf(Blocks.AIR)) {
            ...
            if (this.status.isAtLeast(ChunkStatus.FEATURES)
               && state != lv2
               && (
                  state.getOpacity(this, pos) != lv2.getOpacity(this, pos)
                     || state.getLuminance() != lv2.getLuminance()
            ...
            for (Heightmap.Type lv6 : enumSet) {
               this.heightmaps.get(lv6).trackUpdate(i & 15, j, k & 15, state);
            }
   ...
   public static BlockPos joinBlockPos(short sectionRel, int sectionY, ChunkPos chunkPos) {
      int j = (sectionRel & 15) + (chunkPos.x << 4);
      int k = (sectionRel >>> 4 & 15) + (sectionY << 4);
      int l = (sectionRel >>> 8 & 15) + (chunkPos.z << 4);
   ...
   public CompoundTag getPackedBlockEntityTag(BlockPos pos) {
      ...
      return lv != null ? lv.toTag(new CompoundTag()) : this.blockEntityTags.get(pos);
   }
