...
public abstract class LevelPropagator {
   ...
   public void removePendingUpdateIf(LongPredicate predicate) {
      ...
      LongIterator iterator = this.pendingUpdates.keySet().iterator();
      ...
      LongIterator removeIterator = longList.iterator();

      while (removeIterator.hasNext()) {
         this.removePendingUpdate(removeIterator.nextLong());
      }
   }
   ...
   private void updateLevel(long sourceId, long id, int level, int currentLevel, int pendingLevel, boolean decrease) {
      if (!this.isMarker(id)) {
         ...
         if (currentLevel != n) {
            int q = this.minLevel(currentLevel, n);
            if (p != q && !bl2) {
               this.removePendingUpdate(id, p, q, false);
            }

            this.addPendingUpdate(id, n, q);
         } else if (!bl2) {
