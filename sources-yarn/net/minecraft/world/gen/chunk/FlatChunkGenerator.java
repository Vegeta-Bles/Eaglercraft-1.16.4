...
public class FlatChunkGenerator extends ChunkGenerator {
   ...
   public int getSpawnHeight() {
      BlockState[] lvs = this.config.getLayerBlocks();

      for (int i = 0; i < lvs.length; i++) {
         BlockState lv = lvs[i] == null ? Blocks.AIR.getDefaultState() : lvs[i];
         if (!Heightmap.Type.MOTION_BLOCKING.getBlockPredicate().test(lv)) {
            return i - 1;
         }
      }
   ...
   public int getHeight(int x, int z, Heightmap.Type heightmapType) {
      ...
      for (int k = lvs.length - 1; k >= 0; k--) {
         BlockState lv = lvs[k];
         if (lv != null && heightmapType.getBlockPredicate().test(lv)) {
            return k + 1;
         }
      }
