...
public class EndSpikeFeatureConfig implements FeatureConfig {
   public static final Codec<EndSpikeFeatureConfig> CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
               Codec.BOOL.fieldOf("crystal_invulnerable").orElse(false).forGetter(arg -> arg.crystalInvulnerable),
               EndSpikeFeature.Spike.CODEC.listOf().fieldOf("spikes").forGetter(arg -> arg.spikes),
               BlockPos.CODEC.optionalFieldOf("crystal_beam_target").forGetter(arg -> Optional.ofNullable(arg.crystalBeamTarget))
            )
