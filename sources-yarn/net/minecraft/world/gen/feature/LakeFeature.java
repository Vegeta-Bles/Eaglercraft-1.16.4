...
public class LakeFeature extends Feature<SingleStateFeatureConfig> {
   ...
   public boolean generate(StructureWorldAccess arg, ChunkGenerator arg2, Random random, BlockPos arg3, SingleStateFeatureConfig arg4) {
      ...
      if (arg3.getY() <= 4) {
         ...
         arg3 = arg3.down(4);
         if (arg.getStructures(ChunkSectionPos.from(arg3), StructureFeature.VILLAGE).findAny().isPresent()) {
            return false;
         } else {
               ...
               for (int t = 0; t < 16; t++) {
                  for (int u = 0; u < 8; u++) {
                     ...
                     if (bl) {
                        ...
                        if (u < 4 && !lv.isSolid() && arg.getBlockState(arg3.add(s, u, t)) != arg4.state) {
                           return false;
                        }
                     }
                  }
            ...
            for (int y = 0; y < 16; y++) {
               for (int z = 0; z < 16; z++) {
                  for (int aa = 4; aa < 8; aa++) {
                     if (bls[(y * 16 + z) * 8 + aa]) {
                        BlockPos lv2 = arg3.add(y, aa - 1, z);
                        if (isSoil(arg.getBlockState(lv2).getBlock()) && arg.getLightLevel(LightType.SKY, arg3.add(y, aa, z)) > 0) {
                           Biome lv3 = arg.getBiome(lv2);
                           if (lv3.getGenerationSettings().getSurfaceConfig().getTopMaterial().isOf(Blocks.MYCELIUM)) {
            ...
            if (arg4.state.getMaterial() == Material.WATER) {
               for (int ae = 0; ae < 16; ae++) {
                  for (int af = 0; af < 16; af++) {
                     ...
                     if (arg.getBiome(lv4).canSetIce(arg, lv4, false)) {
