...
public class DarkOakFoliagePlacer extends FoliagePlacer {
   ...
   public DarkOakFoliagePlacer(UniformIntDistribution arg, UniformIntDistribution arg2) {
      super(arg, arg2);
   }
   ...
   protected void generate(
      ...
      if (bl) {
         this.generateSquare(world, random, config, lv, radius + 2, leaves, -1, bl, box);
         this.generateSquare(world, random, config, lv, radius + 3, leaves, 0, bl, box);
         this.generateSquare(world, random, config, lv, radius + 2, leaves, 1, bl, box);
         if (random.nextBoolean()) {
            this.generateSquare(world, random, config, lv, radius, leaves, 2, bl, box);
         }
      } else {
         this.generateSquare(world, random, config, lv, radius + 2, leaves, -1, bl, box);
         this.generateSquare(world, random, config, lv, radius + 1, leaves, 0, bl, box);
      }
   }
