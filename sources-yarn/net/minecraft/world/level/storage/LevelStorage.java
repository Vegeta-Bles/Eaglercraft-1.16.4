...
public class LevelStorage {
   ...
   private static <T> Pair<GeneratorOptions, Lifecycle> readGeneratorProperties(Dynamic<T> dynamic, DataFixer dataFixer, int i) {
      ...
      UnmodifiableIterator<String> iterator = GENERATOR_OPTION_KEYS.iterator();
      ...
      Dynamic<T> updatedSettings = dataFixer.update(TypeReferences.CHUNK_GENERATOR_SETTINGS, dynamic2, i, SharedConstants.getGameVersion().getWorldVersion());
      DataResult<GeneratorOptions> dataResult = GeneratorOptions.CODEC.parse(updatedSettings);
      return Pair.of(
         dataResult.resultOrPartial(Util.method_29188("WorldGenSettings: ", LOGGER::error))
            .orElseGet(
               () -> {
                  Registry<DimensionType> lv = (Registry<DimensionType>)RegistryLookupCodec.of(Registry.DIMENSION_TYPE_KEY)
                     .codec()
                     .parse(dynamic)
                  ...
                  Registry<Biome> lv2 = (Registry<Biome>)RegistryLookupCodec.of(Registry.BIOME_KEY)
                     .codec()
                     .parse(dynamic)
                  ...
                  Registry<ChunkGeneratorSettings> lv3 = (Registry<ChunkGeneratorSettings>)RegistryLookupCodec.of(Registry.NOISE_SETTINGS_WORLDGEN)
                     .codec()
                     .parse(dynamic)
                  ...
                  return GeneratorOptions.getDefaultOptions(lv, lv2, lv3);
               }
            ),
      ...
      if (!Files.isDirectory(this.savesDirectory)) {
         ...
         for (File file : files) {
            if (file.isDirectory()) {
               ...
               LevelSummary lv = this.readLevelProperties(file, this.createLevelDataParser(file, bl));
               if (lv != null) {
                  list.add(lv);
               }
            }
   ...
   private static DataPackSettings readDataPackSettings(File file, DataFixer dataFixer) {
      try {
         ...
         Dynamic<Tag> dynamic = dataFixer.update(
            DataFixTypes.LEVEL.getTypeReference(), new Dynamic(NbtOps.INSTANCE, lv2), i, SharedConstants.getGameVersion().getWorldVersion()
         );
   ...
   private static BiFunction<File, DataFixer, LevelProperties> createLevelDataParser(DynamicOps<Tag> dynamicOps, DataPackSettings arg) {
      return (file, dataFixer) -> {
         try {
            ...
            int i = lv2.contains("DataVersion", 99) ? lv2.getInt("DataVersion") : -1;
            Dynamic<Tag> dynamic = dataFixer.update(
               DataFixTypes.LEVEL.getTypeReference(), new Dynamic(dynamicOps, lv2), i, SharedConstants.getGameVersion().getWorldVersion()
            );
            ...
            LevelInfo lv5 = LevelInfo.fromDynamic(dynamic, arg);
            return LevelProperties.readProperties(dynamic, dataFixer, i, lv3, lv5, lv4, (GeneratorOptions)pair.getFirst(), (Lifecycle)pair.getSecond());
   ...
   private BiFunction<File, DataFixer, LevelSummary> createLevelDataParser(File file, boolean locked) {
      return (file2, dataFixer) -> {
         try {
            CompoundTag lv = NbtIo.readCompressed(file2);
            CompoundTag lv2 = lv.getCompound("Data");
            lv2.remove("Player");
            int i = lv2.contains("DataVersion", 99) ? lv2.getInt("DataVersion") : -1;
            Dynamic<Tag> dynamic = dataFixer.update(
            ...
            if (j != 19132 && j != 19133) {
               ...
               DataPackSettings lv4 = dynamic.get("DataPacks").result().map(LevelStorage::parseDataPackSettings).orElse(DataPackSettings.SAFE_MODE);
               LevelInfo lv5 = LevelInfo.fromDynamic(dynamic, lv4);
               return new LevelSummary(lv5, lv3, file.getName(), bl2, locked, file3);
            }
   ...
   public class Session implements AutoCloseable {
      ...
      public boolean needsConversion() {
         LevelSummary lv = this.getLevelSummary();
      ...
      public SaveProperties readLevelProperties(DynamicOps<Tag> dynamicOps, DataPackSettings arg) {
         this.checkValid();
         return LevelStorage.this.readLevelProperties(this.directory.toFile(), LevelStorage.createLevelDataParser(dynamicOps, arg));
      }
      ...
      public void backupLevelDataFile(DynamicRegistryManager arg, SaveProperties arg2) {
         this.backupLevelDataFile(arg, arg2, null);
      }

      public void backupLevelDataFile(DynamicRegistryManager arg, SaveProperties arg2, @Nullable CompoundTag arg3) {
      ...
      public void save(String name) throws IOException {
         ...
         if (file.exists()) {
            ...
            if (file2.exists()) {
               ...
               NbtIo.writeCompressed(lv, file2);
            }
         }
      }
