...
public class PointOfInterestType {
   ...
   public static final Predicate<PointOfInterestType> IS_USED_BY_PROFESSION = arg -> VILLAGER_WORKSTATIONS.get().contains(arg);
   ...
   private static final Set<BlockState> BED_STATES = ImmutableList.of(
      ...
      .filter(arg -> arg.get(BedBlock.PART) == BedPart.HEAD)
   ...
   private static PointOfInterestType setup(PointOfInterestType arg) {
      arg.blockStates.forEach(arg2 -> {
         PointOfInterestType lv = BLOCK_STATE_TO_POINT_OF_INTEREST_TYPE.put(arg2, arg);
         if (lv != null) {
            throw (IllegalStateException)Util.throwOrPause(new IllegalStateException(String.format("%s is defined in too many tags", arg2)));
         }
      });
      return arg;
   }
