...
public class SerializingRegionBasedStorage<R> implements AutoCloseable {
   ...
   public SerializingRegionBasedStorage(
      File directory, Function<Runnable, Codec<R>> codecFactory, Function<Runnable, R> factory, DataFixer dataFixer, DataFixTypes arg, boolean bl
   ) {
      ...
      if (data == null) {
         ...
         for (int l = 0; l < 16; l++) {
            ...
            Optional<R> optional = optionalDynamic.get(Integer.toString(l))
               .result()
               .flatMap(dynamicx -> this.codecFactory.apply(() -> this.onUpdate(m)).parse(dynamicx).resultOrPartial(LOGGER::error));
   ...
   private void save(ChunkPos arg) {
      ...
      if (lv instanceof CompoundTag) {
         ...
         LOGGER.error("Expected compound tag, got {}", lv);
      }
   }
   ...
   private <T> Dynamic<T> method_20367(ChunkPos arg, DynamicOps<T> dynamicOps) {
      ...
      for (int i = 0; i < 16; i++) {
         ...
         if (optional != null && optional.isPresent()) {
            ...
            dataResult.resultOrPartial(LOGGER::error).ifPresent(object -> map.put((T)dynamicOps.createString(string), (T)object));
         }
      }
